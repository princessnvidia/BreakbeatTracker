...
"""
BreakbeatAI Tracker Editor v35 — slice index
...
class SliceIndexTracker:
    def __init__(self, root, pair_json):
        ...
        self.root.title("BreakbeatAI Tracker Editor v35 — slice index")
    ...
    def detect_external_player(self):
        for name in ["pw-play", "paplay", "aplay", "ffplay"]:
            ...
            if path:
                print(f"[v35] Backend audio : {name} -> {path}")
                return name

        print("[v35] Aucun backend système trouvé, fallback sounddevice.")
        return None
    ...
    def build_initial_pattern(self, randomize=False):
        ...
        print(
            f"[v35] Génération loop32 | break={self.project['safe']} | "
            f"storage={self.project.get('storage')} | method={self.project.get('slice_method')} | randomize={randomize}"
        )

        for pos in range(32):
            pair = self.pick_pair_for_pos(pos, randomize=randomize, rng=rng)
            ...
            print(f"[v35] pos {pos:02d} -> slice {pair:02d}")
    ...
    def build_ui(self):
        ...
        title = tk.Label(
            main,
            text="BreakbeatAI v35 — slice index, sans WAV par slice",
            bg="#111018",
            fg="#ff7acc",
            font=("Sans", 20, "bold"),
        )
        ...
        self.output_label = tk.Label(
            panel,
            text="v35 : slices index-only. Chaque correction est sauvegardée pour apprendre.",
            bg="#1b1824",
            fg="#77f5b5",
            justify="left",
        )
    ...
    def record_correction(self, event_type, before=None, after=None):
        ...
        event = {
            "version": "breakbeatai_slice_index_correction_v35",
            "time": now_stamp(),
            "safe": safe,
        ...
        corrections_path = CORRECTIONS_DIR / f"{safe}_slice_index_corrections_v35.jsonl"
        ...
        print(f"[v35] correction sauvegardée : {event_type}")
    ...
    def play_audio_array(self, audio, label="audio"):
        ...
        live_wav = OUT_DIR / f"{self.project['safe']}_v35_live.wav"
        ...
        if self.external_player:
            try:
                ...
                print(f"[v35] Erreur backend {self.external_player} : {exc}")
    ...
    def render_preview_file(self):
        OUT_DIR.mkdir(parents=True, exist_ok=True)
        wav = OUT_DIR / f"{self.project['safe']}_tracker_app_v35_preview.wav"
    ...
    def write_latest_pattern(self, reason="correction"):
        ...
        path = LATEST_DIR / f"{self.project['safe']}_latest_slice_index_v35.json"

        data = {
            "version": "breakbeatai_latest_slice_index_v35",
            "reason": reason,
    ...
    def save(self):
        ...
        tracker_path = OUT_DIR / f"{self.project['safe']}_tracker_app_edit_v35.json"

        data = {
            "version": "tracker_app_edit_v35_slice_index",
        ...
        stamp = file_stamp()
        validated_path = VALIDATED_DIR / f"{self.project['safe']}_validated_slice_index_loop32_v35_{stamp}.json"

        validated = {
            "version": "breakbeatai_validated_slice_index_loop32_v35",
