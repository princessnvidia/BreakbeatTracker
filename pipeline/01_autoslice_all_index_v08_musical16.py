...
"""
01_autoslice_all_index_v08_musical16.py
...
Usage :
    python pipeline/01_autoslice_all_index_v08_musical16.py --source "camo"
    python pipeline/01_autoslice_all_index_v08_musical16.py --source "amen"
...
OUT_DIR = Path("dataset/slice_indexes_v08")
...
def pick_all_peaks(audio, threshold=0.12, min_sep_ms=25.0, max_slices=64):
    ...
    for peak in peaks:
        ...
        if max_slices > 0 and len(chosen) >= max_slices:
            break

    chosen = sorted(chosen, key=lambda p: int(p["sample"]))

    # On force 0 comme début si le premier transient est loin.
    if not chosen or chosen[0]["sample"] > int(20 * SR / 1000.0):
        chosen = [{"sample": 0, "score": 1.0}] + chosen
...
def main():
    ...
    parser.add_argument("--min-sep-ms", type=float, default=34.0)
    ...
    preview_wav = out_dir / f"{safe}_source_loop_preview_v08.wav"
    ...
    data = {
        "version": "pair_blocks_v02_musical16_index_only_v08",
        ...
        "slice_method": "musical16_index_only_v08",
        "threshold": args.threshold,
        "min_sep_ms": args.min_sep_ms,
    ...
    report = out_dir / f"{safe}_all_slices_v08_report.txt"
    lines = []
    lines.append(f"Musical16 index-only v08: {safe}")
    ...
    for b in blocks:
        lines.append(
            ...
            f"dur {b['duration_ms']:9.2f} ms | onset {b['onset_score']}"
        )

    report.write_text("\\n".join(lines) + "\\n", encoding="utf-8")

    print("OK autoslice musical16 index-only v08")
