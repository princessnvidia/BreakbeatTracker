...
"""
BreakbeatAI Tracker Editor v92 — slice index
...
def warp_audio_to_length_external(audio, target_len, cache_key=None):
    ...
    if rubberband:
        try:
            ...
            print(f"[v92] rubberband échec, fallback ffmpeg/no-speed : {exc}")
    ...
    if ffmpeg:
        try:
            ...
            print(f"[v92] ffmpeg atempo échec, fallback no-speed : {exc}")
...
class SliceIndexTracker:
    def __init__(self, root, pair_json):
        ...
        self.root.title("BreakbeatAI Tracker Editor v92 — slice index")
    ...
    def detect_external_player(self):
        for name in ["pw-play", "paplay", "aplay", "ffplay"]:
            ...
            if path:
                print(f"[v92] Backend audio : {name} -> {path}")
                return name

        print("[v92] Aucun backend système trouvé, fallback sounddevice.")
        return None
    ...
    def build_initial_pattern(self, randomize=False):
        ...
        print(
            f"[v92] Génération pattern | break={self.project['safe']} | "
        ...
        for slot in range(max_items):
            ...
            print(f"[v92] slot {slot:02d} | step {x_step:02d} -> slice {pair:02d}")
    ...
    def record_correction(self, event_type, before=None, after=None):
        ...
        print(f"[v92] correction sauvegardée : {event_type}")
    ...
    def play_audition_array(self, audio, label="audition"):
        ...
        audition_wav = OUT_DIR / f"{self.project['safe']}_v92_audition.wav"
        ...
        if self.external_player:
            try:
                ...
                print(f"[v92] Erreur audition {self.external_player} : {exc}")
    ...
    def on_close(self):
        ...
        """
        print("[v92] Fermeture : arrêt audio + destruction fenêtre")
    ...
    def play_audio_array(self, audio, label="audio"):
        ...
        audio = normalize(audio)
        live_wav = OUT_DIR / f"{self.project['safe']}_v92_live.wav"
        ...
        if self.external_player:
            try:
                ...
                print(f"[v92] Erreur backend {self.external_player} : {exc}")
    ...
    def render_preview_file(self):
        OUT_DIR.mkdir(parents=True, exist_ok=True)
        wav = OUT_DIR / f"{self.project['safe']}_tracker_app_v92_preview.wav"
    ...
    def write_latest_pattern(self, reason="correction"):
        ...
        path = LATEST_DIR / f"{self.project['safe']}_latest_slice_index_v92.json"

        data = {
            "version": "breakbeatai_latest_slice_index_v92",
            "reason": reason,
    ...
    def save(self):
        ...
        tracker_path = OUT_DIR / f"{self.project['safe']}_tracker_app_edit_v92.json"
        ...
        stamp = file_stamp()
        validated_path = VALIDATED_DIR / f"{self.project['safe']}_validated_slice_index_loop32_v92_{stamp}.json"

        validated = {
            "version": "breakbeatai_validated_slice_index_loop32_v92",
...
def v44_record_correction(self, event_type, before=None, after=None):
    ...
    print(f"[v92] correction sauvegardée : {event_type}")
    print(f"[v92] correction path : {corrections_path}")
    print(f"[v92] latest path : {latest_path}")
...
def v45_move_selected(self, dx, dy):
    ...
    if dy != 0 and before != after:
        try:
            ...
            print(f"[v92] audition après flèche haut/bas impossible : {exc}")
...
def v46_warp_loop_pitch_preserve(audio, target_len, cache_key):
    ...
    if rubberband:
        try:
            ...
            print(f"[v92] rubberband global warp échec : {exc}")
    ...
    if ffmpeg:
        try:
            ...
            print(f"[v92] ffmpeg global warp échec : {exc}")

    print("[v92] Aucun warp pitch-preserve dispo, fallback fit no-speed.")
    return v46_fit_audio_no_speed(audio, target_len)
...
def v46_build_initial_pattern(self, randomize=False):
    ...
    print(
        f"[v92] M8 pattern | break={self.project['safe']} | "
...
def v46_get_warped_loop(self):
    ...
    print(
        f"[v92] global warp OK | source_len={loop_end-loop_start} | "
...
def v46_build_ui(self):
    ...
    try:
        ...
        print(f"[v92] UI extension impossible : {exc}")
...
def v47_play_audio_array(self, audio, label="audio"):
    ...
    if len(audio) == 0 or v47_is_silent(audio):
        ...
        print(f"[v92] lecture annulée : audio silencieux ({label})")
        return 0
...
def v47_toggle_loop(self):
    ...
    if not getattr(self, "pattern", None):
        ...
        print("[v92] loop annulée : pattern vide")
        return "break"
...
def v47_delete_selected(self):
    ...
    if not getattr(self, "pattern", None):
        ...
        print("[v92] pattern vide après suppression : audio coupé")
...
def v48_build_initial_pattern(self, randomize=False):
    ...
    print("[v92] pattern initial forcé VIDE : aucun son ne doit jouer.")
    return []
...
def v48_load_first_slices(self):
    ...
    print(f"[v92] Load 16 slices : {len(self.pattern)} items")


def v48_debug_pattern(self, label="render"):
    print("")
    print(f"[v92] DEBUG PATTERN avant {label}")
    print(f"[v92] items = {len(getattr(self, 'pattern', []) or [])}")

    if not getattr(self, "pattern", None):
        print("[v92] pattern vide confirmé")
        print("")
        return
    ...
    for item in self.pattern:
        ...
        print(
            f"[v92] id={item_id} | step {x:02d}/{end:02d} | "
...
def v48_render_audio_with_timeline(self):
    ...
    if not getattr(self, "pattern", None):
        loop_samples = self.get_loop_samples()
        print(f"[v92] rendu silence total : pattern vide | samples={loop_samples}")
        return np.zeros(loop_samples, dtype=np.float32), []
    ...
    print(f"[v92] rendu audio peak={peak:.8f} | timeline_events={len(timeline) if timeline is not None else 'None'}")

    if timeline:
        print("[v92] timeline events:")
        for ev in timeline:
            print(f"[v92]   {ev}")
...
def v48_toggle_loop(self):
    if not getattr(self, "pattern", None):
        ...
        print("[v92] Space ignoré : pattern vide.")
        return "break"
...
def v48_build_ui(self):
    ...
    try:
        ...
        print(f"[v92] ajout UI impossible : {exc}")
...
def v49_add_item_at_xy(self, x, y):
    ...
    try:
        ...
        print(f"[v92] record_correction add impossible : {exc}")
    ...
    try:
        ...
        print(f"[v92] audition après ajout impossible : {exc}")

    print(f"[v92] add | id={item['id']} | step={x_step} | pair={pair} | lane={lane}")
...
def v49_duplicate_selected_forward(self, event=None):
    ...
    if item is None:
        ...
        print("[v92] duplicate annulé : aucune note sélectionnée")
        return "break"
    ...
    if new_x > max_x:
        ...
        print("[v92] duplicate annulé : fin de grille")
        return "break"
    ...
    try:
        ...
        print(f"[v92] record_correction duplicate impossible : {exc}")
    ...
    try:
        ...
        print(f"[v92] audition après duplicate impossible : {exc}")

    print(
        f"[v92] duplicate | old_id={item.get('id')} | new_id={new_item.get('id')} | "
...
def v49_build_ui(self):
    ...
    try:
        ...
        print(f"[v92] bind canvas click impossible : {exc}")
    ...
    try:
        ...
        print(f"[v92] bind keys impossible : {exc}")
...
def v50_load_style_model():
    ...
    try:
        ...
        print(f"[v92] impossible de charger le modèle IA : {exc}")
        return None
...
def v50_generate_ai_pattern(self, event=None):
    ...
    if model is None:
        ...
        print("[v92] modèle absent :", STYLE_MODEL_PATH)
        return "break"
    ...
    print("")
    print(f"[v92] GENERATE IA | safe={safe} | pairs={pair_values} | slots={max_slots}")
    ...
    for slot in range(max_slots):
        ...
        print(
            f"[v92] slot {slot:02d} | step {x_step:02d} | "
    ...
    try:
        ...
        print(f"[v92] write_latest_pattern impossible : {exc}")
...
def v50_build_ui(self):
    ...
    try:
        ...
        print(f"[v92] UI IA impossible : {exc}")
...
def v51_mouse_down(self, event):
    ...
    if item is not None:
        ...
        print(
            f"[v92] mouse down note | id={item.get('id')} | "
            f"step={item.get('x_step')} | pair={item.get('pair')}"
        )
    else:
        print("[v92] mouse down empty")
...
def v51_mouse_up(self, event):
    ...
    if state.get("empty"):
        ...
        print("[v92] drag depuis vide ignoré")
        return "break"
    ...
    if before != after:
        try:
            ...
            print(f"[v92] record_correction drag impossible : {exc}")
        ...
        print(
            f"[v92] drag move | id={after.get('id')} | "
            f"step {before.get('x_step')} -> {after.get('x_step')} | "
        ...
        try:
            ...
            print(f"[v92] audition après drag impossible : {exc}")
...
def v51_build_ui(self):
    ...
    try:
        ...
        print(f"[v92] bind souris impossible : {exc}")
    ...
    try:
        ...
        print(f"[v92] bind Suppr impossible : {exc}")
...
def v52_normalize_loaded_notes(self, notes):
    ...
    for raw in sorted(notes, key=lambda n: (int(float(n.get("x_step", 0))), int(float(n.get("pair", 0))))):
        ...
        if pair not in valid_pairs:
            print(f"[v92] slice ignorée car absente du break actuel : pair={pair}")
            continue
...
def v52_load_saved_beat(self, event=None):
    ...
    if not saves:
        ...
        print("[v92] aucune sauvegarde trouvée")
        return "break"
    ...
    if not pattern:
        ...
        print("[v92] sauvegarde incompatible :", chosen["path"])
        return "break"
    ...
    print("")
    print("[v92] BEAT RESTAURÉ")
    print("[v92] fichier :", chosen["path"])
    print("[v92] clé :", chosen["key"])
    print("[v92] notes :", len(pattern))
    print("")
...
def v52_list_saved_beats(self, event=None):
    ...
    print("")
    print("[v92] SAUVEGARDES TROUVÉES")
    for i, save in enumerate(saves[:30]):
        print(
            f"[v92] {i:02d} | notes={len(save['notes']):02d} | "
...
def v52_build_ui(self):
    ...
    try:
        ...
        print(f"[v92] UI load saved impossible : {exc}")
...
def v53_run_training(self):
    ...
    if not trainer.exists():
        ...
        print("[v92] trainer introuvable :", trainer)
        return False

    print("")
    print("[v92] AUTO-TRAIN IA après Save...")
    print("")
...
def v53_save(self):
    ...
    try:
        ...
        print(f"[v92] auto-train après save impossible : {exc}")
...
def v53_build_ui(self):
    ...
    try:
        ...
        print(f"[v92] UI auto-train impossible : {exc}")
...
def v55_mouse_up(self, event):
    ...
    if not state.get("moved"):
        ...
        try:
            ...
            print(f"[v92] audition clic note impossible : {exc}")
        ...
        print(
            f"[v92] click play/select | id={item.get('id')} | "
            f"step={item.get('x_step')} | pair={item.get('pair')}"
        )
...
def v58_add_item_at_xy(self, x, y):
    ...
    print(
        f"[v92] add | id={item['id']} | step={x_step} | "
...
def v58_toggle_selected_length(self, event=None):
    ...
    try:
        ...
        print(f"[v92] record_correction length impossible : {exc}")

    v58_refresh_after_edit(self, "toggle_note_length")

    v58_status(self, f"Longueur note : {old_len} -> {new_len} case(s).")
    print(f"[v92] length toggle | id={item.get('id')} | {old_len} -> {new_len}")
...
def v58_duplicate_selected_forward(self, event=None):
    ...
    try:
        ...
        print(f"[v92] audition duplicate impossible : {exc}")
...
def v58_build_ui(self):
    ...
    try:
        ...
        print(f"[v92] UI demi-grille impossible : {exc}")
    ...
    try:
        ...
        print(f"[v92] bind demi-grille impossible : {exc}")
...
def v59_rank_current_slices(self):
    ...
    for pair in self.pair_values:
        ...
        try:
            ...
            print(f"[v92] analyse pair {pair} impossible : {exc}")
            role = "other"
    ...
    print("")
    print("[v92] TOP ROLES CURRENT BREAK")
    for r in ("kick", "snare", "hat"):
        print(f"[v92] {r:5s}:", [(x["pair"], round(x["score"], 3)) for x in ranks[r][:6]])
    print("")
...
def v59_load_role_model():
    ...
    try:
        ...
        print(f"[v92] lecture modèle rôle impossible : {exc}")
        return None
...
def v59_generate_role_aware(self, event=None):
    ...
    if model is None:
        ...
        print("[v92] modèle rôle absent, fallback musical snare 6.")
        model = {
            "breaks": {},
            "global": {},
            "patterns_used": 0,
    ...
    print("")
    print("[v92] ROLE-AWARE GENERATION")
    print("[v92] model patterns_used:", model.get("patterns_used", 0))
    for item in pattern:
        print(
            f"[v92] step {item['x_step']:02d}/{item['x_step'] + item['length'] - 1:02d} | "
...
def v59_run_role_training(self, event=None):
    ...
    try:
        ...
        print(f"[v92] training rôle impossible : {exc}")
...
def v59_save(self):
    ...
    try:
        ...
        print(f"[v92] auto-training rôle après save impossible : {exc}")
...
def v59_build_ui(self):
    ...
    try:
        ...
        print(f"[v92] UI role-aware impossible : {exc}")
...
def v62_add_item_at_xy(self, x, y):
    ...
    print(f"[v92] add | id={item['id']} | step={x_step} | len={length} | pair={pair}")
...
def v62_mouse_down(self, event):
    ...
    if item is not None:
        ...
        print(
            f"[v92] mouse down {state['mode']} | id={item.get('id')} | "
            f"step={item.get('x_step')} | len={item.get('length')} | pair={item.get('pair')}"
        )
    else:
        print("[v92] mouse down empty")
    ...
    if mode == "resize":
        if not state.get("moved"):
            ...
            try:
                ...
                print(f"[v92] audition clic resize impossible : {exc}")
        ...
        if before != after:
            try:
                ...
                print(f"[v92] record_correction resize impossible : {exc}")
            ...
            try:
                ...
                print(f"[v92] audition après resize impossible : {exc}")
    ...
    if mode == "move":
        ...
        if before != after:
            try:
                ...
                print(f"[v92] record_correction move impossible : {exc}")
            ...
            try:
                ...
                print(f"[v92] audition après move impossible : {exc}")
...
def v62_toggle_selected_length(self, event=None):
    ...
    try:
        ...
        print(f"[v92] record_correction toggle length impossible : {exc}")

    v62_refresh_after_edit(self, "toggle_note_length_v62")
    v62_status(self, f"Longueur note : {old_len} → {new_len} case(s).")

    print(f"[v92] toggle length | id={item.get('id')} | {old_len} -> {new_len}")
...
def v62_draw(self):
    ...
    try:
        ...
        print(f"[v92] draw handles impossible : {exc}")
...
def v62_build_ui(self):
    ...
    try:
        ...
        print(f"[v92] bind resize impossible : {exc}")
    ...
    try:
        ...
        print(f"[v92] UI resize impossible : {exc}")
...
def v63_load_model():
    ...
    try:
        ...
        print(f"[v92] lecture modèle strict impossible : {exc}")
        return None
...
def v63_generate_safe_role_ai(self, event=None):
    ...
    print("")
    print("[v92] SAFE ROLE AI GENERATED")
    print("[v92] model strict:", "yes" if model else "no")
    print("[v92] snare main/alt:", main_snare, alt_snare)
    print("[v92] kick  main/alt:", main_kick, alt_kick)
    print("[v92] hat   main/alt:", main_hat, alt_hat)
    print("[v92] pattern:")
    for item in pattern:
        print(
            f"[v92] step {item['x_step']:02d}/{item['x_step'] + item['length'] - 1:02d} | "
...
def v63_run_strict_training(self, event=None):
    ...
    print("")
    print("[v92] TRAIN STRICT ROLE MODEL...")
    print("")
    ...
    try:
        ...
        print(f"[v92] training strict impossible : {exc}")
...
def v63_build_ui(self):
    ...
    try:
        ...
        print(f"[v92] UI safe impossible : {exc}")
...
def v65_rank_slices(self):
    ...
    for pair in self.pair_values:
        ...
        try:
            ...
            print(f"[v92] analyse slice {pair} impossible : {exc}")
            scores = {
                "kick": 0.0,
                "snare": 0.0,
                "hat": 0.0,
    ...
    print("")
    print("[v92] TOP SLICES PAR ROLE — break courant")
    for role in ("kick", "snare", "hat", "ghost_snare"):
        print(f"[v92] {role:12s}:", [(x["pair"], round(x["score"], 3)) for x in ranks[role][:8]])
    print("")
...
def v65_generate_from_role_template(self, event=None):
    ...
    try:
        ...
        print(f"[v92] template introuvable : {exc}")
        return "break"
    ...
    print("")
    print("[v92] CROSS-SONG ROLE TEMPLATE GENERATED")
    print("[v92] template:", V65_TEMPLATE_PATH)
    print("[v92] temperature:", temperature, "| hats:", hat_density, "| ghosts:", ghost_density)
    print("[v92] pattern:")
    for item in pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v92] step {item['x_step']:02d}/{end:02d} | "
...
def v67_train_strict_if_available(self):
    ...
    if trainer is None:
        print("[v92] aucun trainer trouvé.")
        return False

    print("")
    print("[v92] TRAIN après validation :", trainer)
    print("")
    ...
    try:
        ...
        print(f"[v92] training impossible : {exc}")
        return False
...
def v67_generate_candidate(self, event=None):
    ...
    print("")
    print("[v92] CANDIDATE GENERATED")
    print("[v92] break:", self.v67_candidate_safe)
    print("[v92] notes:", len(candidate))
    print("[v92] Maintenant : corrige puis clique Good après modifs, ou Reject.")
    print("")
...
def v67_accept_corrected(self, event=None):
    ...
    print("")
    print("[v92] ACCEPTED GOOD AFTER EDIT")
    print("[v92] saved:", validated_path)
    print("[v92] trained:", trained)
    print("")
...
def v67_accept_without_training(self, event=None):
    ...
    print("[v92] accepted no-train:", validated_path)
...
def v67_build_ui(self):
    ...
    try:
        ...
        print(f"[v92] UI feedback impossible : {exc}")
...
def v68_generate_candidate(self, event=None):
    ...
    print("[v92] Length policy appliquée : 2 cases par défaut, 1 case rare.")
...
def v69_force_pattern_hard_2_grid(self, reason="v69_force_hard_2_grid"):
    ...
    print("")
    print("[v92] HARD 2 GRID APPLIQUÉ")
    for item in self.pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v92] step {item['x_step']:02d}/{end:02d} | "
...
def v70_duplicate_selected_forward(self, event=None):
    ...
    print("")
    print("[v92] CTRL+D DUPLICATE +2")
    print(f"[v92] old_id={item.get('id')} new_id={new_item.get('id')}")
    print(f"[v92] step {old_x}/{old_x + length - 1} -> {new_x}/{new_x + length - 1}")
    print(f"[v92] pair={new_item.get('pair')} len={length}")
    print("")
...
def v70_build_ui(self):
    ...
    try:
        ...
        print(f"[v92] bind Ctrl+D impossible : {exc}")
...
def v71_rank_slices(self):
    ...
    print("")
    print("[v92] PRE-LEARNING TOP SLICES")
    for role in ("kick", "snare", "hat", "ghost_snare"):
        print(f"[v92] {role:12s}:", [(x["pair"], round(x["score"], 3)) for x in ranks[role][:8]])
    print("")
...
def v71_prelearn_roles(self, force=False):
    ...
    print("")
    print("[v92] PRELEARN ROLES POUR", safe)
    for role in ("kick", "snare", "hat", "ghost_snare", "bad"):
        print(f"[v92] {role:12s}:", roles.get(role, []))
    print("")
...
def v71_generate_full_candidate(self, event=None):
    ...
    print("")
    print("[v92] FULL CANDIDATE GENERATED")
    print("[v92] break:", v71_safe(self))
    print("[v92] roles:", roles)
    print("[v92] pattern:")
    for item in pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
...
def v72_open_break(self, safe=None):
    ...
    print("")
    print("[v92] OPEN BREAK")
    print("[v92] current:", current)
    print("[v92] next   :", safe)
    print("[v92] cmd    :", " ".join(cmd))
    print("")
...
def v72_build_ui(self):
    ...
    try:
        ...
        print(f"[v92] UI break switcher impossible : {exc}")
...
def v73_sanitize_tracker_pattern(self, reason="v73_tracker_strict_no_overlap"):
    ...
    print("")
    print("[v92] TRACKER STRICT NO OVERLAP")
    print("[v92] kept:", len(accepted), "| rejected overlaps:", len(rejected))

    for item in accepted:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v92] KEEP step {item['x_step']:02d}/{end:02d} | "
    ...
    if rejected:
        ...
        for item in rejected:
            ...
            print(
                f"[v92] DROP step {item['x_step']:02d}/{end:02d} | "
                f"role={item.get('v73_role_key')} | pair={item.get('pair')}"
            )
...
def v81_launch_break(self, source):
    ...
    print("")
    print("[v92] CHARGER BREAK")
    print("[v92] courant :", current)
    print("[v92] nouveau :", source)
    print("[v92] cmd     :", " ".join(cmd))
    print("")
...
def v81_patch_charger_break_buttons(self):
    ...
    print(f"[v92] boutons Charger break patchés : {patched}")
...
def v81_generate_candidate(self, event=None):
    ...
    print("")
    print("[v92] GENERATE CANDIDATE")
    print("[v92] break chargé :", current)
    print("[v92] générateur   :", getattr(fn, "__name__", None))
    print("")
...
def v81_run_training(self):
    ...
    if script is None:
        print("[v92] Aucun script training trouvé.")
...
def v81_save_feedback(self, verdict, train_now=False):
    ...
    print("")
    print("[v92] FEEDBACK SAUVÉ")
    print("[v92] break  :", current)
    print("[v92] verdict:", verdict)
    print("[v92] train  :", train_now)
    print("[v92] fichier:", out_path)
    print("")
...
def v81_build_ui(self):
    ...
    try:
        self.root.title(f"BreakbeatAI v92 — clean UI")
    except Exception:
        pass
    ...
    try:
        ...
        print(f"[v92] patch Charger break impossible : {exc}")

    try:
        v81_install_training_toolbar(self)
    except Exception as exc:
        print(f"[v92] toolbar impossible : {exc}")
...
def v82_auto_role_pools(self):
    ...
    print("")
    print("[v92] ROLE POOLS")
    for r in ["kick", "snare", "hat", "ghost_snare", "bad"]:
        print(f"[v92] {r:12s}:", roles.get(r, []))
    print("")
...
def v82_generate_candidate(self, event=None):
    ...
    print("")
    print("[v92] NEW CANDIDATE")
    print("[v92] break :", current)
    print("[v92] nonce :", nonce)
    print("[v92] rejected known:", len(rejected))
    print("[v92] fingerprint rejeté ?: ", chosen_fp in rejected)
    for item in self.pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v92] step {item['x_step']:02d}/{end:02d} "
...
def v82_patch_training_buttons(self):
    ...
    print(f"[v92] boutons training patchés : {patched}")
...
def v82_build_ui(self):
    ...
    try:
        ...
        print(f"[v92] patch boutons impossible : {exc}")
...
# ---------------------------------------------------------------------
# v92 EXTENSION : UI propre + grille correcte
# ---------------------------------------------------------------------
# Pas de panneaux flottants v83/v84/v85/v86/v87.
# Pas de boutons qui clignotent.
#
# Grille correcte :
# kick  = 00/01, 10/11, 16/17, 26/27
# snare = 04/05, 12/13, 20/21, 28/29
# hat   = 02/03, 06/07, 08/09, 14/15, 18/19, 22/23, 24/25, 30/31
# ---------------------------------------------------------------------

V92_OVERRIDES_PATH = Path("dataset/learning/break_role_overrides_v01.json")
V92_LOCKS_PATH = Path("dataset/learning/main_role_locks_v87.json")
...
V92_KICK_STEPS = [0, 10, 16, 26]
V92_SNARE_STEPS = [4, 12, 20, 28]
V92_HAT_STEPS = [2, 6, 8, 14, 18, 22, 24, 30]


def v92_status(self, text):
    try:
        if hasattr(self, "set_status"):
            self.set_status(text)
        else:
...
def v92_load_json(path, default):
    try:
...
def v92_find_record(data, current):
    br = data.get("breaks", {}).get(current)
    if br is not None:
        return br

    for name, obj in data.get("breaks", {}).items():
        if v92_norm(name) == v92_norm(current):
            return obj

    return None


def v92_all_pairs(self):
    try:
        pairs = [int(x) for x in self.pair_values]
    except Exception:
        pairs = [0]
...
def v92_load_roles(self):
    current = v92_current_break(self)
    ...
    data = v92_load_json(V92_OVERRIDES_PATH, {"breaks": {}})
    br = v92_find_record(data, current)
...
def v92_load_locks(self):
    current = v92_current_break(self)
    data = v92_load_json(V92_LOCKS_PATH, {"breaks": {}})
    br = v92_find_record(data, current)
...
def v92_choose_pairs(self, roles, locks):
    chosen = {}
    used = set()
    ...
    for role in ["snare", "kick", "hat"]:
        ...
        for p in v92_all_pairs(self):
            candidates.append((int(p), "fallback_distinct"))
...
def v92_make_note(self, step, pair, role, source):
    step = int(step)
    ...
    return {
        ...
        "hit_slot": int(step) // 2,
        "ai_generated": True,
        "ai_model": "v92_clean_ui",
        "learned_role": str(role),
        "prior_label": str(role),
        "main_role_source": str(source),
        "v92_clean_ui": True,
        "v92_correct_grid": True,
    }


def v92_validate_pattern(pattern):
    for item in pattern:
        x = int(item["x_step"])
        role = str(item["learned_role"])

        if role == "kick" and x not in V92_KICK_STEPS:
            raise RuntimeError(f"BUG v92 : kick à {x}, interdit")

        if role == "snare" and x not in V92_SNARE_STEPS:
            raise RuntimeError(f"BUG v92 : snare à {x}, interdit")

        if role == "hat" and x not in V92_HAT_STEPS:
            raise RuntimeError(f"BUG v92 : hat à {x}, interdit")


def v92_generate(self, event=None):
    current = v92_current_break(self)
    roles = v92_load_roles(self)
    locks = v92_load_locks(self)
    chosen = v92_choose_pairs(self, roles, locks)

    print("")
    print("[v92] GENERATE CLEAN UI")
    print("[v92] break:", current)
    print("[v92] grille : kick hat snare hat / hat kick snare hat")
    print("[v92] locks :", locks)
    for role in ["kick", "snare", "hat"]:
        print(
            f"[v92] {role:6s}: pair={chosen[role]['pair']} "
    ...
    for step, role in V92_GRID:
        pair = chosen[role]["pair"]
        source = chosen[role]["source"]
        pattern.append(v92_make_note(self, step, pair, role, source))

    v92_validate_pattern(pattern)
    ...
    try:
        ...
        print(f"[v92] draw impossible : {exc}")

    try:
        self.refresh_panel()
    except Exception as exc:
        print(f"[v92] refresh_panel impossible : {exc}")

    try:
        self.write_latest_pattern(reason="v92_clean_ui")
    except Exception as exc:
        print(f"[v92] write_latest_pattern impossible : {exc}")
    ...
    print("")
    print("[v92] GENERATED PATTERN")
    for item in self.pattern:
        x = int(item["x_step"])
        end = x + int(item["length"]) - 1
        print(
            f"[v92] step {x:02d}/{end:02d} "
    ...
    v92_status(self, "v92 clean : grille correcte, sans panneaux flottants.")
    return "break"


def v92_all_widgets(widget):
    out = [widget]
    ...
    for child in children:
        out.extend(v92_all_widgets(child))

    return out


def v92_patch_generate_buttons(self):
    patched = 0

    for widget in v92_all_widgets(self.root):
        try:
            text = str(widget.cget("text")).strip().lower()
        except Exception:
            continue
...
_old_v92_build_ui = SliceIndexTracker.build_ui


def v92_build_ui(self):
    _old_v92_build_ui(self)

    try:
        self.root.title(f"BreakbeatAI v92 clean UI — {v92_current_break(self)}")
    except Exception:
        pass

    try:
        v92_patch_generate_buttons(self)
    except Exception as exc:
        print(f"[v92] patch boutons impossible : {exc}")
    ...
    v92_status(self, "v92 clean UI chargé : utilise Generate Candidate ou F12.")


SliceIndexTracker.generate_candidate = v92_generate
SliceIndexTracker.generate_full_candidate = v92_generate
SliceIndexTracker.generate_ai_pattern = v92_generate
SliceIndexTracker.generate_cross_song_template = v92_generate
SliceIndexTracker.generate_safe_experiment = v92_generate
SliceIndexTracker.generate_locked_roles = v92_generate
SliceIndexTracker.generate_role_aware = v92_generate
SliceIndexTracker.generate_safe_role_ai = v92_generate

SliceIndexTracker.build_ui = v92_build_ui
