...
"""
BreakbeatAI Tracker Editor v79 — slice index
...
def warp_audio_to_length_external(audio, target_len, cache_key=None):
    ...
    if rubberband:
        try:
            ...
            print(f"[v79] rubberband échec, fallback ffmpeg/no-speed : {exc}")
    ...
    if ffmpeg:
        try:
            ...
            print(f"[v79] ffmpeg atempo échec, fallback no-speed : {exc}")
...
class SliceIndexTracker:
    def __init__(self, root, pair_json):
        ...
        self.root.title("BreakbeatAI Tracker Editor v79 — slice index")
    ...
    def detect_external_player(self):
        for name in ["pw-play", "paplay", "aplay", "ffplay"]:
            ...
            if path:
                print(f"[v79] Backend audio : {name} -> {path}")
                return name

        print("[v79] Aucun backend système trouvé, fallback sounddevice.")
        return None
    ...
    def build_initial_pattern(self, randomize=False):
        ...
        print(
            f"[v79] Génération pattern | break={self.project['safe']} | "
        ...
        for slot in range(max_items):
            ...
            print(f"[v79] slot {slot:02d} | step {x_step:02d} -> slice {pair:02d}")
    ...
    def build_ui(self):
        ...
        title = tk.Label(
            main,
            text="BreakbeatAI v79 — selected generate topbar",
            bg="#111018",
            fg="#ff7acc",
            font=("Sans", 20, "bold"),
        )
    ...
    def record_correction(self, event_type, before=None, after=None):
        ...
        print(f"[v79] correction sauvegardée : {event_type}")
    ...
    def play_audition_array(self, audio, label="audition"):
        ...
        audition_wav = OUT_DIR / f"{self.project['safe']}_v79_audition.wav"
        ...
        if self.external_player:
            try:
                ...
                print(f"[v79] Erreur audition {self.external_player} : {exc}")
    ...
    def on_close(self):
        ...
        """
        print("[v79] Fermeture : arrêt audio + destruction fenêtre")
    ...
    def play_audio_array(self, audio, label="audio"):
        ...
        audio = normalize(audio)
        live_wav = OUT_DIR / f"{self.project['safe']}_v79_live.wav"
        ...
        if self.external_player:
            try:
                ...
                print(f"[v79] Erreur backend {self.external_player} : {exc}")
    ...
    def render_preview_file(self):
        OUT_DIR.mkdir(parents=True, exist_ok=True)
        wav = OUT_DIR / f"{self.project['safe']}_tracker_app_v79_preview.wav"
    ...
    def write_latest_pattern(self, reason="correction"):
        ...
        path = LATEST_DIR / f"{self.project['safe']}_latest_slice_index_v79.json"

        data = {
            "version": "breakbeatai_latest_slice_index_v79",
            "reason": reason,
    ...
    def save(self):
        ...
        tracker_path = OUT_DIR / f"{self.project['safe']}_tracker_app_edit_v79.json"
        ...
        stamp = file_stamp()
        validated_path = VALIDATED_DIR / f"{self.project['safe']}_validated_slice_index_loop32_v79_{stamp}.json"

        validated = {
            "version": "breakbeatai_validated_slice_index_loop32_v79",
...
def v44_record_correction(self, event_type, before=None, after=None):
    ...
    print(f"[v79] correction sauvegardée : {event_type}")
    print(f"[v79] correction path : {corrections_path}")
    print(f"[v79] latest path : {latest_path}")
...
def v45_move_selected(self, dx, dy):
    ...
    if dy != 0 and before != after:
        try:
            ...
            print(f"[v79] audition après flèche haut/bas impossible : {exc}")
...
def v46_warp_loop_pitch_preserve(audio, target_len, cache_key):
    ...
    if rubberband:
        try:
            ...
            print(f"[v79] rubberband global warp échec : {exc}")
    ...
    if ffmpeg:
        try:
            ...
            print(f"[v79] ffmpeg global warp échec : {exc}")

    print("[v79] Aucun warp pitch-preserve dispo, fallback fit no-speed.")
    return v46_fit_audio_no_speed(audio, target_len)
...
def v46_build_initial_pattern(self, randomize=False):
    ...
    print(
        f"[v79] M8 pattern | break={self.project['safe']} | "
...
def v46_get_warped_loop(self):
    ...
    print(
        f"[v79] global warp OK | source_len={loop_end-loop_start} | "
...
def v46_build_ui(self):
    ...
    try:
        ...
        print(f"[v79] UI extension impossible : {exc}")
...
def v47_play_audio_array(self, audio, label="audio"):
    ...
    if len(audio) == 0 or v47_is_silent(audio):
        ...
        print(f"[v79] lecture annulée : audio silencieux ({label})")
        return 0
...
def v47_toggle_loop(self):
    ...
    if not getattr(self, "pattern", None):
        ...
        print("[v79] loop annulée : pattern vide")
        return "break"
...
def v47_delete_selected(self):
    ...
    if not getattr(self, "pattern", None):
        ...
        print("[v79] pattern vide après suppression : audio coupé")
...
def v48_build_initial_pattern(self, randomize=False):
    ...
    print("[v79] pattern initial forcé VIDE : aucun son ne doit jouer.")
    return []
...
def v48_load_first_slices(self):
    ...
    print(f"[v79] Load 16 slices : {len(self.pattern)} items")


def v48_debug_pattern(self, label="render"):
    print("")
    print(f"[v79] DEBUG PATTERN avant {label}")
    print(f"[v79] items = {len(getattr(self, 'pattern', []) or [])}")

    if not getattr(self, "pattern", None):
        print("[v79] pattern vide confirmé")
        print("")
        return
    ...
    for item in self.pattern:
        ...
        print(
            f"[v79] id={item_id} | step {x:02d}/{end:02d} | "
...
def v48_render_audio_with_timeline(self):
    ...
    if not getattr(self, "pattern", None):
        loop_samples = self.get_loop_samples()
        print(f"[v79] rendu silence total : pattern vide | samples={loop_samples}")
        return np.zeros(loop_samples, dtype=np.float32), []
    ...
    print(f"[v79] rendu audio peak={peak:.8f} | timeline_events={len(timeline) if timeline is not None else 'None'}")

    if timeline:
        print("[v79] timeline events:")
        for ev in timeline:
            print(f"[v79]   {ev}")
...
def v48_toggle_loop(self):
    if not getattr(self, "pattern", None):
        ...
        print("[v79] Space ignoré : pattern vide.")
        return "break"
...
def v48_build_ui(self):
    ...
    try:
        ...
        print(f"[v79] ajout UI impossible : {exc}")
...
def v49_add_item_at_xy(self, x, y):
    ...
    try:
        ...
        print(f"[v79] record_correction add impossible : {exc}")
    ...
    try:
        ...
        print(f"[v79] audition après ajout impossible : {exc}")

    print(f"[v79] add | id={item['id']} | step={x_step} | pair={pair} | lane={lane}")
...
def v49_duplicate_selected_forward(self, event=None):
    ...
    if item is None:
        ...
        print("[v79] duplicate annulé : aucune note sélectionnée")
        return "break"
    ...
    if new_x > max_x:
        ...
        print("[v79] duplicate annulé : fin de grille")
        return "break"
    ...
    try:
        ...
        print(f"[v79] record_correction duplicate impossible : {exc}")
    ...
    try:
        ...
        print(f"[v79] audition après duplicate impossible : {exc}")

    print(
        f"[v79] duplicate | old_id={item.get('id')} | new_id={new_item.get('id')} | "
...
def v49_build_ui(self):
    ...
    try:
        ...
        print(f"[v79] bind canvas click impossible : {exc}")
    ...
    try:
        ...
        print(f"[v79] bind keys impossible : {exc}")
...
def v50_load_style_model():
    ...
    try:
        ...
        print(f"[v79] impossible de charger le modèle IA : {exc}")
        return None
...
def v50_generate_ai_pattern(self, event=None):
    ...
    if model is None:
        ...
        print("[v79] modèle absent :", STYLE_MODEL_PATH)
        return "break"
    ...
    print("")
    print(f"[v79] GENERATE IA | safe={safe} | pairs={pair_values} | slots={max_slots}")
    ...
    for slot in range(max_slots):
        ...
        print(
            f"[v79] slot {slot:02d} | step {x_step:02d} | "
    ...
    try:
        ...
        print(f"[v79] write_latest_pattern impossible : {exc}")
...
def v50_build_ui(self):
    ...
    try:
        ...
        print(f"[v79] UI IA impossible : {exc}")
...
def v51_mouse_down(self, event):
    ...
    if item is not None:
        ...
        print(
            f"[v79] mouse down note | id={item.get('id')} | "
            f"step={item.get('x_step')} | pair={item.get('pair')}"
        )
    else:
        print("[v79] mouse down empty")
...
def v51_mouse_up(self, event):
    ...
    if state.get("empty"):
        ...
        print("[v79] drag depuis vide ignoré")
        return "break"
    ...
    if before != after:
        try:
            ...
            print(f"[v79] record_correction drag impossible : {exc}")
        ...
        print(
            f"[v79] drag move | id={after.get('id')} | "
            f"step {before.get('x_step')} -> {after.get('x_step')} | "
        ...
        try:
            ...
            print(f"[v79] audition après drag impossible : {exc}")
...
def v51_build_ui(self):
    ...
    try:
        ...
        print(f"[v79] bind souris impossible : {exc}")
    ...
    try:
        ...
        print(f"[v79] bind Suppr impossible : {exc}")
...
def v52_normalize_loaded_notes(self, notes):
    ...
    for raw in sorted(notes, key=lambda n: (int(float(n.get("x_step", 0))), int(float(n.get("pair", 0))))):
        ...
        if pair not in valid_pairs:
            print(f"[v79] slice ignorée car absente du break actuel : pair={pair}")
            continue
...
def v52_load_saved_beat(self, event=None):
    ...
    if not saves:
        ...
        print("[v79] aucune sauvegarde trouvée")
        return "break"
    ...
    if not pattern:
        ...
        print("[v79] sauvegarde incompatible :", chosen["path"])
        return "break"
    ...
    print("")
    print("[v79] BEAT RESTAURÉ")
    print("[v79] fichier :", chosen["path"])
    print("[v79] clé :", chosen["key"])
    print("[v79] notes :", len(pattern))
    print("")
...
def v52_list_saved_beats(self, event=None):
    ...
    print("")
    print("[v79] SAUVEGARDES TROUVÉES")
    for i, save in enumerate(saves[:30]):
        print(
            f"[v79] {i:02d} | notes={len(save['notes']):02d} | "
...
def v52_build_ui(self):
    ...
    try:
        ...
        print(f"[v79] UI load saved impossible : {exc}")
...
def v53_run_training(self):
    ...
    if not trainer.exists():
        ...
        print("[v79] trainer introuvable :", trainer)
        return False

    print("")
    print("[v79] AUTO-TRAIN IA après Save...")
    print("")
...
def v53_save(self):
    ...
    try:
        ...
        print(f"[v79] auto-train après save impossible : {exc}")
...
def v53_build_ui(self):
    ...
    try:
        ...
        print(f"[v79] UI auto-train impossible : {exc}")
...
def v55_mouse_up(self, event):
    ...
    if not state.get("moved"):
        ...
        try:
            ...
            print(f"[v79] audition clic note impossible : {exc}")
        ...
        print(
            f"[v79] click play/select | id={item.get('id')} | "
            f"step={item.get('x_step')} | pair={item.get('pair')}"
        )
...
def v58_add_item_at_xy(self, x, y):
    ...
    print(
        f"[v79] add | id={item['id']} | step={x_step} | "
...
def v58_toggle_selected_length(self, event=None):
    ...
    try:
        ...
        print(f"[v79] record_correction length impossible : {exc}")

    v58_refresh_after_edit(self, "toggle_note_length")

    v58_status(self, f"Longueur note : {old_len} -> {new_len} case(s).")
    print(f"[v79] length toggle | id={item.get('id')} | {old_len} -> {new_len}")
...
def v58_duplicate_selected_forward(self, event=None):
    ...
    try:
        ...
        print(f"[v79] audition duplicate impossible : {exc}")
...
def v58_build_ui(self):
    ...
    try:
        ...
        print(f"[v79] UI demi-grille impossible : {exc}")
    ...
    try:
        ...
        print(f"[v79] bind demi-grille impossible : {exc}")
...
def v59_rank_current_slices(self):
    ...
    for pair in self.pair_values:
        ...
        try:
            ...
            print(f"[v79] analyse pair {pair} impossible : {exc}")
            role = "other"
    ...
    print("")
    print("[v79] TOP ROLES CURRENT BREAK")
    for r in ("kick", "snare", "hat"):
        print(f"[v79] {r:5s}:", [(x["pair"], round(x["score"], 3)) for x in ranks[r][:6]])
    print("")
...
def v59_load_role_model():
    ...
    try:
        ...
        print(f"[v79] lecture modèle rôle impossible : {exc}")
        return None
...
def v59_generate_role_aware(self, event=None):
    ...
    if model is None:
        ...
        print("[v79] modèle rôle absent, fallback musical snare 6.")
        model = {
            "breaks": {},
            "global": {},
            "patterns_used": 0,
    ...
    print("")
    print("[v79] ROLE-AWARE GENERATION")
    print("[v79] model patterns_used:", model.get("patterns_used", 0))
    for item in pattern:
        print(
            f"[v79] step {item['x_step']:02d}/{item['x_step'] + item['length'] - 1:02d} | "
...
def v59_run_role_training(self, event=None):
    ...
    try:
        ...
        print(f"[v79] training rôle impossible : {exc}")
...
def v59_save(self):
    ...
    try:
        ...
        print(f"[v79] auto-training rôle après save impossible : {exc}")
...
def v59_build_ui(self):
    ...
    try:
        ...
        print(f"[v79] UI role-aware impossible : {exc}")
...
def v62_add_item_at_xy(self, x, y):
    ...
    print(f"[v79] add | id={item['id']} | step={x_step} | len={length} | pair={pair}")
...
def v62_mouse_down(self, event):
    ...
    if item is not None:
        ...
        print(
            f"[v79] mouse down {state['mode']} | id={item.get('id')} | "
            f"step={item.get('x_step')} | len={item.get('length')} | pair={item.get('pair')}"
        )
    else:
        print("[v79] mouse down empty")
    ...
    if mode == "resize":
        if not state.get("moved"):
            ...
            try:
                ...
                print(f"[v79] audition clic resize impossible : {exc}")
        ...
        if before != after:
            try:
                ...
                print(f"[v79] record_correction resize impossible : {exc}")
            ...
            try:
                ...
                print(f"[v79] audition après resize impossible : {exc}")
    ...
    if mode == "move":
        ...
        if before != after:
            try:
                ...
                print(f"[v79] record_correction move impossible : {exc}")
            ...
            try:
                ...
                print(f"[v79] audition après move impossible : {exc}")
...
def v62_toggle_selected_length(self, event=None):
    ...
    try:
        ...
        print(f"[v79] record_correction toggle length impossible : {exc}")

    v62_refresh_after_edit(self, "toggle_note_length_v62")
    v62_status(self, f"Longueur note : {old_len} → {new_len} case(s).")

    print(f"[v79] toggle length | id={item.get('id')} | {old_len} -> {new_len}")
...
def v62_draw(self):
    ...
    try:
        ...
        print(f"[v79] draw handles impossible : {exc}")
...
def v62_build_ui(self):
    ...
    try:
        ...
        print(f"[v79] bind resize impossible : {exc}")
    ...
    try:
        ...
        print(f"[v79] UI resize impossible : {exc}")
...
def v63_load_model():
    ...
    try:
        ...
        print(f"[v79] lecture modèle strict impossible : {exc}")
        return None
...
def v63_generate_safe_role_ai(self, event=None):
    ...
    print("")
    print("[v79] SAFE ROLE AI GENERATED")
    print("[v79] model strict:", "yes" if model else "no")
    print("[v79] snare main/alt:", main_snare, alt_snare)
    print("[v79] kick  main/alt:", main_kick, alt_kick)
    print("[v79] hat   main/alt:", main_hat, alt_hat)
    print("[v79] pattern:")
    for item in pattern:
        print(
            f"[v79] step {item['x_step']:02d}/{item['x_step'] + item['length'] - 1:02d} | "
...
def v63_run_strict_training(self, event=None):
    ...
    print("")
    print("[v79] TRAIN STRICT ROLE MODEL...")
    print("")
    ...
    try:
        ...
        print(f"[v79] training strict impossible : {exc}")
...
def v63_build_ui(self):
    ...
    try:
        ...
        print(f"[v79] UI safe impossible : {exc}")
...
def v65_rank_slices(self):
    ...
    for pair in self.pair_values:
        ...
        try:
            ...
            print(f"[v79] analyse slice {pair} impossible : {exc}")
            scores = {
                "kick": 0.0,
                "snare": 0.0,
                "hat": 0.0,
    ...
    print("")
    print("[v79] TOP SLICES PAR ROLE — break courant")
    for role in ("kick", "snare", "hat", "ghost_snare"):
        print(f"[v79] {role:12s}:", [(x["pair"], round(x["score"], 3)) for x in ranks[role][:8]])
    print("")
...
def v65_generate_from_role_template(self, event=None):
    ...
    try:
        ...
        print(f"[v79] template introuvable : {exc}")
        return "break"
    ...
    print("")
    print("[v79] CROSS-SONG ROLE TEMPLATE GENERATED")
    print("[v79] template:", V65_TEMPLATE_PATH)
    print("[v79] temperature:", temperature, "| hats:", hat_density, "| ghosts:", ghost_density)
    print("[v79] pattern:")
    for item in pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v79] step {item['x_step']:02d}/{end:02d} | "
...
def v67_train_strict_if_available(self):
    ...
    if trainer is None:
        print("[v79] aucun trainer trouvé.")
        return False

    print("")
    print("[v79] TRAIN après validation :", trainer)
    print("")
    ...
    try:
        ...
        print(f"[v79] training impossible : {exc}")
        return False
...
def v67_generate_candidate(self, event=None):
    ...
    print("")
    print("[v79] CANDIDATE GENERATED")
    print("[v79] break:", self.v67_candidate_safe)
    print("[v79] notes:", len(candidate))
    print("[v79] Maintenant : corrige puis clique Good après modifs, ou Reject.")
    print("")
...
def v67_accept_corrected(self, event=None):
    ...
    print("")
    print("[v79] ACCEPTED GOOD AFTER EDIT")
    print("[v79] saved:", validated_path)
    print("[v79] trained:", trained)
    print("")
...
def v67_accept_without_training(self, event=None):
    ...
    print("[v79] accepted no-train:", validated_path)
...
def v67_build_ui(self):
    ...
    try:
        ...
        print(f"[v79] UI feedback impossible : {exc}")
...
def v68_generate_candidate(self, event=None):
    ...
    print("[v79] Length policy appliquée : 2 cases par défaut, 1 case rare.")
...
def v69_force_pattern_hard_2_grid(self, reason="v69_force_hard_2_grid"):
    ...
    print("")
    print("[v79] HARD 2 GRID APPLIQUÉ")
    for item in self.pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v79] step {item['x_step']:02d}/{end:02d} | "
...
def v70_duplicate_selected_forward(self, event=None):
    ...
    print("")
    print("[v79] CTRL+D DUPLICATE +2")
    print(f"[v79] old_id={item.get('id')} new_id={new_item.get('id')}")
    print(f"[v79] step {old_x}/{old_x + length - 1} -> {new_x}/{new_x + length - 1}")
    print(f"[v79] pair={new_item.get('pair')} len={length}")
    print("")
...
def v70_build_ui(self):
    ...
    try:
        ...
        print(f"[v79] bind Ctrl+D impossible : {exc}")
...
def v71_rank_slices(self):
    ...
    print("")
    print("[v79] PRE-LEARNING TOP SLICES")
    for role in ("kick", "snare", "hat", "ghost_snare"):
        print(f"[v79] {role:12s}:", [(x["pair"], round(x["score"], 3)) for x in ranks[role][:8]])
    print("")
...
def v71_prelearn_roles(self, force=False):
    ...
    print("")
    print("[v79] PRELEARN ROLES POUR", safe)
    for role in ("kick", "snare", "hat", "ghost_snare", "bad"):
        print(f"[v79] {role:12s}:", roles.get(role, []))
    print("")
...
def v71_generate_full_candidate(self, event=None):
    ...
    print("")
    print("[v79] FULL CANDIDATE GENERATED")
    print("[v79] break:", v71_safe(self))
    print("[v79] roles:", roles)
    print("[v79] pattern:")
    for item in pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
...
def v72_open_break(self, safe=None):
    ...
    print("")
    print("[v79] OPEN BREAK")
    print("[v79] current:", current)
    print("[v79] next   :", safe)
    print("[v79] cmd    :", " ".join(cmd))
    print("")
...
def v72_build_ui(self):
    ...
    try:
        ...
        print(f"[v79] UI break switcher impossible : {exc}")
...
def v73_sanitize_tracker_pattern(self, reason="v73_tracker_strict_no_overlap"):
    ...
    print("")
    print("[v79] TRACKER STRICT NO OVERLAP")
    print("[v79] kept:", len(accepted), "| rejected overlaps:", len(rejected))

    for item in accepted:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v79] KEEP step {item['x_step']:02d}/{end:02d} | "
    ...
    if rejected:
        ...
        for item in rejected:
            ...
            print(
                f"[v79] DROP step {item['x_step']:02d}/{end:02d} | "
                f"role={item.get('v73_role_key')} | pair={item.get('pair')}"
            )
...
# ---------------------------------------------------------------------
# v79 EXTENSION : Generate Candidate sur le break choisi dans l'UI
# ---------------------------------------------------------------------

V79_PAIR_DIR = Path("dataset/pair_blocks_v02")
V79_AUTOGEN_ENV = "BREAKBEATAI_V79_AUTOGEN"


def v79_status(self, text):
    try:
        if hasattr(self, "set_status"):
            self.set_status(text)
        else:
...
def v79_list_breaks():
    if not V79_PAIR_DIR.exists():
        return []

    out = []

    for path in sorted(V79_PAIR_DIR.glob("*_pair_blocks_v02.json")):
        source = v79_pairblock_source(path)
...
def v79_default_break(breaks, current):
    if not breaks:
        return current or ""

    current_n = v79_norm(current)

    for b in breaks:
        if v79_norm(b) == current_n:
            return b

    for b in breaks:
        if current_n and current_n in v79_norm(b):
            return b

    return breaks[0]


def v79_selected_break(self):
    try:
        value = self.v79_break_var.get()
        if value:
            return str(value).strip()
    except Exception:
        pass

    return v79_current_safe(self)


def v79_real_generator():
    """
    Vrai générateur musical, pas le wrapper du menu.
    """
    for name in [
...
def v79_generate_current_only(self, event=None):
    """
    Génère sur le break réellement chargé dans la fenêtre actuelle.
    """
    fn = v79_real_generator()

    print("")
    print("[v79] GENERATE CURRENT")
    print("[v79] break courant:", v79_current_safe(self))
    print("[v79] générateur   :", getattr(fn, "__name__", None))
    print("")

    if fn is None:
        v79_status(self, "Aucun vrai générateur musical trouvé.")
        return "break"
    ...
    try:
        self.v67_candidate_before_edit = json.loads(json.dumps(self.pattern, ensure_ascii=False))
        self.v67_candidate_time = time.strftime("%Y-%m-%d %H:%M:%S")
        self.v67_candidate_safe = v79_current_safe(self)
    except Exception:
        pass

    v79_status(self, f"Generate Candidate OK sur {v79_current_safe(self)}.")
    return result


def v79_launch_break(self, source, autogen=False):
    source = str(source).strip()

    if not source:
        v79_status(self, "Aucun break sélectionné.")
        return "break"
    ...
    if autogen:
        env[V79_AUTOGEN_ENV] = "1"
    else:
        env.pop(V79_AUTOGEN_ENV, None)
    ...
    print("")
    print("[v79] LAUNCH BREAK")
    print("[v79] courant :", v79_current_safe(self))
    print("[v79] cible   :", source)
    print("[v79] autogen :", autogen)
    print("[v79] cmd     :", " ".join(cmd))
    print("")
...
def v79_open_selected_break(self, event=None):
    selected = v79_selected_break(self)
    current = v79_current_safe(self)

    print("")
    print("[v79] OPEN SELECTED")
    print("[v79] current :", current)
    print("[v79] selected:", selected)
    print("")

    if v79_norm(selected) == v79_norm(current):
        v79_status(self, f"Déjà sur {current}.")
        return "break"

    return v79_launch_break(self, selected, autogen=False)


def v79_generate_selected_break(self, event=None):
    """
    Bouton principal :
    sélection dans l'UI -> génération sur ce break.
    """
    selected = v79_selected_break(self)
    current = v79_current_safe(self)

    print("")
    print("[v79] GENERATE SELECTED")
    print("[v79] current :", current)
    print("[v79] selected:", selected)
    print("")
    ...
    if v79_norm(selected) == v79_norm(current):
        return v79_generate_current_only(self, event)

    # Sinon : on ouvre le break choisi et on génère automatiquement.
    return v79_launch_break(self, selected, autogen=True)


def v79_refresh_breaks(self, event=None):
    breaks = v79_list_breaks()
    current = v79_current_safe(self)

    if not breaks:
        v79_status(self, "Aucun break trouvé dans dataset/pair_blocks_v02.")
        return "break"

    default = v79_default_break(breaks, current)

    try:
        self.v79_break_combo["values"] = breaks
        self.v79_break_var.set(default)
    except Exception:
        pass

    v79_status(self, f"{len(breaks)} breaks chargés dans le menu.")
    return "break"


def v79_autogen_if_requested(self):
    if getattr(self, "_v79_autogen_done", False):
        return

    if os.environ.get(V79_AUTOGEN_ENV) != "1":
        return

    self._v79_autogen_done = True

    print("")
    print("[v79] AUTO-GENERATE APRÈS OUVERTURE")
    print("[v79] break:", v79_current_safe(self))
    print("")

    try:
        self.root.after(900, lambda: v79_generate_current_only(self))
    except Exception:
        v79_generate_current_only(self)


_old_v79_build_ui = SliceIndexTracker.build_ui


def v79_build_ui(self):
    _old_v79_build_ui(self)
    ...
    try:
        self.root.title(f"BreakbeatAI v79 — {current}")

        # Bandeau très visible en haut de l'UI.
        panel = tk.Frame(
            self.root,
            bg="#130b1f",
            bd=2,
        ...
        panel.place(x=8, y=8, anchor="nw")
        panel.lift()

        self.v79_panel = panel
        ...
        self.v79_break_var = tk.StringVar(value=default)

        self.v79_break_combo = ttk.Combobox(
            panel,
            textvariable=self.v79_break_var,
            values=breaks,
            width=38,
            state="readonly",
        )
        self.v79_break_combo.pack(side="left", padx=4, pady=6)

        tk.Button(
            panel,
            text="Open Selected",
            command=self.open_selected_break_v79,
            bg="#30384a",
            fg="#f5eefe",
        ...
        tk.Button(
            ...
            command=self.generate_current_v79,
            bg="#30513f",
            fg="#ffffff",
        ).pack(side="left", padx=4, pady=6)
        ...
        tk.Button(
            panel,
            text="Refresh",
            command=self.refresh_breaks_v79,
            bg="#303030",
            fg="#eeeeee",
        ...
        def keep_panel_up(event=None):
            try:
                self.v79_panel.lift()
            except Exception:
                pass

        self.root.bind("<Configure>", keep_panel_up)
        self.root.bind("<F8>", keep_panel_up)

    except Exception as exc:
        print(f"[v79] bandeau UI impossible : {exc}")

    v79_status(
        self,
        f"v79 : choisis un break dans le bandeau rose puis Generate Selected. Courant = {current}."
    )

    v79_autogen_if_requested(self)


SliceIndexTracker.open_selected_break_v79 = v79_open_selected_break
SliceIndexTracker.generate_current_v79 = v79_generate_current_only
SliceIndexTracker.refresh_breaks_v79 = v79_refresh_breaks

# Tous les vieux boutons Generate Candidate passent maintenant par le break choisi dans le bandeau v79.
SliceIndexTracker.generate_candidate = v79_generate_selected_break
SliceIndexTracker.generate_full_candidate = v79_generate_selected_break
SliceIndexTracker.generate_ai_pattern = v79_generate_selected_break
SliceIndexTracker.generate_cross_song_template = v79_generate_selected_break
SliceIndexTracker.generate_safe_experiment = v79_generate_selected_break
SliceIndexTracker.generate_locked_roles = v79_generate_selected_break
SliceIndexTracker.generate_role_aware = v79_generate_selected_break
SliceIndexTracker.generate_safe_role_ai = v79_generate_selected_break

SliceIndexTracker.build_ui = v79_build_ui
