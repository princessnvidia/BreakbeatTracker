...
"""
BreakbeatAI Tracker Editor v153 — slice index
...
def warp_audio_to_length_external(audio, target_len, cache_key=None):
    ...
    if rubberband:
        try:
            ...
            print(f"[v153] rubberband échec, fallback ffmpeg/no-speed : {exc}")
    ...
    if ffmpeg:
        try:
            ...
            print(f"[v153] ffmpeg atempo échec, fallback no-speed : {exc}")
...
class SliceIndexTracker:
    def __init__(self, root, pair_json):
        ...
        self.root.title("BreakbeatAI Tracker Editor v153 — slice index")
    ...
    def detect_external_player(self):
        for name in ["pw-play", "paplay", "aplay", "ffplay"]:
            ...
            if path:
                print(f"[v153] Backend audio : {name} -> {path}")
                return name

        print("[v153] Aucun backend système trouvé, fallback sounddevice.")
        return None
    ...
    def build_initial_pattern(self, randomize=False):
        ...
        print(
            f"[v153] Génération pattern | break={self.project['safe']} | "
        ...
        for slot in range(max_items):
            ...
            print(f"[v153] slot {slot:02d} | step {x_step:02d} -> slice {pair:02d}")
    ...
    def build_ui(self):
        ...
        title = tk.Label(
            main,
            text="BreakbeatAI v153 — random KSH positions",
            bg="#111018",
            fg="#ff7acc",
            font=("Sans", 20, "bold"),
        )
    ...
    def record_correction(self, event_type, before=None, after=None):
        ...
        with corrections_path.open("a", encoding="utf-8") as f:
            f.write(json.dumps(event, ensure_ascii=False) + "[v153]n")
        ...
        self.output_label.config(
            text=(
                f"Auto-learn sauvegardé : {event_type}[v153]n"
                f"Correction : {corrections_path}[v153]n"
                f"Mémoire : {MEMORY_PATH}[v153]n"
                f"Latest : {latest_path}"
            )
        )

        print(f"[v153] correction sauvegardée : {event_type}")
    ...
    def randomize_pattern(self):
        ...
        self.output_label.config(text=f"Randomize OK. Mémoire appliquée.[v153]nLatest : {latest}")
    ...
    def play_audition_array(self, audio, label="audition"):
        ...
        audition_wav = OUT_DIR / f"{self.project['safe']}_v153_audition.wav"
        ...
        if self.external_player:
            try:
                ...
                print(f"[v153] Erreur audition {self.external_player} : {exc}")
    ...
    def on_close(self):
        ...
        """
        print("[v153] Fermeture : arrêt audio + destruction fenêtre")
    ...
    def play_audio_array(self, audio, label="audio"):
        ...
        audio = normalize(audio)
        live_wav = OUT_DIR / f"{self.project['safe']}_v153_live.wav"
        ...
        if self.external_player:
            try:
                ...
                print(f"[v153] Erreur backend {self.external_player} : {exc}")
    ...
    def render_preview_file(self):
        OUT_DIR.mkdir(parents=True, exist_ok=True)
        wav = OUT_DIR / f"{self.project['safe']}_tracker_app_v153_preview.wav"
    ...
    def write_latest_pattern(self, reason="correction"):
        ...
        path = LATEST_DIR / f"{self.project['safe']}_latest_slice_index_v153.json"

        data = {
            "version": "breakbeatai_latest_slice_index_v153",
            "reason": reason,
    ...
    def save(self):
        ...
        tracker_path = OUT_DIR / f"{self.project['safe']}_tracker_app_edit_v153.json"
        ...
        stamp = file_stamp()
        validated_path = VALIDATED_DIR / f"{self.project['safe']}_validated_slice_index_loop64_v153_{stamp}.json"

        validated = {
            "version": "breakbeatai_validated_slice_index_loop64_v153",
        ...
        self.output_label.config(
            text=(
                f"OK sauvegardé[v153]n"
                f"Tracker : {tracker_path}[v153]n"
                f"Validation IA : {validated_path}[v153]n"
                f"Latest : {latest}[v153]n"
                f"Preview : {wav}"
            )
        )
...
def v44_record_correction(self, event_type, before=None, after=None):
    ...
    print(f"[v153] correction sauvegardée : {event_type}")
    print(f"[v153] correction path : {corrections_path}")
    print(f"[v153] latest path : {latest_path}")
...
def v45_move_selected(self, dx, dy):
    ...
    if dy != 0 and before != after:
        try:
            ...
            print(f"[v153] audition après flèche haut/bas impossible : {exc}")
...
def v46_warp_loop_pitch_preserve(audio, target_len, cache_key):
    ...
    if rubberband:
        try:
            ...
            print(f"[v153] rubberband global warp échec : {exc}")
    ...
    if ffmpeg:
        try:
            ...
            print(f"[v153] ffmpeg global warp échec : {exc}")

    print("[v153] Aucun warp pitch-preserve dispo, fallback fit no-speed.")
    return v46_fit_audio_no_speed(audio, target_len)
...
def v46_build_initial_pattern(self, randomize=False):
    ...
    print(
        f"[v153] M8 pattern | break={self.project['safe']} | "
...
def v46_get_warped_loop(self):
    ...
    print(
        f"[v153] global warp OK | source_len={loop_end-loop_start} | "
...
def v46_build_ui(self):
    ...
    try:
        ...
        print(f"[v153] UI extension impossible : {exc}")
...
def v47_play_audio_array(self, audio, label="audio"):
    ...
    if len(audio) == 0 or v47_is_silent(audio):
        ...
        print(f"[v153] lecture annulée : audio silencieux ({label})")
        return 0
...
def v47_toggle_loop(self):
    ...
    if not getattr(self, "pattern", None):
        ...
        print("[v153] loop annulée : pattern vide")
        return "break"
...
def v47_delete_selected(self):
    ...
    if not getattr(self, "pattern", None):
        ...
        print("[v153] pattern vide après suppression : audio coupé")
...
def v48_build_initial_pattern(self, randomize=False):
    ...
    print("[v153] pattern initial forcé VIDE : aucun son ne doit jouer.")
    return []
...
def v48_load_first_slices(self):
    ...
    print(f"[v153] Load 16 slices : {len(self.pattern)} items")


def v48_debug_pattern(self, label="render"):
    print("")
    print(f"[v153] DEBUG PATTERN avant {label}")
    print(f"[v153] items = {len(getattr(self, 'pattern', []) or [])}")

    if not getattr(self, "pattern", None):
        print("[v153] pattern vide confirmé")
        print("")
        return
    ...
    for item in self.pattern:
        ...
        print(
            f"[v153] id={item_id} | step {x:02d}/{end:02d} | "
...
def v48_render_audio_with_timeline(self):
    ...
    if not getattr(self, "pattern", None):
        loop_samples = self.get_loop_samples()
        print(f"[v153] rendu silence total : pattern vide | samples={loop_samples}")
        return np.zeros(loop_samples, dtype=np.float32), []
    ...
    print(f"[v153] rendu audio peak={peak:.8f} | timeline_events={len(timeline) if timeline is not None else 'None'}")

    if timeline:
        print("[v153] timeline events:")
        for ev in timeline:
            print(f"[v153]   {ev}")
...
def v48_toggle_loop(self):
    if not getattr(self, "pattern", None):
        ...
        print("[v153] Space ignoré : pattern vide.")
        return "break"
...
def v48_build_ui(self):
    ...
    try:
        ...
        print(f"[v153] ajout UI impossible : {exc}")
...
def v49_add_item_at_xy(self, x, y):
    ...
    try:
        ...
        print(f"[v153] record_correction add impossible : {exc}")
    ...
    try:
        ...
        print(f"[v153] audition après ajout impossible : {exc}")

    print(f"[v153] add | id={item['id']} | step={x_step} | pair={pair} | lane={lane}")
...
def v49_duplicate_selected_forward(self, event=None):
    ...
    if item is None:
        ...
        print("[v153] duplicate annulé : aucune note sélectionnée")
        return "break"
    ...
    if new_x > max_x:
        ...
        print("[v153] duplicate annulé : fin de grille")
        return "break"
    ...
    try:
        ...
        print(f"[v153] record_correction duplicate impossible : {exc}")
    ...
    try:
        ...
        print(f"[v153] audition après duplicate impossible : {exc}")

    print(
        f"[v153] duplicate | old_id={item.get('id')} | new_id={new_item.get('id')} | "
...
def v49_build_ui(self):
    ...
    try:
        ...
        print(f"[v153] bind canvas click impossible : {exc}")
    ...
    try:
        ...
        print(f"[v153] bind keys impossible : {exc}")
...
def v50_load_style_model():
    ...
    try:
        ...
        print(f"[v153] impossible de charger le modèle IA : {exc}")
        return None
...
def v50_generate_ai_pattern(self, event=None):
    ...
    if model is None:
        ...
        print("[v153] modèle absent :", STYLE_MODEL_PATH)
        return "break"
    ...
    print("")
    print(f"[v153] GENERATE IA | safe={safe} | pairs={pair_values} | slots={max_slots}")
    ...
    for slot in range(max_slots):
        ...
        print(
            f"[v153] slot {slot:02d} | step {x_step:02d} | "
    ...
    try:
        ...
        print(f"[v153] write_latest_pattern impossible : {exc}")
...
def v50_build_ui(self):
    ...
    try:
        ...
        print(f"[v153] UI IA impossible : {exc}")
...
def v51_mouse_down(self, event):
    ...
    if item is not None:
        ...
        print(
            f"[v153] mouse down note | id={item.get('id')} | "
            f"step={item.get('x_step')} | pair={item.get('pair')}"
        )
    else:
        print("[v153] mouse down empty")
...
def v51_mouse_up(self, event):
    ...
    if state.get("empty"):
        ...
        print("[v153] drag depuis vide ignoré")
        return "break"
    ...
    if before != after:
        try:
            ...
            print(f"[v153] record_correction drag impossible : {exc}")
        ...
        print(
            f"[v153] drag move | id={after.get('id')} | "
            f"step {before.get('x_step')} -> {after.get('x_step')} | "
        ...
        try:
            ...
            print(f"[v153] audition après drag impossible : {exc}")
...
def v51_build_ui(self):
    ...
    try:
        ...
        print(f"[v153] bind souris impossible : {exc}")
    ...
    try:
        ...
        print(f"[v153] bind Suppr impossible : {exc}")
...
def v52_normalize_loaded_notes(self, notes):
    ...
    for raw in sorted(notes, key=lambda n: (int(float(n.get("x_step", 0))), int(float(n.get("pair", 0))))):
        ...
        if pair not in valid_pairs:
            print(f"[v153] slice ignorée car absente du break actuel : pair={pair}")
            continue
...
def v52_load_saved_beat(self, event=None):
    ...
    if not saves:
        ...
        print("[v153] aucune sauvegarde trouvée")
        return "break"
    ...
    if not pattern:
        ...
        print("[v153] sauvegarde incompatible :", chosen["path"])
        return "break"
    ...
    print("")
    print("[v153] BEAT RESTAURÉ")
    print("[v153] fichier :", chosen["path"])
    print("[v153] clé :", chosen["key"])
    print("[v153] notes :", len(pattern))
    print("")
...
def v52_list_saved_beats(self, event=None):
    ...
    print("")
    print("[v153] SAUVEGARDES TROUVÉES")
    for i, save in enumerate(saves[:30]):
        print(
            f"[v153] {i:02d} | notes={len(save['notes']):02d} | "
...
def v52_build_ui(self):
    ...
    try:
        ...
        print(f"[v153] UI load saved impossible : {exc}")
...
def v53_run_training(self):
    ...
    if not trainer.exists():
        ...
        print("[v153] trainer introuvable :", trainer)
        return False

    print("")
    print("[v153] AUTO-TRAIN IA après Save...")
    print("")
    ...
    try:
        ...
        if result.returncode != 0:
            ...
            print("[v153] auto-train échec code :", result.returncode)
            return False

        v53_status(self, "Save OK + IA entraînée automatiquement.")
        print("[v153] AUTO-TRAIN OK")
        return True

    except Exception as exc:
        v53_status(self, f"Save OK, mais auto-train impossible : {exc}")
        print("[v153] exception auto-train :", exc)
        return False
...
def v53_save(self):
    ...
    try:
        ...
        print(f"[v153] auto-train après save impossible : {exc}")
...
def v53_build_ui(self):
    ...
    try:
        ...
        print(f"[v153] UI auto-train impossible : {exc}")
...
def v55_mouse_up(self, event):
    ...
    if not state.get("moved"):
        ...
        try:
            ...
            print(f"[v153] audition clic note impossible : {exc}")
        ...
        print(
            f"[v153] click play/select | id={item.get('id')} | "
            f"step={item.get('x_step')} | pair={item.get('pair')}"
        )
...
def v58_add_item_at_xy(self, x, y):
    ...
    print(
        f"[v153] add | id={item['id']} | step={x_step} | "
...
def v58_toggle_selected_length(self, event=None):
    ...
    try:
        ...
        print(f"[v153] record_correction length impossible : {exc}")

    v58_refresh_after_edit(self, "toggle_note_length")

    v58_status(self, f"Longueur note : {old_len} -> {new_len} case(s).")
    print(f"[v153] length toggle | id={item.get('id')} | {old_len} -> {new_len}")
...
def v58_duplicate_selected_forward(self, event=None):
    ...
    try:
        ...
        print(f"[v153] audition duplicate impossible : {exc}")
...
def v58_build_ui(self):
    ...
    try:
        ...
        print(f"[v153] UI demi-grille impossible : {exc}")
    ...
    try:
        ...
        print(f"[v153] bind demi-grille impossible : {exc}")
...
def v59_rank_current_slices(self):
    ...
    for pair in self.pair_values:
        ...
        try:
            ...
            print(f"[v153] analyse pair {pair} impossible : {exc}")
            role = "other"
    ...
    print("")
    print("[v153] TOP ROLES CURRENT BREAK")
    for r in ("kick", "snare", "hat"):
        print(f"[v153] {r:5s}:", [(x["pair"], round(x["score"], 3)) for x in ranks[r][:6]])
    print("")
...
def v59_load_role_model():
    ...
    try:
        ...
        print(f"[v153] lecture modèle rôle impossible : {exc}")
        return None
...
def v59_generate_role_aware(self, event=None):
    ...
    if model is None:
        ...
        print("[v153] modèle rôle absent, fallback musical snare 6.")
        model = {
            "breaks": {},
            "global": {},
            "patterns_used": 0,
    ...
    print("")
    print("[v153] ROLE-AWARE GENERATION")
    print("[v153] model patterns_used:", model.get("patterns_used", 0))
    for item in pattern:
        print(
            f"[v153] step {item['x_step']:02d}/{item['x_step'] + item['length'] - 1:02d} | "
...
def v59_run_role_training(self, event=None):
    ...
    try:
        ...
        print(f"[v153] training rôle impossible : {exc}")
...
def v59_save(self):
    ...
    try:
        ...
        print(f"[v153] auto-training rôle après save impossible : {exc}")
...
def v59_build_ui(self):
    ...
    try:
        ...
        print(f"[v153] UI role-aware impossible : {exc}")
...
def v62_add_item_at_xy(self, x, y):
    ...
    print(f"[v153] add | id={item['id']} | step={x_step} | len={length} | pair={pair}")
...
def v62_mouse_down(self, event):
    ...
    if item is not None:
        ...
        print(
            f"[v153] mouse down {state['mode']} | id={item.get('id')} | "
            f"step={item.get('x_step')} | len={item.get('length')} | pair={item.get('pair')}"
        )
    else:
        print("[v153] mouse down empty")
    ...
    if mode == "resize":
        if not state.get("moved"):
            ...
            try:
                ...
                print(f"[v153] audition clic resize impossible : {exc}")
        ...
        if before != after:
            try:
                ...
                print(f"[v153] record_correction resize impossible : {exc}")
            ...
            try:
                ...
                print(f"[v153] audition après resize impossible : {exc}")
    ...
    if mode == "move":
        ...
        if before != after:
            try:
                ...
                print(f"[v153] record_correction move impossible : {exc}")
            ...
            try:
                ...
                print(f"[v153] audition après move impossible : {exc}")
...
def v62_toggle_selected_length(self, event=None):
    ...
    try:
        ...
        print(f"[v153] record_correction toggle length impossible : {exc}")

    v62_refresh_after_edit(self, "toggle_note_length_v62")
    v62_status(self, f"Longueur note : {old_len} → {new_len} case(s).")

    print(f"[v153] toggle length | id={item.get('id')} | {old_len} -> {new_len}")
...
def v62_draw(self):
    ...
    try:
        ...
        print(f"[v153] draw handles impossible : {exc}")
...
def v62_build_ui(self):
    ...
    try:
        ...
        print(f"[v153] bind resize impossible : {exc}")
    ...
    try:
        ...
        print(f"[v153] UI resize impossible : {exc}")
...
def v63_load_model():
    ...
    try:
        ...
        print(f"[v153] lecture modèle strict impossible : {exc}")
        return None
...
def v63_generate_safe_role_ai(self, event=None):
    ...
    print("")
    print("[v153] SAFE ROLE AI GENERATED")
    print("[v153] model strict:", "yes" if model else "no")
    print("[v153] snare main/alt:", main_snare, alt_snare)
    print("[v153] kick  main/alt:", main_kick, alt_kick)
    print("[v153] hat   main/alt:", main_hat, alt_hat)
    print("[v153] pattern:")
    for item in pattern:
        print(
            f"[v153] step {item['x_step']:02d}/{item['x_step'] + item['length'] - 1:02d} | "
...
def v63_run_strict_training(self, event=None):
    ...
    print("")
    print("[v153] TRAIN STRICT ROLE MODEL...")
    print("")
    ...
    try:
        ...
        print(f"[v153] training strict impossible : {exc}")
...
def v63_build_ui(self):
    ...
    try:
        ...
        print(f"[v153] UI safe impossible : {exc}")
...
def v65_rank_slices(self):
    ...
    for pair in self.pair_values:
        ...
        try:
            ...
            print(f"[v153] analyse slice {pair} impossible : {exc}")
            scores = {
                "kick": 0.0,
                "snare": 0.0,
                "hat": 0.0,
    ...
    print("")
    print("[v153] TOP SLICES PAR ROLE — break courant")
    for role in ("kick", "snare", "hat", "ghost_snare"):
        print(f"[v153] {role:12s}:", [(x["pair"], round(x["score"], 3)) for x in ranks[role][:8]])
    print("")
...
def v65_generate_from_role_template(self, event=None):
    ...
    try:
        ...
        print(f"[v153] template introuvable : {exc}")
        return "break"
    ...
    print("")
    print("[v153] CROSS-SONG ROLE TEMPLATE GENERATED")
    print("[v153] template:", V65_TEMPLATE_PATH)
    print("[v153] temperature:", temperature, "| hats:", hat_density, "| ghosts:", ghost_density)
    print("[v153] pattern:")
    for item in pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v153] step {item['x_step']:02d}/{end:02d} | "
...
def v65_build_ui(self):
    ...
    try:
        ...
        print(f"[v153] UI impossible : {exc}")
...
def v67_train_strict_if_available(self):
    ...
    if trainer is None:
        print("[v153] aucun trainer trouvé.")
        return False

    print("")
    print("[v153] TRAIN après validation :", trainer)
    print("")
    ...
    try:
        ...
        print(f"[v153] training impossible : {exc}")
        return False
...
def v67_generate_candidate(self, event=None):
    ...
    print("")
    print("[v153] CANDIDATE GENERATED")
    print("[v153] break:", self.v67_candidate_safe)
    print("[v153] notes:", len(candidate))
    print("[v153] Maintenant : corrige puis clique Good après modifs, ou Reject.")
    print("")
...
def v67_reject_candidate(self, event=None):
    ...
    print("")
    print("[v153] REJECTED")
    print("[v153] saved:", rejected_path)
    print("")
...
def v67_accept_corrected(self, event=None):
    ...
    print("")
    print("[v153] ACCEPTED GOOD AFTER EDIT")
    print("[v153] saved:", validated_path)
    print("[v153] trained:", trained)
    print("")
...
def v67_accept_without_training(self, event=None):
    ...
    print("[v153] accepted no-train:", validated_path)
...
def v67_build_ui(self):
    ...
    try:
        ...
        print(f"[v153] UI feedback impossible : {exc}")
...
def v68_generate_candidate(self, event=None):
    ...
    print("[v153] Length policy appliquée : 2 cases par défaut, 1 case rare.")
...
def v69_force_pattern_hard_2_grid(self, reason="v69_force_hard_2_grid"):
    ...
    print("")
    print("[v153] HARD 2 GRID APPLIQUÉ")
    for item in self.pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v153] step {item['x_step']:02d}/{end:02d} | "
...
def v70_duplicate_selected_forward(self, event=None):
    ...
    print("")
    print("[v153] CTRL+D DUPLICATE +2")
    print(f"[v153] old_id={item.get('id')} new_id={new_item.get('id')}")
    print(f"[v153] step {old_x}/{old_x + length - 1} -> {new_x}/{new_x + length - 1}")
    print(f"[v153] pair={new_item.get('pair')} len={length}")
    print("")
...
def v70_build_ui(self):
    ...
    try:
        ...
        print(f"[v153] bind Ctrl+D impossible : {exc}")
...
def v71_rank_slices(self):
    ...
    print("")
    print("[v153] PRE-LEARNING TOP SLICES")
    for role in ("kick", "snare", "hat", "ghost_snare"):
        print(f"[v153] {role:12s}:", [(x["pair"], round(x["score"], 3)) for x in ranks[role][:8]])
    print("")
...
def v71_prelearn_roles(self, force=False):
    ...
    print("")
    print("[v153] PRELEARN ROLES POUR", safe)
    for role in ("kick", "snare", "hat", "ghost_snare", "bad"):
        print(f"[v153] {role:12s}:", roles.get(role, []))
    print("")
...
def v71_generate_full_candidate(self, event=None):
    ...
    print("")
    print("[v153] FULL CANDIDATE GENERATED")
    print("[v153] break:", v71_safe(self))
    print("[v153] roles:", roles)
    print("[v153] pattern:")
    for item in pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
...
def v72_open_break(self, safe=None):
    ...
    print("")
    print("[v153] OPEN BREAK")
    print("[v153] current:", current)
    print("[v153] next   :", safe)
    print("[v153] cmd    :", " ".join(cmd))
    print("")
...
def v72_build_ui(self):
    ...
    try:
        ...
        print(f"[v153] UI break switcher impossible : {exc}")
...
def v73_sanitize_tracker_pattern(self, reason="v73_tracker_strict_no_overlap"):
    ...
    print("")
    print("[v153] TRACKER STRICT NO OVERLAP")
    print("[v153] kept:", len(accepted), "| rejected overlaps:", len(rejected))

    for item in accepted:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v153] KEEP step {item['x_step']:02d}/{end:02d} | "
    ...
    if rejected:
        ...
        for item in rejected:
            ...
            print(
                f"[v153] DROP step {item['x_step']:02d}/{end:02d} | "
                f"role={item.get('v73_role_key')} | pair={item.get('pair')}"
            )
...
def v81_launch_break(self, source):
    ...
    print("")
    print("[v153] CHARGER BREAK")
    print("[v153] courant :", current)
    print("[v153] nouveau :", source)
    print("[v153] cmd     :", " ".join(cmd))
    print("")
...
def v81_patch_charger_break_buttons(self):
    ...
    print(f"[v153] boutons Charger break patchés : {patched}")
...
def v81_generate_candidate(self, event=None):
    ...
    print("")
    print("[v153] GENERATE CANDIDATE")
    print("[v153] break chargé :", current)
    print("[v153] générateur   :", getattr(fn, "__name__", None))
    print("")
...
def v81_run_training(self):
    ...
    if script is None:
        print("[v153] Aucun script training trouvé.")
...
def v81_save_feedback(self, verdict, train_now=False):
    ...
    print("")
    print("[v153] FEEDBACK SAUVÉ")
    print("[v153] break  :", current)
    print("[v153] verdict:", verdict)
    print("[v153] train  :", train_now)
    print("[v153] fichier:", out_path)
    print("")
...
def v81_build_ui(self):
    ...
    try:
        self.root.title(f"BreakbeatAI v153 — random KSH positions")
    except Exception:
        pass
    ...
    try:
        ...
        print(f"[v153] patch Charger break impossible : {exc}")

    try:
        v81_install_training_toolbar(self)
    except Exception as exc:
        print(f"[v153] toolbar impossible : {exc}")
...
def v82_auto_role_pools(self):
    ...
    print("")
    print("[v153] ROLE POOLS")
    for r in ["kick", "snare", "hat", "ghost_snare", "bad"]:
        print(f"[v153] {r:12s}:", roles.get(r, []))
    print("")
...
def v82_generate_candidate(self, event=None):
    ...
    print("")
    print("[v153] NEW CANDIDATE")
    print("[v153] break :", current)
    print("[v153] nonce :", nonce)
    print("[v153] rejected known:", len(rejected))
    print("[v153] fingerprint rejeté ?: ", chosen_fp in rejected)
    for item in self.pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v153] step {item['x_step']:02d}/{end:02d} "
...
def v82_patch_training_buttons(self):
    ...
    print(f"[v153] boutons training patchés : {patched}")
...
def v82_build_ui(self):
    ...
    try:
        ...
        print(f"[v153] patch boutons impossible : {exc}")
...
def v110_generate(self, event=None, reject_mode=False):
    ...
    print("")
    if reject_mode:
        print("[v153] REJECT/BAD -> variation premier 32 + autres lignes")
    else:
        print("[v153] GENERATE 64")
    print("[v153] break:", current)
    print("[v153] first32:", first32_id)
    print("[v153] response:", response_id)
    print("[v153] role_offset:", role_offset)
    for role in ["kick", "snare", "hat"]:
        lane = "?"
        try:
            ...
            pass
        print(f"[v153] {role:6s}: pair={chosen[role]['pair']} lane={lane} source={chosen[role]['source']}")
    print("")
    ...
    try:
        ...
        print(f"[v153] draw impossible : {exc}")
    ...
    try:
        ...
        print(f"[v153] refresh_panel impossible : {exc}")
...
def v110_render_pattern_to_audio(self):
    ...
    for item in pattern:
        ...
        if pair not in slice_map:
            skipped += 1
            print(f"[v153] skip pair introuvable : {pair}")
            continue
...
def v110_export_wav_current(self, event=None):
    try:
        ...
        print("")
        print("[v153] EXPORT WAV OK")
        print("[v153] break:", current)
        print("[v153] wav  :", out_path)
        print("[v153] json :", info_path)
        print("[v153] duration:", round(info["duration_seconds"], 3), "s")
        print("[v153] rendered:", info["rendered_notes"], "skipped:", info["skipped_notes"])
        print("")
...
def v110_space_play(self, event=None):
    ...
    try:
        ...
        if btn is not None:
            print("[v153] SPACE -> Loop / Space")
            try:
                btn.invoke()
            except Exception as exc:
                print("[v153] invoke Loop / Space impossible :", exc)
            return "break"

        print("[v153] SPACE : bouton Loop / Space introuvable.")
...
def v110_patch_buttons(self):
    ...
    print(f"[v153] boutons patchés : Generate={gen_patched}, Reject/Bad={bad_patched}")
...
def v110_build_ui(self):
    ...
    try:
        ...
        print("[v153] patch boutons impossible :", exc)

    try:
        v110_add_export_button(self)
    except Exception as exc:
        print("[v153] ajout Export WAV impossible :", exc)

    try:
        v110_install_space_bindings(self)
    except Exception as exc:
        print("[v153] bind espace impossible :", exc)
...
def v113_export(self, event=None):
    ...
    print("[v153] ERREUR : export_wav_current_v110 introuvable.")
...
def v113_place_export_button(self):
    ...
    try:
        ...
        print("[v153] bouton Export WAV visible ajouté.")
        v113_status(self, "v113 : Export WAV visible. Espace reste Loop / Space.")

    except Exception as exc:
        print("[v153] placement bouton Export WAV impossible :", exc)
        try:
            btn.place(x=760, y=32, height=30)
            btn.lift()
            self.export_button_v113 = btn
...
def v115_apply_cells_minus_25(self):
    ...
    print("")
    print("[v153] APPLICATION CASES -3.5px")
    ...
    for name in V115_WIDTH_GLOBAL_NAMES:
        try:
            ...
            if isinstance(value, (int, float)) and value > 6:
                ...
                print(f"[v153] global {name}: {value} -> {globals()[name]}")
        except Exception:
            pass
        ...
        try:
            if hasattr(self, attr):
                ...
                if isinstance(value, (int, float)) and value > 6:
                    ...
                    print(f"[v153] attr {attr}: {value} -> {getattr(self, attr)}")
        except Exception:
            pass
...
def v115_build_ui(self):
    ...
    try:
        self.root.title("BreakbeatAI v153 — random KSH positions")
    except Exception:
        pass
    ...
    try:
        ...
        print("[v153] impossible d'appliquer -3.5px :", exc)
...
def v117_render_pattern_to_audio(self):
    ...
    print("")
    print("[v153] EXPORT EXACT CELLS")
    print("[v153] break:", current)
    print("[v153] pairblock:", pairblock_file)
    print("[v153] sr:", sr)
    print("[v153] median_pair_samples:", median_pair_samples)
    print("[v153] case_samples:", case_samples)
    print("[v153] export_samples:", total_samples)
    print("[v153] export_seconds:", round(total_samples / float(sr), 6))
    print("")
...
def v117_export_wav_current(self, event=None):
    try:
        ...
        print("[v153] wav  :", out_path)
        print("[v153] json :", info_path)
        print("[v153] duration:", round(info["duration_seconds"], 6), "s")
        print("[v153] case_seconds:", round(info["case_seconds"], 6))
        print("[v153] rendered:", info["rendered_notes"], "skipped:", info["skipped_notes"])
        print("")
...
def v118_current_break(self):
    ...
    if candidates:
        ...
        print("[v153] break courant:", stem, "| source:", source, "| raw:", raw)
        return stem

    print("[v153] WARNING : break courant introuvable, fallback Camo.")
    return "Camo_Break_-_3A"
...
def v118_find_best_playback_wav(since):
    ...
    print("")
    print("[v153] WAV candidats playback :")
    for score, mtime, size, wav in candidates[:12]:
        age = time.time() - mtime
        print(f"[v153] score={score:8d} age={age:7.2f}s size={size:10d} path={wav}")
    print("")
...
def v118_copy_playback_wav(self, since):
    ...
    if best is None:
        ...
        print("[v153] aucun WAV playback trouvé.")
        return "break"
    ...
    print("")
    print("[v153] EXPORT PLAYBACK OK")
    print("[v153] source :", best)
    print("[v153] export :", out)
    print("[v153] json   :", info_path)
    print("")
...
def v118_export_playback_wav(self, event=None):
    ...
    try:
        ...
        if btn is not None:
            print("[v153] déclenche Loop / Space pour générer le playback WAV...")
            btn.invoke()
        else:
            print("[v153] bouton Loop / Space introuvable, copie du dernier WAV playback existant.")
    except Exception as exc:
        print("[v153] invoke Loop / Space impossible :", exc)
...
def v121_find_best_long_wav(since):
    ...
    print("")
    print("[v153] WAV candidats longs/propre à raccourcir :")
    for row in candidates[:20]:
        age = time.time() - row["mtime"]
        print(
            f"[v153] score={row['score']:7d} "
...
def v121_export_trimmed_long(self, event=None):
    ...
    try:
        ...
        if btn is not None:
            print("[v153] déclenche Loop / Space pour générer le WAV long/propre...")
            btn.invoke()
    except Exception as exc:
        print("[v153] Loop / Space impossible :", exc)
    ...
    def do_export():
        try:
            ...
            try:
                ...
                print("[v153] ffmpeg trim impossible, fallback wave :", exc)
                ok = False
            ...
            print("")
            print("[v153] EXPORT TRIM OK")
            print("[v153] source :", src_wav)
            print("[v153] source durée :", round(src_dur, 6), "s")
            print("[v153] target durée :", round(target_seconds, 6), "s")
            print("[v153] export :", out)
            print("[v153] export durée :", round(float(out_dur or 0), 6), "s")
            print("[v153] json :", info_path)
            print("")
...
def v124_export_trimmed_long_no_json(self, event=None):
    ...
    def do_export():
        try:
            ...
            print("")
            print("[v153] EXPORT WAV ONLY OK")
            print("[v153] source :", src_wav)
            print("[v153] source durée :", round(src_dur, 6), "s")
            print("[v153] target durée :", round(target_seconds, 6), "s")
            print("[v153] export :", out)
            print("[v153] export durée :", round(float(out_dur or 0), 6), "s")
            print("[v153] aucun JSON exporté")
            print("")
...
def v125_select_lane(self, lane):
    ...
    print("")
    print(f"[v153] Slice {lane + 1} sélectionnée")
    print(f"[v153] notes sélectionnées : {ids}")
    print("")
...
def v125_redraw_after_move(self):
    try:
        ...
        print("[v153] draw impossible :", exc)
...
def v125_build_ui(self):
    ...
    if canvas is not None:
        try:
            ...
            print("[v153] bind canvas click impossible :", exc)
    ...
    try:
        ...
        print("[v153] bind flèches impossible :", exc)
...
def v126_select_slice(self, zero_index, slice_number, reason=""):
    ...
    print("")
    print("[v153] SÉLECTION SLICE")
    print("[v153] reason:", reason)
    print("[v153] Slice affichée:", slice_number)
    print("[v153] zero_index:", zero_index)
    print("[v153] pair_candidates:", pair_candidates)
    print("[v153] ids sélectionnés:", ids)
...
def v126_build_ui(self):
    ...
    if canvas is not None:
        try:
            ...
            print("[v153] bind canvas impossible :", exc)
...
def v127_select_slice(self, lane, displayed, reason=""):
    ...
    print("")
    print("[v153] SÉLECTION SLICE ALIGNÉE")
    print("[v153] reason:", reason)
    print("[v153] label affiché:", displayed)
    print("[v153] lane réelle:", lane)
    print("[v153] labels zero-based:", v127_labels_are_zero_based(self))
    print("[v153] pair_candidates:", pair_candidates)
    print("[v153] ids sélectionnés:", ids)
    for row in debug_rows:
        print("[v153] note:", row)
...
def v128_select_lane(self, lane):
    ...
    print("")
    print("[v153] MULTI-SÉLECTION LIGNE")
    print("[v153] slice/lane:", lane)
    print("[v153] ids:", ids)
    for row in debug:
        print("[v153] note:", row)
    print("")
...
def v129_draw_green_multiselect(self):
    ...
    print(f"[v153] contours verts dessinés : {drawn}")
...
def v129_draw(self, *args, **kwargs):
    ...
    try:
        ...
        print("[v153] contour vert impossible :", exc)
...
def v130_audition_after_move(self, notes):
    ...
    def do_play():
        ...
        for name in method_names:
            ...
            try:
                fn()
                print(f"[v153] audition via {name}()")
                played = True
                break
            except TypeError:
                try:
                    fn(None)
                    print(f"[v153] audition via {name}(None)")
                    played = True
                    break
                except TypeError:
                    try:
                        fn(note)
                        print(f"[v153] audition via {name}(note)")
                        played = True
                        break
                    except Exception:
                        pass
        ...
        if not played:
            ...
            if btn is not None:
                try:
                    btn.invoke()
                    print("[v153] audition via bouton Play slice")
                    played = True
                except Exception as exc:
                    print("[v153] Play slice invoke impossible :", exc)

        if not played:
            print("[v153] audition introuvable : aucune méthode Play slice trouvée")
...
def v131_generate_candidate(self, event=None, reject_mode=False):
    ...
    print("")
    print("[v153] GENERATE CANDIDATE STRICT")
    print("[v153] break:", current)
    print("[v153] sequence: kick hat snare hat | hat kick snare hat")
    print("[v153] répété sur 64 cases")
    for role in ["kick", "snare", "hat"]:
        print(f"[v153] {role:6s}: pair={chosen[role]['pair']} source={chosen[role]['source']}")
    print("")
...
def v131_build_ui(self):
    ...
    try:
        ...
        print("[v153] repatch boutons impossible :", exc)
    ...
    for name in [
        ...
        if callable(fn):
            try:
                ...
                print(f"[v153] {name} erreur :", exc)

    v144_status(self, "Export introuvable.")
    print("[v153] ERREUR : aucun moteur Export WAV trouvé.")
    return "break"
...
def v144_install_inline_toolbar(self):
    ...
    if row is None:
        print("[v153] top row introuvable")
        return
    ...
    print("[v153] toolbar intégrée dans la ligne du haut.")
...
def v144_hide_old_ui(self):
    ...
    print(f"[v153] vieux widgets cachés : {hidden}")
...
def v144_build_ui(self):
    ...
    try:
        ...
        print("[v153] install toolbar impossible :", exc)

    try:
        v144_hide_old_ui(self)
    except Exception as exc:
        print("[v153] hide old UI impossible :", exc)
...
def v148_generate_syncopated(self, event=None):
    ...
    print("")
    print("[v153] SYNCOPÉ")
    print("[v153] template 0-15 :", t1["name"])
    print("[v153] template 16-31:", t2["name"])
    print("[v153] pairs:", pairs)
    print("[v153] hits first32:", [(s, r, a) for s, r, l, a in first32])
    print("[v153] second32 miroir strict")
    print("")
...
def v148_replace_training_panel(self):
    ...
    if panel is None:
        print("[v153] v81_training_panel introuvable")
        return
    ...
    print("[v153] toolbar installée avec bouton Syncopé.")
...
def v148_remove_duplicate_inline_buttons(self):
    ...
    if removed:
        print(f"[v153] doublons inline supprimés : {removed}")
...
def v148_build_ui(self):
    ...
    try:
        ...
        print("[v153] toolbar impossible :", exc)
...
def v149_generate_ascii_syncopated(self, event=None):
    ...
    print("")
    print("[v153] ASCII SYNCOPÉ")
    print("[v153] name      :", name)
    print("[v153] legend    : K kick | k kick ghost | S snare | s ghost snare | h hat | . silence")
    print("[v153] indices   : |01234567|89012345|67890123|45678901|")
    print("[v153] pattern   :", v149_pretty_ascii(ascii_pattern))
    print("[v153] pairs     :", pairs)
    print("[v153] first32   :", [(s, r, ch) for s, r, length, accent, ch in hits_first32])
    print("[v153] second32  : miroir strict +32")
    print("")
...
def v149_repatch_sync_button(self):
    ...
    if patched:
        print(f"[v153] bouton Syncopé patché vers ASCII : {patched}")
...
def v149_build_ui(self):
    ...
    try:
        ...
        print("[v153] patch bouton Syncopé impossible :", exc)
...
def v150_load_pair_records(self, break_name):
    ...
    if pairblocks_path is not None:
        try:
            ...
            print("[v153] lecture pairblocks impossible :", exc)
...
def v150_detect_roles_for_break(self, break_name):
    ...
    if wav_path is not None:
        try:
            ...
            print("[v153] lecture wav impossible :", wav_path, exc)
            wav_path = None
...
def v150_detect_current_break(self, event=None):
    ...
    try:
        ...
        print("[v153] ERREUR reconnaissance :", exc)
    ...
    print("")
    print("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
    print("[v153] RECONNAISSANCE SLICES")
    print("[v153] break      :", break_name)
    print("[v153] pairblocks :", result.get("pairblocks_path"))
    print("[v153] wav        :", result.get("wav_path"))

    for role in ["kick", "snare", "hat", "open_hat", "crash", "tom", "perc", "other"]:
        top = result.get("roles", {}).get(role, [])[:8]
        print(f"[v153] {role:8s} :", [(x["pair"], round(float(x.get("score", 0)), 3)) for x in top])

    print("[v153] cache écrit : dataset/learning/slice_roles_detected_v150.json")
    print("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
    print("")
...
def v131_pick_role_pairs(self):
    try:
        ...
        print("[v153] fallback v131_pick_role_pairs :", exc)
...
def v149_pick_role_pairs(self):
    try:
        ...
        print("[v153] fallback v149_pick_role_pairs :", exc)
...
def v150_generate_ascii_syncopated(self, event=None):
    ...
    try:
        ...
        print("[v153] auto-détection avant Syncopé impossible :", exc)
...
def v150_new_groove(self, event=None):
    try:
        ...
        print("[v153] auto-détection avant Nouveau groove impossible :", exc)
...
def v150_replace_toolbar(self):
    ...
    print("[v153] toolbar installée avec Reconnaître slices.")
...
def v151_auto_prepare_roles(self, force=False, reason="auto"):
    ...
    if not force and v151_cache_has_break(break_name):
        ...
        print(f"[v153] rôles déjà prêts pour {break_name} ({reason})")
        v151_status(self, f"v151 : rôles déjà prêts — {break_name}")
        return "ready"

    if getattr(self, "_v151_auto_detect_busy", False):
        print(f"[v153] détection déjà en cours pour {break_name}")
        return "busy"
    ...
    try:
        print("")
        print("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
        print("[v153] AUTO-RECONNAISSANCE")
        print("[v153] reason :", reason)
        print("[v153] break  :", break_name)
        print("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
        print("")
        ...
        print("[v153] auto-reconnaissance impossible :", exc)
...
def v151_patch_break_selector(self):
    ...
    if patched:
        print(f"[v153] sélecteurs de break patchés auto-rôles : {patched}")
...
def v151_patch_charger_break(self):
    ...
    for widget in v151_walk(self.root):
        ...
        def wrapped(cmd=old_cmd):
            try:
                ...
                print("[v153] ancienne commande Charger break erreur :", exc)
    ...
    if patched:
        print(f"[v153] bouton Charger break patché auto-rôles : {patched}")
...
def v151_repatch_main_buttons(self):
    ...
    if patched:
        print(f"[v153] boutons principaux repatchés auto-rôles : {patched}")
...
def v151_build_ui(self):
    ...
    try:
        ...
        print("[v153] patch UI auto-rôles impossible :", exc)
...
def v152_detected_pairs(self):
    ...
    try:
        ...
        print("[v153] fallback rôles détectés :", exc)
...
def v152_generate_variations(self, event=None):
    ...
    print("")
    print("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
    print("[v153] VARIATION KICK / SNARE / HAT")
    print("[v153] name  :", template["name"])
    print("[v153] pairs :", pairs)
    print("[v153] KICK  :", v152_ascii_line(kick, "K"))
    print("[v153] SNARE :", v152_ascii_line(snare, "S"))
    print("[v153] HAT   :", v152_ascii_line(hat, "h"))
    print("[v153] second32 : miroir strict +32")
    print("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
    print("")
...
def v152_repatch_toolbar(self):
    ...
    if patched:
        print(f"[v153] bouton Variations patché : {patched}")
...
def v152_build_ui(self):
    ...
    try:
        ...
        print("[v153] patch toolbar impossible :", exc)
...
def v153_randomize_ksh(self, event=None):
    ...
    print("")
    print("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
    print("[v153] RANDOM KICK / SNARE / HAT")
    print("[v153] pairs :", pairs)
    print("[v153] KICK  :", v153_ascii_line(kick, "K"))
    print("[v153] SNARE :", v153_ascii_line(snare, "S"))
    print("[v153] HAT   :", v153_ascii_line(hat, "h"))
    print("[v153] règles: cases paires seulement, longueur 2, une note par case")
    print("[v153] second32 : miroir strict +32")
    print("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
    print("")
...
def v153_repatch_toolbar(self):
    ...
    if patched:
        print(f"[v153] bouton Random K/S/H patché : {patched}")
...
def v153_build_ui(self):
    ...
    try:
        self.root.title("BreakbeatAI v153 — Random K/S/H")
    except Exception:
        pass
