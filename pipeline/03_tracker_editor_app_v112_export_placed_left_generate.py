...
"""
BreakbeatAI Tracker Editor v112 — slice index
...
def warp_audio_to_length_external(audio, target_len, cache_key=None):
    ...
    if rubberband:
        try:
            ...
            print(f"[v112] rubberband échec, fallback ffmpeg/no-speed : {exc}")
    ...
    if ffmpeg:
        try:
            ...
            print(f"[v112] ffmpeg atempo échec, fallback no-speed : {exc}")
...
class SliceIndexTracker:
    def __init__(self, root, pair_json):
        ...
        self.root.title("BreakbeatAI Tracker Editor v112 — slice index")
    ...
    def detect_external_player(self):
        for name in ["pw-play", "paplay", "aplay", "ffplay"]:
            ...
            if path:
                print(f"[v112] Backend audio : {name} -> {path}")
                return name

        print("[v112] Aucun backend système trouvé, fallback sounddevice.")
        return None
    ...
    def build_initial_pattern(self, randomize=False):
        ...
        print(
            f"[v112] Génération pattern | break={self.project['safe']} | "
        ...
        for slot in range(max_items):
            ...
            print(f"[v112] slot {slot:02d} | step {x_step:02d} -> slice {pair:02d}")
    ...
    def build_ui(self):
        ...
        title = tk.Label(
            main,
            text="BreakbeatAI v112 — export placed left generate",
            bg="#111018",
            fg="#ff7acc",
            font=("Sans", 20, "bold"),
        )
    ...
    def record_correction(self, event_type, before=None, after=None):
        ...
        print(f"[v112] correction sauvegardée : {event_type}")
    ...
    def play_audition_array(self, audio, label="audition"):
        ...
        audition_wav = OUT_DIR / f"{self.project['safe']}_v112_audition.wav"
        ...
        if self.external_player:
            try:
                ...
                print(f"[v112] Erreur audition {self.external_player} : {exc}")
    ...
    def on_close(self):
        ...
        """
        print("[v112] Fermeture : arrêt audio + destruction fenêtre")
    ...
    def play_audio_array(self, audio, label="audio"):
        ...
        audio = normalize(audio)
        live_wav = OUT_DIR / f"{self.project['safe']}_v112_live.wav"
        ...
        if self.external_player:
            try:
                ...
                print(f"[v112] Erreur backend {self.external_player} : {exc}")
    ...
    def render_preview_file(self):
        OUT_DIR.mkdir(parents=True, exist_ok=True)
        wav = OUT_DIR / f"{self.project['safe']}_tracker_app_v112_preview.wav"
    ...
    def write_latest_pattern(self, reason="correction"):
        ...
        path = LATEST_DIR / f"{self.project['safe']}_latest_slice_index_v112.json"

        data = {
            "version": "breakbeatai_latest_slice_index_v112",
            "reason": reason,
    ...
    def save(self):
        ...
        tracker_path = OUT_DIR / f"{self.project['safe']}_tracker_app_edit_v112.json"
        ...
        stamp = file_stamp()
        validated_path = VALIDATED_DIR / f"{self.project['safe']}_validated_slice_index_loop64_v112_{stamp}.json"

        validated = {
            "version": "breakbeatai_validated_slice_index_loop64_v112",
...
def v44_record_correction(self, event_type, before=None, after=None):
    ...
    print(f"[v112] correction sauvegardée : {event_type}")
    print(f"[v112] correction path : {corrections_path}")
    print(f"[v112] latest path : {latest_path}")
...
def v45_move_selected(self, dx, dy):
    ...
    if dy != 0 and before != after:
        try:
            ...
            print(f"[v112] audition après flèche haut/bas impossible : {exc}")
...
def v46_warp_loop_pitch_preserve(audio, target_len, cache_key):
    ...
    if rubberband:
        try:
            ...
            print(f"[v112] rubberband global warp échec : {exc}")
    ...
    if ffmpeg:
        try:
            ...
            print(f"[v112] ffmpeg global warp échec : {exc}")

    print("[v112] Aucun warp pitch-preserve dispo, fallback fit no-speed.")
    return v46_fit_audio_no_speed(audio, target_len)
...
def v46_build_initial_pattern(self, randomize=False):
    ...
    print(
        f"[v112] M8 pattern | break={self.project['safe']} | "
...
def v46_get_warped_loop(self):
    ...
    print(
        f"[v112] global warp OK | source_len={loop_end-loop_start} | "
...
def v46_build_ui(self):
    ...
    try:
        ...
        print(f"[v112] UI extension impossible : {exc}")
...
def v47_play_audio_array(self, audio, label="audio"):
    ...
    if len(audio) == 0 or v47_is_silent(audio):
        ...
        print(f"[v112] lecture annulée : audio silencieux ({label})")
        return 0
...
def v47_toggle_loop(self):
    ...
    if not getattr(self, "pattern", None):
        ...
        print("[v112] loop annulée : pattern vide")
        return "break"
...
def v47_delete_selected(self):
    ...
    if not getattr(self, "pattern", None):
        ...
        print("[v112] pattern vide après suppression : audio coupé")
...
def v48_build_initial_pattern(self, randomize=False):
    ...
    print("[v112] pattern initial forcé VIDE : aucun son ne doit jouer.")
    return []
...
def v48_load_first_slices(self):
    ...
    print(f"[v112] Load 16 slices : {len(self.pattern)} items")


def v48_debug_pattern(self, label="render"):
    print("")
    print(f"[v112] DEBUG PATTERN avant {label}")
    print(f"[v112] items = {len(getattr(self, 'pattern', []) or [])}")

    if not getattr(self, "pattern", None):
        print("[v112] pattern vide confirmé")
        print("")
        return
    ...
    for item in self.pattern:
        ...
        print(
            f"[v112] id={item_id} | step {x:02d}/{end:02d} | "
...
def v48_render_audio_with_timeline(self):
    ...
    if not getattr(self, "pattern", None):
        loop_samples = self.get_loop_samples()
        print(f"[v112] rendu silence total : pattern vide | samples={loop_samples}")
        return np.zeros(loop_samples, dtype=np.float32), []
    ...
    print(f"[v112] rendu audio peak={peak:.8f} | timeline_events={len(timeline) if timeline is not None else 'None'}")

    if timeline:
        print("[v112] timeline events:")
        for ev in timeline:
            print(f"[v112]   {ev}")
...
def v48_toggle_loop(self):
    if not getattr(self, "pattern", None):
        ...
        print("[v112] Space ignoré : pattern vide.")
        return "break"
...
def v48_build_ui(self):
    ...
    try:
        ...
        print(f"[v112] ajout UI impossible : {exc}")
...
def v49_add_item_at_xy(self, x, y):
    ...
    try:
        ...
        print(f"[v112] record_correction add impossible : {exc}")
    ...
    try:
        ...
        print(f"[v112] audition après ajout impossible : {exc}")

    print(f"[v112] add | id={item['id']} | step={x_step} | pair={pair} | lane={lane}")
...
def v49_duplicate_selected_forward(self, event=None):
    ...
    if item is None:
        ...
        print("[v112] duplicate annulé : aucune note sélectionnée")
        return "break"
    ...
    if new_x > max_x:
        ...
        print("[v112] duplicate annulé : fin de grille")
        return "break"
    ...
    try:
        ...
        print(f"[v112] record_correction duplicate impossible : {exc}")
    ...
    try:
        ...
        print(f"[v112] audition après duplicate impossible : {exc}")

    print(
        f"[v112] duplicate | old_id={item.get('id')} | new_id={new_item.get('id')} | "
...
def v49_build_ui(self):
    ...
    try:
        ...
        print(f"[v112] bind canvas click impossible : {exc}")
    ...
    try:
        ...
        print(f"[v112] bind keys impossible : {exc}")
...
def v50_load_style_model():
    ...
    try:
        ...
        print(f"[v112] impossible de charger le modèle IA : {exc}")
        return None
...
def v50_generate_ai_pattern(self, event=None):
    ...
    if model is None:
        ...
        print("[v112] modèle absent :", STYLE_MODEL_PATH)
        return "break"
    ...
    print("")
    print(f"[v112] GENERATE IA | safe={safe} | pairs={pair_values} | slots={max_slots}")
    ...
    for slot in range(max_slots):
        ...
        print(
            f"[v112] slot {slot:02d} | step {x_step:02d} | "
    ...
    try:
        ...
        print(f"[v112] write_latest_pattern impossible : {exc}")
...
def v50_build_ui(self):
    ...
    try:
        ...
        print(f"[v112] UI IA impossible : {exc}")
...
def v51_mouse_down(self, event):
    ...
    if item is not None:
        ...
        print(
            f"[v112] mouse down note | id={item.get('id')} | "
            f"step={item.get('x_step')} | pair={item.get('pair')}"
        )
    else:
        print("[v112] mouse down empty")
...
def v51_mouse_up(self, event):
    ...
    if state.get("empty"):
        ...
        print("[v112] drag depuis vide ignoré")
        return "break"
    ...
    if before != after:
        try:
            ...
            print(f"[v112] record_correction drag impossible : {exc}")
        ...
        print(
            f"[v112] drag move | id={after.get('id')} | "
            f"step {before.get('x_step')} -> {after.get('x_step')} | "
        ...
        try:
            ...
            print(f"[v112] audition après drag impossible : {exc}")
...
def v51_build_ui(self):
    ...
    try:
        ...
        print(f"[v112] bind souris impossible : {exc}")
    ...
    try:
        ...
        print(f"[v112] bind Suppr impossible : {exc}")
...
def v52_normalize_loaded_notes(self, notes):
    ...
    for raw in sorted(notes, key=lambda n: (int(float(n.get("x_step", 0))), int(float(n.get("pair", 0))))):
        ...
        if pair not in valid_pairs:
            print(f"[v112] slice ignorée car absente du break actuel : pair={pair}")
            continue
...
def v52_load_saved_beat(self, event=None):
    ...
    if not saves:
        ...
        print("[v112] aucune sauvegarde trouvée")
        return "break"
    ...
    if not pattern:
        ...
        print("[v112] sauvegarde incompatible :", chosen["path"])
        return "break"
    ...
    print("")
    print("[v112] BEAT RESTAURÉ")
    print("[v112] fichier :", chosen["path"])
    print("[v112] clé :", chosen["key"])
    print("[v112] notes :", len(pattern))
    print("")
...
def v52_list_saved_beats(self, event=None):
    ...
    print("")
    print("[v112] SAUVEGARDES TROUVÉES")
    for i, save in enumerate(saves[:30]):
        print(
            f"[v112] {i:02d} | notes={len(save['notes']):02d} | "
...
def v52_build_ui(self):
    ...
    try:
        ...
        print(f"[v112] UI load saved impossible : {exc}")
...
def v53_run_training(self):
    ...
    if not trainer.exists():
        ...
        print("[v112] trainer introuvable :", trainer)
        return False

    print("")
    print("[v112] AUTO-TRAIN IA après Save...")
    print("")
    ...
    try:
        ...
        if result.returncode != 0:
            ...
            print("[v112] auto-train échec code :", result.returncode)
            return False

        v53_status(self, "Save OK + IA entraînée automatiquement.")
        print("[v112] AUTO-TRAIN OK")
        return True

    except Exception as exc:
        v53_status(self, f"Save OK, mais auto-train impossible : {exc}")
        print("[v112] exception auto-train :", exc)
        return False
...
def v53_save(self):
    ...
    try:
        ...
        print(f"[v112] auto-train après save impossible : {exc}")
...
def v53_build_ui(self):
    ...
    try:
        ...
        print(f"[v112] UI auto-train impossible : {exc}")
...
def v55_mouse_up(self, event):
    ...
    if not state.get("moved"):
        ...
        try:
            ...
            print(f"[v112] audition clic note impossible : {exc}")
        ...
        print(
            f"[v112] click play/select | id={item.get('id')} | "
            f"step={item.get('x_step')} | pair={item.get('pair')}"
        )
...
def v58_add_item_at_xy(self, x, y):
    ...
    print(
        f"[v112] add | id={item['id']} | step={x_step} | "
...
def v58_toggle_selected_length(self, event=None):
    ...
    try:
        ...
        print(f"[v112] record_correction length impossible : {exc}")

    v58_refresh_after_edit(self, "toggle_note_length")

    v58_status(self, f"Longueur note : {old_len} -> {new_len} case(s).")
    print(f"[v112] length toggle | id={item.get('id')} | {old_len} -> {new_len}")
...
def v58_duplicate_selected_forward(self, event=None):
    ...
    try:
        ...
        print(f"[v112] audition duplicate impossible : {exc}")
...
def v58_build_ui(self):
    ...
    try:
        ...
        print(f"[v112] UI demi-grille impossible : {exc}")
    ...
    try:
        ...
        print(f"[v112] bind demi-grille impossible : {exc}")
...
def v59_rank_current_slices(self):
    ...
    for pair in self.pair_values:
        ...
        try:
            ...
            print(f"[v112] analyse pair {pair} impossible : {exc}")
            role = "other"
    ...
    print("")
    print("[v112] TOP ROLES CURRENT BREAK")
    for r in ("kick", "snare", "hat"):
        print(f"[v112] {r:5s}:", [(x["pair"], round(x["score"], 3)) for x in ranks[r][:6]])
    print("")
...
def v59_load_role_model():
    ...
    try:
        ...
        print(f"[v112] lecture modèle rôle impossible : {exc}")
        return None
...
def v59_generate_role_aware(self, event=None):
    ...
    if model is None:
        ...
        print("[v112] modèle rôle absent, fallback musical snare 6.")
        model = {
            "breaks": {},
            "global": {},
            "patterns_used": 0,
    ...
    print("")
    print("[v112] ROLE-AWARE GENERATION")
    print("[v112] model patterns_used:", model.get("patterns_used", 0))
    for item in pattern:
        print(
            f"[v112] step {item['x_step']:02d}/{item['x_step'] + item['length'] - 1:02d} | "
...
def v59_run_role_training(self, event=None):
    ...
    try:
        ...
        print(f"[v112] training rôle impossible : {exc}")
...
def v59_save(self):
    ...
    try:
        ...
        print(f"[v112] auto-training rôle après save impossible : {exc}")
...
def v59_build_ui(self):
    ...
    try:
        ...
        print(f"[v112] UI role-aware impossible : {exc}")
...
def v62_add_item_at_xy(self, x, y):
    ...
    print(f"[v112] add | id={item['id']} | step={x_step} | len={length} | pair={pair}")
...
def v62_mouse_down(self, event):
    ...
    if item is not None:
        ...
        print(
            f"[v112] mouse down {state['mode']} | id={item.get('id')} | "
            f"step={item.get('x_step')} | len={item.get('length')} | pair={item.get('pair')}"
        )
    else:
        print("[v112] mouse down empty")
    ...
    if mode == "resize":
        if not state.get("moved"):
            ...
            try:
                ...
                print(f"[v112] audition clic resize impossible : {exc}")
        ...
        if before != after:
            try:
                ...
                print(f"[v112] record_correction resize impossible : {exc}")
            ...
            try:
                ...
                print(f"[v112] audition après resize impossible : {exc}")
    ...
    if mode == "move":
        ...
        if before != after:
            try:
                ...
                print(f"[v112] record_correction move impossible : {exc}")
            ...
            try:
                ...
                print(f"[v112] audition après move impossible : {exc}")
...
def v62_toggle_selected_length(self, event=None):
    ...
    try:
        ...
        print(f"[v112] record_correction toggle length impossible : {exc}")

    v62_refresh_after_edit(self, "toggle_note_length_v62")
    v62_status(self, f"Longueur note : {old_len} → {new_len} case(s).")

    print(f"[v112] toggle length | id={item.get('id')} | {old_len} -> {new_len}")
...
def v62_draw(self):
    ...
    try:
        ...
        print(f"[v112] draw handles impossible : {exc}")
...
def v62_build_ui(self):
    ...
    try:
        ...
        print(f"[v112] bind resize impossible : {exc}")
    ...
    try:
        ...
        print(f"[v112] UI resize impossible : {exc}")
...
def v63_load_model():
    ...
    try:
        ...
        print(f"[v112] lecture modèle strict impossible : {exc}")
        return None
...
def v63_generate_safe_role_ai(self, event=None):
    ...
    print("")
    print("[v112] SAFE ROLE AI GENERATED")
    print("[v112] model strict:", "yes" if model else "no")
    print("[v112] snare main/alt:", main_snare, alt_snare)
    print("[v112] kick  main/alt:", main_kick, alt_kick)
    print("[v112] hat   main/alt:", main_hat, alt_hat)
    print("[v112] pattern:")
    for item in pattern:
        print(
            f"[v112] step {item['x_step']:02d}/{item['x_step'] + item['length'] - 1:02d} | "
...
def v63_run_strict_training(self, event=None):
    ...
    print("")
    print("[v112] TRAIN STRICT ROLE MODEL...")
    print("")
    ...
    try:
        ...
        print(f"[v112] training strict impossible : {exc}")
...
def v63_build_ui(self):
    ...
    try:
        ...
        print(f"[v112] UI safe impossible : {exc}")
...
def v65_rank_slices(self):
    ...
    for pair in self.pair_values:
        ...
        try:
            ...
            print(f"[v112] analyse slice {pair} impossible : {exc}")
            scores = {
                "kick": 0.0,
                "snare": 0.0,
                "hat": 0.0,
    ...
    print("")
    print("[v112] TOP SLICES PAR ROLE — break courant")
    for role in ("kick", "snare", "hat", "ghost_snare"):
        print(f"[v112] {role:12s}:", [(x["pair"], round(x["score"], 3)) for x in ranks[role][:8]])
    print("")
...
def v65_generate_from_role_template(self, event=None):
    ...
    try:
        ...
        print(f"[v112] template introuvable : {exc}")
        return "break"
    ...
    print("")
    print("[v112] CROSS-SONG ROLE TEMPLATE GENERATED")
    print("[v112] template:", V65_TEMPLATE_PATH)
    print("[v112] temperature:", temperature, "| hats:", hat_density, "| ghosts:", ghost_density)
    print("[v112] pattern:")
    for item in pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v112] step {item['x_step']:02d}/{end:02d} | "
...
def v65_build_ui(self):
    ...
    try:
        ...
        print(f"[v112] UI impossible : {exc}")
...
def v67_train_strict_if_available(self):
    ...
    if trainer is None:
        print("[v112] aucun trainer trouvé.")
        return False

    print("")
    print("[v112] TRAIN après validation :", trainer)
    print("")
    ...
    try:
        ...
        print(f"[v112] training impossible : {exc}")
        return False
...
def v67_generate_candidate(self, event=None):
    ...
    print("")
    print("[v112] CANDIDATE GENERATED")
    print("[v112] break:", self.v67_candidate_safe)
    print("[v112] notes:", len(candidate))
    print("[v112] Maintenant : corrige puis clique Good après modifs, ou Reject.")
    print("")
...
def v67_reject_candidate(self, event=None):
    ...
    print("")
    print("[v112] REJECTED")
    print("[v112] saved:", rejected_path)
    print("")
...
def v67_accept_corrected(self, event=None):
    ...
    print("")
    print("[v112] ACCEPTED GOOD AFTER EDIT")
    print("[v112] saved:", validated_path)
    print("[v112] trained:", trained)
    print("")
...
def v67_accept_without_training(self, event=None):
    ...
    print("[v112] accepted no-train:", validated_path)
...
def v67_build_ui(self):
    ...
    try:
        ...
        print(f"[v112] UI feedback impossible : {exc}")
...
def v68_generate_candidate(self, event=None):
    ...
    print("[v112] Length policy appliquée : 2 cases par défaut, 1 case rare.")
...
def v69_force_pattern_hard_2_grid(self, reason="v69_force_hard_2_grid"):
    ...
    print("")
    print("[v112] HARD 2 GRID APPLIQUÉ")
    for item in self.pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v112] step {item['x_step']:02d}/{end:02d} | "
...
def v70_duplicate_selected_forward(self, event=None):
    ...
    print("")
    print("[v112] CTRL+D DUPLICATE +2")
    print(f"[v112] old_id={item.get('id')} new_id={new_item.get('id')}")
    print(f"[v112] step {old_x}/{old_x + length - 1} -> {new_x}/{new_x + length - 1}")
    print(f"[v112] pair={new_item.get('pair')} len={length}")
    print("")
...
def v70_build_ui(self):
    ...
    try:
        ...
        print(f"[v112] bind Ctrl+D impossible : {exc}")
...
def v71_rank_slices(self):
    ...
    print("")
    print("[v112] PRE-LEARNING TOP SLICES")
    for role in ("kick", "snare", "hat", "ghost_snare"):
        print(f"[v112] {role:12s}:", [(x["pair"], round(x["score"], 3)) for x in ranks[role][:8]])
    print("")
...
def v71_prelearn_roles(self, force=False):
    ...
    print("")
    print("[v112] PRELEARN ROLES POUR", safe)
    for role in ("kick", "snare", "hat", "ghost_snare", "bad"):
        print(f"[v112] {role:12s}:", roles.get(role, []))
    print("")
...
def v71_generate_full_candidate(self, event=None):
    ...
    print("")
    print("[v112] FULL CANDIDATE GENERATED")
    print("[v112] break:", v71_safe(self))
    print("[v112] roles:", roles)
    print("[v112] pattern:")
    for item in pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
...
def v72_open_break(self, safe=None):
    ...
    print("")
    print("[v112] OPEN BREAK")
    print("[v112] current:", current)
    print("[v112] next   :", safe)
    print("[v112] cmd    :", " ".join(cmd))
    print("")
...
def v72_build_ui(self):
    ...
    try:
        ...
        print(f"[v112] UI break switcher impossible : {exc}")
...
def v73_sanitize_tracker_pattern(self, reason="v73_tracker_strict_no_overlap"):
    ...
    print("")
    print("[v112] TRACKER STRICT NO OVERLAP")
    print("[v112] kept:", len(accepted), "| rejected overlaps:", len(rejected))

    for item in accepted:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v112] KEEP step {item['x_step']:02d}/{end:02d} | "
    ...
    if rejected:
        ...
        for item in rejected:
            ...
            print(
                f"[v112] DROP step {item['x_step']:02d}/{end:02d} | "
                f"role={item.get('v73_role_key')} | pair={item.get('pair')}"
            )
...
def v81_launch_break(self, source):
    ...
    print("")
    print("[v112] CHARGER BREAK")
    print("[v112] courant :", current)
    print("[v112] nouveau :", source)
    print("[v112] cmd     :", " ".join(cmd))
    print("")
...
def v81_patch_charger_break_buttons(self):
    ...
    print(f"[v112] boutons Charger break patchés : {patched}")
...
def v81_generate_candidate(self, event=None):
    ...
    print("")
    print("[v112] GENERATE CANDIDATE")
    print("[v112] break chargé :", current)
    print("[v112] générateur   :", getattr(fn, "__name__", None))
    print("")
...
def v81_run_training(self):
    ...
    if script is None:
        print("[v112] Aucun script training trouvé.")
...
def v81_save_feedback(self, verdict, train_now=False):
    ...
    print("")
    print("[v112] FEEDBACK SAUVÉ")
    print("[v112] break  :", current)
    print("[v112] verdict:", verdict)
    print("[v112] train  :", train_now)
    print("[v112] fichier:", out_path)
    print("")
...
def v81_build_ui(self):
    ...
    try:
        self.root.title(f"BreakbeatAI v112 — export placed left generate")
    except Exception:
        pass
    ...
    try:
        ...
        print(f"[v112] patch Charger break impossible : {exc}")

    try:
        v81_install_training_toolbar(self)
    except Exception as exc:
        print(f"[v112] toolbar impossible : {exc}")
...
def v82_auto_role_pools(self):
    ...
    print("")
    print("[v112] ROLE POOLS")
    for r in ["kick", "snare", "hat", "ghost_snare", "bad"]:
        print(f"[v112] {r:12s}:", roles.get(r, []))
    print("")
...
def v82_generate_candidate(self, event=None):
    ...
    print("")
    print("[v112] NEW CANDIDATE")
    print("[v112] break :", current)
    print("[v112] nonce :", nonce)
    print("[v112] rejected known:", len(rejected))
    print("[v112] fingerprint rejeté ?: ", chosen_fp in rejected)
    for item in self.pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v112] step {item['x_step']:02d}/{end:02d} "
...
def v82_patch_training_buttons(self):
    ...
    print(f"[v112] boutons training patchés : {patched}")
...
def v82_build_ui(self):
    ...
    try:
        ...
        print(f"[v112] patch boutons impossible : {exc}")
...
def v110_current_break(self):
    """
    v111 : résolution robuste du break réellement sélectionné.

    Le problème avant :
    self.safe / self.project['safe'] pouvait rester sur Camo,
    même si le menu affichait Champion_Break_-_7A ou Change_Break_-_6A.

    Priorité :
    1. Valeur visible des Combobox / champs texte
    2. textvariable Tk
    3. ligne de statut "Break chargé : ..."
    4. attributs internes
    """
    stems = v110_pairblock_stems()
    candidates = []

    def add_candidate(value, source, score):
        try:
            value = str(value).strip()
        except Exception:
            return
        ...
        # Nettoie les préfixes visuels possibles.
        ...
        # Match partiel, utile si l'UI tronque un peu.
    ...
    # 1) Widgets visibles : Combobox / Entry / Spinbox en priorité.
    for widget in v110_all_widgets(self.root):
        ...
        if value is not None:
            ...
            if "combo" in cls.lower():
                ...
                score = 160
            elif "spin" in cls.lower():
                score = 70

            add_candidate(value, f"widget.get:{cls}", score)

        # textvariable Tk.
        try:
            tv = widget.cget("textvariable")
            if tv:
                value = self.root.globalgetvar(tv)
                add_candidate(value, f"textvariable:{cls}", 150)
        except Exception:
            pass

        # Texte de labels / boutons, moins prioritaire.
        try:
            text = widget.cget("text")
            if isinstance(text, str):
                if "Break chargé" in text or "Training" in text:
                    add_candidate(text, f"label:{cls}", 120)
        except Exception:
            pass

    # 2) Attributs internes.
    for attr in [
        ...
        try:
            add_candidate(getattr(self, attr), f"attr:{attr}", 60)
        except Exception:
            pass
    ...
    try:
        ...
        if isinstance(project, dict):
            for key in ["current_source", "source", "safe", "name", "break", "break_name"]:
                add_candidate(project.get(key), f"project:{key}", 55)
    except Exception:
        pass
    ...
    if candidates:
        ...
        print("")
        print("[v112] BREAK COURANT RÉSOLU")
        print("[v112] source:", source)
        print("[v112] raw   :", raw)
        print("[v112] break :", stem)
        print("")

        return stem

    print("[v112] WARNING : break courant introuvable, fallback Camo.")
...
def v110_make_note(self, step, pair, role, source, first32_id, response_id, role_offset):
    ...
    return {
        ...
        "hit_slot": int(step) // 2,
        "ai_generated": True,
        "ai_model": "v111_export_real_current",
        "learned_role": str(role),
        "prior_label": str(role),
...
def v110_generate(self, event=None, reject_mode=False):
    ...
    print("")
    if reject_mode:
        print("[v112] REJECT/BAD -> variation premier 32 + autres lignes")
    else:
        print("[v112] GENERATE 64")
    print("[v112] break:", current)
    print("[v112] first32:", first32_id)
    print("[v112] response:", response_id)
    print("[v112] role_offset:", role_offset)
    for role in ["kick", "snare", "hat"]:
        lane = "?"
        try:
            ...
            pass
        print(f"[v112] {role:6s}: pair={chosen[role]['pair']} lane={lane} source={chosen[role]['source']}")
    print("")
    ...
    try:
        ...
        print(f"[v112] draw impossible : {exc}")
    ...
    try:
        ...
        print(f"[v112] refresh_panel impossible : {exc}")

    try:
        self.write_latest_pattern(reason="v111_export_real_current")
    except Exception as exc:
        print(f"[v112] write_latest_pattern impossible : {exc}")
...
def v110_render_pattern_to_audio(self):
    ...
    for item in pattern:
        ...
        if pair not in slice_map:
            skipped += 1
            print(f"[v112] skip pair introuvable : {pair}")
            continue
    ...
    info = {
        ...
        "exporter": "v111_export_real_current",
    }
...
def v110_export_wav_current(self, event=None):
    try:
        ...
        print("")
        print("[v112] EXPORT WAV OK")
        print("[v112] break:", current)
        print("[v112] wav  :", out_path)
        print("[v112] json :", info_path)
        print("[v112] duration:", round(info["duration_seconds"], 3), "s")
        print("[v112] rendered:", info["rendered_notes"], "skipped:", info["skipped_notes"])
        print("")
...
def v110_space_play(self, event=None):
    ...
    try:
        ...
        if btn is not None:
            print("[v112] SPACE -> Loop / Space")
            try:
                btn.invoke()
            except Exception as exc:
                print("[v112] invoke Loop / Space impossible :", exc)
            return "break"

        print("[v112] SPACE : bouton Loop / Space introuvable.")
...
def v110_add_export_button(self):
    """
    v111 : bouton Export WAV dans la même barre que Generate Candidate.

    On le met AVANT Generate Candidate pour éviter qu'il sorte de l'écran
    après Reject / Bad.
    """
    if getattr(self, "_v111_export_button_added", False):
        return
    ...
    generate_widget = widgets.get("generate candidate")
    ...
    # Cas pack : on le place juste AVANT Generate Candidate.
    try:
        if generate_widget is not None and generate_widget.winfo_manager() == "pack":
            btn.pack(side="left", padx=4, pady=3, before=generate_widget)
            placed = True
    except Exception as exc:
        print("[v112] pack before Generate impossible :", exc)

    # Cas grid : juste avant Generate Candidate si possible.
    if not placed:
        try:
            if generate_widget is not None and generate_widget.winfo_manager() == "grid":
                info = generate_widget.grid_info()
                row = int(info.get("row", 0))
                col = int(info.get("column", 0))

                # Décale pas les autres widgets : si col-1 est impossible, on met col+10.
                if col > 0:
                    btn.grid(row=row, column=col - 1, padx=4, pady=3, sticky="w")
                else:
                    btn.grid(row=row, column=col + 10, padx=4, pady=3, sticky="w")

                placed = True
        except Exception as exc:
            print("[v112] grid near Generate impossible :", exc)

    # Fallback : même parent, à gauche.
    if not placed:
        try:
    ...
    self.export_wav_button_v111 = btn
    self._v111_export_button_added = True

    print("[v112] Export WAV ajouté dans la barre Generate Candidate.")
...
def v110_patch_buttons(self):
    ...
    print(f"[v112] boutons patchés : Generate={gen_patched}, Reject/Bad={bad_patched}")
...
def v110_build_ui(self):
    ...
    try:
        self.root.title(f"BreakbeatAI v111 clean — {v110_current_break(self)}")
    except Exception:
        pass
    ...
    try:
        ...
        print("[v112] patch boutons impossible :", exc)

    try:
        v110_add_export_button(self)
    except Exception as exc:
        print("[v112] ajout Export WAV impossible :", exc)

    try:
        v110_install_space_bindings(self)
    except Exception as exc:
        print("[v112] bind espace impossible :", exc)
    ...
    v110_status(self, "v111 clean : Espace réparé, Export WAV en haut, pas de callbacks fantômes.")
...
# ---------------------------------------------------------------------
# v112 : bouton Export WAV placé à gauche de Generate Candidate
# ---------------------------------------------------------------------
# Le layout pack/grid ne marche pas dans cette barre.
# Donc v112 calcule la position réelle de Generate Candidate
# et place Export WAV au même Y, juste à gauche.
# ---------------------------------------------------------------------


def v112_status(self, text):
    try:
        if hasattr(self, "set_status"):
            self.set_status(text)
        elif hasattr(self, "output_label"):
...
def v112_all_widgets(widget):
    ...
    for child in children:
        out.extend(v112_all_widgets(child))

    return out


def v112_widget_text(widget):
    try:
        return str(widget.cget("text")).strip()
    except Exception:
        return ""
...
def v112_find_button(self, text):
    target = text.strip().lower()

    for widget in v112_all_widgets(self.root):
        if v112_widget_text(widget).lower() == target:
            return widget

    return None


def v112_export(self, event=None):
    # v111/v110 gardent le moteur d'export réel.
    for name in [
        "export_wav_current_v110",
        "export_wav_current_v109",
    ...
    print("[v112] ERREUR : aucune fonction Export WAV trouvée.")
    v112_status(self, "Export WAV introuvable.")
    return "break"


def v112_remove_old_export_buttons(self):
    removed = 0

    for widget in list(v112_all_widgets(self.root)):
        text = v112_widget_text(widget).lower()
    ...
    if removed:
        print(f"[v112] anciens boutons Export WAV retirés : {removed}")


def v112_place_export_left_of_generate(self):
    if getattr(self, "_v112_export_button_added", False):
        try:
            self.export_wav_button_v112.lift()
        except Exception:
            pass
        return True
    ...
    generate = v112_find_button(self, "Generate Candidate")

    if generate is None:
        print("[v112] Generate Candidate introuvable pour placer Export WAV.")
        return False

    v112_remove_old_export_buttons(self)

    btn = tk.Button(
        self.root,
        text="Export WAV",
        command=self.export_wav_v112,
        bg="#2d7a4b",
        fg="#ffffff",
        activebackground="#39a96b",
        activeforeground="#ffffff",
    ...
    try:
        ...
        # Même ligne que Generate Candidate, juste à gauche.
        x = max(8, gx - bw - 8)
        y = max(4, gy)
        ...
        self.export_wav_button_v112 = btn
        self._v112_export_button_added = True

        print("")
        print("[v112] Export WAV placé")
        print("[v112] generate x/y :", gx, gy)
        print("[v112] export   x/y :", x, y)
        print("")

        return True

    except Exception as exc:
        print("[v112] placement Export WAV impossible :", exc)
        try:
            btn.destroy()
        except Exception:
            pass
...
def v112_install_export_button_retry(self):
    def attempt(count=0):
        ok = False

        try:
            ok = v112_place_export_left_of_generate(self)
        except Exception as exc:
            print("[v112] tentative placement impossible :", exc)
    ...
    try:
        self.root.after(300, lambda: attempt(0))
    except Exception:
        attempt(0)


_old_v112_build_ui = SliceIndexTracker.build_ui


def v112_build_ui(self):
    _old_v112_build_ui(self)

    try:
        self.root.title("BreakbeatAI v112 — export placé à gauche de Generate")
    except Exception:
        pass

    try:
        v112_install_export_button_retry(self)
    except Exception as exc:
        print("[v112] install export impossible :", exc)

    try:
        self.root.bind("<Control-e>", self.export_wav_v112)
    except Exception:
        pass

    v112_status(self, "v112 : Export WAV placé à gauche de Generate Candidate. Ctrl+E aussi.")


SliceIndexTracker.export_wav_v112 = v112_export
SliceIndexTracker.build_ui = v112_build_ui
