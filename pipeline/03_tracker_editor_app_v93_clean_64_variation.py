...
"""
BreakbeatAI Tracker Editor v93 — slice index
...
def warp_audio_to_length_external(audio, target_len, cache_key=None):
    ...
    if rubberband:
        try:
            ...
            print(f"[v93] rubberband échec, fallback ffmpeg/no-speed : {exc}")
    ...
    if ffmpeg:
        try:
            ...
            print(f"[v93] ffmpeg atempo échec, fallback no-speed : {exc}")
...
class SliceIndexTracker:
    def __init__(self, root, pair_json):
        ...
        self.root.title("BreakbeatAI Tracker Editor v93 — slice index")
    ...
    def detect_external_player(self):
        for name in ["pw-play", "paplay", "aplay", "ffplay"]:
            ...
            if path:
                print(f"[v93] Backend audio : {name} -> {path}")
                return name

        print("[v93] Aucun backend système trouvé, fallback sounddevice.")
        return None
    ...
    def build_initial_pattern(self, randomize=False):
        ...
        print(
            f"[v93] Génération pattern | break={self.project['safe']} | "
        ...
        for slot in range(max_items):
            ...
            print(f"[v93] slot {slot:02d} | step {x_step:02d} -> slice {pair:02d}")
    ...
    def build_ui(self):
        ...
        title = tk.Label(
            main,
            text="BreakbeatAI v93 — clean 64 variation",
            bg="#111018",
            fg="#ff7acc",
            font=("Sans", 20, "bold"),
        )
    ...
    def record_correction(self, event_type, before=None, after=None):
        ...
        print(f"[v93] correction sauvegardée : {event_type}")
    ...
    def play_audition_array(self, audio, label="audition"):
        ...
        audition_wav = OUT_DIR / f"{self.project['safe']}_v93_audition.wav"
        ...
        if self.external_player:
            try:
                ...
                print(f"[v93] Erreur audition {self.external_player} : {exc}")
    ...
    def on_close(self):
        ...
        """
        print("[v93] Fermeture : arrêt audio + destruction fenêtre")
    ...
    def play_audio_array(self, audio, label="audio"):
        ...
        audio = normalize(audio)
        live_wav = OUT_DIR / f"{self.project['safe']}_v93_live.wav"
        ...
        if self.external_player:
            try:
                ...
                print(f"[v93] Erreur backend {self.external_player} : {exc}")
    ...
    def render_preview_file(self):
        OUT_DIR.mkdir(parents=True, exist_ok=True)
        wav = OUT_DIR / f"{self.project['safe']}_tracker_app_v93_preview.wav"
    ...
    def write_latest_pattern(self, reason="correction"):
        ...
        path = LATEST_DIR / f"{self.project['safe']}_latest_slice_index_v93.json"

        data = {
            "version": "breakbeatai_latest_slice_index_v93",
            "reason": reason,
    ...
    def save(self):
        ...
        tracker_path = OUT_DIR / f"{self.project['safe']}_tracker_app_edit_v93.json"
        ...
        stamp = file_stamp()
        validated_path = VALIDATED_DIR / f"{self.project['safe']}_validated_slice_index_loop64_v93_{stamp}.json"

        validated = {
            "version": "breakbeatai_validated_slice_index_loop64_v93",
...
def v44_record_correction(self, event_type, before=None, after=None):
    ...
    print(f"[v93] correction sauvegardée : {event_type}")
    print(f"[v93] correction path : {corrections_path}")
    print(f"[v93] latest path : {latest_path}")
...
def v45_move_selected(self, dx, dy):
    ...
    if dy != 0 and before != after:
        try:
            ...
            print(f"[v93] audition après flèche haut/bas impossible : {exc}")
...
def v46_warp_loop_pitch_preserve(audio, target_len, cache_key):
    ...
    if rubberband:
        try:
            ...
            print(f"[v93] rubberband global warp échec : {exc}")
    ...
    if ffmpeg:
        try:
            ...
            print(f"[v93] ffmpeg global warp échec : {exc}")

    print("[v93] Aucun warp pitch-preserve dispo, fallback fit no-speed.")
    return v46_fit_audio_no_speed(audio, target_len)
...
def v46_build_initial_pattern(self, randomize=False):
    ...
    print(
        f"[v93] M8 pattern | break={self.project['safe']} | "
...
def v46_get_warped_loop(self):
    ...
    print(
        f"[v93] global warp OK | source_len={loop_end-loop_start} | "
...
def v46_build_ui(self):
    ...
    try:
        ...
        print(f"[v93] UI extension impossible : {exc}")
...
def v47_play_audio_array(self, audio, label="audio"):
    ...
    if len(audio) == 0 or v47_is_silent(audio):
        ...
        print(f"[v93] lecture annulée : audio silencieux ({label})")
        return 0
...
def v47_toggle_loop(self):
    ...
    if not getattr(self, "pattern", None):
        ...
        print("[v93] loop annulée : pattern vide")
        return "break"
...
def v47_delete_selected(self):
    ...
    if not getattr(self, "pattern", None):
        ...
        print("[v93] pattern vide après suppression : audio coupé")
...
def v48_build_initial_pattern(self, randomize=False):
    ...
    print("[v93] pattern initial forcé VIDE : aucun son ne doit jouer.")
    return []
...
def v48_load_first_slices(self):
    ...
    print(f"[v93] Load 16 slices : {len(self.pattern)} items")


def v48_debug_pattern(self, label="render"):
    print("")
    print(f"[v93] DEBUG PATTERN avant {label}")
    print(f"[v93] items = {len(getattr(self, 'pattern', []) or [])}")

    if not getattr(self, "pattern", None):
        print("[v93] pattern vide confirmé")
        print("")
        return
    ...
    for item in self.pattern:
        ...
        print(
            f"[v93] id={item_id} | step {x:02d}/{end:02d} | "
...
def v48_render_audio_with_timeline(self):
    ...
    if not getattr(self, "pattern", None):
        loop_samples = self.get_loop_samples()
        print(f"[v93] rendu silence total : pattern vide | samples={loop_samples}")
        return np.zeros(loop_samples, dtype=np.float32), []
    ...
    print(f"[v93] rendu audio peak={peak:.8f} | timeline_events={len(timeline) if timeline is not None else 'None'}")

    if timeline:
        print("[v93] timeline events:")
        for ev in timeline:
            print(f"[v93]   {ev}")
...
def v48_toggle_loop(self):
    if not getattr(self, "pattern", None):
        ...
        print("[v93] Space ignoré : pattern vide.")
        return "break"
...
def v48_build_ui(self):
    ...
    try:
        ...
        print(f"[v93] ajout UI impossible : {exc}")
...
def v49_add_item_at_xy(self, x, y):
    ...
    try:
        ...
        print(f"[v93] record_correction add impossible : {exc}")
    ...
    try:
        ...
        print(f"[v93] audition après ajout impossible : {exc}")

    print(f"[v93] add | id={item['id']} | step={x_step} | pair={pair} | lane={lane}")
...
def v49_duplicate_selected_forward(self, event=None):
    ...
    if item is None:
        ...
        print("[v93] duplicate annulé : aucune note sélectionnée")
        return "break"
    ...
    if new_x > max_x:
        ...
        print("[v93] duplicate annulé : fin de grille")
        return "break"
    ...
    try:
        ...
        print(f"[v93] record_correction duplicate impossible : {exc}")
    ...
    try:
        ...
        print(f"[v93] audition après duplicate impossible : {exc}")

    print(
        f"[v93] duplicate | old_id={item.get('id')} | new_id={new_item.get('id')} | "
...
def v49_build_ui(self):
    ...
    try:
        ...
        print(f"[v93] bind canvas click impossible : {exc}")
    ...
    try:
        ...
        print(f"[v93] bind keys impossible : {exc}")
...
def v50_load_style_model():
    ...
    try:
        ...
        print(f"[v93] impossible de charger le modèle IA : {exc}")
        return None
...
def v50_generate_ai_pattern(self, event=None):
    ...
    if model is None:
        ...
        print("[v93] modèle absent :", STYLE_MODEL_PATH)
        return "break"
    ...
    print("")
    print(f"[v93] GENERATE IA | safe={safe} | pairs={pair_values} | slots={max_slots}")
    ...
    for slot in range(max_slots):
        ...
        print(
            f"[v93] slot {slot:02d} | step {x_step:02d} | "
    ...
    try:
        ...
        print(f"[v93] write_latest_pattern impossible : {exc}")
...
def v50_build_ui(self):
    ...
    try:
        ...
        print(f"[v93] UI IA impossible : {exc}")
...
def v51_mouse_down(self, event):
    ...
    if item is not None:
        ...
        print(
            f"[v93] mouse down note | id={item.get('id')} | "
            f"step={item.get('x_step')} | pair={item.get('pair')}"
        )
    else:
        print("[v93] mouse down empty")
...
def v51_mouse_up(self, event):
    ...
    if state.get("empty"):
        ...
        print("[v93] drag depuis vide ignoré")
        return "break"
    ...
    if before != after:
        try:
            ...
            print(f"[v93] record_correction drag impossible : {exc}")
        ...
        print(
            f"[v93] drag move | id={after.get('id')} | "
            f"step {before.get('x_step')} -> {after.get('x_step')} | "
        ...
        try:
            ...
            print(f"[v93] audition après drag impossible : {exc}")
...
def v51_build_ui(self):
    ...
    try:
        ...
        print(f"[v93] bind souris impossible : {exc}")
    ...
    try:
        ...
        print(f"[v93] bind Suppr impossible : {exc}")
...
def v52_normalize_loaded_notes(self, notes):
    ...
    for raw in sorted(notes, key=lambda n: (int(float(n.get("x_step", 0))), int(float(n.get("pair", 0))))):
        ...
        if pair not in valid_pairs:
            print(f"[v93] slice ignorée car absente du break actuel : pair={pair}")
            continue
...
def v52_load_saved_beat(self, event=None):
    ...
    if not saves:
        ...
        print("[v93] aucune sauvegarde trouvée")
        return "break"
    ...
    if not pattern:
        ...
        print("[v93] sauvegarde incompatible :", chosen["path"])
        return "break"
    ...
    print("")
    print("[v93] BEAT RESTAURÉ")
    print("[v93] fichier :", chosen["path"])
    print("[v93] clé :", chosen["key"])
    print("[v93] notes :", len(pattern))
    print("")
...
def v52_list_saved_beats(self, event=None):
    ...
    print("")
    print("[v93] SAUVEGARDES TROUVÉES")
    for i, save in enumerate(saves[:30]):
        print(
            f"[v93] {i:02d} | notes={len(save['notes']):02d} | "
...
def v52_build_ui(self):
    ...
    try:
        ...
        print(f"[v93] UI load saved impossible : {exc}")
...
def v53_run_training(self):
    ...
    if not trainer.exists():
        ...
        print("[v93] trainer introuvable :", trainer)
        return False

    print("")
    print("[v93] AUTO-TRAIN IA après Save...")
    print("")
...
def v53_save(self):
    ...
    try:
        ...
        print(f"[v93] auto-train après save impossible : {exc}")
...
def v53_build_ui(self):
    ...
    try:
        ...
        print(f"[v93] UI auto-train impossible : {exc}")
...
def v55_mouse_up(self, event):
    ...
    if not state.get("moved"):
        ...
        try:
            ...
            print(f"[v93] audition clic note impossible : {exc}")
        ...
        print(
            f"[v93] click play/select | id={item.get('id')} | "
            f"step={item.get('x_step')} | pair={item.get('pair')}"
        )
...
def v58_add_item_at_xy(self, x, y):
    ...
    print(
        f"[v93] add | id={item['id']} | step={x_step} | "
...
def v58_toggle_selected_length(self, event=None):
    ...
    try:
        ...
        print(f"[v93] record_correction length impossible : {exc}")

    v58_refresh_after_edit(self, "toggle_note_length")

    v58_status(self, f"Longueur note : {old_len} -> {new_len} case(s).")
    print(f"[v93] length toggle | id={item.get('id')} | {old_len} -> {new_len}")
...
def v58_duplicate_selected_forward(self, event=None):
    ...
    try:
        ...
        print(f"[v93] audition duplicate impossible : {exc}")
...
def v58_build_ui(self):
    ...
    try:
        ...
        print(f"[v93] UI demi-grille impossible : {exc}")
    ...
    try:
        ...
        print(f"[v93] bind demi-grille impossible : {exc}")
...
def v59_rank_current_slices(self):
    ...
    for pair in self.pair_values:
        ...
        try:
            ...
            print(f"[v93] analyse pair {pair} impossible : {exc}")
            role = "other"
    ...
    print("")
    print("[v93] TOP ROLES CURRENT BREAK")
    for r in ("kick", "snare", "hat"):
        print(f"[v93] {r:5s}:", [(x["pair"], round(x["score"], 3)) for x in ranks[r][:6]])
    print("")
...
def v59_load_role_model():
    ...
    try:
        ...
        print(f"[v93] lecture modèle rôle impossible : {exc}")
        return None
...
def v59_generate_role_aware(self, event=None):
    ...
    if model is None:
        ...
        print("[v93] modèle rôle absent, fallback musical snare 6.")
        model = {
            "breaks": {},
            "global": {},
            "patterns_used": 0,
    ...
    print("")
    print("[v93] ROLE-AWARE GENERATION")
    print("[v93] model patterns_used:", model.get("patterns_used", 0))
    for item in pattern:
        print(
            f"[v93] step {item['x_step']:02d}/{item['x_step'] + item['length'] - 1:02d} | "
...
def v59_run_role_training(self, event=None):
    ...
    try:
        ...
        print(f"[v93] training rôle impossible : {exc}")
...
def v59_save(self):
    ...
    try:
        ...
        print(f"[v93] auto-training rôle après save impossible : {exc}")
...
def v59_build_ui(self):
    ...
    try:
        ...
        print(f"[v93] UI role-aware impossible : {exc}")
...
def v62_add_item_at_xy(self, x, y):
    ...
    print(f"[v93] add | id={item['id']} | step={x_step} | len={length} | pair={pair}")
...
def v62_mouse_down(self, event):
    ...
    if item is not None:
        ...
        print(
            f"[v93] mouse down {state['mode']} | id={item.get('id')} | "
            f"step={item.get('x_step')} | len={item.get('length')} | pair={item.get('pair')}"
        )
    else:
        print("[v93] mouse down empty")
    ...
    if mode == "resize":
        if not state.get("moved"):
            ...
            try:
                ...
                print(f"[v93] audition clic resize impossible : {exc}")
        ...
        if before != after:
            try:
                ...
                print(f"[v93] record_correction resize impossible : {exc}")
            ...
            try:
                ...
                print(f"[v93] audition après resize impossible : {exc}")
    ...
    if mode == "move":
        ...
        if before != after:
            try:
                ...
                print(f"[v93] record_correction move impossible : {exc}")
            ...
            try:
                ...
                print(f"[v93] audition après move impossible : {exc}")
...
def v62_toggle_selected_length(self, event=None):
    ...
    try:
        ...
        print(f"[v93] record_correction toggle length impossible : {exc}")

    v62_refresh_after_edit(self, "toggle_note_length_v62")
    v62_status(self, f"Longueur note : {old_len} → {new_len} case(s).")

    print(f"[v93] toggle length | id={item.get('id')} | {old_len} -> {new_len}")
...
def v62_draw(self):
    ...
    try:
        ...
        print(f"[v93] draw handles impossible : {exc}")
...
def v62_build_ui(self):
    ...
    try:
        ...
        print(f"[v93] bind resize impossible : {exc}")
    ...
    try:
        ...
        print(f"[v93] UI resize impossible : {exc}")
...
def v63_load_model():
    ...
    try:
        ...
        print(f"[v93] lecture modèle strict impossible : {exc}")
        return None
...
def v63_generate_safe_role_ai(self, event=None):
    ...
    print("")
    print("[v93] SAFE ROLE AI GENERATED")
    print("[v93] model strict:", "yes" if model else "no")
    print("[v93] snare main/alt:", main_snare, alt_snare)
    print("[v93] kick  main/alt:", main_kick, alt_kick)
    print("[v93] hat   main/alt:", main_hat, alt_hat)
    print("[v93] pattern:")
    for item in pattern:
        print(
            f"[v93] step {item['x_step']:02d}/{item['x_step'] + item['length'] - 1:02d} | "
...
def v63_run_strict_training(self, event=None):
    ...
    print("")
    print("[v93] TRAIN STRICT ROLE MODEL...")
    print("")
    ...
    try:
        ...
        print(f"[v93] training strict impossible : {exc}")
...
def v63_build_ui(self):
    ...
    try:
        ...
        print(f"[v93] UI safe impossible : {exc}")
...
def v65_rank_slices(self):
    ...
    for pair in self.pair_values:
        ...
        try:
            ...
            print(f"[v93] analyse slice {pair} impossible : {exc}")
            scores = {
                "kick": 0.0,
                "snare": 0.0,
                "hat": 0.0,
    ...
    print("")
    print("[v93] TOP SLICES PAR ROLE — break courant")
    for role in ("kick", "snare", "hat", "ghost_snare"):
        print(f"[v93] {role:12s}:", [(x["pair"], round(x["score"], 3)) for x in ranks[role][:8]])
    print("")
...
def v65_generate_from_role_template(self, event=None):
    ...
    try:
        ...
        print(f"[v93] template introuvable : {exc}")
        return "break"
    ...
    print("")
    print("[v93] CROSS-SONG ROLE TEMPLATE GENERATED")
    print("[v93] template:", V65_TEMPLATE_PATH)
    print("[v93] temperature:", temperature, "| hats:", hat_density, "| ghosts:", ghost_density)
    print("[v93] pattern:")
    for item in pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v93] step {item['x_step']:02d}/{end:02d} | "
...
def v67_train_strict_if_available(self):
    ...
    if trainer is None:
        print("[v93] aucun trainer trouvé.")
        return False

    print("")
    print("[v93] TRAIN après validation :", trainer)
    print("")
    ...
    try:
        ...
        print(f"[v93] training impossible : {exc}")
        return False
...
def v67_generate_candidate(self, event=None):
    ...
    print("")
    print("[v93] CANDIDATE GENERATED")
    print("[v93] break:", self.v67_candidate_safe)
    print("[v93] notes:", len(candidate))
    print("[v93] Maintenant : corrige puis clique Good après modifs, ou Reject.")
    print("")
...
def v67_accept_corrected(self, event=None):
    ...
    print("")
    print("[v93] ACCEPTED GOOD AFTER EDIT")
    print("[v93] saved:", validated_path)
    print("[v93] trained:", trained)
    print("")
...
def v67_accept_without_training(self, event=None):
    ...
    print("[v93] accepted no-train:", validated_path)
...
def v67_build_ui(self):
    ...
    try:
        ...
        print(f"[v93] UI feedback impossible : {exc}")
...
def v68_generate_candidate(self, event=None):
    ...
    print("[v93] Length policy appliquée : 2 cases par défaut, 1 case rare.")
...
def v69_force_pattern_hard_2_grid(self, reason="v69_force_hard_2_grid"):
    ...
    print("")
    print("[v93] HARD 2 GRID APPLIQUÉ")
    for item in self.pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v93] step {item['x_step']:02d}/{end:02d} | "
...
def v70_duplicate_selected_forward(self, event=None):
    ...
    print("")
    print("[v93] CTRL+D DUPLICATE +2")
    print(f"[v93] old_id={item.get('id')} new_id={new_item.get('id')}")
    print(f"[v93] step {old_x}/{old_x + length - 1} -> {new_x}/{new_x + length - 1}")
    print(f"[v93] pair={new_item.get('pair')} len={length}")
    print("")
...
def v70_build_ui(self):
    ...
    try:
        ...
        print(f"[v93] bind Ctrl+D impossible : {exc}")
...
def v71_rank_slices(self):
    ...
    print("")
    print("[v93] PRE-LEARNING TOP SLICES")
    for role in ("kick", "snare", "hat", "ghost_snare"):
        print(f"[v93] {role:12s}:", [(x["pair"], round(x["score"], 3)) for x in ranks[role][:8]])
    print("")
...
def v71_prelearn_roles(self, force=False):
    ...
    print("")
    print("[v93] PRELEARN ROLES POUR", safe)
    for role in ("kick", "snare", "hat", "ghost_snare", "bad"):
        print(f"[v93] {role:12s}:", roles.get(role, []))
    print("")
...
def v71_generate_full_candidate(self, event=None):
    ...
    print("")
    print("[v93] FULL CANDIDATE GENERATED")
    print("[v93] break:", v71_safe(self))
    print("[v93] roles:", roles)
    print("[v93] pattern:")
    for item in pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
...
def v72_open_break(self, safe=None):
    ...
    print("")
    print("[v93] OPEN BREAK")
    print("[v93] current:", current)
    print("[v93] next   :", safe)
    print("[v93] cmd    :", " ".join(cmd))
    print("")
...
def v72_build_ui(self):
    ...
    try:
        ...
        print(f"[v93] UI break switcher impossible : {exc}")
...
def v73_sanitize_tracker_pattern(self, reason="v73_tracker_strict_no_overlap"):
    ...
    print("")
    print("[v93] TRACKER STRICT NO OVERLAP")
    print("[v93] kept:", len(accepted), "| rejected overlaps:", len(rejected))

    for item in accepted:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v93] KEEP step {item['x_step']:02d}/{end:02d} | "
    ...
    if rejected:
        ...
        for item in rejected:
            ...
            print(
                f"[v93] DROP step {item['x_step']:02d}/{end:02d} | "
                f"role={item.get('v73_role_key')} | pair={item.get('pair')}"
            )
...
def v81_launch_break(self, source):
    ...
    print("")
    print("[v93] CHARGER BREAK")
    print("[v93] courant :", current)
    print("[v93] nouveau :", source)
    print("[v93] cmd     :", " ".join(cmd))
    print("")
...
def v81_patch_charger_break_buttons(self):
    ...
    print(f"[v93] boutons Charger break patchés : {patched}")
...
def v81_generate_candidate(self, event=None):
    ...
    print("")
    print("[v93] GENERATE CANDIDATE")
    print("[v93] break chargé :", current)
    print("[v93] générateur   :", getattr(fn, "__name__", None))
    print("")
...
def v81_run_training(self):
    ...
    if script is None:
        print("[v93] Aucun script training trouvé.")
...
def v81_save_feedback(self, verdict, train_now=False):
    ...
    print("")
    print("[v93] FEEDBACK SAUVÉ")
    print("[v93] break  :", current)
    print("[v93] verdict:", verdict)
    print("[v93] train  :", train_now)
    print("[v93] fichier:", out_path)
    print("")
...
def v81_build_ui(self):
    ...
    try:
        self.root.title(f"BreakbeatAI v93 — clean 64 variation")
    except Exception:
        pass
    ...
    try:
        ...
        print(f"[v93] patch Charger break impossible : {exc}")

    try:
        v81_install_training_toolbar(self)
    except Exception as exc:
        print(f"[v93] toolbar impossible : {exc}")
...
def v82_auto_role_pools(self):
    ...
    print("")
    print("[v93] ROLE POOLS")
    for r in ["kick", "snare", "hat", "ghost_snare", "bad"]:
        print(f"[v93] {r:12s}:", roles.get(r, []))
    print("")
...
def v82_generate_candidate(self, event=None):
    ...
    print("")
    print("[v93] NEW CANDIDATE")
    print("[v93] break :", current)
    print("[v93] nonce :", nonce)
    print("[v93] rejected known:", len(rejected))
    print("[v93] fingerprint rejeté ?: ", chosen_fp in rejected)
    for item in self.pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v93] step {item['x_step']:02d}/{end:02d} "
...
def v82_patch_training_buttons(self):
    ...
    print(f"[v93] boutons training patchés : {patched}")
...
def v82_build_ui(self):
    ...
    try:
        ...
        print(f"[v93] patch boutons impossible : {exc}")
...
# ---------------------------------------------------------------------
# v93 EXTENSION : clean UI + loop 64 + variation sur le deuxième 32
# ---------------------------------------------------------------------
# Premier 32 :
#   kick hat snare hat / hat kick snare hat
#
# Deuxième 32 :
#   même placement de snares relatif, mais variations de kicks/hats.
#
# Snares :
...
V93_OVERRIDES_PATH = Path("dataset/learning/break_role_overrides_v01.json")
V93_LOCKS_PATH = Path("dataset/learning/main_role_locks_v87.json")
...
V93_FIRST_SNARE_STEPS = [4, 12, 20, 28]
V93_SECOND_SNARE_STEPS = [36, 44, 52, 60]
V93_ALL_SNARE_STEPS = V93_FIRST_SNARE_STEPS + V93_SECOND_SNARE_STEPS


def v93_status(self, text):
    try:
        if hasattr(self, "set_status"):
            self.set_status(text)
        else:
...
def v93_load_json(path, default):
    try:
...
def v93_find_record(data, current):
    br = data.get("breaks", {}).get(current)
    if br is not None:
        return br

    for name, obj in data.get("breaks", {}).items():
        if v93_norm(name) == v93_norm(current):
            return obj

    return None


def v93_all_pairs(self):
    try:
        pairs = [int(x) for x in self.pair_values]
    except Exception:
        pairs = [0]
...
def v93_force_64(self):
    """
    Force les attributs courants à 64 quand ils existent dans l'app source.
    Ça évite de rester bloqué en affichage/rendu 32.
    """
    for attr in [
        "total_steps",
        "loop_steps",
    ...
    try:
        if hasattr(self, "canvas"):
            self.canvas.configure(scrollregion=(0, 0, 4096, 2048))
    except Exception:
        pass


def v93_load_roles(self):
    current = v93_current_break(self)
    ...
    data = v93_load_json(V93_OVERRIDES_PATH, {"breaks": {}})
    br = v93_find_record(data, current)
...
def v93_load_locks(self):
    current = v93_current_break(self)
    data = v93_load_json(V93_LOCKS_PATH, {"breaks": {}})
    br = v93_find_record(data, current)
...
def v93_candidates(self, roles, locks, role):
    out = []
...
def v93_choose_main_pairs(self, roles, locks):
    chosen = {}
    used = set()

    # Snare d'abord pour garder une vraie ligne snare.
    for role in ["snare", "kick", "hat"]:
        candidates = v93_candidates(self, roles, locks, role)
...
def v93_alt_pair_for_second_32(self, roles, locks, chosen, role, occurrence):
    """
    Sur le deuxième 32 :
    - snare reste le snare principal pour garder un backbeat propre
    - kick/hat peuvent prendre un autre sample du même rôle si disponible
    """
...
def v93_make_note(self, step, pair, role, source, variant_index):
    step = int(step)
    ...
    return {
        ...
        "hit_slot": int(step) // 2,
        "ai_generated": True,
        "ai_model": "v93_clean_64_variation",
        "learned_role": str(role),
        "prior_label": str(role),
        "main_role_source": str(source),
        "v93_loop64": True,
        "v93_second32_variant": int(variant_index),
    }


def v93_validate_pattern(pattern):
    if len(pattern) != 32:
        raise RuntimeError(f"BUG v93 : pattern devrait avoir 32 notes, obtenu {len(pattern)}")
    ...
    for item in pattern:
        ...
        if x % 2 != 0:
            raise RuntimeError(f"BUG v93 : départ impair {x}")

        if x < 0 or x > 62:
            raise RuntimeError(f"BUG v93 : step hors 64 : {x}")

        if x in occupied or (x + 1) in occupied:
            raise RuntimeError(f"BUG v93 : collision à {x}/{x+1}")

        occupied.add(x)
        occupied.add(x + 1)

        if role == "snare" and x not in V93_ALL_SNARE_STEPS:
            raise RuntimeError(f"BUG v93 : snare à {x}, interdit")


def v93_generate(self, event=None):
    v93_force_64(self)

    current = v93_current_break(self)
    roles = v93_load_roles(self)
    locks = v93_load_locks(self)
    chosen = v93_choose_main_pairs(self, roles, locks)

    nonce = int(getattr(self, "v93_nonce", 0)) + 1
    self.v93_nonce = nonce

    rng = random.Random(time.time_ns() + nonce * 9301)
    variant_index = rng.randrange(len(V93_SECOND_32_VARIANTS))
    second_32 = V93_SECOND_32_VARIANTS[variant_index]

    full_grid = list(V93_FIRST_32) + list(second_32)

    print("")
    print("[v93] GENERATE 64 CLEAN")
    print("[v93] break:", current)
    print("[v93] second 32 variant:", variant_index)
    print("[v93] locks:", locks)
    for role in ["kick", "snare", "hat"]:
        print(
            f"[v93] {role:6s}: pair={chosen[role]['pair']} "
    ...
    for step, role in full_grid:
        if step < 32:
            ...
            pair, source = v93_alt_pair_for_second_32(self, roles, locks, chosen, role, occ)

        pattern.append(v93_make_note(self, step, pair, role, source, variant_index))

    v93_validate_pattern(pattern)
    ...
    try:
        ...
        print(f"[v93] draw impossible : {exc}")

    try:
        self.refresh_panel()
    except Exception as exc:
        print(f"[v93] refresh_panel impossible : {exc}")

    try:
        self.write_latest_pattern(reason="v93_clean_64_variation")
    except Exception as exc:
        print(f"[v93] write_latest_pattern impossible : {exc}")
    ...
    print("")
    print("[v93] GENERATED PATTERN 64")
    for item in self.pattern:
        x = int(item["x_step"])
        end = x + int(item["length"]) - 1
        print(
            f"[v93] step {x:02d}/{end:02d} "
    ...
    v93_status(self, f"v93 : loop 64 généré, variation second 32 #{variant_index}.")
    return "break"


def v93_all_widgets(widget):
    out = [widget]
    ...
    for child in children:
        out.extend(v93_all_widgets(child))

    return out


def v93_patch_generate_buttons(self):
    patched = 0

    for widget in v93_all_widgets(self.root):
        try:
            text = str(widget.cget("text")).strip().lower()
        except Exception:
            continue
...
_old_v93_build_ui = SliceIndexTracker.build_ui


def v93_build_ui(self):
    _old_v93_build_ui(self)

    v93_force_64(self)

    try:
        self.root.title(f"BreakbeatAI v93 clean 64 variation — {v93_current_break(self)}")
    except Exception:
        pass

    try:
        v93_patch_generate_buttons(self)
    except Exception as exc:
        print(f"[v93] patch boutons impossible : {exc}")
    ...
    v93_status(self, "v93 clean 64 chargé : Generate Candidate ou F12.")


SliceIndexTracker.generate_candidate = v93_generate
SliceIndexTracker.generate_full_candidate = v93_generate
SliceIndexTracker.generate_ai_pattern = v93_generate
SliceIndexTracker.generate_cross_song_template = v93_generate
SliceIndexTracker.generate_safe_experiment = v93_generate
SliceIndexTracker.generate_locked_roles = v93_generate
SliceIndexTracker.generate_role_aware = v93_generate
SliceIndexTracker.generate_safe_role_ai = v93_generate

SliceIndexTracker.build_ui = v93_build_ui
