...
"""
03_tracker_editor_app_v15.py

BreakbeatAI Tracker Editor v15
...
Usage :
    ...
    python pipeline/03_tracker_editor_app_v15.py --source "amen"
"""
...
class TrackerEditorApp:
    def __init__(self, root, pair_json):
        ...
        # v15 : pas d’override automatique au lancement.

        self.audio_cache = {}
        self.pair_role_analysis = {}
        self.reanalyse_pairs()

        self.pattern = self.build_initial_pattern_manual()
        ...
        self.root.title("BreakbeatAI Tracker Editor v15")
    ...
    def seed_known_overrides(self):
        ...
        if 1 in self.pair_values and "1" not in self.pair_role_overrides:
            ...
            print("[v15] Override auto confirmé : pair 1 = kick")
    ...
    def detect_external_player(self):
        for name in ["pw-play", "paplay", "aplay", "ffplay"]:
            ...
            if path:
                print(f"[v15] Backend audio système trouvé : {name} -> {path}")
                return name

        print("[v15] Aucun backend système trouvé, fallback sounddevice.")
        return None
    ...
    def bind_keys(self):
        ...
        self.root.bind_all("h", lambda e: self.move_selected_to_role("hat"))
        self.root.bind_all("k", lambda e: self.move_selected_to_role("kick"))
        self.root.bind_all("s", lambda e: self.move_selected_to_role("snare"))
        self.root.bind_all("u", lambda e: self.move_selected_to_role("unknown"))
        print("[v15] Raccourcis : Espace loop | h/k/s/u = classer pair sélectionnée")
    ...
    def analyse_pair_for_auto_sort(self, pair):
        """
        Analyse audio locale, prudente.
        On préfère À PLACER plutôt que de mettre tout en SNARE ou tout en HI-HAT.
        """
        pair = int(pair)
        audio = self.get_audio(pair)

        n = min(len(audio), int(0.22 * SR))
        if n < 128:
            return {
                "role": "placer",
                "confidence": 0.0,
        ...
        sub = float(mag[freqs < 120].sum() / total)
        low = float(mag[(freqs >= 120) & (freqs < 350)].sum() / total)
        body = float(mag[(freqs >= 350) & (freqs < 1200)].sum() / total)
        mid = float(mag[(freqs >= 1200) & (freqs < 4500)].sum() / total)
        high = float(mag[freqs >= 4500].sum() / total)
        centroid = float((freqs * mag).sum() / total)

        block = self.block_by_pair.get(pair, {})
        slicer_role = clean_role(block.get("slicer_role_guess"))
        slicer_conf_raw = block.get("slicer_role_confidence")
        try:
            slicer_conf = float(slicer_conf_raw or 0.0)
        except Exception:
            slicer_conf = 0.0
        ...
        # Le slicer peut aider, mais il ne doit plus dominer.
        if slicer_conf >= 0.80 and slicer_role in ("hat", "kick", "snare"):
            if slicer_role == "hat":
                hat_score += 0.10
            elif slicer_role == "kick":
                kick_score += 0.10
            elif slicer_role == "snare":
                snare_score += 0.10
        ...
        # Règles fortes.
        if (sub + low) > 0.34 and high < 0.34 and centroid < 4200:
            role = "kick"
            reason = "strong_low_kick"

        elif high > 0.50 and (sub + low) < 0.15 and centroid > 5200:
            role = "hat"
            reason = "strong_high_hat"

        elif (mid + body) > 0.43 and high > 0.14 and (sub + low) < 0.32 and centroid > 1800:
            role = "snare"
            reason = "broad_mid_snare"

        # Sinon, score avec marge.
        elif margin >= 0.13:
            role = best_role
            reason = "score_margin"

        confidence = max(0.0, min(1.0, 0.45 + margin * 1.8))
        ...
        return {
            ...
            "slicer_role": slicer_role,
            "slicer_conf": slicer_conf,
        }

    def role_to_auto_lane(self, role, occurrence=0):
        role = clean_role(role)

        if role == "hat":
            return 1 + (occurrence % 2)

        if role == "kick":
            return 3 + (occurrence % 2)

        if role == "snare":
            return 5 + (occurrence % 2)

        return 0

    def auto_assign_pairs(self):
        """
        Retourne un dict pair -> analyse.
        Protection anti-bug : si une famille prend presque tout, on garde seulement
        les plus confiants et on remet le reste dans À PLACER.
        """
        analyses = {}

        for pair in self.pair_values[:32]:
            analyses[int(pair)] = self.analyse_pair_for_auto_sort(pair)

        n = max(1, len(analyses))
        cap = max(2, int(n * 0.55))

        for role in ("hat", "kick", "snare"):
            items = [
                (pair, data)
            ...
            if len(items) > cap:
                ...
                keep = set(pair for pair, _data in items_sorted[:cap])

                for pair, data in items_sorted[cap:]:
                    data["role"] = "placer"
                    data["reason"] = f"distribution_cap_from_{role}"
                    data["confidence"] = 0.0

        return analyses

    def build_initial_pattern_auto(self):
        """
        v15 : auto-tri assisté.
        Les cases restent sans chiffre.
        Leur rôle visible = la ligne où elles tombent.
        """
        analyses = self.auto_assign_pairs()
        role_counts = {"placer": 0, "hat": 0, "kick": 0, "snare": 0}

        pattern = []

        for idx, pair in enumerate(self.pair_values[:32]):
            pair = int(pair)
            data = analyses.get(pair, {"role": "placer", "confidence": 0.0})
            role = clean_role(data.get("role"))

            lane = self.role_to_auto_lane(role, role_counts.get(role, 0))
            ...
            pattern.append({
                ...
                "x_step": idx * 2,
                "lane": lane,
                "role": lane_info(lane)["role"],
                "length": 2,
                "pair": pair,
                "auto_role": role,
                "auto_confidence": data.get("confidence", 0.0),
                "auto_reason": data.get("reason", ""),
            })

        return pattern

    def auto_sort_current_pattern(self):
        """
        Relance l'auto-tri sur les cases actuelles, sans changer leur ordre horizontal.
        """
        analyses = self.auto_assign_pairs()
        role_counts = {"placer": 0, "hat": 0, "kick": 0, "snare": 0}
        ...
        for item in sorted(self.pattern, key=lambda e: (int(e["x_step"]), int(e["id"]))):
            ...
            item["lane"] = lane
            item["role"] = lane_info(lane)["role"]
            item["auto_role"] = role
            item["auto_confidence"] = data.get("confidence", 0.0)
            item["auto_reason"] = data.get("reason", "")

        self.draw()
        self.refresh_panel()
        self.output_label.config(
            text=(
                "Auto-tri v15 relancé : les sons confiants sont placés, "
                "les sons douteux restent dans À PLACER."
            )
        )
    ...
    def build_ui(self):
        ...
        title = tk.Label(
            main,
            text="BreakbeatAI Tracker Editor v15 — mode manuel, plus d’auto-snare/auto-hat",
            bg="#111018",
            fg="#ff7acc",
            font=("Sans", 20, "bold"),
        )
        ...
        subtitle = tk.Label(
            ...
            text=(
                ...
                "Les cases n’affichent plus de chiffres : leur rôle est la ligne où elles sont posées."
            ),
            bg="#111018",
            fg="#b9acc8",
        )
        ...
        tk.Button(panel, text="Vers À CLASSER", command=lambda: self.move_selected_to_role("unknown"), bg="#3b3b48", fg="#f5eefe").grid(row=1, column=4, padx=4)
        tk.Button(panel, text="Vers HI-HAT", command=lambda: self.move_selected_to_role("hat"), bg="#4b3d23", fg="#f5eefe").grid(row=1, column=5, padx=4)
        tk.Button(panel, text="Vers KICK", command=lambda: self.move_selected_to_role("kick"), bg="#4a2630", fg="#f5eefe").grid(row=1, column=6, padx=4)
        tk.Button(panel, text="Vers SNARE", command=lambda: self.move_selected_to_role("snare"), bg="#263a56", fg="#f5eefe").grid(row=1, column=7, padx=4)
        ...
        tk.Button(panel, text="Reset manuel", command=self.reset, bg="#30513f", fg="#f5eefe").grid(row=1, column=11, padx=4)
        ...
        self.output_label = tk.Label(
            panel,
            text="v15 : aucune classification automatique. Glisse les cases sur HI-HAT/KICK/SNARE. Les chiffres ne sont plus dessinés.",
            bg="#1b1824",
            fg="#77f5b5",
            justify="left",
        )
    ...
    def reanalyse_pairs(self):
        ...
        print("[v15] Analyse audio stricte :", counts)
    ...
    def build_initial_pattern_manual(self):
        """
        v15 : aucune auto-classification.
        On place les slices sur À CLASSER.
        Ensuite la ligne où tu poses la case devient l'information de rôle.
        """
        if not self.pair_values:
            return json.loads(json.dumps(DEFAULT_PATTERN))
    ...
    def move_selected_to_role(self, role):
        role = clean_role(role)
        if role is None:
            role = "unknown"

        lane = self.role_to_lane(role)
        self.set_lane(lane)
    ...
    def reset(self):
        ...
        self.draw()
        self.refresh_panel()
        self.output_label.config(text="Reset v15 : toutes les slices sont revenues dans À CLASSER, sans auto-classification.")
    ...
    def play_audio_array(self, audio, label="audio"):
        ...
        live_wav = OUT_DIR / f"{self.project['safe']}_v15_live.wav"
        ...
        if self.external_player:
            ...
            try:
                ...
                print(f"[v15] {label} lancé avec {self.external_player} : {live_wav}")
                return duration_ms
            except Exception as exc:
                print(f"[v15] Erreur backend système {self.external_player} : {exc}")

        try:
            import sounddevice as sd
            sd.play(audio, SR)
            self.output_label.config(text=f"{label} lancé avec sounddevice — {duration_ms} ms")
            print(f"[v15] {label} lancé avec sounddevice")
            return duration_ms
        except Exception as exc:
    ...
    def draw_case(self, item):
        ...
        if False and int(item["length"]) >= 2:
            self.canvas.create_text(
                (x0 + x1) / 2,
                (y0 + y1) / 2,
                text=label,
    ...
    def refresh_panel(self):
        ...
        self.info_label.config(
            text=(
                f"case {item['id']} | pair audio {pair} | step {item['x_step']} | length {item['length']} | "
                f"ligne {lane['label']} | rôle réel = {lane['role']} | "
                f"slicer ignoré : {slicer_guess}/{slicer_conf:.2f}"
            )
        )
    ...
    def set_selected_pair_role(self, role):
        ...
        print(f"[v15] override pair {pair} = {role} -> {self.role_override_path}")
    ...
    def render_preview_file(self):
        OUT_DIR.mkdir(parents=True, exist_ok=True)
        wav = OUT_DIR / f"{self.project['safe']}_tracker_app_v15_preview.wav"
    ...
    def toggle_loop(self):
        if self.looping:
            ...
            print("[v15] Loop arrêtée")
            return

        self.stop_playhead()
        print("[v15] Lancement loop...")
        self.looping = True
    ...
    def save(self):
        ...
        data = {
            "version": "tracker_app_edit_v15_assisted_auto_sort_no_box_numbers",
        ...
        path = OUT_DIR / f"{self.project['safe']}_tracker_app_edit_v15.json"
