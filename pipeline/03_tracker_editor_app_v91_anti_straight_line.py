...
"""
BreakbeatAI Tracker Editor v91 — slice index
...
def warp_audio_to_length_external(audio, target_len, cache_key=None):
    ...
    if rubberband:
        try:
            ...
            print(f"[v91] rubberband échec, fallback ffmpeg/no-speed : {exc}")
    ...
    if ffmpeg:
        try:
            ...
            print(f"[v91] ffmpeg atempo échec, fallback no-speed : {exc}")
...
class SliceIndexTracker:
    def __init__(self, root, pair_json):
        ...
        self.root.title("BreakbeatAI Tracker Editor v91 — slice index")
    ...
    def detect_external_player(self):
        for name in ["pw-play", "paplay", "aplay", "ffplay"]:
            ...
            if path:
                print(f"[v91] Backend audio : {name} -> {path}")
                return name

        print("[v91] Aucun backend système trouvé, fallback sounddevice.")
        return None
    ...
    def build_initial_pattern(self, randomize=False):
        ...
        print(
            f"[v91] Génération pattern | break={self.project['safe']} | "
        ...
        for slot in range(max_items):
            ...
            print(f"[v91] slot {slot:02d} | step {x_step:02d} -> slice {pair:02d}")
    ...
    def build_ui(self):
        ...
        title = tk.Label(
            main,
            text="BreakbeatAI v91 — anti straight line",
            bg="#111018",
            fg="#ff7acc",
            font=("Sans", 20, "bold"),
        )
    ...
    def record_correction(self, event_type, before=None, after=None):
        ...
        print(f"[v91] correction sauvegardée : {event_type}")
    ...
    def play_audition_array(self, audio, label="audition"):
        ...
        audition_wav = OUT_DIR / f"{self.project['safe']}_v91_audition.wav"
        ...
        if self.external_player:
            try:
                ...
                print(f"[v91] Erreur audition {self.external_player} : {exc}")
    ...
    def on_close(self):
        ...
        """
        print("[v91] Fermeture : arrêt audio + destruction fenêtre")
    ...
    def play_audio_array(self, audio, label="audio"):
        ...
        audio = normalize(audio)
        live_wav = OUT_DIR / f"{self.project['safe']}_v91_live.wav"
        ...
        if self.external_player:
            try:
                ...
                print(f"[v91] Erreur backend {self.external_player} : {exc}")
    ...
    def render_preview_file(self):
        OUT_DIR.mkdir(parents=True, exist_ok=True)
        wav = OUT_DIR / f"{self.project['safe']}_tracker_app_v91_preview.wav"
    ...
    def write_latest_pattern(self, reason="correction"):
        ...
        path = LATEST_DIR / f"{self.project['safe']}_latest_slice_index_v91.json"

        data = {
            "version": "breakbeatai_latest_slice_index_v91",
            "reason": reason,
    ...
    def save(self):
        ...
        tracker_path = OUT_DIR / f"{self.project['safe']}_tracker_app_edit_v91.json"
        ...
        stamp = file_stamp()
        validated_path = VALIDATED_DIR / f"{self.project['safe']}_validated_slice_index_loop32_v91_{stamp}.json"

        validated = {
            "version": "breakbeatai_validated_slice_index_loop32_v91",
...
def v44_record_correction(self, event_type, before=None, after=None):
    ...
    print(f"[v91] correction sauvegardée : {event_type}")
    print(f"[v91] correction path : {corrections_path}")
    print(f"[v91] latest path : {latest_path}")
...
def v45_move_selected(self, dx, dy):
    ...
    if dy != 0 and before != after:
        try:
            ...
            print(f"[v91] audition après flèche haut/bas impossible : {exc}")
...
def v46_warp_loop_pitch_preserve(audio, target_len, cache_key):
    ...
    if rubberband:
        try:
            ...
            print(f"[v91] rubberband global warp échec : {exc}")
    ...
    if ffmpeg:
        try:
            ...
            print(f"[v91] ffmpeg global warp échec : {exc}")

    print("[v91] Aucun warp pitch-preserve dispo, fallback fit no-speed.")
    return v46_fit_audio_no_speed(audio, target_len)
...
def v46_build_initial_pattern(self, randomize=False):
    ...
    print(
        f"[v91] M8 pattern | break={self.project['safe']} | "
...
def v46_get_warped_loop(self):
    ...
    print(
        f"[v91] global warp OK | source_len={loop_end-loop_start} | "
...
def v46_build_ui(self):
    ...
    try:
        ...
        print(f"[v91] UI extension impossible : {exc}")
...
def v47_play_audio_array(self, audio, label="audio"):
    ...
    if len(audio) == 0 or v47_is_silent(audio):
        ...
        print(f"[v91] lecture annulée : audio silencieux ({label})")
        return 0
...
def v47_toggle_loop(self):
    ...
    if not getattr(self, "pattern", None):
        ...
        print("[v91] loop annulée : pattern vide")
        return "break"
...
def v47_delete_selected(self):
    ...
    if not getattr(self, "pattern", None):
        ...
        print("[v91] pattern vide après suppression : audio coupé")
...
def v48_build_initial_pattern(self, randomize=False):
    ...
    print("[v91] pattern initial forcé VIDE : aucun son ne doit jouer.")
    return []
...
def v48_load_first_slices(self):
    ...
    print(f"[v91] Load 16 slices : {len(self.pattern)} items")


def v48_debug_pattern(self, label="render"):
    print("")
    print(f"[v91] DEBUG PATTERN avant {label}")
    print(f"[v91] items = {len(getattr(self, 'pattern', []) or [])}")

    if not getattr(self, "pattern", None):
        print("[v91] pattern vide confirmé")
        print("")
        return
    ...
    for item in self.pattern:
        ...
        print(
            f"[v91] id={item_id} | step {x:02d}/{end:02d} | "
...
def v48_render_audio_with_timeline(self):
    ...
    if not getattr(self, "pattern", None):
        loop_samples = self.get_loop_samples()
        print(f"[v91] rendu silence total : pattern vide | samples={loop_samples}")
        return np.zeros(loop_samples, dtype=np.float32), []
    ...
    print(f"[v91] rendu audio peak={peak:.8f} | timeline_events={len(timeline) if timeline is not None else 'None'}")

    if timeline:
        print("[v91] timeline events:")
        for ev in timeline:
            print(f"[v91]   {ev}")
...
def v48_toggle_loop(self):
    if not getattr(self, "pattern", None):
        ...
        print("[v91] Space ignoré : pattern vide.")
        return "break"
...
def v48_build_ui(self):
    ...
    try:
        ...
        print(f"[v91] ajout UI impossible : {exc}")
...
def v49_add_item_at_xy(self, x, y):
    ...
    try:
        ...
        print(f"[v91] record_correction add impossible : {exc}")
    ...
    try:
        ...
        print(f"[v91] audition après ajout impossible : {exc}")

    print(f"[v91] add | id={item['id']} | step={x_step} | pair={pair} | lane={lane}")
...
def v49_duplicate_selected_forward(self, event=None):
    ...
    if item is None:
        ...
        print("[v91] duplicate annulé : aucune note sélectionnée")
        return "break"
    ...
    if new_x > max_x:
        ...
        print("[v91] duplicate annulé : fin de grille")
        return "break"
    ...
    try:
        ...
        print(f"[v91] record_correction duplicate impossible : {exc}")
    ...
    try:
        ...
        print(f"[v91] audition après duplicate impossible : {exc}")

    print(
        f"[v91] duplicate | old_id={item.get('id')} | new_id={new_item.get('id')} | "
...
def v49_build_ui(self):
    ...
    try:
        ...
        print(f"[v91] bind canvas click impossible : {exc}")
    ...
    try:
        ...
        print(f"[v91] bind keys impossible : {exc}")
...
def v50_load_style_model():
    ...
    try:
        ...
        print(f"[v91] impossible de charger le modèle IA : {exc}")
        return None
...
def v50_generate_ai_pattern(self, event=None):
    ...
    if model is None:
        ...
        print("[v91] modèle absent :", STYLE_MODEL_PATH)
        return "break"
    ...
    print("")
    print(f"[v91] GENERATE IA | safe={safe} | pairs={pair_values} | slots={max_slots}")
    ...
    for slot in range(max_slots):
        ...
        print(
            f"[v91] slot {slot:02d} | step {x_step:02d} | "
    ...
    try:
        ...
        print(f"[v91] write_latest_pattern impossible : {exc}")
...
def v50_build_ui(self):
    ...
    try:
        ...
        print(f"[v91] UI IA impossible : {exc}")
...
def v51_mouse_down(self, event):
    ...
    if item is not None:
        ...
        print(
            f"[v91] mouse down note | id={item.get('id')} | "
            f"step={item.get('x_step')} | pair={item.get('pair')}"
        )
    else:
        print("[v91] mouse down empty")
...
def v51_mouse_up(self, event):
    ...
    if state.get("empty"):
        ...
        print("[v91] drag depuis vide ignoré")
        return "break"
    ...
    if before != after:
        try:
            ...
            print(f"[v91] record_correction drag impossible : {exc}")
        ...
        print(
            f"[v91] drag move | id={after.get('id')} | "
            f"step {before.get('x_step')} -> {after.get('x_step')} | "
        ...
        try:
            ...
            print(f"[v91] audition après drag impossible : {exc}")
...
def v51_build_ui(self):
    ...
    try:
        ...
        print(f"[v91] bind souris impossible : {exc}")
    ...
    try:
        ...
        print(f"[v91] bind Suppr impossible : {exc}")
...
def v52_normalize_loaded_notes(self, notes):
    ...
    for raw in sorted(notes, key=lambda n: (int(float(n.get("x_step", 0))), int(float(n.get("pair", 0))))):
        ...
        if pair not in valid_pairs:
            print(f"[v91] slice ignorée car absente du break actuel : pair={pair}")
            continue
...
def v52_load_saved_beat(self, event=None):
    ...
    if not saves:
        ...
        print("[v91] aucune sauvegarde trouvée")
        return "break"
    ...
    if not pattern:
        ...
        print("[v91] sauvegarde incompatible :", chosen["path"])
        return "break"
    ...
    print("")
    print("[v91] BEAT RESTAURÉ")
    print("[v91] fichier :", chosen["path"])
    print("[v91] clé :", chosen["key"])
    print("[v91] notes :", len(pattern))
    print("")
...
def v52_list_saved_beats(self, event=None):
    ...
    print("")
    print("[v91] SAUVEGARDES TROUVÉES")
    for i, save in enumerate(saves[:30]):
        print(
            f"[v91] {i:02d} | notes={len(save['notes']):02d} | "
...
def v52_build_ui(self):
    ...
    try:
        ...
        print(f"[v91] UI load saved impossible : {exc}")
...
def v53_run_training(self):
    ...
    if not trainer.exists():
        ...
        print("[v91] trainer introuvable :", trainer)
        return False

    print("")
    print("[v91] AUTO-TRAIN IA après Save...")
    print("")
...
def v53_save(self):
    ...
    try:
        ...
        print(f"[v91] auto-train après save impossible : {exc}")
...
def v53_build_ui(self):
    ...
    try:
        ...
        print(f"[v91] UI auto-train impossible : {exc}")
...
def v55_mouse_up(self, event):
    ...
    if not state.get("moved"):
        ...
        try:
            ...
            print(f"[v91] audition clic note impossible : {exc}")
        ...
        print(
            f"[v91] click play/select | id={item.get('id')} | "
            f"step={item.get('x_step')} | pair={item.get('pair')}"
        )
...
def v58_add_item_at_xy(self, x, y):
    ...
    print(
        f"[v91] add | id={item['id']} | step={x_step} | "
...
def v58_toggle_selected_length(self, event=None):
    ...
    try:
        ...
        print(f"[v91] record_correction length impossible : {exc}")

    v58_refresh_after_edit(self, "toggle_note_length")

    v58_status(self, f"Longueur note : {old_len} -> {new_len} case(s).")
    print(f"[v91] length toggle | id={item.get('id')} | {old_len} -> {new_len}")
...
def v58_duplicate_selected_forward(self, event=None):
    ...
    try:
        ...
        print(f"[v91] audition duplicate impossible : {exc}")
...
def v58_build_ui(self):
    ...
    try:
        ...
        print(f"[v91] UI demi-grille impossible : {exc}")
    ...
    try:
        ...
        print(f"[v91] bind demi-grille impossible : {exc}")
...
def v59_rank_current_slices(self):
    ...
    for pair in self.pair_values:
        ...
        try:
            ...
            print(f"[v91] analyse pair {pair} impossible : {exc}")
            role = "other"
    ...
    print("")
    print("[v91] TOP ROLES CURRENT BREAK")
    for r in ("kick", "snare", "hat"):
        print(f"[v91] {r:5s}:", [(x["pair"], round(x["score"], 3)) for x in ranks[r][:6]])
    print("")
...
def v59_load_role_model():
    ...
    try:
        ...
        print(f"[v91] lecture modèle rôle impossible : {exc}")
        return None
...
def v59_generate_role_aware(self, event=None):
    ...
    if model is None:
        ...
        print("[v91] modèle rôle absent, fallback musical snare 6.")
        model = {
            "breaks": {},
            "global": {},
            "patterns_used": 0,
    ...
    print("")
    print("[v91] ROLE-AWARE GENERATION")
    print("[v91] model patterns_used:", model.get("patterns_used", 0))
    for item in pattern:
        print(
            f"[v91] step {item['x_step']:02d}/{item['x_step'] + item['length'] - 1:02d} | "
...
def v59_run_role_training(self, event=None):
    ...
    try:
        ...
        print(f"[v91] training rôle impossible : {exc}")
...
def v59_save(self):
    ...
    try:
        ...
        print(f"[v91] auto-training rôle après save impossible : {exc}")
...
def v59_build_ui(self):
    ...
    try:
        ...
        print(f"[v91] UI role-aware impossible : {exc}")
...
def v62_add_item_at_xy(self, x, y):
    ...
    print(f"[v91] add | id={item['id']} | step={x_step} | len={length} | pair={pair}")
...
def v62_mouse_down(self, event):
    ...
    if item is not None:
        ...
        print(
            f"[v91] mouse down {state['mode']} | id={item.get('id')} | "
            f"step={item.get('x_step')} | len={item.get('length')} | pair={item.get('pair')}"
        )
    else:
        print("[v91] mouse down empty")
    ...
    if mode == "resize":
        if not state.get("moved"):
            ...
            try:
                ...
                print(f"[v91] audition clic resize impossible : {exc}")
        ...
        if before != after:
            try:
                ...
                print(f"[v91] record_correction resize impossible : {exc}")
            ...
            try:
                ...
                print(f"[v91] audition après resize impossible : {exc}")
    ...
    if mode == "move":
        ...
        if before != after:
            try:
                ...
                print(f"[v91] record_correction move impossible : {exc}")
            ...
            try:
                ...
                print(f"[v91] audition après move impossible : {exc}")
...
def v62_toggle_selected_length(self, event=None):
    ...
    try:
        ...
        print(f"[v91] record_correction toggle length impossible : {exc}")

    v62_refresh_after_edit(self, "toggle_note_length_v62")
    v62_status(self, f"Longueur note : {old_len} → {new_len} case(s).")

    print(f"[v91] toggle length | id={item.get('id')} | {old_len} -> {new_len}")
...
def v62_draw(self):
    ...
    try:
        ...
        print(f"[v91] draw handles impossible : {exc}")
...
def v62_build_ui(self):
    ...
    try:
        ...
        print(f"[v91] bind resize impossible : {exc}")
    ...
    try:
        ...
        print(f"[v91] UI resize impossible : {exc}")
...
def v63_load_model():
    ...
    try:
        ...
        print(f"[v91] lecture modèle strict impossible : {exc}")
        return None
...
def v63_generate_safe_role_ai(self, event=None):
    ...
    print("")
    print("[v91] SAFE ROLE AI GENERATED")
    print("[v91] model strict:", "yes" if model else "no")
    print("[v91] snare main/alt:", main_snare, alt_snare)
    print("[v91] kick  main/alt:", main_kick, alt_kick)
    print("[v91] hat   main/alt:", main_hat, alt_hat)
    print("[v91] pattern:")
    for item in pattern:
        print(
            f"[v91] step {item['x_step']:02d}/{item['x_step'] + item['length'] - 1:02d} | "
...
def v63_run_strict_training(self, event=None):
    ...
    print("")
    print("[v91] TRAIN STRICT ROLE MODEL...")
    print("")
    ...
    try:
        ...
        print(f"[v91] training strict impossible : {exc}")
...
def v63_build_ui(self):
    ...
    try:
        ...
        print(f"[v91] UI safe impossible : {exc}")
...
def v65_rank_slices(self):
    ...
    for pair in self.pair_values:
        ...
        try:
            ...
            print(f"[v91] analyse slice {pair} impossible : {exc}")
            scores = {
                "kick": 0.0,
                "snare": 0.0,
                "hat": 0.0,
    ...
    print("")
    print("[v91] TOP SLICES PAR ROLE — break courant")
    for role in ("kick", "snare", "hat", "ghost_snare"):
        print(f"[v91] {role:12s}:", [(x["pair"], round(x["score"], 3)) for x in ranks[role][:8]])
    print("")
...
def v65_generate_from_role_template(self, event=None):
    ...
    try:
        ...
        print(f"[v91] template introuvable : {exc}")
        return "break"
    ...
    print("")
    print("[v91] CROSS-SONG ROLE TEMPLATE GENERATED")
    print("[v91] template:", V65_TEMPLATE_PATH)
    print("[v91] temperature:", temperature, "| hats:", hat_density, "| ghosts:", ghost_density)
    print("[v91] pattern:")
    for item in pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v91] step {item['x_step']:02d}/{end:02d} | "
...
def v67_train_strict_if_available(self):
    ...
    if trainer is None:
        print("[v91] aucun trainer trouvé.")
        return False

    print("")
    print("[v91] TRAIN après validation :", trainer)
    print("")
    ...
    try:
        ...
        print(f"[v91] training impossible : {exc}")
        return False
...
def v67_generate_candidate(self, event=None):
    ...
    print("")
    print("[v91] CANDIDATE GENERATED")
    print("[v91] break:", self.v67_candidate_safe)
    print("[v91] notes:", len(candidate))
    print("[v91] Maintenant : corrige puis clique Good après modifs, ou Reject.")
    print("")
...
def v67_accept_corrected(self, event=None):
    ...
    print("")
    print("[v91] ACCEPTED GOOD AFTER EDIT")
    print("[v91] saved:", validated_path)
    print("[v91] trained:", trained)
    print("")
...
def v67_accept_without_training(self, event=None):
    ...
    print("[v91] accepted no-train:", validated_path)
...
def v67_build_ui(self):
    ...
    try:
        ...
        print(f"[v91] UI feedback impossible : {exc}")
...
def v68_generate_candidate(self, event=None):
    ...
    print("[v91] Length policy appliquée : 2 cases par défaut, 1 case rare.")
...
def v69_force_pattern_hard_2_grid(self, reason="v69_force_hard_2_grid"):
    ...
    print("")
    print("[v91] HARD 2 GRID APPLIQUÉ")
    for item in self.pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v91] step {item['x_step']:02d}/{end:02d} | "
...
def v70_duplicate_selected_forward(self, event=None):
    ...
    print("")
    print("[v91] CTRL+D DUPLICATE +2")
    print(f"[v91] old_id={item.get('id')} new_id={new_item.get('id')}")
    print(f"[v91] step {old_x}/{old_x + length - 1} -> {new_x}/{new_x + length - 1}")
    print(f"[v91] pair={new_item.get('pair')} len={length}")
    print("")
...
def v70_build_ui(self):
    ...
    try:
        ...
        print(f"[v91] bind Ctrl+D impossible : {exc}")
...
def v71_rank_slices(self):
    ...
    print("")
    print("[v91] PRE-LEARNING TOP SLICES")
    for role in ("kick", "snare", "hat", "ghost_snare"):
        print(f"[v91] {role:12s}:", [(x["pair"], round(x["score"], 3)) for x in ranks[role][:8]])
    print("")
...
def v71_prelearn_roles(self, force=False):
    ...
    print("")
    print("[v91] PRELEARN ROLES POUR", safe)
    for role in ("kick", "snare", "hat", "ghost_snare", "bad"):
        print(f"[v91] {role:12s}:", roles.get(role, []))
    print("")
...
def v71_generate_full_candidate(self, event=None):
    ...
    print("")
    print("[v91] FULL CANDIDATE GENERATED")
    print("[v91] break:", v71_safe(self))
    print("[v91] roles:", roles)
    print("[v91] pattern:")
    for item in pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
...
def v72_open_break(self, safe=None):
    ...
    print("")
    print("[v91] OPEN BREAK")
    print("[v91] current:", current)
    print("[v91] next   :", safe)
    print("[v91] cmd    :", " ".join(cmd))
    print("")
...
def v72_build_ui(self):
    ...
    try:
        ...
        print(f"[v91] UI break switcher impossible : {exc}")
...
def v73_sanitize_tracker_pattern(self, reason="v73_tracker_strict_no_overlap"):
    ...
    print("")
    print("[v91] TRACKER STRICT NO OVERLAP")
    print("[v91] kept:", len(accepted), "| rejected overlaps:", len(rejected))

    for item in accepted:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v91] KEEP step {item['x_step']:02d}/{end:02d} | "
    ...
    if rejected:
        ...
        for item in rejected:
            ...
            print(
                f"[v91] DROP step {item['x_step']:02d}/{end:02d} | "
                f"role={item.get('v73_role_key')} | pair={item.get('pair')}"
            )
...
def v81_launch_break(self, source):
    ...
    print("")
    print("[v91] CHARGER BREAK")
    print("[v91] courant :", current)
    print("[v91] nouveau :", source)
    print("[v91] cmd     :", " ".join(cmd))
    print("")
...
def v81_patch_charger_break_buttons(self):
    ...
    print(f"[v91] boutons Charger break patchés : {patched}")
...
def v81_generate_candidate(self, event=None):
    ...
    print("")
    print("[v91] GENERATE CANDIDATE")
    print("[v91] break chargé :", current)
    print("[v91] générateur   :", getattr(fn, "__name__", None))
    print("")
...
def v81_run_training(self):
    ...
    if script is None:
        print("[v91] Aucun script training trouvé.")
...
def v81_save_feedback(self, verdict, train_now=False):
    ...
    print("")
    print("[v91] FEEDBACK SAUVÉ")
    print("[v91] break  :", current)
    print("[v91] verdict:", verdict)
    print("[v91] train  :", train_now)
    print("[v91] fichier:", out_path)
    print("")
...
def v81_build_ui(self):
    ...
    try:
        self.root.title(f"BreakbeatAI v91 — anti straight line")
    except Exception:
        pass
    ...
    try:
        ...
        print(f"[v91] patch Charger break impossible : {exc}")

    try:
        v81_install_training_toolbar(self)
    except Exception as exc:
        print(f"[v91] toolbar impossible : {exc}")
...
def v82_score_pair(self, pair):
    ...
    attack = y[:min(len(y), int(sr * 0.12))]
    ...
    ghost = 0.7 * snare + 0.4 * high_r - 0.15 * rms
...
def v82_auto_role_pools(self):
    ...
    print("")
    print("[v91] ROLE POOLS")
    for r in ["kick", "snare", "hat", "ghost_snare", "bad"]:
        print(f"[v91] {r:12s}:", roles.get(r, []))
    print("")
...
def v82_generate_candidate(self, event=None):
    ...
    print("")
    print("[v91] NEW CANDIDATE")
    print("[v91] break :", current)
    print("[v91] nonce :", nonce)
    print("[v91] rejected known:", len(rejected))
    print("[v91] fingerprint rejeté ?: ", chosen_fp in rejected)
    for item in self.pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v91] step {item['x_step']:02d}/{end:02d} "
...
def v82_patch_training_buttons(self):
    ...
    print(f"[v91] boutons training patchés : {patched}")
...
def v82_build_ui(self):
    ...
    try:
        ...
        print(f"[v91] patch boutons impossible : {exc}")
...
# ---------------------------------------------------------------------
# v83 EXTENSION : retient kick/hat/snare/hat
# ---------------------------------------------------------------------

V83_LEARNING_DIR = Path("dataset/learning")
V83_ROLE_MODEL_PATH = V83_LEARNING_DIR / "role_sequence_model_v01.json"
V83_OVERRIDES_PATH = V83_LEARNING_DIR / "break_role_overrides_v01.json"

V83_DEFAULT_SEQUENCE = [
    "kick", "hat", "snare", "hat",
    "hat", "kick", "snare", "hat",
    "kick", "hat", "snare", "hat",
    "hat", "kick", "snare", "hat",
...
def v83_load_json(path, default):
    path = Path(path)
    if not path.exists():
        return default
...
def v83_role_from_item(item):
    raw = " ".join([
        str(item.get("learned_role", "")),
        str(item.get("prior_label", "")),
        str(item.get("v73_role_key", "")),
        str(item.get("v69_role_key", "")),
        str(item.get("role", "")),
    ]).lower()
...
def v83_pattern_to_sequence(pattern):
    seq = ["rest"] * 16

    for item in pattern or []:
        role = v83_role_from_item(item)
        ...
        slot = max(0, min(15, x // 2))
        seq[slot] = role

    return seq


def v83_sequence_valid(seq):
    if not isinstance(seq, list) or len(seq) != 16:
        return False

    hits = [x for x in seq if x != "rest"]
...
def v83_load_sequence_model():
    data = v83_load_json(
        V83_ROLE_MODEL_PATH,
        {
            "version": "role_sequence_model_v01",
            "updated_at": None,
            "global_sequences": [],
            "breaks": {},
        },
    ...
    data.setdefault("global_sequences", [])
    data.setdefault("breaks", {})

    return data


def v83_pick_sequence(self):
    current = v83_current_break(self)
    model = v83_load_sequence_model()

    choices = []

    br = model.get("breaks", {}).get(current, {})
    for entry in br.get("sequences", []):
        seq = entry.get("sequence")
        if v83_sequence_valid(seq):
            choices.append(("break", seq))

    for entry in model.get("global_sequences", []):
        seq = entry.get("sequence")
        if v83_sequence_valid(seq):
            choices.append(("global", seq))

    if choices:
        source, seq = random.choice(choices[-40:])
        print("[v91] séquence choisie :", source, " ".join(seq))
        return list(seq)

    print("[v91] séquence par défaut :", " ".join(V83_DEFAULT_SEQUENCE))
    return list(V83_DEFAULT_SEQUENCE)


def v83_learn_current_sequence(self, source="good"):
    current = v83_current_break(self)
    pattern = getattr(self, "pattern", []) or []
    seq = v83_pattern_to_sequence(pattern)

    if not v83_sequence_valid(seq):
        print("[v91] séquence pas assez claire, ignorée :", seq)
        return None
    ...
    model["global_sequences"].append(entry)
    model["global_sequences"] = model["global_sequences"][-500:]

    br = model["breaks"].setdefault(current, {})
    br.setdefault("sequences", [])
    br["sequences"].append(entry)
    br["sequences"] = br["sequences"][-100:]
    br["updated_at"] = now

    model["updated_at"] = now

    v83_save_json(V83_ROLE_MODEL_PATH, model)

    print("")
    print("[v91] SÉQUENCE APPRISE")
    print("[v91] break :", current)
    print("[v91] seq   :", " ".join(seq))
    print("")

    return seq


def v83_load_role_pools(self):
    current = v83_current_break(self)
    ...
    data = v83_load_json(V83_OVERRIDES_PATH, {"breaks": {}})
    br = data.get("breaks", {}).get(current)

    if br is None:
        for name, obj in data.get("breaks", {}).items():
            if v83_slug(name).lower() == v83_slug(current).lower():
                br = obj
                break
    ...
    fallback = [x for x in fallback if x not in bad] or fallback

    for role in ["kick", "snare", "hat", "ghost_snare"]:
        if not roles[role]:
            roles[role] = fallback[:]

    if not roles["ghost_snare"]:
        roles["ghost_snare"] = roles["snare"][:]
...
def v83_pick_pair(rng, pool):
    pool = [int(x) for x in pool]

    if not pool:
        return 0

    limit = min(len(pool), rng.choice([1, 2, 3, 4, len(pool)]))
    limit = max(1, limit)

    return int(rng.choice(pool[:limit]))


def v83_make_note(self, step, pair, role, nonce):
    step = int(step)
    ...
    return {
        ...
        "hit_slot": int(step) // 2,
        "ai_generated": True,
        "ai_model": "v83_learn_role_pattern",
        "learned_role": str(role),
        "prior_label": str(role),
        "v83_role_sequence": True,
        "v83_nonce": int(nonce),
    }


def v83_generate_candidate(self, event=None):
    current = v83_current_break(self)
    ...
    nonce = int(getattr(self, "v83_nonce", 0)) + 1
    self.v83_nonce = nonce

    rng = random.Random(time.time_ns() + nonce * 10007)

    seq = v83_pick_sequence(self)
    roles = v83_load_role_pools(self)

    pattern = []
    previous_pair = None

    for slot, role in enumerate(seq[:16]):
        if role == "rest":
            continue

        if role not in ["kick", "snare", "hat", "ghost_snare"]:
            role = "hat"

        pool = roles.get(role, [])

        if role == "ghost_snare" and not pool:
            pool = roles.get("snare", [])

        pair = v83_pick_pair(rng, pool)

        if previous_pair is not None and pair == previous_pair and len(pool) > 1:
            for _ in range(8):
                alt = v83_pick_pair(rng, pool)
                if alt != previous_pair:
                    pair = alt
                    break
        ...
        pattern.append(v83_make_note(self, slot * 2, pair, role, nonce))
    ...
    try:
        self.write_latest_pattern(reason="v83_generate_from_role_sequence")
    except Exception as exc:
        print(f"[v91] write_latest_pattern impossible : {exc}")
    ...
    v83_status(self, f"Candidate v83 générée avec phrase apprise sur {current}.")
    return "break"


_old_v83_good_after = getattr(SliceIndexTracker, "good_after_modifs_v82", None) or getattr(SliceIndexTracker, "good_after_modifs_v81", None)
_old_v83_good_no_train = getattr(SliceIndexTracker, "good_no_train_v82", None) or getattr(SliceIndexTracker, "good_no_train_v81", None)


def v83_good_after_modifs(self, event=None):
    v83_learn_current_sequence(self, source="good_after_modifs")

    if callable(_old_v83_good_after):
        return _old_v83_good_after(self, event)

    v83_status(self, "Good après modifs : séquence apprise.")
    return "break"


def v83_good_no_train(self, event=None):
    v83_learn_current_sequence(self, source="good_no_train")

    if callable(_old_v83_good_no_train):
        return _old_v83_good_no_train(self, event)

    v83_status(self, "Good sans train : séquence apprise.")
    return "break"


def v83_remember_default(self, event=None):
    model = v83_load_sequence_model()
    now = time.strftime("%Y-%m-%d %H:%M:%S")
    current = v83_current_break(self)
    ...
    entry = {
        "created_at": now,
        "break": current,
        "source": "button_remember_default_v83",
        "sequence": list(V83_DEFAULT_SEQUENCE),
    }
    ...
    v83_status(self, "Pattern kick hat snare hat / hat kick snare hat mémorisé.")
    print("[v91] default remembered:", " ".join(V83_DEFAULT_SEQUENCE))
    return "break"


def v83_all_widgets(widget):
    out = [widget]
    ...
    for child in children:
        out.extend(v83_all_widgets(child))

    return out


def v83_patch_buttons(self):
    patched = 0

    for widget in v83_all_widgets(self.root):
        try:
            text = str(widget.cget("text")).strip().lower()
        except Exception:
            continue
        ...
        try:
            if text == "generate candidate":
                ...
                widget.configure(command=self.good_after_modifs_v83)
                patched += 1
            elif text == "good sans train":
                widget.configure(command=self.good_no_train_v83)
                patched += 1
        except Exception:
            pass

    print(f"[v91] boutons patchés : {patched}")


_old_v83_build_ui = SliceIndexTracker.build_ui


def v83_build_ui(self):
    _old_v83_build_ui(self)

    try:
        v83_patch_buttons(self)
    except Exception as exc:
        print(f"[v91] patch boutons impossible : {exc}")

    try:
        panel = tk.Frame(self.root, bg="#160b1e", bd=2, relief="ridge")
        panel.place(relx=1.0, x=-8, y=56, anchor="ne")
        panel.lift()

        tk.Button(
            panel,
            text="Remember kick/hat/snare/hat",
            command=self.remember_default_v83,
            bg="#4c3a73",
            fg="#ffffff",
        ...
        tk.Label(
            panel,
            text="v83 : Generate utilise l'ordre appris",
            bg="#160b1e",
            fg="#f5d67b",
        ).pack(side="left", padx=6, pady=5)
        ...
        print(f"[v91] panel impossible : {exc}")

    v83_status(self, "v83 : kick/hat/snare/hat mémorisé + généré.")


SliceIndexTracker.generate_candidate = v83_generate_candidate
SliceIndexTracker.generate_full_candidate = v83_generate_candidate
SliceIndexTracker.generate_ai_pattern = v83_generate_candidate
SliceIndexTracker.generate_cross_song_template = v83_generate_candidate
SliceIndexTracker.generate_safe_experiment = v83_generate_candidate
SliceIndexTracker.generate_locked_roles = v83_generate_candidate
SliceIndexTracker.generate_role_aware = v83_generate_candidate
SliceIndexTracker.generate_safe_role_ai = v83_generate_candidate

SliceIndexTracker.good_after_modifs_v83 = v83_good_after_modifs
SliceIndexTracker.good_no_train_v83 = v83_good_no_train
SliceIndexTracker.remember_default_v83 = v83_remember_default

SliceIndexTracker.build_ui = v83_build_ui



# ---------------------------------------------------------------------
# v84 EXTENSION : génération stricte avec roles détectés, pas de sample random
# ---------------------------------------------------------------------

V84_OVERRIDES_PATH = Path("dataset/learning/break_role_overrides_v01.json")
V84_DETECTOR_SCRIPT = Path("pipeline/05_prelearn_roles_all_breaks_v02_better_detection.py")


def v84_status(self, text):
    try:
        if hasattr(self, "set_status"):
            self.set_status(text)
        else:
...
def v84_load_json(path, default):
    try:
...
def v84_role_sequence(self):
    # Si v83 existe, on garde sa mémoire kick/hat/snare/hat.
    fn = globals().get("v83_pick_sequence")
    if callable(fn):
        return fn(self)
...
def v84_load_strict_roles(self):
    current = v84_current_break(self)
    ...
    data = v84_load_json(V84_OVERRIDES_PATH, {"breaks": {}})
    br = data.get("breaks", {}).get(current)

    if br is None:
        for name, obj in data.get("breaks", {}).items():
            if v84_norm(name) == v84_norm(current):
                br = obj
                break
    ...
    confidence = br.get("role_confidence_v84", {})
    ...
    for role in ["kick", "snare", "hat", "ghost_snare"]:
        roles[role] = [p for p in roles[role] if p not in bad]

    return roles, confidence


def v84_pick_pair(rng, pool):
    pool = [int(x) for x in pool]
    if not pool:
        return None

    # Biais vers les meilleurs, mais variation entre les bons candidats.
    limit = min(len(pool), rng.choice([1, 2, 3, len(pool)]))
    limit = max(1, limit)
    return int(rng.choice(pool[:limit]))


def v84_make_note(self, step, pair, role, nonce):
    step = int(step)
    ...
    return {
        ...
        "hit_slot": int(step) // 2,
        "ai_generated": True,
        "ai_model": "v84_strict_role_detection",
        "learned_role": str(role),
        "prior_label": str(role),
        "v84_strict_roles": True,
    }


def v84_generate_candidate(self, event=None):
    """
    Génère uniquement avec les pools kick/snare/hat détectés.
    Aucun fallback sur toutes les slices.
    """
    current = v84_current_break(self)
    roles, confidence = v84_load_strict_roles(self)
    ...
    if missing:
        v84_status(
            ...
            "Rôles manquants : "
            + ", ".join(missing)
            + ". Lance Detect Roles Current ou le prelearn v02."
        )
        print("[v91] ANNULÉ : rôles manquants", missing)
        return "break"
    ...
    nonce = int(getattr(self, "v84_nonce", 0)) + 1
    self.v84_nonce = nonce
    rng = random.Random(time.time_ns() + nonce * 9001)

    seq = v84_role_sequence(self)
    ...
    for slot, role in enumerate(seq[:16]):
        ...
        if role == "ghost_snare":
            pool = roles.get("ghost_snare") or roles.get("snare")
        else:
            pool = roles.get(role)

        if not pool:
            # Important : pas de random sample.
            print(f"[v91] slot {slot} ignoré, aucun pool pour rôle {role}")
            continue

        pair = v84_pick_pair(rng, pool)
        ...
        if previous_pair is not None and pair == previous_pair and len(pool) > 1:
            for _ in range(8):
                alt = v84_pick_pair(rng, pool)
                if alt != previous_pair:
                    pair = alt
                    break

        previous_pair = pair
        pattern.append(v84_make_note(self, slot * 2, pair, role, nonce))
    ...
    try:
        self.write_latest_pattern(reason="v84_strict_role_detection")
    except Exception as exc:
        print(f"[v91] write_latest_pattern impossible : {exc}")
    ...
    v84_status(self, f"Candidate v84 générée sans sample random sur {current}.")
    return "break"


def v84_detect_roles_current(self, event=None):
    current = v84_current_break(self)

    if not V84_DETECTOR_SCRIPT.exists():
        v84_status(self, "Script détecteur v84 introuvable.")
        return "break"

    cmd = [
        sys.executable,
        str(V84_DETECTOR_SCRIPT),
        "--source",
        current,
        "--force",
    ]
    ...
    print("")
    print("[v91] DETECT ROLES CURRENT")
    print("[v91] cmd:", " ".join(cmd))
    print("")

    try:
        subprocess.Popen(cmd, cwd=str(Path(".").resolve()))
        v84_status(self, f"Détection roles lancée pour {current}. Relance Generate après quelques secondes.")
    except Exception as exc:
        v84_status(self, f"Détection impossible : {exc}")

    return "break"


_old_v84_build_ui = SliceIndexTracker.build_ui


def v84_build_ui(self):
    _old_v84_build_ui(self)

    try:
        frame = tk.Frame(self.root, bg="#111827", bd=2, relief="ridge")
        frame.place(relx=1.0, x=-8, y=104, anchor="ne")
        frame.lift()

        tk.Button(
            frame,
            text="Detect Roles Current",
            command=self.detect_roles_current_v84,
            bg="#34506b",
            fg="#ffffff",
        ).pack(side="left", padx=6, pady=5)
        ...
        tk.Label(
            frame,
            text="v84 : kick/snare/hat stricts, pas de sample random",
            bg="#111827",
            fg="#f5d67b",
        ).pack(side="left", padx=6, pady=5)
        ...
        print(f"[v91] UI detect impossible : {exc}")

    v84_status(self, "v84 : meilleure détection kick/snare/hat + génération stricte.")


SliceIndexTracker.generate_candidate = v84_generate_candidate
SliceIndexTracker.generate_full_candidate = v84_generate_candidate
SliceIndexTracker.generate_ai_pattern = v84_generate_candidate
SliceIndexTracker.generate_cross_song_template = v84_generate_candidate
SliceIndexTracker.generate_safe_experiment = v84_generate_candidate
SliceIndexTracker.generate_locked_roles = v84_generate_candidate
SliceIndexTracker.generate_role_aware = v84_generate_candidate
SliceIndexTracker.generate_safe_role_ai = v84_generate_candidate
SliceIndexTracker.detect_roles_current_v84 = v84_detect_roles_current

SliceIndexTracker.build_ui = v84_build_ui



# ---------------------------------------------------------------------
# v85 EXTENSION : placement FIXE des rôles, aucune improvisation de grille
# ---------------------------------------------------------------------

V85_OVERRIDES_PATH = Path("dataset/learning/break_role_overrides_v01.json")

# Pattern exact demandé :
...
def v85_load_json(path, default):
    try:
        return json.loads(Path(path).read_text(encoding="utf-8"))
    except Exception:
        return default
...
def v85_load_roles(self):
    """
    Charge uniquement les rôles du break actuel.
    Pas de fallback random sur toutes les slices.
    """
    current = v85_current_break(self)
    ...
    data = v85_load_json(V85_OVERRIDES_PATH, {"breaks": {}})
    br = data.get("breaks", {}).get(current)

    if br is None:
        for name, obj in data.get("breaks", {}).items():
            if v85_norm(name) == v85_norm(current):
                br = obj
                break
    ...
    for role in ["kick", "snare", "hat", "ghost_snare"]:
        roles[role] = [p for p in roles.get(role, []) if p not in bad]

    return roles


def v85_pick_pair(self, rng, roles, role, counters):
    pool = roles.get(role, [])

    if not pool and role == "ghost_snare":
        pool = roles.get("snare", [])
    ...
    # Rotation stable + petite variation, mais uniquement dans le bon rôle.
    if len(pool) == 1:
        return int(pool[0])
...
def v85_make_note(self, step, pair, role):
    step = int(step)
    step = max(0, min(30, step))
    ...
    return {
        ...
        "hit_slot": int(step) // 2,
        "ai_generated": True,
        "ai_model": "v85_fixed_role_grid",
        "learned_role": str(role),
        "prior_label": str(role),
        "v85_fixed_grid": True,
        "v85_no_position_ai": True,
    }


def v85_generate_candidate(self, event=None):
    """
    Génération simple et stricte :
    - positions fixes
    - rôle fixe à chaque position
    - aucune superposition
    - aucune case impaire
    - aucune slice hors rôle
    """
    current = v85_current_break(self)
    roles = v85_load_roles(self)

    print("")
    print("[v91] ROLES DISPONIBLES")
    print("[v91] break:", current)
    for role in ["kick", "snare", "hat", "ghost_snare", "bad"]:
        print(f"[v91] {role:12s}:", roles.get(role))
    print("")

    missing = [r for r in ["kick", "snare", "hat"] if not roles.get(r)]

    if missing:
        print("[v91] ANNULÉ : rôles manquants :", missing)
        v85_status(
            self,
            "Rôles manquants pour ce break : "
            + ", ".join(missing)
            + ". Il faut d'abord Mark Kick/Snare/Hat ou relancer la détection."
        )
        return "break"
    ...
    nonce = int(getattr(self, "v85_nonce", 0)) + 1
    self.v85_nonce = nonce

    rng = random.Random(time.time_ns() + nonce * 1777)
    counters = {}

    pattern = []

    for step, role in V85_FIXED_GRID:
        pair = v85_pick_pair(self, rng, roles, role, counters)

        if pair is None:
            print(f"[v91] skip step {step:02d} rôle {role}, aucun sample de ce rôle.")
            continue

        pattern.append(v85_make_note(self, step, pair, role))
    ...
    try:
        self.write_latest_pattern(reason="v85_fixed_role_grid")
    except Exception as exc:
        print(f"[v91] write_latest_pattern impossible : {exc}")
    ...
    print("")
    print("[v91] GENERATED FIXED GRID")
    print("[v91] break:", current)
    for item in self.pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v91] step {item['x_step']:02d}/{end:02d} "
            f"role={item.get('learned_role'):8s} pair={item.get('pair')}"
        )
    print("")

    v85_status(self, f"v85 : placement fixe OK sur {current}.")
    return "break"


def v85_show_expected_grid(self, event=None):
    print("")
    print("[v91] GRILLE ATTENDUE")
    for step, role in V85_FIXED_GRID:
        print(f"[v91] {step:02d}/{step+1:02d} = {role}")
    print("")
    v85_status(self, "Grille attendue affichée dans le terminal.")
    return "break"


def v85_all_widgets(widget):
    out = [widget]
    try:
        children = widget.winfo_children()
    except Exception:
    ...
    for child in children:
        out.extend(v85_all_widgets(child))

    return out


def v85_patch_buttons(self):
    patched = 0

    for widget in v85_all_widgets(self.root):
        try:
            text = str(widget.cget("text")).strip().lower()
        except Exception:
            continue
    ...
    print(f"[v91] boutons Generate Candidate patchés : {patched}")


_old_v85_build_ui = SliceIndexTracker.build_ui


def v85_build_ui(self):
    _old_v85_build_ui(self)

    try:
        v85_patch_buttons(self)
    except Exception as exc:
        print(f"[v91] patch boutons impossible : {exc}")

    try:
        frame = tk.Frame(self.root, bg="#1a1020", bd=2, relief="ridge")
        frame.place(relx=1.0, x=-8, y=152, anchor="ne")
        frame.lift()

        tk.Button(
            frame,
            text="Show Fixed Grid",
            command=self.show_fixed_grid_v85,
            bg="#4c3a73",
            fg="#ffffff",
        ).pack(side="left", padx=6, pady=5)
        ...
        tk.Label(
            frame,
            text="v85 : positions verrouillées kick/hat/snare/hat",
            bg="#1a1020",
            fg="#f5d67b",
        ).pack(side="left", padx=6, pady=5)
    ...
    v85_status(self, "v85 : Generate Candidate utilise une grille fixe, pas l'IA de placement.")


SliceIndexTracker.generate_candidate = v85_generate_candidate
SliceIndexTracker.generate_full_candidate = v85_generate_candidate
SliceIndexTracker.generate_ai_pattern = v85_generate_candidate
SliceIndexTracker.generate_cross_song_template = v85_generate_candidate
SliceIndexTracker.generate_safe_experiment = v85_generate_candidate
SliceIndexTracker.generate_locked_roles = v85_generate_candidate
SliceIndexTracker.generate_role_aware = v85_generate_candidate
SliceIndexTracker.generate_safe_role_ai = v85_generate_candidate
SliceIndexTracker.show_fixed_grid_v85 = v85_show_expected_grid

SliceIndexTracker.build_ui = v85_build_ui



# ---------------------------------------------------------------------
# v86 EXTENSION : snares verrouillées aux bons endroits
# ---------------------------------------------------------------------

V86_OVERRIDES_PATH = Path("dataset/learning/break_role_overrides_v01.json")

# Règle non négociable :
# les snares sont sur 06/07, 14/15, 22/23, 30/31.
V86_SNARE_STEPS = [6, 14, 22, 30]

# Slots restants autorisés pour kick/hat/ghost.
# Aucune note ne peut être générée sur les slots snare.
V86_FILL_GRID = [
    (0,  "kick"),
    (2,  "hat"),
    (4,  "hat"),
...
def v86_load_json(path, default):
    try:
        return json.loads(Path(path).read_text(encoding="utf-8"))
    except Exception:
        return default
...
def v86_load_roles(self):
    current = v86_current_break(self)
    ...
    data = v86_load_json(V86_OVERRIDES_PATH, {"breaks": {}})
    br = data.get("breaks", {}).get(current)

    if br is None:
        for name, obj in data.get("breaks", {}).items():
            if v86_norm(name) == v86_norm(current):
                br = obj
                break
...
def v86_pick_pair(rng, roles, role, counters):
    pool = roles.get(role, [])
    ...
    # Rotation stable, variation légère, mais toujours dans le bon rôle.
    if rng.random() < 0.80:
        return int(pool[idx % len(pool)])
...
def v86_make_note(self, step, pair, role):
    step = int(step)

    # Toujours départ pair.
    if step % 2 != 0:
        step -= 1
    ...
    return {
        ...
        "hit_slot": int(step) // 2,
        "ai_generated": True,
        "ai_model": "v86_snare_anchors",
        "learned_role": str(role),
        "prior_label": str(role),
        "v86_snare_anchor": bool(role == "snare"),
        "v86_no_overlap": True,
        "v86_even_start_len_2": True,
    }


def v86_generate_candidate(self, event=None):
    current = v86_current_break(self)
    roles = v86_load_roles(self)
    ...
    missing = [r for r in ["snare", "kick", "hat"] if not roles.get(r)]
    ...
    if missing:
        ...
        v86_status(
            ...
            "Rôles manquants : "
            + ", ".join(missing)
            + ". Marque Kick/Snare/Hat ou relance la détection."
        )
        return "break"
    ...
    nonce = int(getattr(self, "v86_nonce", 0)) + 1
    self.v86_nonce = nonce

    rng = random.Random(time.time_ns() + nonce * 2027)
    counters = {}

    pattern = []

    # 1) Les snares d'abord, verrouillées.
    for step in V86_SNARE_STEPS:
        pair = v86_pick_pair(rng, roles, "snare", counters)

        if pair is None:
            print(f"[v91] ERREUR : impossible de placer snare sur {step:02d}/{step+1:02d}")
            continue

        pattern.append(v86_make_note(self, step, pair, "snare"))
    ...
    # 2) Remplissage kick/hat uniquement hors emplacements snare.
    for step, role in V86_FILL_GRID:
        if step in occupied or (step + 1) in occupied:
            print(f"[v91] skip {step:02d}/{step+1:02d}, occupé par snare.")
            continue

        pair = v86_pick_pair(rng, roles, role, counters)

        # Si un hat manque, on ne met pas un sample random.
        if pair is None:
            print(f"[v91] skip {step:02d}/{step+1:02d}, aucun sample pour rôle {role}.")
            continue

        note = v86_make_note(self, step, pair, role)
        pattern.append(note)
        occupied.add(step)
        occupied.add(step + 1)

    # Ordre visuel par grille.
    ...
    try:
        self.write_latest_pattern(reason="v86_snare_anchors")
    except Exception as exc:
        print(f"[v91] write_latest_pattern impossible : {exc}")
    ...
    print("")
    print("[v91] GENERATED — SNARE ANCHORS LOCKED")
    print("[v91] break:", current)
    for item in self.pattern:
        x = int(item["x_step"])
        end = x + int(item["length"]) - 1
        print(
            f"[v91] step {x:02d}/{end:02d} "
            f"role={item.get('learned_role'):8s} pair={item.get('pair')}"
        )
    print("")

    v86_status(self, "v86 : snares verrouillées en 06/14/22/30.")
    return "break"


def v86_show_snare_anchors(self, event=None):
    print("")
    print("[v91] SNARE ANCHORS ATTENDUES")
    for step in V86_SNARE_STEPS:
        print(f"[v91] {step:02d}/{step+1:02d} = snare")
    print("")
    v86_status(self, "Snares attendues : 06/07, 14/15, 22/23, 30/31.")
    return "break"


def v86_all_widgets(widget):
    out = [widget]
    try:
        children = widget.winfo_children()
    except Exception:
    ...
    for child in children:
        out.extend(v86_all_widgets(child))

    return out


def v86_patch_buttons(self):
    patched = 0

    for widget in v86_all_widgets(self.root):
        try:
            text = str(widget.cget("text")).strip().lower()
        except Exception:
            continue
...
_old_v86_build_ui = SliceIndexTracker.build_ui


def v86_build_ui(self):
    _old_v86_build_ui(self)

    try:
        v86_patch_buttons(self)
    except Exception as exc:
        print(f"[v91] patch boutons impossible : {exc}")

    try:
        frame = tk.Frame(self.root, bg="#1d1018", bd=2, relief="ridge")
        frame.place(relx=1.0, x=-8, y=200, anchor="ne")
        frame.lift()

        tk.Button(
            frame,
            text="Show Snare Anchors",
            command=self.show_snare_anchors_v86,
            bg="#6b3d67",
            fg="#ffffff",
        ).pack(side="left", padx=6, pady=5)
        ...
        tk.Label(
            frame,
            text="v86 : snare verrouillée sur 06/14/22/30",
            bg="#1d1018",
            fg="#f5d67b",
        ).pack(side="left", padx=6, pady=5)
    ...
    v86_status(self, "v86 : les snares sont ancrées aux bons endroits.")


SliceIndexTracker.generate_candidate = v86_generate_candidate
SliceIndexTracker.generate_full_candidate = v86_generate_candidate
SliceIndexTracker.generate_ai_pattern = v86_generate_candidate
SliceIndexTracker.generate_cross_song_template = v86_generate_candidate
SliceIndexTracker.generate_safe_experiment = v86_generate_candidate
SliceIndexTracker.generate_locked_roles = v86_generate_candidate
SliceIndexTracker.generate_role_aware = v86_generate_candidate
SliceIndexTracker.generate_safe_role_ai = v86_generate_candidate
SliceIndexTracker.show_snare_anchors_v86 = v86_show_snare_anchors

SliceIndexTracker.build_ui = v86_build_ui



# ---------------------------------------------------------------------
# v87 EXTENSION : vrai placement fixe kick/snare/hat + main samples lockés
# ---------------------------------------------------------------------

V87_OVERRIDES_PATH = Path("dataset/learning/break_role_overrides_v01.json")
V87_MAIN_LOCKS_PATH = Path("dataset/learning/main_role_locks_v87.json")

# Placement non négociable.
V87_KICK_STEPS = [0, 10, 16, 26]
V87_SNARE_STEPS = [6, 14, 22, 30]
V87_HAT_STEPS = [2, 4, 8, 12, 18, 20, 24, 28]

V87_FIXED_GRID = []
for _step in V87_KICK_STEPS:
    V87_FIXED_GRID.append((_step, "kick"))
for _step in V87_SNARE_STEPS:
    V87_FIXED_GRID.append((_step, "snare"))
for _step in V87_HAT_STEPS:
    V87_FIXED_GRID.append((_step, "hat"))
V87_FIXED_GRID = sorted(V87_FIXED_GRID, key=lambda x: x[0])


def v87_status(self, text):
    try:
        if hasattr(self, "set_status"):
            self.set_status(text)
        else:
...
def v87_load_json(path, default):
    try:
        return json.loads(Path(path).read_text(encoding="utf-8"))
    except Exception:
        return default
...
def v87_find_break_record(data, current):
    br = data.get("breaks", {}).get(current)
    ...
    for name, obj in data.get("breaks", {}).items():
        if v87_norm(name) == v87_norm(current):
            return obj, name

    return None, current


def v87_load_roles_and_confidence(self):
    current = v87_current_break(self)
    ...
    data = v87_load_json(V87_OVERRIDES_PATH, {"breaks": {}})
    br, matched_name = v87_find_break_record(data, current)
    ...
    if isinstance(br, dict):
        ...
        conf = br.get("role_confidence_v84", {})
...
def v87_save_locks(data):
    data["updated_at"] = time.strftime("%Y-%m-%d %H:%M:%S")
    v87_save_json(V87_MAIN_LOCKS_PATH, data)


def v87_get_locks_for_break(self):
    current = v87_current_break(self)
    data = v87_load_locks()
...
def v87_role_confidence_order(confidence, role):
    rows = confidence.get(role, [])
...
def v87_pick_main_pair(self, roles, confidence, locks, role):
    """
    Priorité :
    1. Lock manuel v87
    2. Meilleur score v84
    3. Premier rôle marqué/détecté
    4. Aucun fallback random
    """
    if role in locks:
        return int(locks[role]), "manual_lock_v87"

    ranked = v87_role_confidence_order(confidence, role)
    pool = [int(p) for p in roles.get(role, [])]

    for p in ranked:
        if p in pool:
            return int(p), "best_confidence_v84"

    if pool:
        return int(pool[0]), "first_role_pool"

    return None, "missing"


def v87_get_selected_pair(self):
    selected_id = getattr(self, "selected_id", None)
    ...
    if selected_id is not None:
        for item in getattr(self, "pattern", []) or []:
            try:
                if int(item.get("id")) == int(selected_id):
                    return int(item.get("pair"))
            except Exception:
                pass

    for attr in ["selected_pair", "current_pair", "audition_pair", "last_pair", "active_pair"]:
        try:
...
def v87_add_pair_to_role_overrides(self, role, pair):
    current = v87_current_break(self)
    data = v87_load_json(
        V87_OVERRIDES_PATH,
        {
    ...
    if pair not in roles[role]:
        roles[role].insert(0, pair)

    # Si on lock un rôle, on évite que le même pair reste bad.
    if pair in roles.get("bad", []):
        roles["bad"] = [x for x in roles["bad"] if int(x) != pair]

    roles[role] = sorted(set(int(x) for x in roles[role]))
    br["updated_at"] = time.strftime("%Y-%m-%d %H:%M:%S")
    br["manual_lock_v87_touched"] = True
    data["updated_at"] = time.strftime("%Y-%m-%d %H:%M:%S")

    v87_save_json(V87_OVERRIDES_PATH, data)


def v87_lock_selected_as(self, role):
    pair = v87_get_selected_pair(self)

    if pair is None:
        v87_status(self, f"Aucune note sélectionnée : impossible de lock {role}.")
        print(f"[v91] aucun pair sélectionné pour {role}")
        return "break"

    current = v87_current_break(self)
    data = v87_load_locks()
    br = data["breaks"].setdefault(current, {})
    main_roles = br.setdefault("main_roles", {})

    main_roles[role] = int(pair)
    br["updated_at"] = time.strftime("%Y-%m-%d %H:%M:%S")

    v87_save_locks(data)
    v87_add_pair_to_role_overrides(self, role, pair)

    print("")
    print("[v91] MAIN ROLE LOCK")
    print("[v91] break:", current)
    print("[v91] role :", role)
    print("[v91] pair :", pair)
    print("[v91] file :", V87_MAIN_LOCKS_PATH)
    print("")

    v87_status(self, f"{role.upper()} principal locké sur pair {pair} pour {current}.")
    return "break"


def v87_lock_kick(self, event=None):
    return v87_lock_selected_as(self, "kick")


def v87_lock_snare(self, event=None):
    return v87_lock_selected_as(self, "snare")


def v87_lock_hat(self, event=None):
    return v87_lock_selected_as(self, "hat")


def v87_make_note(self, step, pair, role, source):
    step = int(step)
    ...
    return {
        ...
        "hit_slot": int(step) // 2,
        "ai_generated": True,
        "ai_model": "v87_main_kick_snare_anchors",
        "learned_role": str(role),
        "prior_label": str(role),
        "main_role_source": str(source),
        "v87_fixed_anchor": True,
        "v87_no_position_ai": True,
        "v87_same_main_sample": True,
    }


def v87_generate_candidate(self, event=None):
    """
    Placement 100% fixe :
    kick  : 00/01, 10/11, 16/17, 26/27
    snare : 06/07, 14/15, 22/23, 30/31
    hat   : slots restants définis

    Même kick partout, même snare partout, même hat partout.
    """
    current = v87_current_break(self)
    roles, confidence = v87_load_roles_and_confidence(self)
    locks = v87_get_locks_for_break(self)

    chosen = {}

    for role in ["kick", "snare", "hat"]:
        pair, source = v87_pick_main_pair(self, roles, confidence, locks, role)
        chosen[role] = {
            "pair": pair,
            "source": source,
        }
    ...
    print("")
    print("[v91] MAIN ROLES")
    print("[v91] break:", current)
    print("[v91] locks:", locks)
    for role in ["kick", "snare", "hat"]:
        print(
            f"[v91] {role:6s}: pair={chosen[role]['pair']} "
            f"source={chosen[role]['source']} pool={roles.get(role)}"
        )
    print("")

    missing = [role for role in ["kick", "snare", "hat"] if chosen[role]["pair"] is None]

    if missing:
        v87_status(
            self,
            "Rôle principal manquant : "
            + ", ".join(missing)
            + ". Clique une note/slice puis Lock Selected Kick/Snare/Hat."
        )
        print("[v91] ANNULÉ : rôles principaux manquants", missing)
        return "break"
    ...
    for step, role in V87_FIXED_GRID:
        if step in occupied or (step + 1) in occupied:
            print(f"[v91] skip collision {step:02d}/{step+1:02d}")
            continue

        pair = chosen[role]["pair"]
        source = chosen[role]["source"]

        note = v87_make_note(self, step, pair, role, source)
        pattern.append(note)
    ...
    try:
        self.write_latest_pattern(reason="v87_main_kick_snare_anchors")
    except Exception as exc:
        print(f"[v91] write_latest_pattern impossible : {exc}")
    ...
    print("")
    print("[v91] GENERATED FIXED MAIN K/S/H")
    print("[v91] break:", current)
    for item in self.pattern:
        x = int(item["x_step"])
        end = x + int(item["length"]) - 1
        print(
            f"[v91] step {x:02d}/{end:02d} "
            f"role={item.get('learned_role'):6s} "
            f"pair={item.get('pair')} "
            f"source={item.get('main_role_source')}"
        )
    print("")

    v87_status(self, "v87 : kick/snare/hat placés fixe avec samples principaux.")
    return "break"


def v87_show_grid(self, event=None):
    print("")
    print("[v91] GRILLE FIXE")
    for step, role in V87_FIXED_GRID:
        print(f"[v91] {step:02d}/{step+1:02d} = {role}")
    print("")
    v87_status(self, "Grille v87 affichée dans le terminal.")
    return "break"


def v87_all_widgets(widget):
    out = [widget]
    try:
        children = widget.winfo_children()
    except Exception:
    ...
    for child in children:
        out.extend(v87_all_widgets(child))

    return out


def v87_patch_buttons(self):
    patched = 0

    for widget in v87_all_widgets(self.root):
        try:
            text = str(widget.cget("text")).strip().lower()
        except Exception:
            continue
...
_old_v87_build_ui = SliceIndexTracker.build_ui


def v87_build_ui(self):
    _old_v87_build_ui(self)

    try:
        v87_patch_buttons(self)
    except Exception as exc:
        print(f"[v91] patch boutons impossible : {exc}")

    try:
        frame = tk.Frame(self.root, bg="#201018", bd=2, relief="ridge")
        frame.place(relx=1.0, x=-8, y=248, anchor="ne")
        frame.lift()

        tk.Button(
            frame,
            text="Lock Selected Kick",
            command=self.lock_selected_kick_v87,
            bg="#6b4a30",
            fg="#ffffff",
        ...
        tk.Button(
            frame,
            text="Lock Selected Snare",
            command=self.lock_selected_snare_v87,
            bg="#6b303e",
            fg="#ffffff",
        ).pack(side="left", padx=4, pady=5)
        ...
        tk.Button(
            frame,
            text="Lock Selected Hat",
            command=self.lock_selected_hat_v87,
            bg="#4b5f38",
            fg="#ffffff",
        ).pack(side="left", padx=4, pady=5)
        ...
        tk.Label(
            frame,
            text="v87 : kick/snare/hat fixes + même sample principal",
            bg="#201018",
            fg="#f5d67b",
        ).pack(side="left", padx=6, pady=5)
    ...
    v87_status(self, "v87 : placement fixe kick/snare/hat, sans IA de placement.")


SliceIndexTracker.generate_candidate = v87_generate_candidate
SliceIndexTracker.generate_full_candidate = v87_generate_candidate
SliceIndexTracker.generate_ai_pattern = v87_generate_candidate
SliceIndexTracker.generate_cross_song_template = v87_generate_candidate
SliceIndexTracker.generate_safe_experiment = v87_generate_candidate
SliceIndexTracker.generate_locked_roles = v87_generate_candidate
SliceIndexTracker.generate_role_aware = v87_generate_candidate
SliceIndexTracker.generate_safe_role_ai = v87_generate_candidate

SliceIndexTracker.lock_selected_kick_v87 = v87_lock_kick
SliceIndexTracker.lock_selected_snare_v87 = v87_lock_snare
SliceIndexTracker.lock_selected_hat_v87 = v87_lock_hat
SliceIndexTracker.show_grid_v87 = v87_show_grid

SliceIndexTracker.build_ui = v87_build_ui



# ---------------------------------------------------------------------
# v88 EXTENSION : HARD ANCHORS, aucune snare ailleurs
# ---------------------------------------------------------------------

V88_OVERRIDES_PATH = Path("dataset/learning/break_role_overrides_v01.json")
V88_LOCKS_PATH = Path("dataset/learning/main_role_locks_v87.json")
...
V88_SNARE_STEPS = [6, 14, 22, 30]
V88_KICK_STEPS = [0, 10, 16, 26]


def v88_status(self, text):
    try:
        if hasattr(self, "set_status"):
            self.set_status(text)
        else:
...
def v88_load_json(path, default):
    try:
        return json.loads(Path(path).read_text(encoding="utf-8"))
    except Exception:
        return default
...
def v88_find_record(data, current):
    br = data.get("breaks", {}).get(current)
    if br is not None:
        return br

    for name, obj in data.get("breaks", {}).items():
        if v88_norm(name) == v88_norm(current):
            return obj

    return None


def v88_load_roles(self):
    current = v88_current_break(self)
    ...
    data = v88_load_json(V88_OVERRIDES_PATH, {"breaks": {}})
    br = v88_find_record(data, current)
...
def v88_load_locks(self):
    current = v88_current_break(self)
    data = v88_load_json(V88_LOCKS_PATH, {"breaks": {}})
    br = v88_find_record(data, current)
...
def v88_pick_main_pair(roles, locks, role):
    if role in locks:
        return int(locks[role]), "manual_lock"

    pool = roles.get(role, [])

    if pool:
        return int(pool[0]), "role_pool_first"

    return None, "missing"


def v88_make_note(self, step, pair, role, source):
    step = int(step)
    ...
    return {
        ...
        "hit_slot": int(step) // 2,
        "ai_generated": True,
        "ai_model": "v88_hard_anchors",
        "learned_role": str(role),
        "prior_label": str(role),
        "main_role_source": str(source),
        "v88_hard_anchor": True,
        "v88_no_position_ai": True,
    }


def v88_generate_hard(self, event=None):
    current = v88_current_break(self)
    roles = v88_load_roles(self)
    locks = v88_load_locks(self)

    chosen = {}

    for role in ["kick", "snare", "hat"]:
        pair, source = v88_pick_main_pair(roles, locks, role)
        chosen[role] = {"pair": pair, "source": source}

    print("")
    print("[v91] HARD GENERATE")
    print("[v91] break:", current)
    print("[v91] locks:", locks)
    for role in ["kick", "snare", "hat"]:
        print(
    ...
    if missing:
        v88_status(
            ...
            "Rôles manquants : "
            + ", ".join(missing)
            + ". Il faut Lock Selected Kick/Snare/Hat ou refaire la détection."
        )
        print("[v91] ANNULÉ, rôles manquants :", missing)
        return "break"
    ...
    for step, role in V88_FIXED_GRID:
        pair = chosen[role]["pair"]
        source = chosen[role]["source"]
        pattern.append(v88_make_note(self, step, pair, role, source))

    # Sécurité absolue : aucune snare hors 6/14/22/30.
    for item in pattern:
        role = item.get("learned_role")
        x = int(item.get("x_step", -1))

        if role == "snare" and x not in V88_SNARE_STEPS:
            raise RuntimeError(f"BUG v88 : snare interdite à {x}")

        if role == "kick" and x not in V88_KICK_STEPS:
            raise RuntimeError(f"BUG v88 : kick interdit à {x}")
    ...
    try:
        self.write_latest_pattern(reason="v88_hard_anchors")
    except Exception as exc:
        print(f"[v91] write_latest_pattern impossible : {exc}")
    ...
    v88_status(self, "v88 HARD : snares = 06/14/22/30, kicks = 00/10/16/26.")
    return "break"


def v88_all_widgets(widget):
    out = [widget]
    ...
    for child in children:
        out.extend(v88_all_widgets(child))

    return out


def v88_patch_buttons(self):
    patched = 0

    for widget in v88_all_widgets(self.root):
        try:
            text = str(widget.cget("text")).strip().lower()
        except Exception:
            continue
...
_old_v88_build_ui = SliceIndexTracker.build_ui


def v88_build_ui(self):
    _old_v88_build_ui(self)

    try:
        self.root.title(f"BreakbeatAI v88 HARD — {v88_current_break(self)}")
    except Exception:
        pass

    try:
        frame = tk.Frame(self.root, bg="#260014", bd=3, relief="ridge")
        frame.place(relx=0.5, y=8, anchor="n")
        frame.lift()

        tk.Label(
            frame,
            text="V88 HARD ANCHORS",
            bg="#260014",
            fg="#ff9cda",
            font=("Sans", 11, "bold"),
        ...
        tk.Label(
            frame,
            text="snare: 06/14/22/30 · kick: 00/10/16/26",
            bg="#260014",
            fg="#f5d67b",
        ).pack(side="left", padx=8, pady=6)

        self.v88_panel = frame

        def keep_patched():
            try:
                patched = v88_patch_buttons(self)
                self.v88_panel.lift()
            except Exception:
                pass

            try:
                self.root.after(500, keep_patched)
            except Exception:
                pass
    ...
    v88_status(self, "v88 HARD chargé : utilise V88 GENERATE HARD.")


SliceIndexTracker.generate_candidate = v88_generate_hard
SliceIndexTracker.generate_full_candidate = v88_generate_hard
SliceIndexTracker.generate_ai_pattern = v88_generate_hard
SliceIndexTracker.generate_cross_song_template = v88_generate_hard
SliceIndexTracker.generate_safe_experiment = v88_generate_hard
SliceIndexTracker.generate_locked_roles = v88_generate_hard
SliceIndexTracker.generate_role_aware = v88_generate_hard
SliceIndexTracker.generate_safe_role_ai = v88_generate_hard

SliceIndexTracker.build_ui = v88_build_ui



# ---------------------------------------------------------------------
# v90 EXTENSION : positions corrigées selon Vio
# ---------------------------------------------------------------------
# Pattern voulu :
# kick hat snare hat / hat kick snare hat
#
# Donc :
# snares = 04/05, 12/13, 20/21, 28/29
# kicks  = 00/01, 10/11, 16/17, 26/27
# hats   = 02/03, 06/07, 08/09, 14/15, 18/19, 22/23, 24/25, 30/31
# ---------------------------------------------------------------------

V90_OVERRIDES_PATH = Path("dataset/learning/break_role_overrides_v01.json")
V90_LOCKS_PATH = Path("dataset/learning/main_role_locks_v87.json")
...
V90_KICK_STEPS = [0, 10, 16, 26]
V90_SNARE_STEPS = [4, 12, 20, 28]
V90_HAT_STEPS = [2, 6, 8, 14, 18, 22, 24, 30]


def v90_status(self, text):
    try:
        if hasattr(self, "set_status"):
            self.set_status(text)
        else:
...
def v90_load_json(path, default):
    try:
        return json.loads(Path(path).read_text(encoding="utf-8"))
    except Exception:
        return default
...
def v90_find_record(data, current):
    br = data.get("breaks", {}).get(current)
    if br is not None:
        return br

    for name, obj in data.get("breaks", {}).items():
        if v90_norm(name) == v90_norm(current):
            return obj

    return None


def v90_fallback_pairs(self):
    try:
        pairs = [int(x) for x in self.pair_values]
    except Exception:
        pairs = [0]
...
def v90_load_roles(self):
    current = v90_current_break(self)
    ...
    data = v90_load_json(V90_OVERRIDES_PATH, {"breaks": {}})
    br = v90_find_record(data, current)
...
def v90_load_locks(self):
    current = v90_current_break(self)
    data = v90_load_json(V90_LOCKS_PATH, {"breaks": {}})
    br = v90_find_record(data, current)
...
def v90_choose_pair(self, roles, locks, role):
    if role in locks:
        return int(locks[role]), "manual_lock"
    ...
    fallback = v90_fallback_pairs(self)
    return int(fallback[0]), "fallback_pair_but_role_forced"


def v90_make_note(self, step, pair, role, source):
    step = int(step)
    ...
    return {
        ...
        "hit_slot": int(step) // 2,
        "ai_generated": True,
        "ai_model": "v90_correct_snare_positions",
        "learned_role": str(role),
        "prior_label": str(role),
        "main_role_source": str(source),
        "v90_correct_grid": True,
        "v90_overwrote_entire_pattern": True,
    }


def v90_validate_pattern(pattern):
    for item in pattern:
        x = int(item["x_step"])
        role = str(item["learned_role"])

        if role == "snare" and x not in V90_SNARE_STEPS:
            raise RuntimeError(f"BUG v90 : snare à {x}, interdit")

        if role == "kick" and x not in V90_KICK_STEPS:
            raise RuntimeError(f"BUG v90 : kick à {x}, interdit")

        if role == "hat" and x not in V90_HAT_STEPS:
            raise RuntimeError(f"BUG v90 : hat à {x}, interdit")


def v90_generate(self, event=None):
    current = v90_current_break(self)
    roles = v90_load_roles(self)
    locks = v90_load_locks(self)

    chosen = {}
    for role in ["kick", "snare", "hat"]:
        pair, source = v90_choose_pair(self, roles, locks, role)
        chosen[role] = {"pair": pair, "source": source}

    print("")
    print("[v91] GENERATE — POSITIONS CORRIGÉES")
    print("[v91] break:", current)
    print("[v91] attendu : kick hat snare hat / hat kick snare hat")
    print("[v91] locks:", locks)
    for role in ["kick", "snare", "hat"]:
        print(
            f"[v91] {role:6s}: pair={chosen[role]['pair']} "
    ...
    for step, role in V90_GRID:
        pair = chosen[role]["pair"]
        source = chosen[role]["source"]
        pattern.append(v90_make_note(self, step, pair, role, source))

    v90_validate_pattern(pattern)
    ...
    # Écrasement total : aucune ancienne candidate ne reste.
    self.pattern = pattern
    self.selected_id = self.pattern[0]["id"] if self.pattern else None
    ...
    try:
        ...
        print(f"[v91] draw impossible : {exc}")

    try:
        self.refresh_panel()
    except Exception as exc:
        print(f"[v91] refresh_panel impossible : {exc}")

    try:
        self.write_latest_pattern(reason="v90_correct_snare_positions")
    except Exception as exc:
        print(f"[v91] write_latest_pattern impossible : {exc}")
    ...
    v90_status(self, "v90 : snares 04/12/20/28, kicks 00/10/16/26.")
    return "break"


def v90_all_widgets(widget):
    out = [widget]
    ...
    for child in children:
        out.extend(v90_all_widgets(child))

    return out


def v90_patch_generate_buttons(self):
    patched = 0

    for widget in v90_all_widgets(self.root):
        try:
            text = str(widget.cget("text")).strip().lower()
        except Exception:
            continue

        if "generate" in text or "candidate" in text:
            try:
                widget.configure(command=self.generate_candidate)
                patched += 1
            except Exception:
...
_old_v90_build_ui = SliceIndexTracker.build_ui


def v90_build_ui(self):
    _old_v90_build_ui(self)
    ...
    try:
        frame = tk.Frame(self.root, bg="#102018", bd=4, relief="ridge")
        frame.place(relx=0.5, y=8, anchor="n")
        frame.lift()
        self.v90_panel = frame

        tk.Label(
            frame,
            text="V90 CORRECT GRID",
            bg="#102018",
            fg="#7dffd1",
            font=("Sans", 12, "bold"),
        ...
        tk.Label(
            frame,
            text="snare: 04/12/20/28 · kick: 00/10/16/26",
            bg="#102018",
            fg="#f5d67b",
            font=("Sans", 10, "bold"),
        ).pack(side="left", padx=10, pady=7)
        ...
        def keep_patched():
            try:
                v90_patch_generate_buttons(self)
                self.v90_panel.lift()
            except Exception:
                pass

            try:
                self.root.after(350, keep_patched)
            except Exception:
                pass
    ...
    v90_status(self, "v90 chargé : clique ✅ V90 GENERATE ou F12.")


SliceIndexTracker.generate_candidate = v90_generate
SliceIndexTracker.generate_full_candidate = v90_generate
SliceIndexTracker.generate_ai_pattern = v90_generate
SliceIndexTracker.generate_cross_song_template = v90_generate
SliceIndexTracker.generate_safe_experiment = v90_generate
SliceIndexTracker.generate_locked_roles = v90_generate
SliceIndexTracker.generate_role_aware = v90_generate
SliceIndexTracker.generate_safe_role_ai = v90_generate

SliceIndexTracker.build_ui = v90_build_ui



# ---------------------------------------------------------------------
# v91 EXTENSION : anti-ligne-droite pour Camo et autres breaks
# ---------------------------------------------------------------------
# Garde la grille correcte v90 :
# kick  = 00/10/16/26
# snare = 04/12/20/28
# hat   = 02/06/08/14/18/22/24/30
#
# Correction :
# kick, snare, hat doivent utiliser des samples/pairs différents.
# ---------------------------------------------------------------------

V91_OVERRIDES_PATH = Path("dataset/learning/break_role_overrides_v01.json")
V91_LOCKS_PATH = Path("dataset/learning/main_role_locks_v87.json")
...
V91_KICK_STEPS = [0, 10, 16, 26]
V91_SNARE_STEPS = [4, 12, 20, 28]
V91_HAT_STEPS = [2, 6, 8, 14, 18, 22, 24, 30]


def v91_status(self, text):
    try:
        if hasattr(self, "set_status"):
            self.set_status(text)
        else:
...
def v91_load_json(path, default):
    try:
        return json.loads(Path(path).read_text(encoding="utf-8"))
    except Exception:
        return default
...
def v91_find_record(data, current):
    br = data.get("breaks", {}).get(current)
    if br is not None:
        return br, current

    for name, obj in data.get("breaks", {}).items():
        if v91_norm(name) == v91_norm(current):
            return obj, name

    return None, current


def v91_all_pairs(self):
    try:
        pairs = [int(x) for x in self.pair_values]
    except Exception:
        pairs = [0]
...
def v91_load_roles_and_confidence(self):
    current = v91_current_break(self)
    ...
    data = v91_load_json(V91_OVERRIDES_PATH, {"breaks": {}})
    br, matched = v91_find_record(data, current)
...
def v91_load_locks(self):
    current = v91_current_break(self)
    data = v91_load_json(V91_LOCKS_PATH, {"breaks": {}})
    br, matched = v91_find_record(data, current)
...
def v91_confidence_order(confidence, role):
    rows = confidence.get(role, [])
...
def v91_candidates_for_role(self, roles, confidence, locks, role):
    """
    Liste de candidats pour un rôle, dans l'ordre :
    - lock manuel
    - confidence v84
    - pool de rôle
    - fallback tous pairs
    ...
    for p in v91_confidence_order(confidence, role):
        out.append((int(p), "best_confidence_v84"))

    for p in roles.get(role, []):
        out.append((int(p), "role_pool"))

    for p in v91_all_pairs(self):
        out.append((int(p), "emergency_distinct_fallback"))
...
def v91_choose_distinct_main_roles(self, roles, confidence, locks):
    """
    Évite la ligne droite :
    snare, kick, hat doivent être différents si possible.
    ...
    for role in ["snare", "kick", "hat"]:
        candidates = v91_candidates_for_role(self, roles, confidence, locks, role)
        ...
        if picked is None and candidates:
            # Cas extrême : pas assez de slices différentes.
            picked = candidates[0]

        if picked is None:
            picked = (0, "missing_fallback_zero")
...
def v91_make_note(self, step, pair, role, source):
    step = int(step)
    ...
    return {
        ...
        "hit_slot": int(step) // 2,
        "ai_generated": True,
        "ai_model": "v91_anti_straight_line",
        "learned_role": str(role),
        "prior_label": str(role),
        "main_role_source": str(source),
        "v91_correct_grid": True,
        "v91_distinct_main_roles": True,
    }


def v91_validate_pattern(pattern):
    for item in pattern:
        x = int(item["x_step"])
        role = str(item["learned_role"])

        if role == "snare" and x not in V91_SNARE_STEPS:
            raise RuntimeError(f"BUG v91 : snare à {x}, interdit")

        if role == "kick" and x not in V91_KICK_STEPS:
            raise RuntimeError(f"BUG v91 : kick à {x}, interdit")

        if role == "hat" and x not in V91_HAT_STEPS:
            raise RuntimeError(f"BUG v91 : hat à {x}, interdit")


def v91_generate(self, event=None):
    current = v91_current_break(self)
    roles, confidence = v91_load_roles_and_confidence(self)
    locks = v91_load_locks(self)
    chosen = v91_choose_distinct_main_roles(self, roles, confidence, locks)

    print("")
    print("[v91] GENERATE — ANTI LIGNE DROITE")
    print("[v91] break:", current)
    print("[v91] locks:", locks)
    for role in ["kick", "snare", "hat"]:
        print(
    ...
    if len({chosen["kick"]["pair"], chosen["snare"]["pair"], chosen["hat"]["pair"]}) < 3:
        print("[v91] WARNING : impossible d'avoir 3 pairs distincts, pas assez de candidats fiables.")
    ...
    for step, role in V91_GRID:
        pair = chosen[role]["pair"]
        source = chosen[role]["source"]
        pattern.append(v91_make_note(self, step, pair, role, source))

    v91_validate_pattern(pattern)
    ...
    try:
        self.write_latest_pattern(reason="v91_anti_straight_line")
    except Exception as exc:
        print(f"[v91] write_latest_pattern impossible : {exc}")
    ...
    v91_status(self, "v91 : grille correcte + kick/snare/hat sur lignes différentes.")
    return "break"


def v91_reset_main_locks_current(self, event=None):
    current = v91_current_break(self)
    data = v91_load_json(V91_LOCKS_PATH, {"version": "main_role_locks_v87", "breaks": {}})
    data.setdefault("breaks", {})

    removed = False

    if current in data["breaks"]:
        data["breaks"].pop(current, None)
        removed = True
    else:
        for name in list(data["breaks"].keys()):
            if v91_norm(name) == v91_norm(current):
                data["breaks"].pop(name, None)
                removed = True

    data["updated_at"] = time.strftime("%Y-%m-%d %H:%M:%S")
    v91_save_json(V91_LOCKS_PATH, data)

    print("")
    print("[v91] RESET MAIN LOCKS")
    print("[v91] break:", current)
    print("[v91] removed:", removed)
    print("")

    v91_status(self, f"Locks principaux réinitialisés pour {current}. Regénère.")
    return "break"


def v91_all_widgets(widget):
    out = [widget]
    ...
    for child in children:
        out.extend(v91_all_widgets(child))

    return out


def v91_patch_generate_buttons(self):
    patched = 0

    for widget in v91_all_widgets(self.root):
        try:
            text = str(widget.cget("text")).strip().lower()
        except Exception:
            continue
...
_old_v91_build_ui = SliceIndexTracker.build_ui


def v91_build_ui(self):
    _old_v91_build_ui(self)

    try:
        self.root.title(f"BreakbeatAI v91 — {v91_current_break(self)}")
    except Exception:
        pass

    try:
        frame = tk.Frame(self.root, bg="#101828", bd=4, relief="ridge")
        frame.place(relx=0.5, y=8, anchor="n")
        frame.lift()
        self.v91_panel = frame

        tk.Label(
            frame,
            text="V91 ANTI-LIGNE-DROITE",
            bg="#101828",
            fg="#8fe8ff",
            font=("Sans", 12, "bold"),
        ).pack(side="left", padx=10, pady=7)
        ...
        tk.Button(
            frame,
            text="Reset Main Locks Current",
            command=self.reset_main_locks_current_v91,
            bg="#544060",
            fg="#ffffff",
        ).pack(side="left", padx=8, pady=7)
        ...
        tk.Label(
            frame,
            text="snare 04/12/20/28 · kick 00/10/16/26 · rôles sur lignes différentes",
            bg="#101828",
            fg="#f5d67b",
            font=("Sans", 10, "bold"),
        ).pack(side="left", padx=10, pady=7)
        ...
        def keep_patched():
            try:
                v91_patch_generate_buttons(self)
                self.v91_panel.lift()
            except Exception:
                pass
    ...
    v91_status(self, "v91 chargé : anti-ligne-droite actif.")


SliceIndexTracker.generate_candidate = v91_generate
SliceIndexTracker.generate_full_candidate = v91_generate
SliceIndexTracker.generate_ai_pattern = v91_generate
SliceIndexTracker.generate_cross_song_template = v91_generate
SliceIndexTracker.generate_safe_experiment = v91_generate
SliceIndexTracker.generate_locked_roles = v91_generate
SliceIndexTracker.generate_role_aware = v91_generate
SliceIndexTracker.generate_safe_role_ai = v91_generate
SliceIndexTracker.reset_main_locks_current_v91 = v91_reset_main_locks_current

SliceIndexTracker.build_ui = v91_build_ui
