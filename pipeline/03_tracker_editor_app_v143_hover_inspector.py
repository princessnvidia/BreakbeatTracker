...
"""
BreakbeatAI Tracker Editor v143 — slice index
...
def warp_audio_to_length_external(audio, target_len, cache_key=None):
    ...
    if rubberband:
        try:
            ...
            print(f"[v143] rubberband échec, fallback ffmpeg/no-speed : {exc}")
    ...
    if ffmpeg:
        try:
            ...
            print(f"[v143] ffmpeg atempo échec, fallback no-speed : {exc}")
...
class SliceIndexTracker:
    def __init__(self, root, pair_json):
        ...
        self.root.title("BreakbeatAI Tracker Editor v143 — slice index")
    ...
    def detect_external_player(self):
        for name in ["pw-play", "paplay", "aplay", "ffplay"]:
            ...
            if path:
                print(f"[v143] Backend audio : {name} -> {path}")
                return name

        print("[v143] Aucun backend système trouvé, fallback sounddevice.")
        return None
    ...
    def build_initial_pattern(self, randomize=False):
        ...
        print(
            f"[v143] Génération pattern | break={self.project['safe']} | "
        ...
        for slot in range(max_items):
            ...
            print(f"[v143] slot {slot:02d} | step {x_step:02d} -> slice {pair:02d}")
    ...
    def build_ui(self):
        ...
        title = tk.Label(
            main,
            text="BreakbeatAI v143 — hover inspector",
            bg="#111018",
            fg="#ff7acc",
            font=("Sans", 20, "bold"),
        )
    ...
    def record_correction(self, event_type, before=None, after=None):
        ...
        print(f"[v143] correction sauvegardée : {event_type}")
    ...
    def play_audition_array(self, audio, label="audition"):
        ...
        audition_wav = OUT_DIR / f"{self.project['safe']}_v143_audition.wav"
        ...
        if self.external_player:
            try:
                ...
                print(f"[v143] Erreur audition {self.external_player} : {exc}")
    ...
    def on_close(self):
        ...
        """
        print("[v143] Fermeture : arrêt audio + destruction fenêtre")
    ...
    def play_audio_array(self, audio, label="audio"):
        ...
        audio = normalize(audio)
        live_wav = OUT_DIR / f"{self.project['safe']}_v143_live.wav"
        ...
        if self.external_player:
            try:
                ...
                print(f"[v143] Erreur backend {self.external_player} : {exc}")
    ...
    def render_preview_file(self):
        OUT_DIR.mkdir(parents=True, exist_ok=True)
        wav = OUT_DIR / f"{self.project['safe']}_tracker_app_v143_preview.wav"
    ...
    def write_latest_pattern(self, reason="correction"):
        ...
        path = LATEST_DIR / f"{self.project['safe']}_latest_slice_index_v143.json"

        data = {
            "version": "breakbeatai_latest_slice_index_v143",
            "reason": reason,
    ...
    def save(self):
        ...
        tracker_path = OUT_DIR / f"{self.project['safe']}_tracker_app_edit_v143.json"
        ...
        stamp = file_stamp()
        validated_path = VALIDATED_DIR / f"{self.project['safe']}_validated_slice_index_loop64_v143_{stamp}.json"

        validated = {
            "version": "breakbeatai_validated_slice_index_loop64_v143",
...
def v44_record_correction(self, event_type, before=None, after=None):
    ...
    print(f"[v143] correction sauvegardée : {event_type}")
    print(f"[v143] correction path : {corrections_path}")
    print(f"[v143] latest path : {latest_path}")
...
def v45_move_selected(self, dx, dy):
    ...
    if dy != 0 and before != after:
        try:
            ...
            print(f"[v143] audition après flèche haut/bas impossible : {exc}")
...
def v46_warp_loop_pitch_preserve(audio, target_len, cache_key):
    ...
    if rubberband:
        try:
            ...
            print(f"[v143] rubberband global warp échec : {exc}")
    ...
    if ffmpeg:
        try:
            ...
            print(f"[v143] ffmpeg global warp échec : {exc}")

    print("[v143] Aucun warp pitch-preserve dispo, fallback fit no-speed.")
    return v46_fit_audio_no_speed(audio, target_len)
...
def v46_build_initial_pattern(self, randomize=False):
    ...
    print(
        f"[v143] M8 pattern | break={self.project['safe']} | "
...
def v46_get_warped_loop(self):
    ...
    print(
        f"[v143] global warp OK | source_len={loop_end-loop_start} | "
...
def v46_build_ui(self):
    ...
    try:
        ...
        print(f"[v143] UI extension impossible : {exc}")
...
def v47_play_audio_array(self, audio, label="audio"):
    ...
    if len(audio) == 0 or v47_is_silent(audio):
        ...
        print(f"[v143] lecture annulée : audio silencieux ({label})")
        return 0
...
def v47_toggle_loop(self):
    ...
    if not getattr(self, "pattern", None):
        ...
        print("[v143] loop annulée : pattern vide")
        return "break"
...
def v47_delete_selected(self):
    ...
    if not getattr(self, "pattern", None):
        ...
        print("[v143] pattern vide après suppression : audio coupé")
...
def v48_build_initial_pattern(self, randomize=False):
    ...
    print("[v143] pattern initial forcé VIDE : aucun son ne doit jouer.")
    return []
...
def v48_load_first_slices(self):
    ...
    print(f"[v143] Load 16 slices : {len(self.pattern)} items")


def v48_debug_pattern(self, label="render"):
    print("")
    print(f"[v143] DEBUG PATTERN avant {label}")
    print(f"[v143] items = {len(getattr(self, 'pattern', []) or [])}")

    if not getattr(self, "pattern", None):
        print("[v143] pattern vide confirmé")
        print("")
        return
    ...
    for item in self.pattern:
        ...
        print(
            f"[v143] id={item_id} | step {x:02d}/{end:02d} | "
...
def v48_render_audio_with_timeline(self):
    ...
    if not getattr(self, "pattern", None):
        loop_samples = self.get_loop_samples()
        print(f"[v143] rendu silence total : pattern vide | samples={loop_samples}")
        return np.zeros(loop_samples, dtype=np.float32), []
    ...
    print(f"[v143] rendu audio peak={peak:.8f} | timeline_events={len(timeline) if timeline is not None else 'None'}")

    if timeline:
        print("[v143] timeline events:")
        for ev in timeline:
            print(f"[v143]   {ev}")
...
def v48_toggle_loop(self):
    if not getattr(self, "pattern", None):
        ...
        print("[v143] Space ignoré : pattern vide.")
        return "break"
...
def v48_build_ui(self):
    ...
    try:
        ...
        print(f"[v143] ajout UI impossible : {exc}")
...
def v49_add_item_at_xy(self, x, y):
    ...
    try:
        ...
        print(f"[v143] record_correction add impossible : {exc}")
    ...
    try:
        ...
        print(f"[v143] audition après ajout impossible : {exc}")

    print(f"[v143] add | id={item['id']} | step={x_step} | pair={pair} | lane={lane}")
...
def v49_duplicate_selected_forward(self, event=None):
    ...
    if item is None:
        ...
        print("[v143] duplicate annulé : aucune note sélectionnée")
        return "break"
    ...
    if new_x > max_x:
        ...
        print("[v143] duplicate annulé : fin de grille")
        return "break"
    ...
    try:
        ...
        print(f"[v143] record_correction duplicate impossible : {exc}")
    ...
    try:
        ...
        print(f"[v143] audition après duplicate impossible : {exc}")

    print(
        f"[v143] duplicate | old_id={item.get('id')} | new_id={new_item.get('id')} | "
...
def v49_build_ui(self):
    ...
    try:
        ...
        print(f"[v143] bind canvas click impossible : {exc}")
    ...
    try:
        ...
        print(f"[v143] bind keys impossible : {exc}")
...
def v50_load_style_model():
    ...
    try:
        ...
        print(f"[v143] impossible de charger le modèle IA : {exc}")
        return None
...
def v50_generate_ai_pattern(self, event=None):
    ...
    if model is None:
        ...
        print("[v143] modèle absent :", STYLE_MODEL_PATH)
        return "break"
    ...
    print("")
    print(f"[v143] GENERATE IA | safe={safe} | pairs={pair_values} | slots={max_slots}")
    ...
    for slot in range(max_slots):
        ...
        print(
            f"[v143] slot {slot:02d} | step {x_step:02d} | "
    ...
    try:
        ...
        print(f"[v143] write_latest_pattern impossible : {exc}")
...
def v50_build_ui(self):
    ...
    try:
        ...
        print(f"[v143] UI IA impossible : {exc}")
...
def v51_mouse_down(self, event):
    ...
    if item is not None:
        ...
        print(
            f"[v143] mouse down note | id={item.get('id')} | "
            f"step={item.get('x_step')} | pair={item.get('pair')}"
        )
    else:
        print("[v143] mouse down empty")
...
def v51_mouse_up(self, event):
    ...
    if state.get("empty"):
        ...
        print("[v143] drag depuis vide ignoré")
        return "break"
    ...
    if before != after:
        try:
            ...
            print(f"[v143] record_correction drag impossible : {exc}")
        ...
        print(
            f"[v143] drag move | id={after.get('id')} | "
            f"step {before.get('x_step')} -> {after.get('x_step')} | "
        ...
        try:
            ...
            print(f"[v143] audition après drag impossible : {exc}")
...
def v51_build_ui(self):
    ...
    try:
        ...
        print(f"[v143] bind souris impossible : {exc}")
    ...
    try:
        ...
        print(f"[v143] bind Suppr impossible : {exc}")
...
def v52_normalize_loaded_notes(self, notes):
    ...
    for raw in sorted(notes, key=lambda n: (int(float(n.get("x_step", 0))), int(float(n.get("pair", 0))))):
        ...
        if pair not in valid_pairs:
            print(f"[v143] slice ignorée car absente du break actuel : pair={pair}")
            continue
...
def v52_load_saved_beat(self, event=None):
    ...
    if not saves:
        ...
        print("[v143] aucune sauvegarde trouvée")
        return "break"
    ...
    if not pattern:
        ...
        print("[v143] sauvegarde incompatible :", chosen["path"])
        return "break"
    ...
    print("")
    print("[v143] BEAT RESTAURÉ")
    print("[v143] fichier :", chosen["path"])
    print("[v143] clé :", chosen["key"])
    print("[v143] notes :", len(pattern))
    print("")
...
def v52_list_saved_beats(self, event=None):
    ...
    print("")
    print("[v143] SAUVEGARDES TROUVÉES")
    for i, save in enumerate(saves[:30]):
        print(
            f"[v143] {i:02d} | notes={len(save['notes']):02d} | "
...
def v52_build_ui(self):
    ...
    try:
        ...
        print(f"[v143] UI load saved impossible : {exc}")
...
def v53_run_training(self):
    ...
    if not trainer.exists():
        ...
        print("[v143] trainer introuvable :", trainer)
        return False

    print("")
    print("[v143] AUTO-TRAIN IA après Save...")
    print("")
    ...
    try:
        ...
        if result.returncode != 0:
            ...
            print("[v143] auto-train échec code :", result.returncode)
            return False

        v53_status(self, "Save OK + IA entraînée automatiquement.")
        print("[v143] AUTO-TRAIN OK")
        return True

    except Exception as exc:
        v53_status(self, f"Save OK, mais auto-train impossible : {exc}")
        print("[v143] exception auto-train :", exc)
        return False
...
def v53_save(self):
    ...
    try:
        ...
        print(f"[v143] auto-train après save impossible : {exc}")
...
def v53_build_ui(self):
    ...
    try:
        ...
        print(f"[v143] UI auto-train impossible : {exc}")
...
def v55_mouse_up(self, event):
    ...
    if not state.get("moved"):
        ...
        try:
            ...
            print(f"[v143] audition clic note impossible : {exc}")
        ...
        print(
            f"[v143] click play/select | id={item.get('id')} | "
            f"step={item.get('x_step')} | pair={item.get('pair')}"
        )
...
def v58_add_item_at_xy(self, x, y):
    ...
    print(
        f"[v143] add | id={item['id']} | step={x_step} | "
...
def v58_toggle_selected_length(self, event=None):
    ...
    try:
        ...
        print(f"[v143] record_correction length impossible : {exc}")

    v58_refresh_after_edit(self, "toggle_note_length")

    v58_status(self, f"Longueur note : {old_len} -> {new_len} case(s).")
    print(f"[v143] length toggle | id={item.get('id')} | {old_len} -> {new_len}")
...
def v58_duplicate_selected_forward(self, event=None):
    ...
    try:
        ...
        print(f"[v143] audition duplicate impossible : {exc}")
...
def v58_build_ui(self):
    ...
    try:
        ...
        print(f"[v143] UI demi-grille impossible : {exc}")
    ...
    try:
        ...
        print(f"[v143] bind demi-grille impossible : {exc}")
...
def v59_rank_current_slices(self):
    ...
    for pair in self.pair_values:
        ...
        try:
            ...
            print(f"[v143] analyse pair {pair} impossible : {exc}")
            role = "other"
    ...
    print("")
    print("[v143] TOP ROLES CURRENT BREAK")
    for r in ("kick", "snare", "hat"):
        print(f"[v143] {r:5s}:", [(x["pair"], round(x["score"], 3)) for x in ranks[r][:6]])
    print("")
...
def v59_load_role_model():
    ...
    try:
        ...
        print(f"[v143] lecture modèle rôle impossible : {exc}")
        return None
...
def v59_generate_role_aware(self, event=None):
    ...
    if model is None:
        ...
        print("[v143] modèle rôle absent, fallback musical snare 6.")
        model = {
            "breaks": {},
            "global": {},
            "patterns_used": 0,
    ...
    print("")
    print("[v143] ROLE-AWARE GENERATION")
    print("[v143] model patterns_used:", model.get("patterns_used", 0))
    for item in pattern:
        print(
            f"[v143] step {item['x_step']:02d}/{item['x_step'] + item['length'] - 1:02d} | "
...
def v59_run_role_training(self, event=None):
    ...
    try:
        ...
        print(f"[v143] training rôle impossible : {exc}")
...
def v59_save(self):
    ...
    try:
        ...
        print(f"[v143] auto-training rôle après save impossible : {exc}")
...
def v59_build_ui(self):
    ...
    try:
        ...
        print(f"[v143] UI role-aware impossible : {exc}")
...
def v62_add_item_at_xy(self, x, y):
    ...
    print(f"[v143] add | id={item['id']} | step={x_step} | len={length} | pair={pair}")
...
def v62_mouse_down(self, event):
    ...
    if item is not None:
        ...
        print(
            f"[v143] mouse down {state['mode']} | id={item.get('id')} | "
            f"step={item.get('x_step')} | len={item.get('length')} | pair={item.get('pair')}"
        )
    else:
        print("[v143] mouse down empty")
    ...
    if mode == "resize":
        if not state.get("moved"):
            ...
            try:
                ...
                print(f"[v143] audition clic resize impossible : {exc}")
        ...
        if before != after:
            try:
                ...
                print(f"[v143] record_correction resize impossible : {exc}")
            ...
            try:
                ...
                print(f"[v143] audition après resize impossible : {exc}")
    ...
    if mode == "move":
        ...
        if before != after:
            try:
                ...
                print(f"[v143] record_correction move impossible : {exc}")
            ...
            try:
                ...
                print(f"[v143] audition après move impossible : {exc}")
...
def v62_toggle_selected_length(self, event=None):
    ...
    try:
        ...
        print(f"[v143] record_correction toggle length impossible : {exc}")

    v62_refresh_after_edit(self, "toggle_note_length_v62")
    v62_status(self, f"Longueur note : {old_len} → {new_len} case(s).")

    print(f"[v143] toggle length | id={item.get('id')} | {old_len} -> {new_len}")
...
def v62_draw(self):
    ...
    try:
        ...
        print(f"[v143] draw handles impossible : {exc}")
...
def v62_build_ui(self):
    ...
    try:
        ...
        print(f"[v143] bind resize impossible : {exc}")
    ...
    try:
        ...
        print(f"[v143] UI resize impossible : {exc}")
...
def v63_load_model():
    ...
    try:
        ...
        print(f"[v143] lecture modèle strict impossible : {exc}")
        return None
...
def v63_generate_safe_role_ai(self, event=None):
    ...
    print("")
    print("[v143] SAFE ROLE AI GENERATED")
    print("[v143] model strict:", "yes" if model else "no")
    print("[v143] snare main/alt:", main_snare, alt_snare)
    print("[v143] kick  main/alt:", main_kick, alt_kick)
    print("[v143] hat   main/alt:", main_hat, alt_hat)
    print("[v143] pattern:")
    for item in pattern:
        print(
            f"[v143] step {item['x_step']:02d}/{item['x_step'] + item['length'] - 1:02d} | "
...
def v63_run_strict_training(self, event=None):
    ...
    print("")
    print("[v143] TRAIN STRICT ROLE MODEL...")
    print("")
    ...
    try:
        ...
        print(f"[v143] training strict impossible : {exc}")
...
def v63_build_ui(self):
    ...
    try:
        ...
        print(f"[v143] UI safe impossible : {exc}")
...
def v65_rank_slices(self):
    ...
    for pair in self.pair_values:
        ...
        try:
            ...
            print(f"[v143] analyse slice {pair} impossible : {exc}")
            scores = {
                "kick": 0.0,
                "snare": 0.0,
                "hat": 0.0,
    ...
    print("")
    print("[v143] TOP SLICES PAR ROLE — break courant")
    for role in ("kick", "snare", "hat", "ghost_snare"):
        print(f"[v143] {role:12s}:", [(x["pair"], round(x["score"], 3)) for x in ranks[role][:8]])
    print("")
...
def v65_generate_from_role_template(self, event=None):
    ...
    try:
        ...
        print(f"[v143] template introuvable : {exc}")
        return "break"
    ...
    print("")
    print("[v143] CROSS-SONG ROLE TEMPLATE GENERATED")
    print("[v143] template:", V65_TEMPLATE_PATH)
    print("[v143] temperature:", temperature, "| hats:", hat_density, "| ghosts:", ghost_density)
    print("[v143] pattern:")
    for item in pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v143] step {item['x_step']:02d}/{end:02d} | "
...
def v65_build_ui(self):
    ...
    try:
        ...
        print(f"[v143] UI impossible : {exc}")
...
def v67_train_strict_if_available(self):
    ...
    if trainer is None:
        print("[v143] aucun trainer trouvé.")
        return False

    print("")
    print("[v143] TRAIN après validation :", trainer)
    print("")
    ...
    try:
        ...
        print(f"[v143] training impossible : {exc}")
        return False
...
def v67_generate_candidate(self, event=None):
    ...
    print("")
    print("[v143] CANDIDATE GENERATED")
    print("[v143] break:", self.v67_candidate_safe)
    print("[v143] notes:", len(candidate))
    print("[v143] Maintenant : corrige puis clique Good après modifs, ou Reject.")
    print("")
...
def v67_reject_candidate(self, event=None):
    ...
    print("")
    print("[v143] REJECTED")
    print("[v143] saved:", rejected_path)
    print("")
...
def v67_accept_corrected(self, event=None):
    ...
    print("")
    print("[v143] ACCEPTED GOOD AFTER EDIT")
    print("[v143] saved:", validated_path)
    print("[v143] trained:", trained)
    print("")
...
def v67_accept_without_training(self, event=None):
    ...
    print("[v143] accepted no-train:", validated_path)
...
def v67_build_ui(self):
    ...
    try:
        ...
        print(f"[v143] UI feedback impossible : {exc}")
...
def v68_generate_candidate(self, event=None):
    ...
    print("[v143] Length policy appliquée : 2 cases par défaut, 1 case rare.")
...
def v69_force_pattern_hard_2_grid(self, reason="v69_force_hard_2_grid"):
    ...
    print("")
    print("[v143] HARD 2 GRID APPLIQUÉ")
    for item in self.pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v143] step {item['x_step']:02d}/{end:02d} | "
...
def v70_duplicate_selected_forward(self, event=None):
    ...
    print("")
    print("[v143] CTRL+D DUPLICATE +2")
    print(f"[v143] old_id={item.get('id')} new_id={new_item.get('id')}")
    print(f"[v143] step {old_x}/{old_x + length - 1} -> {new_x}/{new_x + length - 1}")
    print(f"[v143] pair={new_item.get('pair')} len={length}")
    print("")
...
def v70_build_ui(self):
    ...
    try:
        ...
        print(f"[v143] bind Ctrl+D impossible : {exc}")
...
def v71_rank_slices(self):
    ...
    print("")
    print("[v143] PRE-LEARNING TOP SLICES")
    for role in ("kick", "snare", "hat", "ghost_snare"):
        print(f"[v143] {role:12s}:", [(x["pair"], round(x["score"], 3)) for x in ranks[role][:8]])
    print("")
...
def v71_prelearn_roles(self, force=False):
    ...
    print("")
    print("[v143] PRELEARN ROLES POUR", safe)
    for role in ("kick", "snare", "hat", "ghost_snare", "bad"):
        print(f"[v143] {role:12s}:", roles.get(role, []))
    print("")
...
def v71_generate_full_candidate(self, event=None):
    ...
    print("")
    print("[v143] FULL CANDIDATE GENERATED")
    print("[v143] break:", v71_safe(self))
    print("[v143] roles:", roles)
    print("[v143] pattern:")
    for item in pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
...
def v72_open_break(self, safe=None):
    ...
    print("")
    print("[v143] OPEN BREAK")
    print("[v143] current:", current)
    print("[v143] next   :", safe)
    print("[v143] cmd    :", " ".join(cmd))
    print("")
...
def v72_build_ui(self):
    ...
    try:
        ...
        print(f"[v143] UI break switcher impossible : {exc}")
...
def v73_sanitize_tracker_pattern(self, reason="v73_tracker_strict_no_overlap"):
    ...
    print("")
    print("[v143] TRACKER STRICT NO OVERLAP")
    print("[v143] kept:", len(accepted), "| rejected overlaps:", len(rejected))

    for item in accepted:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v143] KEEP step {item['x_step']:02d}/{end:02d} | "
    ...
    if rejected:
        ...
        for item in rejected:
            ...
            print(
                f"[v143] DROP step {item['x_step']:02d}/{end:02d} | "
                f"role={item.get('v73_role_key')} | pair={item.get('pair')}"
            )
...
def v81_launch_break(self, source):
    ...
    print("")
    print("[v143] CHARGER BREAK")
    print("[v143] courant :", current)
    print("[v143] nouveau :", source)
    print("[v143] cmd     :", " ".join(cmd))
    print("")
...
def v81_patch_charger_break_buttons(self):
    ...
    print(f"[v143] boutons Charger break patchés : {patched}")
...
def v81_generate_candidate(self, event=None):
    ...
    print("")
    print("[v143] GENERATE CANDIDATE")
    print("[v143] break chargé :", current)
    print("[v143] générateur   :", getattr(fn, "__name__", None))
    print("")
...
def v81_run_training(self):
    ...
    if script is None:
        print("[v143] Aucun script training trouvé.")
...
def v81_save_feedback(self, verdict, train_now=False):
    ...
    print("")
    print("[v143] FEEDBACK SAUVÉ")
    print("[v143] break  :", current)
    print("[v143] verdict:", verdict)
    print("[v143] train  :", train_now)
    print("[v143] fichier:", out_path)
    print("")
...
def v81_build_ui(self):
    ...
    try:
        self.root.title(f"BreakbeatAI v143 — hover inspector")
    except Exception:
        pass
    ...
    try:
        ...
        print(f"[v143] patch Charger break impossible : {exc}")

    try:
        v81_install_training_toolbar(self)
    except Exception as exc:
        print(f"[v143] toolbar impossible : {exc}")
...
def v82_auto_role_pools(self):
    ...
    print("")
    print("[v143] ROLE POOLS")
    for r in ["kick", "snare", "hat", "ghost_snare", "bad"]:
        print(f"[v143] {r:12s}:", roles.get(r, []))
    print("")
...
def v82_generate_candidate(self, event=None):
    ...
    print("")
    print("[v143] NEW CANDIDATE")
    print("[v143] break :", current)
    print("[v143] nonce :", nonce)
    print("[v143] rejected known:", len(rejected))
    print("[v143] fingerprint rejeté ?: ", chosen_fp in rejected)
    for item in self.pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v143] step {item['x_step']:02d}/{end:02d} "
...
def v82_patch_training_buttons(self):
    ...
    print(f"[v143] boutons training patchés : {patched}")
...
def v82_build_ui(self):
    ...
    try:
        ...
        print(f"[v143] patch boutons impossible : {exc}")
...
def v110_generate(self, event=None, reject_mode=False):
    ...
    print("")
    if reject_mode:
        print("[v143] REJECT/BAD -> variation premier 32 + autres lignes")
    else:
        print("[v143] GENERATE 64")
    print("[v143] break:", current)
    print("[v143] first32:", first32_id)
    print("[v143] response:", response_id)
    print("[v143] role_offset:", role_offset)
    for role in ["kick", "snare", "hat"]:
        lane = "?"
        try:
            ...
            pass
        print(f"[v143] {role:6s}: pair={chosen[role]['pair']} lane={lane} source={chosen[role]['source']}")
    print("")
    ...
    try:
        ...
        print(f"[v143] draw impossible : {exc}")
    ...
    try:
        ...
        print(f"[v143] refresh_panel impossible : {exc}")
...
def v110_render_pattern_to_audio(self):
    ...
    for item in pattern:
        ...
        if pair not in slice_map:
            skipped += 1
            print(f"[v143] skip pair introuvable : {pair}")
            continue
...
def v110_export_wav_current(self, event=None):
    try:
        ...
        print("")
        print("[v143] EXPORT WAV OK")
        print("[v143] break:", current)
        print("[v143] wav  :", out_path)
        print("[v143] json :", info_path)
        print("[v143] duration:", round(info["duration_seconds"], 3), "s")
        print("[v143] rendered:", info["rendered_notes"], "skipped:", info["skipped_notes"])
        print("")
...
def v110_space_play(self, event=None):
    ...
    try:
        ...
        if btn is not None:
            print("[v143] SPACE -> Loop / Space")
            try:
                btn.invoke()
            except Exception as exc:
                print("[v143] invoke Loop / Space impossible :", exc)
            return "break"

        print("[v143] SPACE : bouton Loop / Space introuvable.")
...
def v110_patch_buttons(self):
    ...
    print(f"[v143] boutons patchés : Generate={gen_patched}, Reject/Bad={bad_patched}")
...
def v110_build_ui(self):
    ...
    try:
        ...
        print("[v143] patch boutons impossible :", exc)

    try:
        v110_add_export_button(self)
    except Exception as exc:
        print("[v143] ajout Export WAV impossible :", exc)

    try:
        v110_install_space_bindings(self)
    except Exception as exc:
        print("[v143] bind espace impossible :", exc)
...
def v113_export(self, event=None):
    ...
    print("[v143] ERREUR : export_wav_current_v110 introuvable.")
...
def v113_place_export_button(self):
    ...
    try:
        ...
        print("[v143] bouton Export WAV visible ajouté.")
        v113_status(self, "v113 : Export WAV visible. Espace reste Loop / Space.")

    except Exception as exc:
        print("[v143] placement bouton Export WAV impossible :", exc)
        try:
            btn.place(x=760, y=32, height=30)
            btn.lift()
            self.export_button_v113 = btn
...
def v115_apply_cells_minus_25(self):
    ...
    print("")
    print("[v143] APPLICATION CASES -3.5px")
    ...
    for name in V115_WIDTH_GLOBAL_NAMES:
        try:
            ...
            if isinstance(value, (int, float)) and value > 6:
                ...
                print(f"[v143] global {name}: {value} -> {globals()[name]}")
        except Exception:
            pass
        ...
        try:
            if hasattr(self, attr):
                ...
                if isinstance(value, (int, float)) and value > 6:
                    ...
                    print(f"[v143] attr {attr}: {value} -> {getattr(self, attr)}")
        except Exception:
            pass
...
def v115_build_ui(self):
    ...
    try:
        self.root.title("BreakbeatAI v143 — hover inspector")
    except Exception:
        pass
    ...
    try:
        ...
        print("[v143] impossible d'appliquer -3.5px :", exc)
...
def v117_render_pattern_to_audio(self):
    ...
    print("")
    print("[v143] EXPORT EXACT CELLS")
    print("[v143] break:", current)
    print("[v143] pairblock:", pairblock_file)
    print("[v143] sr:", sr)
    print("[v143] median_pair_samples:", median_pair_samples)
    print("[v143] case_samples:", case_samples)
    print("[v143] export_samples:", total_samples)
    print("[v143] export_seconds:", round(total_samples / float(sr), 6))
    print("")
...
def v117_export_wav_current(self, event=None):
    try:
        ...
        print("[v143] wav  :", out_path)
        print("[v143] json :", info_path)
        print("[v143] duration:", round(info["duration_seconds"], 6), "s")
        print("[v143] case_seconds:", round(info["case_seconds"], 6))
        print("[v143] rendered:", info["rendered_notes"], "skipped:", info["skipped_notes"])
        print("")
...
def v118_current_break(self):
    ...
    if candidates:
        ...
        print("[v143] break courant:", stem, "| source:", source, "| raw:", raw)
        return stem

    print("[v143] WARNING : break courant introuvable, fallback Camo.")
    return "Camo_Break_-_3A"
...
def v118_find_best_playback_wav(since):
    ...
    print("")
    print("[v143] WAV candidats playback :")
    for score, mtime, size, wav in candidates[:12]:
        age = time.time() - mtime
        print(f"[v143] score={score:8d} age={age:7.2f}s size={size:10d} path={wav}")
    print("")
...
def v118_copy_playback_wav(self, since):
    ...
    if best is None:
        ...
        print("[v143] aucun WAV playback trouvé.")
        return "break"
    ...
    print("")
    print("[v143] EXPORT PLAYBACK OK")
    print("[v143] source :", best)
    print("[v143] export :", out)
    print("[v143] json   :", info_path)
    print("")
...
def v118_export_playback_wav(self, event=None):
    ...
    try:
        ...
        if btn is not None:
            print("[v143] déclenche Loop / Space pour générer le playback WAV...")
            btn.invoke()
        else:
            print("[v143] bouton Loop / Space introuvable, copie du dernier WAV playback existant.")
    except Exception as exc:
        print("[v143] invoke Loop / Space impossible :", exc)
...
def v121_find_best_long_wav(since):
    ...
    print("")
    print("[v143] WAV candidats longs/propre à raccourcir :")
    for row in candidates[:20]:
        age = time.time() - row["mtime"]
        print(
            f"[v143] score={row['score']:7d} "
...
def v121_export_trimmed_long(self, event=None):
    ...
    try:
        ...
        if btn is not None:
            print("[v143] déclenche Loop / Space pour générer le WAV long/propre...")
            btn.invoke()
    except Exception as exc:
        print("[v143] Loop / Space impossible :", exc)
    ...
    def do_export():
        try:
            ...
            try:
                ...
                print("[v143] ffmpeg trim impossible, fallback wave :", exc)
                ok = False
            ...
            print("")
            print("[v143] EXPORT TRIM OK")
            print("[v143] source :", src_wav)
            print("[v143] source durée :", round(src_dur, 6), "s")
            print("[v143] target durée :", round(target_seconds, 6), "s")
            print("[v143] export :", out)
            print("[v143] export durée :", round(float(out_dur or 0), 6), "s")
            print("[v143] json :", info_path)
            print("")
...
def v124_export_trimmed_long_no_json(self, event=None):
    ...
    def do_export():
        try:
            ...
            print("")
            print("[v143] EXPORT WAV ONLY OK")
            print("[v143] source :", src_wav)
            print("[v143] source durée :", round(src_dur, 6), "s")
            print("[v143] target durée :", round(target_seconds, 6), "s")
            print("[v143] export :", out)
            print("[v143] export durée :", round(float(out_dur or 0), 6), "s")
            print("[v143] aucun JSON exporté")
            print("")
...
def v125_select_lane(self, lane):
    ...
    print("")
    print(f"[v143] Slice {lane + 1} sélectionnée")
    print(f"[v143] notes sélectionnées : {ids}")
    print("")
...
def v125_redraw_after_move(self):
    try:
        ...
        print("[v143] draw impossible :", exc)
...
def v125_build_ui(self):
    ...
    if canvas is not None:
        try:
            ...
            print("[v143] bind canvas click impossible :", exc)
    ...
    try:
        ...
        print("[v143] bind flèches impossible :", exc)
...
def v126_select_slice(self, zero_index, slice_number, reason=""):
    ...
    print("")
    print("[v143] SÉLECTION SLICE")
    print("[v143] reason:", reason)
    print("[v143] Slice affichée:", slice_number)
    print("[v143] zero_index:", zero_index)
    print("[v143] pair_candidates:", pair_candidates)
    print("[v143] ids sélectionnés:", ids)
...
def v126_build_ui(self):
    ...
    if canvas is not None:
        try:
            ...
            print("[v143] bind canvas impossible :", exc)
...
def v127_select_slice(self, lane, displayed, reason=""):
    ...
    print("")
    print("[v143] SÉLECTION SLICE ALIGNÉE")
    print("[v143] reason:", reason)
    print("[v143] label affiché:", displayed)
    print("[v143] lane réelle:", lane)
    print("[v143] labels zero-based:", v127_labels_are_zero_based(self))
    print("[v143] pair_candidates:", pair_candidates)
    print("[v143] ids sélectionnés:", ids)
    for row in debug_rows:
        print("[v143] note:", row)
...
def v128_select_lane(self, lane):
    ...
    print("")
    print("[v143] MULTI-SÉLECTION LIGNE")
    print("[v143] slice/lane:", lane)
    print("[v143] ids:", ids)
    for row in debug:
        print("[v143] note:", row)
    print("")
...
def v129_draw_green_multiselect(self):
    ...
    print(f"[v143] contours verts dessinés : {drawn}")
...
def v129_draw(self, *args, **kwargs):
    ...
    try:
        ...
        print("[v143] contour vert impossible :", exc)
...
def v130_audition_after_move(self, notes):
    ...
    def do_play():
        ...
        for name in method_names:
            ...
            try:
                fn()
                print(f"[v143] audition via {name}()")
                played = True
                break
            except TypeError:
                try:
                    fn(None)
                    print(f"[v143] audition via {name}(None)")
                    played = True
                    break
                except TypeError:
                    try:
                        fn(note)
                        print(f"[v143] audition via {name}(note)")
                        played = True
                        break
                    except Exception:
                        pass
        ...
        if not played:
            ...
            if btn is not None:
                try:
                    btn.invoke()
                    print("[v143] audition via bouton Play slice")
                    played = True
                except Exception as exc:
                    print("[v143] Play slice invoke impossible :", exc)

        if not played:
            print("[v143] audition introuvable : aucune méthode Play slice trouvée")
...
def v131_generate_candidate(self, event=None, reject_mode=False):
    ...
    print("")
    print("[v143] GENERATE CANDIDATE STRICT")
    print("[v143] break:", current)
    print("[v143] sequence: kick hat snare hat | hat kick snare hat")
    print("[v143] répété sur 64 cases")
    for role in ["kick", "snare", "hat"]:
        print(f"[v143] {role:6s}: pair={chosen[role]['pair']} source={chosen[role]['source']}")
    print("")
...
def v131_build_ui(self):
    ...
    try:
        ...
        print("[v143] repatch boutons impossible :", exc)
...
def v140_capture_old_commands(self):
    ...
    for widget in v140_walk(self.root):
        ...
        if cmd:
            ...
            print(f"[v143] commande capturée : {low} -> {mapping[low]}")
...
def v140_call_old_command(self, key):
    try:
        ...
        print(f"[v143] ancienne commande {key} impossible :", exc)
...
def v140_export(self, event=None):
    ...
    for name in [
        ...
        if callable(fn):
            try:
                print(f"[v143] export via méthode {name}")
                return fn(event)
            except TypeError:
                try:
                    return fn()
                ...
                print(f"[v143] {name} erreur :", exc)

    v140_status(self, "Export introuvable.")
    print("[v143] ERREUR : aucun moteur export trouvé.")
    return "break"
...
def v140_install_toolbar(self):
    ...
    print("[v143] toolbar visible installée.")
...
def v140_hide_old_ui(self):
    ...
    print(f"[v143] vieux widgets cachés : {hidden}")
...
def v140_build_ui(self):
    ...
    try:
        ...
        print("[v143] toolbar impossible :", exc)

    try:
        v140_hide_old_ui(self)
    except Exception as exc:
        print("[v143] hide old UI impossible :", exc)
...
# ---------------------------------------------------------------------
# v141 : toolbar compacte + hard hide du bas
# ---------------------------------------------------------------------
# Fix :
# - enlève la grosse barre vide en haut à droite
# - remet une petite toolbar compacte
# - garde Exporter .wav visible
# - masque plus agressivement toute la section du bas
# ---------------------------------------------------------------------

P141_BG = "#0b0610"
P141_PANEL = "#170f1f"
P141_PINK = "#ff8be8"
P141_PINK_DARK = "#7a2f68"
P141_PURPLE = "#30223e"
P141_BLUE = "#27304d"
P141_GREEN = "#226b45"
P141_TEXT = "#fff4fb"
...
def v141_walk(widget):
    out = [widget]
    try:
        for child in widget.winfo_children():
            out.extend(v141_walk(child))
    except Exception:
        pass
    return out


def v141_text(widget):
    try:
        return str(widget.cget("text")).strip()
    except Exception:
        return ""
...
def v141_contains(parent, child):
    cur = child
    while cur is not None:
        if cur == parent:
            return True
...
def v141_canvas(self):
    ...
    for widget in v141_walk(self.root):
        try:
            if isinstance(widget, tk.Canvas):
                area = widget.winfo_width() * widget.winfo_height()
                if area > best_area:
...
def v141_new_groove(self, event=None):
    fn = getattr(self, "v140_new_groove", None)
    if callable(fn):
        return fn(event)
    ...
    v141_status(self, "Nouveau groove introuvable.")
    return "break"


def v141_export(self, event=None):
    fn = getattr(self, "v140_export", None)
    if callable(fn):
        return fn(event)
    ...
    v141_status(self, "Export introuvable.")
    print("[v143] ERREUR : aucun moteur export trouvé.")
    return "break"


def v141_loop(self, event=None):
    fn = getattr(self, "v140_loop", None)
    if callable(fn):
        return fn(event)

    v141_status(self, "Loop introuvable.")
    return "break"


def v141_mirror_32(self, event=None):
    fn = getattr(self, "v140_mirror_32", None)
    if callable(fn):
        return fn(event)
    ...
    if not first:
        v141_status(self, "Aucun premier 32 à dupliquer.")
        return "break"
    ...
    try:
        self.write_latest_pattern(reason="v141_mirror_32")
    except Exception:
        pass

    v141_status(self, "Miroir 32 appliqué.")
    return "break"


def v141_button(parent, text, command, bg):
    return tk.Button(
        parent,
        text=text,
        command=command,
...
def v141_destroy_old_toolbars(self):
    for attr in [
        "v141_toolbar",
        "v140_toolbar",
        "v139_toolbar",
...
def v141_install_toolbar(self):
    old = getattr(self, "v141_toolbar", None)
    try:
        if old is not None and old.winfo_exists():
            old.lift()
            return
    ...
    v141_destroy_old_toolbars(self)
    ...
    inner = tk.Frame(toolbar, bg=P141_PANEL)
    ...
    v141_button(inner, "Nouveau groove", self.v141_new_groove, P141_PINK_DARK).pack(side="left", padx=3)
    v141_button(inner, "Miroir 32", self.v141_mirror_32, P141_PURPLE).pack(side="left", padx=3)
    v141_button(inner, "Loop / Space", self.v141_loop, P141_BLUE).pack(side="left", padx=3)

    # Important : pas "Export WAV", on garde "Exporter .wav".
    v141_button(inner, "Exporter .wav", self.v141_export, P141_GREEN).pack(side="left", padx=3)

    # Toolbar compacte : juste au-dessus de la grille, sans énorme largeur.
    toolbar.place(relx=1.0, x=-14, y=34, anchor="ne")
    toolbar.lift()

    self.v141_toolbar = toolbar

    # Alias de protection contre les anciens nettoyages.
    self.v140_toolbar = toolbar
    self.v139_toolbar = toolbar
    self.v138_toolbar = toolbar
    self.p135_toolbar = toolbar
    self.v134_toolbar = toolbar
    self.v132_toolbar = toolbar

    print("[v143] toolbar compacte installée.")
    v141_status(self, "v141 : toolbar compacte + Exporter .wav visible.")


def v141_hide_bottom_hard(self):
    try:
        self.root.update_idletasks()
    except Exception:
        pass

    canvas = v141_canvas(self)
    if canvas is None:
        return
    ...
    for widget in v141_walk(self.root):
        ...
        try:
            if hasattr(self, "v141_toolbar") and v141_contains(self.v141_toolbar, widget):
                continue
        except Exception:
            pass

        # Ne jamais masquer les ancêtres du canvas.
        try:
            if v141_contains(widget, canvas):
                continue
        except Exception:
            pass
        ...
        # Tout ce qui est sous la grille dégage.
        if y >= canvas_bottom + 1:
            v141_hide(widget)
            hidden += 1
            continue

        low = v141_text(widget).lower()
    ...
    try:
        self.v141_toolbar.lift()
    except Exception:
        pass

    print(f"[v143] widgets du bas cachés : {hidden}")


_old_v141_build_ui = SliceIndexTracker.build_ui


def v141_build_ui(self):
    _old_v141_build_ui(self)

    try:
        self.root.title("BreakbeatAI v143 — hover inspector")
        self.root.configure(bg=P141_BG)
    except Exception:
        pass

    try:
        v141_install_toolbar(self)
    except Exception as exc:
        print("[v143] toolbar impossible :", exc)

    try:
        v141_hide_bottom_hard(self)
    except Exception as exc:
        print("[v143] hide bottom impossible :", exc)

    for delay in [200, 600, 1200, 2000]:
        try:
            self.root.after(delay, lambda: v141_install_toolbar(self))
            self.root.after(delay + 50, lambda: v141_hide_bottom_hard(self))
        except Exception:
            pass

    try:
        self.root.bind("<F12>", self.v141_new_groove)
        self.root.bind("<Control-e>", self.v141_export)
    except Exception:
        pass


SliceIndexTracker.v141_new_groove = v141_new_groove
SliceIndexTracker.v141_export = v141_export
SliceIndexTracker.v141_loop = v141_loop
SliceIndexTracker.v141_mirror_32 = v141_mirror_32
SliceIndexTracker.export_wav_current_v141 = v141_export
SliceIndexTracker.build_ui = v141_build_ui



# ---------------------------------------------------------------------
# v142 : toolbar avec texte visible à l'intérieur
# ---------------------------------------------------------------------
# But :
# - écrire clairement dans la barre : "BARRE OUTILS V142"
# - vérifier visuellement quelle barre est la bonne
# - garder Exporter .wav visible
# - ne pas toucher à la grille
# ---------------------------------------------------------------------

P142_PANEL = "#1b1023"
P142_PINK = "#ff8be8"
P142_PINK_DARK = "#7a2f68"
P142_PURPLE = "#30223e"
P142_BLUE = "#27304d"
P142_GREEN = "#226b45"
P142_TEXT = "#fff4fb"
...
def v142_walk(widget):
    out = [widget]
    try:
        for child in widget.winfo_children():
            out.extend(v142_walk(child))
    except Exception:
        pass
    return out


def v142_status(self, text):
    try:
        if hasattr(self, "set_status"):
            self.set_status(text)
        elif hasattr(self, "output_label"):
...
def v142_destroy_old_toolbars(self):
    for attr in [
        "v142_toolbar",
        "v141_toolbar",
        "v140_toolbar",
...
def v142_button(parent, text, command, bg):
    return tk.Button(
        parent,
        text=text,
        command=command,
...
def v142_new_groove(self, event=None):
    fn = getattr(self, "v141_new_groove", None)
    if callable(fn):
        return fn(event)
...
def v142_mirror_32(self, event=None):
    fn = getattr(self, "v141_mirror_32", None)
    if callable(fn):
        return fn(event)
    return "break"


def v142_loop(self, event=None):
    fn = getattr(self, "v141_loop", None)
    if callable(fn):
        return fn(event)
    return "break"


def v142_export(self, event=None):
    fn = getattr(self, "v141_export", None)
    if callable(fn):
        return fn(event)
    ...
    v142_status(self, "Export introuvable.")
    return "break"


def v142_install_toolbar(self):
    old = getattr(self, "v142_toolbar", None)
    try:
        if old is not None and old.winfo_exists():
            old.lift()
            return
    ...
    v142_destroy_old_toolbars(self)
    ...
    toolbar = tk.Frame(
        ...
        highlightbackground=P142_PINK,
    )

    label = tk.Label(
        toolbar,
        text="BARRE OUTILS V142",
        bg=P142_PANEL,
        fg=P142_YELLOW,
        font=("Sans", 10, "bold"),
        padx=12,
    ...
    label.pack(side="left", padx=(6, 8), pady=5)

    v142_button(toolbar, "Nouveau groove", self.v142_new_groove, P142_PINK_DARK).pack(side="left", padx=3, pady=5)
    v142_button(toolbar, "Miroir 32", self.v142_mirror_32, P142_PURPLE).pack(side="left", padx=3, pady=5)
    v142_button(toolbar, "Loop / Space", self.v142_loop, P142_BLUE).pack(side="left", padx=3, pady=5)
    v142_button(toolbar, "Exporter .wav", self.v142_export, P142_GREEN).pack(side="left", padx=3, pady=5)

    toolbar.place(relx=1.0, x=-14, y=34, anchor="ne")
    toolbar.lift()

    self.v142_toolbar = toolbar

    # alias pour que les anciens nettoyages ne la suppriment pas
    self.v141_toolbar = toolbar
    self.v140_toolbar = toolbar
    self.v139_toolbar = toolbar
    self.v138_toolbar = toolbar
    ...
    print("[v143] BARRE OUTILS V142 installée avec texte visible.")
    v142_status(self, "v142 : BARRE OUTILS V142 visible.")


_old_v142_build_ui = SliceIndexTracker.build_ui


def v142_build_ui(self):
    _old_v142_build_ui(self)
    ...
    try:
        v142_install_toolbar(self)
    except Exception as exc:
        print("[v143] toolbar impossible :", exc)

    for delay in [200, 600, 1200, 2000]:
        try:
            self.root.after(delay, lambda: v142_install_toolbar(self))
        except Exception:
            pass

    try:
        self.root.bind("<F12>", self.v142_new_groove)
        self.root.bind("<Control-e>", self.v142_export)
    except Exception:
        pass


SliceIndexTracker.v142_new_groove = v142_new_groove
SliceIndexTracker.v142_mirror_32 = v142_mirror_32
SliceIndexTracker.v142_loop = v142_loop
SliceIndexTracker.v142_export = v142_export
SliceIndexTracker.export_wav_current_v142 = v142_export
SliceIndexTracker.build_ui = v142_build_ui



# ---------------------------------------------------------------------
# v143 : hover inspector terminal
# ---------------------------------------------------------------------
# Objectif :
# - passer la souris sur une barre/bouton/ligne
# - afficher dans le terminal quel widget c'est
# - identifier la mauvaise barre qui masque l'UI
# ---------------------------------------------------------------------


def v143_walk(widget):
    out = [widget]
    try:
        for child in widget.winfo_children():
            out.extend(v143_walk(child))
    except Exception:
        pass
    return out


def v143_text(widget):
    try:
        return str(widget.cget("text")).strip()
    except Exception:
        return ""
...
def v143_children_texts(widget, limit=12):
    texts = []
    ...
    for child in children[:limit]:
        t = v143_text(child)
        cls = v143_class(child)

        if t:
            texts.append(f"{cls}:{t}")
        else:
            texts.append(cls)

    if len(children) > limit:
        texts.append(f"... +{len(children) - limit}")

    return texts


def v143_find_self_attrs(self, widget):
    names = []
    ...
    for name in [
        ...
        try:
            if getattr(self, name, None) is widget and name not in names:
                names.append(name)
        except Exception:
            pass
...
def v143_config_summary(widget):
    keys = ["bg", "background", "fg", "foreground", "highlightbackground", "highlightthickness", "relief", "bd"]
...
def v143_print_widget_hover(self, widget, event=None):
    now = 0
    ...
    last = getattr(widget, "_v143_last_hover_print", 0)

    if now and last and now - last < 0.35:
        return

    try:
        widget._v143_last_hover_print = now
    except Exception:
        pass

    text = v143_text(widget)
    cls = v143_class(widget)
    manager = v143_manager(widget)
    path = v143_widget_path(widget)
    parent = v143_parent_path(widget)
    geom = v143_geom(widget)
    attrs = v143_find_self_attrs(self, widget)
    children = v143_children_texts(widget)
    config = v143_config_summary(widget)
    ...
    if attrs:
        print("self attrs     :", ", ".join(attrs))
    else:
        print("self attrs     : aucun")

    if children:
        print("children_texts :", children)
    else:
...
def v143_bind_hover_widget(self, widget):
    try:
        if getattr(widget, "_v143_hover_bound", False):
            return
    except Exception:
        pass

    try:
        widget.bind("<Enter>", lambda e, w=widget: v143_print_widget_hover(self, w, e), add="+")
        widget.bind("<Motion>", lambda e, w=widget: v143_print_widget_hover(self, w, e), add="+")
        widget._v143_hover_bound = True
    except Exception:
        pass


def v143_bind_canvas_hover(self, canvas):
    try:
        if getattr(canvas, "_v143_canvas_hover_bound", False):
            return
    except Exception:
        pass
    ...
    def on_canvas_motion(event):
        ...
        last = getattr(canvas, "_v143_last_canvas_print", 0)

        if now and last and now - last < 0.7:
            return

        try:
            canvas._v143_last_canvas_print = now
        except Exception:
            pass
        ...
        try:
            ...
            print("")
            print("[v143 CANVAS HOVER]")
            print("canvas xy :", round(x, 2), round(y, 2))
            print("screen    :", f"+{event.x_root}+{event.y_root}")
            print("items     :", item_info, flush=True)
        except Exception:
            pass

    try:
        canvas.bind("<Motion>", on_canvas_motion, add="+")
        canvas._v143_canvas_hover_bound = True
    except Exception:
        pass


def v143_install_hover_inspector(self):
    widgets = v143_walk(self.root)

    for widget in widgets:
        v143_bind_hover_widget(self, widget)

        try:
            if isinstance(widget, tk.Canvas):
                v143_bind_canvas_hover(self, widget)
        except Exception:
            pass

    print(f"[v143] hover inspector actif sur {len(widgets)} widgets.", flush=True)


_old_v143_build_ui = SliceIndexTracker.build_ui


def v143_build_ui(self):
    _old_v143_build_ui(self)
    ...
    try:
        v143_install_hover_inspector(self)
    except Exception as exc:
        print("[v143] hover install impossible :", exc, flush=True)

    for delay in [300, 800, 1500, 2500]:
        try:
            self.root.after(delay, lambda: v143_install_hover_inspector(self))
        except Exception:
            pass

    print("")
    print("==============================================================")
    print("[v143] MODE INSPECTION HOVER ACTIF")
    print("Passe la souris sur la barre suspecte.")
    print("Le terminal affichera class/text/geometry/children/self attrs.")
    print("==============================================================", flush=True)


SliceIndexTracker.build_ui = v143_build_ui
