...
"""
BreakbeatAI Tracker Editor v119 — slice index
...
def warp_audio_to_length_external(audio, target_len, cache_key=None):
    ...
    if rubberband:
        try:
            ...
            print(f"[v119] rubberband échec, fallback ffmpeg/no-speed : {exc}")
    ...
    if ffmpeg:
        try:
            ...
            print(f"[v119] ffmpeg atempo échec, fallback no-speed : {exc}")
...
class SliceIndexTracker:
    def __init__(self, root, pair_json):
        ...
        self.root.title("BreakbeatAI Tracker Editor v119 — slice index")
    ...
    def detect_external_player(self):
        for name in ["pw-play", "paplay", "aplay", "ffplay"]:
            ...
            if path:
                print(f"[v119] Backend audio : {name} -> {path}")
                return name

        print("[v119] Aucun backend système trouvé, fallback sounddevice.")
        return None
    ...
    def build_initial_pattern(self, randomize=False):
        ...
        print(
            f"[v119] Génération pattern | break={self.project['safe']} | "
        ...
        for slot in range(max_items):
            ...
            print(f"[v119] slot {slot:02d} | step {x_step:02d} -> slice {pair:02d}")
    ...
    def build_ui(self):
        ...
        title = tk.Label(
            main,
            text="BreakbeatAI v119 — robust slice row select",
            bg="#111018",
            fg="#ff7acc",
            font=("Sans", 20, "bold"),
        )
    ...
    def record_correction(self, event_type, before=None, after=None):
        ...
        print(f"[v119] correction sauvegardée : {event_type}")
    ...
    def play_audition_array(self, audio, label="audition"):
        ...
        audition_wav = OUT_DIR / f"{self.project['safe']}_v119_audition.wav"
        ...
        if self.external_player:
            try:
                ...
                print(f"[v119] Erreur audition {self.external_player} : {exc}")
    ...
    def on_close(self):
        ...
        """
        print("[v119] Fermeture : arrêt audio + destruction fenêtre")
    ...
    def play_audio_array(self, audio, label="audio"):
        ...
        audio = normalize(audio)
        live_wav = OUT_DIR / f"{self.project['safe']}_v119_live.wav"
        ...
        if self.external_player:
            try:
                ...
                print(f"[v119] Erreur backend {self.external_player} : {exc}")
    ...
    def render_preview_file(self):
        OUT_DIR.mkdir(parents=True, exist_ok=True)
        wav = OUT_DIR / f"{self.project['safe']}_tracker_app_v119_preview.wav"
    ...
    def write_latest_pattern(self, reason="correction"):
        ...
        path = LATEST_DIR / f"{self.project['safe']}_latest_slice_index_v119.json"

        data = {
            "version": "breakbeatai_latest_slice_index_v119",
            "reason": reason,
    ...
    def save(self):
        ...
        tracker_path = OUT_DIR / f"{self.project['safe']}_tracker_app_edit_v119.json"
        ...
        stamp = file_stamp()
        validated_path = VALIDATED_DIR / f"{self.project['safe']}_validated_slice_index_loop64_v119_{stamp}.json"

        validated = {
            "version": "breakbeatai_validated_slice_index_loop64_v119",
...
def v44_record_correction(self, event_type, before=None, after=None):
    ...
    print(f"[v119] correction sauvegardée : {event_type}")
    print(f"[v119] correction path : {corrections_path}")
    print(f"[v119] latest path : {latest_path}")
...
def v45_move_selected(self, dx, dy):
    ...
    if dy != 0 and before != after:
        try:
            ...
            print(f"[v119] audition après flèche haut/bas impossible : {exc}")
...
def v46_warp_loop_pitch_preserve(audio, target_len, cache_key):
    ...
    if rubberband:
        try:
            ...
            print(f"[v119] rubberband global warp échec : {exc}")
    ...
    if ffmpeg:
        try:
            ...
            print(f"[v119] ffmpeg global warp échec : {exc}")

    print("[v119] Aucun warp pitch-preserve dispo, fallback fit no-speed.")
    return v46_fit_audio_no_speed(audio, target_len)
...
def v46_build_initial_pattern(self, randomize=False):
    ...
    print(
        f"[v119] M8 pattern | break={self.project['safe']} | "
...
def v46_get_warped_loop(self):
    ...
    print(
        f"[v119] global warp OK | source_len={loop_end-loop_start} | "
...
def v46_build_ui(self):
    ...
    try:
        ...
        print(f"[v119] UI extension impossible : {exc}")
...
def v47_play_audio_array(self, audio, label="audio"):
    ...
    if len(audio) == 0 or v47_is_silent(audio):
        ...
        print(f"[v119] lecture annulée : audio silencieux ({label})")
        return 0
...
def v47_toggle_loop(self):
    ...
    if not getattr(self, "pattern", None):
        ...
        print("[v119] loop annulée : pattern vide")
        return "break"
...
def v47_delete_selected(self):
    ...
    if not getattr(self, "pattern", None):
        ...
        print("[v119] pattern vide après suppression : audio coupé")
...
def v48_build_initial_pattern(self, randomize=False):
    ...
    print("[v119] pattern initial forcé VIDE : aucun son ne doit jouer.")
    return []
...
def v48_load_first_slices(self):
    ...
    print(f"[v119] Load 16 slices : {len(self.pattern)} items")


def v48_debug_pattern(self, label="render"):
    print("")
    print(f"[v119] DEBUG PATTERN avant {label}")
    print(f"[v119] items = {len(getattr(self, 'pattern', []) or [])}")

    if not getattr(self, "pattern", None):
        print("[v119] pattern vide confirmé")
        print("")
        return
    ...
    for item in self.pattern:
        ...
        print(
            f"[v119] id={item_id} | step {x:02d}/{end:02d} | "
...
def v48_render_audio_with_timeline(self):
    ...
    if not getattr(self, "pattern", None):
        loop_samples = self.get_loop_samples()
        print(f"[v119] rendu silence total : pattern vide | samples={loop_samples}")
        return np.zeros(loop_samples, dtype=np.float32), []
    ...
    print(f"[v119] rendu audio peak={peak:.8f} | timeline_events={len(timeline) if timeline is not None else 'None'}")

    if timeline:
        print("[v119] timeline events:")
        for ev in timeline:
            print(f"[v119]   {ev}")
...
def v48_toggle_loop(self):
    if not getattr(self, "pattern", None):
        ...
        print("[v119] Space ignoré : pattern vide.")
        return "break"
...
def v48_build_ui(self):
    ...
    try:
        ...
        print(f"[v119] ajout UI impossible : {exc}")
...
def v49_add_item_at_xy(self, x, y):
    ...
    try:
        ...
        print(f"[v119] record_correction add impossible : {exc}")
    ...
    try:
        ...
        print(f"[v119] audition après ajout impossible : {exc}")

    print(f"[v119] add | id={item['id']} | step={x_step} | pair={pair} | lane={lane}")
...
def v49_duplicate_selected_forward(self, event=None):
    ...
    if item is None:
        ...
        print("[v119] duplicate annulé : aucune note sélectionnée")
        return "break"
    ...
    if new_x > max_x:
        ...
        print("[v119] duplicate annulé : fin de grille")
        return "break"
    ...
    try:
        ...
        print(f"[v119] record_correction duplicate impossible : {exc}")
    ...
    try:
        ...
        print(f"[v119] audition après duplicate impossible : {exc}")

    print(
        f"[v119] duplicate | old_id={item.get('id')} | new_id={new_item.get('id')} | "
...
def v49_build_ui(self):
    ...
    try:
        ...
        print(f"[v119] bind canvas click impossible : {exc}")
    ...
    try:
        ...
        print(f"[v119] bind keys impossible : {exc}")
...
def v50_load_style_model():
    ...
    try:
        ...
        print(f"[v119] impossible de charger le modèle IA : {exc}")
        return None
...
def v50_generate_ai_pattern(self, event=None):
    ...
    if model is None:
        ...
        print("[v119] modèle absent :", STYLE_MODEL_PATH)
        return "break"
    ...
    print("")
    print(f"[v119] GENERATE IA | safe={safe} | pairs={pair_values} | slots={max_slots}")
    ...
    for slot in range(max_slots):
        ...
        print(
            f"[v119] slot {slot:02d} | step {x_step:02d} | "
    ...
    try:
        ...
        print(f"[v119] write_latest_pattern impossible : {exc}")
...
def v50_build_ui(self):
    ...
    try:
        ...
        print(f"[v119] UI IA impossible : {exc}")
...
def v51_mouse_down(self, event):
    ...
    if item is not None:
        ...
        print(
            f"[v119] mouse down note | id={item.get('id')} | "
            f"step={item.get('x_step')} | pair={item.get('pair')}"
        )
    else:
        print("[v119] mouse down empty")
...
def v51_mouse_up(self, event):
    ...
    if state.get("empty"):
        ...
        print("[v119] drag depuis vide ignoré")
        return "break"
    ...
    if before != after:
        try:
            ...
            print(f"[v119] record_correction drag impossible : {exc}")
        ...
        print(
            f"[v119] drag move | id={after.get('id')} | "
            f"step {before.get('x_step')} -> {after.get('x_step')} | "
        ...
        try:
            ...
            print(f"[v119] audition après drag impossible : {exc}")
...
def v51_build_ui(self):
    ...
    try:
        ...
        print(f"[v119] bind souris impossible : {exc}")
    ...
    try:
        ...
        print(f"[v119] bind Suppr impossible : {exc}")
...
def v52_normalize_loaded_notes(self, notes):
    ...
    for raw in sorted(notes, key=lambda n: (int(float(n.get("x_step", 0))), int(float(n.get("pair", 0))))):
        ...
        if pair not in valid_pairs:
            print(f"[v119] slice ignorée car absente du break actuel : pair={pair}")
            continue
...
def v52_load_saved_beat(self, event=None):
    ...
    if not saves:
        ...
        print("[v119] aucune sauvegarde trouvée")
        return "break"
    ...
    if not pattern:
        ...
        print("[v119] sauvegarde incompatible :", chosen["path"])
        return "break"
    ...
    print("")
    print("[v119] BEAT RESTAURÉ")
    print("[v119] fichier :", chosen["path"])
    print("[v119] clé :", chosen["key"])
    print("[v119] notes :", len(pattern))
    print("")
...
def v52_list_saved_beats(self, event=None):
    ...
    print("")
    print("[v119] SAUVEGARDES TROUVÉES")
    for i, save in enumerate(saves[:30]):
        print(
            f"[v119] {i:02d} | notes={len(save['notes']):02d} | "
...
def v52_build_ui(self):
    ...
    try:
        ...
        print(f"[v119] UI load saved impossible : {exc}")
...
def v53_run_training(self):
    ...
    if not trainer.exists():
        ...
        print("[v119] trainer introuvable :", trainer)
        return False

    print("")
    print("[v119] AUTO-TRAIN IA après Save...")
    print("")
    ...
    try:
        ...
        if result.returncode != 0:
            ...
            print("[v119] auto-train échec code :", result.returncode)
            return False

        v53_status(self, "Save OK + IA entraînée automatiquement.")
        print("[v119] AUTO-TRAIN OK")
        return True

    except Exception as exc:
        v53_status(self, f"Save OK, mais auto-train impossible : {exc}")
        print("[v119] exception auto-train :", exc)
        return False
...
def v53_save(self):
    ...
    try:
        ...
        print(f"[v119] auto-train après save impossible : {exc}")
...
def v53_build_ui(self):
    ...
    try:
        ...
        print(f"[v119] UI auto-train impossible : {exc}")
...
def v55_mouse_up(self, event):
    ...
    if not state.get("moved"):
        ...
        try:
            ...
            print(f"[v119] audition clic note impossible : {exc}")
        ...
        print(
            f"[v119] click play/select | id={item.get('id')} | "
            f"step={item.get('x_step')} | pair={item.get('pair')}"
        )
...
def v58_add_item_at_xy(self, x, y):
    ...
    print(
        f"[v119] add | id={item['id']} | step={x_step} | "
...
def v58_toggle_selected_length(self, event=None):
    ...
    try:
        ...
        print(f"[v119] record_correction length impossible : {exc}")

    v58_refresh_after_edit(self, "toggle_note_length")

    v58_status(self, f"Longueur note : {old_len} -> {new_len} case(s).")
    print(f"[v119] length toggle | id={item.get('id')} | {old_len} -> {new_len}")
...
def v58_duplicate_selected_forward(self, event=None):
    ...
    try:
        ...
        print(f"[v119] audition duplicate impossible : {exc}")
...
def v58_build_ui(self):
    ...
    try:
        ...
        print(f"[v119] UI demi-grille impossible : {exc}")
    ...
    try:
        ...
        print(f"[v119] bind demi-grille impossible : {exc}")
...
def v59_rank_current_slices(self):
    ...
    for pair in self.pair_values:
        ...
        try:
            ...
            print(f"[v119] analyse pair {pair} impossible : {exc}")
            role = "other"
    ...
    print("")
    print("[v119] TOP ROLES CURRENT BREAK")
    for r in ("kick", "snare", "hat"):
        print(f"[v119] {r:5s}:", [(x["pair"], round(x["score"], 3)) for x in ranks[r][:6]])
    print("")
...
def v59_load_role_model():
    ...
    try:
        ...
        print(f"[v119] lecture modèle rôle impossible : {exc}")
        return None
...
def v59_generate_role_aware(self, event=None):
    ...
    if model is None:
        ...
        print("[v119] modèle rôle absent, fallback musical snare 6.")
        model = {
            "breaks": {},
            "global": {},
            "patterns_used": 0,
    ...
    print("")
    print("[v119] ROLE-AWARE GENERATION")
    print("[v119] model patterns_used:", model.get("patterns_used", 0))
    for item in pattern:
        print(
            f"[v119] step {item['x_step']:02d}/{item['x_step'] + item['length'] - 1:02d} | "
...
def v59_run_role_training(self, event=None):
    ...
    try:
        ...
        print(f"[v119] training rôle impossible : {exc}")
...
def v59_save(self):
    ...
    try:
        ...
        print(f"[v119] auto-training rôle après save impossible : {exc}")
...
def v59_build_ui(self):
    ...
    try:
        ...
        print(f"[v119] UI role-aware impossible : {exc}")
...
def v62_add_item_at_xy(self, x, y):
    ...
    print(f"[v119] add | id={item['id']} | step={x_step} | len={length} | pair={pair}")
...
def v62_mouse_down(self, event):
    ...
    if item is not None:
        ...
        print(
            f"[v119] mouse down {state['mode']} | id={item.get('id')} | "
            f"step={item.get('x_step')} | len={item.get('length')} | pair={item.get('pair')}"
        )
    else:
        print("[v119] mouse down empty")
    ...
    if mode == "resize":
        if not state.get("moved"):
            ...
            try:
                ...
                print(f"[v119] audition clic resize impossible : {exc}")
        ...
        if before != after:
            try:
                ...
                print(f"[v119] record_correction resize impossible : {exc}")
            ...
            try:
                ...
                print(f"[v119] audition après resize impossible : {exc}")
    ...
    if mode == "move":
        ...
        if before != after:
            try:
                ...
                print(f"[v119] record_correction move impossible : {exc}")
            ...
            try:
                ...
                print(f"[v119] audition après move impossible : {exc}")
...
def v62_toggle_selected_length(self, event=None):
    ...
    try:
        ...
        print(f"[v119] record_correction toggle length impossible : {exc}")

    v62_refresh_after_edit(self, "toggle_note_length_v62")
    v62_status(self, f"Longueur note : {old_len} → {new_len} case(s).")

    print(f"[v119] toggle length | id={item.get('id')} | {old_len} -> {new_len}")
...
def v62_draw(self):
    ...
    try:
        ...
        print(f"[v119] draw handles impossible : {exc}")
...
def v62_build_ui(self):
    ...
    try:
        ...
        print(f"[v119] bind resize impossible : {exc}")
    ...
    try:
        ...
        print(f"[v119] UI resize impossible : {exc}")
...
def v63_load_model():
    ...
    try:
        ...
        print(f"[v119] lecture modèle strict impossible : {exc}")
        return None
...
def v63_generate_safe_role_ai(self, event=None):
    ...
    print("")
    print("[v119] SAFE ROLE AI GENERATED")
    print("[v119] model strict:", "yes" if model else "no")
    print("[v119] snare main/alt:", main_snare, alt_snare)
    print("[v119] kick  main/alt:", main_kick, alt_kick)
    print("[v119] hat   main/alt:", main_hat, alt_hat)
    print("[v119] pattern:")
    for item in pattern:
        print(
            f"[v119] step {item['x_step']:02d}/{item['x_step'] + item['length'] - 1:02d} | "
...
def v63_run_strict_training(self, event=None):
    ...
    print("")
    print("[v119] TRAIN STRICT ROLE MODEL...")
    print("")
    ...
    try:
        ...
        print(f"[v119] training strict impossible : {exc}")
...
def v63_build_ui(self):
    ...
    try:
        ...
        print(f"[v119] UI safe impossible : {exc}")
...
def v65_rank_slices(self):
    ...
    for pair in self.pair_values:
        ...
        try:
            ...
            print(f"[v119] analyse slice {pair} impossible : {exc}")
            scores = {
                "kick": 0.0,
                "snare": 0.0,
                "hat": 0.0,
    ...
    print("")
    print("[v119] TOP SLICES PAR ROLE — break courant")
    for role in ("kick", "snare", "hat", "ghost_snare"):
        print(f"[v119] {role:12s}:", [(x["pair"], round(x["score"], 3)) for x in ranks[role][:8]])
    print("")
...
def v65_generate_from_role_template(self, event=None):
    ...
    try:
        ...
        print(f"[v119] template introuvable : {exc}")
        return "break"
    ...
    print("")
    print("[v119] CROSS-SONG ROLE TEMPLATE GENERATED")
    print("[v119] template:", V65_TEMPLATE_PATH)
    print("[v119] temperature:", temperature, "| hats:", hat_density, "| ghosts:", ghost_density)
    print("[v119] pattern:")
    for item in pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v119] step {item['x_step']:02d}/{end:02d} | "
...
def v65_build_ui(self):
    ...
    try:
        ...
        print(f"[v119] UI impossible : {exc}")
...
def v67_train_strict_if_available(self):
    ...
    if trainer is None:
        print("[v119] aucun trainer trouvé.")
        return False

    print("")
    print("[v119] TRAIN après validation :", trainer)
    print("")
    ...
    try:
        ...
        print(f"[v119] training impossible : {exc}")
        return False
...
def v67_generate_candidate(self, event=None):
    ...
    print("")
    print("[v119] CANDIDATE GENERATED")
    print("[v119] break:", self.v67_candidate_safe)
    print("[v119] notes:", len(candidate))
    print("[v119] Maintenant : corrige puis clique Good après modifs, ou Reject.")
    print("")
...
def v67_reject_candidate(self, event=None):
    ...
    print("")
    print("[v119] REJECTED")
    print("[v119] saved:", rejected_path)
    print("")
...
def v67_accept_corrected(self, event=None):
    ...
    print("")
    print("[v119] ACCEPTED GOOD AFTER EDIT")
    print("[v119] saved:", validated_path)
    print("[v119] trained:", trained)
    print("")
...
def v67_accept_without_training(self, event=None):
    ...
    print("[v119] accepted no-train:", validated_path)
...
def v67_build_ui(self):
    ...
    try:
        ...
        print(f"[v119] UI feedback impossible : {exc}")
...
def v68_generate_candidate(self, event=None):
    ...
    print("[v119] Length policy appliquée : 2 cases par défaut, 1 case rare.")
...
def v69_force_pattern_hard_2_grid(self, reason="v69_force_hard_2_grid"):
    ...
    print("")
    print("[v119] HARD 2 GRID APPLIQUÉ")
    for item in self.pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v119] step {item['x_step']:02d}/{end:02d} | "
...
def v70_duplicate_selected_forward(self, event=None):
    ...
    print("")
    print("[v119] CTRL+D DUPLICATE +2")
    print(f"[v119] old_id={item.get('id')} new_id={new_item.get('id')}")
    print(f"[v119] step {old_x}/{old_x + length - 1} -> {new_x}/{new_x + length - 1}")
    print(f"[v119] pair={new_item.get('pair')} len={length}")
    print("")
...
def v70_build_ui(self):
    ...
    try:
        ...
        print(f"[v119] bind Ctrl+D impossible : {exc}")
...
def v71_rank_slices(self):
    ...
    print("")
    print("[v119] PRE-LEARNING TOP SLICES")
    for role in ("kick", "snare", "hat", "ghost_snare"):
        print(f"[v119] {role:12s}:", [(x["pair"], round(x["score"], 3)) for x in ranks[role][:8]])
    print("")
...
def v71_prelearn_roles(self, force=False):
    ...
    print("")
    print("[v119] PRELEARN ROLES POUR", safe)
    for role in ("kick", "snare", "hat", "ghost_snare", "bad"):
        print(f"[v119] {role:12s}:", roles.get(role, []))
    print("")
...
def v71_generate_full_candidate(self, event=None):
    ...
    print("")
    print("[v119] FULL CANDIDATE GENERATED")
    print("[v119] break:", v71_safe(self))
    print("[v119] roles:", roles)
    print("[v119] pattern:")
    for item in pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
...
def v72_open_break(self, safe=None):
    ...
    print("")
    print("[v119] OPEN BREAK")
    print("[v119] current:", current)
    print("[v119] next   :", safe)
    print("[v119] cmd    :", " ".join(cmd))
    print("")
...
def v72_build_ui(self):
    ...
    try:
        ...
        print(f"[v119] UI break switcher impossible : {exc}")
...
def v73_sanitize_tracker_pattern(self, reason="v73_tracker_strict_no_overlap"):
    ...
    print("")
    print("[v119] TRACKER STRICT NO OVERLAP")
    print("[v119] kept:", len(accepted), "| rejected overlaps:", len(rejected))

    for item in accepted:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v119] KEEP step {item['x_step']:02d}/{end:02d} | "
    ...
    if rejected:
        ...
        for item in rejected:
            ...
            print(
                f"[v119] DROP step {item['x_step']:02d}/{end:02d} | "
                f"role={item.get('v73_role_key')} | pair={item.get('pair')}"
            )
...
def v81_launch_break(self, source):
    ...
    print("")
    print("[v119] CHARGER BREAK")
    print("[v119] courant :", current)
    print("[v119] nouveau :", source)
    print("[v119] cmd     :", " ".join(cmd))
    print("")
...
def v81_patch_charger_break_buttons(self):
    ...
    print(f"[v119] boutons Charger break patchés : {patched}")
...
def v81_generate_candidate(self, event=None):
    ...
    print("")
    print("[v119] GENERATE CANDIDATE")
    print("[v119] break chargé :", current)
    print("[v119] générateur   :", getattr(fn, "__name__", None))
    print("")
...
def v81_run_training(self):
    ...
    if script is None:
        print("[v119] Aucun script training trouvé.")
...
def v81_save_feedback(self, verdict, train_now=False):
    ...
    print("")
    print("[v119] FEEDBACK SAUVÉ")
    print("[v119] break  :", current)
    print("[v119] verdict:", verdict)
    print("[v119] train  :", train_now)
    print("[v119] fichier:", out_path)
    print("")
...
def v81_build_ui(self):
    ...
    try:
        self.root.title(f"BreakbeatAI v119 — robust slice row select")
    except Exception:
        pass
    ...
    try:
        ...
        print(f"[v119] patch Charger break impossible : {exc}")

    try:
        v81_install_training_toolbar(self)
    except Exception as exc:
        print(f"[v119] toolbar impossible : {exc}")
...
def v82_auto_role_pools(self):
    ...
    print("")
    print("[v119] ROLE POOLS")
    for r in ["kick", "snare", "hat", "ghost_snare", "bad"]:
        print(f"[v119] {r:12s}:", roles.get(r, []))
    print("")
...
def v82_generate_candidate(self, event=None):
    ...
    print("")
    print("[v119] NEW CANDIDATE")
    print("[v119] break :", current)
    print("[v119] nonce :", nonce)
    print("[v119] rejected known:", len(rejected))
    print("[v119] fingerprint rejeté ?: ", chosen_fp in rejected)
    for item in self.pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v119] step {item['x_step']:02d}/{end:02d} "
...
def v82_patch_training_buttons(self):
    ...
    print(f"[v119] boutons training patchés : {patched}")
...
def v82_build_ui(self):
    ...
    try:
        ...
        print(f"[v119] patch boutons impossible : {exc}")
...
def v110_make_second32_from_first32(first_grid, response_id):
    ...
    if response_id == "answer_kick_bar4":
        return [
            (32, "kick"), (34, "hat"),  (36, "snare"), (38, "hat"),
    ...
    if response_id == "late_kick_response":
        return [
            (32, "kick"), (34, "hat"),  (36, "snare"), (38, "hat"),
            (40, "hat"),  (42, "kick"), (44, "snare"), (46, "hat"),
...
def v110_generate(self, event=None, reject_mode=False):
    ...
    print("")
    if reject_mode:
        print("[v119] REJECT/BAD -> variation premier 32 + autres lignes")
    else:
        print("[v119] GENERATE 64")
    print("[v119] break:", current)
    print("[v119] first32:", first32_id)
    print("[v119] response:", response_id)
    print("[v119] role_offset:", role_offset)
    for role in ["kick", "snare", "hat"]:
        lane = "?"
        try:
            ...
            pass
        print(f"[v119] {role:6s}: pair={chosen[role]['pair']} lane={lane} source={chosen[role]['source']}")
    print("")
    ...
    try:
        ...
        print(f"[v119] draw impossible : {exc}")
    ...
    try:
        ...
        print(f"[v119] refresh_panel impossible : {exc}")
...
def v110_render_pattern_to_audio(self):
    ...
    for item in pattern:
        ...
        if pair not in slice_map:
            skipped += 1
            print(f"[v119] skip pair introuvable : {pair}")
            continue
...
def v110_export_wav_current(self, event=None):
    try:
        ...
        print("")
        print("[v119] EXPORT WAV OK")
        print("[v119] break:", current)
        print("[v119] wav  :", out_path)
        print("[v119] json :", info_path)
        print("[v119] duration:", round(info["duration_seconds"], 3), "s")
        print("[v119] rendered:", info["rendered_notes"], "skipped:", info["skipped_notes"])
        print("")
...
def v110_space_play(self, event=None):
    ...
    try:
        ...
        if btn is not None:
            print("[v119] SPACE -> Loop / Space")
            try:
                btn.invoke()
            except Exception as exc:
                print("[v119] invoke Loop / Space impossible :", exc)
            return "break"

        print("[v119] SPACE : bouton Loop / Space introuvable.")
...
def v110_patch_buttons(self):
    ...
    print(f"[v119] boutons patchés : Generate={gen_patched}, Reject/Bad={bad_patched}")
...
def v110_build_ui(self):
    ...
    try:
        ...
        print("[v119] patch boutons impossible :", exc)

    try:
        v110_add_export_button(self)
    except Exception as exc:
        print("[v119] ajout Export WAV impossible :", exc)

    try:
        v110_install_space_bindings(self)
    except Exception as exc:
        print("[v119] bind espace impossible :", exc)
...
def v113_export(self, event=None):
    ...
    print("[v119] ERREUR : export_wav_current_v110 introuvable.")
...
def v113_place_export_button(self):
    ...
    try:
        ...
        print("[v119] bouton Export WAV visible ajouté.")
        v113_status(self, "v113 : Export WAV visible. Espace reste Loop / Space.")

    except Exception as exc:
        print("[v119] placement bouton Export WAV impossible :", exc)
        try:
            btn.place(x=760, y=32, height=30)
            btn.lift()
            self.export_button_v113 = btn
...
def v115_apply_cells_minus_25(self):
    ...
    print("")
    print("[v119] APPLICATION CASES -3.5px")
    ...
    for name in V115_WIDTH_GLOBAL_NAMES:
        try:
            ...
            if isinstance(value, (int, float)) and value > 6:
                ...
                print(f"[v119] global {name}: {value} -> {globals()[name]}")
        except Exception:
            pass
        ...
        try:
            if hasattr(self, attr):
                ...
                if isinstance(value, (int, float)) and value > 6:
                    ...
                    print(f"[v119] attr {attr}: {value} -> {getattr(self, attr)}")
        except Exception:
            pass
...
def v115_build_ui(self):
    ...
    try:
        self.root.title("BreakbeatAI v119 — robust slice row select")
    except Exception:
        pass
    ...
    try:
        ...
        print("[v119] impossible d'appliquer -3.5px :", exc)
...
def v117_render_pattern_to_audio(self):
    ...
    print("")
    print("[v119] EXPORT EXACT CELLS")
    print("[v119] break:", current)
    print("[v119] pairblock:", pairblock_file)
    print("[v119] sr:", sr)
    print("[v119] median_pair_samples:", median_pair_samples)
    print("[v119] case_samples:", case_samples)
    print("[v119] export_samples:", total_samples)
    print("[v119] export_seconds:", round(total_samples / float(sr), 6))
    print("")
...
def v117_export_wav_current(self, event=None):
    try:
        ...
        print("[v119] wav  :", out_path)
        print("[v119] json :", info_path)
        print("[v119] duration:", round(info["duration_seconds"], 6), "s")
        print("[v119] case_seconds:", round(info["case_seconds"], 6))
        print("[v119] rendered:", info["rendered_notes"], "skipped:", info["skipped_notes"])
        print("")
...
# ---------------------------------------------------------------------
# v119 : sélection robuste des lignes Slice
# ---------------------------------------------------------------------
# Base : v117 propre.
#
# Corrige v118 :
# - capte les clics sur widgets Tk ET Canvas
# - bind global via bindtags avant les callbacks normaux
# - clic sur Slice N = sélection de toute la ligne N
# - glisser = déplacer toute la ligne
# - flèches = déplacement fin
# - Échap = désélection
# ---------------------------------------------------------------------


def v119_status(self, text):
    try:
        if hasattr(self, "set_status"):
            self.set_status(text)
        elif hasattr(self, "output_label"):
...
def v119_all_widgets(widget):
    ...
    for child in children:
        out.extend(v119_all_widgets(child))

    return out


def v119_widget_text(widget):
    for key in ["text", "label"]:
        try:
            value = str(widget.cget(key)).strip()
            if value:
                return value
        except Exception:
            pass
...
def v119_parse_slice_lane(text):
    text = str(text).strip()
    m = re.search(r"\bslice\s*([0-9]+)\b", text, flags=re.I)
    if not m:
        return None
    return max(0, int(m.group(1)) - 1)


def v119_canvas(self):
    for attr in ["canvas", "grid_canvas", "tracker_canvas"]:
        try:
            c = getattr(self, attr)
    ...
    for w in v119_all_widgets(self.root):
        try:
            if str(w.winfo_class()).lower() == "canvas":
                return w
        except Exception:
            pass
...
def v119_step_width(self):
    for attr in [
        "step_w", "cell_w", "col_w", "column_w", "grid_step_w",
        "step_width", "cell_width", "col_width", "note_width",
        "beat_w", "x_step_w", "x_cell_w", "tracker_step_w",
    ]:
        try:
            value = getattr(self, attr)
...
def v119_row_height(self):
    canvas = v119_canvas(self)
    ...
    if canvas is not None:
            ...
            for item in canvas.find_all():
                try:
                    if canvas.type(item) == "text":
                        ...
                        if v119_parse_slice_lane(text) is not None:
        ...
        diffs = [b - a for a, b in zip(ys[:-1], ys[1:]) if b - a > 4]
        if diffs:
            diffs.sort()
            return float(diffs[len(diffs) // 2])

    for attr in ["row_h", "lane_h", "row_height", "lane_height", "cell_h", "cell_height"]:
        try:
            value = getattr(self, attr)
...
def v119_max_lane(self):
    try:
        pairs = list(getattr(self, "pair_values", []))
        if pairs:
    ...
    try:
        for item in getattr(self, "pattern", []) or []:
            lanes.append(int(item.get("lane", 0)))
    except Exception:
        pass
...
def v119_pair_for_lane(self, lane, fallback_pair=0):
    lane = int(lane)

    try:
        for pair, ln in getattr(self, "pair_to_lane", {}).items():
            if int(ln) == lane:
                return int(pair)
    except Exception:
        pass
...
def v119_indices_for_lane(self, lane):
    out = []
    for idx, item in enumerate(list(getattr(self, "pattern", []) or [])):
        try:
            if int(item.get("lane", -1)) == int(lane):
                out.append(idx)
        except Exception:
            pass
    return out
...
def v119_make_snapshot(self, lane):
    snap = []
    for idx in v119_indices_for_lane(self, lane):
        try:
            snap.append((idx, dict(self.pattern[idx])))
        except Exception:
            pass
    return snap


def v119_select_lane(self, lane, event=None):
    lane = int(max(0, min(v119_max_lane(self), int(lane))))

    self.v119_selected_lane = lane
    self.v119_selected_label = f"Slice {lane + 1}"
    self.v119_drag_snapshot = v119_make_snapshot(self, lane)
    self.v119_dragging = event is not None

    if event is not None:
        try:
            self.v119_drag_start_x = int(event.x_root)
            self.v119_drag_start_y = int(event.y_root)
        except Exception:
            self.v119_drag_start_x = 0
            self.v119_drag_start_y = 0

    count = len(self.v119_drag_snapshot)

    # Sélectionne au moins le premier bloc pour que l'UI native montre quelque chose.
    try:
        if count:
            first_idx = self.v119_drag_snapshot[0][0]
            self.selected_id = self.pattern[first_idx].get("id", first_idx)
    except Exception:
        pass

    print("")
    print(f"[v119] SÉLECTION LIGNE : Slice {lane + 1}")
    print(f"[v119] blocs sélectionnés : {count}")
    print("[v119] flèches = déplacer, drag = déplacer, Échap = annuler")
    print("")
    ...
    try:
        v119_draw_highlight(self)
    except Exception:
        pass

    v119_status(self, f"v119 : Slice {lane + 1} sélectionnée, {count} blocs.")
    return "break"


def v119_find_canvas_slice_from_event(self, event):
    widget = getattr(event, "widget", None)
    canvas = None

    try:
        if widget is not None and hasattr(widget, "find_closest") and str(widget.winfo_class()).lower() == "canvas":
            canvas = widget
    except Exception:
        pass
    ...
    # Cherche les textes Slice dans le canvas.
    labels = []
    ...
    try:
        for item in canvas.find_all():
            try:
                ...
                lane = v119_parse_slice_lane(text)
                ...
                yc = (float(bbox[1]) + float(bbox[3])) / 2.0
                x2 = float(bbox[2])

                labels.append((lane, text, yc, x2))
            except Exception:
                pass
    except Exception:
        pass
    ...
    row_h = v119_row_height(self)

    # Zone à gauche des labels.
    max_label_x = max(x2 for _, _, _, x2 in labels) + 40.0
    ...
    for lane, text, yc, x2 in labels:
        d = abs(cy - yc)
        if best_d is None or d < best_d:
            best_d = d
            best = (lane, text)

    if best is not None and best_d is not None and best_d <= row_h * 0.65:
        return best[0]

    return None


def v119_event_slice_lane(self, event):
    widget = getattr(event, "widget", None)

    # 1) Si c'est un Label/Button Tk avec texte Slice N.
    if widget is not None:
        text = v119_widget_text(widget)
        lane = v119_parse_slice_lane(text)
        if lane is not None:
            return lane

    # 2) Si c'est du texte Canvas ou la zone de gauche du Canvas.
    lane = v119_find_canvas_slice_from_event(self, event)
    if lane is not None:
        return lane
...
def v119_on_click_pre(self, event):
    lane = v119_event_slice_lane(self, event)

    if lane is None:
        return None

    return v119_select_lane(self, lane, event=event)


def v119_apply_snapshot_move(self, step_delta=0, lane_delta=0):
    snap = list(getattr(self, "v119_drag_snapshot", []) or [])
    if not snap:
        return

    max_lane = v119_max_lane(self)

    for idx, old in snap:
        if idx < 0 or idx >= len(self.pattern):
            continue

        try:
            length = int(old.get("length", 2) or 2)
        except Exception:
            length = 2

        old_step = int(old.get("x_step", 0))
        old_lane = int(old.get("lane", 0))
        old_pair = old.get("pair", 0)

        new_step = max(0, min(64 - length, old_step + int(step_delta)))
        new_lane = max(0, min(max_lane, old_lane + int(lane_delta)))
        new_pair = v119_pair_for_lane(self, new_lane, fallback_pair=old_pair)

        self.pattern[idx]["x_step"] = int(new_step)
        self.pattern[idx]["lane"] = int(new_lane)
        self.pattern[idx]["pair"] = int(new_pair)
        self.pattern[idx]["v119_row_move"] = True

    try:
        base_lane = int(getattr(self, "v119_selected_lane", 0))
        self.v119_selected_lane = max(0, min(max_lane, base_lane + int(lane_delta)))
        self.v119_selected_label = f"Slice {int(self.v119_selected_lane) + 1}"
    except Exception:
        pass
    ...
    try:
        ...
        print("[v119] draw impossible après move :", exc)
...
def v119_on_motion_pre(self, event):
    if not getattr(self, "v119_dragging", False):
        return None

    snap = getattr(self, "v119_drag_snapshot", [])
    if not snap:
        return "break"

    try:
        dx = int(event.x_root) - int(getattr(self, "v119_drag_start_x", event.x_root))
        dy = int(event.y_root) - int(getattr(self, "v119_drag_start_y", event.y_root))
    except Exception:
        return "break"

    step_w = max(1.0, v119_step_width(self))
    row_h = max(1.0, v119_row_height(self))

    step_delta = int(round(dx / step_w))
    lane_delta = int(round(dy / row_h))

    v119_apply_snapshot_move(self, step_delta=step_delta, lane_delta=lane_delta)
    return "break"


def v119_on_release_pre(self, event):
    if not getattr(self, "v119_dragging", False):
        return None

    self.v119_dragging = False

    # Après un drag, le snapshot devient le nouvel état de la ligne.
    try:
        lane = int(getattr(self, "v119_selected_lane", 0))
        self.v119_drag_snapshot = v119_make_snapshot(self, lane)
    except Exception:
        pass

    try:
        self.write_latest_pattern(reason="v119_slice_row_move")
    except Exception:
        pass

    v119_status(self, f"v119 : {getattr(self, 'v119_selected_label', 'Slice')} déplacée.")
    return "break"


def v119_move_selected(self, step_delta=0, lane_delta=0):
    lane = getattr(self, "v119_selected_lane", None)
    if lane is None:
        v119_status(self, "v119 : clique d'abord sur Slice N.")
        return "break"

    self.v119_drag_snapshot = v119_make_snapshot(self, lane)

    if not self.v119_drag_snapshot:
        v119_status(self, f"v119 : Slice {int(lane)+1} vide.")
        return "break"

    v119_apply_snapshot_move(self, step_delta=step_delta, lane_delta=lane_delta)

    try:
        lane2 = int(getattr(self, "v119_selected_lane", lane))
        self.v119_drag_snapshot = v119_make_snapshot(self, lane2)
    except Exception:
        pass

    try:
        self.write_latest_pattern(reason="v119_slice_row_arrow")
    except Exception:
        pass
...
def v119_left(self, event=None):
    return v119_move_selected(self, step_delta=-1, lane_delta=0)


def v119_right(self, event=None):
    return v119_move_selected(self, step_delta=1, lane_delta=0)


def v119_up(self, event=None):
    return v119_move_selected(self, step_delta=0, lane_delta=-1)


def v119_down(self, event=None):
    return v119_move_selected(self, step_delta=0, lane_delta=1)


def v119_escape(self, event=None):
    self.v119_selected_lane = None
    self.v119_selected_label = None
    self.v119_dragging = False
    self.v119_drag_snapshot = []
    ...
    v119_status(self, "v119 : sélection ligne annulée.")
    return "break"


def v119_draw_highlight(self):
    canvas = v119_canvas(self)
    if canvas is None:
        return

    try:
        canvas.delete("v119_selection")
    except Exception:
        pass
    ...
    # Essaie de trouver le Y via les labels Canvas.
    target_y = None
        ...
        for item in canvas.find_all():
            try:
                if canvas.type(item) != "text":
                    continue
                text = str(canvas.itemcget(item, "text")).strip()
                ln = v119_parse_slice_lane(text)
                if ln == int(lane):
                    bbox = canvas.bbox(item)
                    if bbox:
                        target_y = (float(bbox[1]) + float(bbox[3])) / 2.0
                        break
            except Exception:
                pass
    except Exception:
    ...
    try:
        ...
        x2 = bbox_all[2] if bbox_all else 4096
        ...
        canvas.create_text(
            8,
            target_y - row_h / 2.0 + 3,
            anchor="nw",
            text=f"SELECTED Slice {int(lane) + 1}",
            fill="#ff66cc",
            font=("Sans", 9, "bold"),
            tags=("v119_selection",),
        )
        canvas.tag_raise("v119_selection")
    except Exception:
        pass


def v119_install_bindtags(self):
    # Pré-bind global AVANT les callbacks normaux.
    for widget in v119_all_widgets(self.root):
        try:
            tags = tuple(widget.bindtags())
            if "V119Pre" not in tags:
                widget.bindtags(("V119Pre",) + tags)
        except Exception:
            pass

    try:
        self.root.bind_class("V119Pre", "<Button-1>", self.v119_on_click_pre)
        self.root.bind_class("V119Pre", "<B1-Motion>", self.v119_on_motion_pre)
        self.root.bind_class("V119Pre", "<ButtonRelease-1>", self.v119_on_release_pre)
    except Exception as exc:
        print("[v119] bind_class souris impossible :", exc)

    try:
        self.root.bind("<Left>", self.v119_left)
        self.root.bind("<Right>", self.v119_right)
        self.root.bind("<Up>", self.v119_up)
        self.root.bind("<Down>", self.v119_down)
        self.root.bind("<Escape>", self.v119_escape)
    except Exception as exc:
        print("[v119] bind clavier root impossible :", exc)

    try:
        self.root.bind_all("<Left>", self.v119_left)
        self.root.bind_all("<Right>", self.v119_right)
        self.root.bind_all("<Up>", self.v119_up)
        self.root.bind_all("<Down>", self.v119_down)
        self.root.bind_all("<Escape>", self.v119_escape)
    except Exception:
        pass

    print("[v119] bind robuste Slice installé.")


_old_v119_draw = getattr(SliceIndexTracker, "draw", None)


def v119_draw(self, *args, **kwargs):
    result = None

    if callable(_old_v119_draw):
        result = _old_v119_draw(self, *args, **kwargs)
...
_old_v119_build_ui = SliceIndexTracker.build_ui


def v119_build_ui(self):
    _old_v119_build_ui(self)

    try:
        self.root.title("BreakbeatAI v119 — Slice row select robust")
    except Exception:
        pass

    try:
        v119_install_bindtags(self)
    except Exception as exc:
        print("[v119] install bindtags impossible :", exc)

    # Deuxième passe après création tardive éventuelle des widgets.
    try:
        self.root.after(700, lambda: v119_install_bindtags(self))
    except Exception:
        pass

    v119_status(self, "v119 : clique sur Slice N pour sélectionner toute la ligne.")


SliceIndexTracker.v119_on_click_pre = v119_on_click_pre
SliceIndexTracker.v119_on_motion_pre = v119_on_motion_pre
SliceIndexTracker.v119_on_release_pre = v119_on_release_pre
SliceIndexTracker.v119_left = v119_left
SliceIndexTracker.v119_right = v119_right
SliceIndexTracker.v119_up = v119_up
SliceIndexTracker.v119_down = v119_down
SliceIndexTracker.v119_escape = v119_escape

if callable(_old_v119_draw):
    SliceIndexTracker.draw = v119_draw

SliceIndexTracker.build_ui = v119_build_ui
