...
"""
BreakbeatAI Tracker Editor v26
...
# v26 : rôle réel des samples/pairs.
...
def block_role(block):
    """
    v26 : rôle réel du sample.
    pair 7 est override en snare.
    """
    try:
...
def load_wav(path):
    ...
    # v26 : pas de normalisation/fade par sample.
...
class TrackerEditorApp:
    def __init__(self, root, pair_json):
        ...
        # v26 : grille 32 positions = 4 cycles de break8.
        ...
        self.root.title("BreakbeatAI Tracker Editor v26")
    ...
    def guess_step_ms(self):
        # v26 : une colonne = une position break8.
    ...
    def detect_external_player(self):
        for name in ["pw-play", "paplay", "aplay", "ffplay"]:
            ...
            if path:
                print(f"[v26] Backend audio système trouvé : {name} -> {path}")
                return name

        print("[v26] Aucun backend système trouvé, fallback sounddevice.")
        return None
    ...
    def base_pair_for_position(self, local_pos):
        """
        Cycle de base.
        Si le sample de même numéro n'a pas le bon rôle réel,
        on choisit un autre sample de la bonne famille.
        Exemple : position 7 veut un hat, mais pair 7 est snare,
        donc on prend un hat disponible à la place.
        """
        ...
        if pool:
            return int(pool[local_pos % len(pool)])
    ...
    def build_initial_pattern(self):
        ...
        print("[v26] Génération loop32 verrouillée")
        print("[v26] pattern =", " ".join(BREAK8_ROLES))
        ...
        for pos in range(32):
            ...
            if actual_role != target_role:
                print(
                    f"[v26][WARN] pos {pos}: target {target_role}, "
            ...
            print(
                f"[v26] pos {pos:02d} | target {target_role:5s} | "
    ...
    def build_ui(self):
        ...
        title = tk.Label(
            main,
            text="BreakbeatAI Tracker Editor v26 — pair 7 = snare",
            bg="#111018",
            fg="#ff7acc",
            font=("Sans", 20, "bold"),
        )
        ...
        self.output_label = tk.Label(
            panel,
            text="v26 : pair 7 est une snare. La position 7 reste hat et choisit un autre hat disponible.",
            bg="#1b1824",
            fg="#77f5b5",
            justify="left",
        )
    ...
    def bind_keys(self):
        ...
        print("[v26] Raccourcis : Espace loop | flèches = déplacer | Delete = supprimer")
    ...
    def play_audio_array(self, audio, label="audio"):
        ...
        live_wav = OUT_DIR / f"{self.project['safe']}_v26_live.wav"
        ...
        if self.external_player:
            ...
            try:
                ...
                print(f"[v26] {label} lancé avec {self.external_player} : {live_wav}")
                return duration_ms
            except Exception as exc:
                print(f"[v26] Erreur backend système {self.external_player} : {exc}")
    ...
    def render_preview_file(self):
        OUT_DIR.mkdir(parents=True, exist_ok=True)
        wav = OUT_DIR / f"{self.project['safe']}_tracker_app_v26_preview.wav"
    ...
    def toggle_loop(self):
        if self.looping:
            ...
            print("[v26] Loop arrêtée")
            return

        self.stop_playhead()
        print("[v26] Lancement loop...")
        self.looping = True
    ...
    def save(self):
        ...
        data = {
            "version": "tracker_app_edit_v26_exact_cells_no_sample_fade",
        ...
        path = OUT_DIR / f"{self.project['safe']}_tracker_app_edit_v26.json"
