...
"""
BreakbeatAI Tracker Editor v133 — slice index
...
def warp_audio_to_length_external(audio, target_len, cache_key=None):
    ...
    if rubberband:
        try:
            ...
            print(f"[v133] rubberband échec, fallback ffmpeg/no-speed : {exc}")
    ...
    if ffmpeg:
        try:
            ...
            print(f"[v133] ffmpeg atempo échec, fallback no-speed : {exc}")
...
class SliceIndexTracker:
    def __init__(self, root, pair_json):
        ...
        self.root.title("BreakbeatAI Tracker Editor v133 — slice index")
    ...
    def detect_external_player(self):
        for name in ["pw-play", "paplay", "aplay", "ffplay"]:
            ...
            if path:
                print(f"[v133] Backend audio : {name} -> {path}")
                return name

        print("[v133] Aucun backend système trouvé, fallback sounddevice.")
        return None
    ...
    def build_initial_pattern(self, randomize=False):
        ...
        print(
            f"[v133] Génération pattern | break={self.project['safe']} | "
        ...
        for slot in range(max_items):
            ...
            print(f"[v133] slot {slot:02d} | step {x_step:02d} -> slice {pair:02d}")
    ...
    def build_ui(self):
        ...
        title = tk.Label(
            main,
            text="BreakbeatAI v133 — hide bottom section",
            bg="#111018",
            fg="#ff7acc",
            font=("Sans", 20, "bold"),
        )
    ...
    def record_correction(self, event_type, before=None, after=None):
        ...
        print(f"[v133] correction sauvegardée : {event_type}")
    ...
    def play_audition_array(self, audio, label="audition"):
        ...
        audition_wav = OUT_DIR / f"{self.project['safe']}_v133_audition.wav"
        ...
        if self.external_player:
            try:
                ...
                print(f"[v133] Erreur audition {self.external_player} : {exc}")
    ...
    def on_close(self):
        ...
        """
        print("[v133] Fermeture : arrêt audio + destruction fenêtre")
    ...
    def play_audio_array(self, audio, label="audio"):
        ...
        audio = normalize(audio)
        live_wav = OUT_DIR / f"{self.project['safe']}_v133_live.wav"
        ...
        if self.external_player:
            try:
                ...
                print(f"[v133] Erreur backend {self.external_player} : {exc}")
    ...
    def render_preview_file(self):
        OUT_DIR.mkdir(parents=True, exist_ok=True)
        wav = OUT_DIR / f"{self.project['safe']}_tracker_app_v133_preview.wav"
    ...
    def write_latest_pattern(self, reason="correction"):
        ...
        path = LATEST_DIR / f"{self.project['safe']}_latest_slice_index_v133.json"

        data = {
            "version": "breakbeatai_latest_slice_index_v133",
            "reason": reason,
    ...
    def save(self):
        ...
        tracker_path = OUT_DIR / f"{self.project['safe']}_tracker_app_edit_v133.json"
        ...
        stamp = file_stamp()
        validated_path = VALIDATED_DIR / f"{self.project['safe']}_validated_slice_index_loop64_v133_{stamp}.json"

        validated = {
            "version": "breakbeatai_validated_slice_index_loop64_v133",
...
def v44_record_correction(self, event_type, before=None, after=None):
    ...
    print(f"[v133] correction sauvegardée : {event_type}")
    print(f"[v133] correction path : {corrections_path}")
    print(f"[v133] latest path : {latest_path}")
...
def v45_move_selected(self, dx, dy):
    ...
    if dy != 0 and before != after:
        try:
            ...
            print(f"[v133] audition après flèche haut/bas impossible : {exc}")
...
def v46_warp_loop_pitch_preserve(audio, target_len, cache_key):
    ...
    if rubberband:
        try:
            ...
            print(f"[v133] rubberband global warp échec : {exc}")
    ...
    if ffmpeg:
        try:
            ...
            print(f"[v133] ffmpeg global warp échec : {exc}")

    print("[v133] Aucun warp pitch-preserve dispo, fallback fit no-speed.")
    return v46_fit_audio_no_speed(audio, target_len)
...
def v46_build_initial_pattern(self, randomize=False):
    ...
    print(
        f"[v133] M8 pattern | break={self.project['safe']} | "
...
def v46_get_warped_loop(self):
    ...
    print(
        f"[v133] global warp OK | source_len={loop_end-loop_start} | "
...
def v46_build_ui(self):
    ...
    try:
        ...
        print(f"[v133] UI extension impossible : {exc}")
...
def v47_play_audio_array(self, audio, label="audio"):
    ...
    if len(audio) == 0 or v47_is_silent(audio):
        ...
        print(f"[v133] lecture annulée : audio silencieux ({label})")
        return 0
...
def v47_toggle_loop(self):
    ...
    if not getattr(self, "pattern", None):
        ...
        print("[v133] loop annulée : pattern vide")
        return "break"
...
def v47_delete_selected(self):
    ...
    if not getattr(self, "pattern", None):
        ...
        print("[v133] pattern vide après suppression : audio coupé")
...
def v48_build_initial_pattern(self, randomize=False):
    ...
    print("[v133] pattern initial forcé VIDE : aucun son ne doit jouer.")
    return []
...
def v48_load_first_slices(self):
    ...
    print(f"[v133] Load 16 slices : {len(self.pattern)} items")


def v48_debug_pattern(self, label="render"):
    print("")
    print(f"[v133] DEBUG PATTERN avant {label}")
    print(f"[v133] items = {len(getattr(self, 'pattern', []) or [])}")

    if not getattr(self, "pattern", None):
        print("[v133] pattern vide confirmé")
        print("")
        return
    ...
    for item in self.pattern:
        ...
        print(
            f"[v133] id={item_id} | step {x:02d}/{end:02d} | "
...
def v48_render_audio_with_timeline(self):
    ...
    if not getattr(self, "pattern", None):
        loop_samples = self.get_loop_samples()
        print(f"[v133] rendu silence total : pattern vide | samples={loop_samples}")
        return np.zeros(loop_samples, dtype=np.float32), []
    ...
    print(f"[v133] rendu audio peak={peak:.8f} | timeline_events={len(timeline) if timeline is not None else 'None'}")

    if timeline:
        print("[v133] timeline events:")
        for ev in timeline:
            print(f"[v133]   {ev}")
...
def v48_toggle_loop(self):
    if not getattr(self, "pattern", None):
        ...
        print("[v133] Space ignoré : pattern vide.")
        return "break"
...
def v48_build_ui(self):
    ...
    try:
        ...
        print(f"[v133] ajout UI impossible : {exc}")
...
def v49_add_item_at_xy(self, x, y):
    ...
    try:
        ...
        print(f"[v133] record_correction add impossible : {exc}")
    ...
    try:
        ...
        print(f"[v133] audition après ajout impossible : {exc}")

    print(f"[v133] add | id={item['id']} | step={x_step} | pair={pair} | lane={lane}")
...
def v49_duplicate_selected_forward(self, event=None):
    ...
    if item is None:
        ...
        print("[v133] duplicate annulé : aucune note sélectionnée")
        return "break"
    ...
    if new_x > max_x:
        ...
        print("[v133] duplicate annulé : fin de grille")
        return "break"
    ...
    try:
        ...
        print(f"[v133] record_correction duplicate impossible : {exc}")
    ...
    try:
        ...
        print(f"[v133] audition après duplicate impossible : {exc}")

    print(
        f"[v133] duplicate | old_id={item.get('id')} | new_id={new_item.get('id')} | "
...
def v49_build_ui(self):
    ...
    try:
        ...
        print(f"[v133] bind canvas click impossible : {exc}")
    ...
    try:
        ...
        print(f"[v133] bind keys impossible : {exc}")
...
def v50_load_style_model():
    ...
    try:
        ...
        print(f"[v133] impossible de charger le modèle IA : {exc}")
        return None
...
def v50_generate_ai_pattern(self, event=None):
    ...
    if model is None:
        ...
        print("[v133] modèle absent :", STYLE_MODEL_PATH)
        return "break"
    ...
    print("")
    print(f"[v133] GENERATE IA | safe={safe} | pairs={pair_values} | slots={max_slots}")
    ...
    for slot in range(max_slots):
        ...
        print(
            f"[v133] slot {slot:02d} | step {x_step:02d} | "
    ...
    try:
        ...
        print(f"[v133] write_latest_pattern impossible : {exc}")
...
def v50_build_ui(self):
    ...
    try:
        ...
        print(f"[v133] UI IA impossible : {exc}")
...
def v51_mouse_down(self, event):
    ...
    if item is not None:
        ...
        print(
            f"[v133] mouse down note | id={item.get('id')} | "
            f"step={item.get('x_step')} | pair={item.get('pair')}"
        )
    else:
        print("[v133] mouse down empty")
...
def v51_mouse_up(self, event):
    ...
    if state.get("empty"):
        ...
        print("[v133] drag depuis vide ignoré")
        return "break"
    ...
    if before != after:
        try:
            ...
            print(f"[v133] record_correction drag impossible : {exc}")
        ...
        print(
            f"[v133] drag move | id={after.get('id')} | "
            f"step {before.get('x_step')} -> {after.get('x_step')} | "
        ...
        try:
            ...
            print(f"[v133] audition après drag impossible : {exc}")
...
def v51_build_ui(self):
    ...
    try:
        ...
        print(f"[v133] bind souris impossible : {exc}")
    ...
    try:
        ...
        print(f"[v133] bind Suppr impossible : {exc}")
...
def v52_normalize_loaded_notes(self, notes):
    ...
    for raw in sorted(notes, key=lambda n: (int(float(n.get("x_step", 0))), int(float(n.get("pair", 0))))):
        ...
        if pair not in valid_pairs:
            print(f"[v133] slice ignorée car absente du break actuel : pair={pair}")
            continue
...
def v52_load_saved_beat(self, event=None):
    ...
    if not saves:
        ...
        print("[v133] aucune sauvegarde trouvée")
        return "break"
    ...
    if not pattern:
        ...
        print("[v133] sauvegarde incompatible :", chosen["path"])
        return "break"
    ...
    print("")
    print("[v133] BEAT RESTAURÉ")
    print("[v133] fichier :", chosen["path"])
    print("[v133] clé :", chosen["key"])
    print("[v133] notes :", len(pattern))
    print("")
...
def v52_list_saved_beats(self, event=None):
    ...
    print("")
    print("[v133] SAUVEGARDES TROUVÉES")
    for i, save in enumerate(saves[:30]):
        print(
            f"[v133] {i:02d} | notes={len(save['notes']):02d} | "
...
def v52_build_ui(self):
    ...
    try:
        ...
        print(f"[v133] UI load saved impossible : {exc}")
...
def v53_run_training(self):
    ...
    if not trainer.exists():
        ...
        print("[v133] trainer introuvable :", trainer)
        return False

    print("")
    print("[v133] AUTO-TRAIN IA après Save...")
    print("")
    ...
    try:
        ...
        if result.returncode != 0:
            ...
            print("[v133] auto-train échec code :", result.returncode)
            return False

        v53_status(self, "Save OK + IA entraînée automatiquement.")
        print("[v133] AUTO-TRAIN OK")
        return True

    except Exception as exc:
        v53_status(self, f"Save OK, mais auto-train impossible : {exc}")
        print("[v133] exception auto-train :", exc)
        return False
...
def v53_save(self):
    ...
    try:
        ...
        print(f"[v133] auto-train après save impossible : {exc}")
...
def v53_build_ui(self):
    ...
    try:
        ...
        print(f"[v133] UI auto-train impossible : {exc}")
...
def v55_mouse_up(self, event):
    ...
    if not state.get("moved"):
        ...
        try:
            ...
            print(f"[v133] audition clic note impossible : {exc}")
        ...
        print(
            f"[v133] click play/select | id={item.get('id')} | "
            f"step={item.get('x_step')} | pair={item.get('pair')}"
        )
...
def v58_add_item_at_xy(self, x, y):
    ...
    print(
        f"[v133] add | id={item['id']} | step={x_step} | "
...
def v58_toggle_selected_length(self, event=None):
    ...
    try:
        ...
        print(f"[v133] record_correction length impossible : {exc}")

    v58_refresh_after_edit(self, "toggle_note_length")

    v58_status(self, f"Longueur note : {old_len} -> {new_len} case(s).")
    print(f"[v133] length toggle | id={item.get('id')} | {old_len} -> {new_len}")
...
def v58_duplicate_selected_forward(self, event=None):
    ...
    try:
        ...
        print(f"[v133] audition duplicate impossible : {exc}")
...
def v58_build_ui(self):
    ...
    try:
        ...
        print(f"[v133] UI demi-grille impossible : {exc}")
    ...
    try:
        ...
        print(f"[v133] bind demi-grille impossible : {exc}")
...
def v59_rank_current_slices(self):
    ...
    for pair in self.pair_values:
        ...
        try:
            ...
            print(f"[v133] analyse pair {pair} impossible : {exc}")
            role = "other"
    ...
    print("")
    print("[v133] TOP ROLES CURRENT BREAK")
    for r in ("kick", "snare", "hat"):
        print(f"[v133] {r:5s}:", [(x["pair"], round(x["score"], 3)) for x in ranks[r][:6]])
    print("")
...
def v59_load_role_model():
    ...
    try:
        ...
        print(f"[v133] lecture modèle rôle impossible : {exc}")
        return None
...
def v59_generate_role_aware(self, event=None):
    ...
    if model is None:
        ...
        print("[v133] modèle rôle absent, fallback musical snare 6.")
        model = {
            "breaks": {},
            "global": {},
            "patterns_used": 0,
    ...
    print("")
    print("[v133] ROLE-AWARE GENERATION")
    print("[v133] model patterns_used:", model.get("patterns_used", 0))
    for item in pattern:
        print(
            f"[v133] step {item['x_step']:02d}/{item['x_step'] + item['length'] - 1:02d} | "
...
def v59_run_role_training(self, event=None):
    ...
    try:
        ...
        print(f"[v133] training rôle impossible : {exc}")
...
def v59_save(self):
    ...
    try:
        ...
        print(f"[v133] auto-training rôle après save impossible : {exc}")
...
def v59_build_ui(self):
    ...
    try:
        ...
        print(f"[v133] UI role-aware impossible : {exc}")
...
def v62_add_item_at_xy(self, x, y):
    ...
    print(f"[v133] add | id={item['id']} | step={x_step} | len={length} | pair={pair}")
...
def v62_mouse_down(self, event):
    ...
    if item is not None:
        ...
        print(
            f"[v133] mouse down {state['mode']} | id={item.get('id')} | "
            f"step={item.get('x_step')} | len={item.get('length')} | pair={item.get('pair')}"
        )
    else:
        print("[v133] mouse down empty")
    ...
    if mode == "resize":
        if not state.get("moved"):
            ...
            try:
                ...
                print(f"[v133] audition clic resize impossible : {exc}")
        ...
        if before != after:
            try:
                ...
                print(f"[v133] record_correction resize impossible : {exc}")
            ...
            try:
                ...
                print(f"[v133] audition après resize impossible : {exc}")
    ...
    if mode == "move":
        ...
        if before != after:
            try:
                ...
                print(f"[v133] record_correction move impossible : {exc}")
            ...
            try:
                ...
                print(f"[v133] audition après move impossible : {exc}")
...
def v62_toggle_selected_length(self, event=None):
    ...
    try:
        ...
        print(f"[v133] record_correction toggle length impossible : {exc}")

    v62_refresh_after_edit(self, "toggle_note_length_v62")
    v62_status(self, f"Longueur note : {old_len} → {new_len} case(s).")

    print(f"[v133] toggle length | id={item.get('id')} | {old_len} -> {new_len}")
...
def v62_draw(self):
    ...
    try:
        ...
        print(f"[v133] draw handles impossible : {exc}")
...
def v62_build_ui(self):
    ...
    try:
        ...
        print(f"[v133] bind resize impossible : {exc}")
    ...
    try:
        ...
        print(f"[v133] UI resize impossible : {exc}")
...
def v63_load_model():
    ...
    try:
        ...
        print(f"[v133] lecture modèle strict impossible : {exc}")
        return None
...
def v63_generate_safe_role_ai(self, event=None):
    ...
    print("")
    print("[v133] SAFE ROLE AI GENERATED")
    print("[v133] model strict:", "yes" if model else "no")
    print("[v133] snare main/alt:", main_snare, alt_snare)
    print("[v133] kick  main/alt:", main_kick, alt_kick)
    print("[v133] hat   main/alt:", main_hat, alt_hat)
    print("[v133] pattern:")
    for item in pattern:
        print(
            f"[v133] step {item['x_step']:02d}/{item['x_step'] + item['length'] - 1:02d} | "
...
def v63_run_strict_training(self, event=None):
    ...
    print("")
    print("[v133] TRAIN STRICT ROLE MODEL...")
    print("")
    ...
    try:
        ...
        print(f"[v133] training strict impossible : {exc}")
...
def v63_build_ui(self):
    ...
    try:
        ...
        print(f"[v133] UI safe impossible : {exc}")
...
def v65_rank_slices(self):
    ...
    for pair in self.pair_values:
        ...
        try:
            ...
            print(f"[v133] analyse slice {pair} impossible : {exc}")
            scores = {
                "kick": 0.0,
                "snare": 0.0,
                "hat": 0.0,
    ...
    print("")
    print("[v133] TOP SLICES PAR ROLE — break courant")
    for role in ("kick", "snare", "hat", "ghost_snare"):
        print(f"[v133] {role:12s}:", [(x["pair"], round(x["score"], 3)) for x in ranks[role][:8]])
    print("")
...
def v65_generate_from_role_template(self, event=None):
    ...
    try:
        ...
        print(f"[v133] template introuvable : {exc}")
        return "break"
    ...
    print("")
    print("[v133] CROSS-SONG ROLE TEMPLATE GENERATED")
    print("[v133] template:", V65_TEMPLATE_PATH)
    print("[v133] temperature:", temperature, "| hats:", hat_density, "| ghosts:", ghost_density)
    print("[v133] pattern:")
    for item in pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v133] step {item['x_step']:02d}/{end:02d} | "
...
def v65_build_ui(self):
    ...
    try:
        ...
        print(f"[v133] UI impossible : {exc}")
...
def v67_train_strict_if_available(self):
    ...
    if trainer is None:
        print("[v133] aucun trainer trouvé.")
        return False

    print("")
    print("[v133] TRAIN après validation :", trainer)
    print("")
    ...
    try:
        ...
        print(f"[v133] training impossible : {exc}")
        return False
...
def v67_generate_candidate(self, event=None):
    ...
    print("")
    print("[v133] CANDIDATE GENERATED")
    print("[v133] break:", self.v67_candidate_safe)
    print("[v133] notes:", len(candidate))
    print("[v133] Maintenant : corrige puis clique Good après modifs, ou Reject.")
    print("")
...
def v67_reject_candidate(self, event=None):
    ...
    print("")
    print("[v133] REJECTED")
    print("[v133] saved:", rejected_path)
    print("")
...
def v67_accept_corrected(self, event=None):
    ...
    print("")
    print("[v133] ACCEPTED GOOD AFTER EDIT")
    print("[v133] saved:", validated_path)
    print("[v133] trained:", trained)
    print("")
...
def v67_accept_without_training(self, event=None):
    ...
    print("[v133] accepted no-train:", validated_path)
...
def v67_build_ui(self):
    ...
    try:
        ...
        print(f"[v133] UI feedback impossible : {exc}")
...
def v68_generate_candidate(self, event=None):
    ...
    print("[v133] Length policy appliquée : 2 cases par défaut, 1 case rare.")
...
def v69_force_pattern_hard_2_grid(self, reason="v69_force_hard_2_grid"):
    ...
    print("")
    print("[v133] HARD 2 GRID APPLIQUÉ")
    for item in self.pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v133] step {item['x_step']:02d}/{end:02d} | "
...
def v70_duplicate_selected_forward(self, event=None):
    ...
    print("")
    print("[v133] CTRL+D DUPLICATE +2")
    print(f"[v133] old_id={item.get('id')} new_id={new_item.get('id')}")
    print(f"[v133] step {old_x}/{old_x + length - 1} -> {new_x}/{new_x + length - 1}")
    print(f"[v133] pair={new_item.get('pair')} len={length}")
    print("")
...
def v70_build_ui(self):
    ...
    try:
        ...
        print(f"[v133] bind Ctrl+D impossible : {exc}")
...
def v71_rank_slices(self):
    ...
    print("")
    print("[v133] PRE-LEARNING TOP SLICES")
    for role in ("kick", "snare", "hat", "ghost_snare"):
        print(f"[v133] {role:12s}:", [(x["pair"], round(x["score"], 3)) for x in ranks[role][:8]])
    print("")
...
def v71_prelearn_roles(self, force=False):
    ...
    print("")
    print("[v133] PRELEARN ROLES POUR", safe)
    for role in ("kick", "snare", "hat", "ghost_snare", "bad"):
        print(f"[v133] {role:12s}:", roles.get(role, []))
    print("")
...
def v71_generate_full_candidate(self, event=None):
    ...
    print("")
    print("[v133] FULL CANDIDATE GENERATED")
    print("[v133] break:", v71_safe(self))
    print("[v133] roles:", roles)
    print("[v133] pattern:")
    for item in pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
...
def v72_open_break(self, safe=None):
    ...
    print("")
    print("[v133] OPEN BREAK")
    print("[v133] current:", current)
    print("[v133] next   :", safe)
    print("[v133] cmd    :", " ".join(cmd))
    print("")
...
def v72_build_ui(self):
    ...
    try:
        ...
        print(f"[v133] UI break switcher impossible : {exc}")
...
def v73_sanitize_tracker_pattern(self, reason="v73_tracker_strict_no_overlap"):
    ...
    print("")
    print("[v133] TRACKER STRICT NO OVERLAP")
    print("[v133] kept:", len(accepted), "| rejected overlaps:", len(rejected))

    for item in accepted:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v133] KEEP step {item['x_step']:02d}/{end:02d} | "
    ...
    if rejected:
        ...
        for item in rejected:
            ...
            print(
                f"[v133] DROP step {item['x_step']:02d}/{end:02d} | "
                f"role={item.get('v73_role_key')} | pair={item.get('pair')}"
            )
...
def v81_launch_break(self, source):
    ...
    print("")
    print("[v133] CHARGER BREAK")
    print("[v133] courant :", current)
    print("[v133] nouveau :", source)
    print("[v133] cmd     :", " ".join(cmd))
    print("")
...
def v81_patch_charger_break_buttons(self):
    ...
    print(f"[v133] boutons Charger break patchés : {patched}")
...
def v81_generate_candidate(self, event=None):
    ...
    print("")
    print("[v133] GENERATE CANDIDATE")
    print("[v133] break chargé :", current)
    print("[v133] générateur   :", getattr(fn, "__name__", None))
    print("")
...
def v81_run_training(self):
    ...
    if script is None:
        print("[v133] Aucun script training trouvé.")
...
def v81_save_feedback(self, verdict, train_now=False):
    ...
    print("")
    print("[v133] FEEDBACK SAUVÉ")
    print("[v133] break  :", current)
    print("[v133] verdict:", verdict)
    print("[v133] train  :", train_now)
    print("[v133] fichier:", out_path)
    print("")
...
def v81_build_ui(self):
    ...
    try:
        self.root.title(f"BreakbeatAI v133 — hide bottom section")
    except Exception:
        pass
    ...
    try:
        ...
        print(f"[v133] patch Charger break impossible : {exc}")

    try:
        v81_install_training_toolbar(self)
    except Exception as exc:
        print(f"[v133] toolbar impossible : {exc}")
...
def v82_auto_role_pools(self):
    ...
    print("")
    print("[v133] ROLE POOLS")
    for r in ["kick", "snare", "hat", "ghost_snare", "bad"]:
        print(f"[v133] {r:12s}:", roles.get(r, []))
    print("")
...
def v82_generate_candidate(self, event=None):
    ...
    print("")
    print("[v133] NEW CANDIDATE")
    print("[v133] break :", current)
    print("[v133] nonce :", nonce)
    print("[v133] rejected known:", len(rejected))
    print("[v133] fingerprint rejeté ?: ", chosen_fp in rejected)
    for item in self.pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v133] step {item['x_step']:02d}/{end:02d} "
...
def v82_patch_training_buttons(self):
    ...
    print(f"[v133] boutons training patchés : {patched}")
...
def v82_build_ui(self):
    ...
    try:
        ...
        print(f"[v133] patch boutons impossible : {exc}")
...
def v110_generate(self, event=None, reject_mode=False):
    ...
    print("")
    if reject_mode:
        print("[v133] REJECT/BAD -> variation premier 32 + autres lignes")
    else:
        print("[v133] GENERATE 64")
    print("[v133] break:", current)
    print("[v133] first32:", first32_id)
    print("[v133] response:", response_id)
    print("[v133] role_offset:", role_offset)
    for role in ["kick", "snare", "hat"]:
        lane = "?"
        try:
            ...
            pass
        print(f"[v133] {role:6s}: pair={chosen[role]['pair']} lane={lane} source={chosen[role]['source']}")
    print("")
    ...
    try:
        ...
        print(f"[v133] draw impossible : {exc}")
    ...
    try:
        ...
        print(f"[v133] refresh_panel impossible : {exc}")
...
def v110_render_pattern_to_audio(self):
    ...
    for item in pattern:
        ...
        if pair not in slice_map:
            skipped += 1
            print(f"[v133] skip pair introuvable : {pair}")
            continue
...
def v110_export_wav_current(self, event=None):
    try:
        ...
        print("")
        print("[v133] EXPORT WAV OK")
        print("[v133] break:", current)
        print("[v133] wav  :", out_path)
        print("[v133] json :", info_path)
        print("[v133] duration:", round(info["duration_seconds"], 3), "s")
        print("[v133] rendered:", info["rendered_notes"], "skipped:", info["skipped_notes"])
        print("")
...
def v110_space_play(self, event=None):
    ...
    try:
        ...
        if btn is not None:
            print("[v133] SPACE -> Loop / Space")
            try:
                btn.invoke()
            except Exception as exc:
                print("[v133] invoke Loop / Space impossible :", exc)
            return "break"

        print("[v133] SPACE : bouton Loop / Space introuvable.")
...
def v110_patch_buttons(self):
    ...
    print(f"[v133] boutons patchés : Generate={gen_patched}, Reject/Bad={bad_patched}")
...
def v110_build_ui(self):
    ...
    try:
        ...
        print("[v133] patch boutons impossible :", exc)

    try:
        v110_add_export_button(self)
    except Exception as exc:
        print("[v133] ajout Export WAV impossible :", exc)

    try:
        v110_install_space_bindings(self)
    except Exception as exc:
        print("[v133] bind espace impossible :", exc)
...
def v113_export(self, event=None):
    ...
    print("[v133] ERREUR : export_wav_current_v110 introuvable.")
...
def v113_place_export_button(self):
    ...
    try:
        ...
        print("[v133] bouton Export WAV visible ajouté.")
        v113_status(self, "v113 : Export WAV visible. Espace reste Loop / Space.")

    except Exception as exc:
        print("[v133] placement bouton Export WAV impossible :", exc)
        try:
            btn.place(x=760, y=32, height=30)
            btn.lift()
            self.export_button_v113 = btn
...
def v115_apply_cells_minus_25(self):
    ...
    print("")
    print("[v133] APPLICATION CASES -3.5px")
    ...
    for name in V115_WIDTH_GLOBAL_NAMES:
        try:
            ...
            if isinstance(value, (int, float)) and value > 6:
                ...
                print(f"[v133] global {name}: {value} -> {globals()[name]}")
        except Exception:
            pass
        ...
        try:
            if hasattr(self, attr):
                ...
                if isinstance(value, (int, float)) and value > 6:
                    ...
                    print(f"[v133] attr {attr}: {value} -> {getattr(self, attr)}")
        except Exception:
            pass
...
def v115_build_ui(self):
    ...
    try:
        self.root.title("BreakbeatAI v133 — hide bottom section")
    except Exception:
        pass
    ...
    try:
        ...
        print("[v133] impossible d'appliquer -3.5px :", exc)
...
def v117_render_pattern_to_audio(self):
    ...
    print("")
    print("[v133] EXPORT EXACT CELLS")
    print("[v133] break:", current)
    print("[v133] pairblock:", pairblock_file)
    print("[v133] sr:", sr)
    print("[v133] median_pair_samples:", median_pair_samples)
    print("[v133] case_samples:", case_samples)
    print("[v133] export_samples:", total_samples)
    print("[v133] export_seconds:", round(total_samples / float(sr), 6))
    print("")
...
def v117_export_wav_current(self, event=None):
    try:
        ...
        print("[v133] wav  :", out_path)
        print("[v133] json :", info_path)
        print("[v133] duration:", round(info["duration_seconds"], 6), "s")
        print("[v133] case_seconds:", round(info["case_seconds"], 6))
        print("[v133] rendered:", info["rendered_notes"], "skipped:", info["skipped_notes"])
        print("")
...
def v118_current_break(self):
    ...
    if candidates:
        ...
        print("[v133] break courant:", stem, "| source:", source, "| raw:", raw)
        return stem

    print("[v133] WARNING : break courant introuvable, fallback Camo.")
    return "Camo_Break_-_3A"
...
def v118_find_best_playback_wav(since):
    ...
    print("")
    print("[v133] WAV candidats playback :")
    for score, mtime, size, wav in candidates[:12]:
        age = time.time() - mtime
        print(f"[v133] score={score:8d} age={age:7.2f}s size={size:10d} path={wav}")
    print("")
...
def v118_copy_playback_wav(self, since):
    ...
    if best is None:
        ...
        print("[v133] aucun WAV playback trouvé.")
        return "break"
    ...
    print("")
    print("[v133] EXPORT PLAYBACK OK")
    print("[v133] source :", best)
    print("[v133] export :", out)
    print("[v133] json   :", info_path)
    print("")
...
def v118_export_playback_wav(self, event=None):
    ...
    try:
        ...
        if btn is not None:
            print("[v133] déclenche Loop / Space pour générer le playback WAV...")
            btn.invoke()
        else:
            print("[v133] bouton Loop / Space introuvable, copie du dernier WAV playback existant.")
    except Exception as exc:
        print("[v133] invoke Loop / Space impossible :", exc)
...
def v121_find_best_long_wav(since):
    ...
    print("")
    print("[v133] WAV candidats longs/propre à raccourcir :")
    for row in candidates[:20]:
        age = time.time() - row["mtime"]
        print(
            f"[v133] score={row['score']:7d} "
...
def v121_export_trimmed_long(self, event=None):
    ...
    try:
        ...
        if btn is not None:
            print("[v133] déclenche Loop / Space pour générer le WAV long/propre...")
            btn.invoke()
    except Exception as exc:
        print("[v133] Loop / Space impossible :", exc)
    ...
    def do_export():
        try:
            ...
            try:
                ...
                print("[v133] ffmpeg trim impossible, fallback wave :", exc)
                ok = False
            ...
            print("")
            print("[v133] EXPORT TRIM OK")
            print("[v133] source :", src_wav)
            print("[v133] source durée :", round(src_dur, 6), "s")
            print("[v133] target durée :", round(target_seconds, 6), "s")
            print("[v133] export :", out)
            print("[v133] export durée :", round(float(out_dur or 0), 6), "s")
            print("[v133] json :", info_path)
            print("")
...
def v124_export_trimmed_long_no_json(self, event=None):
    ...
    def do_export():
        try:
            ...
            print("")
            print("[v133] EXPORT WAV ONLY OK")
            print("[v133] source :", src_wav)
            print("[v133] source durée :", round(src_dur, 6), "s")
            print("[v133] target durée :", round(target_seconds, 6), "s")
            print("[v133] export :", out)
            print("[v133] export durée :", round(float(out_dur or 0), 6), "s")
            print("[v133] aucun JSON exporté")
            print("")
...
def v125_select_lane(self, lane):
    ...
    print("")
    print(f"[v133] Slice {lane + 1} sélectionnée")
    print(f"[v133] notes sélectionnées : {ids}")
    print("")
...
def v125_redraw_after_move(self):
    try:
        ...
        print("[v133] draw impossible :", exc)
...
def v125_build_ui(self):
    ...
    if canvas is not None:
        try:
            ...
            print("[v133] bind canvas click impossible :", exc)
    ...
    try:
        ...
        print("[v133] bind flèches impossible :", exc)
...
def v126_select_slice(self, zero_index, slice_number, reason=""):
    ...
    print("")
    print("[v133] SÉLECTION SLICE")
    print("[v133] reason:", reason)
    print("[v133] Slice affichée:", slice_number)
    print("[v133] zero_index:", zero_index)
    print("[v133] pair_candidates:", pair_candidates)
    print("[v133] ids sélectionnés:", ids)
...
def v126_build_ui(self):
    ...
    if canvas is not None:
        try:
            ...
            print("[v133] bind canvas impossible :", exc)
...
def v127_select_slice(self, lane, displayed, reason=""):
    ...
    print("")
    print("[v133] SÉLECTION SLICE ALIGNÉE")
    print("[v133] reason:", reason)
    print("[v133] label affiché:", displayed)
    print("[v133] lane réelle:", lane)
    print("[v133] labels zero-based:", v127_labels_are_zero_based(self))
    print("[v133] pair_candidates:", pair_candidates)
    print("[v133] ids sélectionnés:", ids)
    for row in debug_rows:
        print("[v133] note:", row)
...
def v128_select_lane(self, lane):
    ...
    print("")
    print("[v133] MULTI-SÉLECTION LIGNE")
    print("[v133] slice/lane:", lane)
    print("[v133] ids:", ids)
    for row in debug:
        print("[v133] note:", row)
    print("")
...
def v129_draw_green_multiselect(self):
    ...
    print(f"[v133] contours verts dessinés : {drawn}")
...
def v129_draw(self, *args, **kwargs):
    ...
    try:
        ...
        print("[v133] contour vert impossible :", exc)
...
def v130_audition_after_move(self, notes):
    ...
    def do_play():
        ...
        for name in method_names:
            ...
            try:
                fn()
                print(f"[v133] audition via {name}()")
                played = True
                break
            except TypeError:
                try:
                    fn(None)
                    print(f"[v133] audition via {name}(None)")
                    played = True
                    break
                except TypeError:
                    try:
                        fn(note)
                        print(f"[v133] audition via {name}(note)")
                        played = True
                        break
                    except Exception:
                        pass
        ...
        if not played:
            ...
            if btn is not None:
                try:
                    btn.invoke()
                    print("[v133] audition via bouton Play slice")
                    played = True
                except Exception as exc:
                    print("[v133] Play slice invoke impossible :", exc)

        if not played:
            print("[v133] audition introuvable : aucune méthode Play slice trouvée")
...
def v131_generate_candidate(self, event=None, reject_mode=False):
    ...
    print("")
    print("[v133] GENERATE CANDIDATE STRICT")
    print("[v133] break:", current)
    print("[v133] sequence: kick hat snare hat | hat kick snare hat")
    print("[v133] répété sur 64 cases")
    for role in ["kick", "snare", "hat"]:
        print(f"[v133] {role:6s}: pair={chosen[role]['pair']} source={chosen[role]['source']}")
    print("")
...
def v131_build_ui(self):
    ...
    try:
        ...
        print("[v133] repatch boutons impossible :", exc)
...
# ---------------------------------------------------------------------
# v133 : cache toute la section du bas
# ---------------------------------------------------------------------
# Objectif :
# - ne pas toucher à la grille
# - garder la topbar / break selector / export en haut
# - enlever tout ce qui est SOUS la grille
# ---------------------------------------------------------------------


def v133_status(self, text):
    try:
        if hasattr(self, "set_status"):
            self.set_status(text)
        elif hasattr(self, "output_label"):
...
def v133_walk(widget):
    out = [widget]
    try:
        for child in widget.winfo_children():
            out.extend(v133_walk(child))
    except Exception:
        pass
    return out


def v133_is_ancestor(ancestor, widget):
    current = widget
...
def v133_find_canvas(self):
    # Priorité à self.canvas
    canvas = getattr(self, "canvas", None)
    if canvas is not None:
        return canvas

    for widget in v133_walk(self.root):
        try:
...
def v133_find_top_controls_y(self):
    y_min = None

    for wanted in ["Break", "Charger break", "Randomize loop64", "Refresh breaks", "Export WAV"]:
        for widget in v133_walk(self.root):
            try:
...
def v133_hide_bottom_section(self):
    self.root.update_idletasks()

    canvas = v133_find_canvas(self)
    if canvas is None:
        print("[v133] canvas introuvable")
        return
    ...
    try:
        ...
        print("[v133] géométrie canvas introuvable")
        return

    cutoff_y = canvas_root_y + canvas_h + 6
    ...
    for widget in v133_walk(self.root):
        ...
        # Ignore les widgets non encore posés
        if width <= 1 and height <= 1:
            continue

        # Si le widget est entièrement sous la grille => on le cache
        if root_y >= cutoff_y:
            v133_hide_widget(widget)
            hidden += 1

    # Réduit la fenêtre à la hauteur utile
    try:
        self.root.update_idletasks()
        top_y = v133_find_top_controls_y(self)
        if top_y is None:
            top_y = self.root.winfo_rooty()

        root_x = self.root.winfo_rootx()
        root_y = self.root.winfo_rooty()
        root_w = self.root.winfo_width()

        canvas_bottom = canvas.winfo_y() + canvas.winfo_height()
        new_h = int(canvas_bottom + 24)

        # Taille mini pour éviter une fenêtre trop serrée
        new_h = max(new_h, 500)

        self.root.geometry(f"{root_w}x{new_h}+{root_x}+{root_y}")
        try:
            self.root.minsize(root_w, new_h)
        except Exception:
            pass
    except Exception as exc:
        print("[v133] resize impossible :", exc)

    print(f"[v133] widgets du bas cachés : {hidden}")
    v133_status(self, "v133 : section du bas masquée.")


_old_v133_build_ui = SliceIndexTracker.build_ui


def v133_build_ui(self):
    _old_v133_build_ui(self)
    ...
    # On attend que toute l'UI soit réellement posée
    try:
        self.root.after(100, lambda: v133_hide_bottom_section(self))
        self.root.after(300, lambda: v133_hide_bottom_section(self))
    except Exception:
        v133_hide_bottom_section(self)


SliceIndexTracker.build_ui = v133_build_ui
