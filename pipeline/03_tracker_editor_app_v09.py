...
"""
03_tracker_editor_app_v09.py

BreakbeatAI Tracker Editor v09

Changements v09 :
- 2 lignes par famille, avec la même couleur :
    HI-HAT A / HI-HAT B
    KICK A   / KICK B
    SNARE A  / SNARE B
- Les sous-lignes servent à ranger les variantes ou découpes imparfaites.
- Le backend audio robuste de la v07 est gardé : pw-play, paplay, aplay, ffplay, puis sounddevice.
- Le rendu audio reste basé sur l'ordre horizontal x_step.
"""
...
def find_pair_json(source_query):
    ...
    if not matches:
        ...
        print(f'Essaie : python pipeline/01_find_pair_blocks_v02.py --source "{source_query}"')
        sys.exit(1)
...
class TrackerEditorApp:
    def __init__(self, root, pair_json):
        ...
        # v09 : au lieu de relancer le player à chaque tour,
        # on crée un long buffer répété pour éviter la latence entre les boucles.
        self.loop_target_sec = 180.0
        self.loop_max_repeats = 256

        self.root.title("BreakbeatAI Tracker Editor v09")
    ...
    def detect_external_player(self):
        for name in ["pw-play", "paplay", "aplay", "ffplay"]:
            ...
            if path:
                print(f"[v09] Backend audio système trouvé : {name} -> {path}")
                return name

        print("[v09] Aucun backend système trouvé, fallback sounddevice.")
        return None
    ...
    def build_ui(self):
        ...
        title = tk.Label(
            main,
            text="BreakbeatAI Tracker Editor v09 — loop sans latence + 2 lignes par famille",
            bg="#111018",
            fg="#ff7acc",
            font=("Sans", 20, "bold"),
        )
        ...
        subtitle = tk.Label(
            main,
            text=(
                f"Backend audio : {backend} | Même couleur par famille | "
                "Les sous-lignes A/B servent à ranger les variantes de découpe."
            ),
            bg="#111018",
            fg="#b9acc8",
        )
        ...
        self.info_label.grid(row=0, column=0, columnspan=11, sticky="w", padx=10, pady=8)
        ...
        tk.Button(panel, text="Audio test", command=self.play_test_tone, bg="#4d3d23", fg="#f5eefe").grid(row=1, column=4, padx=5)
        tk.Button(panel, text="Play pair", command=self.play_selected_pair, bg="#30283f", fg="#f5eefe").grid(row=1, column=5, padx=5)
        tk.Button(panel, text="Play Loop / Space", command=self.toggle_loop, bg="#30283f", fg="#f5eefe").grid(row=1, column=6, padx=5)
        tk.Button(panel, text="Render preview", command=self.render_preview_only, bg="#30283f", fg="#f5eefe").grid(row=1, column=7, padx=5)
        tk.Button(panel, text="Save data", command=self.save, bg="#30513f", fg="#f5eefe").grid(row=1, column=8, padx=5)
        tk.Button(panel, text="Delete", command=self.delete_selected, bg="#4a2630", fg="#f5eefe").grid(row=1, column=9, padx=5)
        tk.Button(panel, text="Reset", command=self.reset, bg="#30283f", fg="#f5eefe").grid(row=1, column=10, padx=5)

        self.output_label = tk.Label(
            panel,
            text=(
                "Clic vide = nouvelle case. Drag vertical = change de sous-ligne. "
                "HI-HAT A/B, KICK A/B, SNARE A/B gardent la même couleur par famille."
            ),
            bg="#1b1824",
            fg="#77f5b5",
            justify="left",
        )
        self.output_label.grid(row=2, column=0, columnspan=11, sticky="w", padx=10, pady=8)
    ...
    def bind_keys(self):
        ...
        print("[v09] Raccourcis clavier actifs : Espace = Play/Stop loop")
    ...
    def play_audio_array(self, audio, label="audio"):
        ...
        live_wav = OUT_DIR / f"{self.project['safe']}_v09_live.wav"
        ...
        if self.external_player:
            ...
            try:
                ...
                print(f"[v09] {label} lancé avec {self.external_player} : {live_wav}")
                return duration_ms
            except Exception as exc:
                print(f"[v09] Erreur backend système {self.external_player} : {exc}")
        ...
        try:
            ...
            print(f"[v09] {label} lancé avec sounddevice")
            return duration_ms
        except Exception as exc:
    ...
    def refresh_panel(self):
        ...
        self.info_label.config(
            text=(
                ...
                f"ligne {lane['label']} | rôle {lane['role']} | pair audio {item['pair']}"
            )
        )
    ...
    def render_preview_file(self):
        OUT_DIR.mkdir(parents=True, exist_ok=True)
        wav = OUT_DIR / f"{self.project['safe']}_tracker_app_v09_preview.wav"
    ...
    def build_gapless_loop_buffer(self):
        ...
        return long_loop, repeats, one_loop_ms

    def loop_tick(self):
        if not self.looping:
            return

        audio, repeats, one_loop_ms = self.build_gapless_loop_buffer()
        ...
        self.output_label.config(
            text=(
                f"Loop sans latence : motif {one_loop_ms} ms répété {repeats}x "
                f"({duration_ms / 1000:.1f}s). Stop/Start pour recharger les changements."
            )
        )

        # Redémarrage très rare seulement à la fin du long buffer.
        # La micro-latence éventuelle n'arrive donc plus à chaque boucle.
        self.loop_after_id = self.root.after(max(1000, duration_ms + 20), self.loop_tick)

    def toggle_loop_event(self, event=None):
        print("[v09] Espace détecté")
        self.toggle_loop()
        return "break"
    ...
    def toggle_loop(self):
        if self.looping:
            ...
            print("[v09] Loop arrêtée")
            return

        print("[v09] Lancement loop...")
        self.looping = True
    ...
    def save(self):
        ...
        data = {
            "version": "tracker_app_edit_v09_double_lanes_per_role",
        ...
        path = OUT_DIR / f"{self.project['safe']}_tracker_app_edit_v09.json"
