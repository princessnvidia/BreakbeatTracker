...
"""
BreakbeatAI Tracker Editor v42 — slice index
...
def warp_audio_to_length(audio, target_len):
    """
    v42 : calage exact SANS accélérer le sample.

    Contrairement à la v41, on ne resample plus la slice.
    Donc :
    - pas de pitch plus aigu
    - pas de sample accéléré
    - si c'est trop long : on coupe à la taille de case
    - si c'est trop court : on ajoute du silence

    C'est plus proche d'un mode "slice exact / gate" que d'un vrai warp Ableton.
    """
    ...
    if len(audio) > target_len:
        ...
        # Fade-out très court uniquement à la fin pour éviter le clic.
...
class SliceIndexTracker:
    def __init__(self, root, pair_json):
        ...
        self.root.title("BreakbeatAI Tracker Editor v42 — slice index")
    ...
    def detect_external_player(self):
        for name in ["pw-play", "paplay", "aplay", "ffplay"]:
            ...
            if path:
                print(f"[v42] Backend audio : {name} -> {path}")
                return name

        print("[v42] Aucun backend système trouvé, fallback sounddevice.")
        return None
    ...
    def build_initial_pattern(self, randomize=False):
        ...
        for slot in range(HIT_SLOTS):
            ...
            print(f"[v42] slot {slot:02d} | step {x_step:02d} -> slice {pair:02d}")
    ...
    def build_ui(self):
        ...
        tk.Checkbutton(panel, text="caler sans accélérer", variable=self.warp_var, bg="#1b1824", fg="#f5eefe", selectcolor="#30283f", activebackground="#1b1824", activeforeground="#f5eefe").grid(row=1, column=7, padx=4)
        ...
        self.output_label = tk.Label(
            panel,
            text="v42 : tempo par défaut 155 BPM ; clic case = son immédiat ; fermeture = stop audio.",
            bg="#1b1824",
            fg="#77f5b5",
            justify="left",
        )
    ...
    def record_correction(self, event_type, before=None, after=None):
        ...
        event = {
            "version": "breakbeatai_slice_index_correction_v42",
            "time": now_stamp(),
            "safe": safe,
        ...
        corrections_path = CORRECTIONS_DIR / f"{safe}_slice_index_corrections_v42.jsonl"
        ...
        print(f"[v42] correction sauvegardée : {event_type}")
    ...
    def stop_audition(self):
        """
        v42 : stoppe seulement le petit player d'écoute au clic,
    ...
    def play_audition_array(self, audio, label="audition"):
        ...
        audition_wav = OUT_DIR / f"{self.project['safe']}_v42_audition.wav"
        ...
        if self.external_player:
            try:
                ...
                print(f"[v42] Erreur audition {self.external_player} : {exc}")
    ...
    def audition_selected_case(self):
        """
        v42 : clic sur une case = on joue la slice de cette case.
        """
        item = self.selected()
    ...
    def on_close(self):
        """
        v42 : quand tu fermes la fenêtre, on coupe la loop, le playhead
        et le player externe avant de détruire Tk.
        """
        print("[v42] Fermeture : arrêt audio + destruction fenêtre")
    ...
    def play_audio_array(self, audio, label="audio"):
        ...
        audio = normalize(audio)
        live_wav = OUT_DIR / f"{self.project['safe']}_v42_live.wav"
        ...
        if self.external_player:
            try:
                ...
                print(f"[v42] Erreur backend {self.external_player} : {exc}")
    ...
    def render_audio_with_timeline(self):
        ...
        for item in sorted(self.pattern, key=lambda e: (int(e["x_step"]), int(e["id"]))):
            ...
            # v42 :
            # caler sans accélérer = garde la vitesse originale.
    ...
    def render_preview_file(self):
        OUT_DIR.mkdir(parents=True, exist_ok=True)
        wav = OUT_DIR / f"{self.project['safe']}_tracker_app_v42_preview.wav"
    ...
    def write_latest_pattern(self, reason="correction"):
        ...
        path = LATEST_DIR / f"{self.project['safe']}_latest_slice_index_v42.json"

        data = {
            "version": "breakbeatai_latest_slice_index_v42",
            "reason": reason,
            ...
            "warp_mode": "fit_to_case_length_no_speed_v42",
    ...
    def save(self):
        ...
        tracker_path = OUT_DIR / f"{self.project['safe']}_tracker_app_edit_v42.json"

        data = {
            "version": "tracker_app_edit_v42_slice_index",
        ...
        stamp = file_stamp()
        validated_path = VALIDATED_DIR / f"{self.project['safe']}_validated_slice_index_loop32_v42_{stamp}.json"

        validated = {
            "version": "breakbeatai_validated_slice_index_loop32_v42",
...
# ---------------------------------------------------------------------
# v42 EXTENSION : statut compact, sans casser la v39
# ---------------------------------------------------------------------

def v42_set_status(self, text):
    """
    Statut compact :
...
_old_v42_build_ui = SliceIndexTracker.build_ui


def v42_build_ui(self):
    _old_v42_build_ui(self)

    try:
        self.output_label.config(
            text="v42 : calage sans accélérer ON | 1 hit = 2 cases | 155 BPM.",
            width=130,
            height=2,
            wraplength=1180,
            anchor="w",
...
def v42_record_correction(self, event_type, before=None, after=None):
    """
    ...
    print(f"[v42] correction sauvegardée : {event_type}")
    print(f"[v42] correction path : {corrections_path}")
    print(f"[v42] latest path : {latest_path}")


def v42_randomize_pattern(self):
    self.stop_playhead()
    self.stop_audio()
    self.looping = False
...
def v42_render_preview_only(self):
    wav = self.render_preview_file()
    self.set_status(f"Preview rendue : {wav.name}")


def v42_switch_break(self, event=None):
    safe = self.break_var.get()
...
# Monkey patch final.
SliceIndexTracker.set_status = v42_set_status
SliceIndexTracker.build_ui = v42_build_ui
SliceIndexTracker.record_correction = v42_record_correction
SliceIndexTracker.randomize_pattern = v42_randomize_pattern
SliceIndexTracker.render_preview_only = v42_render_preview_only
SliceIndexTracker.switch_break = v42_switch_break
SliceIndexTracker.refresh_breaks = v42_refresh_breaks
