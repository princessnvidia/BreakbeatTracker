...
"""
BreakbeatAI Tracker Editor v109 — slice index
...
def warp_audio_to_length_external(audio, target_len, cache_key=None):
    ...
    if rubberband:
        try:
            ...
            print(f"[v109] rubberband échec, fallback ffmpeg/no-speed : {exc}")
    ...
    if ffmpeg:
        try:
            ...
            print(f"[v109] ffmpeg atempo échec, fallback no-speed : {exc}")
...
class SliceIndexTracker:
    def __init__(self, root, pair_json):
        ...
        self.root.title("BreakbeatAI Tracker Editor v109 — slice index")
    ...
    def detect_external_player(self):
        for name in ["pw-play", "paplay", "aplay", "ffplay"]:
            ...
            if path:
                print(f"[v109] Backend audio : {name} -> {path}")
                return name

        print("[v109] Aucun backend système trouvé, fallback sounddevice.")
        return None
    ...
    def build_initial_pattern(self, randomize=False):
        ...
        print(
            f"[v109] Génération pattern | break={self.project['safe']} | "
        ...
        for slot in range(max_items):
            ...
            print(f"[v109] slot {slot:02d} | step {x_step:02d} -> slice {pair:02d}")
    ...
    def build_ui(self):
        ...
        title = tk.Label(
            main,
            text="BreakbeatAI v109 — space export fixed",
            bg="#111018",
            fg="#ff7acc",
            font=("Sans", 20, "bold"),
        )
    ...
    def record_correction(self, event_type, before=None, after=None):
        ...
        print(f"[v109] correction sauvegardée : {event_type}")
    ...
    def play_audition_array(self, audio, label="audition"):
        ...
        audition_wav = OUT_DIR / f"{self.project['safe']}_v109_audition.wav"
        ...
        if self.external_player:
            try:
                ...
                print(f"[v109] Erreur audition {self.external_player} : {exc}")
    ...
    def on_close(self):
        ...
        """
        print("[v109] Fermeture : arrêt audio + destruction fenêtre")
    ...
    def play_audio_array(self, audio, label="audio"):
        ...
        audio = normalize(audio)
        live_wav = OUT_DIR / f"{self.project['safe']}_v109_live.wav"
        ...
        if self.external_player:
            try:
                ...
                print(f"[v109] Erreur backend {self.external_player} : {exc}")
    ...
    def render_preview_file(self):
        OUT_DIR.mkdir(parents=True, exist_ok=True)
        wav = OUT_DIR / f"{self.project['safe']}_tracker_app_v109_preview.wav"
    ...
    def write_latest_pattern(self, reason="correction"):
        ...
        path = LATEST_DIR / f"{self.project['safe']}_latest_slice_index_v109.json"

        data = {
            "version": "breakbeatai_latest_slice_index_v109",
            "reason": reason,
    ...
    def save(self):
        ...
        tracker_path = OUT_DIR / f"{self.project['safe']}_tracker_app_edit_v109.json"
        ...
        stamp = file_stamp()
        validated_path = VALIDATED_DIR / f"{self.project['safe']}_validated_slice_index_loop64_v109_{stamp}.json"

        validated = {
            "version": "breakbeatai_validated_slice_index_loop64_v109",
...
def v44_record_correction(self, event_type, before=None, after=None):
    ...
    print(f"[v109] correction sauvegardée : {event_type}")
    print(f"[v109] correction path : {corrections_path}")
    print(f"[v109] latest path : {latest_path}")
...
def v45_move_selected(self, dx, dy):
    ...
    if dy != 0 and before != after:
        try:
            ...
            print(f"[v109] audition après flèche haut/bas impossible : {exc}")
...
def v46_warp_loop_pitch_preserve(audio, target_len, cache_key):
    ...
    if rubberband:
        try:
            ...
            print(f"[v109] rubberband global warp échec : {exc}")
    ...
    if ffmpeg:
        try:
            ...
            print(f"[v109] ffmpeg global warp échec : {exc}")

    print("[v109] Aucun warp pitch-preserve dispo, fallback fit no-speed.")
    return v46_fit_audio_no_speed(audio, target_len)
...
def v46_build_initial_pattern(self, randomize=False):
    ...
    print(
        f"[v109] M8 pattern | break={self.project['safe']} | "
...
def v46_get_warped_loop(self):
    ...
    print(
        f"[v109] global warp OK | source_len={loop_end-loop_start} | "
...
def v46_build_ui(self):
    ...
    try:
        ...
        print(f"[v109] UI extension impossible : {exc}")
...
def v47_play_audio_array(self, audio, label="audio"):
    ...
    if len(audio) == 0 or v47_is_silent(audio):
        ...
        print(f"[v109] lecture annulée : audio silencieux ({label})")
        return 0
...
def v47_toggle_loop(self):
    ...
    if not getattr(self, "pattern", None):
        ...
        print("[v109] loop annulée : pattern vide")
        return "break"
...
def v47_delete_selected(self):
    ...
    if not getattr(self, "pattern", None):
        ...
        print("[v109] pattern vide après suppression : audio coupé")
...
def v48_build_initial_pattern(self, randomize=False):
    ...
    print("[v109] pattern initial forcé VIDE : aucun son ne doit jouer.")
    return []
...
def v48_load_first_slices(self):
    ...
    print(f"[v109] Load 16 slices : {len(self.pattern)} items")


def v48_debug_pattern(self, label="render"):
    print("")
    print(f"[v109] DEBUG PATTERN avant {label}")
    print(f"[v109] items = {len(getattr(self, 'pattern', []) or [])}")

    if not getattr(self, "pattern", None):
        print("[v109] pattern vide confirmé")
        print("")
        return
    ...
    for item in self.pattern:
        ...
        print(
            f"[v109] id={item_id} | step {x:02d}/{end:02d} | "
...
def v48_render_audio_with_timeline(self):
    ...
    if not getattr(self, "pattern", None):
        loop_samples = self.get_loop_samples()
        print(f"[v109] rendu silence total : pattern vide | samples={loop_samples}")
        return np.zeros(loop_samples, dtype=np.float32), []
    ...
    print(f"[v109] rendu audio peak={peak:.8f} | timeline_events={len(timeline) if timeline is not None else 'None'}")

    if timeline:
        print("[v109] timeline events:")
        for ev in timeline:
            print(f"[v109]   {ev}")
...
def v48_toggle_loop(self):
    if not getattr(self, "pattern", None):
        ...
        print("[v109] Space ignoré : pattern vide.")
        return "break"
...
def v48_build_ui(self):
    ...
    try:
        ...
        print(f"[v109] ajout UI impossible : {exc}")
...
def v49_add_item_at_xy(self, x, y):
    ...
    try:
        ...
        print(f"[v109] record_correction add impossible : {exc}")
    ...
    try:
        ...
        print(f"[v109] audition après ajout impossible : {exc}")

    print(f"[v109] add | id={item['id']} | step={x_step} | pair={pair} | lane={lane}")
...
def v49_duplicate_selected_forward(self, event=None):
    ...
    if item is None:
        ...
        print("[v109] duplicate annulé : aucune note sélectionnée")
        return "break"
    ...
    if new_x > max_x:
        ...
        print("[v109] duplicate annulé : fin de grille")
        return "break"
    ...
    try:
        ...
        print(f"[v109] record_correction duplicate impossible : {exc}")
    ...
    try:
        ...
        print(f"[v109] audition après duplicate impossible : {exc}")

    print(
        f"[v109] duplicate | old_id={item.get('id')} | new_id={new_item.get('id')} | "
...
def v49_build_ui(self):
    ...
    try:
        ...
        print(f"[v109] bind canvas click impossible : {exc}")
    ...
    try:
        ...
        print(f"[v109] bind keys impossible : {exc}")
...
def v50_load_style_model():
    ...
    try:
        ...
        print(f"[v109] impossible de charger le modèle IA : {exc}")
        return None
...
def v50_generate_ai_pattern(self, event=None):
    ...
    if model is None:
        ...
        print("[v109] modèle absent :", STYLE_MODEL_PATH)
        return "break"
    ...
    print("")
    print(f"[v109] GENERATE IA | safe={safe} | pairs={pair_values} | slots={max_slots}")
    ...
    for slot in range(max_slots):
        ...
        print(
            f"[v109] slot {slot:02d} | step {x_step:02d} | "
    ...
    try:
        ...
        print(f"[v109] write_latest_pattern impossible : {exc}")
...
def v50_build_ui(self):
    ...
    try:
        ...
        print(f"[v109] UI IA impossible : {exc}")
...
def v51_mouse_down(self, event):
    ...
    if item is not None:
        ...
        print(
            f"[v109] mouse down note | id={item.get('id')} | "
            f"step={item.get('x_step')} | pair={item.get('pair')}"
        )
    else:
        print("[v109] mouse down empty")
...
def v51_mouse_up(self, event):
    ...
    if state.get("empty"):
        ...
        print("[v109] drag depuis vide ignoré")
        return "break"
    ...
    if before != after:
        try:
            ...
            print(f"[v109] record_correction drag impossible : {exc}")
        ...
        print(
            f"[v109] drag move | id={after.get('id')} | "
            f"step {before.get('x_step')} -> {after.get('x_step')} | "
        ...
        try:
            ...
            print(f"[v109] audition après drag impossible : {exc}")
...
def v51_build_ui(self):
    ...
    try:
        ...
        print(f"[v109] bind souris impossible : {exc}")
    ...
    try:
        ...
        print(f"[v109] bind Suppr impossible : {exc}")
...
def v52_normalize_loaded_notes(self, notes):
    ...
    for raw in sorted(notes, key=lambda n: (int(float(n.get("x_step", 0))), int(float(n.get("pair", 0))))):
        ...
        if pair not in valid_pairs:
            print(f"[v109] slice ignorée car absente du break actuel : pair={pair}")
            continue
...
def v52_load_saved_beat(self, event=None):
    ...
    if not saves:
        ...
        print("[v109] aucune sauvegarde trouvée")
        return "break"
    ...
    if not pattern:
        ...
        print("[v109] sauvegarde incompatible :", chosen["path"])
        return "break"
    ...
    print("")
    print("[v109] BEAT RESTAURÉ")
    print("[v109] fichier :", chosen["path"])
    print("[v109] clé :", chosen["key"])
    print("[v109] notes :", len(pattern))
    print("")
...
def v52_list_saved_beats(self, event=None):
    ...
    print("")
    print("[v109] SAUVEGARDES TROUVÉES")
    for i, save in enumerate(saves[:30]):
        print(
            f"[v109] {i:02d} | notes={len(save['notes']):02d} | "
...
def v52_build_ui(self):
    ...
    try:
        ...
        print(f"[v109] UI load saved impossible : {exc}")
...
def v53_run_training(self):
    ...
    if not trainer.exists():
        ...
        print("[v109] trainer introuvable :", trainer)
        return False

    print("")
    print("[v109] AUTO-TRAIN IA après Save...")
    print("")
    ...
    try:
        ...
        if result.returncode != 0:
            ...
            print("[v109] auto-train échec code :", result.returncode)
            return False

        v53_status(self, "Save OK + IA entraînée automatiquement.")
        print("[v109] AUTO-TRAIN OK")
        return True

    except Exception as exc:
        v53_status(self, f"Save OK, mais auto-train impossible : {exc}")
        print("[v109] exception auto-train :", exc)
        return False
...
def v53_save(self):
    ...
    try:
        ...
        print(f"[v109] auto-train après save impossible : {exc}")
...
def v53_build_ui(self):
    ...
    try:
        ...
        print(f"[v109] UI auto-train impossible : {exc}")
...
def v55_mouse_up(self, event):
    ...
    if not state.get("moved"):
        ...
        try:
            ...
            print(f"[v109] audition clic note impossible : {exc}")
        ...
        print(
            f"[v109] click play/select | id={item.get('id')} | "
            f"step={item.get('x_step')} | pair={item.get('pair')}"
        )
...
def v58_add_item_at_xy(self, x, y):
    ...
    print(
        f"[v109] add | id={item['id']} | step={x_step} | "
...
def v58_toggle_selected_length(self, event=None):
    ...
    try:
        ...
        print(f"[v109] record_correction length impossible : {exc}")

    v58_refresh_after_edit(self, "toggle_note_length")

    v58_status(self, f"Longueur note : {old_len} -> {new_len} case(s).")
    print(f"[v109] length toggle | id={item.get('id')} | {old_len} -> {new_len}")
...
def v58_duplicate_selected_forward(self, event=None):
    ...
    try:
        ...
        print(f"[v109] audition duplicate impossible : {exc}")
...
def v58_build_ui(self):
    ...
    try:
        ...
        print(f"[v109] UI demi-grille impossible : {exc}")
    ...
    try:
        ...
        print(f"[v109] bind demi-grille impossible : {exc}")
...
def v59_rank_current_slices(self):
    ...
    for pair in self.pair_values:
        ...
        try:
            ...
            print(f"[v109] analyse pair {pair} impossible : {exc}")
            role = "other"
    ...
    print("")
    print("[v109] TOP ROLES CURRENT BREAK")
    for r in ("kick", "snare", "hat"):
        print(f"[v109] {r:5s}:", [(x["pair"], round(x["score"], 3)) for x in ranks[r][:6]])
    print("")
...
def v59_load_role_model():
    ...
    try:
        ...
        print(f"[v109] lecture modèle rôle impossible : {exc}")
        return None
...
def v59_generate_role_aware(self, event=None):
    ...
    if model is None:
        ...
        print("[v109] modèle rôle absent, fallback musical snare 6.")
        model = {
            "breaks": {},
            "global": {},
            "patterns_used": 0,
    ...
    print("")
    print("[v109] ROLE-AWARE GENERATION")
    print("[v109] model patterns_used:", model.get("patterns_used", 0))
    for item in pattern:
        print(
            f"[v109] step {item['x_step']:02d}/{item['x_step'] + item['length'] - 1:02d} | "
...
def v59_run_role_training(self, event=None):
    ...
    try:
        ...
        print(f"[v109] training rôle impossible : {exc}")
...
def v59_save(self):
    ...
    try:
        ...
        print(f"[v109] auto-training rôle après save impossible : {exc}")
...
def v59_build_ui(self):
    ...
    try:
        ...
        print(f"[v109] UI role-aware impossible : {exc}")
...
def v62_add_item_at_xy(self, x, y):
    ...
    print(f"[v109] add | id={item['id']} | step={x_step} | len={length} | pair={pair}")
...
def v62_mouse_down(self, event):
    ...
    if item is not None:
        ...
        print(
            f"[v109] mouse down {state['mode']} | id={item.get('id')} | "
            f"step={item.get('x_step')} | len={item.get('length')} | pair={item.get('pair')}"
        )
    else:
        print("[v109] mouse down empty")
    ...
    if mode == "resize":
        if not state.get("moved"):
            ...
            try:
                ...
                print(f"[v109] audition clic resize impossible : {exc}")
        ...
        if before != after:
            try:
                ...
                print(f"[v109] record_correction resize impossible : {exc}")
            ...
            try:
                ...
                print(f"[v109] audition après resize impossible : {exc}")
    ...
    if mode == "move":
        ...
        if before != after:
            try:
                ...
                print(f"[v109] record_correction move impossible : {exc}")
            ...
            try:
                ...
                print(f"[v109] audition après move impossible : {exc}")
...
def v62_toggle_selected_length(self, event=None):
    ...
    try:
        ...
        print(f"[v109] record_correction toggle length impossible : {exc}")

    v62_refresh_after_edit(self, "toggle_note_length_v62")
    v62_status(self, f"Longueur note : {old_len} → {new_len} case(s).")

    print(f"[v109] toggle length | id={item.get('id')} | {old_len} -> {new_len}")
...
def v62_draw(self):
    ...
    try:
        ...
        print(f"[v109] draw handles impossible : {exc}")
...
def v62_build_ui(self):
    ...
    try:
        ...
        print(f"[v109] bind resize impossible : {exc}")
    ...
    try:
        ...
        print(f"[v109] UI resize impossible : {exc}")
...
def v63_load_model():
    ...
    try:
        ...
        print(f"[v109] lecture modèle strict impossible : {exc}")
        return None
...
def v63_generate_safe_role_ai(self, event=None):
    ...
    print("")
    print("[v109] SAFE ROLE AI GENERATED")
    print("[v109] model strict:", "yes" if model else "no")
    print("[v109] snare main/alt:", main_snare, alt_snare)
    print("[v109] kick  main/alt:", main_kick, alt_kick)
    print("[v109] hat   main/alt:", main_hat, alt_hat)
    print("[v109] pattern:")
    for item in pattern:
        print(
            f"[v109] step {item['x_step']:02d}/{item['x_step'] + item['length'] - 1:02d} | "
...
def v63_run_strict_training(self, event=None):
    ...
    print("")
    print("[v109] TRAIN STRICT ROLE MODEL...")
    print("")
    ...
    try:
        ...
        print(f"[v109] training strict impossible : {exc}")
...
def v63_build_ui(self):
    ...
    try:
        ...
        print(f"[v109] UI safe impossible : {exc}")
...
def v65_rank_slices(self):
    ...
    for pair in self.pair_values:
        ...
        try:
            ...
            print(f"[v109] analyse slice {pair} impossible : {exc}")
            scores = {
                "kick": 0.0,
                "snare": 0.0,
                "hat": 0.0,
    ...
    print("")
    print("[v109] TOP SLICES PAR ROLE — break courant")
    for role in ("kick", "snare", "hat", "ghost_snare"):
        print(f"[v109] {role:12s}:", [(x["pair"], round(x["score"], 3)) for x in ranks[role][:8]])
    print("")
...
def v65_generate_from_role_template(self, event=None):
    ...
    try:
        ...
        print(f"[v109] template introuvable : {exc}")
        return "break"
    ...
    print("")
    print("[v109] CROSS-SONG ROLE TEMPLATE GENERATED")
    print("[v109] template:", V65_TEMPLATE_PATH)
    print("[v109] temperature:", temperature, "| hats:", hat_density, "| ghosts:", ghost_density)
    print("[v109] pattern:")
    for item in pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v109] step {item['x_step']:02d}/{end:02d} | "
...
def v65_build_ui(self):
    ...
    try:
        ...
        print(f"[v109] UI impossible : {exc}")
...
def v67_train_strict_if_available(self):
    ...
    if trainer is None:
        print("[v109] aucun trainer trouvé.")
        return False

    print("")
    print("[v109] TRAIN après validation :", trainer)
    print("")
    ...
    try:
        ...
        print(f"[v109] training impossible : {exc}")
        return False
...
def v67_generate_candidate(self, event=None):
    ...
    print("")
    print("[v109] CANDIDATE GENERATED")
    print("[v109] break:", self.v67_candidate_safe)
    print("[v109] notes:", len(candidate))
    print("[v109] Maintenant : corrige puis clique Good après modifs, ou Reject.")
    print("")
...
def v67_reject_candidate(self, event=None):
    ...
    print("")
    print("[v109] REJECTED")
    print("[v109] saved:", rejected_path)
    print("")
...
def v67_accept_corrected(self, event=None):
    ...
    print("")
    print("[v109] ACCEPTED GOOD AFTER EDIT")
    print("[v109] saved:", validated_path)
    print("[v109] trained:", trained)
    print("")
...
def v67_accept_without_training(self, event=None):
    ...
    print("[v109] accepted no-train:", validated_path)
...
def v67_build_ui(self):
    ...
    try:
        ...
        print(f"[v109] UI feedback impossible : {exc}")
...
def v68_generate_candidate(self, event=None):
    ...
    print("[v109] Length policy appliquée : 2 cases par défaut, 1 case rare.")
...
def v69_force_pattern_hard_2_grid(self, reason="v69_force_hard_2_grid"):
    ...
    print("")
    print("[v109] HARD 2 GRID APPLIQUÉ")
    for item in self.pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v109] step {item['x_step']:02d}/{end:02d} | "
...
def v70_duplicate_selected_forward(self, event=None):
    ...
    print("")
    print("[v109] CTRL+D DUPLICATE +2")
    print(f"[v109] old_id={item.get('id')} new_id={new_item.get('id')}")
    print(f"[v109] step {old_x}/{old_x + length - 1} -> {new_x}/{new_x + length - 1}")
    print(f"[v109] pair={new_item.get('pair')} len={length}")
    print("")
...
def v70_build_ui(self):
    ...
    try:
        ...
        print(f"[v109] bind Ctrl+D impossible : {exc}")
...
def v71_rank_slices(self):
    ...
    print("")
    print("[v109] PRE-LEARNING TOP SLICES")
    for role in ("kick", "snare", "hat", "ghost_snare"):
        print(f"[v109] {role:12s}:", [(x["pair"], round(x["score"], 3)) for x in ranks[role][:8]])
    print("")
...
def v71_prelearn_roles(self, force=False):
    ...
    print("")
    print("[v109] PRELEARN ROLES POUR", safe)
    for role in ("kick", "snare", "hat", "ghost_snare", "bad"):
        print(f"[v109] {role:12s}:", roles.get(role, []))
    print("")
...
def v71_generate_full_candidate(self, event=None):
    ...
    print("")
    print("[v109] FULL CANDIDATE GENERATED")
    print("[v109] break:", v71_safe(self))
    print("[v109] roles:", roles)
    print("[v109] pattern:")
    for item in pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
...
def v72_open_break(self, safe=None):
    ...
    print("")
    print("[v109] OPEN BREAK")
    print("[v109] current:", current)
    print("[v109] next   :", safe)
    print("[v109] cmd    :", " ".join(cmd))
    print("")
...
def v72_build_ui(self):
    ...
    try:
        ...
        print(f"[v109] UI break switcher impossible : {exc}")
...
def v73_sanitize_tracker_pattern(self, reason="v73_tracker_strict_no_overlap"):
    ...
    print("")
    print("[v109] TRACKER STRICT NO OVERLAP")
    print("[v109] kept:", len(accepted), "| rejected overlaps:", len(rejected))

    for item in accepted:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v109] KEEP step {item['x_step']:02d}/{end:02d} | "
    ...
    if rejected:
        ...
        for item in rejected:
            ...
            print(
                f"[v109] DROP step {item['x_step']:02d}/{end:02d} | "
                f"role={item.get('v73_role_key')} | pair={item.get('pair')}"
            )
...
def v81_launch_break(self, source):
    ...
    print("")
    print("[v109] CHARGER BREAK")
    print("[v109] courant :", current)
    print("[v109] nouveau :", source)
    print("[v109] cmd     :", " ".join(cmd))
    print("")
...
def v81_patch_charger_break_buttons(self):
    ...
    print(f"[v109] boutons Charger break patchés : {patched}")
...
def v81_generate_candidate(self, event=None):
    ...
    print("")
    print("[v109] GENERATE CANDIDATE")
    print("[v109] break chargé :", current)
    print("[v109] générateur   :", getattr(fn, "__name__", None))
    print("")
...
def v81_run_training(self):
    ...
    if script is None:
        print("[v109] Aucun script training trouvé.")
...
def v81_save_feedback(self, verdict, train_now=False):
    ...
    print("")
    print("[v109] FEEDBACK SAUVÉ")
    print("[v109] break  :", current)
    print("[v109] verdict:", verdict)
    print("[v109] train  :", train_now)
    print("[v109] fichier:", out_path)
    print("")
...
def v81_build_ui(self):
    ...
    try:
        self.root.title(f"BreakbeatAI v109 — space export fixed")
    except Exception:
        pass
    ...
    try:
        ...
        print(f"[v109] patch Charger break impossible : {exc}")

    try:
        v81_install_training_toolbar(self)
    except Exception as exc:
        print(f"[v109] toolbar impossible : {exc}")
...
def v82_auto_role_pools(self):
    ...
    print("")
    print("[v109] ROLE POOLS")
    for r in ["kick", "snare", "hat", "ghost_snare", "bad"]:
        print(f"[v109] {r:12s}:", roles.get(r, []))
    print("")
...
def v82_generate_candidate(self, event=None):
    ...
    print("")
    print("[v109] NEW CANDIDATE")
    print("[v109] break :", current)
    print("[v109] nonce :", nonce)
    print("[v109] rejected known:", len(rejected))
    print("[v109] fingerprint rejeté ?: ", chosen_fp in rejected)
    for item in self.pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v109] step {item['x_step']:02d}/{end:02d} "
...
def v82_patch_training_buttons(self):
    ...
    print(f"[v109] boutons training patchés : {patched}")
...
def v82_build_ui(self):
    ...
    try:
        ...
        print(f"[v109] patch boutons impossible : {exc}")
...
# ---------------------------------------------------------------------
# v106 EXTENSION PROPRE
# ---------------------------------------------------------------------
# Un seul bloc final :
# - loop 64
# - variations du premier 32 sur Reject / Bad
# - Reject / Bad change aussi les lignes kick/snare/hat si possible
# - Export WAV Current dans la barre Generate Candidate
# - Ctrl+E exporte
# - Espace relance Loop / Space
# - colonnes comme avant -3px
# ---------------------------------------------------------------------

V106_OVERRIDES_PATH = Path("dataset/learning/break_role_overrides_v01.json")
V106_LOCKS_PATH = Path("dataset/learning/main_role_locks_v87.json")
V106_STATE_PATH = Path("dataset/learning/first32_variants_v106.json")
V106_EXPORT_DIR = Path("exports")
V106_PAIR_DIR = Path("dataset/pair_blocks_v02")

V106_FIRST_32_VARIANTS = [
    {
...
V106_SNARE_STEPS = [4, 12, 20, 28, 36, 44, 52, 60]

V106_WIDTH_ATTR_NAMES = [
...
def v106_load_json(path, default):
    try:
...
def v106_save_json(path, data):
    Path(path).parent.mkdir(parents=True, exist_ok=True)
    Path(path).write_text(json.dumps(data, indent=2, ensure_ascii=False), encoding="utf-8")


def v106_pairblock_stems():
    out = {}
    for p in sorted(V106_PAIR_DIR.glob("*_pair_blocks_v02.json")):
        stem = p.name.replace("_pair_blocks_v02.json", "")
        out[v106_norm(stem)] = stem
    return out


def v106_all_widgets(widget):
    out = [widget]
    try:
        children = widget.winfo_children()
    except Exception:
        children = []
    for child in children:
        out.extend(v106_all_widgets(child))
    return out


def v106_widget_text(widget):
    try:
...
def v106_current_break(self):
    stems = v106_pairblock_stems()

    # Priorité 1 : le menu déroulant visible.
    for widget in v106_all_widgets(self.root):
        try:
    ...
    # Priorité 2 : chemins pair_blocks stockés dans l'objet.
    for value in list(vars(self).values()):
        ...
        name = p.name
        if name.endswith("_pair_blocks_v02.json"):
            return name.replace("_pair_blocks_v02.json", "")

    # Priorité 3 : attributs texte.
    for attr in [
...
def v106_force_64_columns_minus_3(self):
    for attr in [
    ...
    if not getattr(self, "_v106_columns_minus_3_done", False):
        self._v106_columns_minus_3_done = True
        for attr in V106_WIDTH_ATTR_NAMES:
            try:
                if hasattr(self, attr):
                    value = getattr(self, attr)
                    if isinstance(value, (int, float)) and value > 4:
                        setattr(self, attr, float(value) - 3.0)
                        print(f"[v109] attr width {attr}: {value} -> {getattr(self, attr)}")
            except Exception:
                pass
...
def v106_find_record(data, current):
    br = data.get("breaks", {}).get(current)
    if br is not None:
        return br
    for name, obj in data.get("breaks", {}).items():
        if v106_norm(name) == v106_norm(current):
            return obj
    return None


def v106_load_roles(self):
    current = v106_current_break(self)
    roles = {"kick": [], "snare": [], "hat": [], "ghost_snare": [], "bad": []}

    data = v106_load_json(V106_OVERRIDES_PATH, {"breaks": {}})
    br = v106_find_record(data, current)
...
def v106_load_locks(self):
    current = v106_current_break(self)
    data = v106_load_json(V106_LOCKS_PATH, {"breaks": {}})
    br = v106_find_record(data, current)
...
def v106_choose_main_pairs(self, roles, locks, role_offset=0):
    chosen = {}
    used = set()

    for role in ["snare", "kick", "hat"]:
        candidates = v106_candidates(self, roles, locks, role)
        if not candidates:
...
def v106_load_state():
    data = v106_load_json(
        V106_STATE_PATH,
        {"version": "first32_variants_v106", "updated_at": None, "breaks": {}},
    )
    data.setdefault("breaks", {})
    return data


def v106_save_state(data):
    data["updated_at"] = time.strftime("%Y-%m-%d %H:%M:%S")
    v106_save_json(V106_STATE_PATH, data)


def v106_next_first32_index(self, reject_mode=False):
    current = v106_current_break(self)
    data = v106_load_state()
    ...
    idx = (last + 1) % len(V106_FIRST_32_VARIANTS)

    br["first32_index"] = idx
    br["last_variant"] = V106_FIRST_32_VARIANTS[idx]["id"]
    br["updated_at"] = time.strftime("%Y-%m-%d %H:%M:%S")
    v106_save_state(data)

    role_offset = int(br.get("reject_count", 0))
    return idx, role_offset


def v106_make_second32_from_first32(first_grid, response_id):
    second = [(step + 32, role) for step, role in first_grid]
...
def v106_alt_pair_for_second32(self, roles, chosen, role, occurrence, role_offset):
...
def v106_make_note(self, step, pair, role, source, first32_id, response_id, role_offset):
    step = int(step)
    if step % 2 != 0:
        step -= 1
    step = max(0, min(62, step))
    ...
    return {
        ...
        "hit_slot": int(step) // 2,
        "ai_generated": True,
        "ai_model": "v106_clean_space_export",
        "learned_role": str(role),
        "prior_label": str(role),
        "main_role_source": str(source),
        "v106_loop64": True,
        "v106_first32_variant": str(first32_id),
        "v106_second32_response": str(response_id),
        "v106_role_offset": int(role_offset),
    }


def v106_validate_pattern(pattern):
    if len(pattern) != 32:
        raise RuntimeError(f"BUG v106 : pattern devrait avoir 32 notes, obtenu {len(pattern)}")
    ...
    for item in pattern:
        ...
        if x % 2 != 0:
            raise RuntimeError(f"BUG v106 : départ impair {x}")

        if x < 0 or x > 62:
            raise RuntimeError(f"BUG v106 : step hors 64 : {x}")

        if x in occupied or (x + 1) in occupied:
            raise RuntimeError(f"BUG v106 : collision à {x}/{x+1}")

        occupied.add(x)
        occupied.add(x + 1)

        if role == "snare" and x not in V106_SNARE_STEPS:
            raise RuntimeError(f"BUG v106 : snare à {x}, interdit")


def v106_generate(self, event=None, reject_mode=False):
    v106_force_64_columns_minus_3(self)

    current = v106_current_break(self)
    roles = v106_load_roles(self)
    locks = v106_load_locks(self)

    first_idx, role_offset = v106_next_first32_index(self, reject_mode=reject_mode)
    first_variant = V106_FIRST_32_VARIANTS[first_idx]
    first32_id = first_variant["id"]
    first_grid = first_variant["grid"]

    response_id = V106_SECOND_32_RESPONSES[first_idx % len(V106_SECOND_32_RESPONSES)]
    second_grid = v106_make_second32_from_first32(first_grid, response_id)

    chosen = v106_choose_main_pairs(self, roles, locks, role_offset=role_offset)

    self.v106_first32_index = int(first_idx)
    self.v106_current_first32_id = str(first32_id)
    self.v106_current_response_id = str(response_id)
    self.v106_role_offset = int(role_offset)

    full_grid = list(first_grid) + list(second_grid)

    print("")
    if reject_mode:
        print("[v109] REJECT/BAD → variation premier 32 + autres lignes")
    else:
        print("[v109] GENERATE 64")
    print("[v109] break:", current)
    print("[v109] first32:", first32_id)
    print("[v109] response:", response_id)
    print("[v109] role_offset:", role_offset)
    for role in ["kick", "snare", "hat"]:
        lane = "?"
        try:
            ...
            pass
        print(f"[v109] {role:6s}: pair={chosen[role]['pair']} lane={lane} source={chosen[role]['source']}")
    print("")
    ...
    for step, role in full_grid:
        if step < 32:
            ...
            pair, source = v106_alt_pair_for_second32(self, roles, chosen, role, occ, role_offset)

        pattern.append(v106_make_note(self, step, pair, role, source, first32_id, response_id, role_offset))

    v106_validate_pattern(pattern)
    ...
    try:
        ...
        print(f"[v109] draw impossible : {exc}")
    ...
    try:
        ...
        print(f"[v109] refresh_panel impossible : {exc}")

    try:
        self.write_latest_pattern(reason="v106_clean_space_export")
    except Exception as exc:
        print(f"[v109] write_latest_pattern impossible : {exc}")

    v106_status(self, f"v106 : {first32_id}. Espace = Loop / Space. Export en haut.")
    return "break"


def v106_reject_bad(self, event=None):
    return v106_generate(self, event=event, reject_mode=True)


def v106_read_wav(path):
...
def v106_read_audio(path):
    path = Path(path).resolve()
    if path in _V106_AUDIO_CACHE:
        return _V106_AUDIO_CACHE[path]

    loaders = []
    try:
        import soundfile  # noqa
        loaders.append(v106_read_soundfile)
    except Exception:
        pass

    loaders.append(v106_read_wav)
    loaders.append(v106_read_ffmpeg)

    last = None
    for loader in loaders:
        try:
            result = loader(path)
            _V106_AUDIO_CACHE[path] = result
            return result
        except Exception as exc:
            last = exc
...
def v106_find_pairblock_file(current):
    exact = V106_PAIR_DIR / f"{current}_pair_blocks_v02.json"
    if exact.exists():
        return exact

    n_current = v106_norm(current)
    for p in sorted(V106_PAIR_DIR.glob("*_pair_blocks_v02.json")):
        stem = p.name.replace("_pair_blocks_v02.json", "")
        if v106_norm(stem) == n_current:
            return p

    for p in sorted(V106_PAIR_DIR.glob("*_pair_blocks_v02.json")):
        stem = p.name.replace("_pair_blocks_v02.json", "")
        if n_current in v106_norm(stem) or v106_norm(stem) in n_current:
            return p

    return None


def v106_extract_blocks(data):
    if isinstance(data, list):
        return data
    if not isinstance(data, dict):
        return []
...
def v106_find_existing_path(value):
    if not value:
        return None
...
def v106_resolve_audio_path(block, root):
    for obj in [block, root]:
        if not isinstance(obj, dict):
            continue
        for key in ["source_audio", "audio_path", "source_path", "file", "path", "filename", "wav", "audio"]:
            p = v106_find_existing_path(obj.get(key))
            if p:
                return p
    return None


def v106_num(obj, keys, default=None):
    if not isinstance(obj, dict):
        return default
    for key in keys:
        if key not in obj:
...
def v106_pair_id(block, fallback):
    try:
        return int(v106_num(block, ["pair", "slice", "slice_index", "index", "id"], fallback))
    except Exception:
        return int(fallback)


def v106_build_slice_map(current):
    pairblock_file = v106_find_pairblock_file(current)
    if pairblock_file is None:
        raise RuntimeError(f"pair_blocks introuvable pour {current}")

    root = v106_load_json(pairblock_file, None)
    blocks = v106_extract_blocks(root)
    if not blocks:
    ...
    for i, block in enumerate(blocks):
        ...
        pair = v106_pair_id(block, i)
        audio_path = v106_resolve_audio_path(block, root)

        if audio_path is None:
            continue

        source_audio_path = audio_path
        y, sr = v106_read_audio(audio_path)

        start = v106_num(block, ["source_start_sample", "start_sample", "start", "start_frame"], None)
        end = v106_num(block, ["source_end_sample", "end_sample", "end", "end_frame"], None)

        if start is None:
            start_ms = v106_num(block, ["source_start_ms", "start_ms"], None)
            if start_ms is not None:
                start = int(start_ms * sr / 1000.0)

        if end is None:
            end_ms = v106_num(block, ["source_end_ms", "end_ms"], None)
        ...
        if end is None:
            dur_ms = v106_num(block, ["duration_ms", "dur_ms"], 250.0)
...
def v106_infer_case_seconds(self, source_audio_path):
    ...
    try:
        if source_audio_path:
            y, sr = v106_read_audio(source_audio_path)
            dur = len(y) / float(sr)
            if dur > 0.2:
                return dur / 64.0, "source_duration_div_64"
    except Exception:
        pass

    return 60.0 / 155.0 / 4.0, "default_155bpm"


def v106_render_pattern_to_audio(self):
    current = v106_current_break(self)
    ...
    slice_map, source_audio_path, pairblock_file = v106_build_slice_map(current)
    case_seconds, timing_source = v106_infer_case_seconds(self, source_audio_path)
    ...
    for item in pattern:
        ...
        if pair not in slice_map:
            skipped += 1
            print(f"[v109] skip pair introuvable : {pair}")
            continue
    ...
    info = {
        ...
        "exporter": "v106_clean_space_export",
    }

    return out.astype(np.float32), sr, info


def v106_write_wav(path, audio, sr):
    path = Path(path)
    path.parent.mkdir(parents=True, exist_ok=True)
...
def v106_export_wav_current(self, event=None):
    try:
        current = v106_current_break(self)
        stamp = time.strftime("%Y%m%d_%H%M%S")
        variant = getattr(self, "v106_current_first32_id", None) or "pattern"

        audio, sr, info = v106_render_pattern_to_audio(self)

        out_name = f"{v106_slug(current)}_{v106_slug(variant)}_v106_{stamp}.wav"
        out_path = V106_EXPORT_DIR / out_name

        v106_write_wav(out_path, audio, sr)

        info_path = out_path.with_suffix(".json")
        info_path.write_text(json.dumps(info, indent=2, ensure_ascii=False), encoding="utf-8")

        print("")
        print("[v109] EXPORT WAV OK")
        print("[v109] break:", current)
        print("[v109] wav  :", out_path)
        print("[v109] json :", info_path)
        print("[v109] duration:", round(info["duration_seconds"], 3), "s")
        print("[v109] rendered:", info["rendered_notes"], "skipped:", info["skipped_notes"])
        print("")

        v106_status(self, f"Export WAV OK : {out_path}")

    except Exception as exc:
        print("")
        print("[v109] EXPORT WAV ERREUR")
        print("[v109]", exc)
        print("")
        v106_status(self, f"Export WAV erreur : {exc}")

    return "break"


def v106_find_button(self, label):
    target = label.strip().lower()
    for widget in v106_all_widgets(self.root):
        if v106_widget_text(widget).lower() == target:
            return widget
    return None


def v106_space_play(self, event=None):
    # Ne laisse pas l'espace déclencher deux actions Tk.
    if getattr(self, "_v106_space_busy", False):
        return "break"

    self._v106_space_busy = True
    try:
        btn = v106_find_button(self, "Loop / Space")
        if btn is not None:
            print("[v109] SPACE -> Loop / Space button")
            try:
                btn.invoke()
                return "break"
            except Exception as exc:
                print("[v109] invoke Loop / Space impossible:", exc)
        ...
        for name in [
            ...
            if callable(fn):
                print(f"[v109] SPACE -> {name}()")
                try:
                    return fn(event)
                except TypeError:
                    return fn()

        print("[v109] SPACE : aucune fonction Loop / Space trouvée.")
        v106_status(self, "Espace : bouton Loop / Space introuvable.")
        return "break"
    finally:
        try:
            self.root.after(80, lambda: setattr(self, "_v106_space_busy", False))
        except Exception:
            self._v106_space_busy = False


def v106_destroy_export_buttons(self):
    destroyed = 0
    for widget in list(v106_all_widgets(self.root)):
        text = v106_widget_text(widget).lower()
        if "export wav" in text:
            try:
                ...
                pass
    if destroyed:
        print(f"[v109] anciens boutons Export WAV supprimés : {destroyed}")


def v106_find_generate_bar(self):
...
def v106_add_export_button(self):
    if getattr(self, "_v106_export_button_added", False):
        return

    v106_destroy_export_buttons(self)

    parent, widgets = v106_find_generate_bar(self)
    if parent is None:
        parent = self.root
        widgets = {}
    ...
    btn = tk.Button(
        ...
        command=self.export_wav_current_v106,
        bg="#2d7a4b",
        fg="#ffffff",
    ...
    self.export_wav_current_button_v106 = btn
    self._v106_export_button_added = True
    print("[v109] Export WAV Current ajouté dans la barre Generate.")


def v106_patch_buttons(self):
    gen_patched = 0
    bad_patched = 0
    ...
    for widget in v106_all_widgets(self.root):
        ...
        try:
            if text == "generate candidate":
                ...
                widget.configure(command=self.reject_bad_v106)
                bad_patched += 1
        except Exception:
            pass

    print(f"[v109] boutons patchés : Generate={gen_patched}, Reject/Bad={bad_patched}")


_old_v106_draw = getattr(SliceIndexTracker, "draw", None)


def v106_draw(self, *args, **kwargs):
    v106_force_64_columns_minus_3(self)

    if callable(_old_v106_draw):
        result = _old_v106_draw(self, *args, **kwargs)
    else:
        result = None
...
_old_v106_build_ui = SliceIndexTracker.build_ui


def v106_build_ui(self):
    _old_v106_build_ui(self)

    v106_force_64_columns_minus_3(self)

    try:
        self.root.title(f"BreakbeatAI v106 clean — {v106_current_break(self)}")
    except Exception:
        pass

    try:
        v106_patch_buttons(self)
    except Exception as exc:
        print(f"[v109] patch boutons impossible : {exc}")

    try:
        v106_add_export_button(self)
    except Exception as exc:
        print(f"[v109] ajout Export WAV impossible : {exc}")

    try:
        self.root.bind_all("<space>", self.space_play_v106)
        self.root.bind_all("<Key-space>", self.space_play_v106)
        self.root.bind("<Control-e>", self.export_wav_current_v106)
        self.root.bind("<F12>", self.generate_candidate)
        self.root.bind("<F9>", self.reject_bad_v106)
    except Exception as exc:
        print(f"[v109] bind touches impossible : {exc}")

    v106_status(self, "v106 clean : Espace = Loop / Space, Export en haut, F12/F9.")


SliceIndexTracker.generate_candidate = v106_generate
SliceIndexTracker.generate_full_candidate = v106_generate
SliceIndexTracker.generate_ai_pattern = v106_generate
SliceIndexTracker.generate_cross_song_template = v106_generate
SliceIndexTracker.generate_safe_experiment = v106_generate
SliceIndexTracker.generate_locked_roles = v106_generate
SliceIndexTracker.generate_role_aware = v106_generate
SliceIndexTracker.generate_safe_role_ai = v106_generate

SliceIndexTracker.reject_bad_v106 = v106_reject_bad
SliceIndexTracker.reject_bad = v106_reject_bad
SliceIndexTracker.bad_candidate = v106_reject_bad
SliceIndexTracker.reject_candidate = v106_reject_bad
SliceIndexTracker.reject_pattern = v106_reject_bad

SliceIndexTracker.export_wav_current_v106 = v106_export_wav_current
SliceIndexTracker.space_play_v106 = v106_space_play

if callable(_old_v106_draw):
    SliceIndexTracker.draw = v106_draw

SliceIndexTracker.build_ui = v106_build_ui



# ---------------------------------------------------------------------
# v107 EXTENSION : bouton Export WAV dans la barre du haut, sans superposition
# ---------------------------------------------------------------------
# Garde v106 :
# - Espace = Loop / Space
# - F12 = Generate Candidate
# - F9 = Reject / Bad
# - Ctrl+E = Export WAV
#
# Corrige :
# - Export WAV est visible dans / près de la barre Generate Candidate.
# ---------------------------------------------------------------------


def v107_status(self, text):
    try:
        if hasattr(self, "set_status"):
            self.set_status(text)
        else:
...
def v107_export(self, event=None):
    for name in [
        "export_wav_current_v106",
        "export_wav_current_v105",
        "export_wav_visible_top_v104",
        "export_wav_v103",
        "export_wav_v101",
    ]:
        try:
    ...
    print("[v109] ERREUR : aucune fonction export WAV trouvée.")
    v107_status(self, "Export WAV introuvable.")
    return "break"


def v107_all_widgets(widget):
    out = [widget]
    ...
    for child in children:
        out.extend(v107_all_widgets(child))

    return out


def v107_widget_text(widget):
    try:
        return str(widget.cget("text")).strip()
    except Exception:
        return ""
...
def v107_destroy_export_buttons(self):
    destroyed = 0

    for widget in list(v107_all_widgets(self.root)):
        text = v107_widget_text(widget).lower()
...
def v107_find_top_toolbar(self):
    """
    Trouve la barre du haut en se basant sur les boutons visibles :
    Generate Candidate / Good après modifs / Good sans train / Reject Bad.
    On préfère le parent le plus haut dans la fenêtre.
    """
    wanted = {
        "generate candidate",
        "good après modifs",
    ...
    for widget in v107_all_widgets(self.root):
        text = v107_widget_text(widget).lower()
        ...
        try:
            groups[parent]["root_y"] = min(groups[parent]["root_y"], int(parent.winfo_rooty()))
            groups[parent]["root_x"] = min(groups[parent]["root_x"], int(parent.winfo_rootx()))
        except Exception:
            pass
    ...
    for parent, info in groups.items():
        # On veut surtout la barre la plus haute.
        # Puis celle qui contient le plus de boutons.
        score = (info["root_y"], -info["count"])
...
def v107_make_export_button(parent):
    return tk.Button(
        parent,
        text="Export WAV",
        command=None,
...
def v107_place_button_same_bar(self, parent, widgets):
    reject_widget = widgets.get("reject / bad")

    btn = v107_make_export_button(parent)
    btn.configure(command=self.export_wav_toolbar_v107)
    ...
    # Cas pack : normalement la barre du haut utilise ça.
    if manager == "pack" and reject_widget is not None:
        try:
            btn.pack(side="left", padx=4, pady=3, after=reject_widget)
            placed = True
        except Exception as exc:
            print("[v109] pack after reject impossible :", exc)
    ...
    if not placed and manager == "grid" and reject_widget is not None:
        try:
            ...
            print("[v109] grid after reject impossible :", exc)

    # Cas fallback dans le même parent.
    if not placed:
        try:
            btn.pack(side="left", padx=4, pady=3)
            placed = True
    ...
    self.export_wav_toolbar_button_v107 = btn
    return btn


def v107_button_is_visible_enough(self, btn):
    try:
        self.root.update_idletasks()
        rx = int(self.root.winfo_rootx())
        rw = int(self.root.winfo_width())
        bx = int(btn.winfo_rootx())
        bw = int(btn.winfo_width())

        return bx >= rx and (bx + bw) <= (rx + rw)
    except Exception:
        return True


def v107_place_button_left_of_toolbar(self, toolbar_parent):
    """
    Si la barre est déjà collée au bord droit, ajouter après Reject peut sortir écran.
    Dans ce cas on met un mini bouton à gauche de la barre, même hauteur.
    """
    btn = tk.Button(
        self.root,
        text="Export WAV",
        command=self.export_wav_toolbar_v107,
        bg="#2d7a4b",
        fg="#ffffff",
        activebackground="#39a96b",
        activeforeground="#ffffff",
    ...
    try:
        ...
        parent_x = int(toolbar_parent.winfo_rootx()) - root_x
        parent_y = int(toolbar_parent.winfo_rooty()) - root_y

        # Juste à gauche de la barre, avec marge.
        x = max(8, parent_x - 96)
        y = max(4, parent_y)
        ...
        self.export_wav_toolbar_button_v107 = btn
        print("[v109] bouton Export WAV placé à gauche de la barre du haut.")
        return btn

    except Exception as exc:
        print("[v109] fallback place impossible :", exc)
        try:
            btn.pack(side="top", anchor="e", padx=8, pady=2)
            return btn
        except Exception:
            pass
...
def v107_add_export_toolbar_button_now(self):
    if getattr(self, "_v107_export_done", False):
        try:
            btn = getattr(self, "export_wav_toolbar_button_v107", None)
            if btn is not None:
                btn.lift()
        except Exception:
            pass
    ...
    v107_destroy_export_buttons(self)

    toolbar_parent, widgets = v107_find_top_toolbar(self)

    if toolbar_parent is None:
        print("[v109] barre Generate introuvable pour l'instant.")
        return False

    btn = v107_place_button_same_bar(self, toolbar_parent, widgets)
    ...
    if btn is not None:
        ...
        if not v107_button_is_visible_enough(self, btn):
            print("[v109] bouton dans la barre hors écran, fallback à gauche.")
            try:
                btn.destroy()
            except Exception:
                pass
            btn = v107_place_button_left_of_toolbar(self, toolbar_parent)

    if btn is None:
        btn = v107_place_button_left_of_toolbar(self, toolbar_parent)

    if btn is not None:
        self._v107_export_done = True
        print("[v109] bouton Export WAV OK.")
        return True

    return False


def v107_add_export_toolbar_button_delayed(self):
    def attempt(count=0):
        try:
            ok = v107_add_export_toolbar_button_now(self)
        except Exception as exc:
            print("[v109] tentative ajout export impossible :", exc)
            ok = False

        if not ok and count < 20:
            try:
                self.root.after(250, lambda: attempt(count + 1))
            except Exception:
                pass

    try:
        self.root.after(250, lambda: attempt(0))
    except Exception:
        attempt(0)


_old_v107_build_ui = SliceIndexTracker.build_ui


def v107_build_ui(self):
    _old_v107_build_ui(self)

    try:
        self.root.title("BreakbeatAI v107 clean — export toolbar fixed")
    except Exception:
        pass

    try:
        v107_add_export_toolbar_button_delayed(self)
    except Exception as exc:
        print("[v109] delayed export impossible :", exc)

    try:
        self.root.bind("<Control-e>", self.export_wav_toolbar_v107)
    except Exception:
        pass

    v107_status(self, "v107 : Export WAV ajouté dans la barre du haut. Ctrl+E aussi.")


SliceIndexTracker.export_wav_toolbar_v107 = v107_export
SliceIndexTracker.build_ui = v107_build_ui



# ---------------------------------------------------------------------
# v108 EXTENSION : fix définitif barre espace
# ---------------------------------------------------------------------
# Le problème venait du focus Tk :
# si un bouton a le focus, espace active le bouton au lieu de lancer le son.
#
# v108 :
# - bind l'espace sur root
# - bind l'espace sur tous les widgets
# - intercepte KeyPress-space avant les bindings Button
# - appelle directement le bouton Loop / Space
# ---------------------------------------------------------------------


def v108_status(self, text):
    try:
        if hasattr(self, "set_status"):
            self.set_status(text)
        else:
...
def v108_all_widgets(widget):
    ...
    for child in children:
        out.extend(v108_all_widgets(child))

    return out


def v108_widget_text(widget):
    try:
        return str(widget.cget("text")).strip()
    except Exception:
        return ""
...
def v108_find_loop_space_button(self):
    for widget in v108_all_widgets(self.root):
        if v108_widget_text(widget).lower() == "loop / space":
            return widget

    return None


def v108_space_play(self, event=None):
    """
    Espace = Loop / Space, peu importe le focus.
    Retourne toujours 'break' pour empêcher Tk d'activer un bouton.
    """
    if getattr(self, "_v108_space_busy", False):
        return "break"

    self._v108_space_busy = True

    try:
        btn = v108_find_loop_space_button(self)

        if btn is not None:
            print("[v109] SPACE -> Loop / Space")
            try:
                btn.invoke()
            except Exception as exc:
                print("[v109] invoke Loop / Space impossible :", exc)
        ...
        print("[v109] SPACE : aucune action audio trouvée.")
        v108_status(self, "Espace : action Loop / Space introuvable.")
        return "break"

    finally:
        try:
            self.root.after(120, lambda: setattr(self, "_v108_space_busy", False))
        except Exception:
            self._v108_space_busy = False


def v108_bind_space_everywhere(self):
    """
    On bind directement sur chaque widget.
    C'est plus fort que bind_all quand un bouton a le focus.
    """
    widgets = v108_all_widgets(self.root)
    count = 0

    for widget in widgets:
        try:
            widget.bind("<KeyPress-space>", self.space_play_v108, add="+")
            widget.bind("<space>", self.space_play_v108, add="+")
            count += 1
        except Exception:
            pass

    try:
        self.root.bind("<KeyPress-space>", self.space_play_v108, add="+")
        self.root.bind("<space>", self.space_play_v108, add="+")
        self.root.bind_all("<KeyPress-space>", self.space_play_v108, add="+")
        self.root.bind_all("<space>", self.space_play_v108, add="+")
    except Exception:
        pass

    print(f"[v109] espace bindé sur {count} widgets.")


_old_v108_build_ui = SliceIndexTracker.build_ui


def v108_build_ui(self):
    _old_v108_build_ui(self)

    try:
        self.root.title("BreakbeatAI v108 clean — space fixed")
    except Exception:
        pass

    try:
        v108_bind_space_everywhere(self)
    except Exception as exc:
        print("[v109] bind espace impossible :", exc)

    # Deuxième passe après que tous les boutons dynamiques soient créés.
    def late_bind():
        try:
            v108_bind_space_everywhere(self)
        except Exception as exc:
            print("[v109] late bind espace impossible :", exc)

    try:
        self.root.after(800, late_bind)
    except Exception:
        pass

    v108_status(self, "v108 : Espace réparé = Loop / Space.")


SliceIndexTracker.space_play_v108 = v108_space_play
SliceIndexTracker.build_ui = v108_build_ui



# ---------------------------------------------------------------------
# v109 EXTENSION : Espace + Export WAV ensemble
# ---------------------------------------------------------------------
# Cette version répare le conflit :
# - v108 avait réparé Espace
# - mais le bouton Export WAV avait disparu
#
# v109 force les deux :
# - Espace = bouton Loop / Space
# - Export WAV visible dans la barre du haut
# - Ctrl+E = Export WAV
# ---------------------------------------------------------------------

V109_EXPORT_DIR = Path("exports")
V109_PAIR_DIR = Path("dataset/pair_blocks_v02")

_V109_AUDIO_CACHE = {}


def v109_status(self, text):
    try:
        if hasattr(self, "set_status"):
            self.set_status(text)
        else:
...
def v109_all_widgets(widget):
    ...
    for child in children:
        out.extend(v109_all_widgets(child))

    return out


def v109_widget_text(widget):
    try:
        return str(widget.cget("text")).strip()
    except Exception:
        return ""
...
def v109_pairblock_stems():
    out = {}

    for p in sorted(V109_PAIR_DIR.glob("*_pair_blocks_v02.json")):
        stem = p.name.replace("_pair_blocks_v02.json", "")
        out[v109_norm(stem)] = stem

    return out


def v109_current_break(self):
    stems = v109_pairblock_stems()

    # Le menu déroulant visible est la source la plus fiable.
    for widget in v109_all_widgets(self.root):
        try:
            value = str(widget.get()).strip()
        except Exception:
            continue
...
def v109_find_loop_space_button(self):
    for widget in v109_all_widgets(self.root):
        if v109_widget_text(widget).lower() == "loop / space":
            return widget

    return None


def v109_space_play(self, event=None):
    if getattr(self, "_v109_space_busy", False):
        return "break"

    self._v109_space_busy = True

    try:
        btn = v109_find_loop_space_button(self)
        ...
        print("[v109] SPACE : aucune action audio trouvée.")
        v109_status(self, "Espace : action Loop / Space introuvable.")
        return "break"

    finally:
        try:
            self.root.after(120, lambda: setattr(self, "_v109_space_busy", False))
        except Exception:
            self._v109_space_busy = False


def v109_bind_space_everywhere(self):
    count = 0

    for widget in v109_all_widgets(self.root):
        try:
            widget.bind("<KeyPress-space>", self.space_play_v109, add="+")
            widget.bind("<space>", self.space_play_v109, add="+")
            count += 1
        except Exception:
            pass

    try:
        self.root.bind("<KeyPress-space>", self.space_play_v109, add="+")
        self.root.bind("<space>", self.space_play_v109, add="+")
        self.root.bind_all("<KeyPress-space>", self.space_play_v109, add="+")
        self.root.bind_all("<space>", self.space_play_v109, add="+")
    except Exception:
        pass
...
def v109_load_json(path, default):
    try:
        return json.loads(Path(path).read_text(encoding="utf-8"))
    except Exception:
        return default
...
def v109_find_pairblock_file(current):
    exact = V109_PAIR_DIR / f"{current}_pair_blocks_v02.json"

    if exact.exists():
        return exact

    n_current = v109_norm(current)

    for p in sorted(V109_PAIR_DIR.glob("*_pair_blocks_v02.json")):
        stem = p.name.replace("_pair_blocks_v02.json", "")
        if v109_norm(stem) == n_current:
            return p

    for p in sorted(V109_PAIR_DIR.glob("*_pair_blocks_v02.json")):
        stem = p.name.replace("_pair_blocks_v02.json", "")
        if n_current in v109_norm(stem) or v109_norm(stem) in n_current:
            return p

    return None


def v109_extract_blocks(data):
    if isinstance(data, list):
        return data
...
def v109_find_existing_path(value):
    if not value:
        return None
...
def v109_resolve_audio_path(block, root):
    for obj in [block, root]:
        if not isinstance(obj, dict):
            continue

        for key in ["source_audio", "audio_path", "source_path", "file", "path", "filename", "wav", "audio"]:
            p = v109_find_existing_path(obj.get(key))
            if p:
                return p
...
def v109_num(obj, keys, default=None):
    if not isinstance(obj, dict):
        return default
...
def v109_pair_id(block, fallback):
    try:
        return int(v109_num(block, ["pair", "slice", "slice_index", "index", "id"], fallback))
    except Exception:
        return int(fallback)


def v109_read_wav(path):
    with wave.open(str(path), "rb") as wf:
        sr = wf.getframerate()
        ch = wf.getnchannels()
        sw = wf.getsampwidth()
...
def v109_read_audio(path):
    path = Path(path).resolve()

    if path in _V109_AUDIO_CACHE:
        return _V109_AUDIO_CACHE[path]

    loaders = []

    try:
        import soundfile  # noqa
        loaders.append(v109_read_soundfile)
    except Exception:
        pass

    loaders.append(v109_read_wav)
    loaders.append(v109_read_ffmpeg)
...
def v109_build_slice_map(current):
    pairblock_file = v109_find_pairblock_file(current)

    if pairblock_file is None:
        raise RuntimeError(f"pair_blocks introuvable pour {current}")

    root = v109_load_json(pairblock_file, None)
    blocks = v109_extract_blocks(root)
    ...
    for i, block in enumerate(blocks):
        ...
        pair = v109_pair_id(block, i)
        audio_path = v109_resolve_audio_path(block, root)

        if audio_path is None:
            continue

        source_audio_path = audio_path
        y, sr = v109_read_audio(audio_path)

        start = v109_num(block, ["source_start_sample", "start_sample", "start", "start_frame"], None)
        end = v109_num(block, ["source_end_sample", "end_sample", "end", "end_frame"], None)

        if start is None:
            start_ms = v109_num(block, ["source_start_ms", "start_ms"], None)
            if start_ms is not None:
                start = int(start_ms * sr / 1000.0)

        if end is None:
            end_ms = v109_num(block, ["source_end_ms", "end_ms"], None)
            if end_ms is not None:
                end = int(end_ms * sr / 1000.0)
        ...
        if end is None:
            dur_ms = v109_num(block, ["duration_ms", "dur_ms"], 250.0)
            end = int(start) + int(dur_ms * sr / 1000.0)
...
def v109_infer_case_seconds(self, source_audio_path):
    for attr in ["step_ms", "step_ms_var"]:
        try:
            value = getattr(self, attr)
            if hasattr(value, "get"):
    ...
    try:
        if source_audio_path:
            y, sr = v109_read_audio(source_audio_path)
            dur = len(y) / float(sr)
            if dur > 0.2:
                return dur / 64.0, "source_duration_div_64"
    except Exception:
...
def v109_render_pattern_to_audio(self):
    current = v109_current_break(self)
    pattern = list(getattr(self, "pattern", []) or [])

    if not pattern:
        raise RuntimeError("pattern vide : clique Generate Candidate avant Export WAV")

    slice_map, source_audio_path, pairblock_file = v109_build_slice_map(current)
    case_seconds, timing_source = v109_infer_case_seconds(self, source_audio_path)
    ...
    info = {
        ...
        "skipped_notes": skipped,
        "duration_seconds": len(out) / float(sr),
        "exporter": "v109_space_export_fixed",
    }

    return out.astype(np.float32), sr, info


def v109_write_wav(path, audio, sr):
    path = Path(path)
    path.parent.mkdir(parents=True, exist_ok=True)
...
def v109_export_wav_current(self, event=None):
    try:
        current = v109_current_break(self)
        stamp = time.strftime("%Y%m%d_%H%M%S")

        variant = (
            getattr(self, "v106_current_first32_id", None)
            or getattr(self, "v102_current_first32_id", None)
            or "pattern"
        )

        audio, sr, info = v109_render_pattern_to_audio(self)

        out_name = f"{v109_slug(current)}_{v109_slug(variant)}_v109_{stamp}.wav"
        out_path = V109_EXPORT_DIR / out_name

        v109_write_wav(out_path, audio, sr)
        ...
        v109_status(self, f"Export WAV OK : {out_path}")
        ...
        print("[v109]", exc)
        print("")
        v109_status(self, f"Export WAV erreur : {exc}")
...
def v109_destroy_export_buttons(self):
    destroyed = 0

    for widget in list(v109_all_widgets(self.root)):
        text = v109_widget_text(widget).lower()
...
def v109_find_top_toolbar(self):
    wanted = {
        "generate candidate",
        "good après modifs",
        "good sans train",
    ...
    for widget in v109_all_widgets(self.root):
        ...
        groups.setdefault(parent, {"widgets": {}, "count": 0, "root_y": 999999})
        groups[parent]["widgets"][text] = widget
        groups[parent]["count"] = len(groups[parent]["widgets"])
...
def v109_add_export_button(self):
    if getattr(self, "_v109_export_button_added", False):
        try:
            btn = getattr(self, "export_wav_button_v109", None)
            if btn is not None:
                btn.lift()
        except Exception:
            pass
    ...
    v109_destroy_export_buttons(self)

    toolbar_parent, widgets = v109_find_top_toolbar(self)

    if toolbar_parent is None:
        print("[v109] barre top introuvable, fallback root.")
        toolbar_parent = self.root
        widgets = {}
    ...
    btn = tk.Button(
        toolbar_parent,
        text="Export WAV",
        command=self.export_wav_current_v109,
        bg="#2d7a4b",
        fg="#ffffff",
        activebackground="#39a96b",
        activeforeground="#ffffff",
    ...
    self.export_wav_button_v109 = btn
    self._v109_export_button_added = True
    print("[v109] bouton Export WAV ajouté.")


def v109_patch_generate_reject_buttons(self):
    gen_patched = 0
    bad_patched = 0
    ...
    for widget in v109_all_widgets(self.root):
        ...
        try:
            if text == "generate candidate" and hasattr(self, "generate_candidate"):
                widget.configure(command=self.generate_candidate)
                gen_patched += 1
            elif text in ["reject / bad", "reject", "bad"]:
                if hasattr(self, "reject_bad_v106"):
                    widget.configure(command=self.reject_bad_v106)
                elif hasattr(self, "reject_bad"):
...
_old_v109_build_ui = SliceIndexTracker.build_ui


def v109_build_ui(self):
    _old_v109_build_ui(self)

    try:
        self.root.title(f"BreakbeatAI v109 clean — {v109_current_break(self)}")
    except Exception:
        pass

    try:
        v109_patch_generate_reject_buttons(self)
    except Exception as exc:
        print("[v109] patch boutons impossible :", exc)

    try:
        v109_add_export_button(self)
    except Exception as exc:
        print("[v109] ajout export impossible :", exc)

    try:
        v109_bind_space_everywhere(self)
    except Exception as exc:
        print("[v109] bind espace impossible :", exc)
    ...
    def late():
        try:
            ...
            print("[v109] late bind/add impossible :", exc)

    try:
        self.root.after(900, late)
    except Exception:
        pass

    try:
        self.root.bind("<Control-e>", self.export_wav_current_v109)
    except Exception:
        pass

    v109_status(self, "v109 : Espace + Export WAV réparés. Ctrl+E aussi.")


SliceIndexTracker.space_play_v109 = v109_space_play
SliceIndexTracker.export_wav_current_v109 = v109_export_wav_current
SliceIndexTracker.build_ui = v109_build_ui
