...
"""
BreakbeatAI Tracker Editor v33 — plain samples
...
# v33 : ordre de base corrigé pour Atlantis Amen actuel.
# Affichage toujours simple : sample 0, sample 1...
# Mais la position musicale 6 utilise le sample 7.
#
# position : 0 1 2 3 4 5 6 7
# sample   : 0 3 2 4 5 1 7 6
BASE_SEQUENCE_8 = [0, 3, 2, 4, 5, 1, 7, 6]

# Pools cachés pour randomize propre, sans afficher kick/hat/snare.
# 0/5 = samples de kick, 2/6 = samples de snare, autres = hats.
POSITION_POOLS_8 = {
    0: [0, 1],
    1: [3, 4, 5, 6],
    2: [2, 7],
...
class PlainSampleTracker:
    def __init__(self, root, pair_json):
        ...
        self.root.title("BreakbeatAI Tracker Editor v33 — plain samples")
    ...
    def detect_external_player(self):
        for name in ["pw-play", "paplay", "aplay", "ffplay"]:
            ...
            if path:
                print(f"[v33] Backend audio : {name} -> {path}")
                return name

        print("[v33] Aucun backend système trouvé, fallback sounddevice.")
        return None
    ...
    def pool_for_pos(self, pos):
        """
        v33 : pool caché par position musicale.
        L'UI reste sample-only, mais Randomize ne met plus le sample 6
        à l'endroit où on attend le dernier snare.
        """
        local = int(pos) % 8
        existing = set(int(p) for p in self.pair_values)

        pool = [
            int(pair)
            for pair in POSITION_POOLS_8.get(local, list(self.pair_values))
            if int(pair) in existing
        ]

        if pool:
            return pool

        return list(self.pair_values)
    ...
    def base_pair_for_pos(self, pos):
        ...
        wanted = BASE_SEQUENCE_8[local]

        if int(wanted) in existing:
            return int(wanted)
    ...
    def pick_pair_for_pos(self, pos, randomize=False, rng=None):
        ...
        # Base : ordre corrigé, pas 0..7 brut.
    ...
    def build_initial_pattern(self, randomize=False):
        ...
        print(f"[v33] Génération loop32 | break={self.project['safe']} | randomize={randomize}")
        ...
        for pos in range(32):
            ...
            print(f"[v33] pos {pos:02d} -> sample {pair:02d}")
    ...
    def build_ui(self):
        ...
        title = tk.Label(
            main,
            text="BreakbeatAI v33 — plain samples, ordre corrigé",
            bg="#111018",
            fg="#ff7acc",
            font=("Sans", 20, "bold"),
        )
        ...
        self.output_label = tk.Label(
            panel,
            text="v33 : affichage sample-only. Base corrigée : pos 6 = sample 7, pos 7 = sample 6.",
            bg="#1b1824",
            fg="#77f5b5",
            justify="left",
        )
    ...
    def bind_keys(self):
        ...
        print("[v33] Raccourcis : Espace loop | flèches déplacer | Delete supprimer")
    ...
    def record_correction(self, event_type, before=None, after=None):
        ...
        event = {
            "version": "breakbeatai_plain_correction_v33",
            "time": now_stamp(),
            "safe": safe,
        ...
        corrections_path = CORRECTIONS_DIR / f"{safe}_plain_corrections_v33.jsonl"
        ...
        print(f"[v33] correction sauvegardée : {event_type}")
    ...
    def play_audio_array(self, audio, label="audio"):
        ...
        live_wav = OUT_DIR / f"{self.project['safe']}_v33_live.wav"
        ...
        if self.external_player:
            try:
                ...
                print(f"[v33] Erreur backend {self.external_player} : {exc}")
    ...
    def render_preview_file(self):
        OUT_DIR.mkdir(parents=True, exist_ok=True)
        wav = OUT_DIR / f"{self.project['safe']}_tracker_app_v33_preview.wav"
    ...
    def write_latest_pattern(self, reason="correction"):
        ...
        path = LATEST_DIR / f"{self.project['safe']}_latest_plain_v33.json"

        data = {
            "version": "breakbeatai_latest_plain_v33",
            "reason": reason,
            ...
            "base_sequence_8": BASE_SEQUENCE_8,
            "position_pools_8": POSITION_POOLS_8,
    ...
    def save(self):
        ...
        tracker_path = OUT_DIR / f"{self.project['safe']}_tracker_app_edit_v33.json"

        data = {
            "version": "tracker_app_edit_v33_plain_samples",
            "display_mode": "plain_samples_no_kick_hat_snare",
            "base_sequence_8": BASE_SEQUENCE_8,
            "position_pools_8": POSITION_POOLS_8,
        ...
        stamp = file_stamp()
        validated_path = VALIDATED_DIR / f"{self.project['safe']}_validated_plain_loop32_v33_{stamp}.json"

        validated = {
            "version": "breakbeatai_validated_plain_loop32_v33",
