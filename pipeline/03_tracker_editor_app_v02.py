...
"""
03_tracker_editor_app_v02.py

Vraie app Python native pour tracker BreakbeatAI.

Nouveautés v02 :
- vraie grille 16 cases
- blocs snapés sur la grille
- longueur de bloc : 1 case ou 2 cases
- espaces possibles entre les blocs
- Kick + Hat peut être 1 case chacun
- Espace = render + play la boucle
- Entrée = sauvegarder + render
- flèches = déplacer bloc sélectionné
- +/- = changer longueur du bloc
...
Usage :
    python pipeline/03_tracker_editor_app_v02.py --source "Amen"

Avec image :
    python pipeline/03_tracker_editor_app_v02.py --source "Amen" --image ~/Téléchargements/'break(1).png'
"""
...
# Pattern de départ sur grille 16 :
# kick hihat snare hihat | hihat kick snare hihat
# Chaque événement dure 2 cases par défaut.
DEFAULT_PATTERN = [
...
def stretch_or_crop(audio, target_len):
    """
    Pour un bloc d'une case, on prend la première moitié.
    Pour deux cases, bloc entier.
    Si la longueur ne colle pas, crop/pad simple.
    """
    if target_len <= 0:
...
def pattern_from_image(image_path):
    ...
    for i, e in enumerate(events):
        ...
        length = 1 if width_steps <= 1 else 2
...
class TrackerEditorApp:
    ...
    def build_ui(self):
        ...
        title = tk.Label(
            main,
            text="BreakbeatAI Tracker Editor v02",
            bg="#111018",
            fg="#ff7acc",
            font=("Sans", 20, "bold"),
        )
        ...
        src = tk.Label(
            main,
            text=f"Source : {self.project['source_audio']}    Espace = render + play    +/- = longueur 1/2 cases    1..8 = pair",
            bg="#111018",
            fg="#b9acc8",
        )
        src.pack(anchor="w", pady=(0, 12))
    ...
    def build_blocks_panel(self, parent):
        ...
        for i, block in enumerate(self.blocks):
            ...
            tk.Label(
                frame,
                text=f"{i + 1} — pair {block['pair']:02d}",
                bg="#241f31",
                fg="#ffe08a",
                font=("Sans", 11, "bold"),
    ...
    def build_edit_panel(self, parent):
        ...
        tk.Label(parent, text="Longueur case", bg="#1b1824", fg="#b9acc8").pack(anchor="w", padx=12)
        self.length_var = tk.IntVar(value=2)
        self.length_box = ttk.Combobox(parent, textvariable=self.length_var, values=[1, 2], state="readonly")
        ...
        buttons = [
            ...
            ("Espace: play loop", self.render_and_play),
        ]
        ...
        tk.Button(parent, text="Trier par step", command=self.sort_pattern, bg="#30283f", fg="#f5eefe").pack(fill="x", padx=12, pady=4)
    ...
    def draw_tracker(self):
        ...
        for lane in range(3):
            ...
            self.canvas.create_text(10, y0 + 18, text=LANE_LABELS[LANE_TO_ROLE[lane]], fill="#b9acc8", anchor="w", font=("Sans", 10, "bold"))

        for i in range(STEPS + 1):
            x = i * self.step_width
            color = "#5a4c6c" if i % 4 == 0 else "#30283f"
            width = 2 if i % 4 == 0 else 1
            self.canvas.create_line(x, 0, x, self.canvas_height, fill=color, width=width)

        for i in range(0, STEPS, 2):
            x = i * self.step_width + 3
            self.canvas.create_text(x, self.canvas_height - 10, text=str(i + 1), fill="#786a89", anchor="w", font=("Sans", 8))
    ...
    def draw_block(self, item):
        ...
        self.canvas.create_text(x + w / 2, y + block_h / 2, text=str(item["pair"]), fill="#1a0d14", font=("Sans", 10, "bold"), tags=tags)
    ...
    def set_length(self, length):
        ...
        if item is None:
            return
        item["length"] = int(max(1, min(2, length)))
    ...
    def change_length(self, delta):
        ...
        if item is None:
            return
        new_len = 2 if item["length"] == 1 else 1
    ...
    def render_preview(self, pattern, loops):
        ordered = sorted(pattern, key=lambda e: (e["step"], e["lane"]))

        # On part de la durée d'un bloc 2 cases.
        base_block = self.load_audio_for_pair(0)
        one_case_samples = max(1, len(base_block) // 2)
        ...
        for item in ordered:
            ...
            audio = self.load_audio_for_pair(pair)
            target_len = one_case_samples * length
            ...
            start = step * one_case_samples
            end = min(total_samples, start + len(audio))

            # Mix léger si deux blocs se croisent sur la même case.
            loop_audio[start:end] += audio[:end - start]

        loop_audio = normalize(loop_audio)

        out = np.concatenate([loop_audio for _ in range(loops)])
        out = normalize(out)

        OUT_DIR.mkdir(parents=True, exist_ok=True)
        wav = OUT_DIR / f"{self.project['safe']}_tracker_app_v02_preview.wav"
        sf.write(wav, out, SR)
        return str(wav)
    ...
    def save(self):
        ...
        data = {
            "version": "tracker_app_edit_v02_grid16",
            ...
            "grid": "16 steps, 3 lanes hat/kick/snare, events length 1 or 2 cases",
            "controls": {
                "space": "render and play loop",
                "arrows": "move selected block",
                "+/-": "toggle length 1/2 cases",
                "1-8": "assign pair audio",
            },
            "loops_preview": loops,
        ...
        json_path = OUT_DIR / f"{self.project['safe']}_tracker_app_edit_v02.json"
