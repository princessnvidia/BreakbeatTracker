...
"""
BreakbeatAI Tracker Editor v80 — slice index
...
def warp_audio_to_length_external(audio, target_len, cache_key=None):
    ...
    if rubberband:
        try:
            ...
            print(f"[v80] rubberband échec, fallback ffmpeg/no-speed : {exc}")
    ...
    if ffmpeg:
        try:
            ...
            print(f"[v80] ffmpeg atempo échec, fallback no-speed : {exc}")
...
class SliceIndexTracker:
    def __init__(self, root, pair_json):
        ...
        self.root.title("BreakbeatAI Tracker Editor v80 — slice index")
    ...
    def detect_external_player(self):
        for name in ["pw-play", "paplay", "aplay", "ffplay"]:
            ...
            if path:
                print(f"[v80] Backend audio : {name} -> {path}")
                return name

        print("[v80] Aucun backend système trouvé, fallback sounddevice.")
        return None
    ...
    def build_initial_pattern(self, randomize=False):
        ...
        print(
            f"[v80] Génération pattern | break={self.project['safe']} | "
        ...
        for slot in range(max_items):
            ...
            print(f"[v80] slot {slot:02d} | step {x_step:02d} -> slice {pair:02d}")
    ...
    def build_ui(self):
        ...
        title = tk.Label(
            main,
            text="BreakbeatAI v80 — same buttons every break",
            bg="#111018",
            fg="#ff7acc",
            font=("Sans", 20, "bold"),
        )
    ...
    def record_correction(self, event_type, before=None, after=None):
        ...
        print(f"[v80] correction sauvegardée : {event_type}")
    ...
    def play_audition_array(self, audio, label="audition"):
        ...
        audition_wav = OUT_DIR / f"{self.project['safe']}_v80_audition.wav"
        ...
        if self.external_player:
            try:
                ...
                print(f"[v80] Erreur audition {self.external_player} : {exc}")
    ...
    def on_close(self):
        ...
        """
        print("[v80] Fermeture : arrêt audio + destruction fenêtre")
    ...
    def play_audio_array(self, audio, label="audio"):
        ...
        audio = normalize(audio)
        live_wav = OUT_DIR / f"{self.project['safe']}_v80_live.wav"
        ...
        if self.external_player:
            try:
                ...
                print(f"[v80] Erreur backend {self.external_player} : {exc}")
    ...
    def render_preview_file(self):
        OUT_DIR.mkdir(parents=True, exist_ok=True)
        wav = OUT_DIR / f"{self.project['safe']}_tracker_app_v80_preview.wav"
    ...
    def write_latest_pattern(self, reason="correction"):
        ...
        path = LATEST_DIR / f"{self.project['safe']}_latest_slice_index_v80.json"

        data = {
            "version": "breakbeatai_latest_slice_index_v80",
            "reason": reason,
    ...
    def save(self):
        ...
        tracker_path = OUT_DIR / f"{self.project['safe']}_tracker_app_edit_v80.json"
        ...
        stamp = file_stamp()
        validated_path = VALIDATED_DIR / f"{self.project['safe']}_validated_slice_index_loop32_v80_{stamp}.json"

        validated = {
            "version": "breakbeatai_validated_slice_index_loop32_v80",
...
def v44_record_correction(self, event_type, before=None, after=None):
    ...
    print(f"[v80] correction sauvegardée : {event_type}")
    print(f"[v80] correction path : {corrections_path}")
    print(f"[v80] latest path : {latest_path}")
...
def v45_move_selected(self, dx, dy):
    ...
    if dy != 0 and before != after:
        try:
            ...
            print(f"[v80] audition après flèche haut/bas impossible : {exc}")
...
def v46_warp_loop_pitch_preserve(audio, target_len, cache_key):
    ...
    if rubberband:
        try:
            ...
            print(f"[v80] rubberband global warp échec : {exc}")
    ...
    if ffmpeg:
        try:
            ...
            print(f"[v80] ffmpeg global warp échec : {exc}")

    print("[v80] Aucun warp pitch-preserve dispo, fallback fit no-speed.")
    return v46_fit_audio_no_speed(audio, target_len)
...
def v46_build_initial_pattern(self, randomize=False):
    ...
    print(
        f"[v80] M8 pattern | break={self.project['safe']} | "
...
def v46_get_warped_loop(self):
    ...
    print(
        f"[v80] global warp OK | source_len={loop_end-loop_start} | "
...
def v46_build_ui(self):
    ...
    try:
        ...
        print(f"[v80] UI extension impossible : {exc}")
...
def v47_play_audio_array(self, audio, label="audio"):
    ...
    if len(audio) == 0 or v47_is_silent(audio):
        ...
        print(f"[v80] lecture annulée : audio silencieux ({label})")
        return 0
...
def v47_toggle_loop(self):
    ...
    if not getattr(self, "pattern", None):
        ...
        print("[v80] loop annulée : pattern vide")
        return "break"
...
def v47_delete_selected(self):
    ...
    if not getattr(self, "pattern", None):
        ...
        print("[v80] pattern vide après suppression : audio coupé")
...
def v48_build_initial_pattern(self, randomize=False):
    ...
    print("[v80] pattern initial forcé VIDE : aucun son ne doit jouer.")
    return []
...
def v48_load_first_slices(self):
    ...
    print(f"[v80] Load 16 slices : {len(self.pattern)} items")


def v48_debug_pattern(self, label="render"):
    print("")
    print(f"[v80] DEBUG PATTERN avant {label}")
    print(f"[v80] items = {len(getattr(self, 'pattern', []) or [])}")

    if not getattr(self, "pattern", None):
        print("[v80] pattern vide confirmé")
        print("")
        return
    ...
    for item in self.pattern:
        ...
        print(
            f"[v80] id={item_id} | step {x:02d}/{end:02d} | "
...
def v48_render_audio_with_timeline(self):
    ...
    if not getattr(self, "pattern", None):
        loop_samples = self.get_loop_samples()
        print(f"[v80] rendu silence total : pattern vide | samples={loop_samples}")
        return np.zeros(loop_samples, dtype=np.float32), []
    ...
    print(f"[v80] rendu audio peak={peak:.8f} | timeline_events={len(timeline) if timeline is not None else 'None'}")

    if timeline:
        print("[v80] timeline events:")
        for ev in timeline:
            print(f"[v80]   {ev}")
...
def v48_toggle_loop(self):
    if not getattr(self, "pattern", None):
        ...
        print("[v80] Space ignoré : pattern vide.")
        return "break"
...
def v48_build_ui(self):
    ...
    try:
        ...
        print(f"[v80] ajout UI impossible : {exc}")
...
def v49_add_item_at_xy(self, x, y):
    ...
    try:
        ...
        print(f"[v80] record_correction add impossible : {exc}")
    ...
    try:
        ...
        print(f"[v80] audition après ajout impossible : {exc}")

    print(f"[v80] add | id={item['id']} | step={x_step} | pair={pair} | lane={lane}")
...
def v49_duplicate_selected_forward(self, event=None):
    ...
    if item is None:
        ...
        print("[v80] duplicate annulé : aucune note sélectionnée")
        return "break"
    ...
    if new_x > max_x:
        ...
        print("[v80] duplicate annulé : fin de grille")
        return "break"
    ...
    try:
        ...
        print(f"[v80] record_correction duplicate impossible : {exc}")
    ...
    try:
        ...
        print(f"[v80] audition après duplicate impossible : {exc}")

    print(
        f"[v80] duplicate | old_id={item.get('id')} | new_id={new_item.get('id')} | "
...
def v49_build_ui(self):
    ...
    try:
        ...
        print(f"[v80] bind canvas click impossible : {exc}")
    ...
    try:
        ...
        print(f"[v80] bind keys impossible : {exc}")
...
def v50_load_style_model():
    ...
    try:
        ...
        print(f"[v80] impossible de charger le modèle IA : {exc}")
        return None
...
def v50_generate_ai_pattern(self, event=None):
    ...
    if model is None:
        ...
        print("[v80] modèle absent :", STYLE_MODEL_PATH)
        return "break"
    ...
    print("")
    print(f"[v80] GENERATE IA | safe={safe} | pairs={pair_values} | slots={max_slots}")
    ...
    for slot in range(max_slots):
        ...
        print(
            f"[v80] slot {slot:02d} | step {x_step:02d} | "
    ...
    try:
        ...
        print(f"[v80] write_latest_pattern impossible : {exc}")
...
def v50_build_ui(self):
    ...
    try:
        ...
        print(f"[v80] UI IA impossible : {exc}")
...
def v51_mouse_down(self, event):
    ...
    if item is not None:
        ...
        print(
            f"[v80] mouse down note | id={item.get('id')} | "
            f"step={item.get('x_step')} | pair={item.get('pair')}"
        )
    else:
        print("[v80] mouse down empty")
...
def v51_mouse_up(self, event):
    ...
    if state.get("empty"):
        ...
        print("[v80] drag depuis vide ignoré")
        return "break"
    ...
    if before != after:
        try:
            ...
            print(f"[v80] record_correction drag impossible : {exc}")
        ...
        print(
            f"[v80] drag move | id={after.get('id')} | "
            f"step {before.get('x_step')} -> {after.get('x_step')} | "
        ...
        try:
            ...
            print(f"[v80] audition après drag impossible : {exc}")
...
def v51_build_ui(self):
    ...
    try:
        ...
        print(f"[v80] bind souris impossible : {exc}")
    ...
    try:
        ...
        print(f"[v80] bind Suppr impossible : {exc}")
...
def v52_normalize_loaded_notes(self, notes):
    ...
    for raw in sorted(notes, key=lambda n: (int(float(n.get("x_step", 0))), int(float(n.get("pair", 0))))):
        ...
        if pair not in valid_pairs:
            print(f"[v80] slice ignorée car absente du break actuel : pair={pair}")
            continue
...
def v52_load_saved_beat(self, event=None):
    ...
    if not saves:
        ...
        print("[v80] aucune sauvegarde trouvée")
        return "break"
    ...
    if not pattern:
        ...
        print("[v80] sauvegarde incompatible :", chosen["path"])
        return "break"
    ...
    print("")
    print("[v80] BEAT RESTAURÉ")
    print("[v80] fichier :", chosen["path"])
    print("[v80] clé :", chosen["key"])
    print("[v80] notes :", len(pattern))
    print("")
...
def v52_list_saved_beats(self, event=None):
    ...
    print("")
    print("[v80] SAUVEGARDES TROUVÉES")
    for i, save in enumerate(saves[:30]):
        print(
            f"[v80] {i:02d} | notes={len(save['notes']):02d} | "
...
def v52_build_ui(self):
    ...
    try:
        ...
        print(f"[v80] UI load saved impossible : {exc}")
...
def v53_run_training(self):
    ...
    if not trainer.exists():
        ...
        print("[v80] trainer introuvable :", trainer)
        return False

    print("")
    print("[v80] AUTO-TRAIN IA après Save...")
    print("")
...
def v53_save(self):
    ...
    try:
        ...
        print(f"[v80] auto-train après save impossible : {exc}")
...
def v53_build_ui(self):
    ...
    try:
        ...
        print(f"[v80] UI auto-train impossible : {exc}")
...
def v55_mouse_up(self, event):
    ...
    if not state.get("moved"):
        ...
        try:
            ...
            print(f"[v80] audition clic note impossible : {exc}")
        ...
        print(
            f"[v80] click play/select | id={item.get('id')} | "
            f"step={item.get('x_step')} | pair={item.get('pair')}"
        )
...
def v58_add_item_at_xy(self, x, y):
    ...
    print(
        f"[v80] add | id={item['id']} | step={x_step} | "
...
def v58_toggle_selected_length(self, event=None):
    ...
    try:
        ...
        print(f"[v80] record_correction length impossible : {exc}")

    v58_refresh_after_edit(self, "toggle_note_length")

    v58_status(self, f"Longueur note : {old_len} -> {new_len} case(s).")
    print(f"[v80] length toggle | id={item.get('id')} | {old_len} -> {new_len}")
...
def v58_duplicate_selected_forward(self, event=None):
    ...
    try:
        ...
        print(f"[v80] audition duplicate impossible : {exc}")
...
def v58_build_ui(self):
    ...
    try:
        ...
        print(f"[v80] UI demi-grille impossible : {exc}")
    ...
    try:
        ...
        print(f"[v80] bind demi-grille impossible : {exc}")
...
def v59_rank_current_slices(self):
    ...
    for pair in self.pair_values:
        ...
        try:
            ...
            print(f"[v80] analyse pair {pair} impossible : {exc}")
            role = "other"
    ...
    print("")
    print("[v80] TOP ROLES CURRENT BREAK")
    for r in ("kick", "snare", "hat"):
        print(f"[v80] {r:5s}:", [(x["pair"], round(x["score"], 3)) for x in ranks[r][:6]])
    print("")
...
def v59_load_role_model():
    ...
    try:
        ...
        print(f"[v80] lecture modèle rôle impossible : {exc}")
        return None
...
def v59_generate_role_aware(self, event=None):
    ...
    if model is None:
        ...
        print("[v80] modèle rôle absent, fallback musical snare 6.")
        model = {
            "breaks": {},
            "global": {},
            "patterns_used": 0,
    ...
    print("")
    print("[v80] ROLE-AWARE GENERATION")
    print("[v80] model patterns_used:", model.get("patterns_used", 0))
    for item in pattern:
        print(
            f"[v80] step {item['x_step']:02d}/{item['x_step'] + item['length'] - 1:02d} | "
...
def v59_run_role_training(self, event=None):
    ...
    try:
        ...
        print(f"[v80] training rôle impossible : {exc}")
...
def v59_save(self):
    ...
    try:
        ...
        print(f"[v80] auto-training rôle après save impossible : {exc}")
...
def v59_build_ui(self):
    ...
    try:
        ...
        print(f"[v80] UI role-aware impossible : {exc}")
...
def v62_add_item_at_xy(self, x, y):
    ...
    print(f"[v80] add | id={item['id']} | step={x_step} | len={length} | pair={pair}")
...
def v62_mouse_down(self, event):
    ...
    if item is not None:
        ...
        print(
            f"[v80] mouse down {state['mode']} | id={item.get('id')} | "
            f"step={item.get('x_step')} | len={item.get('length')} | pair={item.get('pair')}"
        )
    else:
        print("[v80] mouse down empty")
    ...
    if mode == "resize":
        if not state.get("moved"):
            ...
            try:
                ...
                print(f"[v80] audition clic resize impossible : {exc}")
        ...
        if before != after:
            try:
                ...
                print(f"[v80] record_correction resize impossible : {exc}")
            ...
            try:
                ...
                print(f"[v80] audition après resize impossible : {exc}")
    ...
    if mode == "move":
        ...
        if before != after:
            try:
                ...
                print(f"[v80] record_correction move impossible : {exc}")
            ...
            try:
                ...
                print(f"[v80] audition après move impossible : {exc}")
...
def v62_toggle_selected_length(self, event=None):
    ...
    try:
        ...
        print(f"[v80] record_correction toggle length impossible : {exc}")

    v62_refresh_after_edit(self, "toggle_note_length_v62")
    v62_status(self, f"Longueur note : {old_len} → {new_len} case(s).")

    print(f"[v80] toggle length | id={item.get('id')} | {old_len} -> {new_len}")
...
def v62_draw(self):
    ...
    try:
        ...
        print(f"[v80] draw handles impossible : {exc}")
...
def v62_build_ui(self):
    ...
    try:
        ...
        print(f"[v80] bind resize impossible : {exc}")
    ...
    try:
        ...
        print(f"[v80] UI resize impossible : {exc}")
...
def v63_load_model():
    ...
    try:
        ...
        print(f"[v80] lecture modèle strict impossible : {exc}")
        return None
...
def v63_generate_safe_role_ai(self, event=None):
    ...
    print("")
    print("[v80] SAFE ROLE AI GENERATED")
    print("[v80] model strict:", "yes" if model else "no")
    print("[v80] snare main/alt:", main_snare, alt_snare)
    print("[v80] kick  main/alt:", main_kick, alt_kick)
    print("[v80] hat   main/alt:", main_hat, alt_hat)
    print("[v80] pattern:")
    for item in pattern:
        print(
            f"[v80] step {item['x_step']:02d}/{item['x_step'] + item['length'] - 1:02d} | "
...
def v63_run_strict_training(self, event=None):
    ...
    print("")
    print("[v80] TRAIN STRICT ROLE MODEL...")
    print("")
    ...
    try:
        ...
        print(f"[v80] training strict impossible : {exc}")
...
def v63_build_ui(self):
    ...
    try:
        ...
        print(f"[v80] UI safe impossible : {exc}")
...
def v65_rank_slices(self):
    ...
    for pair in self.pair_values:
        ...
        try:
            ...
            print(f"[v80] analyse slice {pair} impossible : {exc}")
            scores = {
                "kick": 0.0,
                "snare": 0.0,
                "hat": 0.0,
    ...
    print("")
    print("[v80] TOP SLICES PAR ROLE — break courant")
    for role in ("kick", "snare", "hat", "ghost_snare"):
        print(f"[v80] {role:12s}:", [(x["pair"], round(x["score"], 3)) for x in ranks[role][:8]])
    print("")
...
def v65_generate_from_role_template(self, event=None):
    ...
    try:
        ...
        print(f"[v80] template introuvable : {exc}")
        return "break"
    ...
    print("")
    print("[v80] CROSS-SONG ROLE TEMPLATE GENERATED")
    print("[v80] template:", V65_TEMPLATE_PATH)
    print("[v80] temperature:", temperature, "| hats:", hat_density, "| ghosts:", ghost_density)
    print("[v80] pattern:")
    for item in pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v80] step {item['x_step']:02d}/{end:02d} | "
...
def v67_train_strict_if_available(self):
    ...
    if trainer is None:
        print("[v80] aucun trainer trouvé.")
        return False

    print("")
    print("[v80] TRAIN après validation :", trainer)
    print("")
    ...
    try:
        ...
        print(f"[v80] training impossible : {exc}")
        return False
...
def v67_generate_candidate(self, event=None):
    ...
    print("")
    print("[v80] CANDIDATE GENERATED")
    print("[v80] break:", self.v67_candidate_safe)
    print("[v80] notes:", len(candidate))
    print("[v80] Maintenant : corrige puis clique Good après modifs, ou Reject.")
    print("")
...
def v67_accept_corrected(self, event=None):
    ...
    print("")
    print("[v80] ACCEPTED GOOD AFTER EDIT")
    print("[v80] saved:", validated_path)
    print("[v80] trained:", trained)
    print("")
...
def v67_accept_without_training(self, event=None):
    ...
    print("[v80] accepted no-train:", validated_path)
...
def v67_build_ui(self):
    ...
    try:
        ...
        print(f"[v80] UI feedback impossible : {exc}")
...
def v68_generate_candidate(self, event=None):
    ...
    print("[v80] Length policy appliquée : 2 cases par défaut, 1 case rare.")
...
def v69_force_pattern_hard_2_grid(self, reason="v69_force_hard_2_grid"):
    ...
    print("")
    print("[v80] HARD 2 GRID APPLIQUÉ")
    for item in self.pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v80] step {item['x_step']:02d}/{end:02d} | "
...
def v70_duplicate_selected_forward(self, event=None):
    ...
    print("")
    print("[v80] CTRL+D DUPLICATE +2")
    print(f"[v80] old_id={item.get('id')} new_id={new_item.get('id')}")
    print(f"[v80] step {old_x}/{old_x + length - 1} -> {new_x}/{new_x + length - 1}")
    print(f"[v80] pair={new_item.get('pair')} len={length}")
    print("")
...
def v70_build_ui(self):
    ...
    try:
        ...
        print(f"[v80] bind Ctrl+D impossible : {exc}")
...
def v71_rank_slices(self):
    ...
    print("")
    print("[v80] PRE-LEARNING TOP SLICES")
    for role in ("kick", "snare", "hat", "ghost_snare"):
        print(f"[v80] {role:12s}:", [(x["pair"], round(x["score"], 3)) for x in ranks[role][:8]])
    print("")
...
def v71_prelearn_roles(self, force=False):
    ...
    print("")
    print("[v80] PRELEARN ROLES POUR", safe)
    for role in ("kick", "snare", "hat", "ghost_snare", "bad"):
        print(f"[v80] {role:12s}:", roles.get(role, []))
    print("")
...
def v71_generate_full_candidate(self, event=None):
    ...
    print("")
    print("[v80] FULL CANDIDATE GENERATED")
    print("[v80] break:", v71_safe(self))
    print("[v80] roles:", roles)
    print("[v80] pattern:")
    for item in pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
...
def v72_open_break(self, safe=None):
    ...
    print("")
    print("[v80] OPEN BREAK")
    print("[v80] current:", current)
    print("[v80] next   :", safe)
    print("[v80] cmd    :", " ".join(cmd))
    print("")
...
def v72_build_ui(self):
    ...
    try:
        ...
        print(f"[v80] UI break switcher impossible : {exc}")
...
def v73_sanitize_tracker_pattern(self, reason="v73_tracker_strict_no_overlap"):
    ...
    print("")
    print("[v80] TRACKER STRICT NO OVERLAP")
    print("[v80] kept:", len(accepted), "| rejected overlaps:", len(rejected))

    for item in accepted:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v80] KEEP step {item['x_step']:02d}/{end:02d} | "
    ...
    if rejected:
        ...
        for item in rejected:
            ...
            print(
                f"[v80] DROP step {item['x_step']:02d}/{end:02d} | "
                f"role={item.get('v73_role_key')} | pair={item.get('pair')}"
            )
...
# ---------------------------------------------------------------------
# v80 EXTENSION : les mêmes boutons training sur chaque break chargé
# ---------------------------------------------------------------------

V80_LEARNING_DIR = Path("dataset/learning")
V80_VALIDATED_DIR = V80_LEARNING_DIR / "validated_patterns"
V80_REJECTED_DIR = V80_LEARNING_DIR / "rejected_patterns"
V80_FEEDBACK_PATH = V80_LEARNING_DIR / "role_feedback_v01.jsonl"


def v80_status(self, text):
    try:
        if hasattr(self, "set_status"):
            self.set_status(text)
        else:
...
def v80_slug(text):
    ...
    text = text.strip("_")
    return text or "unknown_break"


def v80_all_widgets(widget):
    out = [widget]
    try:
    ...
    for child in children:
        out.extend(v80_all_widgets(child))

    return out


def v80_get_selected_break_from_existing_ui(self):
    """
    Récupère le break choisi dans le menu déjà existant en haut :
    Break [combo] [Charger break]
    """
    current = v80_current_safe(self)

    # Variables possibles selon les anciennes versions.
    for attr in [
        "break_var",
        "source_var",
        "selected_break_var",
    ...
    # Sinon on lit directement les combobox/widgets qui ont .get().
    try:
        for widget in v80_all_widgets(self.root):
            try:
                value = widget.get()
            except Exception:
                continue
            ...
            # On évite les spinbox numériques slice/ligne/step.
            if value.isdigit():
                continue

            if "Break" in value or "Amen" in value or "_" in value:
                return value
    except Exception:
        pass
...
def v80_launch_break(self, source):
    source = str(source).strip()

    if not source:
        v80_status(self, "Aucun break sélectionné.")
        return "break"
    ...
    print("")
    print("[v80] CHARGER BREAK")
    print("[v80] courant :", v80_current_safe(self))
    print("[v80] nouveau :", source)
    print("[v80] cmd     :", " ".join(cmd))
    print("")
...
def v80_load_selected_break(self, event=None):
    selected = v80_get_selected_break_from_existing_ui(self)
    current = v80_current_safe(self)
    ...
    if v80_slug(selected).lower() == v80_slug(current).lower():
        v80_status(self, f"Déjà sur {current}.")
        return "break"

    return v80_launch_break(self, selected)


def v80_patch_charger_break_buttons(self):
    """
    Le bouton existant 'Charger break' doit relancer l'app sur le break choisi,
    pour que toute l'UI complète revienne sur ce break.
    """
    patched = 0
    ...
    for widget in v80_all_widgets(self.root):
        ...
        if text in ["charger break", "charger le break", "load break", "open break"]:
            try:
                widget.configure(command=self.v80_load_selected_break)
                patched += 1
            except Exception:
                pass

    print(f"[v80] boutons Charger break patchés : {patched}")


def v80_real_generator():
    """
    Générateur musical réel.
    On ne veut pas de wrapper de sélection de break ici :
    ces boutons bossent sur le break chargé.
    """
    for name in [
...
def v80_generate_candidate_current(self, event=None):
    """
    Generate Candidate du break chargé.
    """
    fn = v80_real_generator()
    current = v80_current_safe(self)

    print("")
    print("[v80] GENERATE CANDIDATE")
    print("[v80] break chargé :", current)
    print("[v80] générateur   :", getattr(fn, "__name__", None))
    print("")

    if fn is None:
        v80_status(self, "Aucun générateur musical trouvé.")
        return "break"
    ...
    v80_status(self, f"Generate Candidate OK sur {current}.")
    return result


def v80_feedback_payload(self, verdict, train_now):
    current = v80_current_safe(self)
    ...
    return {
        "version": "v80_same_buttons_every_break",
        "created_at": time.strftime("%Y-%m-%d %H:%M:%S"),
        "break": current,
        "verdict": verdict,
        ...
        "note": "Feedback sauvegardé depuis les boutons globaux du break chargé."
    }


def v80_save_feedback(self, verdict, train_now=False):
    """
    Sauvegarde Good / Reject pour le break chargé.
    """
    current = v80_current_safe(self)
    pattern = getattr(self, "pattern", []) or []

    if not pattern and verdict != "reject_bad":
        v80_status(self, "Pattern vide : rien à sauvegarder.")
        return "break"

    V80_VALIDATED_DIR.mkdir(parents=True, exist_ok=True)
    V80_REJECTED_DIR.mkdir(parents=True, exist_ok=True)
    V80_FEEDBACK_PATH.parent.mkdir(parents=True, exist_ok=True)

    payload = v80_feedback_payload(self, verdict, train_now)

    stamp = time.strftime("%Y%m%d_%H%M%S")
    safe_slug = v80_slug(current)

    if verdict == "reject_bad":
        out_path = V80_REJECTED_DIR / f"{stamp}_{safe_slug}_reject_bad.json"
    elif train_now:
        out_path = V80_VALIDATED_DIR / f"{stamp}_{safe_slug}_good_train.json"
    else:
        out_path = V80_VALIDATED_DIR / f"{stamp}_{safe_slug}_good_save_only.json"
    ...
    with V80_FEEDBACK_PATH.open("a", encoding="utf-8") as f:
        f.write(json.dumps(payload, ensure_ascii=False) + "\n")

    print("")
    print("[v80] FEEDBACK SAUVÉ")
    print("[v80] break  :", current)
    print("[v80] verdict:", verdict)
    print("[v80] train  :", train_now)
    print("[v80] fichier:", out_path)
    print("")

    if train_now:
        v80_run_training(self)

    if verdict == "reject_bad":
        v80_status(self, f"Reject / Bad sauvegardé pour {current}.")
    elif train_now:
        v80_status(self, f"Good + training lancé pour {current}.")
    else:
        v80_status(self, f"Good sans train sauvegardé pour {current}.")

    return "break"


def v80_run_training(self):
    """
    Lance le training strict si disponible.
    """
    candidates = [
        Path("pipeline/04_train_beat_roles_v04_strict.py"),
        Path("pipeline/04_train_beat_roles_v03.py"),
    ...
    if script is None:
        print("[v80] Aucun script training trouvé.")
        v80_status(self, "Feedback sauvegardé, mais aucun script training trouvé.")
        return
...
def v80_good_after_modifs(self, event=None):
    return v80_save_feedback(self, "good_after_modifs", train_now=True)


def v80_good_no_train(self, event=None):
    return v80_save_feedback(self, "good_no_train", train_now=False)


def v80_reject_bad(self, event=None):
    return v80_save_feedback(self, "reject_bad", train_now=False)


_old_v80_build_ui = SliceIndexTracker.build_ui


def v80_build_ui(self):
    _old_v80_build_ui(self)

    try:
        self.root.title(f"BreakbeatAI v80 — {v80_current_safe(self)}")
    except Exception:
        pass

    # Le bouton déjà existant "Charger break" doit charger le break complet
    # avec les mêmes boutons, pas juste bidouiller Camo.
    try:
        v80_patch_charger_break_buttons(self)
    except Exception as exc:
        print(f"[v80] patch Charger break impossible : {exc}")

    try:
        frame = tk.Frame(self.root, bg="#0c1820")
        frame.pack(fill="x", padx=4, pady=(8, 4))

        tk.Label(
            frame,
            text=f"Training du break chargé : {v80_current_safe(self)}",
            bg="#0c1820",
            fg="#7dffd1",
            font=("Sans", 10, "bold"),
        ).pack(side="left", padx=8)
        ...
        tk.Button(
            frame,
            text="Good après modifs",
            command=self.good_after_modifs_v80,
            bg="#3f5f48",
            fg="#ffffff",
        ).pack(side="left", padx=4)
        ...
        tk.Button(
            frame,
            text="Good sans train",
            command=self.good_no_train_v80,
            bg="#506050",
            fg="#ffffff",
        ).pack(side="left", padx=4)
        ...
        tk.Label(
            frame,
            text="v80 : ces boutons s'appliquent toujours au break actuellement chargé.",
            bg="#0c1820",
            fg="#f5d67b",
        ).pack(side="left", padx=8)

    except Exception as exc:
        print(f"[v80] barre training impossible : {exc}")

    v80_status(
        self,
        f"v80 : mêmes boutons training pour chaque break chargé. Actuel = {v80_current_safe(self)}."
    )


SliceIndexTracker.v80_load_selected_break = v80_load_selected_break
SliceIndexTracker.good_after_modifs_v80 = v80_good_after_modifs
SliceIndexTracker.good_no_train_v80 = v80_good_no_train
SliceIndexTracker.reject_bad_v80 = v80_reject_bad

# Tous les anciens boutons Generate Candidate bossent maintenant sur le break chargé.
SliceIndexTracker.generate_candidate = v80_generate_candidate_current
SliceIndexTracker.generate_full_candidate = v80_generate_candidate_current
SliceIndexTracker.generate_ai_pattern = v80_generate_candidate_current
SliceIndexTracker.generate_cross_song_template = v80_generate_candidate_current
SliceIndexTracker.generate_safe_experiment = v80_generate_candidate_current
SliceIndexTracker.generate_locked_roles = v80_generate_candidate_current
SliceIndexTracker.generate_role_aware = v80_generate_candidate_current
SliceIndexTracker.generate_safe_role_ai = v80_generate_candidate_current

SliceIndexTracker.build_ui = v80_build_ui
