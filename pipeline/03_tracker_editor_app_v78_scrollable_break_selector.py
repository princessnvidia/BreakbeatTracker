...
"""
BreakbeatAI Tracker Editor v78 — slice index
...
def warp_audio_to_length_external(audio, target_len, cache_key=None):
    ...
    if rubberband:
        try:
            ...
            print(f"[v78] rubberband échec, fallback ffmpeg/no-speed : {exc}")
    ...
    if ffmpeg:
        try:
            ...
            print(f"[v78] ffmpeg atempo échec, fallback no-speed : {exc}")
...
class SliceIndexTracker:
    def __init__(self, root, pair_json):
        ...
        self.root.title("BreakbeatAI Tracker Editor v78 — slice index")
    ...
    def detect_external_player(self):
        for name in ["pw-play", "paplay", "aplay", "ffplay"]:
            ...
            if path:
                print(f"[v78] Backend audio : {name} -> {path}")
                return name

        print("[v78] Aucun backend système trouvé, fallback sounddevice.")
        return None
    ...
    def build_initial_pattern(self, randomize=False):
        ...
        print(
            f"[v78] Génération pattern | break={self.project['safe']} | "
        ...
        for slot in range(max_items):
            ...
            print(f"[v78] slot {slot:02d} | step {x_step:02d} -> slice {pair:02d}")
    ...
    def build_ui(self):
        ...
        title = tk.Label(
            main,
            text="BreakbeatAI v78 — scrollable break selector",
            bg="#111018",
            fg="#ff7acc",
            font=("Sans", 20, "bold"),
        )
    ...
    def record_correction(self, event_type, before=None, after=None):
        ...
        print(f"[v78] correction sauvegardée : {event_type}")
    ...
    def play_audition_array(self, audio, label="audition"):
        ...
        audition_wav = OUT_DIR / f"{self.project['safe']}_v78_audition.wav"
        ...
        if self.external_player:
            try:
                ...
                print(f"[v78] Erreur audition {self.external_player} : {exc}")
    ...
    def on_close(self):
        ...
        """
        print("[v78] Fermeture : arrêt audio + destruction fenêtre")
    ...
    def play_audio_array(self, audio, label="audio"):
        ...
        audio = normalize(audio)
        live_wav = OUT_DIR / f"{self.project['safe']}_v78_live.wav"
        ...
        if self.external_player:
            try:
                ...
                print(f"[v78] Erreur backend {self.external_player} : {exc}")
    ...
    def render_preview_file(self):
        OUT_DIR.mkdir(parents=True, exist_ok=True)
        wav = OUT_DIR / f"{self.project['safe']}_tracker_app_v78_preview.wav"
    ...
    def write_latest_pattern(self, reason="correction"):
        ...
        path = LATEST_DIR / f"{self.project['safe']}_latest_slice_index_v78.json"

        data = {
            "version": "breakbeatai_latest_slice_index_v78",
            "reason": reason,
    ...
    def save(self):
        ...
        tracker_path = OUT_DIR / f"{self.project['safe']}_tracker_app_edit_v78.json"
        ...
        stamp = file_stamp()
        validated_path = VALIDATED_DIR / f"{self.project['safe']}_validated_slice_index_loop32_v78_{stamp}.json"

        validated = {
            "version": "breakbeatai_validated_slice_index_loop32_v78",
...
def v44_record_correction(self, event_type, before=None, after=None):
    ...
    print(f"[v78] correction sauvegardée : {event_type}")
    print(f"[v78] correction path : {corrections_path}")
    print(f"[v78] latest path : {latest_path}")
...
def v45_move_selected(self, dx, dy):
    ...
    if dy != 0 and before != after:
        try:
            ...
            print(f"[v78] audition après flèche haut/bas impossible : {exc}")
...
def v46_warp_loop_pitch_preserve(audio, target_len, cache_key):
    ...
    if rubberband:
        try:
            ...
            print(f"[v78] rubberband global warp échec : {exc}")
    ...
    if ffmpeg:
        try:
            ...
            print(f"[v78] ffmpeg global warp échec : {exc}")

    print("[v78] Aucun warp pitch-preserve dispo, fallback fit no-speed.")
    return v46_fit_audio_no_speed(audio, target_len)
...
def v46_build_initial_pattern(self, randomize=False):
    ...
    print(
        f"[v78] M8 pattern | break={self.project['safe']} | "
...
def v46_get_warped_loop(self):
    ...
    print(
        f"[v78] global warp OK | source_len={loop_end-loop_start} | "
...
def v46_build_ui(self):
    ...
    try:
        ...
        print(f"[v78] UI extension impossible : {exc}")
...
def v47_play_audio_array(self, audio, label="audio"):
    ...
    if len(audio) == 0 or v47_is_silent(audio):
        ...
        print(f"[v78] lecture annulée : audio silencieux ({label})")
        return 0
...
def v47_toggle_loop(self):
    ...
    if not getattr(self, "pattern", None):
        ...
        print("[v78] loop annulée : pattern vide")
        return "break"
...
def v47_delete_selected(self):
    ...
    if not getattr(self, "pattern", None):
        ...
        print("[v78] pattern vide après suppression : audio coupé")
...
def v48_build_initial_pattern(self, randomize=False):
    ...
    print("[v78] pattern initial forcé VIDE : aucun son ne doit jouer.")
    return []
...
def v48_load_first_slices(self):
    ...
    print(f"[v78] Load 16 slices : {len(self.pattern)} items")


def v48_debug_pattern(self, label="render"):
    print("")
    print(f"[v78] DEBUG PATTERN avant {label}")
    print(f"[v78] items = {len(getattr(self, 'pattern', []) or [])}")

    if not getattr(self, "pattern", None):
        print("[v78] pattern vide confirmé")
        print("")
        return
    ...
    for item in self.pattern:
        ...
        print(
            f"[v78] id={item_id} | step {x:02d}/{end:02d} | "
...
def v48_render_audio_with_timeline(self):
    ...
    if not getattr(self, "pattern", None):
        loop_samples = self.get_loop_samples()
        print(f"[v78] rendu silence total : pattern vide | samples={loop_samples}")
        return np.zeros(loop_samples, dtype=np.float32), []
    ...
    print(f"[v78] rendu audio peak={peak:.8f} | timeline_events={len(timeline) if timeline is not None else 'None'}")

    if timeline:
        print("[v78] timeline events:")
        for ev in timeline:
            print(f"[v78]   {ev}")
...
def v48_toggle_loop(self):
    if not getattr(self, "pattern", None):
        ...
        print("[v78] Space ignoré : pattern vide.")
        return "break"
...
def v48_build_ui(self):
    ...
    try:
        ...
        print(f"[v78] ajout UI impossible : {exc}")
...
def v49_add_item_at_xy(self, x, y):
    ...
    try:
        ...
        print(f"[v78] record_correction add impossible : {exc}")
    ...
    try:
        ...
        print(f"[v78] audition après ajout impossible : {exc}")

    print(f"[v78] add | id={item['id']} | step={x_step} | pair={pair} | lane={lane}")
...
def v49_duplicate_selected_forward(self, event=None):
    ...
    if item is None:
        ...
        print("[v78] duplicate annulé : aucune note sélectionnée")
        return "break"
    ...
    if new_x > max_x:
        ...
        print("[v78] duplicate annulé : fin de grille")
        return "break"
    ...
    try:
        ...
        print(f"[v78] record_correction duplicate impossible : {exc}")
    ...
    try:
        ...
        print(f"[v78] audition après duplicate impossible : {exc}")

    print(
        f"[v78] duplicate | old_id={item.get('id')} | new_id={new_item.get('id')} | "
...
def v49_build_ui(self):
    ...
    try:
        ...
        print(f"[v78] bind canvas click impossible : {exc}")
    ...
    try:
        ...
        print(f"[v78] bind keys impossible : {exc}")
...
def v50_load_style_model():
    ...
    try:
        ...
        print(f"[v78] impossible de charger le modèle IA : {exc}")
        return None
...
def v50_generate_ai_pattern(self, event=None):
    ...
    if model is None:
        ...
        print("[v78] modèle absent :", STYLE_MODEL_PATH)
        return "break"
    ...
    print("")
    print(f"[v78] GENERATE IA | safe={safe} | pairs={pair_values} | slots={max_slots}")
    ...
    for slot in range(max_slots):
        ...
        print(
            f"[v78] slot {slot:02d} | step {x_step:02d} | "
    ...
    try:
        ...
        print(f"[v78] write_latest_pattern impossible : {exc}")
...
def v50_build_ui(self):
    ...
    try:
        ...
        print(f"[v78] UI IA impossible : {exc}")
...
def v51_mouse_down(self, event):
    ...
    if item is not None:
        ...
        print(
            f"[v78] mouse down note | id={item.get('id')} | "
            f"step={item.get('x_step')} | pair={item.get('pair')}"
        )
    else:
        print("[v78] mouse down empty")
...
def v51_mouse_up(self, event):
    ...
    if state.get("empty"):
        ...
        print("[v78] drag depuis vide ignoré")
        return "break"
    ...
    if before != after:
        try:
            ...
            print(f"[v78] record_correction drag impossible : {exc}")
        ...
        print(
            f"[v78] drag move | id={after.get('id')} | "
            f"step {before.get('x_step')} -> {after.get('x_step')} | "
        ...
        try:
            ...
            print(f"[v78] audition après drag impossible : {exc}")
...
def v51_build_ui(self):
    ...
    try:
        ...
        print(f"[v78] bind souris impossible : {exc}")
    ...
    try:
        ...
        print(f"[v78] bind Suppr impossible : {exc}")
...
def v52_normalize_loaded_notes(self, notes):
    ...
    for raw in sorted(notes, key=lambda n: (int(float(n.get("x_step", 0))), int(float(n.get("pair", 0))))):
        ...
        if pair not in valid_pairs:
            print(f"[v78] slice ignorée car absente du break actuel : pair={pair}")
            continue
...
def v52_load_saved_beat(self, event=None):
    ...
    if not saves:
        ...
        print("[v78] aucune sauvegarde trouvée")
        return "break"
    ...
    if not pattern:
        ...
        print("[v78] sauvegarde incompatible :", chosen["path"])
        return "break"
    ...
    print("")
    print("[v78] BEAT RESTAURÉ")
    print("[v78] fichier :", chosen["path"])
    print("[v78] clé :", chosen["key"])
    print("[v78] notes :", len(pattern))
    print("")
...
def v52_list_saved_beats(self, event=None):
    ...
    print("")
    print("[v78] SAUVEGARDES TROUVÉES")
    for i, save in enumerate(saves[:30]):
        print(
            f"[v78] {i:02d} | notes={len(save['notes']):02d} | "
...
def v52_build_ui(self):
    ...
    try:
        ...
        print(f"[v78] UI load saved impossible : {exc}")
...
def v53_run_training(self):
    ...
    if not trainer.exists():
        ...
        print("[v78] trainer introuvable :", trainer)
        return False

    print("")
    print("[v78] AUTO-TRAIN IA après Save...")
    print("")
...
def v53_save(self):
    ...
    try:
        ...
        print(f"[v78] auto-train après save impossible : {exc}")
...
def v53_build_ui(self):
    ...
    try:
        ...
        print(f"[v78] UI auto-train impossible : {exc}")
...
def v55_mouse_up(self, event):
    ...
    if not state.get("moved"):
        ...
        try:
            ...
            print(f"[v78] audition clic note impossible : {exc}")
        ...
        print(
            f"[v78] click play/select | id={item.get('id')} | "
            f"step={item.get('x_step')} | pair={item.get('pair')}"
        )
...
def v58_add_item_at_xy(self, x, y):
    ...
    print(
        f"[v78] add | id={item['id']} | step={x_step} | "
...
def v58_toggle_selected_length(self, event=None):
    ...
    try:
        ...
        print(f"[v78] record_correction length impossible : {exc}")

    v58_refresh_after_edit(self, "toggle_note_length")

    v58_status(self, f"Longueur note : {old_len} -> {new_len} case(s).")
    print(f"[v78] length toggle | id={item.get('id')} | {old_len} -> {new_len}")
...
def v58_duplicate_selected_forward(self, event=None):
    ...
    try:
        ...
        print(f"[v78] audition duplicate impossible : {exc}")
...
def v58_build_ui(self):
    ...
    try:
        ...
        print(f"[v78] UI demi-grille impossible : {exc}")
    ...
    try:
        ...
        print(f"[v78] bind demi-grille impossible : {exc}")
...
def v59_rank_current_slices(self):
    ...
    for pair in self.pair_values:
        ...
        try:
            ...
            print(f"[v78] analyse pair {pair} impossible : {exc}")
            role = "other"
    ...
    print("")
    print("[v78] TOP ROLES CURRENT BREAK")
    for r in ("kick", "snare", "hat"):
        print(f"[v78] {r:5s}:", [(x["pair"], round(x["score"], 3)) for x in ranks[r][:6]])
    print("")
...
def v59_load_role_model():
    ...
    try:
        ...
        print(f"[v78] lecture modèle rôle impossible : {exc}")
        return None
...
def v59_generate_role_aware(self, event=None):
    ...
    if model is None:
        ...
        print("[v78] modèle rôle absent, fallback musical snare 6.")
        model = {
            "breaks": {},
            "global": {},
            "patterns_used": 0,
    ...
    print("")
    print("[v78] ROLE-AWARE GENERATION")
    print("[v78] model patterns_used:", model.get("patterns_used", 0))
    for item in pattern:
        print(
            f"[v78] step {item['x_step']:02d}/{item['x_step'] + item['length'] - 1:02d} | "
...
def v59_run_role_training(self, event=None):
    ...
    try:
        ...
        print(f"[v78] training rôle impossible : {exc}")
...
def v59_save(self):
    ...
    try:
        ...
        print(f"[v78] auto-training rôle après save impossible : {exc}")
...
def v59_build_ui(self):
    ...
    try:
        ...
        print(f"[v78] UI role-aware impossible : {exc}")
...
def v62_add_item_at_xy(self, x, y):
    ...
    print(f"[v78] add | id={item['id']} | step={x_step} | len={length} | pair={pair}")
...
def v62_mouse_down(self, event):
    ...
    if item is not None:
        ...
        print(
            f"[v78] mouse down {state['mode']} | id={item.get('id')} | "
            f"step={item.get('x_step')} | len={item.get('length')} | pair={item.get('pair')}"
        )
    else:
        print("[v78] mouse down empty")
    ...
    if mode == "resize":
        if not state.get("moved"):
            ...
            try:
                ...
                print(f"[v78] audition clic resize impossible : {exc}")
        ...
        if before != after:
            try:
                ...
                print(f"[v78] record_correction resize impossible : {exc}")
            ...
            try:
                ...
                print(f"[v78] audition après resize impossible : {exc}")
    ...
    if mode == "move":
        ...
        if before != after:
            try:
                ...
                print(f"[v78] record_correction move impossible : {exc}")
            ...
            try:
                ...
                print(f"[v78] audition après move impossible : {exc}")
...
def v62_toggle_selected_length(self, event=None):
    ...
    try:
        ...
        print(f"[v78] record_correction toggle length impossible : {exc}")

    v62_refresh_after_edit(self, "toggle_note_length_v62")
    v62_status(self, f"Longueur note : {old_len} → {new_len} case(s).")

    print(f"[v78] toggle length | id={item.get('id')} | {old_len} -> {new_len}")
...
def v62_draw(self):
    ...
    try:
        ...
        print(f"[v78] draw handles impossible : {exc}")
...
def v62_build_ui(self):
    ...
    try:
        ...
        print(f"[v78] bind resize impossible : {exc}")
    ...
    try:
        ...
        print(f"[v78] UI resize impossible : {exc}")
...
def v63_load_model():
    ...
    try:
        ...
        print(f"[v78] lecture modèle strict impossible : {exc}")
        return None
...
def v63_generate_safe_role_ai(self, event=None):
    ...
    print("")
    print("[v78] SAFE ROLE AI GENERATED")
    print("[v78] model strict:", "yes" if model else "no")
    print("[v78] snare main/alt:", main_snare, alt_snare)
    print("[v78] kick  main/alt:", main_kick, alt_kick)
    print("[v78] hat   main/alt:", main_hat, alt_hat)
    print("[v78] pattern:")
    for item in pattern:
        print(
            f"[v78] step {item['x_step']:02d}/{item['x_step'] + item['length'] - 1:02d} | "
...
def v63_run_strict_training(self, event=None):
    ...
    print("")
    print("[v78] TRAIN STRICT ROLE MODEL...")
    print("")
    ...
    try:
        ...
        print(f"[v78] training strict impossible : {exc}")
...
def v63_build_ui(self):
    ...
    try:
        ...
        print(f"[v78] UI safe impossible : {exc}")
...
def v65_rank_slices(self):
    ...
    for pair in self.pair_values:
        ...
        try:
            ...
            print(f"[v78] analyse slice {pair} impossible : {exc}")
            scores = {
                "kick": 0.0,
                "snare": 0.0,
                "hat": 0.0,
    ...
    print("")
    print("[v78] TOP SLICES PAR ROLE — break courant")
    for role in ("kick", "snare", "hat", "ghost_snare"):
        print(f"[v78] {role:12s}:", [(x["pair"], round(x["score"], 3)) for x in ranks[role][:8]])
    print("")
...
def v65_generate_from_role_template(self, event=None):
    ...
    try:
        ...
        print(f"[v78] template introuvable : {exc}")
        return "break"
    ...
    print("")
    print("[v78] CROSS-SONG ROLE TEMPLATE GENERATED")
    print("[v78] template:", V65_TEMPLATE_PATH)
    print("[v78] temperature:", temperature, "| hats:", hat_density, "| ghosts:", ghost_density)
    print("[v78] pattern:")
    for item in pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v78] step {item['x_step']:02d}/{end:02d} | "
...
def v67_train_strict_if_available(self):
    ...
    if trainer is None:
        print("[v78] aucun trainer trouvé.")
        return False

    print("")
    print("[v78] TRAIN après validation :", trainer)
    print("")
    ...
    try:
        ...
        print(f"[v78] training impossible : {exc}")
        return False
...
def v67_generate_candidate(self, event=None):
    ...
    print("")
    print("[v78] CANDIDATE GENERATED")
    print("[v78] break:", self.v67_candidate_safe)
    print("[v78] notes:", len(candidate))
    print("[v78] Maintenant : corrige puis clique Good après modifs, ou Reject.")
    print("")
...
def v67_accept_corrected(self, event=None):
    ...
    print("")
    print("[v78] ACCEPTED GOOD AFTER EDIT")
    print("[v78] saved:", validated_path)
    print("[v78] trained:", trained)
    print("")
...
def v67_accept_without_training(self, event=None):
    ...
    print("[v78] accepted no-train:", validated_path)
...
def v67_build_ui(self):
    ...
    try:
        ...
        print(f"[v78] UI feedback impossible : {exc}")
...
def v68_generate_candidate(self, event=None):
    ...
    print("[v78] Length policy appliquée : 2 cases par défaut, 1 case rare.")
...
def v69_force_pattern_hard_2_grid(self, reason="v69_force_hard_2_grid"):
    ...
    print("")
    print("[v78] HARD 2 GRID APPLIQUÉ")
    for item in self.pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v78] step {item['x_step']:02d}/{end:02d} | "
...
def v70_duplicate_selected_forward(self, event=None):
    ...
    print("")
    print("[v78] CTRL+D DUPLICATE +2")
    print(f"[v78] old_id={item.get('id')} new_id={new_item.get('id')}")
    print(f"[v78] step {old_x}/{old_x + length - 1} -> {new_x}/{new_x + length - 1}")
    print(f"[v78] pair={new_item.get('pair')} len={length}")
    print("")
...
def v70_build_ui(self):
    ...
    try:
        ...
        print(f"[v78] bind Ctrl+D impossible : {exc}")
...
def v71_rank_slices(self):
    ...
    print("")
    print("[v78] PRE-LEARNING TOP SLICES")
    for role in ("kick", "snare", "hat", "ghost_snare"):
        print(f"[v78] {role:12s}:", [(x["pair"], round(x["score"], 3)) for x in ranks[role][:8]])
    print("")
...
def v71_prelearn_roles(self, force=False):
    ...
    print("")
    print("[v78] PRELEARN ROLES POUR", safe)
    for role in ("kick", "snare", "hat", "ghost_snare", "bad"):
        print(f"[v78] {role:12s}:", roles.get(role, []))
    print("")
...
def v71_generate_full_candidate(self, event=None):
    ...
    print("")
    print("[v78] FULL CANDIDATE GENERATED")
    print("[v78] break:", v71_safe(self))
    print("[v78] roles:", roles)
    print("[v78] pattern:")
    for item in pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
...
def v72_open_break(self, safe=None):
    ...
    print("")
    print("[v78] OPEN BREAK")
    print("[v78] current:", current)
    print("[v78] next   :", safe)
    print("[v78] cmd    :", " ".join(cmd))
    print("")
...
def v72_build_ui(self):
    ...
    try:
        ...
        print(f"[v78] UI break switcher impossible : {exc}")
...
def v73_sanitize_tracker_pattern(self, reason="v73_tracker_strict_no_overlap"):
    ...
    print("")
    print("[v78] TRACKER STRICT NO OVERLAP")
    print("[v78] kept:", len(accepted), "| rejected overlaps:", len(rejected))

    for item in accepted:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v78] KEEP step {item['x_step']:02d}/{end:02d} | "
    ...
    if rejected:
        ...
        for item in rejected:
            ...
            print(
                f"[v78] DROP step {item['x_step']:02d}/{end:02d} | "
                f"role={item.get('v73_role_key')} | pair={item.get('pair')}"
            )
...
def v74_launch_break(self, safe, auto_generate=False):
    ...
    print("")
    print("[v78] LAUNCH BREAK")
    print("[v78] current:", v74_current_safe(self))
    print("[v78] selected:", safe)
    print("[v78] auto_generate:", auto_generate)
    print("[v78] cmd:", " ".join(cmd))
    print("")
...
def v74_open_selected_and_generate(self):
    ...
    if selected == current:
        print("[v78] selected == current, génération directe sur", current)
...
def v74_generate_current(self):
    ...
    print("")
    print("[v78] GENERATE CURRENT")
    print("[v78] break:", v74_current_safe(self))
    print("")
...
def v74_autogenerate_if_requested(self):
    ...
    print("")
    print("[v78] AUTO GENERATE ON OPEN")
    print("[v78] break:", v74_current_safe(self))
    print("")
...
def v75_generate_current_only(self, event=None):
    ...
    print("")
    print("[v78] GENERATE CURRENT ONLY")
    print("[v78] break courant:", v75_current_safe(self))
    print("")
...
def v75_launch_break_and_generate(self, safe):
    ...
    print("")
    print("[v78] RELANCE SUR BREAK SÉLECTIONNÉ")
    print("[v78] break courant    :", v75_current_safe(self))
    print("[v78] break sélectionné:", safe)
    print("[v78] autogen          : oui")
    print("[v78] cmd              :", " ".join(cmd))
    print("")
...
def v75_generate_selected_break(self, event=None):
    ...
    print("")
    print("[v78] GENERATE SELECTED BREAK")
    print("[v78] current :", current)
    print("[v78] selected:", selected)
    print("")
...
def v75_autogen_after_open(self):
    ...
    print("")
    print("[v78] AUTO-GENERATE APRÈS OUVERTURE")
    print("[v78] break:", v75_current_safe(self))
    print("")
...
def v76_launch_break(self, safe, autogen=False):
    ...
    print("")
    print("[v78] LAUNCH BREAK")
    print("[v78] current:", v76_current_safe(self))
    print("[v78] target :", safe)
    print("[v78] autogen:", autogen)
    print("[v78] cmd    :", " ".join(cmd))
    print("")
...
def v76_generate_current_break(self, event=None):
    ...
    print("")
    print("[v78] GENERATE CURRENT BREAK")
    print("[v78] current:", v76_current_safe(self))
    print("[v78] generator:", getattr(fn, "__name__", None))
    print("")
...
def v77_generate_current(self, event=None):
    ...
    print("")
    print("[v78] GENERATE CURRENT")
    print("[v78] break courant:", v77_current_safe(self))
    print("[v78] générateur   :", getattr(fn, "__name__", None))
    print("")
...
def v77_launch_source(self, source, autogen=False, open_library=False):
    ...
    print("")
    print("[v78] LAUNCH SOURCE")
    print("[v78] courant :", v77_current_safe(self))
    print("[v78] target  :", source)
    print("[v78] autogen :", autogen)
    print("[v78] cmd     :", " ".join(cmd))
    print("")
...
def v77_open_library(self):
    ...
    win = tk.Toplevel(self.root)
    win.title("BreakbeatAI v78 — scrollable break selector")
...
def v77_autogen_if_requested(self):
    ...
    print("")
    print("[v78] AUTO-GENERATE AU DÉMARRAGE")
    print("[v78] break:", v77_current_safe(self))
    print("")
...
def v77_build_ui(self):
    ...
    try:
        ...
        print(f"[v78] barre UI impossible : {exc}")
...
# ---------------------------------------------------------------------
# v78 EXTENSION : sélecteur de breaks scrollable dans l'UI principale
# ---------------------------------------------------------------------

V78_PAIR_DIR = Path("dataset/pair_blocks_v02")
V78_AUTOGEN_ENV = "BREAKBEATAI_V78_AUTOGEN"


def v78_status(self, text):
    try:
        if hasattr(self, "set_status"):
            self.set_status(text)
        else:
...
def v78_source_from_pairblock(path):
    name = path.name
    if name.endswith("_pair_blocks_v02.json"):
        name = name[:-len("_pair_blocks_v02.json")]
    return name
...
def v78_list_breaks():
    if not V78_PAIR_DIR.exists():
        return []

    out = []
    seen = set()

    for path in sorted(V78_PAIR_DIR.glob("*_pair_blocks_v02.json")):
        source = v78_source_from_pairblock(path)
        if not source:
            continue
...
def v78_selected_break(self):
    try:
        indices = self.v78_break_listbox.curselection()
        if indices:
            return self.v78_break_listbox.get(indices[0]).strip()
    except Exception:
        pass

    try:
        return self.v78_selected_var.get().strip()
    except Exception:
        return v78_current_safe(self)


def v78_find_real_generator():
    """
    Générateur interne réel :
    - v73 = tracker strict/no-overlap
    - v71 = full candidate + prelearn
    - fallback anciens générateurs propres
    """
    for name in [
        "v73_generate_tracker_strict",
        "v71_generate_full_candidate",
...
def v78_generate_current(self, event=None):
    fn = v78_find_real_generator()

    print("")
    print("[v78] GENERATE CURRENT")
    print("[v78] break courant:", v78_current_safe(self))
    print("[v78] générateur   :", getattr(fn, "__name__", None))
    print("")

    if fn is None:
        v78_status(self, "Aucun vrai générateur trouvé.")
        return "break"
    ...
    try:
        self.v67_candidate_before_edit = json.loads(json.dumps(self.pattern, ensure_ascii=False))
        self.v67_candidate_time = time.strftime("%Y-%m-%d %H:%M:%S")
        self.v67_candidate_safe = v78_current_safe(self)
    except Exception:
        pass

    v78_status(self, f"Generate Candidate OK sur {v78_current_safe(self)}.")
    return result


def v78_launch_break(self, source, autogen=False):
    source = str(source).strip()

    if not source:
        v78_status(self, "Aucun break sélectionné.")
        return "break"
    ...
    if autogen:
        env[V78_AUTOGEN_ENV] = "1"
    else:
        env.pop(V78_AUTOGEN_ENV, None)
    ...
    print("")
    print("[v78] LAUNCH BREAK")
    print("[v78] courant :", v78_current_safe(self))
    print("[v78] target  :", source)
    print("[v78] autogen :", autogen)
    print("[v78] cmd     :", " ".join(cmd))
    print("")
...
def v78_open_selected(self, event=None):
    source = v78_selected_break(self)
    current = v78_current_safe(self)
    ...
    if source.lower() == current.lower():
        v78_status(self, f"Déjà sur {current}.")
        return "break"

    return v78_launch_break(self, source, autogen=False)


def v78_generate_selected(self, event=None):
    source = v78_selected_break(self)
    current = v78_current_safe(self)
    ...
    print("")
    print("[v78] GENERATE SELECTED")
    print("[v78] courant :", current)
    print("[v78] selected:", source)
    print("")

    if source.lower() == current.lower():
        return v78_generate_current(self, event)

    return v78_launch_break(self, source, autogen=True)


def v78_refresh_break_list(self):
    breaks = v78_list_breaks()
    current = v78_current_safe(self)

    try:
        self.v78_break_listbox.delete(0, "end")

        for source in breaks:
            self.v78_break_listbox.insert("end", source)

        # sélectionne le break courant si présent
        for i, source in enumerate(breaks):
            if source.lower() == current.lower():
                self.v78_break_listbox.selection_set(i)
                self.v78_break_listbox.see(i)
                self.v78_selected_var.set(source)
                break

    except Exception as exc:
        print(f"[v78] refresh list impossible : {exc}")

    v78_status(self, f"{len(breaks)} breaks chargés dans la liste scrollable.")
    return "break"


def v78_on_select(self, event=None):
    source = v78_selected_break(self)
    ...
    v78_status(self, f"Break sélectionné : {source}")
    return "break"


def v78_mousewheel_listbox(self, event):
    try:
        if event.num == 4:
            self.v78_break_listbox.yview_scroll(-3, "units")
        elif event.num == 5:
            self.v78_break_listbox.yview_scroll(3, "units")
        else:
            delta = int(-1 * (event.delta / 120))
            self.v78_break_listbox.yview_scroll(delta, "units")
    except Exception:
        pass
...
def v78_autogen_if_requested(self):
    if getattr(self, "_v78_autogen_done", False):
        return

    if os.environ.get(V78_AUTOGEN_ENV) != "1":
        return

    self._v78_autogen_done = True

    print("")
    print("[v78] AUTO-GENERATE AU DÉMARRAGE")
    print("[v78] break:", v78_current_safe(self))
    print("")

    try:
        self.root.after(800, lambda: v78_generate_current(self))
    except Exception:
        v78_generate_current(self)


_old_v78_build_ui = SliceIndexTracker.build_ui


def v78_build_ui(self):
    _old_v78_build_ui(self)

    current = v78_current_safe(self)

    try:
        # Agrandit un peu la fenêtre pour laisser de la place au sélecteur.
        try:
        ...
        panel = tk.Frame(self.root, bg="#101820", bd=2, relief="groove")
        panel.pack(fill="x", padx=14, pady=(0, 10))

        top = tk.Frame(panel, bg="#101820")
        top.pack(fill="x", padx=8, pady=(8, 4))

        tk.Label(
            top,
            text="Break selector scrollable",
            bg="#101820",
            fg="#f5d67b",
            font=("Sans", 11, "bold"),
        ).pack(side="left", padx=4)

        self.v78_selected_var = tk.StringVar(value=current)
        ...
        tk.Label(
            top,
            textvariable=self.v78_selected_var,
            bg="#182330",
            fg="#f5eefe",
            width=46,
            anchor="w",
        ...
        tk.Button(
            top,
            text="Open Selected",
            command=self.open_selected_break_v78,
            bg="#30384a",
            fg="#f5eefe",
        ).pack(side="left", padx=4)
        ...
        tk.Button(
            top,
            text="Generate Current",
            command=self.generate_current_v78,
            bg="#30513f",
            fg="#f5eefe",
        ).pack(side="left", padx=4)
        ...
        tk.Button(
            top,
            text="Refresh",
            command=self.refresh_break_list_v78,
            bg="#303030",
            fg="#eeeeee",
        ).pack(side="left", padx=4)

        list_frame = tk.Frame(panel, bg="#101820")
        list_frame.pack(fill="x", padx=8, pady=(4, 8))

        scrollbar = tk.Scrollbar(list_frame, orient="vertical")
        scrollbar.pack(side="right", fill="y")

        self.v78_break_listbox = tk.Listbox(
            list_frame,
            height=9,
            width=72,
            ...
            bg="#182330",
            fg="#f5eefe",
            selectbackground="#6b3d67",
            selectforeground="#ffffff",
            activestyle="none",
            yscrollcommand=scrollbar.set,
        )
        self.v78_break_listbox.pack(side="left", fill="x", expand=True)

        scrollbar.config(command=self.v78_break_listbox.yview)

        self.v78_break_listbox.bind("<<ListboxSelect>>", lambda e: v78_on_select(self, e))
        self.v78_break_listbox.bind("<Double-Button-1>", lambda e: v78_generate_selected(self, e))
        self.v78_break_listbox.bind("<MouseWheel>", lambda e: v78_mousewheel_listbox(self, e))
        self.v78_break_listbox.bind("<Button-4>", lambda e: v78_mousewheel_listbox(self, e))
        self.v78_break_listbox.bind("<Button-5>", lambda e: v78_mousewheel_listbox(self, e))

        hint = tk.Label(
            panel,
            text="Scroll dans la liste, clique un break, puis Generate Selected. Double-clic = Generate Selected.",
            bg="#101820",
            fg="#b9acc8",
        )
        hint.pack(fill="x", padx=8, pady=(0, 8))

        v78_refresh_break_list(self)

    except Exception as exc:
        print(f"[v78] UI selector impossible : {exc}")

    v78_status(
        self,
        f"v78 : sélection scrollable dans l'UI. Break courant = {current}."
    )

    v78_autogen_if_requested(self)


SliceIndexTracker.generate_current_v78 = v78_generate_current
SliceIndexTracker.open_selected_break_v78 = v78_open_selected
SliceIndexTracker.refresh_break_list_v78 = v78_refresh_break_list

# L'ancien bouton Generate Candidate utilise maintenant le break sélectionné dans la liste.
SliceIndexTracker.generate_candidate = v78_generate_selected
SliceIndexTracker.generate_full_candidate = v78_generate_selected
SliceIndexTracker.generate_ai_pattern = v78_generate_selected
SliceIndexTracker.generate_cross_song_template = v78_generate_selected
SliceIndexTracker.generate_safe_experiment = v78_generate_selected
SliceIndexTracker.generate_locked_roles = v78_generate_selected
SliceIndexTracker.generate_role_aware = v78_generate_selected
SliceIndexTracker.generate_safe_role_ai = v78_generate_selected

SliceIndexTracker.build_ui = v78_build_ui
