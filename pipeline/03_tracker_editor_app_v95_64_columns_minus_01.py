...
"""
BreakbeatAI Tracker Editor v95 — slice index
...
def warp_audio_to_length_external(audio, target_len, cache_key=None):
    ...
    if rubberband:
        try:
            ...
            print(f"[v95] rubberband échec, fallback ffmpeg/no-speed : {exc}")
    ...
    if ffmpeg:
        try:
            ...
            print(f"[v95] ffmpeg atempo échec, fallback no-speed : {exc}")
...
class SliceIndexTracker:
    def __init__(self, root, pair_json):
        ...
        self.root.title("BreakbeatAI Tracker Editor v95 — slice index")
    ...
    def detect_external_player(self):
        for name in ["pw-play", "paplay", "aplay", "ffplay"]:
            ...
            if path:
                print(f"[v95] Backend audio : {name} -> {path}")
                return name

        print("[v95] Aucun backend système trouvé, fallback sounddevice.")
        return None
    ...
    def build_initial_pattern(self, randomize=False):
        ...
        print(
            f"[v95] Génération pattern | break={self.project['safe']} | "
        ...
        for slot in range(max_items):
            ...
            print(f"[v95] slot {slot:02d} | step {x_step:02d} -> slice {pair:02d}")
    ...
    def build_ui(self):
        ...
        title = tk.Label(
            main,
            text="BreakbeatAI v95 — 64 columns minus 0.1",
            bg="#111018",
            fg="#ff7acc",
            font=("Sans", 20, "bold"),
        )
    ...
    def record_correction(self, event_type, before=None, after=None):
        ...
        print(f"[v95] correction sauvegardée : {event_type}")
    ...
    def play_audition_array(self, audio, label="audition"):
        ...
        audition_wav = OUT_DIR / f"{self.project['safe']}_v95_audition.wav"
        ...
        if self.external_player:
            try:
                ...
                print(f"[v95] Erreur audition {self.external_player} : {exc}")
    ...
    def on_close(self):
        ...
        """
        print("[v95] Fermeture : arrêt audio + destruction fenêtre")
    ...
    def play_audio_array(self, audio, label="audio"):
        ...
        audio = normalize(audio)
        live_wav = OUT_DIR / f"{self.project['safe']}_v95_live.wav"
        ...
        if self.external_player:
            try:
                ...
                print(f"[v95] Erreur backend {self.external_player} : {exc}")
    ...
    def render_preview_file(self):
        OUT_DIR.mkdir(parents=True, exist_ok=True)
        wav = OUT_DIR / f"{self.project['safe']}_tracker_app_v95_preview.wav"
    ...
    def write_latest_pattern(self, reason="correction"):
        ...
        path = LATEST_DIR / f"{self.project['safe']}_latest_slice_index_v95.json"

        data = {
            "version": "breakbeatai_latest_slice_index_v95",
            "reason": reason,
    ...
    def save(self):
        ...
        tracker_path = OUT_DIR / f"{self.project['safe']}_tracker_app_edit_v95.json"
        ...
        stamp = file_stamp()
        validated_path = VALIDATED_DIR / f"{self.project['safe']}_validated_slice_index_loop64_v95_{stamp}.json"

        validated = {
            "version": "breakbeatai_validated_slice_index_loop64_v95",
...
def v44_record_correction(self, event_type, before=None, after=None):
    ...
    print(f"[v95] correction sauvegardée : {event_type}")
    print(f"[v95] correction path : {corrections_path}")
    print(f"[v95] latest path : {latest_path}")
...
def v45_move_selected(self, dx, dy):
    ...
    if dy != 0 and before != after:
        try:
            ...
            print(f"[v95] audition après flèche haut/bas impossible : {exc}")
...
def v46_warp_loop_pitch_preserve(audio, target_len, cache_key):
    ...
    if rubberband:
        try:
            ...
            print(f"[v95] rubberband global warp échec : {exc}")
    ...
    if ffmpeg:
        try:
            ...
            print(f"[v95] ffmpeg global warp échec : {exc}")

    print("[v95] Aucun warp pitch-preserve dispo, fallback fit no-speed.")
    return v46_fit_audio_no_speed(audio, target_len)
...
def v46_build_initial_pattern(self, randomize=False):
    ...
    print(
        f"[v95] M8 pattern | break={self.project['safe']} | "
...
def v46_get_warped_loop(self):
    ...
    print(
        f"[v95] global warp OK | source_len={loop_end-loop_start} | "
...
def v46_build_ui(self):
    ...
    try:
        ...
        print(f"[v95] UI extension impossible : {exc}")
...
def v47_play_audio_array(self, audio, label="audio"):
    ...
    if len(audio) == 0 or v47_is_silent(audio):
        ...
        print(f"[v95] lecture annulée : audio silencieux ({label})")
        return 0
...
def v47_toggle_loop(self):
    ...
    if not getattr(self, "pattern", None):
        ...
        print("[v95] loop annulée : pattern vide")
        return "break"
...
def v47_delete_selected(self):
    ...
    if not getattr(self, "pattern", None):
        ...
        print("[v95] pattern vide après suppression : audio coupé")
...
def v48_build_initial_pattern(self, randomize=False):
    ...
    print("[v95] pattern initial forcé VIDE : aucun son ne doit jouer.")
    return []
...
def v48_load_first_slices(self):
    ...
    print(f"[v95] Load 16 slices : {len(self.pattern)} items")


def v48_debug_pattern(self, label="render"):
    print("")
    print(f"[v95] DEBUG PATTERN avant {label}")
    print(f"[v95] items = {len(getattr(self, 'pattern', []) or [])}")

    if not getattr(self, "pattern", None):
        print("[v95] pattern vide confirmé")
        print("")
        return
    ...
    for item in self.pattern:
        ...
        print(
            f"[v95] id={item_id} | step {x:02d}/{end:02d} | "
...
def v48_render_audio_with_timeline(self):
    ...
    if not getattr(self, "pattern", None):
        loop_samples = self.get_loop_samples()
        print(f"[v95] rendu silence total : pattern vide | samples={loop_samples}")
        return np.zeros(loop_samples, dtype=np.float32), []
    ...
    print(f"[v95] rendu audio peak={peak:.8f} | timeline_events={len(timeline) if timeline is not None else 'None'}")

    if timeline:
        print("[v95] timeline events:")
        for ev in timeline:
            print(f"[v95]   {ev}")
...
def v48_toggle_loop(self):
    if not getattr(self, "pattern", None):
        ...
        print("[v95] Space ignoré : pattern vide.")
        return "break"
...
def v48_build_ui(self):
    ...
    try:
        ...
        print(f"[v95] ajout UI impossible : {exc}")
...
def v49_add_item_at_xy(self, x, y):
    ...
    try:
        ...
        print(f"[v95] record_correction add impossible : {exc}")
    ...
    try:
        ...
        print(f"[v95] audition après ajout impossible : {exc}")

    print(f"[v95] add | id={item['id']} | step={x_step} | pair={pair} | lane={lane}")
...
def v49_duplicate_selected_forward(self, event=None):
    ...
    if item is None:
        ...
        print("[v95] duplicate annulé : aucune note sélectionnée")
        return "break"
    ...
    if new_x > max_x:
        ...
        print("[v95] duplicate annulé : fin de grille")
        return "break"
    ...
    try:
        ...
        print(f"[v95] record_correction duplicate impossible : {exc}")
    ...
    try:
        ...
        print(f"[v95] audition après duplicate impossible : {exc}")

    print(
        f"[v95] duplicate | old_id={item.get('id')} | new_id={new_item.get('id')} | "
...
def v49_build_ui(self):
    ...
    try:
        ...
        print(f"[v95] bind canvas click impossible : {exc}")
    ...
    try:
        ...
        print(f"[v95] bind keys impossible : {exc}")
...
def v50_load_style_model():
    ...
    try:
        ...
        print(f"[v95] impossible de charger le modèle IA : {exc}")
        return None
...
def v50_generate_ai_pattern(self, event=None):
    ...
    if model is None:
        ...
        print("[v95] modèle absent :", STYLE_MODEL_PATH)
        return "break"
    ...
    print("")
    print(f"[v95] GENERATE IA | safe={safe} | pairs={pair_values} | slots={max_slots}")
    ...
    for slot in range(max_slots):
        ...
        print(
            f"[v95] slot {slot:02d} | step {x_step:02d} | "
    ...
    try:
        ...
        print(f"[v95] write_latest_pattern impossible : {exc}")
...
def v50_build_ui(self):
    ...
    try:
        ...
        print(f"[v95] UI IA impossible : {exc}")
...
def v51_mouse_down(self, event):
    ...
    if item is not None:
        ...
        print(
            f"[v95] mouse down note | id={item.get('id')} | "
            f"step={item.get('x_step')} | pair={item.get('pair')}"
        )
    else:
        print("[v95] mouse down empty")
...
def v51_mouse_up(self, event):
    ...
    if state.get("empty"):
        ...
        print("[v95] drag depuis vide ignoré")
        return "break"
    ...
    if before != after:
        try:
            ...
            print(f"[v95] record_correction drag impossible : {exc}")
        ...
        print(
            f"[v95] drag move | id={after.get('id')} | "
            f"step {before.get('x_step')} -> {after.get('x_step')} | "
        ...
        try:
            ...
            print(f"[v95] audition après drag impossible : {exc}")
...
def v51_build_ui(self):
    ...
    try:
        ...
        print(f"[v95] bind souris impossible : {exc}")
    ...
    try:
        ...
        print(f"[v95] bind Suppr impossible : {exc}")
...
def v52_normalize_loaded_notes(self, notes):
    ...
    for raw in sorted(notes, key=lambda n: (int(float(n.get("x_step", 0))), int(float(n.get("pair", 0))))):
        ...
        if pair not in valid_pairs:
            print(f"[v95] slice ignorée car absente du break actuel : pair={pair}")
            continue
...
def v52_load_saved_beat(self, event=None):
    ...
    if not saves:
        ...
        print("[v95] aucune sauvegarde trouvée")
        return "break"
    ...
    if not pattern:
        ...
        print("[v95] sauvegarde incompatible :", chosen["path"])
        return "break"
    ...
    print("")
    print("[v95] BEAT RESTAURÉ")
    print("[v95] fichier :", chosen["path"])
    print("[v95] clé :", chosen["key"])
    print("[v95] notes :", len(pattern))
    print("")
...
def v52_list_saved_beats(self, event=None):
    ...
    print("")
    print("[v95] SAUVEGARDES TROUVÉES")
    for i, save in enumerate(saves[:30]):
        print(
            f"[v95] {i:02d} | notes={len(save['notes']):02d} | "
...
def v52_build_ui(self):
    ...
    try:
        ...
        print(f"[v95] UI load saved impossible : {exc}")
...
def v53_run_training(self):
    ...
    if not trainer.exists():
        ...
        print("[v95] trainer introuvable :", trainer)
        return False

    print("")
    print("[v95] AUTO-TRAIN IA après Save...")
    print("")
...
def v53_save(self):
    ...
    try:
        ...
        print(f"[v95] auto-train après save impossible : {exc}")
...
def v53_build_ui(self):
    ...
    try:
        ...
        print(f"[v95] UI auto-train impossible : {exc}")
...
def v55_mouse_up(self, event):
    ...
    if not state.get("moved"):
        ...
        try:
            ...
            print(f"[v95] audition clic note impossible : {exc}")
        ...
        print(
            f"[v95] click play/select | id={item.get('id')} | "
            f"step={item.get('x_step')} | pair={item.get('pair')}"
        )
...
def v58_add_item_at_xy(self, x, y):
    ...
    print(
        f"[v95] add | id={item['id']} | step={x_step} | "
...
def v58_toggle_selected_length(self, event=None):
    ...
    try:
        ...
        print(f"[v95] record_correction length impossible : {exc}")

    v58_refresh_after_edit(self, "toggle_note_length")

    v58_status(self, f"Longueur note : {old_len} -> {new_len} case(s).")
    print(f"[v95] length toggle | id={item.get('id')} | {old_len} -> {new_len}")
...
def v58_duplicate_selected_forward(self, event=None):
    ...
    try:
        ...
        print(f"[v95] audition duplicate impossible : {exc}")
...
def v58_build_ui(self):
    ...
    try:
        ...
        print(f"[v95] UI demi-grille impossible : {exc}")
    ...
    try:
        ...
        print(f"[v95] bind demi-grille impossible : {exc}")
...
def v59_rank_current_slices(self):
    ...
    for pair in self.pair_values:
        ...
        try:
            ...
            print(f"[v95] analyse pair {pair} impossible : {exc}")
            role = "other"
    ...
    print("")
    print("[v95] TOP ROLES CURRENT BREAK")
    for r in ("kick", "snare", "hat"):
        print(f"[v95] {r:5s}:", [(x["pair"], round(x["score"], 3)) for x in ranks[r][:6]])
    print("")
...
def v59_load_role_model():
    ...
    try:
        ...
        print(f"[v95] lecture modèle rôle impossible : {exc}")
        return None
...
def v59_generate_role_aware(self, event=None):
    ...
    if model is None:
        ...
        print("[v95] modèle rôle absent, fallback musical snare 6.")
        model = {
            "breaks": {},
            "global": {},
            "patterns_used": 0,
    ...
    print("")
    print("[v95] ROLE-AWARE GENERATION")
    print("[v95] model patterns_used:", model.get("patterns_used", 0))
    for item in pattern:
        print(
            f"[v95] step {item['x_step']:02d}/{item['x_step'] + item['length'] - 1:02d} | "
...
def v59_run_role_training(self, event=None):
    ...
    try:
        ...
        print(f"[v95] training rôle impossible : {exc}")
...
def v59_save(self):
    ...
    try:
        ...
        print(f"[v95] auto-training rôle après save impossible : {exc}")
...
def v59_build_ui(self):
    ...
    try:
        ...
        print(f"[v95] UI role-aware impossible : {exc}")
...
def v62_add_item_at_xy(self, x, y):
    ...
    print(f"[v95] add | id={item['id']} | step={x_step} | len={length} | pair={pair}")
...
def v62_mouse_down(self, event):
    ...
    if item is not None:
        ...
        print(
            f"[v95] mouse down {state['mode']} | id={item.get('id')} | "
            f"step={item.get('x_step')} | len={item.get('length')} | pair={item.get('pair')}"
        )
    else:
        print("[v95] mouse down empty")
    ...
    if mode == "resize":
        if not state.get("moved"):
            ...
            try:
                ...
                print(f"[v95] audition clic resize impossible : {exc}")
        ...
        if before != after:
            try:
                ...
                print(f"[v95] record_correction resize impossible : {exc}")
            ...
            try:
                ...
                print(f"[v95] audition après resize impossible : {exc}")
    ...
    if mode == "move":
        ...
        if before != after:
            try:
                ...
                print(f"[v95] record_correction move impossible : {exc}")
            ...
            try:
                ...
                print(f"[v95] audition après move impossible : {exc}")
...
def v62_toggle_selected_length(self, event=None):
    ...
    try:
        ...
        print(f"[v95] record_correction toggle length impossible : {exc}")

    v62_refresh_after_edit(self, "toggle_note_length_v62")
    v62_status(self, f"Longueur note : {old_len} → {new_len} case(s).")

    print(f"[v95] toggle length | id={item.get('id')} | {old_len} -> {new_len}")
...
def v62_draw(self):
    ...
    try:
        ...
        print(f"[v95] draw handles impossible : {exc}")
...
def v62_build_ui(self):
    ...
    try:
        ...
        print(f"[v95] bind resize impossible : {exc}")
    ...
    try:
        ...
        print(f"[v95] UI resize impossible : {exc}")
...
def v63_load_model():
    ...
    try:
        ...
        print(f"[v95] lecture modèle strict impossible : {exc}")
        return None
...
def v63_generate_safe_role_ai(self, event=None):
    ...
    print("")
    print("[v95] SAFE ROLE AI GENERATED")
    print("[v95] model strict:", "yes" if model else "no")
    print("[v95] snare main/alt:", main_snare, alt_snare)
    print("[v95] kick  main/alt:", main_kick, alt_kick)
    print("[v95] hat   main/alt:", main_hat, alt_hat)
    print("[v95] pattern:")
    for item in pattern:
        print(
            f"[v95] step {item['x_step']:02d}/{item['x_step'] + item['length'] - 1:02d} | "
...
def v63_run_strict_training(self, event=None):
    ...
    print("")
    print("[v95] TRAIN STRICT ROLE MODEL...")
    print("")
    ...
    try:
        ...
        print(f"[v95] training strict impossible : {exc}")
...
def v63_build_ui(self):
    ...
    try:
        ...
        print(f"[v95] UI safe impossible : {exc}")
...
def v65_rank_slices(self):
    ...
    for pair in self.pair_values:
        ...
        try:
            ...
            print(f"[v95] analyse slice {pair} impossible : {exc}")
            scores = {
                "kick": 0.0,
                "snare": 0.0,
                "hat": 0.0,
    ...
    print("")
    print("[v95] TOP SLICES PAR ROLE — break courant")
    for role in ("kick", "snare", "hat", "ghost_snare"):
        print(f"[v95] {role:12s}:", [(x["pair"], round(x["score"], 3)) for x in ranks[role][:8]])
    print("")
...
def v65_generate_from_role_template(self, event=None):
    ...
    try:
        ...
        print(f"[v95] template introuvable : {exc}")
        return "break"
    ...
    print("")
    print("[v95] CROSS-SONG ROLE TEMPLATE GENERATED")
    print("[v95] template:", V65_TEMPLATE_PATH)
    print("[v95] temperature:", temperature, "| hats:", hat_density, "| ghosts:", ghost_density)
    print("[v95] pattern:")
    for item in pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v95] step {item['x_step']:02d}/{end:02d} | "
...
def v67_train_strict_if_available(self):
    ...
    if trainer is None:
        print("[v95] aucun trainer trouvé.")
        return False

    print("")
    print("[v95] TRAIN après validation :", trainer)
    print("")
    ...
    try:
        ...
        print(f"[v95] training impossible : {exc}")
        return False
...
def v67_generate_candidate(self, event=None):
    ...
    print("")
    print("[v95] CANDIDATE GENERATED")
    print("[v95] break:", self.v67_candidate_safe)
    print("[v95] notes:", len(candidate))
    print("[v95] Maintenant : corrige puis clique Good après modifs, ou Reject.")
    print("")
...
def v67_accept_corrected(self, event=None):
    ...
    print("")
    print("[v95] ACCEPTED GOOD AFTER EDIT")
    print("[v95] saved:", validated_path)
    print("[v95] trained:", trained)
    print("")
...
def v67_accept_without_training(self, event=None):
    ...
    print("[v95] accepted no-train:", validated_path)
...
def v67_build_ui(self):
    ...
    try:
        ...
        print(f"[v95] UI feedback impossible : {exc}")
...
def v68_generate_candidate(self, event=None):
    ...
    print("[v95] Length policy appliquée : 2 cases par défaut, 1 case rare.")
...
def v69_force_pattern_hard_2_grid(self, reason="v69_force_hard_2_grid"):
    ...
    print("")
    print("[v95] HARD 2 GRID APPLIQUÉ")
    for item in self.pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v95] step {item['x_step']:02d}/{end:02d} | "
...
def v70_duplicate_selected_forward(self, event=None):
    ...
    print("")
    print("[v95] CTRL+D DUPLICATE +2")
    print(f"[v95] old_id={item.get('id')} new_id={new_item.get('id')}")
    print(f"[v95] step {old_x}/{old_x + length - 1} -> {new_x}/{new_x + length - 1}")
    print(f"[v95] pair={new_item.get('pair')} len={length}")
    print("")
...
def v70_build_ui(self):
    ...
    try:
        ...
        print(f"[v95] bind Ctrl+D impossible : {exc}")
...
def v71_rank_slices(self):
    ...
    print("")
    print("[v95] PRE-LEARNING TOP SLICES")
    for role in ("kick", "snare", "hat", "ghost_snare"):
        print(f"[v95] {role:12s}:", [(x["pair"], round(x["score"], 3)) for x in ranks[role][:8]])
    print("")
...
def v71_prelearn_roles(self, force=False):
    ...
    print("")
    print("[v95] PRELEARN ROLES POUR", safe)
    for role in ("kick", "snare", "hat", "ghost_snare", "bad"):
        print(f"[v95] {role:12s}:", roles.get(role, []))
    print("")
...
def v71_generate_full_candidate(self, event=None):
    ...
    print("")
    print("[v95] FULL CANDIDATE GENERATED")
    print("[v95] break:", v71_safe(self))
    print("[v95] roles:", roles)
    print("[v95] pattern:")
    for item in pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
...
def v72_open_break(self, safe=None):
    ...
    print("")
    print("[v95] OPEN BREAK")
    print("[v95] current:", current)
    print("[v95] next   :", safe)
    print("[v95] cmd    :", " ".join(cmd))
    print("")
...
def v72_build_ui(self):
    ...
    try:
        ...
        print(f"[v95] UI break switcher impossible : {exc}")
...
def v73_sanitize_tracker_pattern(self, reason="v73_tracker_strict_no_overlap"):
    ...
    print("")
    print("[v95] TRACKER STRICT NO OVERLAP")
    print("[v95] kept:", len(accepted), "| rejected overlaps:", len(rejected))

    for item in accepted:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v95] KEEP step {item['x_step']:02d}/{end:02d} | "
    ...
    if rejected:
        ...
        for item in rejected:
            ...
            print(
                f"[v95] DROP step {item['x_step']:02d}/{end:02d} | "
                f"role={item.get('v73_role_key')} | pair={item.get('pair')}"
            )
...
def v81_launch_break(self, source):
    ...
    print("")
    print("[v95] CHARGER BREAK")
    print("[v95] courant :", current)
    print("[v95] nouveau :", source)
    print("[v95] cmd     :", " ".join(cmd))
    print("")
...
def v81_patch_charger_break_buttons(self):
    ...
    print(f"[v95] boutons Charger break patchés : {patched}")
...
def v81_generate_candidate(self, event=None):
    ...
    print("")
    print("[v95] GENERATE CANDIDATE")
    print("[v95] break chargé :", current)
    print("[v95] générateur   :", getattr(fn, "__name__", None))
    print("")
...
def v81_run_training(self):
    ...
    if script is None:
        print("[v95] Aucun script training trouvé.")
...
def v81_save_feedback(self, verdict, train_now=False):
    ...
    print("")
    print("[v95] FEEDBACK SAUVÉ")
    print("[v95] break  :", current)
    print("[v95] verdict:", verdict)
    print("[v95] train  :", train_now)
    print("[v95] fichier:", out_path)
    print("")
...
def v81_build_ui(self):
    ...
    try:
        self.root.title(f"BreakbeatAI v95 — 64 columns minus 0.1")
    except Exception:
        pass
    ...
    try:
        ...
        print(f"[v95] patch Charger break impossible : {exc}")

    try:
        v81_install_training_toolbar(self)
    except Exception as exc:
        print(f"[v95] toolbar impossible : {exc}")
...
def v82_auto_role_pools(self):
    ...
    print("")
    print("[v95] ROLE POOLS")
    for r in ["kick", "snare", "hat", "ghost_snare", "bad"]:
        print(f"[v95] {r:12s}:", roles.get(r, []))
    print("")
...
def v82_generate_candidate(self, event=None):
    ...
    print("")
    print("[v95] NEW CANDIDATE")
    print("[v95] break :", current)
    print("[v95] nonce :", nonce)
    print("[v95] rejected known:", len(rejected))
    print("[v95] fingerprint rejeté ?: ", chosen_fp in rejected)
    for item in self.pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v95] step {item['x_step']:02d}/{end:02d} "
...
def v82_patch_training_buttons(self):
    ...
    print(f"[v95] boutons training patchés : {patched}")
...
def v82_build_ui(self):
    ...
    try:
        ...
        print(f"[v95] patch boutons impossible : {exc}")
...
# ---------------------------------------------------------------------
# v95 EXTENSION : loop 64 + colonnes comme avant - 0.1px
# ---------------------------------------------------------------------
# - On garde les largeurs originales de l'app.
# - On retire seulement 0.1px aux colonnes connues.
# - Reject / Bad passe à une variation structurée suivante.
# - Pas de random libre.
# ---------------------------------------------------------------------

V95_OVERRIDES_PATH = Path("dataset/learning/break_role_overrides_v01.json")
V95_LOCKS_PATH = Path("dataset/learning/main_role_locks_v87.json")
V95_REJECT_PATH = Path("dataset/learning/rejected_variants_v95.json")
...
V95_SNARE_STEPS = [4, 12, 20, 28, 36, 44, 52, 60]

V95_WIDTH_GLOBAL_NAMES = [
    "STEP_W", "CELL_W", "COL_W", "COLUMN_W",
    "GRID_W", "GRID_STEP_W", "STEP_WIDTH",
    "CELL_WIDTH", "COL_WIDTH", "NOTE_WIDTH",
]

V95_WIDTH_ATTR_NAMES = [
    "step_w", "cell_w", "col_w", "column_w",
...
def v95_load_json(path, default):
    try:
...
def v95_find_record(data, current):
    br = data.get("breaks", {}).get(current)
    if br is not None:
        return br

    for name, obj in data.get("breaks", {}).items():
        if v95_norm(name) == v95_norm(current):
            return obj

    return None


def v95_apply_width_minus_01_once(self):
    """
    Remet les colonnes comme avant, mais avec -0.1px.
    Important : on le fait une seule fois pour éviter de retirer 0.1px à chaque redraw.
    """
    if getattr(self, "_v95_width_minus_01_done", False):
        return

    self._v95_width_minus_01_done = True

    for name in V95_WIDTH_GLOBAL_NAMES:
        try:
            value = globals().get(name, None)
            if isinstance(value, (int, float)) and value > 1:
                globals()[name] = float(value) - 0.1
                print(f"[v95] global width {name}: {value} -> {globals()[name]}")
        except Exception:
            pass

    for attr in V95_WIDTH_ATTR_NAMES:
        try:
            if hasattr(self, attr):
                value = getattr(self, attr)
                if isinstance(value, (int, float)) and value > 1:
                    setattr(self, attr, float(value) - 0.1)
                    print(f"[v95] attr width {attr}: {value} -> {getattr(self, attr)}")
        except Exception:
            pass


def v95_force_64_keep_original_width(self):
    for attr in [
    ...
    v95_apply_width_minus_01_once(self)
...
def v95_load_roles(self):
    current = v95_current_break(self)
    ...
    data = v95_load_json(V95_OVERRIDES_PATH, {"breaks": {}})
    br = v95_find_record(data, current)
...
def v95_load_locks(self):
    current = v95_current_break(self)
    data = v95_load_json(V95_LOCKS_PATH, {"breaks": {}})
    br = v95_find_record(data, current)
...
def v95_candidates(self, roles, locks, role):
    out = []
...
def v95_choose_main_pairs(self, roles, locks):
    chosen = {}
    used = set()

    for role in ["snare", "kick", "hat"]:
        candidates = v95_candidates(self, roles, locks, role)
...
def v95_variant_signature(variant_index):
    return V95_SECOND_32_VARIANTS[variant_index]["id"]


def v95_load_rejects():
    data = v95_load_json(
        V95_REJECT_PATH,
        {
            "version": "rejected_variants_v95",
            "updated_at": None,
            "breaks": {},
        },
    )
...
def v95_save_rejects(data):
    data["updated_at"] = time.strftime("%Y-%m-%d %H:%M:%S")
    v95_save_json(V95_REJECT_PATH, data)


def v95_rejected_variants_for_break(current):
    data = v95_load_rejects()
    br = data.get("breaks", {}).get(current, {})
...
def v95_remember_rejected_variant(self):
    current = v95_current_break(self)
    variant_id = getattr(self, "v95_current_variant_id", None)
    ...
    v95_save_rejects(data)

    print("[v95] variant rejetée :", variant_id)


def v95_next_variant_index(self):
    current = v95_current_break(self)
    rejected = v95_rejected_variants_for_break(current)
    last = int(getattr(self, "v95_variant_index", -1))
    total = len(V95_SECOND_32_VARIANTS)

    for offset in range(1, total + 1):
        idx = (last + offset) % total
        variant_id = v95_variant_signature(idx)
        if variant_id not in rejected:
            return idx
...
def v95_alt_pair_for_second_32(self, roles, chosen, role, occurrence):
...
def v95_make_note(self, step, pair, role, source, variant_id):
    step = int(step)
    ...
    return {
        ...
        "hit_slot": int(step) // 2,
        "ai_generated": True,
        "ai_model": "v95_64_columns_minus_01",
        "learned_role": str(role),
        "prior_label": str(role),
        "main_role_source": str(source),
        "v95_loop64": True,
        "v95_variant_id": str(variant_id),
        "v95_columns_minus_01": True,
    }


def v95_validate_pattern(pattern):
    if len(pattern) != 32:
        raise RuntimeError(f"BUG v95 : pattern devrait avoir 32 notes, obtenu {len(pattern)}")
    ...
    for item in pattern:
        ...
        if x % 2 != 0:
            raise RuntimeError(f"BUG v95 : départ impair {x}")

        if x < 0 or x > 62:
            raise RuntimeError(f"BUG v95 : step hors 64 : {x}")

        if x in occupied or (x + 1) in occupied:
            raise RuntimeError(f"BUG v95 : collision à {x}/{x+1}")

        occupied.add(x)
        occupied.add(x + 1)

        if role == "snare" and x not in V95_SNARE_STEPS:
            raise RuntimeError(f"BUG v95 : snare à {x}, interdit")


def v95_generate(self, event=None, reject_mode=False):
    v95_force_64_keep_original_width(self)

    current = v95_current_break(self)
    roles = v95_load_roles(self)
    locks = v95_load_locks(self)
    chosen = v95_choose_main_pairs(self, roles, locks)

    variant_index = v95_next_variant_index(self)
    variant = V95_SECOND_32_VARIANTS[variant_index]
    variant_id = variant["id"]

    self.v95_variant_index = int(variant_index)
    self.v95_current_variant_id = str(variant_id)

    full_grid = list(V95_BASE_32) + list(variant["grid"])

    print("")
    if reject_mode:
        print("[v95] REJECT/BAD → VARIATION STRUCTURÉE SUIVANTE")
    else:
        print("[v95] GENERATE 64 — colonnes avant -0.1px")
    print("[v95] break:", current)
    print("[v95] variant:", variant_id)
    print("[v95] locks:", locks)
    for role in ["kick", "snare", "hat"]:
        print(
            f"[v95] {role:6s}: pair={chosen[role]['pair']} "
    ...
    for step, role in full_grid:
        if step < 32:
            ...
            pair, source = v95_alt_pair_for_second_32(self, roles, chosen, role, occ)

        pattern.append(v95_make_note(self, step, pair, role, source, variant_id))

    v95_validate_pattern(pattern)
    ...
    try:
        ...
        print(f"[v95] draw impossible : {exc}")
    ...
    try:
        ...
        print(f"[v95] refresh_panel impossible : {exc}")

    try:
        self.write_latest_pattern(reason="v95_64_columns_minus_01")
    except Exception as exc:
        print(f"[v95] write_latest_pattern impossible : {exc}")
    ...
    print("")
    print("[v95] GENERATED PATTERN 64")
    for item in self.pattern:
        x = int(item["x_step"])
        end = x + int(item["length"]) - 1
        print(
            f"[v95] step {x:02d}/{end:02d} "
            f"role={item.get('learned_role'):6s} "
            f"pair={item.get('pair')} "
            f"lane={item.get('lane')} "
            f"variant={item.get('v95_variant_id')}"
        )
    print("")

    v95_status(self, f"v95 : colonnes comme avant -0.1px, variant {variant_id}.")
    return "break"


def v95_reject_bad(self, event=None):
    v95_remember_rejected_variant(self)
    return v95_generate(self, event=event, reject_mode=True)


def v95_all_widgets(widget):
    out = [widget]
    ...
    for child in children:
        out.extend(v95_all_widgets(child))

    return out


def v95_patch_buttons(self):
    gen_patched = 0
    bad_patched = 0

    for widget in v95_all_widgets(self.root):
        try:
            text = str(widget.cget("text")).strip().lower()
        except Exception:
            continue
        ...
        try:
            if text == "generate candidate":
                ...
                widget.configure(command=self.reject_bad_v95)
                bad_patched += 1
        except Exception:
            pass
...
_old_v95_draw = getattr(SliceIndexTracker, "draw", None)

def v95_draw(self, *args, **kwargs):
    v95_force_64_keep_original_width(self)

    if callable(_old_v95_draw):
        result = _old_v95_draw(self, *args, **kwargs)
    else:
        result = None
...
_old_v95_build_ui = SliceIndexTracker.build_ui


def v95_build_ui(self):
    _old_v95_build_ui(self)

    v95_force_64_keep_original_width(self)

    try:
        self.root.title(f"BreakbeatAI v95 loop64 columns -0.1px — {v95_current_break(self)}")
    except Exception:
        pass

    try:
        gen_patched, bad_patched = v95_patch_buttons(self)
        print(f"[v95] boutons patchés : Generate={gen_patched}, Reject/Bad={bad_patched}")
    except Exception as exc:
        print(f"[v95] patch boutons impossible : {exc}")
    ...
    try:
        self.root.bind("<F9>", self.reject_bad_v95)
    except Exception:
        pass

    v95_status(self, "v95 : colonnes comme avant -0.1px. Generate=F12, Reject/Bad=F9.")


SliceIndexTracker.generate_candidate = v95_generate
SliceIndexTracker.generate_full_candidate = v95_generate
SliceIndexTracker.generate_ai_pattern = v95_generate
SliceIndexTracker.generate_cross_song_template = v95_generate
SliceIndexTracker.generate_safe_experiment = v95_generate
SliceIndexTracker.generate_locked_roles = v95_generate
SliceIndexTracker.generate_role_aware = v95_generate
SliceIndexTracker.generate_safe_role_ai = v95_generate

SliceIndexTracker.reject_bad_v95 = v95_reject_bad
SliceIndexTracker.reject_bad = v95_reject_bad
SliceIndexTracker.bad_candidate = v95_reject_bad
SliceIndexTracker.reject_candidate = v95_reject_bad
SliceIndexTracker.reject_pattern = v95_reject_bad

if callable(_old_v95_draw):
    SliceIndexTracker.draw = v95_draw

SliceIndexTracker.build_ui = v95_build_ui
