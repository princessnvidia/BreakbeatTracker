...
"""
03_tracker_editor_app_v07.py

BreakbeatAI Tracker Editor v07

Changements :
- Corrige le cas où sounddevice ne sort aucun son.
- Utilise en priorité les lecteurs système Linux :
    pw-play, paplay, aplay, ffplay
- Garde sounddevice en fallback.
- Ajoute un bouton "Audio test".
- Garde les lanes propres :
    HI-HAT
    KICK
    SNARE
- Audio rendu par ordre horizontal x_step uniquement.
"""
...
class TrackerEditorApp:
    def __init__(self, root, pair_json):
        ...
        self.root.title("BreakbeatAI Tracker Editor v07")
    ...
    def detect_external_player(self):
        for name in ["pw-play", "paplay", "aplay", "ffplay"]:
            ...
            if path:
                print(f"[v07] Backend audio système trouvé : {name} -> {path}")
                return name

        print("[v07] Aucun backend système trouvé, fallback sounddevice.")
        return None
    ...
    def build_ui(self):
        ...
        title = tk.Label(
            main,
            text="BreakbeatAI Tracker Editor v07 — audio robuste + HI-HAT/KICK/SNARE",
            bg="#111018",
            fg="#ff7acc",
            font=("Sans", 20, "bold"),
        )
        ...
        subtitle = tk.Label(
            main,
            text=f"Backend audio : {backend} | Espace = Play/Stop loop | Audio rendu par ordre horizontal x_step.",
            bg="#111018",
            fg="#b9acc8",
        )
        ...
        self.output_label = tk.Label(
            panel,
            text=f"Backend audio : {backend}. Clique Audio test, puis Espace.",
            bg="#1b1824",
            fg="#77f5b5",
            justify="left",
        )
    ...
    def bind_keys(self):
        ...
        print("[v07] Raccourcis clavier actifs : Espace = Play/Stop loop")
    ...
    def play_audio_array(self, audio, label="audio"):
        ...
        live_wav = OUT_DIR / f"{self.project['safe']}_v07_live.wav"
        ...
        if self.external_player:
            ...
            try:
                ...
                print(f"[v07] {label} lancé avec {self.external_player} : {live_wav}")
                return duration_ms
            except Exception as exc:
                print(f"[v07] Erreur backend système {self.external_player} : {exc}")
        ...
        try:
            ...
            print(f"[v07] {label} lancé avec sounddevice")
            return duration_ms
        except Exception as exc:
    ...
    def render_preview_file(self):
        OUT_DIR.mkdir(parents=True, exist_ok=True)
        wav = OUT_DIR / f"{self.project['safe']}_tracker_app_v07_preview.wav"
    ...
    def toggle_loop(self):
        if self.looping:
            ...
            print("[v07] Loop arrêtée")
            return

        print("[v07] Lancement loop...")
        self.looping = True
    ...
    def save(self):
        ...
        data = {
            "version": "tracker_app_edit_v07_external_audio_backend",
        ...
        path = OUT_DIR / f"{self.project['safe']}_tracker_app_edit_v07.json"
