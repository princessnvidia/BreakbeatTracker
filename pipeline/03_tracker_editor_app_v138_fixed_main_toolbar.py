...
"""
BreakbeatAI Tracker Editor v138 — slice index
...
def warp_audio_to_length_external(audio, target_len, cache_key=None):
    ...
    if rubberband:
        try:
            ...
            print(f"[v138] rubberband échec, fallback ffmpeg/no-speed : {exc}")
    ...
    if ffmpeg:
        try:
            ...
            print(f"[v138] ffmpeg atempo échec, fallback no-speed : {exc}")
...
class SliceIndexTracker:
    def __init__(self, root, pair_json):
        ...
        self.root.title("BreakbeatAI Tracker Editor v138 — slice index")
    ...
    def detect_external_player(self):
        for name in ["pw-play", "paplay", "aplay", "ffplay"]:
            ...
            if path:
                print(f"[v138] Backend audio : {name} -> {path}")
                return name

        print("[v138] Aucun backend système trouvé, fallback sounddevice.")
        return None
    ...
    def build_initial_pattern(self, randomize=False):
        ...
        print(
            f"[v138] Génération pattern | break={self.project['safe']} | "
        ...
        for slot in range(max_items):
            ...
            print(f"[v138] slot {slot:02d} | step {x_step:02d} -> slice {pair:02d}")
    ...
    def build_ui(self):
        ...
        title = tk.Label(
            main,
            text="BreakbeatAI v138 — fixed main toolbar",
            bg="#111018",
            fg="#ff7acc",
            font=("Sans", 20, "bold"),
        )
    ...
    def record_correction(self, event_type, before=None, after=None):
        ...
        print(f"[v138] correction sauvegardée : {event_type}")
    ...
    def play_audition_array(self, audio, label="audition"):
        ...
        audition_wav = OUT_DIR / f"{self.project['safe']}_v138_audition.wav"
        ...
        if self.external_player:
            try:
                ...
                print(f"[v138] Erreur audition {self.external_player} : {exc}")
    ...
    def on_close(self):
        ...
        """
        print("[v138] Fermeture : arrêt audio + destruction fenêtre")
    ...
    def play_audio_array(self, audio, label="audio"):
        ...
        audio = normalize(audio)
        live_wav = OUT_DIR / f"{self.project['safe']}_v138_live.wav"
        ...
        if self.external_player:
            try:
                ...
                print(f"[v138] Erreur backend {self.external_player} : {exc}")
    ...
    def render_preview_file(self):
        OUT_DIR.mkdir(parents=True, exist_ok=True)
        wav = OUT_DIR / f"{self.project['safe']}_tracker_app_v138_preview.wav"
    ...
    def write_latest_pattern(self, reason="correction"):
        ...
        path = LATEST_DIR / f"{self.project['safe']}_latest_slice_index_v138.json"

        data = {
            "version": "breakbeatai_latest_slice_index_v138",
            "reason": reason,
    ...
    def save(self):
        ...
        tracker_path = OUT_DIR / f"{self.project['safe']}_tracker_app_edit_v138.json"
        ...
        stamp = file_stamp()
        validated_path = VALIDATED_DIR / f"{self.project['safe']}_validated_slice_index_loop64_v138_{stamp}.json"

        validated = {
            "version": "breakbeatai_validated_slice_index_loop64_v138",
...
def v44_record_correction(self, event_type, before=None, after=None):
    ...
    print(f"[v138] correction sauvegardée : {event_type}")
    print(f"[v138] correction path : {corrections_path}")
    print(f"[v138] latest path : {latest_path}")
...
def v45_move_selected(self, dx, dy):
    ...
    if dy != 0 and before != after:
        try:
            ...
            print(f"[v138] audition après flèche haut/bas impossible : {exc}")
...
def v46_warp_loop_pitch_preserve(audio, target_len, cache_key):
    ...
    if rubberband:
        try:
            ...
            print(f"[v138] rubberband global warp échec : {exc}")
    ...
    if ffmpeg:
        try:
            ...
            print(f"[v138] ffmpeg global warp échec : {exc}")

    print("[v138] Aucun warp pitch-preserve dispo, fallback fit no-speed.")
    return v46_fit_audio_no_speed(audio, target_len)
...
def v46_build_initial_pattern(self, randomize=False):
    ...
    print(
        f"[v138] M8 pattern | break={self.project['safe']} | "
...
def v46_get_warped_loop(self):
    ...
    print(
        f"[v138] global warp OK | source_len={loop_end-loop_start} | "
...
def v46_build_ui(self):
    ...
    try:
        ...
        print(f"[v138] UI extension impossible : {exc}")
...
def v47_play_audio_array(self, audio, label="audio"):
    ...
    if len(audio) == 0 or v47_is_silent(audio):
        ...
        print(f"[v138] lecture annulée : audio silencieux ({label})")
        return 0
...
def v47_toggle_loop(self):
    ...
    if not getattr(self, "pattern", None):
        ...
        print("[v138] loop annulée : pattern vide")
        return "break"
...
def v47_delete_selected(self):
    ...
    if not getattr(self, "pattern", None):
        ...
        print("[v138] pattern vide après suppression : audio coupé")
...
def v48_build_initial_pattern(self, randomize=False):
    ...
    print("[v138] pattern initial forcé VIDE : aucun son ne doit jouer.")
    return []
...
def v48_load_first_slices(self):
    ...
    print(f"[v138] Load 16 slices : {len(self.pattern)} items")


def v48_debug_pattern(self, label="render"):
    print("")
    print(f"[v138] DEBUG PATTERN avant {label}")
    print(f"[v138] items = {len(getattr(self, 'pattern', []) or [])}")

    if not getattr(self, "pattern", None):
        print("[v138] pattern vide confirmé")
        print("")
        return
    ...
    for item in self.pattern:
        ...
        print(
            f"[v138] id={item_id} | step {x:02d}/{end:02d} | "
...
def v48_render_audio_with_timeline(self):
    ...
    if not getattr(self, "pattern", None):
        loop_samples = self.get_loop_samples()
        print(f"[v138] rendu silence total : pattern vide | samples={loop_samples}")
        return np.zeros(loop_samples, dtype=np.float32), []
    ...
    print(f"[v138] rendu audio peak={peak:.8f} | timeline_events={len(timeline) if timeline is not None else 'None'}")

    if timeline:
        print("[v138] timeline events:")
        for ev in timeline:
            print(f"[v138]   {ev}")
...
def v48_toggle_loop(self):
    if not getattr(self, "pattern", None):
        ...
        print("[v138] Space ignoré : pattern vide.")
        return "break"
...
def v48_build_ui(self):
    ...
    try:
        ...
        print(f"[v138] ajout UI impossible : {exc}")
...
def v49_add_item_at_xy(self, x, y):
    ...
    try:
        ...
        print(f"[v138] record_correction add impossible : {exc}")
    ...
    try:
        ...
        print(f"[v138] audition après ajout impossible : {exc}")

    print(f"[v138] add | id={item['id']} | step={x_step} | pair={pair} | lane={lane}")
...
def v49_duplicate_selected_forward(self, event=None):
    ...
    if item is None:
        ...
        print("[v138] duplicate annulé : aucune note sélectionnée")
        return "break"
    ...
    if new_x > max_x:
        ...
        print("[v138] duplicate annulé : fin de grille")
        return "break"
    ...
    try:
        ...
        print(f"[v138] record_correction duplicate impossible : {exc}")
    ...
    try:
        ...
        print(f"[v138] audition après duplicate impossible : {exc}")

    print(
        f"[v138] duplicate | old_id={item.get('id')} | new_id={new_item.get('id')} | "
...
def v49_build_ui(self):
    ...
    try:
        ...
        print(f"[v138] bind canvas click impossible : {exc}")
    ...
    try:
        ...
        print(f"[v138] bind keys impossible : {exc}")
...
def v50_load_style_model():
    ...
    try:
        ...
        print(f"[v138] impossible de charger le modèle IA : {exc}")
        return None
...
def v50_generate_ai_pattern(self, event=None):
    ...
    if model is None:
        ...
        print("[v138] modèle absent :", STYLE_MODEL_PATH)
        return "break"
    ...
    print("")
    print(f"[v138] GENERATE IA | safe={safe} | pairs={pair_values} | slots={max_slots}")
    ...
    for slot in range(max_slots):
        ...
        print(
            f"[v138] slot {slot:02d} | step {x_step:02d} | "
    ...
    try:
        ...
        print(f"[v138] write_latest_pattern impossible : {exc}")
...
def v50_build_ui(self):
    ...
    try:
        ...
        print(f"[v138] UI IA impossible : {exc}")
...
def v51_mouse_down(self, event):
    ...
    if item is not None:
        ...
        print(
            f"[v138] mouse down note | id={item.get('id')} | "
            f"step={item.get('x_step')} | pair={item.get('pair')}"
        )
    else:
        print("[v138] mouse down empty")
...
def v51_mouse_up(self, event):
    ...
    if state.get("empty"):
        ...
        print("[v138] drag depuis vide ignoré")
        return "break"
    ...
    if before != after:
        try:
            ...
            print(f"[v138] record_correction drag impossible : {exc}")
        ...
        print(
            f"[v138] drag move | id={after.get('id')} | "
            f"step {before.get('x_step')} -> {after.get('x_step')} | "
        ...
        try:
            ...
            print(f"[v138] audition après drag impossible : {exc}")
...
def v51_build_ui(self):
    ...
    try:
        ...
        print(f"[v138] bind souris impossible : {exc}")
    ...
    try:
        ...
        print(f"[v138] bind Suppr impossible : {exc}")
...
def v52_normalize_loaded_notes(self, notes):
    ...
    for raw in sorted(notes, key=lambda n: (int(float(n.get("x_step", 0))), int(float(n.get("pair", 0))))):
        ...
        if pair not in valid_pairs:
            print(f"[v138] slice ignorée car absente du break actuel : pair={pair}")
            continue
...
def v52_load_saved_beat(self, event=None):
    ...
    if not saves:
        ...
        print("[v138] aucune sauvegarde trouvée")
        return "break"
    ...
    if not pattern:
        ...
        print("[v138] sauvegarde incompatible :", chosen["path"])
        return "break"
    ...
    print("")
    print("[v138] BEAT RESTAURÉ")
    print("[v138] fichier :", chosen["path"])
    print("[v138] clé :", chosen["key"])
    print("[v138] notes :", len(pattern))
    print("")
...
def v52_list_saved_beats(self, event=None):
    ...
    print("")
    print("[v138] SAUVEGARDES TROUVÉES")
    for i, save in enumerate(saves[:30]):
        print(
            f"[v138] {i:02d} | notes={len(save['notes']):02d} | "
...
def v52_build_ui(self):
    ...
    try:
        ...
        print(f"[v138] UI load saved impossible : {exc}")
...
def v53_run_training(self):
    ...
    if not trainer.exists():
        ...
        print("[v138] trainer introuvable :", trainer)
        return False

    print("")
    print("[v138] AUTO-TRAIN IA après Save...")
    print("")
    ...
    try:
        ...
        if result.returncode != 0:
            ...
            print("[v138] auto-train échec code :", result.returncode)
            return False

        v53_status(self, "Save OK + IA entraînée automatiquement.")
        print("[v138] AUTO-TRAIN OK")
        return True

    except Exception as exc:
        v53_status(self, f"Save OK, mais auto-train impossible : {exc}")
        print("[v138] exception auto-train :", exc)
        return False
...
def v53_save(self):
    ...
    try:
        ...
        print(f"[v138] auto-train après save impossible : {exc}")
...
def v53_build_ui(self):
    ...
    try:
        ...
        print(f"[v138] UI auto-train impossible : {exc}")
...
def v55_mouse_up(self, event):
    ...
    if not state.get("moved"):
        ...
        try:
            ...
            print(f"[v138] audition clic note impossible : {exc}")
        ...
        print(
            f"[v138] click play/select | id={item.get('id')} | "
            f"step={item.get('x_step')} | pair={item.get('pair')}"
        )
...
def v58_add_item_at_xy(self, x, y):
    ...
    print(
        f"[v138] add | id={item['id']} | step={x_step} | "
...
def v58_toggle_selected_length(self, event=None):
    ...
    try:
        ...
        print(f"[v138] record_correction length impossible : {exc}")

    v58_refresh_after_edit(self, "toggle_note_length")

    v58_status(self, f"Longueur note : {old_len} -> {new_len} case(s).")
    print(f"[v138] length toggle | id={item.get('id')} | {old_len} -> {new_len}")
...
def v58_duplicate_selected_forward(self, event=None):
    ...
    try:
        ...
        print(f"[v138] audition duplicate impossible : {exc}")
...
def v58_build_ui(self):
    ...
    try:
        ...
        print(f"[v138] UI demi-grille impossible : {exc}")
    ...
    try:
        ...
        print(f"[v138] bind demi-grille impossible : {exc}")
...
def v59_rank_current_slices(self):
    ...
    for pair in self.pair_values:
        ...
        try:
            ...
            print(f"[v138] analyse pair {pair} impossible : {exc}")
            role = "other"
    ...
    print("")
    print("[v138] TOP ROLES CURRENT BREAK")
    for r in ("kick", "snare", "hat"):
        print(f"[v138] {r:5s}:", [(x["pair"], round(x["score"], 3)) for x in ranks[r][:6]])
    print("")
...
def v59_load_role_model():
    ...
    try:
        ...
        print(f"[v138] lecture modèle rôle impossible : {exc}")
        return None
...
def v59_generate_role_aware(self, event=None):
    ...
    if model is None:
        ...
        print("[v138] modèle rôle absent, fallback musical snare 6.")
        model = {
            "breaks": {},
            "global": {},
            "patterns_used": 0,
    ...
    print("")
    print("[v138] ROLE-AWARE GENERATION")
    print("[v138] model patterns_used:", model.get("patterns_used", 0))
    for item in pattern:
        print(
            f"[v138] step {item['x_step']:02d}/{item['x_step'] + item['length'] - 1:02d} | "
...
def v59_run_role_training(self, event=None):
    ...
    try:
        ...
        print(f"[v138] training rôle impossible : {exc}")
...
def v59_save(self):
    ...
    try:
        ...
        print(f"[v138] auto-training rôle après save impossible : {exc}")
...
def v59_build_ui(self):
    ...
    try:
        ...
        print(f"[v138] UI role-aware impossible : {exc}")
...
def v62_add_item_at_xy(self, x, y):
    ...
    print(f"[v138] add | id={item['id']} | step={x_step} | len={length} | pair={pair}")
...
def v62_mouse_down(self, event):
    ...
    if item is not None:
        ...
        print(
            f"[v138] mouse down {state['mode']} | id={item.get('id')} | "
            f"step={item.get('x_step')} | len={item.get('length')} | pair={item.get('pair')}"
        )
    else:
        print("[v138] mouse down empty")
    ...
    if mode == "resize":
        if not state.get("moved"):
            ...
            try:
                ...
                print(f"[v138] audition clic resize impossible : {exc}")
        ...
        if before != after:
            try:
                ...
                print(f"[v138] record_correction resize impossible : {exc}")
            ...
            try:
                ...
                print(f"[v138] audition après resize impossible : {exc}")
    ...
    if mode == "move":
        ...
        if before != after:
            try:
                ...
                print(f"[v138] record_correction move impossible : {exc}")
            ...
            try:
                ...
                print(f"[v138] audition après move impossible : {exc}")
...
def v62_toggle_selected_length(self, event=None):
    ...
    try:
        ...
        print(f"[v138] record_correction toggle length impossible : {exc}")

    v62_refresh_after_edit(self, "toggle_note_length_v62")
    v62_status(self, f"Longueur note : {old_len} → {new_len} case(s).")

    print(f"[v138] toggle length | id={item.get('id')} | {old_len} -> {new_len}")
...
def v62_draw(self):
    ...
    try:
        ...
        print(f"[v138] draw handles impossible : {exc}")
...
def v62_build_ui(self):
    ...
    try:
        ...
        print(f"[v138] bind resize impossible : {exc}")
    ...
    try:
        ...
        print(f"[v138] UI resize impossible : {exc}")
...
def v63_load_model():
    ...
    try:
        ...
        print(f"[v138] lecture modèle strict impossible : {exc}")
        return None
...
def v63_generate_safe_role_ai(self, event=None):
    ...
    print("")
    print("[v138] SAFE ROLE AI GENERATED")
    print("[v138] model strict:", "yes" if model else "no")
    print("[v138] snare main/alt:", main_snare, alt_snare)
    print("[v138] kick  main/alt:", main_kick, alt_kick)
    print("[v138] hat   main/alt:", main_hat, alt_hat)
    print("[v138] pattern:")
    for item in pattern:
        print(
            f"[v138] step {item['x_step']:02d}/{item['x_step'] + item['length'] - 1:02d} | "
...
def v63_run_strict_training(self, event=None):
    ...
    print("")
    print("[v138] TRAIN STRICT ROLE MODEL...")
    print("")
    ...
    try:
        ...
        print(f"[v138] training strict impossible : {exc}")
...
def v63_build_ui(self):
    ...
    try:
        ...
        print(f"[v138] UI safe impossible : {exc}")
...
def v65_rank_slices(self):
    ...
    for pair in self.pair_values:
        ...
        try:
            ...
            print(f"[v138] analyse slice {pair} impossible : {exc}")
            scores = {
                "kick": 0.0,
                "snare": 0.0,
                "hat": 0.0,
    ...
    print("")
    print("[v138] TOP SLICES PAR ROLE — break courant")
    for role in ("kick", "snare", "hat", "ghost_snare"):
        print(f"[v138] {role:12s}:", [(x["pair"], round(x["score"], 3)) for x in ranks[role][:8]])
    print("")
...
def v65_generate_from_role_template(self, event=None):
    ...
    try:
        ...
        print(f"[v138] template introuvable : {exc}")
        return "break"
    ...
    print("")
    print("[v138] CROSS-SONG ROLE TEMPLATE GENERATED")
    print("[v138] template:", V65_TEMPLATE_PATH)
    print("[v138] temperature:", temperature, "| hats:", hat_density, "| ghosts:", ghost_density)
    print("[v138] pattern:")
    for item in pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v138] step {item['x_step']:02d}/{end:02d} | "
...
def v65_build_ui(self):
    ...
    try:
        ...
        print(f"[v138] UI impossible : {exc}")
...
def v67_train_strict_if_available(self):
    ...
    if trainer is None:
        print("[v138] aucun trainer trouvé.")
        return False

    print("")
    print("[v138] TRAIN après validation :", trainer)
    print("")
    ...
    try:
        ...
        print(f"[v138] training impossible : {exc}")
        return False
...
def v67_generate_candidate(self, event=None):
    ...
    print("")
    print("[v138] CANDIDATE GENERATED")
    print("[v138] break:", self.v67_candidate_safe)
    print("[v138] notes:", len(candidate))
    print("[v138] Maintenant : corrige puis clique Good après modifs, ou Reject.")
    print("")
...
def v67_reject_candidate(self, event=None):
    ...
    print("")
    print("[v138] REJECTED")
    print("[v138] saved:", rejected_path)
    print("")
...
def v67_accept_corrected(self, event=None):
    ...
    print("")
    print("[v138] ACCEPTED GOOD AFTER EDIT")
    print("[v138] saved:", validated_path)
    print("[v138] trained:", trained)
    print("")
...
def v67_accept_without_training(self, event=None):
    ...
    print("[v138] accepted no-train:", validated_path)
...
def v67_build_ui(self):
    ...
    try:
        ...
        print(f"[v138] UI feedback impossible : {exc}")
...
def v68_generate_candidate(self, event=None):
    ...
    print("[v138] Length policy appliquée : 2 cases par défaut, 1 case rare.")
...
def v69_force_pattern_hard_2_grid(self, reason="v69_force_hard_2_grid"):
    ...
    print("")
    print("[v138] HARD 2 GRID APPLIQUÉ")
    for item in self.pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v138] step {item['x_step']:02d}/{end:02d} | "
...
def v70_duplicate_selected_forward(self, event=None):
    ...
    print("")
    print("[v138] CTRL+D DUPLICATE +2")
    print(f"[v138] old_id={item.get('id')} new_id={new_item.get('id')}")
    print(f"[v138] step {old_x}/{old_x + length - 1} -> {new_x}/{new_x + length - 1}")
    print(f"[v138] pair={new_item.get('pair')} len={length}")
    print("")
...
def v70_build_ui(self):
    ...
    try:
        ...
        print(f"[v138] bind Ctrl+D impossible : {exc}")
...
def v71_rank_slices(self):
    ...
    print("")
    print("[v138] PRE-LEARNING TOP SLICES")
    for role in ("kick", "snare", "hat", "ghost_snare"):
        print(f"[v138] {role:12s}:", [(x["pair"], round(x["score"], 3)) for x in ranks[role][:8]])
    print("")
...
def v71_prelearn_roles(self, force=False):
    ...
    print("")
    print("[v138] PRELEARN ROLES POUR", safe)
    for role in ("kick", "snare", "hat", "ghost_snare", "bad"):
        print(f"[v138] {role:12s}:", roles.get(role, []))
    print("")
...
def v71_generate_full_candidate(self, event=None):
    ...
    print("")
    print("[v138] FULL CANDIDATE GENERATED")
    print("[v138] break:", v71_safe(self))
    print("[v138] roles:", roles)
    print("[v138] pattern:")
    for item in pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
...
def v72_open_break(self, safe=None):
    ...
    print("")
    print("[v138] OPEN BREAK")
    print("[v138] current:", current)
    print("[v138] next   :", safe)
    print("[v138] cmd    :", " ".join(cmd))
    print("")
...
def v72_build_ui(self):
    ...
    try:
        ...
        print(f"[v138] UI break switcher impossible : {exc}")
...
def v73_sanitize_tracker_pattern(self, reason="v73_tracker_strict_no_overlap"):
    ...
    print("")
    print("[v138] TRACKER STRICT NO OVERLAP")
    print("[v138] kept:", len(accepted), "| rejected overlaps:", len(rejected))

    for item in accepted:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v138] KEEP step {item['x_step']:02d}/{end:02d} | "
    ...
    if rejected:
        ...
        for item in rejected:
            ...
            print(
                f"[v138] DROP step {item['x_step']:02d}/{end:02d} | "
                f"role={item.get('v73_role_key')} | pair={item.get('pair')}"
            )
...
def v81_launch_break(self, source):
    ...
    print("")
    print("[v138] CHARGER BREAK")
    print("[v138] courant :", current)
    print("[v138] nouveau :", source)
    print("[v138] cmd     :", " ".join(cmd))
    print("")
...
def v81_patch_charger_break_buttons(self):
    ...
    print(f"[v138] boutons Charger break patchés : {patched}")
...
def v81_generate_candidate(self, event=None):
    ...
    print("")
    print("[v138] GENERATE CANDIDATE")
    print("[v138] break chargé :", current)
    print("[v138] générateur   :", getattr(fn, "__name__", None))
    print("")
...
def v81_run_training(self):
    ...
    if script is None:
        print("[v138] Aucun script training trouvé.")
...
def v81_save_feedback(self, verdict, train_now=False):
    ...
    print("")
    print("[v138] FEEDBACK SAUVÉ")
    print("[v138] break  :", current)
    print("[v138] verdict:", verdict)
    print("[v138] train  :", train_now)
    print("[v138] fichier:", out_path)
    print("")
...
def v81_build_ui(self):
    ...
    try:
        self.root.title(f"BreakbeatAI v138 — fixed main toolbar")
    except Exception:
        pass
    ...
    try:
        ...
        print(f"[v138] patch Charger break impossible : {exc}")

    try:
        v81_install_training_toolbar(self)
    except Exception as exc:
        print(f"[v138] toolbar impossible : {exc}")
...
def v82_auto_role_pools(self):
    ...
    print("")
    print("[v138] ROLE POOLS")
    for r in ["kick", "snare", "hat", "ghost_snare", "bad"]:
        print(f"[v138] {r:12s}:", roles.get(r, []))
    print("")
...
def v82_generate_candidate(self, event=None):
    ...
    print("")
    print("[v138] NEW CANDIDATE")
    print("[v138] break :", current)
    print("[v138] nonce :", nonce)
    print("[v138] rejected known:", len(rejected))
    print("[v138] fingerprint rejeté ?: ", chosen_fp in rejected)
    for item in self.pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v138] step {item['x_step']:02d}/{end:02d} "
...
def v82_patch_training_buttons(self):
    ...
    print(f"[v138] boutons training patchés : {patched}")
...
def v82_build_ui(self):
    ...
    try:
        ...
        print(f"[v138] patch boutons impossible : {exc}")
...
def v110_generate(self, event=None, reject_mode=False):
    ...
    print("")
    if reject_mode:
        print("[v138] REJECT/BAD -> variation premier 32 + autres lignes")
    else:
        print("[v138] GENERATE 64")
    print("[v138] break:", current)
    print("[v138] first32:", first32_id)
    print("[v138] response:", response_id)
    print("[v138] role_offset:", role_offset)
    for role in ["kick", "snare", "hat"]:
        lane = "?"
        try:
            ...
            pass
        print(f"[v138] {role:6s}: pair={chosen[role]['pair']} lane={lane} source={chosen[role]['source']}")
    print("")
    ...
    try:
        ...
        print(f"[v138] draw impossible : {exc}")
    ...
    try:
        ...
        print(f"[v138] refresh_panel impossible : {exc}")
...
def v110_render_pattern_to_audio(self):
    ...
    for item in pattern:
        ...
        if pair not in slice_map:
            skipped += 1
            print(f"[v138] skip pair introuvable : {pair}")
            continue
...
def v110_export_wav_current(self, event=None):
    try:
        ...
        print("")
        print("[v138] EXPORT WAV OK")
        print("[v138] break:", current)
        print("[v138] wav  :", out_path)
        print("[v138] json :", info_path)
        print("[v138] duration:", round(info["duration_seconds"], 3), "s")
        print("[v138] rendered:", info["rendered_notes"], "skipped:", info["skipped_notes"])
        print("")
...
def v110_space_play(self, event=None):
    ...
    try:
        ...
        if btn is not None:
            print("[v138] SPACE -> Loop / Space")
            try:
                btn.invoke()
            except Exception as exc:
                print("[v138] invoke Loop / Space impossible :", exc)
            return "break"

        print("[v138] SPACE : bouton Loop / Space introuvable.")
...
def v110_patch_buttons(self):
    ...
    print(f"[v138] boutons patchés : Generate={gen_patched}, Reject/Bad={bad_patched}")
...
def v110_build_ui(self):
    ...
    try:
        ...
        print("[v138] patch boutons impossible :", exc)

    try:
        v110_add_export_button(self)
    except Exception as exc:
        print("[v138] ajout Export WAV impossible :", exc)

    try:
        v110_install_space_bindings(self)
    except Exception as exc:
        print("[v138] bind espace impossible :", exc)
...
def v113_export(self, event=None):
    ...
    print("[v138] ERREUR : export_wav_current_v110 introuvable.")
...
def v113_place_export_button(self):
    ...
    try:
        ...
        print("[v138] bouton Export WAV visible ajouté.")
        v113_status(self, "v113 : Export WAV visible. Espace reste Loop / Space.")

    except Exception as exc:
        print("[v138] placement bouton Export WAV impossible :", exc)
        try:
            btn.place(x=760, y=32, height=30)
            btn.lift()
            self.export_button_v113 = btn
...
def v115_apply_cells_minus_25(self):
    ...
    print("")
    print("[v138] APPLICATION CASES -3.5px")
    ...
    for name in V115_WIDTH_GLOBAL_NAMES:
        try:
            ...
            if isinstance(value, (int, float)) and value > 6:
                ...
                print(f"[v138] global {name}: {value} -> {globals()[name]}")
        except Exception:
            pass
        ...
        try:
            if hasattr(self, attr):
                ...
                if isinstance(value, (int, float)) and value > 6:
                    ...
                    print(f"[v138] attr {attr}: {value} -> {getattr(self, attr)}")
        except Exception:
            pass
...
def v115_build_ui(self):
    ...
    try:
        self.root.title("BreakbeatAI v138 — fixed main toolbar")
    except Exception:
        pass
    ...
    try:
        ...
        print("[v138] impossible d'appliquer -3.5px :", exc)
...
def v117_render_pattern_to_audio(self):
    ...
    print("")
    print("[v138] EXPORT EXACT CELLS")
    print("[v138] break:", current)
    print("[v138] pairblock:", pairblock_file)
    print("[v138] sr:", sr)
    print("[v138] median_pair_samples:", median_pair_samples)
    print("[v138] case_samples:", case_samples)
    print("[v138] export_samples:", total_samples)
    print("[v138] export_seconds:", round(total_samples / float(sr), 6))
    print("")
...
def v117_export_wav_current(self, event=None):
    try:
        ...
        print("[v138] wav  :", out_path)
        print("[v138] json :", info_path)
        print("[v138] duration:", round(info["duration_seconds"], 6), "s")
        print("[v138] case_seconds:", round(info["case_seconds"], 6))
        print("[v138] rendered:", info["rendered_notes"], "skipped:", info["skipped_notes"])
        print("")
...
def v118_current_break(self):
    ...
    if candidates:
        ...
        print("[v138] break courant:", stem, "| source:", source, "| raw:", raw)
        return stem

    print("[v138] WARNING : break courant introuvable, fallback Camo.")
    return "Camo_Break_-_3A"
...
def v118_find_best_playback_wav(since):
    ...
    print("")
    print("[v138] WAV candidats playback :")
    for score, mtime, size, wav in candidates[:12]:
        age = time.time() - mtime
        print(f"[v138] score={score:8d} age={age:7.2f}s size={size:10d} path={wav}")
    print("")
...
def v118_copy_playback_wav(self, since):
    ...
    if best is None:
        ...
        print("[v138] aucun WAV playback trouvé.")
        return "break"
    ...
    print("")
    print("[v138] EXPORT PLAYBACK OK")
    print("[v138] source :", best)
    print("[v138] export :", out)
    print("[v138] json   :", info_path)
    print("")
...
def v118_export_playback_wav(self, event=None):
    ...
    try:
        ...
        if btn is not None:
            print("[v138] déclenche Loop / Space pour générer le playback WAV...")
            btn.invoke()
        else:
            print("[v138] bouton Loop / Space introuvable, copie du dernier WAV playback existant.")
    except Exception as exc:
        print("[v138] invoke Loop / Space impossible :", exc)
...
def v121_find_best_long_wav(since):
    ...
    print("")
    print("[v138] WAV candidats longs/propre à raccourcir :")
    for row in candidates[:20]:
        age = time.time() - row["mtime"]
        print(
            f"[v138] score={row['score']:7d} "
...
def v121_export_trimmed_long(self, event=None):
    ...
    try:
        ...
        if btn is not None:
            print("[v138] déclenche Loop / Space pour générer le WAV long/propre...")
            btn.invoke()
    except Exception as exc:
        print("[v138] Loop / Space impossible :", exc)
    ...
    def do_export():
        try:
            ...
            try:
                ...
                print("[v138] ffmpeg trim impossible, fallback wave :", exc)
                ok = False
            ...
            print("")
            print("[v138] EXPORT TRIM OK")
            print("[v138] source :", src_wav)
            print("[v138] source durée :", round(src_dur, 6), "s")
            print("[v138] target durée :", round(target_seconds, 6), "s")
            print("[v138] export :", out)
            print("[v138] export durée :", round(float(out_dur or 0), 6), "s")
            print("[v138] json :", info_path)
            print("")
...
def v124_export_trimmed_long_no_json(self, event=None):
    ...
    def do_export():
        try:
            ...
            print("")
            print("[v138] EXPORT WAV ONLY OK")
            print("[v138] source :", src_wav)
            print("[v138] source durée :", round(src_dur, 6), "s")
            print("[v138] target durée :", round(target_seconds, 6), "s")
            print("[v138] export :", out)
            print("[v138] export durée :", round(float(out_dur or 0), 6), "s")
            print("[v138] aucun JSON exporté")
            print("")
...
def v125_select_lane(self, lane):
    ...
    print("")
    print(f"[v138] Slice {lane + 1} sélectionnée")
    print(f"[v138] notes sélectionnées : {ids}")
    print("")
...
def v125_redraw_after_move(self):
    try:
        ...
        print("[v138] draw impossible :", exc)
...
def v125_build_ui(self):
    ...
    if canvas is not None:
        try:
            ...
            print("[v138] bind canvas click impossible :", exc)
    ...
    try:
        ...
        print("[v138] bind flèches impossible :", exc)
...
def v126_select_slice(self, zero_index, slice_number, reason=""):
    ...
    print("")
    print("[v138] SÉLECTION SLICE")
    print("[v138] reason:", reason)
    print("[v138] Slice affichée:", slice_number)
    print("[v138] zero_index:", zero_index)
    print("[v138] pair_candidates:", pair_candidates)
    print("[v138] ids sélectionnés:", ids)
...
def v126_build_ui(self):
    ...
    if canvas is not None:
        try:
            ...
            print("[v138] bind canvas impossible :", exc)
...
def v127_select_slice(self, lane, displayed, reason=""):
    ...
    print("")
    print("[v138] SÉLECTION SLICE ALIGNÉE")
    print("[v138] reason:", reason)
    print("[v138] label affiché:", displayed)
    print("[v138] lane réelle:", lane)
    print("[v138] labels zero-based:", v127_labels_are_zero_based(self))
    print("[v138] pair_candidates:", pair_candidates)
    print("[v138] ids sélectionnés:", ids)
    for row in debug_rows:
        print("[v138] note:", row)
...
def v128_select_lane(self, lane):
    ...
    print("")
    print("[v138] MULTI-SÉLECTION LIGNE")
    print("[v138] slice/lane:", lane)
    print("[v138] ids:", ids)
    for row in debug:
        print("[v138] note:", row)
    print("")
...
def v129_draw_green_multiselect(self):
    ...
    print(f"[v138] contours verts dessinés : {drawn}")
...
def v129_draw(self, *args, **kwargs):
    ...
    try:
        ...
        print("[v138] contour vert impossible :", exc)
...
def v130_audition_after_move(self, notes):
    ...
    def do_play():
        ...
        for name in method_names:
            ...
            try:
                fn()
                print(f"[v138] audition via {name}()")
                played = True
                break
            except TypeError:
                try:
                    fn(None)
                    print(f"[v138] audition via {name}(None)")
                    played = True
                    break
                except TypeError:
                    try:
                        fn(note)
                        print(f"[v138] audition via {name}(note)")
                        played = True
                        break
                    except Exception:
                        pass
        ...
        if not played:
            ...
            if btn is not None:
                try:
                    btn.invoke()
                    print("[v138] audition via bouton Play slice")
                    played = True
                except Exception as exc:
                    print("[v138] Play slice invoke impossible :", exc)

        if not played:
            print("[v138] audition introuvable : aucune méthode Play slice trouvée")
...
def v131_generate_candidate(self, event=None, reject_mode=False):
    ...
    print("")
    print("[v138] GENERATE CANDIDATE STRICT")
    print("[v138] break:", current)
    print("[v138] sequence: kick hat snare hat | hat kick snare hat")
    print("[v138] répété sur 64 cases")
    for role in ["kick", "snare", "hat"]:
        print(f"[v138] {role:6s}: pair={chosen[role]['pair']} source={chosen[role]['source']}")
    print("")
...
def v131_build_ui(self):
    ...
    try:
        ...
        print("[v138] repatch boutons impossible :", exc)
...
# ---------------------------------------------------------------------
# v134 : UI minimale, grille seulement
# ---------------------------------------------------------------------
# Garde :
# - la grille
# - le sélecteur de break
# - le moteur audio/export
# - Generate strict v131
# - multisélection/flèches/audition
#
# Cache :
# - toute la section du bas
# - toutes les lignes v48/v50/v52/v53...
# - tous les boutons IA/train/good/reject
# ---------------------------------------------------------------------


def v134_walk(widget):
    out = [widget]
    try:
        for child in widget.winfo_children():
            out.extend(v134_walk(child))
    except Exception:
        pass
    return out


def v134_text(widget):
    try:
        return str(widget.cget("text")).strip()
    except Exception:
        return ""
...
def v134_contains(parent, child):
    cur = child
...
def v134_find_canvas(self):
    ...
    for widget in v134_walk(self.root):
        try:
            if not isinstance(widget, tk.Canvas):
                continue
            area = int(widget.winfo_width()) * int(widget.winfo_height())
            if area > best_area:
                best = widget
                best_area = area
        except Exception:
...
def v134_container_to_hide(self, widget, canvas):
    """
    Remonte jusqu'au plus grand conteneur qui ne contient pas la grille.
    Comme ça on cache toute une ligne/frame d'un coup.
    """
    if widget is None or widget == self.root or widget == canvas:
        return None

    if v134_contains(widget, canvas):
        return None
    ...
    while True:
        ...
        if v134_contains(parent, canvas):
            return cur

        cur = parent


def v134_status(self, text):
    try:
        if hasattr(self, "set_status"):
            self.set_status(text)
        elif hasattr(self, "output_label"):
...
def v134_find_button(self, label):
    target = label.lower().strip()

    for widget in v134_walk(self.root):
        if v134_text(widget).lower().strip() == target:
            return widget

    return None


def v134_export(self, event=None):
    for name in [
    ...
    print("[v138] Export WAV introuvable")
    v134_status(self, "Export WAV introuvable.")
    return "break"


def v134_new_groove(self, event=None):
    ...
    v134_status(self, "Nouveau groove introuvable.")
    return "break"


def v134_loop(self, event=None):
    btn = v134_find_button(self, "Loop / Space")
    if btn is not None:
        try:
            btn.invoke()
            return "break"
    ...
    v134_status(self, "Loop introuvable.")
    return "break"


def v134_duplicate_32(self, event=None):
    pattern = list(getattr(self, "pattern", []) or [])
    first = []
    ...
    if not first:
        v134_status(self, "Aucun premier 32 à dupliquer.")
        return "break"
    ...
    try:
        self.write_latest_pattern(reason="v134_duplicate_32")
    except Exception:
        pass

    v134_status(self, "Premier 32 dupliqué sur le deuxième 32.")
    return "break"


def v134_button(parent, text, command, bg, fg="#ffffff"):
    return tk.Button(
        parent,
        text=text,
        command=command,
...
def v134_install_toolbar(self):
    if getattr(self, "_v134_toolbar_done", False):
        return

    toolbar = tk.Frame(self.root, bg="#111018", bd=0, highlightthickness=1, highlightbackground="#4b3757")

    v134_button(toolbar, "Nouveau groove", self.v134_new_groove, "#55306b").pack(side="left", padx=3, pady=3)
    v134_button(toolbar, "Dupliquer 32", self.v134_duplicate_32, "#303040").pack(side="left", padx=3, pady=3)
    v134_button(toolbar, "Loop / Space", self.v134_loop, "#27324f").pack(side="left", padx=3, pady=3)
    v134_button(toolbar, "Export WAV", self.v134_export, "#226b45").pack(side="left", padx=3, pady=3)

    try:
        toolbar.place(relx=1.0, x=-10, y=32, anchor="ne")
        toolbar.lift()
    except Exception:
        try:
            toolbar.pack(side="top", anchor="e", padx=8, pady=2)
        except Exception:
            pass

    self.v134_toolbar = toolbar
    self._v134_toolbar_done = True


def v134_hide_old_ui(self):
    try:
        self.root.update_idletasks()
    except Exception:
        pass

    canvas = v134_find_canvas(self)
    if canvas is None:
        print("[v138] canvas introuvable")
        return
    ...
    for widget in v134_walk(self.root):
        ...
        # Ne jamais cacher la nouvelle toolbar.
        try:
            if hasattr(self, "v134_toolbar") and v134_contains(self.v134_toolbar, widget):
                continue
        except Exception:
            pass

        text = v134_text(widget)
        ...
        if any(part in low for part in junk_contains):
            should_hide = True

        # Tout widget placé sous la grille disparaît.
        try:
        ...
        if should_hide:
            c = v134_container_to_hide(self, widget, canvas)
            if c is not None:
                to_hide.add(c)

    # Cache d'abord les plus profonds, puis leurs parents éventuels.
    def depth(w):
        d = 0
        cur = w
        while cur is not None:
    ...
    for widget in sorted(to_hide, key=depth, reverse=True):
        try:
            # Ne pas cacher un conteneur qui contient la grille.
            if v134_contains(widget, canvas):
                continue
            v134_hide(widget)
        except Exception:
    ...
    # Resize fenêtre à la grille uniquement.
    try:
        self.root.update_idletasks()
        ...
        useful_bottom = canvas.winfo_y() + canvas.winfo_height() + 18
        useful_bottom = max(520, int(useful_bottom))

        self.root.geometry(f"{root_w}x{useful_bottom}+{root_x}+{root_y}")
    except Exception as exc:
        print("[v138] resize impossible :", exc)

    print(f"[v138] conteneurs cachés : {len(to_hide)}")
    v134_status(self, "v134 : UI nettoyée, bas masqué.")


_old_v134_build_ui = SliceIndexTracker.build_ui


def v134_build_ui(self):
    _old_v134_build_ui(self)
    ...
    try:
        v134_install_toolbar(self)
    except Exception as exc:
        print("[v138] toolbar impossible :", exc)

    try:
        self.root.after(150, lambda: v134_hide_old_ui(self))
        self.root.after(600, lambda: v134_hide_old_ui(self))
        self.root.after(1200, lambda: v134_hide_old_ui(self))
    except Exception:
        v134_hide_old_ui(self)

    try:
        self.root.bind("<F12>", self.v134_new_groove)
        self.root.bind("<Control-e>", self.v134_export)
    except Exception:
        pass


SliceIndexTracker.v134_new_groove = v134_new_groove
SliceIndexTracker.v134_duplicate_32 = v134_duplicate_32
SliceIndexTracker.v134_loop = v134_loop
SliceIndexTracker.v134_export = v134_export
SliceIndexTracker.build_ui = v134_build_ui



# ---------------------------------------------------------------------
# v135 : Princess iTunes pink UI
# ---------------------------------------------------------------------
# Objectif :
# - rendre l'UI plus jolie, rose/noir/glass
# - garder la grille intacte
# - cacher l'ancien bazar IA/train
# - garder les fonctions v134/v131/v130/v129
# ---------------------------------------------------------------------
...
def p135_walk(widget):
    out = [widget]
    try:
        for child in widget.winfo_children():
            out.extend(p135_walk(child))
    except Exception:
        pass
    return out


def p135_text(widget):
    try:
        return str(widget.cget("text")).strip()
    except Exception:
        return ""
...
def p135_contains(parent, child):
    cur = child
    while cur is not None:
        if cur == parent:
            return True
...
def p135_find_canvas(self):
    ...
    for widget in p135_walk(self.root):
        try:
...
def p135_find_button(self, label):
    target = label.lower().strip()
    for widget in p135_walk(self.root):
        if p135_text(widget).lower().strip() == target:
            return widget
    return None


def p135_export(self, event=None):
    for name in [
        "export_wav_current_v122",
        "export_wav_current_v121",
        "export_wav_current_v120",
    ...
    fn = globals().get("v134_export")
    if callable(fn):
        return fn(self, event)

    p135_status(self, "Export WAV introuvable.")
    return "break"


def p135_new_groove(self, event=None):
    fn = getattr(self, "generate_candidate", None)
    if callable(fn):
        return fn(event)
    ...
    p135_status(self, "Nouveau groove introuvable.")
    return "break"


def p135_loop(self, event=None):
    btn = p135_find_button(self, "Loop / Space")
    if btn is not None:
        try:
            btn.invoke()
            return "break"
    ...
    p135_status(self, "Loop introuvable.")
    return "break"


def p135_duplicate_32(self, event=None):
    fn = getattr(self, "v134_duplicate_32", None)
    if callable(fn):
        return fn(event)

    fn = globals().get("v134_duplicate_32")
    if callable(fn):
        return fn(self, event)
    ...
    if not first:
        p135_status(self, "Aucun premier 32 à dupliquer.")
        return "break"
    ...
    try:
        self.write_latest_pattern(reason="v135_duplicate_32")
    except Exception:
        pass

    p135_status(self, "Premier 32 dupliqué.")
    return "break"


def p135_style_widget(widget):
    cls = ""
    try:
    ...
    text = p135_text(widget)
    low = text.lower()

    try:
        if isinstance(widget, tk.Frame):
            widget.configure(bg=P135_BG)
    except Exception:
        pass
    ...
    try:
        if isinstance(widget, tk.Label):
            ...
            if "BreakbeatAI" in text:
                fg = P135_PINK_2
            elif "Backend audio" in text or "BPM" in text or "méthode" in text:
                fg = P135_MUTED
            elif "Break" == text:
                fg = P135_PINK
    ...
    try:
        if isinstance(widget, tk.Button):
            ...
            if "export" in low:
                ...
                bg = "#27304d"
            elif "generate" in low or "nouveau" in low:
                bg = P135_PINK_DARK
            elif "charger" in low or "refresh" in low or "randomize" in low:
                bg = "#2a1d35"
            elif "delete" in low:
                bg = "#5a2635"
            ...
            widget.configure(
                bg=bg,
                fg=fg,
                activebackground=P135_PINK_DARK,
                activeforeground="#ffffff",
                relief="flat",
                bd=0,
                highlightthickness=1,
                highlightbackground=P135_BORDER,
                padx=8,
                pady=4,
            )
    except Exception:
    ...
    # Widgets ttk/combobox via configure classique quand possible.
    try:
        if "Combo" in cls or "TCombobox" in cls:
            widget.configure()
    except Exception:
        pass


def p135_style_everything(self):
    try:
        self.root.configure(bg=P135_BG)
    except Exception:
        pass

    for widget in p135_walk(self.root):
        p135_style_widget(widget)


def p135_container_to_hide(self, widget, canvas):
    if widget is None or widget == self.root or widget == canvas:
        return None

    if p135_contains(widget, canvas):
        return None
    ...
    while True:
        ...
        if p135_contains(parent, canvas):
            return cur

        cur = parent


def p135_hide_old_ui(self):
    try:
        self.root.update_idletasks()
    except Exception:
        pass

    canvas = p135_find_canvas(self)
    if canvas is None:
        return
    ...
    for widget in p135_walk(self.root):
        ...
        try:
            if hasattr(self, "p135_toolbar") and p135_contains(self.p135_toolbar, widget):
                continue
        except Exception:
            pass
        ...
        if should_hide:
            c = p135_container_to_hide(self, widget, canvas)
            if c is not None:
                to_hide.add(c)
    ...
    print(f"[v138] conteneurs UI cachés : {len(to_hide)}")


def p135_make_button(parent, text, command, kind="normal"):
    if kind == "pink":
        bg = "#7a2f68"
        active = "#a33a86"
        fg = "#fff3fb"
    ...
    btn = tk.Button(
        ...
        cursor="hand2",
        highlightthickness=1,
        highlightbackground="#7b4a86",
    )
    return btn


def p135_install_toolbar(self):
    if getattr(self, "_p135_toolbar_done", False):
        return
    ...
    toolbar = tk.Frame(
        ...
        bd=0,
        highlightthickness=1,
        highlightbackground="#6c3d75",
    )

    left = tk.Frame(toolbar, bg="#130b19")
    ...
    subtitle = tk.Label(
        left,
        text="tracker mode · jungle grid · pink glass",
        bg="#130b19",
        fg=P135_MUTED,
        font=("Sans", 8),
    )
    ...
    buttons = tk.Frame(toolbar, bg="#130b19")
    buttons.pack(side="left", padx=8, pady=6)

    p135_make_button(buttons, "Nouveau groove", self.p135_new_groove, "pink").pack(side="left", padx=4)
    p135_make_button(buttons, "Miroir 32", self.p135_duplicate_32, "normal").pack(side="left", padx=4)
    p135_make_button(buttons, "Loop / Space", self.p135_loop, "blue").pack(side="left", padx=4)
    p135_make_button(buttons, "Export WAV", self.p135_export, "green").pack(side="left", padx=4)

    hint = tk.Label(
        toolbar,
        text="clic slice = multisélection · flèches = déplacer",
        bg="#130b19",
        fg="#c8a7d0",
        font=("Sans", 8),
        padx=10,
    ...
    try:
        toolbar.place(relx=1.0, x=-12, y=28, anchor="ne")
        toolbar.lift()
    except Exception:
        toolbar.pack(side="top", anchor="e", padx=8, pady=2)

    self.p135_toolbar = toolbar
    self._p135_toolbar_done = True
    print("[v138] toolbar Princess iTunes installée.")


def p135_reposition_export_button(self):
    """
    Si l'ancien bouton Export WAV flottant existe encore,
    on le cache pour éviter le double bouton.
    """
    for widget in p135_walk(self.root):
        try:
            if hasattr(self, "p135_toolbar") and p135_contains(self.p135_toolbar, widget):
        ...
        if p135_text(widget).lower() == "export wav":
            p135_hide(widget)


_old_p135_build_ui = SliceIndexTracker.build_ui


def p135_build_ui(self):
    _old_p135_build_ui(self)
    ...
    try:
        p135_style_everything(self)
    except Exception as exc:
        print("[v138] style impossible :", exc)

    try:
        p135_install_toolbar(self)
    except Exception as exc:
        print("[v138] toolbar impossible :", exc)

    try:
        p135_reposition_export_button(self)
    except Exception:
        pass

    try:
        self.root.after(150, lambda: p135_hide_old_ui(self))
        self.root.after(600, lambda: p135_hide_old_ui(self))
        self.root.after(1200, lambda: p135_hide_old_ui(self))
    except Exception:
        p135_hide_old_ui(self)

    try:
        self.root.bind("<F12>", self.p135_new_groove)
        self.root.bind("<Control-e>", self.p135_export)
    except Exception:
        pass

    p135_status(self, "v135 : Princess iTunes UI. Grille intacte.")


SliceIndexTracker.p135_new_groove = p135_new_groove
SliceIndexTracker.p135_duplicate_32 = p135_duplicate_32
SliceIndexTracker.p135_loop = p135_loop
SliceIndexTracker.p135_export = p135_export

SliceIndexTracker.build_ui = p135_build_ui



# ---------------------------------------------------------------------
# v136 : fix focus menu déroulant / flèches
# ---------------------------------------------------------------------
# Problème :
# après clic sur le choix du break, la Combobox garde le focus.
# Donc les flèches changent encore le break au lieu de bouger les notes.
#
# v136 :
# - après sélection d'un break => focus retourne à la grille
# - après "Charger break" => focus retourne à la grille
# - si multisélection active, les flèches ne contrôlent jamais la Combobox
# ---------------------------------------------------------------------


def v136_walk(widget):
    out = [widget]
    try:
        for child in widget.winfo_children():
            out.extend(v136_walk(child))
    except Exception:
        pass
    return out


def v136_text(widget):
    try:
        return str(widget.cget("text")).strip()
    except Exception:
        return ""
...
def v136_canvas(self):
    canvas = getattr(self, "canvas", None)
    if canvas is not None:
        return canvas

    for widget in v136_walk(self.root):
        try:
            if isinstance(widget, tk.Canvas):
                return widget
        except Exception:
...
def v136_focus_grid(self, event=None):
    canvas = v136_canvas(self)
...
def v136_focus_grid_later(self, delay=80):
    try:
        self.root.after(delay, lambda: v136_focus_grid(self))
    except Exception:
        v136_focus_grid(self)


def v136_has_multiselect(self):
    for attr in [
        "selected_ids_v128",
        "selected_ids_v129",
        "selected_ids_v127",
...
def v136_call_multiselect_arrow(self, direction):
    """
    Quand la Combobox a le focus mais qu'une slice est multisélectionnée,
    les flèches doivent bouger la sélection, pas changer le break.
    """
    methods = {
        "Left":  ["v130_key_left", "v128_key_left", "v125_key_left"],
        "Right": ["v130_key_right", "v128_key_right", "v125_key_right"],
        "Up":    ["v130_key_up", "v128_key_up", "v125_key_up"],
        "Down":  ["v130_key_down", "v128_key_down", "v125_key_down"],
    }

    for name in methods.get(direction, []):
        fn = getattr(self, name, None)
        if callable(fn):
            try:
                return fn(None)
...
def v136_arrow_from_combobox(self, direction, event=None):
    if v136_has_multiselect(self):
        v136_focus_grid(self)
        return v136_call_multiselect_arrow(self, direction)

    # Même sans multisélection, on évite que le break change par accident.
    v136_focus_grid(self)
    v136_status(self, "Focus revenu sur la grille. Clique une slice/note puis utilise les flèches.")
    return "break"


def v136_bind_break_widgets(self):
    """
    Trouve les Combobox/Entry du choix de break et leur enlève le focus
    dès qu'on a fini de cliquer/sélectionner.
    """
    count = 0
    ...
    for widget in v136_walk(self.root):
        ...
        if not is_combo and not is_entry:
            continue

        # On cible surtout le sélecteur de break :
        # il a une valeur qui ressemble à un nom de break.
        value = ""
        try:
        ...
        maybe_break_selector = (
            "_Break" in value
            or "Break_" in value
            or "_-" in value
            or "Camo" in value
            or "Amen" in value
            or "Champion" in value
            or "Change" in value
        )

        if not maybe_break_selector and is_entry:
            continue

        try:
            widget.bind("<<ComboboxSelected>>", lambda e: v136_focus_grid_later(self, 60), add="+")
        except Exception:
            pass

        try:
            widget.bind("<ButtonRelease-1>", lambda e: v136_focus_grid_later(self, 140), add="+")
            widget.bind("<Return>", lambda e: v136_focus_grid_later(self, 20), add="+")
            widget.bind("<Escape>", lambda e: v136_focus_grid(self), add="+")
        except Exception:
            pass

        # Flèches dans le sélecteur = jamais changer le break par accident.
        try:
            widget.bind("<Left>", lambda e: v136_arrow_from_combobox(self, "Left", e), add=False)
            widget.bind("<Right>", lambda e: v136_arrow_from_combobox(self, "Right", e), add=False)
            widget.bind("<Up>", lambda e: v136_arrow_from_combobox(self, "Up", e), add=False)
            widget.bind("<Down>", lambda e: v136_arrow_from_combobox(self, "Down", e), add=False)
        except Exception:
            pass

        count += 1

    print(f"[v138] break widgets focus-fix bindés : {count}")


def v136_patch_charger_break_button(self):
    """
    Après Charger break, l'ancien bouton peut redonner le focus au menu.
    On wrap le bouton pour rendre le focus à la grille.
    """
    patched = 0

    for widget in v136_walk(self.root):
        if v136_text(widget).lower() != "charger break":
            continue
        ...
        def wrapped(cmd=old_cmd):
            try:
                ...
                print("[v138] Charger break ancien callback erreur :", exc)

            v136_focus_grid_later(self, 120)
            v136_bind_break_widgets(self)
    ...
    print(f"[v138] bouton Charger break patché : {patched}")


_old_v136_build_ui = SliceIndexTracker.build_ui


def v136_build_ui(self):
    _old_v136_build_ui(self)
    ...
    try:
        v136_bind_break_widgets(self)
        v136_patch_charger_break_button(self)
    except Exception as exc:
        print("[v138] focus fix install impossible :", exc)

    # Plusieurs passes parce que la UI peut recréer certains widgets après build.
    try:
        self.root.after(300, lambda: v136_bind_break_widgets(self))
        self.root.after(800, lambda: v136_bind_break_widgets(self))
        self.root.after(1200, lambda: v136_focus_grid(self))
    except Exception:
        pass

    v136_status(self, "v136 : le menu break ne vole plus les flèches.")


SliceIndexTracker.build_ui = v136_build_ui



# ---------------------------------------------------------------------
# v137 : remet Export WAV dans l'UI
# ---------------------------------------------------------------------
# Base : v136.
# Changement unique :
# - bouton Export WAV visible et persistant
# - il utilise le bon export v122/v121 : WAV only, sans JSON
# - ne touche pas à la grille
# - ne touche pas au focus des flèches
# ---------------------------------------------------------------------


def v137_walk(widget):
    out = [widget]
    try:
        for child in widget.winfo_children():
            out.extend(v137_walk(child))
    except Exception:
        pass
    return out


def v137_text(widget):
    try:
        return str(widget.cget("text")).strip()
    except Exception:
        return ""
...
def v137_export(self, event=None):
    """
    Export WAV fiable :
    priorité au moteur v122/v121 qui marchait :
    WAV seulement, long propre raccourci, pas de JSON.
    """
    for name in [
        "export_wav_current_v122",
        "export_wav_current_v121",
    ...
    for name in ["v121_export_trimmed_long", "v122_export_trimmed_long_no_json", "p135_export", "v134_export"]:
        fn = globals().get(name)
        if callable(fn):
            return fn(self, event)

    print("[v138] ERREUR : moteur Export WAV introuvable.")
    v137_status(self, "Export WAV introuvable.")
    return "break"


def v137_button(parent, text, command):
    return tk.Button(
        parent,
        text=text,
        command=command,
        ...
        cursor="hand2",
        highlightthickness=1,
        highlightbackground="#79e6b2",
    )


def v137_find_princess_toolbar(self):
    for attr in ["p135_toolbar", "v134_toolbar", "v132_toolbar"]:
    ...
    # Sinon, cherche un frame contenant "Princess Breakbeat".
    for widget in v137_walk(self.root):
        try:
        ...
        for child in v137_walk(widget):
            if "Princess Breakbeat" in v137_text(child):
                return widget

    return None


def v137_export_button_exists(self):
    for widget in v137_walk(self.root):
        if v137_text(widget).lower() in ["export wav", "export to wav"]:
            try:
...
def v137_restore_export_button(self):
    if getattr(self, "_v137_export_button_done", False):
        try:
            btn = getattr(self, "v137_export_button", None)
    ...
    # Si un Export WAV visible existe déjà, ne duplique pas.
    if v137_export_button_exists(self):
        self._v137_export_button_done = True
        return

    toolbar = v137_find_princess_toolbar(self)

    if toolbar is not None:
        try:
            btn = v137_button(toolbar, "Export WAV", self.v137_export)
            btn.pack(side="left", padx=4, pady=6)
            btn.lift()
            self.v137_export_button = btn
            self._v137_export_button_done = True
            print("[v138] Export WAV ajouté dans la toolbar.")
            v137_status(self, "v137 : Export WAV restauré.")
            return
        except Exception as exc:
            print("[v138] ajout toolbar impossible :", exc)

    # Fallback ultra visible, en haut à droite.
    try:
        btn = v137_button(self.root, "Export WAV", self.v137_export)
        btn.place(relx=1.0, x=-18, y=86, anchor="ne")
        btn.lift()
        self.v137_export_button = btn
        self._v137_export_button_done = True
        print("[v138] Export WAV placé en fallback.")
        v137_status(self, "v137 : Export WAV restauré.")
    except Exception as exc:
        print("[v138] placement fallback impossible :", exc)


_old_v137_build_ui = SliceIndexTracker.build_ui


def v137_build_ui(self):
    _old_v137_build_ui(self)
    ...
    try:
        v137_restore_export_button(self)
        self.root.after(300, lambda: v137_restore_export_button(self))
        self.root.after(900, lambda: v137_restore_export_button(self))
        self.root.after(1500, lambda: v137_restore_export_button(self))
    except Exception as exc:
        print("[v138] restore export impossible :", exc)

    try:
        self.root.bind("<Control-e>", self.v137_export)
    except Exception:
        pass


SliceIndexTracker.v137_export = v137_export

# On repointe aussi les noms usuels vers le bouton restauré.
SliceIndexTracker.export_wav_current_v137 = v137_export

SliceIndexTracker.build_ui = v137_build_ui



# ---------------------------------------------------------------------
# v138 : toolbar principale fixe
# ---------------------------------------------------------------------
# Répare :
# - bouton Nouveau groove disparu
# - bouton Export WAV disparu
#
# Cette toolbar est indépendante de l'ancienne UI.
# Elle est posée directement sur root et relevée régulièrement.
# ---------------------------------------------------------------------
...
def v138_new_groove(self, event=None):
    # Force le générateur strict restauré.
    fn = globals().get("v131_generate_candidate")
    if callable(fn):
        return fn(self, event)
    ...
    print("[v138] Nouveau groove introuvable.")
    v138_status(self, "Nouveau groove introuvable.")
    return "break"


def v138_duplicate_32(self, event=None):
    for name in ["v134_duplicate_32", "p135_duplicate_32"]:
        fn = getattr(self, name, None)
        if callable(fn):
            return fn(event)
    ...
    if not first:
        v138_status(self, "Aucun premier 32 à dupliquer.")
        return "break"
    ...
    try:
        self.write_latest_pattern(reason="v138_duplicate_32")
    except Exception:
        pass

    v138_status(self, "Miroir 32 appliqué.")
    return "break"


def v138_loop(self, event=None):
    # Bouton Loop / Space existant.
    def walk(widget):
        out = [widget]
        try:
            for child in widget.winfo_children():
    ...
    for widget in walk(self.root):
        try:
            if str(widget.cget("text")).strip().lower() == "loop / space":
                widget.invoke()
                return "break"
        except Exception:
            pass
    ...
    v138_status(self, "Loop / Space introuvable.")
    return "break"


def v138_export(self, event=None):
    """
    Cherche d'abord le bon export WAV-only qui venait de v122/v121,
    y compris s'il a été renommé par les versions suivantes.
    """

    # Fonctions globales créées par les versions successives.
    for n in range(138, 120, -1):
        for name in [
            f"v{n}_export_trimmed_long_no_json",
            f"v{n}_export_trimmed_long",
            f"v{n}_export_wav",
        ]:
            fn = globals().get(name)
            if callable(fn) and name != "v138_export":
                try:
                    print(f"[v138] export via global {name}")
                    return fn(self, event)
                except TypeError:
                    try:
                        return fn(self)
                    ...
                    print(f"[v138] {name} erreur :", exc)

    # Méthodes attachées à l'objet.
    for n in range(138, 109, -1):
        for name in [
            f"export_wav_current_v{n}",
            f"export_wav_v{n}",
            f"v{n}_export",
            f"p135_export",
            f"v134_export",
        ]:
            if name in ["v138_export", "export_wav_current_v138"]:
                continue
            ...
            if callable(fn):
                try:
                    print(f"[v138] export via méthode {name}")
                    return fn(event)
                except TypeError:
                    try:
                        return fn()
    ...
    print("[v138] ERREUR : aucun moteur Export WAV trouvé.")
    v138_status(self, "Export WAV introuvable.")
    return "break"


def v138_button(parent, text, command, bg, fg="#ffffff"):
    return tk.Button(
        parent,
        text=text,
        command=command,
        ...
        cursor="hand2",
        highlightthickness=1,
        highlightbackground="#8d4f96",
    )


def v138_install_toolbar(self):
    if getattr(self, "_v138_toolbar_done", False):
        try:
            self.v138_toolbar.lift()
        except Exception:
            pass
        return
    ...
    title_box = tk.Frame(toolbar, bg=V138_PANEL)
    ...
    buttons = tk.Frame(toolbar, bg=V138_PANEL)
    buttons.pack(side="left", padx=6, pady=6)

    v138_button(buttons, "Nouveau groove", self.v138_new_groove, V138_PURPLE).pack(side="left", padx=4)
    v138_button(buttons, "Miroir 32", self.v138_duplicate_32, "#30223e").pack(side="left", padx=4)
    v138_button(buttons, "Loop / Space", self.v138_loop, V138_BLUE).pack(side="left", padx=4)
    v138_button(buttons, "Export WAV", self.v138_export, V138_GREEN).pack(side="left", padx=4)

    try:
        toolbar.place(relx=1.0, x=-12, y=28, anchor="ne")
    except Exception:
        toolbar.pack(side="top", anchor="e", padx=8, pady=4)

    toolbar.lift()

    self.v138_toolbar = toolbar
    self._v138_toolbar_done = True

    print("[v138] toolbar fixe installée : Nouveau groove / Export WAV visibles.")
    v138_status(self, "v138 : boutons Nouveau groove et Export WAV restaurés.")


def v138_keep_toolbar_visible(self):
    try:
        if getattr(self, "v138_toolbar", None) is not None:
            self.v138_toolbar.lift()
    except Exception:
        pass


_old_v138_build_ui = SliceIndexTracker.build_ui


def v138_build_ui(self):
    _old_v138_build_ui(self)
    ...
    try:
        v138_install_toolbar(self)
    except Exception as exc:
        print("[v138] toolbar impossible :", exc)

    try:
        self.root.after(300, lambda: v138_install_toolbar(self))
        self.root.after(900, lambda: v138_install_toolbar(self))
        self.root.after(1500, lambda: v138_keep_toolbar_visible(self))
        self.root.after(2500, lambda: v138_keep_toolbar_visible(self))
    except Exception:
        pass

    try:
        self.root.bind("<F12>", self.v138_new_groove)
        self.root.bind("<Control-e>", self.v138_export)
    except Exception:
        pass


SliceIndexTracker.v138_new_groove = v138_new_groove
SliceIndexTracker.v138_duplicate_32 = v138_duplicate_32
SliceIndexTracker.v138_loop = v138_loop
SliceIndexTracker.v138_export = v138_export
SliceIndexTracker.export_wav_current_v138 = v138_export

SliceIndexTracker.build_ui = v138_build_ui
