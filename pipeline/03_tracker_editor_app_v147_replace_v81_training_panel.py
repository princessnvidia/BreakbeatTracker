...
"""
BreakbeatAI Tracker Editor v147 — slice index
...
def warp_audio_to_length_external(audio, target_len, cache_key=None):
    ...
    if rubberband:
        try:
            ...
            print(f"[v147] rubberband échec, fallback ffmpeg/no-speed : {exc}")
    ...
    if ffmpeg:
        try:
            ...
            print(f"[v147] ffmpeg atempo échec, fallback no-speed : {exc}")
...
class SliceIndexTracker:
    def __init__(self, root, pair_json):
        ...
        self.root.title("BreakbeatAI Tracker Editor v147 — slice index")
    ...
    def detect_external_player(self):
        for name in ["pw-play", "paplay", "aplay", "ffplay"]:
            ...
            if path:
                print(f"[v147] Backend audio : {name} -> {path}")
                return name

        print("[v147] Aucun backend système trouvé, fallback sounddevice.")
        return None
    ...
    def build_initial_pattern(self, randomize=False):
        ...
        print(
            f"[v147] Génération pattern | break={self.project['safe']} | "
        ...
        for slot in range(max_items):
            ...
            print(f"[v147] slot {slot:02d} | step {x_step:02d} -> slice {pair:02d}")
    ...
    def build_ui(self):
        ...
        title = tk.Label(
            main,
            text="BreakbeatAI v147 — replace v81 training panel",
            bg="#111018",
            fg="#ff7acc",
            font=("Sans", 20, "bold"),
        )
    ...
    def record_correction(self, event_type, before=None, after=None):
        ...
        with corrections_path.open("a", encoding="utf-8") as f:
            f.write(json.dumps(event, ensure_ascii=False) + "[v147]n")
        ...
        self.output_label.config(
            text=(
                f"Auto-learn sauvegardé : {event_type}[v147]n"
                f"Correction : {corrections_path}[v147]n"
                f"Mémoire : {MEMORY_PATH}[v147]n"
                f"Latest : {latest_path}"
            )
        )

        print(f"[v147] correction sauvegardée : {event_type}")
    ...
    def randomize_pattern(self):
        ...
        self.output_label.config(text=f"Randomize OK. Mémoire appliquée.[v147]nLatest : {latest}")
    ...
    def play_audition_array(self, audio, label="audition"):
        ...
        audition_wav = OUT_DIR / f"{self.project['safe']}_v147_audition.wav"
        ...
        if self.external_player:
            try:
                ...
                print(f"[v147] Erreur audition {self.external_player} : {exc}")
    ...
    def on_close(self):
        ...
        """
        print("[v147] Fermeture : arrêt audio + destruction fenêtre")
    ...
    def play_audio_array(self, audio, label="audio"):
        ...
        audio = normalize(audio)
        live_wav = OUT_DIR / f"{self.project['safe']}_v147_live.wav"
        ...
        if self.external_player:
            try:
                ...
                print(f"[v147] Erreur backend {self.external_player} : {exc}")
    ...
    def render_preview_file(self):
        OUT_DIR.mkdir(parents=True, exist_ok=True)
        wav = OUT_DIR / f"{self.project['safe']}_tracker_app_v147_preview.wav"
    ...
    def write_latest_pattern(self, reason="correction"):
        ...
        path = LATEST_DIR / f"{self.project['safe']}_latest_slice_index_v147.json"

        data = {
            "version": "breakbeatai_latest_slice_index_v147",
            "reason": reason,
    ...
    def save(self):
        ...
        tracker_path = OUT_DIR / f"{self.project['safe']}_tracker_app_edit_v147.json"
        ...
        stamp = file_stamp()
        validated_path = VALIDATED_DIR / f"{self.project['safe']}_validated_slice_index_loop64_v147_{stamp}.json"

        validated = {
            "version": "breakbeatai_validated_slice_index_loop64_v147",
        ...
        self.output_label.config(
            text=(
                f"OK sauvegardé[v147]n"
                f"Tracker : {tracker_path}[v147]n"
                f"Validation IA : {validated_path}[v147]n"
                f"Latest : {latest}[v147]n"
                f"Preview : {wav}"
            )
        )
...
def v44_record_correction(self, event_type, before=None, after=None):
    ...
    print(f"[v147] correction sauvegardée : {event_type}")
    print(f"[v147] correction path : {corrections_path}")
    print(f"[v147] latest path : {latest_path}")
...
def v45_move_selected(self, dx, dy):
    ...
    if dy != 0 and before != after:
        try:
            ...
            print(f"[v147] audition après flèche haut/bas impossible : {exc}")
...
def v46_warp_loop_pitch_preserve(audio, target_len, cache_key):
    ...
    if rubberband:
        try:
            ...
            print(f"[v147] rubberband global warp échec : {exc}")
    ...
    if ffmpeg:
        try:
            ...
            print(f"[v147] ffmpeg global warp échec : {exc}")

    print("[v147] Aucun warp pitch-preserve dispo, fallback fit no-speed.")
    return v46_fit_audio_no_speed(audio, target_len)
...
def v46_build_initial_pattern(self, randomize=False):
    ...
    print(
        f"[v147] M8 pattern | break={self.project['safe']} | "
...
def v46_get_warped_loop(self):
    ...
    print(
        f"[v147] global warp OK | source_len={loop_end-loop_start} | "
...
def v46_build_ui(self):
    ...
    try:
        ...
        print(f"[v147] UI extension impossible : {exc}")
...
def v47_play_audio_array(self, audio, label="audio"):
    ...
    if len(audio) == 0 or v47_is_silent(audio):
        ...
        print(f"[v147] lecture annulée : audio silencieux ({label})")
        return 0
...
def v47_toggle_loop(self):
    ...
    if not getattr(self, "pattern", None):
        ...
        print("[v147] loop annulée : pattern vide")
        return "break"
...
def v47_delete_selected(self):
    ...
    if not getattr(self, "pattern", None):
        ...
        print("[v147] pattern vide après suppression : audio coupé")
...
def v48_build_initial_pattern(self, randomize=False):
    ...
    print("[v147] pattern initial forcé VIDE : aucun son ne doit jouer.")
    return []
...
def v48_load_first_slices(self):
    ...
    print(f"[v147] Load 16 slices : {len(self.pattern)} items")


def v48_debug_pattern(self, label="render"):
    print("")
    print(f"[v147] DEBUG PATTERN avant {label}")
    print(f"[v147] items = {len(getattr(self, 'pattern', []) or [])}")

    if not getattr(self, "pattern", None):
        print("[v147] pattern vide confirmé")
        print("")
        return
    ...
    for item in self.pattern:
        ...
        print(
            f"[v147] id={item_id} | step {x:02d}/{end:02d} | "
...
def v48_render_audio_with_timeline(self):
    ...
    if not getattr(self, "pattern", None):
        loop_samples = self.get_loop_samples()
        print(f"[v147] rendu silence total : pattern vide | samples={loop_samples}")
        return np.zeros(loop_samples, dtype=np.float32), []
    ...
    print(f"[v147] rendu audio peak={peak:.8f} | timeline_events={len(timeline) if timeline is not None else 'None'}")

    if timeline:
        print("[v147] timeline events:")
        for ev in timeline:
            print(f"[v147]   {ev}")
...
def v48_toggle_loop(self):
    if not getattr(self, "pattern", None):
        ...
        print("[v147] Space ignoré : pattern vide.")
        return "break"
...
def v48_build_ui(self):
    ...
    try:
        ...
        print(f"[v147] ajout UI impossible : {exc}")
...
def v49_add_item_at_xy(self, x, y):
    ...
    try:
        ...
        print(f"[v147] record_correction add impossible : {exc}")
    ...
    try:
        ...
        print(f"[v147] audition après ajout impossible : {exc}")

    print(f"[v147] add | id={item['id']} | step={x_step} | pair={pair} | lane={lane}")
...
def v49_duplicate_selected_forward(self, event=None):
    ...
    if item is None:
        ...
        print("[v147] duplicate annulé : aucune note sélectionnée")
        return "break"
    ...
    if new_x > max_x:
        ...
        print("[v147] duplicate annulé : fin de grille")
        return "break"
    ...
    try:
        ...
        print(f"[v147] record_correction duplicate impossible : {exc}")
    ...
    try:
        ...
        print(f"[v147] audition après duplicate impossible : {exc}")

    print(
        f"[v147] duplicate | old_id={item.get('id')} | new_id={new_item.get('id')} | "
...
def v49_build_ui(self):
    ...
    try:
        ...
        print(f"[v147] bind canvas click impossible : {exc}")
    ...
    try:
        ...
        print(f"[v147] bind keys impossible : {exc}")
...
def v50_load_style_model():
    ...
    try:
        ...
        print(f"[v147] impossible de charger le modèle IA : {exc}")
        return None
...
def v50_generate_ai_pattern(self, event=None):
    ...
    if model is None:
        ...
        print("[v147] modèle absent :", STYLE_MODEL_PATH)
        return "break"
    ...
    print("")
    print(f"[v147] GENERATE IA | safe={safe} | pairs={pair_values} | slots={max_slots}")
    ...
    for slot in range(max_slots):
        ...
        print(
            f"[v147] slot {slot:02d} | step {x_step:02d} | "
    ...
    try:
        ...
        print(f"[v147] write_latest_pattern impossible : {exc}")
...
def v50_build_ui(self):
    ...
    try:
        ...
        print(f"[v147] UI IA impossible : {exc}")
...
def v51_mouse_down(self, event):
    ...
    if item is not None:
        ...
        print(
            f"[v147] mouse down note | id={item.get('id')} | "
            f"step={item.get('x_step')} | pair={item.get('pair')}"
        )
    else:
        print("[v147] mouse down empty")
...
def v51_mouse_up(self, event):
    ...
    if state.get("empty"):
        ...
        print("[v147] drag depuis vide ignoré")
        return "break"
    ...
    if before != after:
        try:
            ...
            print(f"[v147] record_correction drag impossible : {exc}")
        ...
        print(
            f"[v147] drag move | id={after.get('id')} | "
            f"step {before.get('x_step')} -> {after.get('x_step')} | "
        ...
        try:
            ...
            print(f"[v147] audition après drag impossible : {exc}")
...
def v51_build_ui(self):
    ...
    try:
        ...
        print(f"[v147] bind souris impossible : {exc}")
    ...
    try:
        ...
        print(f"[v147] bind Suppr impossible : {exc}")
...
def v52_normalize_loaded_notes(self, notes):
    ...
    for raw in sorted(notes, key=lambda n: (int(float(n.get("x_step", 0))), int(float(n.get("pair", 0))))):
        ...
        if pair not in valid_pairs:
            print(f"[v147] slice ignorée car absente du break actuel : pair={pair}")
            continue
...
def v52_load_saved_beat(self, event=None):
    ...
    if not saves:
        ...
        print("[v147] aucune sauvegarde trouvée")
        return "break"
    ...
    if not pattern:
        ...
        print("[v147] sauvegarde incompatible :", chosen["path"])
        return "break"
    ...
    print("")
    print("[v147] BEAT RESTAURÉ")
    print("[v147] fichier :", chosen["path"])
    print("[v147] clé :", chosen["key"])
    print("[v147] notes :", len(pattern))
    print("")
...
def v52_list_saved_beats(self, event=None):
    ...
    print("")
    print("[v147] SAUVEGARDES TROUVÉES")
    for i, save in enumerate(saves[:30]):
        print(
            f"[v147] {i:02d} | notes={len(save['notes']):02d} | "
...
def v52_build_ui(self):
    ...
    try:
        ...
        print(f"[v147] UI load saved impossible : {exc}")
...
def v53_run_training(self):
    ...
    if not trainer.exists():
        ...
        print("[v147] trainer introuvable :", trainer)
        return False

    print("")
    print("[v147] AUTO-TRAIN IA après Save...")
    print("")
    ...
    try:
        ...
        if result.returncode != 0:
            ...
            print("[v147] auto-train échec code :", result.returncode)
            return False

        v53_status(self, "Save OK + IA entraînée automatiquement.")
        print("[v147] AUTO-TRAIN OK")
        return True

    except Exception as exc:
        v53_status(self, f"Save OK, mais auto-train impossible : {exc}")
        print("[v147] exception auto-train :", exc)
        return False
...
def v53_save(self):
    ...
    try:
        ...
        print(f"[v147] auto-train après save impossible : {exc}")
...
def v53_build_ui(self):
    ...
    try:
        ...
        print(f"[v147] UI auto-train impossible : {exc}")
...
def v55_mouse_up(self, event):
    ...
    if not state.get("moved"):
        ...
        try:
            ...
            print(f"[v147] audition clic note impossible : {exc}")
        ...
        print(
            f"[v147] click play/select | id={item.get('id')} | "
            f"step={item.get('x_step')} | pair={item.get('pair')}"
        )
...
def v58_add_item_at_xy(self, x, y):
    ...
    print(
        f"[v147] add | id={item['id']} | step={x_step} | "
...
def v58_toggle_selected_length(self, event=None):
    ...
    try:
        ...
        print(f"[v147] record_correction length impossible : {exc}")

    v58_refresh_after_edit(self, "toggle_note_length")

    v58_status(self, f"Longueur note : {old_len} -> {new_len} case(s).")
    print(f"[v147] length toggle | id={item.get('id')} | {old_len} -> {new_len}")
...
def v58_duplicate_selected_forward(self, event=None):
    ...
    try:
        ...
        print(f"[v147] audition duplicate impossible : {exc}")
...
def v58_build_ui(self):
    ...
    try:
        ...
        print(f"[v147] UI demi-grille impossible : {exc}")
    ...
    try:
        ...
        print(f"[v147] bind demi-grille impossible : {exc}")
...
def v59_rank_current_slices(self):
    ...
    for pair in self.pair_values:
        ...
        try:
            ...
            print(f"[v147] analyse pair {pair} impossible : {exc}")
            role = "other"
    ...
    print("")
    print("[v147] TOP ROLES CURRENT BREAK")
    for r in ("kick", "snare", "hat"):
        print(f"[v147] {r:5s}:", [(x["pair"], round(x["score"], 3)) for x in ranks[r][:6]])
    print("")
...
def v59_load_role_model():
    ...
    try:
        ...
        print(f"[v147] lecture modèle rôle impossible : {exc}")
        return None
...
def v59_generate_role_aware(self, event=None):
    ...
    if model is None:
        ...
        print("[v147] modèle rôle absent, fallback musical snare 6.")
        model = {
            "breaks": {},
            "global": {},
            "patterns_used": 0,
    ...
    print("")
    print("[v147] ROLE-AWARE GENERATION")
    print("[v147] model patterns_used:", model.get("patterns_used", 0))
    for item in pattern:
        print(
            f"[v147] step {item['x_step']:02d}/{item['x_step'] + item['length'] - 1:02d} | "
...
def v59_run_role_training(self, event=None):
    ...
    try:
        ...
        print(f"[v147] training rôle impossible : {exc}")
...
def v59_save(self):
    ...
    try:
        ...
        print(f"[v147] auto-training rôle après save impossible : {exc}")
...
def v59_build_ui(self):
    ...
    try:
        ...
        print(f"[v147] UI role-aware impossible : {exc}")
...
def v62_add_item_at_xy(self, x, y):
    ...
    print(f"[v147] add | id={item['id']} | step={x_step} | len={length} | pair={pair}")
...
def v62_mouse_down(self, event):
    ...
    if item is not None:
        ...
        print(
            f"[v147] mouse down {state['mode']} | id={item.get('id')} | "
            f"step={item.get('x_step')} | len={item.get('length')} | pair={item.get('pair')}"
        )
    else:
        print("[v147] mouse down empty")
    ...
    if mode == "resize":
        if not state.get("moved"):
            ...
            try:
                ...
                print(f"[v147] audition clic resize impossible : {exc}")
        ...
        if before != after:
            try:
                ...
                print(f"[v147] record_correction resize impossible : {exc}")
            ...
            try:
                ...
                print(f"[v147] audition après resize impossible : {exc}")
    ...
    if mode == "move":
        ...
        if before != after:
            try:
                ...
                print(f"[v147] record_correction move impossible : {exc}")
            ...
            try:
                ...
                print(f"[v147] audition après move impossible : {exc}")
...
def v62_toggle_selected_length(self, event=None):
    ...
    try:
        ...
        print(f"[v147] record_correction toggle length impossible : {exc}")

    v62_refresh_after_edit(self, "toggle_note_length_v62")
    v62_status(self, f"Longueur note : {old_len} → {new_len} case(s).")

    print(f"[v147] toggle length | id={item.get('id')} | {old_len} -> {new_len}")
...
def v62_draw(self):
    ...
    try:
        ...
        print(f"[v147] draw handles impossible : {exc}")
...
def v62_build_ui(self):
    ...
    try:
        ...
        print(f"[v147] bind resize impossible : {exc}")
    ...
    try:
        ...
        print(f"[v147] UI resize impossible : {exc}")
...
def v63_load_model():
    ...
    try:
        ...
        print(f"[v147] lecture modèle strict impossible : {exc}")
        return None
...
def v63_generate_safe_role_ai(self, event=None):
    ...
    print("")
    print("[v147] SAFE ROLE AI GENERATED")
    print("[v147] model strict:", "yes" if model else "no")
    print("[v147] snare main/alt:", main_snare, alt_snare)
    print("[v147] kick  main/alt:", main_kick, alt_kick)
    print("[v147] hat   main/alt:", main_hat, alt_hat)
    print("[v147] pattern:")
    for item in pattern:
        print(
            f"[v147] step {item['x_step']:02d}/{item['x_step'] + item['length'] - 1:02d} | "
...
def v63_run_strict_training(self, event=None):
    ...
    print("")
    print("[v147] TRAIN STRICT ROLE MODEL...")
    print("")
    ...
    try:
        ...
        print(f"[v147] training strict impossible : {exc}")
...
def v63_build_ui(self):
    ...
    try:
        ...
        print(f"[v147] UI safe impossible : {exc}")
...
def v65_rank_slices(self):
    ...
    for pair in self.pair_values:
        ...
        try:
            ...
            print(f"[v147] analyse slice {pair} impossible : {exc}")
            scores = {
                "kick": 0.0,
                "snare": 0.0,
                "hat": 0.0,
    ...
    print("")
    print("[v147] TOP SLICES PAR ROLE — break courant")
    for role in ("kick", "snare", "hat", "ghost_snare"):
        print(f"[v147] {role:12s}:", [(x["pair"], round(x["score"], 3)) for x in ranks[role][:8]])
    print("")
...
def v65_generate_from_role_template(self, event=None):
    ...
    try:
        ...
        print(f"[v147] template introuvable : {exc}")
        return "break"
    ...
    print("")
    print("[v147] CROSS-SONG ROLE TEMPLATE GENERATED")
    print("[v147] template:", V65_TEMPLATE_PATH)
    print("[v147] temperature:", temperature, "| hats:", hat_density, "| ghosts:", ghost_density)
    print("[v147] pattern:")
    for item in pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v147] step {item['x_step']:02d}/{end:02d} | "
...
def v65_build_ui(self):
    ...
    try:
        ...
        print(f"[v147] UI impossible : {exc}")
...
def v67_train_strict_if_available(self):
    ...
    if trainer is None:
        print("[v147] aucun trainer trouvé.")
        return False

    print("")
    print("[v147] TRAIN après validation :", trainer)
    print("")
    ...
    try:
        ...
        print(f"[v147] training impossible : {exc}")
        return False
...
def v67_generate_candidate(self, event=None):
    ...
    print("")
    print("[v147] CANDIDATE GENERATED")
    print("[v147] break:", self.v67_candidate_safe)
    print("[v147] notes:", len(candidate))
    print("[v147] Maintenant : corrige puis clique Good après modifs, ou Reject.")
    print("")
...
def v67_reject_candidate(self, event=None):
    ...
    print("")
    print("[v147] REJECTED")
    print("[v147] saved:", rejected_path)
    print("")
...
def v67_accept_corrected(self, event=None):
    ...
    print("")
    print("[v147] ACCEPTED GOOD AFTER EDIT")
    print("[v147] saved:", validated_path)
    print("[v147] trained:", trained)
    print("")
...
def v67_accept_without_training(self, event=None):
    ...
    print("[v147] accepted no-train:", validated_path)
...
def v67_build_ui(self):
    ...
    try:
        ...
        print(f"[v147] UI feedback impossible : {exc}")
...
def v68_generate_candidate(self, event=None):
    ...
    print("[v147] Length policy appliquée : 2 cases par défaut, 1 case rare.")
...
def v69_force_pattern_hard_2_grid(self, reason="v69_force_hard_2_grid"):
    ...
    print("")
    print("[v147] HARD 2 GRID APPLIQUÉ")
    for item in self.pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v147] step {item['x_step']:02d}/{end:02d} | "
...
def v70_duplicate_selected_forward(self, event=None):
    ...
    print("")
    print("[v147] CTRL+D DUPLICATE +2")
    print(f"[v147] old_id={item.get('id')} new_id={new_item.get('id')}")
    print(f"[v147] step {old_x}/{old_x + length - 1} -> {new_x}/{new_x + length - 1}")
    print(f"[v147] pair={new_item.get('pair')} len={length}")
    print("")
...
def v70_build_ui(self):
    ...
    try:
        ...
        print(f"[v147] bind Ctrl+D impossible : {exc}")
...
def v71_rank_slices(self):
    ...
    print("")
    print("[v147] PRE-LEARNING TOP SLICES")
    for role in ("kick", "snare", "hat", "ghost_snare"):
        print(f"[v147] {role:12s}:", [(x["pair"], round(x["score"], 3)) for x in ranks[role][:8]])
    print("")
...
def v71_prelearn_roles(self, force=False):
    ...
    print("")
    print("[v147] PRELEARN ROLES POUR", safe)
    for role in ("kick", "snare", "hat", "ghost_snare", "bad"):
        print(f"[v147] {role:12s}:", roles.get(role, []))
    print("")
...
def v71_generate_full_candidate(self, event=None):
    ...
    print("")
    print("[v147] FULL CANDIDATE GENERATED")
    print("[v147] break:", v71_safe(self))
    print("[v147] roles:", roles)
    print("[v147] pattern:")
    for item in pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
...
def v72_open_break(self, safe=None):
    ...
    print("")
    print("[v147] OPEN BREAK")
    print("[v147] current:", current)
    print("[v147] next   :", safe)
    print("[v147] cmd    :", " ".join(cmd))
    print("")
...
def v72_build_ui(self):
    ...
    try:
        ...
        print(f"[v147] UI break switcher impossible : {exc}")
...
def v73_sanitize_tracker_pattern(self, reason="v73_tracker_strict_no_overlap"):
    ...
    print("")
    print("[v147] TRACKER STRICT NO OVERLAP")
    print("[v147] kept:", len(accepted), "| rejected overlaps:", len(rejected))

    for item in accepted:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v147] KEEP step {item['x_step']:02d}/{end:02d} | "
    ...
    if rejected:
        ...
        for item in rejected:
            ...
            print(
                f"[v147] DROP step {item['x_step']:02d}/{end:02d} | "
                f"role={item.get('v73_role_key')} | pair={item.get('pair')}"
            )
...
def v81_launch_break(self, source):
    ...
    print("")
    print("[v147] CHARGER BREAK")
    print("[v147] courant :", current)
    print("[v147] nouveau :", source)
    print("[v147] cmd     :", " ".join(cmd))
    print("")
...
def v81_patch_charger_break_buttons(self):
    ...
    print(f"[v147] boutons Charger break patchés : {patched}")
...
def v81_generate_candidate(self, event=None):
    ...
    print("")
    print("[v147] GENERATE CANDIDATE")
    print("[v147] break chargé :", current)
    print("[v147] générateur   :", getattr(fn, "__name__", None))
    print("")
...
def v81_run_training(self):
    ...
    if script is None:
        print("[v147] Aucun script training trouvé.")
...
def v81_save_feedback(self, verdict, train_now=False):
    ...
    print("")
    print("[v147] FEEDBACK SAUVÉ")
    print("[v147] break  :", current)
    print("[v147] verdict:", verdict)
    print("[v147] train  :", train_now)
    print("[v147] fichier:", out_path)
    print("")
...
def v81_build_ui(self):
    ...
    try:
        self.root.title(f"BreakbeatAI v147 — replace v81 training panel")
    except Exception:
        pass
    ...
    try:
        ...
        print(f"[v147] patch Charger break impossible : {exc}")

    try:
        v81_install_training_toolbar(self)
    except Exception as exc:
        print(f"[v147] toolbar impossible : {exc}")
...
def v82_auto_role_pools(self):
    ...
    print("")
    print("[v147] ROLE POOLS")
    for r in ["kick", "snare", "hat", "ghost_snare", "bad"]:
        print(f"[v147] {r:12s}:", roles.get(r, []))
    print("")
...
def v82_generate_candidate(self, event=None):
    ...
    print("")
    print("[v147] NEW CANDIDATE")
    print("[v147] break :", current)
    print("[v147] nonce :", nonce)
    print("[v147] rejected known:", len(rejected))
    print("[v147] fingerprint rejeté ?: ", chosen_fp in rejected)
    for item in self.pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v147] step {item['x_step']:02d}/{end:02d} "
...
def v82_patch_training_buttons(self):
    ...
    print(f"[v147] boutons training patchés : {patched}")
...
def v82_build_ui(self):
    ...
    try:
        ...
        print(f"[v147] patch boutons impossible : {exc}")
...
def v110_generate(self, event=None, reject_mode=False):
    ...
    print("")
    if reject_mode:
        print("[v147] REJECT/BAD -> variation premier 32 + autres lignes")
    else:
        print("[v147] GENERATE 64")
    print("[v147] break:", current)
    print("[v147] first32:", first32_id)
    print("[v147] response:", response_id)
    print("[v147] role_offset:", role_offset)
    for role in ["kick", "snare", "hat"]:
        lane = "?"
        try:
            ...
            pass
        print(f"[v147] {role:6s}: pair={chosen[role]['pair']} lane={lane} source={chosen[role]['source']}")
    print("")
    ...
    try:
        ...
        print(f"[v147] draw impossible : {exc}")
    ...
    try:
        ...
        print(f"[v147] refresh_panel impossible : {exc}")
...
def v110_render_pattern_to_audio(self):
    ...
    for item in pattern:
        ...
        if pair not in slice_map:
            skipped += 1
            print(f"[v147] skip pair introuvable : {pair}")
            continue
...
def v110_export_wav_current(self, event=None):
    try:
        ...
        print("")
        print("[v147] EXPORT WAV OK")
        print("[v147] break:", current)
        print("[v147] wav  :", out_path)
        print("[v147] json :", info_path)
        print("[v147] duration:", round(info["duration_seconds"], 3), "s")
        print("[v147] rendered:", info["rendered_notes"], "skipped:", info["skipped_notes"])
        print("")
...
def v110_space_play(self, event=None):
    ...
    try:
        ...
        if btn is not None:
            print("[v147] SPACE -> Loop / Space")
            try:
                btn.invoke()
            except Exception as exc:
                print("[v147] invoke Loop / Space impossible :", exc)
            return "break"

        print("[v147] SPACE : bouton Loop / Space introuvable.")
...
def v110_patch_buttons(self):
    ...
    print(f"[v147] boutons patchés : Generate={gen_patched}, Reject/Bad={bad_patched}")
...
def v110_build_ui(self):
    ...
    try:
        ...
        print("[v147] patch boutons impossible :", exc)

    try:
        v110_add_export_button(self)
    except Exception as exc:
        print("[v147] ajout Export WAV impossible :", exc)

    try:
        v110_install_space_bindings(self)
    except Exception as exc:
        print("[v147] bind espace impossible :", exc)
...
def v113_export(self, event=None):
    ...
    print("[v147] ERREUR : export_wav_current_v110 introuvable.")
...
def v113_place_export_button(self):
    ...
    try:
        ...
        print("[v147] bouton Export WAV visible ajouté.")
        v113_status(self, "v113 : Export WAV visible. Espace reste Loop / Space.")

    except Exception as exc:
        print("[v147] placement bouton Export WAV impossible :", exc)
        try:
            btn.place(x=760, y=32, height=30)
            btn.lift()
            self.export_button_v113 = btn
...
def v115_apply_cells_minus_25(self):
    ...
    print("")
    print("[v147] APPLICATION CASES -3.5px")
    ...
    for name in V115_WIDTH_GLOBAL_NAMES:
        try:
            ...
            if isinstance(value, (int, float)) and value > 6:
                ...
                print(f"[v147] global {name}: {value} -> {globals()[name]}")
        except Exception:
            pass
        ...
        try:
            if hasattr(self, attr):
                ...
                if isinstance(value, (int, float)) and value > 6:
                    ...
                    print(f"[v147] attr {attr}: {value} -> {getattr(self, attr)}")
        except Exception:
            pass
...
def v115_build_ui(self):
    ...
    try:
        self.root.title("BreakbeatAI v147 — replace v81 training panel")
    except Exception:
        pass
    ...
    try:
        ...
        print("[v147] impossible d'appliquer -3.5px :", exc)
...
def v117_render_pattern_to_audio(self):
    ...
    print("")
    print("[v147] EXPORT EXACT CELLS")
    print("[v147] break:", current)
    print("[v147] pairblock:", pairblock_file)
    print("[v147] sr:", sr)
    print("[v147] median_pair_samples:", median_pair_samples)
    print("[v147] case_samples:", case_samples)
    print("[v147] export_samples:", total_samples)
    print("[v147] export_seconds:", round(total_samples / float(sr), 6))
    print("")
...
def v117_export_wav_current(self, event=None):
    try:
        ...
        print("[v147] wav  :", out_path)
        print("[v147] json :", info_path)
        print("[v147] duration:", round(info["duration_seconds"], 6), "s")
        print("[v147] case_seconds:", round(info["case_seconds"], 6))
        print("[v147] rendered:", info["rendered_notes"], "skipped:", info["skipped_notes"])
        print("")
...
def v118_current_break(self):
    ...
    if candidates:
        ...
        print("[v147] break courant:", stem, "| source:", source, "| raw:", raw)
        return stem

    print("[v147] WARNING : break courant introuvable, fallback Camo.")
    return "Camo_Break_-_3A"
...
def v118_find_best_playback_wav(since):
    ...
    print("")
    print("[v147] WAV candidats playback :")
    for score, mtime, size, wav in candidates[:12]:
        age = time.time() - mtime
        print(f"[v147] score={score:8d} age={age:7.2f}s size={size:10d} path={wav}")
    print("")
...
def v118_copy_playback_wav(self, since):
    ...
    if best is None:
        ...
        print("[v147] aucun WAV playback trouvé.")
        return "break"
    ...
    print("")
    print("[v147] EXPORT PLAYBACK OK")
    print("[v147] source :", best)
    print("[v147] export :", out)
    print("[v147] json   :", info_path)
    print("")
...
def v118_export_playback_wav(self, event=None):
    ...
    try:
        ...
        if btn is not None:
            print("[v147] déclenche Loop / Space pour générer le playback WAV...")
            btn.invoke()
        else:
            print("[v147] bouton Loop / Space introuvable, copie du dernier WAV playback existant.")
    except Exception as exc:
        print("[v147] invoke Loop / Space impossible :", exc)
...
def v121_find_best_long_wav(since):
    ...
    print("")
    print("[v147] WAV candidats longs/propre à raccourcir :")
    for row in candidates[:20]:
        age = time.time() - row["mtime"]
        print(
            f"[v147] score={row['score']:7d} "
...
def v121_export_trimmed_long(self, event=None):
    ...
    try:
        ...
        if btn is not None:
            print("[v147] déclenche Loop / Space pour générer le WAV long/propre...")
            btn.invoke()
    except Exception as exc:
        print("[v147] Loop / Space impossible :", exc)
    ...
    def do_export():
        try:
            ...
            try:
                ...
                print("[v147] ffmpeg trim impossible, fallback wave :", exc)
                ok = False
            ...
            print("")
            print("[v147] EXPORT TRIM OK")
            print("[v147] source :", src_wav)
            print("[v147] source durée :", round(src_dur, 6), "s")
            print("[v147] target durée :", round(target_seconds, 6), "s")
            print("[v147] export :", out)
            print("[v147] export durée :", round(float(out_dur or 0), 6), "s")
            print("[v147] json :", info_path)
            print("")
...
def v124_export_trimmed_long_no_json(self, event=None):
    ...
    def do_export():
        try:
            ...
            print("")
            print("[v147] EXPORT WAV ONLY OK")
            print("[v147] source :", src_wav)
            print("[v147] source durée :", round(src_dur, 6), "s")
            print("[v147] target durée :", round(target_seconds, 6), "s")
            print("[v147] export :", out)
            print("[v147] export durée :", round(float(out_dur or 0), 6), "s")
            print("[v147] aucun JSON exporté")
            print("")
...
def v125_select_lane(self, lane):
    ...
    print("")
    print(f"[v147] Slice {lane + 1} sélectionnée")
    print(f"[v147] notes sélectionnées : {ids}")
    print("")
...
def v125_redraw_after_move(self):
    try:
        ...
        print("[v147] draw impossible :", exc)
...
def v125_build_ui(self):
    ...
    if canvas is not None:
        try:
            ...
            print("[v147] bind canvas click impossible :", exc)
    ...
    try:
        ...
        print("[v147] bind flèches impossible :", exc)
...
def v126_select_slice(self, zero_index, slice_number, reason=""):
    ...
    print("")
    print("[v147] SÉLECTION SLICE")
    print("[v147] reason:", reason)
    print("[v147] Slice affichée:", slice_number)
    print("[v147] zero_index:", zero_index)
    print("[v147] pair_candidates:", pair_candidates)
    print("[v147] ids sélectionnés:", ids)
...
def v126_build_ui(self):
    ...
    if canvas is not None:
        try:
            ...
            print("[v147] bind canvas impossible :", exc)
...
def v127_select_slice(self, lane, displayed, reason=""):
    ...
    print("")
    print("[v147] SÉLECTION SLICE ALIGNÉE")
    print("[v147] reason:", reason)
    print("[v147] label affiché:", displayed)
    print("[v147] lane réelle:", lane)
    print("[v147] labels zero-based:", v127_labels_are_zero_based(self))
    print("[v147] pair_candidates:", pair_candidates)
    print("[v147] ids sélectionnés:", ids)
    for row in debug_rows:
        print("[v147] note:", row)
...
def v128_select_lane(self, lane):
    ...
    print("")
    print("[v147] MULTI-SÉLECTION LIGNE")
    print("[v147] slice/lane:", lane)
    print("[v147] ids:", ids)
    for row in debug:
        print("[v147] note:", row)
    print("")
...
def v129_draw_green_multiselect(self):
    ...
    print(f"[v147] contours verts dessinés : {drawn}")
...
def v129_draw(self, *args, **kwargs):
    ...
    try:
        ...
        print("[v147] contour vert impossible :", exc)
...
def v130_audition_after_move(self, notes):
    ...
    def do_play():
        ...
        for name in method_names:
            ...
            try:
                fn()
                print(f"[v147] audition via {name}()")
                played = True
                break
            except TypeError:
                try:
                    fn(None)
                    print(f"[v147] audition via {name}(None)")
                    played = True
                    break
                except TypeError:
                    try:
                        fn(note)
                        print(f"[v147] audition via {name}(note)")
                        played = True
                        break
                    except Exception:
                        pass
        ...
        if not played:
            ...
            if btn is not None:
                try:
                    btn.invoke()
                    print("[v147] audition via bouton Play slice")
                    played = True
                except Exception as exc:
                    print("[v147] Play slice invoke impossible :", exc)

        if not played:
            print("[v147] audition introuvable : aucune méthode Play slice trouvée")
...
def v131_generate_candidate(self, event=None, reject_mode=False):
    ...
    print("")
    print("[v147] GENERATE CANDIDATE STRICT")
    print("[v147] break:", current)
    print("[v147] sequence: kick hat snare hat | hat kick snare hat")
    print("[v147] répété sur 64 cases")
    for role in ["kick", "snare", "hat"]:
        print(f"[v147] {role:6s}: pair={chosen[role]['pair']} source={chosen[role]['source']}")
    print("")
...
def v131_build_ui(self):
    ...
    try:
        ...
        print("[v147] repatch boutons impossible :", exc)
    ...
    for name in [
        ...
        if callable(fn):
            try:
                ...
                print(f"[v147] {name} erreur :", exc)

    v144_status(self, "Export introuvable.")
    print("[v147] ERREUR : aucun moteur Export WAV trouvé.")
    return "break"
...
def v144_install_inline_toolbar(self):
    ...
    if row is None:
        print("[v147] top row introuvable")
        return
    ...
    print("[v147] toolbar intégrée dans la ligne du haut.")
...
def v144_hide_old_ui(self):
    ...
    print(f"[v147] vieux widgets cachés : {hidden}")
...
def v144_build_ui(self):
    ...
    try:
        ...
        print("[v147] install toolbar impossible :", exc)

    try:
        v144_hide_old_ui(self)
    except Exception as exc:
        print("[v147] hide old UI impossible :", exc)
...
# ---------------------------------------------------------------------
# v146 : hover retest propre
# ---------------------------------------------------------------------
# Repart de v144 :
# - enlève le style jaune BARRE MOCHE V145
# - garde l'UI actuelle
# - ajoute un hover inspector plus fiable
#
# À faire :
# - passe la souris sur la vraie barre moche
# - copie le bloc [v146 HOVER EXACT]
# ---------------------------------------------------------------------


def v146_walk(widget):
    out = [widget]
    try:
        for child in widget.winfo_children():
            out.extend(v146_walk(child))
    except Exception:
        pass
    return out


def v146_text(widget):
    try:
        return str(widget.cget("text")).strip()
    except Exception:
        return ""
...
def v146_parent(widget):
    try:
        return str(widget.master)
    except Exception:
        return "None"


def v146_geom(widget):
    try:
        return (
            f"{widget.winfo_width()}x{widget.winfo_height()}"
            f"+{widget.winfo_x()}+{widget.winfo_y()}"
            f" root=+{widget.winfo_rootx()}+{widget.winfo_rooty()}"
        )
    except Exception:
        return "?"


def v146_children_summary(widget, limit=24):
    rows = []
    ...
    for child in children[:limit]:
        txt = v146_text(child)
        cls = v146_class(child)
        geom = v146_geom(child)

        if txt:
            rows.append(f"{cls} text={txt!r} geom={geom}")
        else:
            rows.append(f"{cls} text='' geom={geom}")

    if len(children) > limit:
        rows.append(f"... +{len(children) - limit} autres enfants")

    return rows


def v146_config(widget):
    keys = [
        "bg",
        "background",
        "fg",
...
def v146_attrs_for_widget(self, widget):
    attrs = []

    for name, value in sorted(getattr(self, "__dict__", {}).items()):
        try:
            if value is widget:
                attrs.append(name)
        except Exception:
    ...
    for name in [
        ...
        try:
            if getattr(self, name, None) is widget and name not in attrs:
                attrs.append(name)
        except Exception:
            pass
...
def v146_ancestor_chain(widget, max_depth=12):
    chain = []
    cur = widget
    ...
    for _ in range(max_depth):
        ...
        chain.append(
            f"{v146_class(cur)} path={v146_path(cur)} text={v146_text(cur)!r} geom={v146_geom(cur)}"
        )
...
def v146_print_exact_hover(self, event):
    try:
        import time
        now = time.time()
    except Exception:
    ...
    last = getattr(self, "_v146_last_exact_hover", 0)

    if now and last and now - last < 0.45:
        return

    self._v146_last_exact_hover = now
    ...
    print("")
    print("════════════════════════════════════════════════════════════════════")
    print("[v146 HOVER EXACT]")
    print("pointer        :", f"+{event.x_root}+{event.y_root}")
    print("widget class   :", v146_class(widget))
    print("widget text    :", repr(v146_text(widget)))
    print("widget manager :", v146_manager(widget))
    print("widget path    :", v146_path(widget))
    print("widget parent  :", v146_parent(widget))
    print("widget geom    :", v146_geom(widget))

    attrs = v146_attrs_for_widget(self, widget)
    print("self attrs     :", ", ".join(attrs) if attrs else "aucun")

    cfg = v146_config(widget)
    print("config         :", cfg if cfg else "aucune config lisible")

    print("children:")
    children = v146_children_summary(widget)
    if children:
        for row in children:
            print("  -", row)
    else:
    ...
    for row in v146_ancestor_chain(widget):
        print("  ->", row)

    # Diagnostic simple
    if widget is self.root:
        print("")
        print("DIAGNOSTIC     : tu es sur le fond Tk/root vide, pas sur une toolbar.")
        print("                 Essaie de viser le bord/le texte/le bouton de la barre moche.")
    elif v146_class(widget) in {"Frame", "TFrame"} and not v146_text(widget):
        print("")
        print("DIAGNOSTIC     : Frame vide ou conteneur. C'est probablement une vraie barre/conteneur.")
    elif v146_text(widget):
        print("")
        print("DIAGNOSTIC     : widget avec texte, utile pour identifier son parent toolbar.")

    print("════════════════════════════════════════════════════════════════════", flush=True)


def v146_bind_hover(self):
    widgets = v146_walk(self.root)

    for widget in widgets:
        try:
            if getattr(widget, "_v146_hover_bound", False):
                continue
        except Exception:
            pass

        try:
            widget.bind("<Enter>", lambda e: v146_print_exact_hover(self, e), add="+")
            widget.bind("<Motion>", lambda e: v146_print_exact_hover(self, e), add="+")
            widget._v146_hover_bound = True
        except Exception:
            pass

    # Bind root en capture globale : utile quand on survole un espace vide.
    try:
        self.root.bind("<Motion>", lambda e: v146_print_exact_hover(self, e), add="+")
    except Exception:
        pass

    print(f"[v147] hover exact actif sur {len(widgets)} widgets.", flush=True)


_old_v146_build_ui = SliceIndexTracker.build_ui


def v146_build_ui(self):
    _old_v146_build_ui(self)
    ...
    try:
        v146_bind_hover(self)
    except Exception as exc:
        print("[v147] hover impossible :", exc, flush=True)

    for delay in [300, 900, 1600, 2600]:
        try:
            self.root.after(delay, lambda: v146_bind_hover(self))
        except Exception:
            pass

    print("")
    print("==============================================================")
    print("[v147] HOVER RETEST PROPRE ACTIF")
    print("Le style jaune v145 est retiré.")
    print("Passe la souris sur la vraie barre moche.")
    print("Copie le bloc [v146 HOVER EXACT].")
    print("==============================================================", flush=True)


SliceIndexTracker.build_ui = v146_build_ui



# ---------------------------------------------------------------------
# v147 : remplace la vraie barre moche trouvée au hover
# ---------------------------------------------------------------------
# Hover v146 :
# widget path : .!frame19
# self attrs  : v81_training_panel
# children    : Training / Generate Candidate / Good / Reject
#
# Fix :
# - on vide self.v81_training_panel
# - on le transforme en toolbar Princess propre
# - on garde exactement son emplacement
# - on ne touche pas à la grille
# ---------------------------------------------------------------------

P147_PANEL = "#1a1024"
P147_PANEL_2 = "#251631"
P147_PINK = "#ff8be8"
P147_PINK_DARK = "#7a2f68"
P147_PURPLE = "#332442"
P147_BLUE = "#27304d"
P147_GREEN = "#226b45"
P147_TEXT = "#fff4fb"
...
def v147_walk(widget):
    out = [widget]
    try:
        for child in widget.winfo_children():
            out.extend(v147_walk(child))
    except Exception:
        pass
    return out


def v147_text(widget):
    try:
        return str(widget.cget("text")).strip()
    except Exception:
        return ""
...
def v147_find_training_panel(self):
    ...
    # fallback : retrouve la frame par ses enfants
    for widget in v147_walk(self.root):
        try:
        ...
        child_texts = [v147_text(c) for c in widget.winfo_children()]
...
def v147_button(parent, text, command, bg):
    return tk.Button(
        parent,
        text=text,
        command=command,
...
def v147_new_groove(self, event=None):
    ...
    v147_status(self, "Nouveau groove introuvable.")
    return "break"


def v147_mirror_32(self, event=None):
    ...
    if not first:
        v147_status(self, "Aucun premier 32 à dupliquer.")
        return "break"
    ...
    try:
        self.write_latest_pattern(reason="v147_mirror_32")
    except Exception:
        pass

    v147_status(self, "Miroir 32 appliqué.")
    return "break"


def v147_loop(self, event=None):
    ...
    v147_status(self, "Loop introuvable.")
    return "break"


def v147_export(self, event=None):
    ...
    v147_status(self, "Export introuvable.")
    print("[v147] ERREUR : aucun moteur Export WAV trouvé.")
    return "break"


def v147_replace_training_panel(self):
    panel = v147_find_training_panel(self)

    if panel is None:
        print("[v147] v81_training_panel introuvable")
        return

    if getattr(self, "_v147_training_panel_replaced", False):
        try:
            panel.lift()
        except Exception:
            pass
    ...
    try:
        panel.place_configure(width=870, height=54)
    except Exception:
        pass

    title_box = tk.Frame(panel, bg=P147_PANEL)
    ...
    tk.Label(
        title_box,
        text="toolbar remplace v81_training_panel",
        bg=P147_PANEL,
        fg=P147_MUTED,
        font=("Sans", 8),
    ).pack(side="top", anchor="w")

    buttons = tk.Frame(panel, bg=P147_PANEL)
    buttons.pack(side="left", padx=6, pady=7)

    v147_button(buttons, "Nouveau groove", self.v147_new_groove, P147_PINK_DARK).pack(side="left", padx=4)
    v147_button(buttons, "Miroir 32", self.v147_mirror_32, P147_PURPLE).pack(side="left", padx=4)
    v147_button(buttons, "Loop / Space", self.v147_loop, P147_BLUE).pack(side="left", padx=4)
    v147_button(buttons, "Exporter .wav", self.v147_export, P147_GREEN).pack(side="left", padx=4)

    self.v147_toolbar = panel
    self.v81_training_panel = panel
    self._v147_training_panel_replaced = True
    ...
    print("")
    print("[v147] v81_training_panel remplacé par toolbar propre.")
    print("[v147] boutons : Nouveau groove / Miroir 32 / Loop Space / Exporter .wav")
    print("")

    v147_status(self, "v147 : la barre moche v81_training_panel est remplacée.")


def v147_remove_duplicate_inline_buttons(self):
    """
    Si v144 a ajouté une toolbar inline dans la ligne du haut,
    on retire ses boutons pour éviter doublons.
    """
    labels = {
        "Nouveau groove",
        "Miroir 32",
    ...
    for widget in list(v147_walk(self.root)):
        try:
            if getattr(self, "v147_toolbar", None) is not None:
                # ne jamais supprimer nos nouveaux boutons dans v81_training_panel
                parent = widget
                inside = False
                while parent is not None:
                    if parent is self.v147_toolbar:
                        inside = True
                        break
    ...
    if removed:
        print(f"[v147] boutons inline doublons supprimés : {removed}")


_old_v147_build_ui = SliceIndexTracker.build_ui


def v147_build_ui(self):
    _old_v147_build_ui(self)
    ...
    try:
        v147_replace_training_panel(self)
        v147_remove_duplicate_inline_buttons(self)
    except Exception as exc:
        print("[v147] remplacement panel impossible :", exc)

    for delay in [250, 700, 1200, 2000]:
        try:
            self.root.after(delay, lambda: v147_replace_training_panel(self))
            self.root.after(delay + 50, lambda: v147_remove_duplicate_inline_buttons(self))
        except Exception:
            pass

    try:
        self.root.bind("<F12>", self.v147_new_groove)
        self.root.bind("<Control-e>", self.v147_export)
    except Exception:
        pass


SliceIndexTracker.v147_new_groove = v147_new_groove
SliceIndexTracker.v147_mirror_32 = v147_mirror_32
SliceIndexTracker.v147_loop = v147_loop
SliceIndexTracker.v147_export = v147_export
SliceIndexTracker.export_wav_current_v147 = v147_export
SliceIndexTracker.build_ui = v147_build_ui
