...
"""
BreakbeatAI Tracker Editor v25
...
def block_role(block):
    """
    v25 : rôle structurel forcé par la position du sample dans le cycle break8.
    On ignore les vieux role_guess du slicer.
    """
    try:
...
def load_wav(path):
    ...
    # v25 : pas de normalisation/fade par sample.
...
class TrackerEditorApp:
    def __init__(self, root, pair_json):
        ...
        # v25 : grille 32 positions = 4 cycles de break8.
        ...
        self.root.title("BreakbeatAI Tracker Editor v25")
    ...
    def guess_step_ms(self):
        # v25 : une colonne = une position break8.
    ...
    def detect_external_player(self):
        for name in ["pw-play", "paplay", "aplay", "ffplay"]:
            ...
            if path:
                print(f"[v25] Backend audio système trouvé : {name} -> {path}")
                return name

        print("[v25] Aucun backend système trouvé, fallback sounddevice.")
        return None
    ...
    def formal_role_for_pair(self, pair):
        """
        Rôle structurel du sample selon pair % 8 :
        0 kick / 1 hat / 2 snare / 3 hat / 4 hat / 5 kick / 6 snare / 7 hat
        """
        return BREAK8_ROLES[int(pair) % 8]
    ...
    def base_pair_for_position(self, local_pos):
        ...
        pos 2 -> pair 2
        ...
        si ces pairs existent.
        """
        local_pos = int(local_pos) % 8

        for pair in self.pair_values:
            if int(pair) % 8 == local_pos:
                return int(pair)

        role = BREAK8_ROLES[local_pos]
    ...
    def build_initial_pattern(self):
        ...
        print("[v25] Génération loop32 verrouillée")
        print("[v25] pattern =", " ".join(BREAK8_ROLES))
        ...
        for pos in range(32):
            ...
            if actual_role != target_role:
                print(
                    f"[v25][WARN] pos {pos}: target {target_role}, "
            ...
            print(
                f"[v25] pos {pos:02d} | target {target_role:5s} | "
    ...
    def build_ui(self):
        ...
        title = tk.Label(
            main,
            text="BreakbeatAI Tracker Editor v25 — K H S H H K S H verrouillé",
            bg="#111018",
            fg="#ff7acc",
            font=("Sans", 20, "bold"),
        )
        ...
        self.output_label = tk.Label(
            panel,
            text="v25 : grammaire verrouillée K H S H H K S H sur 32 positions. Modifie puis Save validation.",
            bg="#1b1824",
            fg="#77f5b5",
            justify="left",
        )
    ...
    def bind_keys(self):
        ...
        print("[v25] Raccourcis : Espace loop | flèches = déplacer | Delete = supprimer")
    ...
    def play_audio_array(self, audio, label="audio"):
        ...
        live_wav = OUT_DIR / f"{self.project['safe']}_v25_live.wav"
        ...
        if self.external_player:
            ...
            try:
                ...
                print(f"[v25] {label} lancé avec {self.external_player} : {live_wav}")
                return duration_ms
            except Exception as exc:
                print(f"[v25] Erreur backend système {self.external_player} : {exc}")
    ...
    def render_preview_file(self):
        OUT_DIR.mkdir(parents=True, exist_ok=True)
        wav = OUT_DIR / f"{self.project['safe']}_tracker_app_v25_preview.wav"
    ...
    def toggle_loop(self):
        if self.looping:
            ...
            print("[v25] Loop arrêtée")
            return

        self.stop_playhead()
        print("[v25] Lancement loop...")
        self.looping = True
    ...
    def save(self):
        ...
        data = {
            "version": "tracker_app_edit_v25_exact_cells_no_sample_fade",
        ...
        path = OUT_DIR / f"{self.project['safe']}_tracker_app_edit_v25.json"
