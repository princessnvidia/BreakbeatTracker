...
"""
03_tracker_editor_app_v11.py

BreakbeatAI Tracker Editor v11
...
class TrackerEditorApp:
    def __init__(self, root, pair_json):
        ...
        # v11 : playhead visuelle synchronisée à la boucle.
        self.playhead_after_id = None
        self.loop_started_at = None
        self.current_loop_sec = 0.0
        self.current_timeline = []

        self.root.title("BreakbeatAI Tracker Editor v11")
    ...
    def detect_external_player(self):
        for name in ["pw-play", "paplay", "aplay", "ffplay"]:
            ...
            if path:
                print(f"[v11] Backend audio système trouvé : {name} -> {path}")
                return name

        print("[v11] Aucun backend système trouvé, fallback sounddevice.")
        return None
    ...
    def build_ui(self):
        ...
        title = tk.Label(
            main,
            text="BreakbeatAI Tracker Editor v11 — playhead + correction KICK / SNARE / HI-HAT",
            bg="#111018",
            fg="#ff7acc",
            font=("Sans", 20, "bold"),
        )
        ...
        subtitle = tk.Label(
            ...
            text=(
                ...
                "La ligne verte montre où on se trouve dans le playback."
            ),
            bg="#111018",
            fg="#b9acc8",
        )
    ...
    def bind_keys(self):
        ...
        print("[v11] Raccourcis : Espace loop | h/k/s = classer pair sélectionnée")
    ...
    def play_audio_array(self, audio, label="audio"):
        ...
        live_wav = OUT_DIR / f"{self.project['safe']}_v11_live.wav"
        ...
        if self.external_player:
            ...
            try:
                ...
                print(f"[v11] {label} lancé avec {self.external_player} : {live_wav}")
                return duration_ms
            except Exception as exc:
                print(f"[v11] Erreur backend système {self.external_player} : {exc}")
        ...
        try:
            ...
            print(f"[v11] {label} lancé avec sounddevice")
            return duration_ms
        except Exception as exc:
    ...
    def set_selected_pair_role(self, role):
        ...
        print(f"[v11] override pair {pair} = {role} -> {self.role_override_path}")
    ...
    def render_preview_file(self):
        OUT_DIR.mkdir(parents=True, exist_ok=True)
        wav = OUT_DIR / f"{self.project['safe']}_tracker_app_v11_preview.wav"
    ...
    def toggle_loop(self):
        if self.looping:
            ...
            print("[v11] Loop arrêtée")
            return

        self.stop_playhead()
        print("[v11] Lancement loop...")
        self.looping = True
    ...
    def save(self):
        ...
        data = {
            "version": "tracker_app_edit_v11_playhead_progress_line",
        ...
        path = OUT_DIR / f"{self.project['safe']}_tracker_app_edit_v11.json"
