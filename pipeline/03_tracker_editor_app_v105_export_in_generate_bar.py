...
"""
BreakbeatAI Tracker Editor v105 — slice index
...
def warp_audio_to_length_external(audio, target_len, cache_key=None):
    ...
    if rubberband:
        try:
            ...
            print(f"[v105] rubberband échec, fallback ffmpeg/no-speed : {exc}")
    ...
    if ffmpeg:
        try:
            ...
            print(f"[v105] ffmpeg atempo échec, fallback no-speed : {exc}")
...
class SliceIndexTracker:
    def __init__(self, root, pair_json):
        ...
        self.root.title("BreakbeatAI Tracker Editor v105 — slice index")
    ...
    def detect_external_player(self):
        for name in ["pw-play", "paplay", "aplay", "ffplay"]:
            ...
            if path:
                print(f"[v105] Backend audio : {name} -> {path}")
                return name

        print("[v105] Aucun backend système trouvé, fallback sounddevice.")
        return None
    ...
    def build_initial_pattern(self, randomize=False):
        ...
        print(
            f"[v105] Génération pattern | break={self.project['safe']} | "
        ...
        for slot in range(max_items):
            ...
            print(f"[v105] slot {slot:02d} | step {x_step:02d} -> slice {pair:02d}")
    ...
    def build_ui(self):
        ...
        title = tk.Label(
            main,
            text="BreakbeatAI v105 — export in generate bar",
            bg="#111018",
            fg="#ff7acc",
            font=("Sans", 20, "bold"),
        )
    ...
    def record_correction(self, event_type, before=None, after=None):
        ...
        print(f"[v105] correction sauvegardée : {event_type}")
    ...
    def play_audition_array(self, audio, label="audition"):
        ...
        audition_wav = OUT_DIR / f"{self.project['safe']}_v105_audition.wav"
        ...
        if self.external_player:
            try:
                ...
                print(f"[v105] Erreur audition {self.external_player} : {exc}")
    ...
    def on_close(self):
        ...
        """
        print("[v105] Fermeture : arrêt audio + destruction fenêtre")
    ...
    def play_audio_array(self, audio, label="audio"):
        ...
        audio = normalize(audio)
        live_wav = OUT_DIR / f"{self.project['safe']}_v105_live.wav"
        ...
        if self.external_player:
            try:
                ...
                print(f"[v105] Erreur backend {self.external_player} : {exc}")
    ...
    def render_preview_file(self):
        OUT_DIR.mkdir(parents=True, exist_ok=True)
        wav = OUT_DIR / f"{self.project['safe']}_tracker_app_v105_preview.wav"
    ...
    def write_latest_pattern(self, reason="correction"):
        ...
        path = LATEST_DIR / f"{self.project['safe']}_latest_slice_index_v105.json"

        data = {
            "version": "breakbeatai_latest_slice_index_v105",
            "reason": reason,
    ...
    def save(self):
        ...
        tracker_path = OUT_DIR / f"{self.project['safe']}_tracker_app_edit_v105.json"
        ...
        stamp = file_stamp()
        validated_path = VALIDATED_DIR / f"{self.project['safe']}_validated_slice_index_loop64_v105_{stamp}.json"

        validated = {
            "version": "breakbeatai_validated_slice_index_loop64_v105",
...
def v44_record_correction(self, event_type, before=None, after=None):
    ...
    print(f"[v105] correction sauvegardée : {event_type}")
    print(f"[v105] correction path : {corrections_path}")
    print(f"[v105] latest path : {latest_path}")
...
def v45_move_selected(self, dx, dy):
    ...
    if dy != 0 and before != after:
        try:
            ...
            print(f"[v105] audition après flèche haut/bas impossible : {exc}")
...
def v46_warp_loop_pitch_preserve(audio, target_len, cache_key):
    ...
    if rubberband:
        try:
            ...
            print(f"[v105] rubberband global warp échec : {exc}")
    ...
    if ffmpeg:
        try:
            ...
            print(f"[v105] ffmpeg global warp échec : {exc}")

    print("[v105] Aucun warp pitch-preserve dispo, fallback fit no-speed.")
    return v46_fit_audio_no_speed(audio, target_len)
...
def v46_build_initial_pattern(self, randomize=False):
    ...
    print(
        f"[v105] M8 pattern | break={self.project['safe']} | "
...
def v46_get_warped_loop(self):
    ...
    print(
        f"[v105] global warp OK | source_len={loop_end-loop_start} | "
...
def v46_build_ui(self):
    ...
    try:
        ...
        print(f"[v105] UI extension impossible : {exc}")
...
def v47_play_audio_array(self, audio, label="audio"):
    ...
    if len(audio) == 0 or v47_is_silent(audio):
        ...
        print(f"[v105] lecture annulée : audio silencieux ({label})")
        return 0
...
def v47_toggle_loop(self):
    ...
    if not getattr(self, "pattern", None):
        ...
        print("[v105] loop annulée : pattern vide")
        return "break"
...
def v47_delete_selected(self):
    ...
    if not getattr(self, "pattern", None):
        ...
        print("[v105] pattern vide après suppression : audio coupé")
...
def v48_build_initial_pattern(self, randomize=False):
    ...
    print("[v105] pattern initial forcé VIDE : aucun son ne doit jouer.")
    return []
...
def v48_load_first_slices(self):
    ...
    print(f"[v105] Load 16 slices : {len(self.pattern)} items")


def v48_debug_pattern(self, label="render"):
    print("")
    print(f"[v105] DEBUG PATTERN avant {label}")
    print(f"[v105] items = {len(getattr(self, 'pattern', []) or [])}")

    if not getattr(self, "pattern", None):
        print("[v105] pattern vide confirmé")
        print("")
        return
    ...
    for item in self.pattern:
        ...
        print(
            f"[v105] id={item_id} | step {x:02d}/{end:02d} | "
...
def v48_render_audio_with_timeline(self):
    ...
    if not getattr(self, "pattern", None):
        loop_samples = self.get_loop_samples()
        print(f"[v105] rendu silence total : pattern vide | samples={loop_samples}")
        return np.zeros(loop_samples, dtype=np.float32), []
    ...
    print(f"[v105] rendu audio peak={peak:.8f} | timeline_events={len(timeline) if timeline is not None else 'None'}")

    if timeline:
        print("[v105] timeline events:")
        for ev in timeline:
            print(f"[v105]   {ev}")
...
def v48_toggle_loop(self):
    if not getattr(self, "pattern", None):
        ...
        print("[v105] Space ignoré : pattern vide.")
        return "break"
...
def v48_build_ui(self):
    ...
    try:
        ...
        print(f"[v105] ajout UI impossible : {exc}")
...
def v49_add_item_at_xy(self, x, y):
    ...
    try:
        ...
        print(f"[v105] record_correction add impossible : {exc}")
    ...
    try:
        ...
        print(f"[v105] audition après ajout impossible : {exc}")

    print(f"[v105] add | id={item['id']} | step={x_step} | pair={pair} | lane={lane}")
...
def v49_duplicate_selected_forward(self, event=None):
    ...
    if item is None:
        ...
        print("[v105] duplicate annulé : aucune note sélectionnée")
        return "break"
    ...
    if new_x > max_x:
        ...
        print("[v105] duplicate annulé : fin de grille")
        return "break"
    ...
    try:
        ...
        print(f"[v105] record_correction duplicate impossible : {exc}")
    ...
    try:
        ...
        print(f"[v105] audition après duplicate impossible : {exc}")

    print(
        f"[v105] duplicate | old_id={item.get('id')} | new_id={new_item.get('id')} | "
...
def v49_build_ui(self):
    ...
    try:
        ...
        print(f"[v105] bind canvas click impossible : {exc}")
    ...
    try:
        ...
        print(f"[v105] bind keys impossible : {exc}")
...
def v50_load_style_model():
    ...
    try:
        ...
        print(f"[v105] impossible de charger le modèle IA : {exc}")
        return None
...
def v50_generate_ai_pattern(self, event=None):
    ...
    if model is None:
        ...
        print("[v105] modèle absent :", STYLE_MODEL_PATH)
        return "break"
    ...
    print("")
    print(f"[v105] GENERATE IA | safe={safe} | pairs={pair_values} | slots={max_slots}")
    ...
    for slot in range(max_slots):
        ...
        print(
            f"[v105] slot {slot:02d} | step {x_step:02d} | "
    ...
    try:
        ...
        print(f"[v105] write_latest_pattern impossible : {exc}")
...
def v50_build_ui(self):
    ...
    try:
        ...
        print(f"[v105] UI IA impossible : {exc}")
...
def v51_mouse_down(self, event):
    ...
    if item is not None:
        ...
        print(
            f"[v105] mouse down note | id={item.get('id')} | "
            f"step={item.get('x_step')} | pair={item.get('pair')}"
        )
    else:
        print("[v105] mouse down empty")
...
def v51_mouse_up(self, event):
    ...
    if state.get("empty"):
        ...
        print("[v105] drag depuis vide ignoré")
        return "break"
    ...
    if before != after:
        try:
            ...
            print(f"[v105] record_correction drag impossible : {exc}")
        ...
        print(
            f"[v105] drag move | id={after.get('id')} | "
            f"step {before.get('x_step')} -> {after.get('x_step')} | "
        ...
        try:
            ...
            print(f"[v105] audition après drag impossible : {exc}")
...
def v51_build_ui(self):
    ...
    try:
        ...
        print(f"[v105] bind souris impossible : {exc}")
    ...
    try:
        ...
        print(f"[v105] bind Suppr impossible : {exc}")
...
def v52_normalize_loaded_notes(self, notes):
    ...
    for raw in sorted(notes, key=lambda n: (int(float(n.get("x_step", 0))), int(float(n.get("pair", 0))))):
        ...
        if pair not in valid_pairs:
            print(f"[v105] slice ignorée car absente du break actuel : pair={pair}")
            continue
...
def v52_load_saved_beat(self, event=None):
    ...
    if not saves:
        ...
        print("[v105] aucune sauvegarde trouvée")
        return "break"
    ...
    if not pattern:
        ...
        print("[v105] sauvegarde incompatible :", chosen["path"])
        return "break"
    ...
    print("")
    print("[v105] BEAT RESTAURÉ")
    print("[v105] fichier :", chosen["path"])
    print("[v105] clé :", chosen["key"])
    print("[v105] notes :", len(pattern))
    print("")
...
def v52_list_saved_beats(self, event=None):
    ...
    print("")
    print("[v105] SAUVEGARDES TROUVÉES")
    for i, save in enumerate(saves[:30]):
        print(
            f"[v105] {i:02d} | notes={len(save['notes']):02d} | "
...
def v52_build_ui(self):
    ...
    try:
        ...
        print(f"[v105] UI load saved impossible : {exc}")
...
def v53_run_training(self):
    ...
    if not trainer.exists():
        ...
        print("[v105] trainer introuvable :", trainer)
        return False

    print("")
    print("[v105] AUTO-TRAIN IA après Save...")
    print("")
    ...
    try:
        ...
        if result.returncode != 0:
            ...
            print("[v105] auto-train échec code :", result.returncode)
            return False

        v53_status(self, "Save OK + IA entraînée automatiquement.")
        print("[v105] AUTO-TRAIN OK")
        return True

    except Exception as exc:
        v53_status(self, f"Save OK, mais auto-train impossible : {exc}")
        print("[v105] exception auto-train :", exc)
        return False
...
def v53_save(self):
    ...
    try:
        ...
        print(f"[v105] auto-train après save impossible : {exc}")
...
def v53_build_ui(self):
    ...
    try:
        ...
        print(f"[v105] UI auto-train impossible : {exc}")
...
def v55_mouse_up(self, event):
    ...
    if not state.get("moved"):
        ...
        try:
            ...
            print(f"[v105] audition clic note impossible : {exc}")
        ...
        print(
            f"[v105] click play/select | id={item.get('id')} | "
            f"step={item.get('x_step')} | pair={item.get('pair')}"
        )
...
def v58_add_item_at_xy(self, x, y):
    ...
    print(
        f"[v105] add | id={item['id']} | step={x_step} | "
...
def v58_toggle_selected_length(self, event=None):
    ...
    try:
        ...
        print(f"[v105] record_correction length impossible : {exc}")

    v58_refresh_after_edit(self, "toggle_note_length")

    v58_status(self, f"Longueur note : {old_len} -> {new_len} case(s).")
    print(f"[v105] length toggle | id={item.get('id')} | {old_len} -> {new_len}")
...
def v58_duplicate_selected_forward(self, event=None):
    ...
    try:
        ...
        print(f"[v105] audition duplicate impossible : {exc}")
...
def v58_build_ui(self):
    ...
    try:
        ...
        print(f"[v105] UI demi-grille impossible : {exc}")
    ...
    try:
        ...
        print(f"[v105] bind demi-grille impossible : {exc}")
...
def v59_rank_current_slices(self):
    ...
    for pair in self.pair_values:
        ...
        try:
            ...
            print(f"[v105] analyse pair {pair} impossible : {exc}")
            role = "other"
    ...
    print("")
    print("[v105] TOP ROLES CURRENT BREAK")
    for r in ("kick", "snare", "hat"):
        print(f"[v105] {r:5s}:", [(x["pair"], round(x["score"], 3)) for x in ranks[r][:6]])
    print("")
...
def v59_load_role_model():
    ...
    try:
        ...
        print(f"[v105] lecture modèle rôle impossible : {exc}")
        return None
...
def v59_generate_role_aware(self, event=None):
    ...
    if model is None:
        ...
        print("[v105] modèle rôle absent, fallback musical snare 6.")
        model = {
            "breaks": {},
            "global": {},
            "patterns_used": 0,
    ...
    print("")
    print("[v105] ROLE-AWARE GENERATION")
    print("[v105] model patterns_used:", model.get("patterns_used", 0))
    for item in pattern:
        print(
            f"[v105] step {item['x_step']:02d}/{item['x_step'] + item['length'] - 1:02d} | "
...
def v59_run_role_training(self, event=None):
    ...
    try:
        ...
        print(f"[v105] training rôle impossible : {exc}")
...
def v59_save(self):
    ...
    try:
        ...
        print(f"[v105] auto-training rôle après save impossible : {exc}")
...
def v59_build_ui(self):
    ...
    try:
        ...
        print(f"[v105] UI role-aware impossible : {exc}")
...
def v62_add_item_at_xy(self, x, y):
    ...
    print(f"[v105] add | id={item['id']} | step={x_step} | len={length} | pair={pair}")
...
def v62_mouse_down(self, event):
    ...
    if item is not None:
        ...
        print(
            f"[v105] mouse down {state['mode']} | id={item.get('id')} | "
            f"step={item.get('x_step')} | len={item.get('length')} | pair={item.get('pair')}"
        )
    else:
        print("[v105] mouse down empty")
    ...
    if mode == "resize":
        if not state.get("moved"):
            ...
            try:
                ...
                print(f"[v105] audition clic resize impossible : {exc}")
        ...
        if before != after:
            try:
                ...
                print(f"[v105] record_correction resize impossible : {exc}")
            ...
            try:
                ...
                print(f"[v105] audition après resize impossible : {exc}")
    ...
    if mode == "move":
        ...
        if before != after:
            try:
                ...
                print(f"[v105] record_correction move impossible : {exc}")
            ...
            try:
                ...
                print(f"[v105] audition après move impossible : {exc}")
...
def v62_toggle_selected_length(self, event=None):
    ...
    try:
        ...
        print(f"[v105] record_correction toggle length impossible : {exc}")

    v62_refresh_after_edit(self, "toggle_note_length_v62")
    v62_status(self, f"Longueur note : {old_len} → {new_len} case(s).")

    print(f"[v105] toggle length | id={item.get('id')} | {old_len} -> {new_len}")
...
def v62_draw(self):
    ...
    try:
        ...
        print(f"[v105] draw handles impossible : {exc}")
...
def v62_build_ui(self):
    ...
    try:
        ...
        print(f"[v105] bind resize impossible : {exc}")
    ...
    try:
        ...
        print(f"[v105] UI resize impossible : {exc}")
...
def v63_load_model():
    ...
    try:
        ...
        print(f"[v105] lecture modèle strict impossible : {exc}")
        return None
...
def v63_generate_safe_role_ai(self, event=None):
    ...
    print("")
    print("[v105] SAFE ROLE AI GENERATED")
    print("[v105] model strict:", "yes" if model else "no")
    print("[v105] snare main/alt:", main_snare, alt_snare)
    print("[v105] kick  main/alt:", main_kick, alt_kick)
    print("[v105] hat   main/alt:", main_hat, alt_hat)
    print("[v105] pattern:")
    for item in pattern:
        print(
            f"[v105] step {item['x_step']:02d}/{item['x_step'] + item['length'] - 1:02d} | "
...
def v63_run_strict_training(self, event=None):
    ...
    print("")
    print("[v105] TRAIN STRICT ROLE MODEL...")
    print("")
    ...
    try:
        ...
        print(f"[v105] training strict impossible : {exc}")
...
def v63_build_ui(self):
    ...
    try:
        ...
        print(f"[v105] UI safe impossible : {exc}")
...
def v65_rank_slices(self):
    ...
    for pair in self.pair_values:
        ...
        try:
            ...
            print(f"[v105] analyse slice {pair} impossible : {exc}")
            scores = {
                "kick": 0.0,
                "snare": 0.0,
                "hat": 0.0,
    ...
    print("")
    print("[v105] TOP SLICES PAR ROLE — break courant")
    for role in ("kick", "snare", "hat", "ghost_snare"):
        print(f"[v105] {role:12s}:", [(x["pair"], round(x["score"], 3)) for x in ranks[role][:8]])
    print("")
...
def v65_generate_from_role_template(self, event=None):
    ...
    try:
        ...
        print(f"[v105] template introuvable : {exc}")
        return "break"
    ...
    print("")
    print("[v105] CROSS-SONG ROLE TEMPLATE GENERATED")
    print("[v105] template:", V65_TEMPLATE_PATH)
    print("[v105] temperature:", temperature, "| hats:", hat_density, "| ghosts:", ghost_density)
    print("[v105] pattern:")
    for item in pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v105] step {item['x_step']:02d}/{end:02d} | "
...
def v65_build_ui(self):
    ...
    try:
        ...
        print(f"[v105] UI impossible : {exc}")
...
def v67_train_strict_if_available(self):
    ...
    if trainer is None:
        print("[v105] aucun trainer trouvé.")
        return False

    print("")
    print("[v105] TRAIN après validation :", trainer)
    print("")
    ...
    try:
        ...
        print(f"[v105] training impossible : {exc}")
        return False
...
def v67_generate_candidate(self, event=None):
    ...
    print("")
    print("[v105] CANDIDATE GENERATED")
    print("[v105] break:", self.v67_candidate_safe)
    print("[v105] notes:", len(candidate))
    print("[v105] Maintenant : corrige puis clique Good après modifs, ou Reject.")
    print("")
...
def v67_reject_candidate(self, event=None):
    ...
    print("")
    print("[v105] REJECTED")
    print("[v105] saved:", rejected_path)
    print("")
...
def v67_accept_corrected(self, event=None):
    ...
    print("")
    print("[v105] ACCEPTED GOOD AFTER EDIT")
    print("[v105] saved:", validated_path)
    print("[v105] trained:", trained)
    print("")
...
def v67_accept_without_training(self, event=None):
    ...
    print("[v105] accepted no-train:", validated_path)
...
def v67_build_ui(self):
    ...
    try:
        ...
        print(f"[v105] UI feedback impossible : {exc}")
...
def v68_generate_candidate(self, event=None):
    ...
    print("[v105] Length policy appliquée : 2 cases par défaut, 1 case rare.")
...
def v69_force_pattern_hard_2_grid(self, reason="v69_force_hard_2_grid"):
    ...
    print("")
    print("[v105] HARD 2 GRID APPLIQUÉ")
    for item in self.pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v105] step {item['x_step']:02d}/{end:02d} | "
...
def v70_duplicate_selected_forward(self, event=None):
    ...
    print("")
    print("[v105] CTRL+D DUPLICATE +2")
    print(f"[v105] old_id={item.get('id')} new_id={new_item.get('id')}")
    print(f"[v105] step {old_x}/{old_x + length - 1} -> {new_x}/{new_x + length - 1}")
    print(f"[v105] pair={new_item.get('pair')} len={length}")
    print("")
...
def v70_build_ui(self):
    ...
    try:
        ...
        print(f"[v105] bind Ctrl+D impossible : {exc}")
...
def v71_rank_slices(self):
    ...
    print("")
    print("[v105] PRE-LEARNING TOP SLICES")
    for role in ("kick", "snare", "hat", "ghost_snare"):
        print(f"[v105] {role:12s}:", [(x["pair"], round(x["score"], 3)) for x in ranks[role][:8]])
    print("")
...
def v71_prelearn_roles(self, force=False):
    ...
    print("")
    print("[v105] PRELEARN ROLES POUR", safe)
    for role in ("kick", "snare", "hat", "ghost_snare", "bad"):
        print(f"[v105] {role:12s}:", roles.get(role, []))
    print("")
...
def v71_generate_full_candidate(self, event=None):
    ...
    print("")
    print("[v105] FULL CANDIDATE GENERATED")
    print("[v105] break:", v71_safe(self))
    print("[v105] roles:", roles)
    print("[v105] pattern:")
    for item in pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
...
def v72_open_break(self, safe=None):
    ...
    print("")
    print("[v105] OPEN BREAK")
    print("[v105] current:", current)
    print("[v105] next   :", safe)
    print("[v105] cmd    :", " ".join(cmd))
    print("")
...
def v72_build_ui(self):
    ...
    try:
        ...
        print(f"[v105] UI break switcher impossible : {exc}")
...
def v73_sanitize_tracker_pattern(self, reason="v73_tracker_strict_no_overlap"):
    ...
    print("")
    print("[v105] TRACKER STRICT NO OVERLAP")
    print("[v105] kept:", len(accepted), "| rejected overlaps:", len(rejected))

    for item in accepted:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v105] KEEP step {item['x_step']:02d}/{end:02d} | "
    ...
    if rejected:
        ...
        for item in rejected:
            ...
            print(
                f"[v105] DROP step {item['x_step']:02d}/{end:02d} | "
                f"role={item.get('v73_role_key')} | pair={item.get('pair')}"
            )
...
def v81_launch_break(self, source):
    ...
    print("")
    print("[v105] CHARGER BREAK")
    print("[v105] courant :", current)
    print("[v105] nouveau :", source)
    print("[v105] cmd     :", " ".join(cmd))
    print("")
...
def v81_patch_charger_break_buttons(self):
    ...
    print(f"[v105] boutons Charger break patchés : {patched}")
...
def v81_generate_candidate(self, event=None):
    ...
    print("")
    print("[v105] GENERATE CANDIDATE")
    print("[v105] break chargé :", current)
    print("[v105] générateur   :", getattr(fn, "__name__", None))
    print("")
...
def v81_run_training(self):
    ...
    if script is None:
        print("[v105] Aucun script training trouvé.")
...
def v81_save_feedback(self, verdict, train_now=False):
    ...
    print("")
    print("[v105] FEEDBACK SAUVÉ")
    print("[v105] break  :", current)
    print("[v105] verdict:", verdict)
    print("[v105] train  :", train_now)
    print("[v105] fichier:", out_path)
    print("")
...
def v81_build_ui(self):
    ...
    try:
        self.root.title(f"BreakbeatAI v105 — export in generate bar")
    except Exception:
        pass
    ...
    try:
        ...
        print(f"[v105] patch Charger break impossible : {exc}")

    try:
        v81_install_training_toolbar(self)
    except Exception as exc:
        print(f"[v105] toolbar impossible : {exc}")
...
def v82_auto_role_pools(self):
    ...
    print("")
    print("[v105] ROLE POOLS")
    for r in ["kick", "snare", "hat", "ghost_snare", "bad"]:
        print(f"[v105] {r:12s}:", roles.get(r, []))
    print("")
...
def v82_generate_candidate(self, event=None):
    ...
    print("")
    print("[v105] NEW CANDIDATE")
    print("[v105] break :", current)
    print("[v105] nonce :", nonce)
    print("[v105] rejected known:", len(rejected))
    print("[v105] fingerprint rejeté ?: ", chosen_fp in rejected)
    for item in self.pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v105] step {item['x_step']:02d}/{end:02d} "
...
def v82_patch_training_buttons(self):
    ...
    print(f"[v105] boutons training patchés : {patched}")
...
def v82_build_ui(self):
    ...
    try:
        ...
        print(f"[v105] patch boutons impossible : {exc}")
...
# ---------------------------------------------------------------------
# v100 EXTENSION : Reject/Bad échange aussi les lignes kick/snare/hat
# ---------------------------------------------------------------------
# - Loop 64.
# - Colonnes comme avant -3px.
# - Generate Candidate : génération normale.
# - Reject / Bad :
#     1. variation structurée suivante du deuxième 32
#     2. kick/snare/hat changent de pair/lane si possible
#     3. aucune position random libre
# ---------------------------------------------------------------------

V100_OVERRIDES_PATH = Path("dataset/learning/break_role_overrides_v01.json")
V100_LOCKS_PATH = Path("dataset/learning/main_role_locks_v87.json")
V100_REJECT_PATH = Path("dataset/learning/rejected_variants_v100.json")
...
V100_SECOND_32_VARIANTS = [
    {
        "id": "A_same",
        "grid": [
            (32, "kick"), (34, "hat"), (36, "snare"), (38, "hat"),
            (40, "hat"),  (42, "kick"), (44, "snare"), (46, "hat"),
            (48, "kick"), (50, "hat"),  (52, "snare"), (54, "hat"),
            (56, "hat"),  (58, "kick"), (60, "snare"), (62, "hat"),
        ],
    },
    {
        "id": "B_kick_forward",
        "grid": [
            (32, "kick"), (34, "hat"),  (36, "snare"), (38, "hat"),
            (40, "kick"), (42, "hat"),  (44, "snare"), (46, "hat"),
            (48, "kick"), (50, "hat"),  (52, "snare"), (54, "hat"),
            (56, "hat"),  (58, "kick"), (60, "snare"), (62, "hat"),
        ],
    },
    {
        ...
        "grid": [
            (32, "kick"), (34, "hat"),  (36, "snare"), (38, "hat"),
            (40, "hat"),  (42, "kick"), (44, "snare"), (46, "hat"),
            (48, "hat"),  (50, "kick"), (52, "snare"), (54, "hat"),
            (56, "hat"),  (58, "kick"), (60, "snare"), (62, "hat"),
        ],
    },
    {
        ...
        "grid": [
            ...
            (40, "hat"),  (42, "kick"), (44, "snare"), (46, "hat"),
            (48, "kick"), (50, "hat"),  (52, "snare"), (54, "hat"),
            (56, "kick"), (58, "hat"),  (60, "snare"), (62, "hat"),
        ],
    },
    {
        "id": "E_more_hats",
        "grid": [
            (32, "kick"), (34, "hat"), (36, "snare"), (38, "hat"),
            (40, "hat"),  (42, "hat"), (44, "snare"), (46, "kick"),
            (48, "kick"), (50, "hat"), (52, "snare"), (54, "hat"),
            (56, "hat"),  (58, "kick"), (60, "snare"), (62, "hat"),
        ],
    },
    {
        ...
        "grid": [
            (32, "kick"), (34, "hat"),  (36, "snare"), (38, "hat"),
            (40, "hat"),  (42, "kick"), (44, "snare"), (46, "hat"),
            (48, "kick"), (50, "hat"),  (52, "snare"), (54, "kick"),
            (56, "hat"),  (58, "hat"),  (60, "snare"), (62, "hat"),
        ],
    },
]

V100_SNARE_STEPS = [4, 12, 20, 28, 36, 44, 52, 60]

V100_WIDTH_ATTR_NAMES = [
    "step_w", "cell_w", "col_w", "column_w",
    "grid_step_w", "step_width", "cell_width",
    "col_width", "note_width", "beat_w",
    "x_step_w", "x_cell_w", "tracker_step_w",
]


def v100_status(self, text):
    try:
        if hasattr(self, "set_status"):
            self.set_status(text)
        else:
...
def v100_load_json(path, default):
    try:
...
def v100_save_json(path, data):
    Path(path).parent.mkdir(parents=True, exist_ok=True)
    Path(path).write_text(json.dumps(data, indent=2, ensure_ascii=False), encoding="utf-8")


def v100_find_record(data, current):
    br = data.get("breaks", {}).get(current)
    if br is not None:
        return br

    for name, obj in data.get("breaks", {}).items():
        if v100_norm(name) == v100_norm(current):
            return obj

    return None


def v100_force_64_columns_minus_3(self):
    for attr in [
    ...
    if not getattr(self, "_v100_columns_minus_3_done", False):
        self._v100_columns_minus_3_done = True

        for attr in V100_WIDTH_ATTR_NAMES:
            try:
                if hasattr(self, attr):
                    value = getattr(self, attr)
                    if isinstance(value, (int, float)) and value > 4:
                        setattr(self, attr, float(value) - 3.0)
                        print(f"[v105] attr width {attr}: {value} -> {getattr(self, attr)}")
            except Exception:
                pass
...
def v100_load_roles(self):
    current = v100_current_break(self)
    ...
    data = v100_load_json(V100_OVERRIDES_PATH, {"breaks": {}})
    br = v100_find_record(data, current)
...
def v100_load_locks(self):
    current = v100_current_break(self)
    data = v100_load_json(V100_LOCKS_PATH, {"breaks": {}})
    br = v100_find_record(data, current)
...
def v100_candidates(self, roles, locks, role):
    out = []
...
def v100_choose_main_pairs(self, roles, locks, role_offset=0):
    """
    Choix structuré des lignes :
    - Generate normal : offset courant
    - Reject/Bad : offset + 1, donc les rôles changent de ligne
    - pas de random libre
    """
    chosen = {}
    used = set()
    ...
    for role in ["snare", "kick", "hat"]:
        candidates = v100_candidates(self, roles, locks, role)
        ...
        # On avance dans la liste de candidats selon role_offset.
        ...
        chosen[role] = {
            "pair": int(pair),
            "source": f"{source}_offset_{role_offset}",
        }
        used.add(int(pair))
...
def v100_load_state():
    data = v100_load_json(
        V100_REJECT_PATH,
        {
            "version": "rejected_variants_v100",
            "updated_at": None,
            "breaks": {},
        },
    )
...
def v100_save_state(data):
    data["updated_at"] = time.strftime("%Y-%m-%d %H:%M:%S")
    v100_save_json(V100_REJECT_PATH, data)


def v100_break_state(current):
    data = v100_load_state()
    br = data["breaks"].setdefault(current, {})
    br.setdefault("reject_count", 0)
    br.setdefault("rejected", [])
    return data, br


def v100_next_variant_and_role_offset(self, reject_mode=False):
    current = v100_current_break(self)
    data, br = v100_break_state(current)

    if reject_mode:
        br["reject_count"] = int(br.get("reject_count", 0)) + 1
        current_variant = getattr(self, "v100_current_variant_id", None)
        current_role_offset = getattr(self, "v100_role_offset", None)

        if current_variant:
            br["rejected"].append({
                "time": time.strftime("%Y-%m-%d %H:%M:%S"),
                "variant": str(current_variant),
                "role_offset": int(current_role_offset or 0),
            })

        v100_save_state(data)

    count = int(br.get("reject_count", 0))

    # Generate normal peut aussi avancer un peu entre clics, mais Reject force surtout l'offset.
    local_nonce = int(getattr(self, "v100_generate_count", -1)) + 1
    self.v100_generate_count = local_nonce

    variant_index = (count + local_nonce) % len(V100_SECOND_32_VARIANTS)

    # C'est ça la partie importante :
    # Reject/Bad augmente cet offset, donc kick/snare/hat prennent d'autres pairs/lignes.
    role_offset = count

    return variant_index, role_offset


def v100_alt_pair_for_second_32(self, roles, chosen, role, occurrence, role_offset):
    main_pair = int(chosen[role]["pair"])

    # La snare garde la même ligne dans un même candidat.
    # Elle change de ligne au prochain Reject via role_offset.
    if role == "snare":
        return main_pair, chosen[role]["source"]

    pool = [int(p) for p in roles.get(role, []) if int(p) != main_pair]

    if not pool:
        return main_pair, chosen[role]["source"]

    idx = (occurrence + role_offset) % len(pool)
    pair = pool[idx]
    return int(pair), f"second32_role_variation_offset_{role_offset}"


def v100_make_note(self, step, pair, role, source, variant_id, role_offset):
    step = int(step)
    ...
    return {
        ...
        "hit_slot": int(step) // 2,
        "ai_generated": True,
        "ai_model": "v100_64_reject_swap_lines",
        "learned_role": str(role),
        "prior_label": str(role),
        "main_role_source": str(source),
        "v100_loop64": True,
        "v100_variant_id": str(variant_id),
        "v100_role_offset": int(role_offset),
        "v100_reject_swaps_lines": True,
    }


def v100_validate_pattern(pattern):
    if len(pattern) != 32:
        raise RuntimeError(f"BUG v100 : pattern devrait avoir 32 notes, obtenu {len(pattern)}")
    ...
    for item in pattern:
        ...
        if x % 2 != 0:
            raise RuntimeError(f"BUG v100 : départ impair {x}")

        if x < 0 or x > 62:
            raise RuntimeError(f"BUG v100 : step hors 64 : {x}")

        if x in occupied or (x + 1) in occupied:
            raise RuntimeError(f"BUG v100 : collision à {x}/{x+1}")

        occupied.add(x)
        occupied.add(x + 1)

        if role == "snare" and x not in V100_SNARE_STEPS:
            raise RuntimeError(f"BUG v100 : snare à {x}, interdit")


def v100_generate(self, event=None, reject_mode=False):
    v100_force_64_columns_minus_3(self)

    current = v100_current_break(self)
    roles = v100_load_roles(self)
    locks = v100_load_locks(self)

    variant_index, role_offset = v100_next_variant_and_role_offset(self, reject_mode=reject_mode)

    variant = V100_SECOND_32_VARIANTS[variant_index]
    variant_id = variant["id"]

    chosen = v100_choose_main_pairs(self, roles, locks, role_offset=role_offset)

    self.v100_variant_index = int(variant_index)
    self.v100_current_variant_id = str(variant_id)
    self.v100_role_offset = int(role_offset)

    full_grid = list(V100_BASE_32) + list(variant["grid"])

    print("")
    if reject_mode:
        print("[v105] REJECT/BAD → VARIATION + AUTRES LIGNES KICK/SNARE/HAT")
    else:
        print("[v105] GENERATE 64")
    print("[v105] break:", current)
    print("[v105] variant:", variant_id)
    print("[v105] role_offset:", role_offset)
    print("[v105] locks:", locks)
    for role in ["kick", "snare", "hat"]:
        print(
            f"[v105] {role:6s}: pair={chosen[role]['pair']} "
            f"source={chosen[role]['source']} "
            f"lane={self.pair_to_lane.get(chosen[role]['pair'], '?') if hasattr(self, 'pair_to_lane') else '?'} "
            f"pool={roles.get(role)}"
        )
    print("")
    ...
    for step, role in full_grid:
        if step < 32:
            ...
            pair, source = v100_alt_pair_for_second_32(self, roles, chosen, role, occ, role_offset)

        pattern.append(v100_make_note(self, step, pair, role, source, variant_id, role_offset))

    v100_validate_pattern(pattern)
    ...
    try:
        ...
        print(f"[v105] draw impossible : {exc}")
    ...
    try:
        ...
        print(f"[v105] refresh_panel impossible : {exc}")

    try:
        self.write_latest_pattern(reason="v100_64_reject_swap_lines")
    except Exception as exc:
        print(f"[v105] write_latest_pattern impossible : {exc}")
    ...
    print("")
    print("[v105] GENERATED PATTERN 64")
    for item in self.pattern:
        x = int(item["x_step"])
        end = x + int(item["length"]) - 1
        print(
            f"[v105] step {x:02d}/{end:02d} "
            f"role={item.get('learned_role'):6s} "
            f"pair={item.get('pair')} "
            f"lane={item.get('lane')} "
            f"variant={item.get('v100_variant_id')} "
            f"offset={item.get('v100_role_offset')}"
        )
    print("")

    if reject_mode:
        v100_status(self, f"v100 Reject : variant {variant_id}, lignes K/S/H changées offset {role_offset}.")
    else:
        v100_status(self, f"v100 : loop 64 variant {variant_id}. Reject change aussi les lignes.")
    return "break"


def v100_reject_bad(self, event=None):
    return v100_generate(self, event=event, reject_mode=True)


def v100_all_widgets(widget):
    out = [widget]
    ...
    for child in children:
        out.extend(v100_all_widgets(child))

    return out


def v100_patch_buttons(self):
    gen_patched = 0
    bad_patched = 0

    for widget in v100_all_widgets(self.root):
        try:
            text = str(widget.cget("text")).strip().lower()
        except Exception:
            continue
        ...
        try:
            if text == "generate candidate":
                ...
                widget.configure(command=self.reject_bad_v100)
                bad_patched += 1
        except Exception:
            pass

    return gen_patched, bad_patched


_old_v100_draw = getattr(SliceIndexTracker, "draw", None)

def v100_draw(self, *args, **kwargs):
    v100_force_64_columns_minus_3(self)

    if callable(_old_v100_draw):
        result = _old_v100_draw(self, *args, **kwargs)
    else:
        result = None
...
_old_v100_build_ui = SliceIndexTracker.build_ui


def v100_build_ui(self):
    _old_v100_build_ui(self)

    v100_force_64_columns_minus_3(self)

    try:
        self.root.title(f"BreakbeatAI v100 reject swap lines — {v100_current_break(self)}")
    except Exception:
        pass

    try:
        gen_patched, bad_patched = v100_patch_buttons(self)
        print(f"[v105] boutons patchés : Generate={gen_patched}, Reject/Bad={bad_patched}")
    except Exception as exc:
        print(f"[v105] patch boutons impossible : {exc}")
    ...
    try:
        self.root.bind("<F9>", self.reject_bad_v100)
    except Exception:
        pass

    v100_status(self, "v100 : Reject/Bad change aussi les lignes kick/snare/hat. F12/F9.")


SliceIndexTracker.generate_candidate = v100_generate
SliceIndexTracker.generate_full_candidate = v100_generate
SliceIndexTracker.generate_ai_pattern = v100_generate
SliceIndexTracker.generate_cross_song_template = v100_generate
SliceIndexTracker.generate_safe_experiment = v100_generate
SliceIndexTracker.generate_locked_roles = v100_generate
SliceIndexTracker.generate_role_aware = v100_generate
SliceIndexTracker.generate_safe_role_ai = v100_generate

SliceIndexTracker.reject_bad_v100 = v100_reject_bad
SliceIndexTracker.reject_bad = v100_reject_bad
SliceIndexTracker.bad_candidate = v100_reject_bad
SliceIndexTracker.reject_candidate = v100_reject_bad
SliceIndexTracker.reject_pattern = v100_reject_bad

if callable(_old_v100_draw):
    SliceIndexTracker.draw = v100_draw

SliceIndexTracker.build_ui = v100_build_ui



# ---------------------------------------------------------------------
# v101 EXTENSION : bouton Export WAV
# ---------------------------------------------------------------------
# Exporte le pattern actuellement affiché en WAV dans ./exports/
# Garde le comportement v100 :
# - loop 64
# - Reject/Bad change les lignes kick/snare/hat
# - colonnes -3px
# ---------------------------------------------------------------------

V101_PAIR_DIR = Path("dataset/pair_blocks_v02")
V101_EXPORT_DIR = Path("exports")


def v101_status(self, text):
    try:
        if hasattr(self, "set_status"):
            self.set_status(text)
        else:
...
def v101_load_json(path, default):
    try:
        return json.loads(Path(path).read_text(encoding="utf-8"))
    except Exception:
        return default
...
def v101_find_pairblock_file(current):
    exact = V101_PAIR_DIR / f"{current}_pair_blocks_v02.json"

    if exact.exists():
        return exact

    n_current = v101_norm(current)

    for p in sorted(V101_PAIR_DIR.glob("*_pair_blocks_v02.json")):
        stem = p.name.replace("_pair_blocks_v02.json", "")
        if v101_norm(stem) == n_current:
            return p

    for p in sorted(V101_PAIR_DIR.glob("*_pair_blocks_v02.json")):
        stem = p.name.replace("_pair_blocks_v02.json", "")
        if n_current in v101_norm(stem) or v101_norm(stem) in n_current:
            return p

    return None


def v101_extract_blocks(data):
    if isinstance(data, list):
        return data
...
def v101_find_existing_path(value):
    if not value:
        return None
...
def v101_resolve_audio_path(block, root):
    for obj in [block, root]:
        ...
        for key in [
            ...
            p = v101_find_existing_path(obj.get(key))
            if p:
                return p
...
def v101_num(obj, keys, default=None):
    if not isinstance(obj, dict):
        return default
...
def v101_read_audio(path):
    path = Path(path).resolve()

    if path in _V101_AUDIO_CACHE:
        return _V101_AUDIO_CACHE[path]

    loaders = []

    try:
        import soundfile  # noqa
        loaders.append(v101_read_soundfile)
    except Exception:
        pass

    loaders.append(v101_read_wav)
    loaders.append(v101_read_ffmpeg)
...
def v101_pair_id(block, fallback):
    try:
        return int(v101_num(block, ["pair", "slice", "slice_index", "index", "id"], fallback))
    except Exception:
        return int(fallback)


def v101_build_slice_map(current):
    pairblock_file = v101_find_pairblock_file(current)

    if pairblock_file is None:
        raise RuntimeError(f"pair_blocks introuvable pour {current}")

    root = v101_load_json(pairblock_file, None)
    blocks = v101_extract_blocks(root)
    ...
    for i, block in enumerate(blocks):
        ...
        pair = v101_pair_id(block, i)
        audio_path = v101_resolve_audio_path(block, root)

        if audio_path is None:
            continue

        source_audio_path = audio_path
        y, sr = v101_read_audio(audio_path)

        start = v101_num(block, ["source_start_sample", "start_sample", "start", "start_frame"], None)
        end = v101_num(block, ["source_end_sample", "end_sample", "end", "end_frame"], None)

        if start is None:
            start_ms = v101_num(block, ["source_start_ms", "start_ms"], None)
            if start_ms is not None:
                start = int(start_ms * sr / 1000.0)

        if end is None:
            end_ms = v101_num(block, ["source_end_ms", "end_ms"], None)
        ...
        if end is None:
            dur_ms = v101_num(block, ["duration_ms", "dur_ms"], 250.0)
...
def v101_infer_case_seconds(self, slice_map, source_audio_path):
    """
    Priorité :
    1. BPM de l'app si disponible
    2. longueur du break source / 32
    3. défaut jungle 170 BPM, case = double-croche / 2 = 1/32
    """
    bpm = None
    ...
    if bpm is not None and bpm > 20:
        # Une case = 1/32 note.
        return 60.0 / bpm / 8.0, f"bpm_{bpm:g}"

    try:
        if source_audio_path:
            y, sr = v101_read_audio(source_audio_path)
            dur = len(y) / float(sr)
            if dur > 0.2:
                return dur / 32.0, "source_duration_div_32"
    except Exception:
        pass

    return 60.0 / 170.0 / 8.0, "default_170bpm"


def v101_render_pattern_to_audio(self):
    current = v101_current_break(self)
    pattern = list(getattr(self, "pattern", []) or [])

    if not pattern:
        raise RuntimeError("pattern vide : clique Generate Candidate avant Export WAV")

    slice_map, source_audio_path, pairblock_file = v101_build_slice_map(current)
    case_seconds, timing_source = v101_infer_case_seconds(self, slice_map, source_audio_path)

    sr = 44100

    # Si les slices sont en 48k, on garde le sr de la première slice.
    try:
    ...
    total_cases = 64
    tail_seconds = 0.12
    total_samples = int((total_cases * case_seconds + tail_seconds) * sr)
    ...
    for item in pattern:
        ...
        if pair not in slice_map:
            print(f"[v105] skip pair introuvable : {pair}")
            skipped += 1
            continue
        ...
        if seg_sr != sr:
            # Resampling simple par interpolation, suffisant pour export preview.
    ...
    # Coupe exactement à 64 cases.
    exact_len = int(total_cases * case_seconds * sr)
    out = out[:exact_len]
...
def v101_write_wav(path, audio, sr):
    path = Path(path)
    path.parent.mkdir(parents=True, exist_ok=True)
...
def v101_export_wav(self, event=None):
    try:
        current = v101_current_break(self)
        stamp = time.strftime("%Y%m%d_%H%M%S")
        variant = (
            getattr(self, "v100_current_variant_id", None)
            or getattr(self, "v95_current_variant_id", None)
            or getattr(self, "v94_current_variant_id", None)
            or "pattern"
        )

        audio, sr, info = v101_render_pattern_to_audio(self)

        out_name = f"{v101_slug(current)}_{variant}_v101_{stamp}.wav"
        out_path = V101_EXPORT_DIR / out_name

        v101_write_wav(out_path, audio, sr)
        ...
        print("")
        print("[v105] EXPORT WAV OK")
        print("[v105] wav :", out_path)
        print("[v105] json:", info_path)
        print("[v105] duration:", round(info["duration_seconds"], 3), "s")
        print("[v105] rendered:", info["rendered_notes"], "skipped:", info["skipped_notes"])
        print("")

        v101_status(self, f"Export WAV OK : {out_path}")

    except Exception as exc:
        print("")
        print("[v105] EXPORT WAV ERREUR")
        print("[v105]", exc)
        print("")
        v101_status(self, f"Export WAV erreur : {exc}")

    return "break"


def v101_all_widgets(widget):
    out = [widget]
    ...
    for child in children:
        out.extend(v101_all_widgets(child))

    return out


def v101_find_generate_parent(self):
    for widget in v101_all_widgets(self.root):
        try:
            text = str(widget.cget("text")).strip().lower()
        except Exception:
            continue
...
_old_v101_build_ui = SliceIndexTracker.build_ui


def v101_build_ui(self):
    _old_v101_build_ui(self)

    try:
        self.root.title(f"BreakbeatAI v101 export WAV — {v101_current_break(self)}")
    except Exception:
        pass

    if getattr(self, "_v101_export_button_added", False):
        return

    self._v101_export_button_added = True

    try:
        parent = v101_find_generate_parent(self)
        ...
        try:
            ...
            btn.place(relx=1.0, x=-8, y=8, anchor="ne")

        self.export_wav_button_v101 = btn

    except Exception as exc:
        print(f"[v105] bouton Export WAV impossible : {exc}")

    try:
        self.root.bind("<Control-e>", self.export_wav_v101)
    except Exception:
        pass

    v101_status(self, "v101 : bouton Export WAV ajouté. Ctrl+E exporte aussi.")


SliceIndexTracker.export_wav_v101 = v101_export_wav
SliceIndexTracker.build_ui = v101_build_ui



# ---------------------------------------------------------------------
# v102 EXTENSION : Reject/Bad varie la PREMIÈRE colonne de 32
# ---------------------------------------------------------------------
# - Loop 64 conservé.
# - Export WAV conservé si v101 est présent.
# - Reject / Bad change la première moitié 0-31.
# - Le deuxième 32 devient une réponse structurée.
# - Pas de random libre : uniquement des templates kick/snare/hat.
# - Colonnes : comme v100/v101, -3px.
# ---------------------------------------------------------------------

V102_OVERRIDES_PATH = Path("dataset/learning/break_role_overrides_v01.json")
V102_LOCKS_PATH = Path("dataset/learning/main_role_locks_v87.json")
V102_STATE_PATH = Path("dataset/learning/first32_variants_v102.json")

# Variations STRUCTURÉES du premier 32.
# Les snares restent sur la logique que tu as validée :
# 04/05, 12/13, 20/21, 28/29.
V102_FIRST_32_VARIANTS = [
    {
...
# Deuxième 32 : réponse structurée, moins prioritaire.
# Il suit la variation du premier 32 avec +32,
# puis quelques réponses peuvent apparaître selon l'index.
V102_SECOND_32_RESPONSES = [
    "same_plus_32",
    "answer_kick_bar2",
    "answer_kick_bar4",
    "late_kick_response",
...
V102_SNARE_STEPS = [4, 12, 20, 28, 36, 44, 52, 60]

V102_WIDTH_ATTR_NAMES = [
    "step_w", "cell_w", "col_w", "column_w",
    "grid_step_w", "step_width", "cell_width",
    "col_width", "note_width", "beat_w",
    "x_step_w", "x_cell_w", "tracker_step_w",
...
def v102_load_json(path, default):
    try:
        return json.loads(Path(path).read_text(encoding="utf-8"))
    except Exception:
        return default
...
def v102_find_record(data, current):
    br = data.get("breaks", {}).get(current)
    if br is not None:
        return br

    for name, obj in data.get("breaks", {}).items():
        if v102_norm(name) == v102_norm(current):
            return obj

    return None


def v102_force_64_columns_minus_3(self):
    for attr in [
        "total_steps", "loop_steps", "grid_steps", "pattern_steps",
        "num_steps", "step_count", "n_steps", "steps",
        "cols", "columns", "grid_cols", "loop_len_steps",
    ...
    if not getattr(self, "_v102_columns_minus_3_done", False):
        self._v102_columns_minus_3_done = True

        for attr in V102_WIDTH_ATTR_NAMES:
            try:
                if hasattr(self, attr):
                    value = getattr(self, attr)
                    if isinstance(value, (int, float)) and value > 4:
...
def v102_load_roles(self):
    current = v102_current_break(self)
    ...
    data = v102_load_json(V102_OVERRIDES_PATH, {"breaks": {}})
    br = v102_find_record(data, current)
...
def v102_load_locks(self):
    current = v102_current_break(self)
    data = v102_load_json(V102_LOCKS_PATH, {"breaks": {}})
    br = v102_find_record(data, current)
...
def v102_candidates(self, roles, locks, role):
    out = []
...
def v102_choose_main_pairs(self, roles, locks, role_offset=0):
    chosen = {}
    used = set()

    for role in ["snare", "kick", "hat"]:
        candidates = v102_candidates(self, roles, locks, role)
...
def v102_save_state(data):
    data["updated_at"] = time.strftime("%Y-%m-%d %H:%M:%S")
    v102_save_json(V102_STATE_PATH, data)


def v102_next_first32_index(self, reject_mode=False):
    current = v102_current_break(self)
    data = v102_load_state()
    br = data["breaks"].setdefault(current, {})
    ...
    idx = (last + 1) % len(V102_FIRST_32_VARIANTS)

    br["first32_index"] = idx
    br["last_variant"] = V102_FIRST_32_VARIANTS[idx]["id"]
    br["updated_at"] = time.strftime("%Y-%m-%d %H:%M:%S")

    v102_save_state(data)
...
def v102_make_second32_from_first32(first_grid, response_id):
    # Base : copie du premier 32 avec +32.
    second = []

    for step, role in first_grid:
        second.append((step + 32, role))

    # Réponses structurées sans casser les snares.
    if response_id == "answer_kick_bar2":
        second = [
            (32, "kick"), (34, "hat"),  (36, "snare"), (38, "hat"),
            (40, "kick"), (42, "hat"),  (44, "snare"), (46, "hat"),
            (48, "kick"), (50, "hat"),  (52, "snare"), (54, "hat"),
    ...
    elif response_id == "answer_kick_bar4":
        second = [
            (32, "kick"), (34, "hat"),  (36, "snare"), (38, "hat"),
            (40, "hat"),  (42, "kick"), (44, "snare"), (46, "hat"),
            (48, "kick"), (50, "hat"),  (52, "snare"), (54, "hat"),
    ...
    elif response_id == "late_kick_response":
        second = [
            (32, "kick"), (34, "hat"),  (36, "snare"), (38, "hat"),
            (40, "hat"),  (42, "kick"), (44, "snare"), (46, "hat"),
            (48, "hat"),  (50, "kick"), (52, "snare"), (54, "hat"),
...
def v102_alt_pair_for_second_32(self, roles, chosen, role, occurrence, role_offset):
    main_pair = int(chosen[role]["pair"])
...
def v102_make_note(self, step, pair, role, source, first32_id, response_id, role_offset):
    step = int(step)
    ...
    return {
        ...
        "hit_slot": int(step) // 2,
        "ai_generated": True,
        "ai_model": "v102_first32_variations",
        "learned_role": str(role),
        "prior_label": str(role),
        "main_role_source": str(source),
        "v102_loop64": True,
        "v102_first32_variant": str(first32_id),
        "v102_second32_response": str(response_id),
        "v102_role_offset": int(role_offset),
    }


def v102_validate_pattern(pattern):
    if len(pattern) != 32:
        raise RuntimeError(f"BUG v102 : pattern devrait avoir 32 notes, obtenu {len(pattern)}")
    ...
    for item in pattern:
        ...
        if x % 2 != 0:
            raise RuntimeError(f"BUG v102 : départ impair {x}")

        if x < 0 or x > 62:
            raise RuntimeError(f"BUG v102 : step hors 64 : {x}")

        if x in occupied or (x + 1) in occupied:
            raise RuntimeError(f"BUG v102 : collision à {x}/{x+1}")

        occupied.add(x)
        occupied.add(x + 1)

        if role == "snare" and x not in V102_SNARE_STEPS:
            raise RuntimeError(f"BUG v102 : snare à {x}, interdit")


def v102_generate(self, event=None, reject_mode=False):
    v102_force_64_columns_minus_3(self)

    current = v102_current_break(self)
    roles = v102_load_roles(self)
    locks = v102_load_locks(self)

    first_idx, role_offset = v102_next_first32_index(self, reject_mode=reject_mode)
    first_variant = V102_FIRST_32_VARIANTS[first_idx]
    first32_id = first_variant["id"]
    first_grid = first_variant["grid"]

    response_id = V102_SECOND_32_RESPONSES[first_idx % len(V102_SECOND_32_RESPONSES)]
    second_grid = v102_make_second32_from_first32(first_grid, response_id)

    chosen = v102_choose_main_pairs(self, roles, locks, role_offset=role_offset)

    self.v102_first32_index = int(first_idx)
    self.v102_current_first32_id = str(first32_id)
    self.v102_current_response_id = str(response_id)
    self.v102_role_offset = int(role_offset)

    full_grid = list(first_grid) + list(second_grid)

    print("")
    if reject_mode:
        print("[v105] REJECT/BAD → VARIATION DU PREMIER 32 + AUTRES LIGNES")
    else:
        print("[v105] GENERATE 64 — PREMIER 32 VARIABLE")
    print("[v105] break:", current)
    print("[v105] first32:", first32_id)
    print("[v105] second32 response:", response_id)
    print("[v105] role_offset:", role_offset)
    print("[v105] locks:", locks)
    for role in ["kick", "snare", "hat"]:
        print(
    ...
    for step, role in full_grid:
        if step < 32:
            ...
            occ = second_occ.get(role, 0)
            second_occ[role] = occ + 1
            pair, source = v102_alt_pair_for_second_32(self, roles, chosen, role, occ, role_offset)

        pattern.append(v102_make_note(self, step, pair, role, source, first32_id, response_id, role_offset))

    v102_validate_pattern(pattern)
    ...
    try:
        self.write_latest_pattern(reason="v102_first32_variations")
    except Exception as exc:
        print(f"[v105] write_latest_pattern impossible : {exc}")
    ...
    for item in self.pattern:
        ...
        print(
            ...
            f"pair={item.get('pair')} "
            f"lane={item.get('lane')} "
            f"first32={item.get('v102_first32_variant')} "
            f"response={item.get('v102_second32_response')}"
        )
    print("")

    if reject_mode:
        v102_status(self, f"v102 Reject : premier 32 = {first32_id}, lignes offset {role_offset}.")
    else:
        v102_status(self, f"v102 : premier 32 variable = {first32_id}. Reject change cette zone.")
    return "break"


def v102_reject_bad(self, event=None):
    return v102_generate(self, event=event, reject_mode=True)


def v102_all_widgets(widget):
    out = [widget]
    ...
    for child in children:
        out.extend(v102_all_widgets(child))

    return out


def v102_patch_buttons(self):
    gen_patched = 0
    bad_patched = 0

    for widget in v102_all_widgets(self.root):
        try:
            text = str(widget.cget("text")).strip().lower()
        except Exception:
            continue
        ...
        try:
            if text == "generate candidate":
                ...
                widget.configure(command=self.reject_bad_v102)
                bad_patched += 1
        except Exception:
            pass
...
_old_v102_draw = getattr(SliceIndexTracker, "draw", None)

def v102_draw(self, *args, **kwargs):
    v102_force_64_columns_minus_3(self)

    if callable(_old_v102_draw):
        result = _old_v102_draw(self, *args, **kwargs)
    else:
        result = None
...
_old_v102_build_ui = SliceIndexTracker.build_ui


def v102_build_ui(self):
    _old_v102_build_ui(self)

    v102_force_64_columns_minus_3(self)

    try:
        self.root.title(f"BreakbeatAI v102 first32 variations — {v102_current_break(self)}")
    except Exception:
        pass

    try:
        gen_patched, bad_patched = v102_patch_buttons(self)
        print(f"[v105] boutons patchés : Generate={gen_patched}, Reject/Bad={bad_patched}")
    except Exception as exc:
        print(f"[v105] patch boutons impossible : {exc}")
    ...
    try:
        self.root.bind("<F9>", self.reject_bad_v102)
    except Exception:
        pass

    v102_status(self, "v102 : Reject/Bad change la première colonne de 32. F12/F9.")


SliceIndexTracker.generate_candidate = v102_generate
SliceIndexTracker.generate_full_candidate = v102_generate
SliceIndexTracker.generate_ai_pattern = v102_generate
SliceIndexTracker.generate_cross_song_template = v102_generate
SliceIndexTracker.generate_safe_experiment = v102_generate
SliceIndexTracker.generate_locked_roles = v102_generate
SliceIndexTracker.generate_role_aware = v102_generate
SliceIndexTracker.generate_safe_role_ai = v102_generate

SliceIndexTracker.reject_bad_v102 = v102_reject_bad
SliceIndexTracker.reject_bad = v102_reject_bad
SliceIndexTracker.bad_candidate = v102_reject_bad
SliceIndexTracker.reject_candidate = v102_reject_bad
SliceIndexTracker.reject_pattern = v102_reject_bad

if callable(_old_v102_draw):
    SliceIndexTracker.draw = v102_draw

SliceIndexTracker.build_ui = v102_build_ui



# ---------------------------------------------------------------------
# v103 EXTENSION : Export WAV Current tout en haut
# ---------------------------------------------------------------------
# Corrige :
# - Export WAV ne doit pas rester bloqué sur Camo.
# - Le bouton Export WAV doit être visible tout en haut, comme Generate Candidate.
# - Les anciens boutons Export WAV sont repatchés vers cette fonction.
# ---------------------------------------------------------------------

V103_EXPORT_DIR = Path("exports")
V103_PAIR_DIR = Path("dataset/pair_blocks_v02")


def v103_status(self, text):
    try:
        if hasattr(self, "set_status"):
            self.set_status(text)
        else:
...
def v103_pairblock_name_from_path(path):
    name = Path(path).name
    if name.endswith("_pair_blocks_v02.json"):
        return name[:-len("_pair_blocks_v02.json")]
    return Path(path).stem


def v103_candidate_path(value):
    if value is None:
        return None
    ...
    if p.is_absolute():
        ...
        candidates.append(Path(".") / p)
        candidates.append(V103_PAIR_DIR / p.name)

    for c in candidates:
        if c.exists() and c.name.endswith("_pair_blocks_v02.json"):
            return c

    return None


def v103_find_current_pairblock_file(self):
    """
    Trouve le pair_block réellement chargé dans l'app.
    Priorité aux chemins pair_blocks présents dans l'objet app,
    parce que self.project['safe'] peut rester bloqué sur Camo.
    """
    priority_attrs = [
        "pairblock_path",
        "pairblocks_path",
        "pair_blocks_path",
        "pair_block_path",
        "current_pairblock_path",
        "current_pair_blocks_path",
        "pairblock_file",
        "pair_blocks_file",
        "pair_block_file",
        "current_pairblock_file",
        "current_pair_blocks_file",
        "blocks_path",
        "current_blocks_path",
    ]
    ...
    for attr in priority_attrs:
        try:
            if hasattr(self, attr):
                p = v103_candidate_path(getattr(self, attr))
                if p:
                    return p, f"attr:{attr}"
        except Exception:
            pass
    ...
    try:
        ...
        if isinstance(project, dict):
            for key, value in project.items():
                p = v103_candidate_path(value)
                if p:
                    return p, f"project:{key}"
    except Exception:
        pass
        ...
        for key, value in vars(self).items():
            ...
            if isinstance(value, dict):
                for k2, v2 in value.items():
                    p = v103_candidate_path(v2)
                    if p:
                        return p, f"self:{key}.{k2}"
    except Exception:
        pass
...
def v103_guess_current_break_from_attrs(self):
    priority_attrs = [
        "current_source",
        "source",
        "selected_source",
        ...
        "selected_break",
        "active_break",
        "v102_current_first32_id",
        "v100_current_variant_id",
        "v95_current_variant_id",
    ]
        ...
        try:
            if hasattr(self, attr):
                ...
                if isinstance(value, str) and value.strip():
                    if "_pair_blocks_v02" not in value:
                        return value.strip(), f"attr:{attr}"
        except Exception:
            pass
    ...
    try:
        ...
        if isinstance(project, dict):
            for key in ["current_source", "source", "safe", "name", "break", "break_name"]:
                ...
                if isinstance(value, str) and value.strip():
                    return value.strip(), f"project:{key}"
    except Exception:
        pass

    return "Camo_Break_-_3A", "fallback"


def v103_current_break(self):
    pairblock_path, source = v103_find_current_pairblock_file(self)

    if pairblock_path:
        current = v103_pairblock_name_from_path(pairblock_path)
        print(f"[v105] current break via pairblock {source}: {current}")
        return current

    current, source = v103_guess_current_break_from_attrs(self)
    print(f"[v105] current break via {source}: {current}")
    return current


def v103_export_wav(self, event=None):
    """
    Export WAV robuste :
    - force v101_current_break à utiliser le break réellement chargé
    - donc l'ancien moteur v101 n'exporte plus systématiquement Camo
    """
    try:
        if "v101_render_pattern_to_audio" not in globals():
            raise RuntimeError("moteur export WAV v101 introuvable dans ce fichier")

        if "v101_write_wav" not in globals():
            raise RuntimeError("fonction write WAV v101 introuvable dans ce fichier")

        current = v103_current_break(self)

        # Monkeypatch volontaire : le moteur v101 continue de faire le rendu,
        # mais son current_break est corrigé par v103.
        globals()["v101_current_break"] = v103_current_break

        stamp = time.strftime("%Y%m%d_%H%M%S")

        variant = (
            getattr(self, "v102_current_first32_id", None)
            or getattr(self, "v100_current_variant_id", None)
            or getattr(self, "v95_current_variant_id", None)
            or getattr(self, "v94_current_variant_id", None)
            or "pattern"
        )

        audio, sr, info = globals()["v101_render_pattern_to_audio"](self)

        # Sécurité : on force le nom exporté sur le break résolu v103.
        info["break_v103_resolved"] = current
        info["exporter"] = "v103_export_current_top"

        out_name = f"{v103_slug(current)}_{v103_slug(variant)}_v103_{stamp}.wav"
        out_path = V103_EXPORT_DIR / out_name

        globals()["v101_write_wav"](out_path, audio, sr)
        ...
        print("")
        print("[v105] EXPORT WAV CURRENT OK")
        print("[v105] break:", current)
        print("[v105] wav  :", out_path)
        print("[v105] json :", info_path)
        print("[v105] duration:", round(float(info.get("duration_seconds", 0)), 3), "s")
        print("[v105] rendered:", info.get("rendered_notes"), "skipped:", info.get("skipped_notes"))
        print("")

        v103_status(self, f"Export WAV OK : {out_path}")
        ...
        print("[v105]", exc)
        print("")
        v103_status(self, f"Export WAV erreur : {exc}")

    return "break"


def v103_all_widgets(widget):
    out = [widget]
    ...
    for child in children:
        out.extend(v103_all_widgets(child))

    return out


def v103_find_generate_parent(self):
    for widget in v103_all_widgets(self.root):
        try:
            text = str(widget.cget("text")).strip().lower()
        except Exception:
            continue
...
def v103_patch_export_buttons(self):
    patched = 0
    ...
    for widget in v103_all_widgets(self.root):
        ...
        if "export wav" in text:
            try:
                widget.configure(command=self.export_wav_v103)
                patched += 1
            except Exception:
                pass
...
_old_v103_build_ui = SliceIndexTracker.build_ui


def v103_build_ui(self):
    _old_v103_build_ui(self)

    try:
        self.root.title(f"BreakbeatAI v103 export current — {v103_current_break(self)}")
    except Exception:
        pass

    # Tous les anciens boutons Export WAV pointent maintenant vers v103.
    try:
        patched = v103_patch_export_buttons(self)
        print(f"[v105] boutons Export WAV patchés : {patched}")
    except Exception as exc:
        print(f"[v105] patch export buttons impossible : {exc}")

    # Ajout d'un bouton visible dans la même zone que Generate Candidate.
    try:
        if not getattr(self, "_v103_export_top_button_added", False):
            parent = v103_find_generate_parent(self)
            ...
            btn = tk.Button(
                parent,
                text="Export WAV Current",
                command=self.export_wav_v103,
                bg="#2d4f3a",
                fg="#ffffff",
            )
            ...
            try:
                ...
                btn.place(relx=0.5, y=38, anchor="n")

            self.export_wav_current_button_v103 = btn
            self._v103_export_top_button_added = True
    except Exception as exc:
        print(f"[v105] bouton top Export WAV impossible : {exc}")

    try:
        self.root.bind("<Control-e>", self.export_wav_v103)
    except Exception:
        pass

    v103_status(self, "v103 : Export WAV Current ajouté en haut. Ctrl+E exporte aussi.")


SliceIndexTracker.export_wav_v103 = v103_export_wav

# Les anciens boutons qui appelaient v101 appellent maintenant le correctif v103.
SliceIndexTracker.export_wav_v101 = v103_export_wav

SliceIndexTracker.build_ui = v103_build_ui



# ---------------------------------------------------------------------
# v104 EXTENSION : bouton Export WAV Current visible tout en haut
# ---------------------------------------------------------------------
# Le bouton est placé en overlay stable en haut à droite,
# à côté de la zone Generate Candidate / Good / Reject.
# ---------------------------------------------------------------------


def v104_status(self, text):
    try:
        if hasattr(self, "set_status"):
            self.set_status(text)
        else:
...
def v104_call_export_current(self, event=None):
    """
    Utilise le meilleur export disponible :
    - v103 si présent : export du break réellement chargé
    - v101 sinon : ancien moteur export
    """
    if hasattr(self, "export_wav_v103"):
        return self.export_wav_v103(event)

    if hasattr(self, "export_wav_v101"):
        return self.export_wav_v101(event)

    v104_status(self, "Export WAV introuvable dans cette version.")
    print("[v105] ERREUR : aucune fonction export_wav_v103/v101 trouvée.")
    return "break"


def v104_current_break_label(self):
    try:
        if "v103_current_break" in globals():
            return globals()["v103_current_break"](self)
    except Exception:
        pass

    try:
        if "v102_current_break" in globals():
            return globals()["v102_current_break"](self)
    except Exception:
        pass
...
def v104_add_visible_export_top(self):
    if getattr(self, "_v104_export_visible_top_added", False):
        try:
            self.v104_export_top_frame.lift()
        except Exception:
            pass
        return

    self._v104_export_visible_top_added = True
    ...
    frame = tk.Frame(
        ...
        relief="ridge",
        highlightthickness=1,
        highlightbackground="#7dffd1",
    )

    # Position volontairement haute, entre le titre et les boutons Generate.
    # Dans ta capture, c'est la zone libre avant le bloc Generate Candidate.
    frame.place(relx=0.54, y=30, anchor="n")
    frame.lift()

    btn = tk.Button(
        frame,
        text="Export WAV Current",
        command=self.export_wav_visible_top_v104,
        bg="#2d7a4b",
        fg="#ffffff",
    ...
    self.v104_export_top_frame = frame
    self.v104_export_top_button = btn

    def keep_visible():
        try:
            frame.lift()
            self.root.after(1200, keep_visible)
        except Exception:
            pass
...
def v104_all_widgets(widget):
    ...
    for child in children:
        out.extend(v104_all_widgets(child))

    return out


def v104_patch_export_buttons(self):
    patched = 0

    for widget in v104_all_widgets(self.root):
        try:
            text = str(widget.cget("text")).strip().lower()
        except Exception:
            continue
        ...
        if "export wav" in text:
            try:
                widget.configure(command=self.export_wav_visible_top_v104)
                patched += 1
            except Exception:
                pass
...
_old_v104_build_ui = SliceIndexTracker.build_ui


def v104_build_ui(self):
    _old_v104_build_ui(self)

    try:
        self.root.title(f"BreakbeatAI v104 export visible — {v104_current_break_label(self)}")
    except Exception:
        pass

    try:
        v104_add_visible_export_top(self)
    except Exception as exc:
        print(f"[v105] bouton Export visible impossible : {exc}")

    try:
        patched = v104_patch_export_buttons(self)
        print(f"[v105] boutons Export WAV patchés : {patched}")
    except Exception as exc:
        print(f"[v105] patch export impossible : {exc}")

    try:
        self.root.bind("<Control-e>", self.export_wav_visible_top_v104)
    except Exception:
        pass

    v104_status(self, "v104 : bouton Export WAV Current visible en haut. Ctrl+E aussi.")


SliceIndexTracker.export_wav_visible_top_v104 = v104_call_export_current
SliceIndexTracker.build_ui = v104_build_ui



# ---------------------------------------------------------------------
# v105 EXTENSION : Export WAV Current dans la barre Generate Candidate
# ---------------------------------------------------------------------
# Corrige v104 :
# - plus de bouton flottant superposé
# - Export WAV Current est ajouté juste après Reject / Bad
# - les anciens boutons Export WAV du bas sont retirés
# ---------------------------------------------------------------------


def v105_status(self, text):
    try:
        if hasattr(self, "set_status"):
            self.set_status(text)
        else:
...
def v105_call_export_current(self, event=None):
    if hasattr(self, "export_wav_v103"):
        return self.export_wav_v103(event)
    ...
    if hasattr(self, "export_wav_visible_top_v104"):
        return self.export_wav_visible_top_v104(event)

    v105_status(self, "Export WAV introuvable.")
    print("[v105] ERREUR : aucune fonction export trouvée.")
    return "break"


def v105_all_widgets(widget):
    out = [widget]
    ...
    for child in children:
        out.extend(v105_all_widgets(child))

    return out


def v105_widget_text(widget):
    try:
...
def v105_destroy_old_export_buttons(self):
    """
    Supprime :
    - bouton flottant v104
    - boutons Export WAV du bas
    Puis v105 remet un seul bouton dans la barre Generate.
    """
    try:
        frame = getattr(self, "v104_export_top_frame", None)
        if frame is not None:
            frame.destroy()
            self.v104_export_top_frame = None
    except Exception:
        pass

    destroyed = 0

    for widget in list(v105_all_widgets(self.root)):
        text = v105_widget_text(widget).lower()
    ...
    print(f"[v105] anciens boutons Export WAV supprimés : {destroyed}")


def v105_find_generate_bar(self):
    """
    Trouve la vraie barre du haut :
    Generate Candidate / Good après modifs / Good sans train / Reject Bad.
    On choisit le parent qui contient le plus de ces boutons,
    et le plus haut dans la fenêtre.
    """
    wanted = {
        "generate candidate",
        "good après modifs",
    ...
    for widget in v105_all_widgets(self.root):
        text = v105_widget_text(widget).lower()
        ...
        try:
            y = int(parent.winfo_rooty())
            groups[parent]["y"] = min(groups[parent]["y"], y)
        except Exception:
            pass
    ...
    for parent, info in groups.items():
        # Plus il contient de boutons, mieux c'est.
        # À égalité, on prend le plus haut.
        score = (-info["count"], info["y"])
...
def v105_add_export_after_reject(self):
    v105_destroy_old_export_buttons(self)

    parent, widgets = v105_find_generate_bar(self)

    if parent is None:
        print("[v105] barre Generate introuvable, fallback root.")
        parent = self.root
        widgets = {}
    ...
    btn = tk.Button(
        parent,
        text="Export WAV Current",
        command=self.export_wav_current_v105,
        bg="#2d7a4b",
        fg="#ffffff",
        activebackground="#39a96b",
        activeforeground="#ffffff",
    ...
    try:
        manager = reject_widget.winfo_manager() if reject_widget is not None else ""
    except Exception:
        manager = ""
    ...
    if manager == "pack":
        try:
            ...
            print(f"[v105] pack after Reject impossible : {exc}")

    if not placed and manager == "grid":
        try:
            ...
            print(f"[v105] grid après Reject impossible : {exc}")
    ...
    self.export_wav_current_button_v105 = btn

    print("[v105] bouton Export WAV Current ajouté dans la barre Generate.")
    return btn


_old_v105_build_ui = SliceIndexTracker.build_ui


def v105_build_ui(self):
    _old_v105_build_ui(self)

    try:
        self.root.title("BreakbeatAI v105 export in generate bar")
    except Exception:
        pass

    try:
        v105_add_export_after_reject(self)
    except Exception as exc:
        print(f"[v105] ajout bouton export impossible : {exc}")

    try:
        self.root.bind("<Control-e>", self.export_wav_current_v105)
    except Exception:
        pass

    v105_status(self, "v105 : Export WAV Current placé après Reject / Bad. Ctrl+E aussi.")


SliceIndexTracker.export_wav_current_v105 = v105_call_export_current

# Les anciens noms pointent aussi vers la bonne fonction.
SliceIndexTracker.export_wav_visible_top_v104 = v105_call_export_current
SliceIndexTracker.export_wav_v103 = getattr(SliceIndexTracker, "export_wav_v103", v105_call_export_current)

SliceIndexTracker.build_ui = v105_build_ui
