...
"""
BreakbeatAI Tracker Editor v118 — slice index
...
def warp_audio_to_length_external(audio, target_len, cache_key=None):
    ...
    if rubberband:
        try:
            ...
            print(f"[v118] rubberband échec, fallback ffmpeg/no-speed : {exc}")
    ...
    if ffmpeg:
        try:
            ...
            print(f"[v118] ffmpeg atempo échec, fallback no-speed : {exc}")
...
class SliceIndexTracker:
    def __init__(self, root, pair_json):
        ...
        self.root.title("BreakbeatAI Tracker Editor v118 — slice index")
    ...
    def detect_external_player(self):
        for name in ["pw-play", "paplay", "aplay", "ffplay"]:
            ...
            if path:
                print(f"[v118] Backend audio : {name} -> {path}")
                return name

        print("[v118] Aucun backend système trouvé, fallback sounddevice.")
        return None
    ...
    def build_initial_pattern(self, randomize=False):
        ...
        print(
            f"[v118] Génération pattern | break={self.project['safe']} | "
        ...
        for slot in range(max_items):
            ...
            print(f"[v118] slot {slot:02d} | step {x_step:02d} -> slice {pair:02d}")
    ...
    def build_ui(self):
        ...
        title = tk.Label(
            main,
            text="BreakbeatAI v118 — select slice rows",
            bg="#111018",
            fg="#ff7acc",
            font=("Sans", 20, "bold"),
        )
    ...
    def record_correction(self, event_type, before=None, after=None):
        ...
        print(f"[v118] correction sauvegardée : {event_type}")
    ...
    def play_audition_array(self, audio, label="audition"):
        ...
        audition_wav = OUT_DIR / f"{self.project['safe']}_v118_audition.wav"
        ...
        if self.external_player:
            try:
                ...
                print(f"[v118] Erreur audition {self.external_player} : {exc}")
    ...
    def on_close(self):
        ...
        """
        print("[v118] Fermeture : arrêt audio + destruction fenêtre")
    ...
    def play_audio_array(self, audio, label="audio"):
        ...
        audio = normalize(audio)
        live_wav = OUT_DIR / f"{self.project['safe']}_v118_live.wav"
        ...
        if self.external_player:
            try:
                ...
                print(f"[v118] Erreur backend {self.external_player} : {exc}")
    ...
    def render_preview_file(self):
        OUT_DIR.mkdir(parents=True, exist_ok=True)
        wav = OUT_DIR / f"{self.project['safe']}_tracker_app_v118_preview.wav"
    ...
    def write_latest_pattern(self, reason="correction"):
        ...
        path = LATEST_DIR / f"{self.project['safe']}_latest_slice_index_v118.json"

        data = {
            "version": "breakbeatai_latest_slice_index_v118",
            "reason": reason,
    ...
    def save(self):
        ...
        tracker_path = OUT_DIR / f"{self.project['safe']}_tracker_app_edit_v118.json"
        ...
        stamp = file_stamp()
        validated_path = VALIDATED_DIR / f"{self.project['safe']}_validated_slice_index_loop64_v118_{stamp}.json"

        validated = {
            "version": "breakbeatai_validated_slice_index_loop64_v118",
...
def v44_record_correction(self, event_type, before=None, after=None):
    ...
    print(f"[v118] correction sauvegardée : {event_type}")
    print(f"[v118] correction path : {corrections_path}")
    print(f"[v118] latest path : {latest_path}")
...
def v45_move_selected(self, dx, dy):
    ...
    if dy != 0 and before != after:
        try:
            ...
            print(f"[v118] audition après flèche haut/bas impossible : {exc}")
...
def v46_warp_loop_pitch_preserve(audio, target_len, cache_key):
    ...
    if rubberband:
        try:
            ...
            print(f"[v118] rubberband global warp échec : {exc}")
    ...
    if ffmpeg:
        try:
            ...
            print(f"[v118] ffmpeg global warp échec : {exc}")

    print("[v118] Aucun warp pitch-preserve dispo, fallback fit no-speed.")
    return v46_fit_audio_no_speed(audio, target_len)
...
def v46_build_initial_pattern(self, randomize=False):
    ...
    print(
        f"[v118] M8 pattern | break={self.project['safe']} | "
...
def v46_get_warped_loop(self):
    ...
    print(
        f"[v118] global warp OK | source_len={loop_end-loop_start} | "
...
def v46_build_ui(self):
    ...
    try:
        ...
        print(f"[v118] UI extension impossible : {exc}")
...
def v47_play_audio_array(self, audio, label="audio"):
    ...
    if len(audio) == 0 or v47_is_silent(audio):
        ...
        print(f"[v118] lecture annulée : audio silencieux ({label})")
        return 0
...
def v47_toggle_loop(self):
    ...
    if not getattr(self, "pattern", None):
        ...
        print("[v118] loop annulée : pattern vide")
        return "break"
...
def v47_delete_selected(self):
    ...
    if not getattr(self, "pattern", None):
        ...
        print("[v118] pattern vide après suppression : audio coupé")
...
def v48_build_initial_pattern(self, randomize=False):
    ...
    print("[v118] pattern initial forcé VIDE : aucun son ne doit jouer.")
    return []
...
def v48_load_first_slices(self):
    ...
    print(f"[v118] Load 16 slices : {len(self.pattern)} items")


def v48_debug_pattern(self, label="render"):
    print("")
    print(f"[v118] DEBUG PATTERN avant {label}")
    print(f"[v118] items = {len(getattr(self, 'pattern', []) or [])}")

    if not getattr(self, "pattern", None):
        print("[v118] pattern vide confirmé")
        print("")
        return
    ...
    for item in self.pattern:
        ...
        print(
            f"[v118] id={item_id} | step {x:02d}/{end:02d} | "
...
def v48_render_audio_with_timeline(self):
    ...
    if not getattr(self, "pattern", None):
        loop_samples = self.get_loop_samples()
        print(f"[v118] rendu silence total : pattern vide | samples={loop_samples}")
        return np.zeros(loop_samples, dtype=np.float32), []
    ...
    print(f"[v118] rendu audio peak={peak:.8f} | timeline_events={len(timeline) if timeline is not None else 'None'}")

    if timeline:
        print("[v118] timeline events:")
        for ev in timeline:
            print(f"[v118]   {ev}")
...
def v48_toggle_loop(self):
    if not getattr(self, "pattern", None):
        ...
        print("[v118] Space ignoré : pattern vide.")
        return "break"
...
def v48_build_ui(self):
    ...
    try:
        ...
        print(f"[v118] ajout UI impossible : {exc}")
...
def v49_add_item_at_xy(self, x, y):
    ...
    try:
        ...
        print(f"[v118] record_correction add impossible : {exc}")
    ...
    try:
        ...
        print(f"[v118] audition après ajout impossible : {exc}")

    print(f"[v118] add | id={item['id']} | step={x_step} | pair={pair} | lane={lane}")
...
def v49_duplicate_selected_forward(self, event=None):
    ...
    if item is None:
        ...
        print("[v118] duplicate annulé : aucune note sélectionnée")
        return "break"
    ...
    if new_x > max_x:
        ...
        print("[v118] duplicate annulé : fin de grille")
        return "break"
    ...
    try:
        ...
        print(f"[v118] record_correction duplicate impossible : {exc}")
    ...
    try:
        ...
        print(f"[v118] audition après duplicate impossible : {exc}")

    print(
        f"[v118] duplicate | old_id={item.get('id')} | new_id={new_item.get('id')} | "
...
def v49_build_ui(self):
    ...
    try:
        ...
        print(f"[v118] bind canvas click impossible : {exc}")
    ...
    try:
        ...
        print(f"[v118] bind keys impossible : {exc}")
...
def v50_load_style_model():
    ...
    try:
        ...
        print(f"[v118] impossible de charger le modèle IA : {exc}")
        return None
...
def v50_generate_ai_pattern(self, event=None):
    ...
    if model is None:
        ...
        print("[v118] modèle absent :", STYLE_MODEL_PATH)
        return "break"
    ...
    print("")
    print(f"[v118] GENERATE IA | safe={safe} | pairs={pair_values} | slots={max_slots}")
    ...
    for slot in range(max_slots):
        ...
        print(
            f"[v118] slot {slot:02d} | step {x_step:02d} | "
    ...
    try:
        ...
        print(f"[v118] write_latest_pattern impossible : {exc}")
...
def v50_build_ui(self):
    ...
    try:
        ...
        print(f"[v118] UI IA impossible : {exc}")
...
def v51_mouse_down(self, event):
    ...
    if item is not None:
        ...
        print(
            f"[v118] mouse down note | id={item.get('id')} | "
            f"step={item.get('x_step')} | pair={item.get('pair')}"
        )
    else:
        print("[v118] mouse down empty")
...
def v51_mouse_up(self, event):
    ...
    if state.get("empty"):
        ...
        print("[v118] drag depuis vide ignoré")
        return "break"
    ...
    if before != after:
        try:
            ...
            print(f"[v118] record_correction drag impossible : {exc}")
        ...
        print(
            f"[v118] drag move | id={after.get('id')} | "
            f"step {before.get('x_step')} -> {after.get('x_step')} | "
        ...
        try:
            ...
            print(f"[v118] audition après drag impossible : {exc}")
...
def v51_build_ui(self):
    ...
    try:
        ...
        print(f"[v118] bind souris impossible : {exc}")
    ...
    try:
        ...
        print(f"[v118] bind Suppr impossible : {exc}")
...
def v52_normalize_loaded_notes(self, notes):
    ...
    for raw in sorted(notes, key=lambda n: (int(float(n.get("x_step", 0))), int(float(n.get("pair", 0))))):
        ...
        if pair not in valid_pairs:
            print(f"[v118] slice ignorée car absente du break actuel : pair={pair}")
            continue
...
def v52_load_saved_beat(self, event=None):
    ...
    if not saves:
        ...
        print("[v118] aucune sauvegarde trouvée")
        return "break"
    ...
    if not pattern:
        ...
        print("[v118] sauvegarde incompatible :", chosen["path"])
        return "break"
    ...
    print("")
    print("[v118] BEAT RESTAURÉ")
    print("[v118] fichier :", chosen["path"])
    print("[v118] clé :", chosen["key"])
    print("[v118] notes :", len(pattern))
    print("")
...
def v52_list_saved_beats(self, event=None):
    ...
    print("")
    print("[v118] SAUVEGARDES TROUVÉES")
    for i, save in enumerate(saves[:30]):
        print(
            f"[v118] {i:02d} | notes={len(save['notes']):02d} | "
...
def v52_build_ui(self):
    ...
    try:
        ...
        print(f"[v118] UI load saved impossible : {exc}")
...
def v53_run_training(self):
    ...
    if not trainer.exists():
        ...
        print("[v118] trainer introuvable :", trainer)
        return False

    print("")
    print("[v118] AUTO-TRAIN IA après Save...")
    print("")
    ...
    try:
        ...
        if result.returncode != 0:
            ...
            print("[v118] auto-train échec code :", result.returncode)
            return False

        v53_status(self, "Save OK + IA entraînée automatiquement.")
        print("[v118] AUTO-TRAIN OK")
        return True

    except Exception as exc:
        v53_status(self, f"Save OK, mais auto-train impossible : {exc}")
        print("[v118] exception auto-train :", exc)
        return False
...
def v53_save(self):
    ...
    try:
        ...
        print(f"[v118] auto-train après save impossible : {exc}")
...
def v53_build_ui(self):
    ...
    try:
        ...
        print(f"[v118] UI auto-train impossible : {exc}")
...
def v55_mouse_up(self, event):
    ...
    if not state.get("moved"):
        ...
        try:
            ...
            print(f"[v118] audition clic note impossible : {exc}")
        ...
        print(
            f"[v118] click play/select | id={item.get('id')} | "
            f"step={item.get('x_step')} | pair={item.get('pair')}"
        )
...
def v58_add_item_at_xy(self, x, y):
    ...
    print(
        f"[v118] add | id={item['id']} | step={x_step} | "
...
def v58_toggle_selected_length(self, event=None):
    ...
    try:
        ...
        print(f"[v118] record_correction length impossible : {exc}")

    v58_refresh_after_edit(self, "toggle_note_length")

    v58_status(self, f"Longueur note : {old_len} -> {new_len} case(s).")
    print(f"[v118] length toggle | id={item.get('id')} | {old_len} -> {new_len}")
...
def v58_duplicate_selected_forward(self, event=None):
    ...
    try:
        ...
        print(f"[v118] audition duplicate impossible : {exc}")
...
def v58_build_ui(self):
    ...
    try:
        ...
        print(f"[v118] UI demi-grille impossible : {exc}")
    ...
    try:
        ...
        print(f"[v118] bind demi-grille impossible : {exc}")
...
def v59_rank_current_slices(self):
    ...
    for pair in self.pair_values:
        ...
        try:
            ...
            print(f"[v118] analyse pair {pair} impossible : {exc}")
            role = "other"
    ...
    print("")
    print("[v118] TOP ROLES CURRENT BREAK")
    for r in ("kick", "snare", "hat"):
        print(f"[v118] {r:5s}:", [(x["pair"], round(x["score"], 3)) for x in ranks[r][:6]])
    print("")
...
def v59_load_role_model():
    ...
    try:
        ...
        print(f"[v118] lecture modèle rôle impossible : {exc}")
        return None
...
def v59_generate_role_aware(self, event=None):
    ...
    if model is None:
        ...
        print("[v118] modèle rôle absent, fallback musical snare 6.")
        model = {
            "breaks": {},
            "global": {},
            "patterns_used": 0,
    ...
    print("")
    print("[v118] ROLE-AWARE GENERATION")
    print("[v118] model patterns_used:", model.get("patterns_used", 0))
    for item in pattern:
        print(
            f"[v118] step {item['x_step']:02d}/{item['x_step'] + item['length'] - 1:02d} | "
...
def v59_run_role_training(self, event=None):
    ...
    try:
        ...
        print(f"[v118] training rôle impossible : {exc}")
...
def v59_save(self):
    ...
    try:
        ...
        print(f"[v118] auto-training rôle après save impossible : {exc}")
...
def v59_build_ui(self):
    ...
    try:
        ...
        print(f"[v118] UI role-aware impossible : {exc}")
...
def v62_add_item_at_xy(self, x, y):
    ...
    print(f"[v118] add | id={item['id']} | step={x_step} | len={length} | pair={pair}")
...
def v62_mouse_down(self, event):
    ...
    if item is not None:
        ...
        print(
            f"[v118] mouse down {state['mode']} | id={item.get('id')} | "
            f"step={item.get('x_step')} | len={item.get('length')} | pair={item.get('pair')}"
        )
    else:
        print("[v118] mouse down empty")
    ...
    if mode == "resize":
        if not state.get("moved"):
            ...
            try:
                ...
                print(f"[v118] audition clic resize impossible : {exc}")
        ...
        if before != after:
            try:
                ...
                print(f"[v118] record_correction resize impossible : {exc}")
            ...
            try:
                ...
                print(f"[v118] audition après resize impossible : {exc}")
    ...
    if mode == "move":
        ...
        if before != after:
            try:
                ...
                print(f"[v118] record_correction move impossible : {exc}")
            ...
            try:
                ...
                print(f"[v118] audition après move impossible : {exc}")
...
def v62_toggle_selected_length(self, event=None):
    ...
    try:
        ...
        print(f"[v118] record_correction toggle length impossible : {exc}")

    v62_refresh_after_edit(self, "toggle_note_length_v62")
    v62_status(self, f"Longueur note : {old_len} → {new_len} case(s).")

    print(f"[v118] toggle length | id={item.get('id')} | {old_len} -> {new_len}")
...
def v62_draw(self):
    ...
    try:
        ...
        print(f"[v118] draw handles impossible : {exc}")
...
def v62_build_ui(self):
    ...
    try:
        ...
        print(f"[v118] bind resize impossible : {exc}")
    ...
    try:
        ...
        print(f"[v118] UI resize impossible : {exc}")
...
def v63_load_model():
    ...
    try:
        ...
        print(f"[v118] lecture modèle strict impossible : {exc}")
        return None
...
def v63_generate_safe_role_ai(self, event=None):
    ...
    print("")
    print("[v118] SAFE ROLE AI GENERATED")
    print("[v118] model strict:", "yes" if model else "no")
    print("[v118] snare main/alt:", main_snare, alt_snare)
    print("[v118] kick  main/alt:", main_kick, alt_kick)
    print("[v118] hat   main/alt:", main_hat, alt_hat)
    print("[v118] pattern:")
    for item in pattern:
        print(
            f"[v118] step {item['x_step']:02d}/{item['x_step'] + item['length'] - 1:02d} | "
...
def v63_run_strict_training(self, event=None):
    ...
    print("")
    print("[v118] TRAIN STRICT ROLE MODEL...")
    print("")
    ...
    try:
        ...
        print(f"[v118] training strict impossible : {exc}")
...
def v63_build_ui(self):
    ...
    try:
        ...
        print(f"[v118] UI safe impossible : {exc}")
...
def v65_rank_slices(self):
    ...
    for pair in self.pair_values:
        ...
        try:
            ...
            print(f"[v118] analyse slice {pair} impossible : {exc}")
            scores = {
                "kick": 0.0,
                "snare": 0.0,
                "hat": 0.0,
    ...
    print("")
    print("[v118] TOP SLICES PAR ROLE — break courant")
    for role in ("kick", "snare", "hat", "ghost_snare"):
        print(f"[v118] {role:12s}:", [(x["pair"], round(x["score"], 3)) for x in ranks[role][:8]])
    print("")
...
def v65_generate_from_role_template(self, event=None):
    ...
    try:
        ...
        print(f"[v118] template introuvable : {exc}")
        return "break"
    ...
    print("")
    print("[v118] CROSS-SONG ROLE TEMPLATE GENERATED")
    print("[v118] template:", V65_TEMPLATE_PATH)
    print("[v118] temperature:", temperature, "| hats:", hat_density, "| ghosts:", ghost_density)
    print("[v118] pattern:")
    for item in pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v118] step {item['x_step']:02d}/{end:02d} | "
...
def v65_build_ui(self):
    ...
    try:
        ...
        print(f"[v118] UI impossible : {exc}")
...
def v67_train_strict_if_available(self):
    ...
    if trainer is None:
        print("[v118] aucun trainer trouvé.")
        return False

    print("")
    print("[v118] TRAIN après validation :", trainer)
    print("")
    ...
    try:
        ...
        print(f"[v118] training impossible : {exc}")
        return False
...
def v67_generate_candidate(self, event=None):
    ...
    print("")
    print("[v118] CANDIDATE GENERATED")
    print("[v118] break:", self.v67_candidate_safe)
    print("[v118] notes:", len(candidate))
    print("[v118] Maintenant : corrige puis clique Good après modifs, ou Reject.")
    print("")
...
def v67_reject_candidate(self, event=None):
    ...
    print("")
    print("[v118] REJECTED")
    print("[v118] saved:", rejected_path)
    print("")
...
def v67_accept_corrected(self, event=None):
    ...
    print("")
    print("[v118] ACCEPTED GOOD AFTER EDIT")
    print("[v118] saved:", validated_path)
    print("[v118] trained:", trained)
    print("")
...
def v67_accept_without_training(self, event=None):
    ...
    print("[v118] accepted no-train:", validated_path)
...
def v67_build_ui(self):
    ...
    try:
        ...
        print(f"[v118] UI feedback impossible : {exc}")
...
def v68_generate_candidate(self, event=None):
    ...
    print("[v118] Length policy appliquée : 2 cases par défaut, 1 case rare.")
...
def v69_force_pattern_hard_2_grid(self, reason="v69_force_hard_2_grid"):
    ...
    print("")
    print("[v118] HARD 2 GRID APPLIQUÉ")
    for item in self.pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v118] step {item['x_step']:02d}/{end:02d} | "
...
def v70_duplicate_selected_forward(self, event=None):
    ...
    print("")
    print("[v118] CTRL+D DUPLICATE +2")
    print(f"[v118] old_id={item.get('id')} new_id={new_item.get('id')}")
    print(f"[v118] step {old_x}/{old_x + length - 1} -> {new_x}/{new_x + length - 1}")
    print(f"[v118] pair={new_item.get('pair')} len={length}")
    print("")
...
def v70_build_ui(self):
    ...
    try:
        ...
        print(f"[v118] bind Ctrl+D impossible : {exc}")
...
def v71_rank_slices(self):
    ...
    print("")
    print("[v118] PRE-LEARNING TOP SLICES")
    for role in ("kick", "snare", "hat", "ghost_snare"):
        print(f"[v118] {role:12s}:", [(x["pair"], round(x["score"], 3)) for x in ranks[role][:8]])
    print("")
...
def v71_prelearn_roles(self, force=False):
    ...
    print("")
    print("[v118] PRELEARN ROLES POUR", safe)
    for role in ("kick", "snare", "hat", "ghost_snare", "bad"):
        print(f"[v118] {role:12s}:", roles.get(role, []))
    print("")
...
def v71_generate_full_candidate(self, event=None):
    ...
    print("")
    print("[v118] FULL CANDIDATE GENERATED")
    print("[v118] break:", v71_safe(self))
    print("[v118] roles:", roles)
    print("[v118] pattern:")
    for item in pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
...
def v72_open_break(self, safe=None):
    ...
    print("")
    print("[v118] OPEN BREAK")
    print("[v118] current:", current)
    print("[v118] next   :", safe)
    print("[v118] cmd    :", " ".join(cmd))
    print("")
...
def v72_build_ui(self):
    ...
    try:
        ...
        print(f"[v118] UI break switcher impossible : {exc}")
...
def v73_sanitize_tracker_pattern(self, reason="v73_tracker_strict_no_overlap"):
    ...
    print("")
    print("[v118] TRACKER STRICT NO OVERLAP")
    print("[v118] kept:", len(accepted), "| rejected overlaps:", len(rejected))

    for item in accepted:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v118] KEEP step {item['x_step']:02d}/{end:02d} | "
    ...
    if rejected:
        ...
        for item in rejected:
            ...
            print(
                f"[v118] DROP step {item['x_step']:02d}/{end:02d} | "
                f"role={item.get('v73_role_key')} | pair={item.get('pair')}"
            )
...
def v81_launch_break(self, source):
    ...
    print("")
    print("[v118] CHARGER BREAK")
    print("[v118] courant :", current)
    print("[v118] nouveau :", source)
    print("[v118] cmd     :", " ".join(cmd))
    print("")
...
def v81_patch_charger_break_buttons(self):
    ...
    print(f"[v118] boutons Charger break patchés : {patched}")
...
def v81_generate_candidate(self, event=None):
    ...
    print("")
    print("[v118] GENERATE CANDIDATE")
    print("[v118] break chargé :", current)
    print("[v118] générateur   :", getattr(fn, "__name__", None))
    print("")
...
def v81_run_training(self):
    ...
    if script is None:
        print("[v118] Aucun script training trouvé.")
...
def v81_save_feedback(self, verdict, train_now=False):
    ...
    print("")
    print("[v118] FEEDBACK SAUVÉ")
    print("[v118] break  :", current)
    print("[v118] verdict:", verdict)
    print("[v118] train  :", train_now)
    print("[v118] fichier:", out_path)
    print("")
...
def v81_build_ui(self):
    ...
    try:
        self.root.title(f"BreakbeatAI v118 — select slice rows")
    except Exception:
        pass
    ...
    try:
        ...
        print(f"[v118] patch Charger break impossible : {exc}")

    try:
        v81_install_training_toolbar(self)
    except Exception as exc:
        print(f"[v118] toolbar impossible : {exc}")
...
def v82_auto_role_pools(self):
    ...
    print("")
    print("[v118] ROLE POOLS")
    for r in ["kick", "snare", "hat", "ghost_snare", "bad"]:
        print(f"[v118] {r:12s}:", roles.get(r, []))
    print("")
...
def v82_generate_candidate(self, event=None):
    ...
    print("")
    print("[v118] NEW CANDIDATE")
    print("[v118] break :", current)
    print("[v118] nonce :", nonce)
    print("[v118] rejected known:", len(rejected))
    print("[v118] fingerprint rejeté ?: ", chosen_fp in rejected)
    for item in self.pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v118] step {item['x_step']:02d}/{end:02d} "
...
def v82_patch_training_buttons(self):
    ...
    print(f"[v118] boutons training patchés : {patched}")
...
def v82_build_ui(self):
    ...
    try:
        ...
        print(f"[v118] patch boutons impossible : {exc}")
...
def v110_generate(self, event=None, reject_mode=False):
    ...
    print("")
    if reject_mode:
        print("[v118] REJECT/BAD -> variation premier 32 + autres lignes")
    else:
        print("[v118] GENERATE 64")
    print("[v118] break:", current)
    print("[v118] first32:", first32_id)
    print("[v118] response:", response_id)
    print("[v118] role_offset:", role_offset)
    for role in ["kick", "snare", "hat"]:
        lane = "?"
        try:
            ...
            pass
        print(f"[v118] {role:6s}: pair={chosen[role]['pair']} lane={lane} source={chosen[role]['source']}")
    print("")
    ...
    try:
        ...
        print(f"[v118] draw impossible : {exc}")
    ...
    try:
        ...
        print(f"[v118] refresh_panel impossible : {exc}")
...
def v110_render_pattern_to_audio(self):
    ...
    for item in pattern:
        ...
        if pair not in slice_map:
            skipped += 1
            print(f"[v118] skip pair introuvable : {pair}")
            continue
...
def v110_export_wav_current(self, event=None):
    try:
        ...
        print("")
        print("[v118] EXPORT WAV OK")
        print("[v118] break:", current)
        print("[v118] wav  :", out_path)
        print("[v118] json :", info_path)
        print("[v118] duration:", round(info["duration_seconds"], 3), "s")
        print("[v118] rendered:", info["rendered_notes"], "skipped:", info["skipped_notes"])
        print("")
...
def v110_space_play(self, event=None):
    ...
    try:
        ...
        if btn is not None:
            print("[v118] SPACE -> Loop / Space")
            try:
                btn.invoke()
            except Exception as exc:
                print("[v118] invoke Loop / Space impossible :", exc)
            return "break"

        print("[v118] SPACE : bouton Loop / Space introuvable.")
...
def v110_patch_buttons(self):
    ...
    print(f"[v118] boutons patchés : Generate={gen_patched}, Reject/Bad={bad_patched}")
...
def v110_build_ui(self):
    ...
    try:
        ...
        print("[v118] patch boutons impossible :", exc)

    try:
        v110_add_export_button(self)
    except Exception as exc:
        print("[v118] ajout Export WAV impossible :", exc)

    try:
        v110_install_space_bindings(self)
    except Exception as exc:
        print("[v118] bind espace impossible :", exc)
...
def v113_export(self, event=None):
    ...
    print("[v118] ERREUR : export_wav_current_v110 introuvable.")
...
def v113_place_export_button(self):
    ...
    try:
        ...
        print("[v118] bouton Export WAV visible ajouté.")
        v113_status(self, "v113 : Export WAV visible. Espace reste Loop / Space.")

    except Exception as exc:
        print("[v118] placement bouton Export WAV impossible :", exc)
        try:
            btn.place(x=760, y=32, height=30)
            btn.lift()
            self.export_button_v113 = btn
...
def v115_apply_cells_minus_25(self):
    ...
    print("")
    print("[v118] APPLICATION CASES -3.5px")
    ...
    for name in V115_WIDTH_GLOBAL_NAMES:
        try:
            ...
            if isinstance(value, (int, float)) and value > 6:
                ...
                print(f"[v118] global {name}: {value} -> {globals()[name]}")
        except Exception:
            pass
        ...
        try:
            if hasattr(self, attr):
                ...
                if isinstance(value, (int, float)) and value > 6:
                    ...
                    print(f"[v118] attr {attr}: {value} -> {getattr(self, attr)}")
        except Exception:
            pass
...
def v115_build_ui(self):
    ...
    try:
        self.root.title("BreakbeatAI v118 — select slice rows")
    except Exception:
        pass
    ...
    try:
        ...
        print("[v118] impossible d'appliquer -3.5px :", exc)
...
def v117_render_pattern_to_audio(self):
    ...
    print("")
    print("[v118] EXPORT EXACT CELLS")
    print("[v118] break:", current)
    print("[v118] pairblock:", pairblock_file)
    print("[v118] sr:", sr)
    print("[v118] median_pair_samples:", median_pair_samples)
    print("[v118] case_samples:", case_samples)
    print("[v118] export_samples:", total_samples)
    print("[v118] export_seconds:", round(total_samples / float(sr), 6))
    print("")
...
def v117_export_wav_current(self, event=None):
    try:
        ...
        print("[v118] wav  :", out_path)
        print("[v118] json :", info_path)
        print("[v118] duration:", round(info["duration_seconds"], 6), "s")
        print("[v118] case_seconds:", round(info["case_seconds"], 6))
        print("[v118] rendered:", info["rendered_notes"], "skipped:", info["skipped_notes"])
        print("")
...
# ---------------------------------------------------------------------
# v118 : clic sur Slice N = sélection de toute la ligne
# ---------------------------------------------------------------------
# Clic sur le texte "Slice 1/2/3/..." :
# - sélectionne tous les blocs de cette ligne
# - glisser la souris déplace toute la ligne
# - flèches gauche/droite déplacent horizontalement
# - flèches haut/bas changent de ligne
# - Échap désélectionne
# ---------------------------------------------------------------------
...
def v118_canvas(self):
    return getattr(self, "canvas", None)


def v118_all_canvas_items(self):
    canvas = v118_canvas(self)
    if canvas is None:
        return []
    try:
        return list(canvas.find_all())
    except Exception:
        return []


def v118_item_text(self, item):
    canvas = v118_canvas(self)
    if canvas is None:
        return ""
    try:
        if canvas.type(item) != "text":
            return ""
        return str(canvas.itemcget(item, "text")).strip()
    except Exception:
        return ""


def v118_get_slice_labels(self):
    """
    Renvoie les labels Slice visibles, triés par hauteur.
    lane = index visuel de la ligne, donc Slice 1 -> lane 0.
    """
    canvas = v118_canvas(self)
    if canvas is None:
        return []
    ...
    for item in v118_all_canvas_items(self):
        text = v118_item_text(self, item)
        if not text:
            continue

        if "slice" not in text.lower():
            continue
...
def v118_row_height(self):
    labels = v118_get_slice_labels(self)

    if len(labels) >= 2:
        diffs = []
        for a, b in zip(labels[:-1], labels[1:]):
            d = float(b["yc"]) - float(a["yc"])
            if d > 4:
                diffs.append(d)
...
def v118_pair_for_lane(self, lane, fallback_pair=None):
    lane = int(lane)
...
def v118_lane_from_click(self, event):
    canvas = v118_canvas(self)
    if canvas is None:
        return None
    ...
    row_h = v118_row_height()

    # Zone cliquable à gauche : label Slice + marge.
    label_zone_x = 0
    try:
        label_zone_x = max(140, max(float(r["bbox"][2]) for r in labels) + 24)
    except Exception:
        label_zone_x = 160

    # Directement dans la zone des labels.
    if cx <= label_zone_x:
        best = None
        best_d = None
        ...
        if best is not None and best_d <= row_h * 0.55:
            return best

    # Si on clique exactement sur le texte Canvas.
    try:
        closest = canvas.find_closest(cx, cy)
        if closest:
            item = closest[0]
            text = v118_item_text(self, item)
...
def v118_selected_indices(self):
    lane = getattr(self, "v118_selected_lane", None)
    if lane is None:
        return []
...
def v118_clear_highlight(self):
    canvas = v118_canvas(self)
    if canvas is None:
        return

    try:
        canvas.delete("v118_selection")
    except Exception:
        pass


def v118_highlight_selection(self):
    canvas = v118_canvas(self)
    if canvas is None:
        return

    v118_clear_highlight(self)
    ...
    row_h = v118_row_height()
    y1 = float(rec["yc"]) - row_h / 2.0 + 1
    y2 = float(rec["yc"]) + row_h / 2.0 - 1
    ...
    try:
        ...
        canvas.create_text(
            8, y1 + 4,
            anchor="nw",
            text=f"SELECTED {rec['text']}",
            fill="#ff66cc",
            font=("Sans", 9, "bold"),
            tags=("v118_selection",),
        )

        canvas.tag_raise("v118_selection")
    except Exception:
        pass


def v118_select_lane(self, lane, label_text=None, start_event=None):
    self.v118_selected_lane = int(lane)
    self.v118_selected_label = label_text or f"Slice {int(lane) + 1}"

    indices = v118_selected_indices(self)

    self.v118_drag_snapshot = []
    for idx in indices:
        try:
            item = dict(self.pattern[idx])
            self.v118_drag_snapshot.append((idx, item))
        except Exception:
            pass
    ...
    if start_event is not None:
        canvas = v118_canvas(self)
        try:
            self.v118_drag_start_x = float(canvas.canvasx(start_event.x))
            self.v118_drag_start_y = float(canvas.canvasy(start_event.y))
        except Exception:
            self.v118_drag_start_x = float(start_event.x)
            self.v118_drag_start_y = float(start_event.y)
        self.v118_dragging_row = True
    else:
        self.v118_dragging_row = False

    v118_highlight_selection(self)

    print("")
    print(f"[v118] ligne sélectionnée : {self.v118_selected_label} / lane {lane}")
    print(f"[v118] blocs sélectionnés : {len(indices)}")
    print("")

    v118_status(self, f"v118 : {self.v118_selected_label} sélectionnée, {len(indices)} blocs. Glisse ou utilise les flèches.")


def v118_move_snapshot(self, step_delta=0, lane_delta=0):
    snapshot = getattr(self, "v118_drag_snapshot", [])
    if not snapshot:
        return

    max_lane = v118_max_lane(self)

    for idx, old in snapshot:
        ...
        new_pair = v118_pair_for_lane(self, new_lane, fallback_pair=old_pair)
        ...
        try:
            self.pattern[idx]["v118_row_move"] = True
        except Exception:
            pass

    # Si on a changé de ligne, la sélection suit la nouvelle ligne.
    if lane_delta:
        try:
            self.v118_selected_lane = max(0, min(max_lane, int(getattr(self, "v118_selected_lane", 0)) + int(lane_delta)))
            self.v118_selected_label = f"Slice {int(self.v118_selected_lane) + 1}"
        except Exception:
            pass
    ...
    try:
        ...
        print("[v118] draw après déplacement impossible :", exc)
...
def v118_move_current_selection(self, step_delta=0, lane_delta=0):
    indices = v118_selected_indices(self)
    if not indices:
        return "break"
    ...
    for idx in indices:
        try:
            self.v118_drag_snapshot.append((idx, dict(self.pattern[idx])))
        except Exception:
            pass

    v118_move_snapshot(self, step_delta=step_delta, lane_delta=lane_delta)

    try:
        self.write_latest_pattern(reason="v118_row_move")
    except Exception:
        pass
...
def v118_on_button1(self, event):
    rec = v118_lane_from_click(self, event)

    if rec is None:
        return None

    v118_select_lane(self, rec["lane"], rec["text"], start_event=event)
    return "break"


def v118_on_motion(self, event):
    if not getattr(self, "v118_dragging_row", False):
        return None

    if getattr(self, "v118_selected_lane", None) is None:
        return None
    ...
    dx = cx - float(getattr(self, "v118_drag_start_x", cx))
    dy = cy - float(getattr(self, "v118_drag_start_y", cy))

    step_w = max(1.0, v118_step_width(self))
    row_h = max(1.0, v118_row_height(self))
...
def v118_on_release(self, event):
    if not getattr(self, "v118_dragging_row", False):
        return None
    ...
    v118_highlight_selection(self)
    v118_status(self, f"v118 : ligne déplacée. {getattr(self, 'v118_selected_label', 'Slice')} reste sélectionnée.")
    return "break"


def v118_escape(self, event=None):
    self.v118_selected_lane = None
    self.v118_selected_label = None
    self.v118_dragging_row = False
    self.v118_drag_snapshot = []
    v118_clear_highlight(self)
    v118_status(self, "v118 : sélection de ligne annulée.")
    return "break"


def v118_key_left(self, event=None):
    return v118_move_current_selection(self, step_delta=-1, lane_delta=0)


def v118_key_right(self, event=None):
    return v118_move_current_selection(self, step_delta=1, lane_delta=0)


def v118_key_up(self, event=None):
    return v118_move_current_selection(self, step_delta=0, lane_delta=-1)


def v118_key_down(self, event=None):
    return v118_move_current_selection(self, step_delta=0, lane_delta=1)


def v118_install_row_selection(self):
    canvas = v118_canvas(self)
    if canvas is None:
        print("[v118] canvas introuvable.")
        return

    try:
        canvas.bind("<Button-1>", self.v118_on_button1, add="+")
        canvas.bind("<B1-Motion>", self.v118_on_motion, add="+")
        canvas.bind("<ButtonRelease-1>", self.v118_on_release, add="+")
    except Exception as exc:
        print("[v118] bind souris impossible :", exc)

    try:
        self.root.bind("<Escape>", self.v118_escape)
        self.root.bind("<Left>", self.v118_key_left)
        self.root.bind("<Right>", self.v118_key_right)
        self.root.bind("<Up>", self.v118_key_up)
        self.root.bind("<Down>", self.v118_key_down)
    except Exception as exc:
        print("[v118] bind clavier impossible :", exc)

    print("[v118] sélection de ligne Slice installée.")


_old_v118_draw = getattr(SliceIndexTracker, "draw", None)


def v118_draw(self, *args, **kwargs):
    result = None

    if callable(_old_v118_draw):
        result = _old_v118_draw(self, *args, **kwargs)
...
_old_v118_build_ui = SliceIndexTracker.build_ui


def v118_build_ui(self):
    _old_v118_build_ui(self)

    try:
        self.root.title("BreakbeatAI v118 — clic Slice = sélection ligne")
    except Exception:
        pass

    try:
        v118_install_row_selection(self)
    except Exception as exc:
        print("[v118] installation sélection ligne impossible :", exc)

    v118_status(self, "v118 : clique sur Slice 1/2/3... pour sélectionner toute la ligne.")


SliceIndexTracker.v118_on_button1 = v118_on_button1
SliceIndexTracker.v118_on_motion = v118_on_motion
SliceIndexTracker.v118_on_release = v118_on_release
SliceIndexTracker.v118_escape = v118_escape
SliceIndexTracker.v118_key_left = v118_key_left
SliceIndexTracker.v118_key_right = v118_key_right
SliceIndexTracker.v118_key_up = v118_key_up
SliceIndexTracker.v118_key_down = v118_key_down

if callable(_old_v118_draw):
    SliceIndexTracker.draw = v118_draw

SliceIndexTracker.build_ui = v118_build_ui
