...
"""
03_tracker_editor_app_v08.py

BreakbeatAI Tracker Editor v08
...
class TrackerEditorApp:
    def __init__(self, root, pair_json):
        ...
        self.root.title("BreakbeatAI Tracker Editor v08")
    ...
    def detect_external_player(self):
        for name in ["pw-play", "paplay", "aplay", "ffplay"]:
            ...
            if path:
                print(f"[v08] Backend audio système trouvé : {name} -> {path}")
                return name

        print("[v08] Aucun backend système trouvé, fallback sounddevice.")
        return None
    ...
    def build_ui(self):
        ...
        title = tk.Label(
            main,
            text="BreakbeatAI Tracker Editor v08 — 2 lignes par HI-HAT / KICK / SNARE",
            bg="#111018",
            fg="#ff7acc",
            font=("Sans", 20, "bold"),
        )
    ...
    def bind_keys(self):
        ...
        print("[v08] Raccourcis clavier actifs : Espace = Play/Stop loop")
    ...
    def play_audio_array(self, audio, label="audio"):
        ...
        live_wav = OUT_DIR / f"{self.project['safe']}_v08_live.wav"
        ...
        if self.external_player:
            ...
            try:
                ...
                print(f"[v08] {label} lancé avec {self.external_player} : {live_wav}")
                return duration_ms
            except Exception as exc:
                print(f"[v08] Erreur backend système {self.external_player} : {exc}")
        ...
        try:
            ...
            print(f"[v08] {label} lancé avec sounddevice")
            return duration_ms
        except Exception as exc:
    ...
    def render_preview_file(self):
        OUT_DIR.mkdir(parents=True, exist_ok=True)
        wav = OUT_DIR / f"{self.project['safe']}_tracker_app_v08_preview.wav"
    ...
    def loop_tick(self):
        ...
        duration_ms = self.play_audio_array(audio, label="Loop")

        if duration_ms <= 0:
            self.looping = False
            return

        self.loop_after_id = self.root.after(max(100, duration_ms + 20), self.loop_tick)

    def toggle_loop_event(self, event=None):
        print("[v08] Espace détecté")
        self.toggle_loop()
        return "break"
    ...
    def toggle_loop(self):
        if self.looping:
            ...
            print("[v08] Loop arrêtée")
            return

        print("[v08] Lancement loop...")
        self.looping = True
    ...
    def save(self):
        ...
        data = {
            "version": "tracker_app_edit_v08_double_lanes_per_role",
        ...
        path = OUT_DIR / f"{self.project['safe']}_tracker_app_edit_v08.json"
