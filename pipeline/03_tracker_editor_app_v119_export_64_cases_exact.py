...
class SliceIndexTracker:
    ...
    def build_ui(self):
        ...
        title = tk.Label(
            main,
            text="BreakbeatAI v119 — export 64 cases exact",
            bg="#111018",
            fg="#ff7acc",
            font=("Sans", 20, "bold"),
        )
...
def v81_build_ui(self):
    ...
    try:
        self.root.title(f"BreakbeatAI v119 — export 64 cases exact")
    except Exception:
        pass
...
def v110_make_note(self, step, pair, role, source, first32_id, response_id, role_offset):
    ...
    return {
        ...
        "hit_slot": int(step) // 2,
        "ai_generated": True,
        "ai_model": "v110_clean_no_callbacks",
        "learned_role": str(role),
        "prior_label": str(role),
...
def v110_generate(self, event=None, reject_mode=False):
    ...
    try:
        self.write_latest_pattern(reason="v110_clean_no_callbacks")
    except Exception as exc:
        print(f"[v119] write_latest_pattern impossible : {exc}")
...
def v110_render_pattern_to_audio(self):
    ...
    info = {
        ...
        "exporter": "v110_clean_no_callbacks",
    }
...
def v115_apply_cells_minus_25(self):
    if getattr(self, "_v115_cells_minus_35_done", False):
        return

    self._v115_cells_minus_35_done = True
...
def v115_build_ui(self):
    ...
    try:
        self.root.title("BreakbeatAI v119 — export 64 cases exact")
    except Exception:
        pass
...
# ---------------------------------------------------------------------
# v119 EXPORT : 64 cases exactes, jamais le sample source de 3 minutes
# ---------------------------------------------------------------------
# v118 copiait parfois le mauvais WAV : le sample source complet.
# v119 revient à un export de grille :
# - sortie = exactement 64 cases
# - case_samples dérivé des pair_blocks
# - chaque note length=2 occupe exactement 2 cases
# - aucun export de 3 minutes
# ---------------------------------------------------------------------

V119_EXPORT_DIR = Path("exports")
V119_PAIR_DIR = Path("dataset/pair_blocks_v02")
_V119_AUDIO_CACHE = {}
...
def v119_current_break(self):
    # Réutilise la fonction propre v110 si présente.
    try:
        return v110_current_break(self)
    except Exception:
        return "Camo_Break_-_3A"


def v119_read_audio(path):
    path = Path(path).resolve()

    if path in _V119_AUDIO_CACHE:
        return _V119_AUDIO_CACHE[path]

    # soundfile si dispo, sinon wave/ffmpeg des versions précédentes.
    try:
        import soundfile as sf
        y, sr = sf.read(str(path), always_2d=False, dtype="float32")
        y = np.asarray(y, dtype=np.float32)
        if y.ndim == 2:
            y = y.mean(axis=1)
        _V119_AUDIO_CACHE[path] = (y, int(sr))
        return y, int(sr)
    except Exception:
        pass

    try:
        y, sr = v110_read_audio(path)
        _V119_AUDIO_CACHE[path] = (y, int(sr))
        return y, int(sr)
    except Exception as exc:
        raise RuntimeError(f"audio illisible {path}: {exc}")


def v119_find_pairblock_file(current):
    exact = V119_PAIR_DIR / f"{current}_pair_blocks_v02.json"
    if exact.exists():
        return exact

    n_current = v119_norm(current)

    for p in sorted(V119_PAIR_DIR.glob("*_pair_blocks_v02.json")):
        stem = p.name.replace("_pair_blocks_v02.json", "")
        if v119_norm(stem) == n_current:
            return p
...
def v119_find_existing_path(value):
    if not value:
        return None
...
def v119_resolve_audio_path(block, root):
    for obj in [block, root]:
        if not isinstance(obj, dict):
            continue
        ...
        for key in [
            ...
            p = v119_find_existing_path(obj.get(key))
            if p:
                return p
...
def v119_num(obj, keys, default=None):
    if not isinstance(obj, dict):
        return default
...
def v119_pair_id(block, fallback):
    try:
        return int(v119_num(block, ["pair", "slice", "slice_index", "index", "id"], fallback))
    except Exception:
        return int(fallback)


def v119_bounds_from_block(block, sr, audio_len):
    """
    Important : on évite de traiter les clés génériques start/end comme samples.
    Sur certains JSON, start/end peuvent être autre chose.
    On privilégie les clés explicites sample/ms/sec.
    """

    start = v119_num(block, ["source_start_sample", "start_sample"], None)
    end = v119_num(block, ["source_end_sample", "end_sample"], None)

    if start is None:
        start_ms = v119_num(block, ["source_start_ms", "start_ms"], None)
        if start_ms is not None:
            start = start_ms * sr / 1000.0

    if end is None:
        end_ms = v119_num(block, ["source_end_ms", "end_ms"], None)
        if end_ms is not None:
            end = end_ms * sr / 1000.0

    if start is None:
        start_s = v119_num(block, ["source_start_sec", "start_sec", "start_s"], None)
        if start_s is not None:
            start = start_s * sr

    if end is None:
        end_s = v119_num(block, ["source_end_sec", "end_sec", "end_s"], None)
    ...
    if end is None:
        dur_ms = v119_num(block, ["duration_ms", "dur_ms"], None)
        if dur_ms is not None:
            end = start + dur_ms * sr / 1000.0

    if end is None:
        dur_s = v119_num(block, ["duration_sec", "dur_sec", "duration_s", "dur_s"], None)
        if dur_s is not None:
            end = start + dur_s * sr

    if end is None:
        # fallback court : 2 cases approximatives
        end = start + int(2 * 60.0 / 155.0 / 4.0 * sr)

    start = int(max(0, min(audio_len, int(round(start)))))
    end = int(max(start + 1, min(audio_len, int(round(end)))))

    return start, end


def v119_pad_or_crop(seg, target_len):
    ...
    # Petit maintien de tail pour éviter un trou trop brutal si un slice est court.
    if len(seg) > 8:
        tail_len = min(target_len - len(seg), len(seg) // 8, 512)
        if tail_len > 0:
            tail = seg[-tail_len:].copy()
            tail *= np.linspace(0.8, 0.0, tail_len, dtype=np.float32)
            out[len(seg):len(seg) + tail_len] = tail

    return out


def v119_build_slice_map(current):
    pairblock_file = v119_find_pairblock_file(current)

    if pairblock_file is None:
        raise RuntimeError(f"pair_blocks introuvable pour {current}")

    root = v110_load_json(pairblock_file, None)
    blocks = v119_extract_blocks(root)
    ...
    print("[v119] nb blocks:", len(blocks))
    if isinstance(blocks[0], dict):
        print("[v119] keys block[0]:", sorted(blocks[0].keys()))
    print("")
    ...
    for i, block in enumerate(blocks):
        ...
        pair = v119_pair_id(block, i)
        audio_path = v119_resolve_audio_path(block, root)

        if audio_path is None:
            continue

        source_audio_path = audio_path
        y, sr = v119_read_audio(audio_path)

        if sr_ref is None:
            sr_ref = int(sr)

        start, end = v119_bounds_from_block(block, sr, len(y))
        seg = y[start:end].astype(np.float32)
    ...
    median_pair = int(round(float(np.median(durations_np))))
    min_pair = int(round(float(np.min(durations_np))))
    max_pair = int(round(float(np.max(durations_np))))

    # Un pairblock = note length 2 = 2 cases.
    case_samples = int(round(median_pair / 2.0))
    case_samples = max(1, case_samples)

    return {
        "slice_map": slice_map,
        "source_audio_path": source_audio_path,
        "pairblock_file": pairblock_file,
        "sr": int(sr_ref or 44100),
        "case_samples": int(case_samples),
        "median_pair": int(median_pair),
...
def v119_render_64_cases(self):
    current = v119_current_break(self)
    pattern = list(getattr(self, "pattern", []) or [])

    if not pattern:
        raise RuntimeError("pattern vide : clique Generate Candidate avant Export WAV")

    pack = v119_build_slice_map(current)

    slice_map = pack["slice_map"]
    sr = int(pack["sr"])
    case_samples = int(pack["case_samples"])
    ...
    print("")
    print("[v119] EXPORT 64 CASES EXACT")
    print("[v119] break:", current)
    print("[v119] sr:", sr)
    print("[v119] case_samples:", case_samples)
    print("[v119] case_seconds:", round(case_samples / sr, 6))
    print("[v119] total_samples:", total_samples)
    print("[v119] total_seconds:", round(total_samples / sr, 6))
    print("[v119] pair duration median/min/max:", pack["median_pair"], pack["min_pair"], pack["max_pair"])
    print("")
    ...
    for item in pattern:
        ...
        if seg_sr != sr:
            # resample uniquement si sample-rate différent
            old_x = np.linspace(0.0, 1.0, len(raw_seg), endpoint=False)
            new_len = max(1, int(round(len(raw_seg) * sr / float(seg_sr))))
            new_x = np.linspace(0.0, 1.0, new_len, endpoint=False)
            raw_seg = np.interp(new_x, old_x, raw_seg).astype(np.float32)

        note_len = int(max(1, length) * case_samples)
        seg = v119_pad_or_crop(raw_seg, note_len)
    ...
    info = {
        "exporter": "v119_export_64_cases_exact",
        "break": current,
        "pairblock_file": str(pack["pairblock_file"]),
        "source_audio_path": str(pack["source_audio_path"]),
        "sample_rate": sr,
        "total_cases": total_cases,
        "case_samples": case_samples,
        "case_seconds": case_samples / float(sr),
        "duration_samples": len(out),
        "duration_seconds": len(out) / float(sr),
        "pair_duration_median_samples": pack["median_pair"],
        "pair_duration_min_samples": pack["min_pair"],
        "pair_duration_max_samples": pack["max_pair"],
        "rendered_notes": rendered,
        "skipped_notes": skipped,
        "note": "Durée forcée à 64 cases. Aucun WAV source complet copié.",
    }

    return out.astype(np.float32), sr, info


def v119_write_wav(path, audio, sr):
    path = Path(path)
    path.parent.mkdir(parents=True, exist_ok=True)
...
def v119_export_wav(self, event=None):
    try:
        current = v119_current_break(self)
        stamp = time.strftime("%Y%m%d_%H%M%S")
        variant = getattr(self, "v110_current_first32_id", None) or "pattern"

        audio, sr, info = v119_render_64_cases(self)

        V119_EXPORT_DIR.mkdir(parents=True, exist_ok=True)

        out = V119_EXPORT_DIR / f"{v119_slug(current)}_{v119_slug(variant)}_v119_64cases_{stamp}.wav"
        v119_write_wav(out, audio, sr)

        info_path = out.with_suffix(".json")
        info_path.write_text(json.dumps(info, indent=2, ensure_ascii=False), encoding="utf-8")

        print("")
        print("[v119] EXPORT OK")
        print("[v119] wav:", out)
        print("[v119] json:", info_path)
        print("[v119] durée:", round(info["duration_seconds"], 6), "s")
        print("[v119] rendered:", info["rendered_notes"], "skipped:", info["skipped_notes"])
        print("")

        v119_status(self, f"Export v119 OK : {out}")

    except Exception as exc:
        print("")
        print("[v119] EXPORT ERREUR")
        print("[v119]", exc)
        print(traceback.format_exc())
        print("")
        v119_status(self, f"Export v119 erreur : {exc}")

    return "break"


# Tous les boutons export pointent vers v119.
SliceIndexTracker.export_wav_current_v110 = v119_export_wav
SliceIndexTracker.export_wav_v113 = v119_export_wav
SliceIndexTracker.export_wav_current_v117 = v119_export_wav
SliceIndexTracker.export_wav_current_v118 = v119_export_wav
SliceIndexTracker.export_wav_current_v119 = v119_export_wav
