...
"""
BreakbeatAI Tracker Editor v31
...
# v31 : rôle réel des samples/pairs.
...
def block_role(block):
    """
    v31 : rôle réel du sample.
    pair 1 est override en kick, pair 5 et pair 6 sont override en hat, pair 7 est override en snare.
    """
    try:
...
def load_wav(path):
    ...
    # v31 : pas de normalisation/fade par sample.
...
class TrackerEditorApp:
    def __init__(self, root, pair_json):
        ...
        # v31 : grille 32 positions = 4 cycles de break8.
        ...
        self.root.title("BreakbeatAI Tracker Editor v31")
    ...
    def guess_step_ms(self):
        # v31 : une colonne = une position break8.
    ...
    def detect_external_player(self):
        for name in ["pw-play", "paplay", "aplay", "ffplay"]:
            ...
            if path:
                print(f"[v31] Backend audio système trouvé : {name} -> {path}")
                return name

        print("[v31] Aucun backend système trouvé, fallback sounddevice.")
        return None
    ...
    def formal_role_for_pair(self, pair):
        ...
        v31 : pair 1 = kick / pair 5 = hat / pair 6 = hat / pair 7 = snare.
        """
        pair = int(pair)
    ...
    def base_pair_for_position(self, local_pos):
        ...
        Donc :
            position 1 veut un hat    -> ne choisit plus pair 1
            position 5 veut un kick   -> peut choisir pair 0 ou pair 1, pas pair 5
            position 6 veut une snare -> choisit pair 7 si disponible
            position 7 veut un hat    -> peut choisir pair 5 ou pair 6
        """
    ...
    def build_initial_pattern(self):
        ...
        print("[v31] Génération loop32 verrouillée")
        print("[v31] pattern =", " ".join(BREAK8_ROLES))
        ...
        for pos in range(32):
            ...
            if actual_role != target_role:
                print(
                    f"[v31][WARN] pos {pos}: target {target_role}, "
            ...
            print(
                f"[v31] pos {pos:02d} | target {target_role:5s} | "
    ...
    def build_ui(self):
        ...
        title = tk.Label(
            main,
            text="BreakbeatAI Tracker Editor v31 — pair 1 = kick / pair 5 = hat / pair 6 = hat / pair 7 = snare",
            bg="#111018",
            fg="#ff7acc",
            font=("Sans", 20, "bold"),
        )
        ...
        self.output_label = tk.Label(
            panel,
            text="v31 : pair 1 est un kick, pair 5 et pair 6 sont des hats, pair 7 est une snare. Les positions restent K H S H H K S H.",
            bg="#1b1824",
            fg="#77f5b5",
            justify="left",
        )
    ...
    def bind_keys(self):
        ...
        print("[v31] Raccourcis : Espace loop | flèches = déplacer | Delete = supprimer")
    ...
    def play_audio_array(self, audio, label="audio"):
        ...
        live_wav = OUT_DIR / f"{self.project['safe']}_v31_live.wav"
        ...
        if self.external_player:
            ...
            try:
                ...
                print(f"[v31] {label} lancé avec {self.external_player} : {live_wav}")
                return duration_ms
            except Exception as exc:
                print(f"[v31] Erreur backend système {self.external_player} : {exc}")
    ...
    def render_preview_file(self):
        OUT_DIR.mkdir(parents=True, exist_ok=True)
        wav = OUT_DIR / f"{self.project['safe']}_tracker_app_v31_preview.wav"
    ...
    def toggle_loop(self):
        if self.looping:
            ...
            print("[v31] Loop arrêtée")
            return

        self.stop_playhead()
        print("[v31] Lancement loop...")
        self.looping = True
    ...
    def save(self):
        ...
        data = {
            "version": "tracker_app_edit_v31_exact_cells_no_sample_fade",
        ...
        path = OUT_DIR / f"{self.project['safe']}_tracker_app_edit_v31.json"
...
# ---------------------------------------------------------------------
# v31 DIRECT FROM v29
# Choix de break + Randomize loop32 + auto-learn à chaque correction
# ---------------------------------------------------------------------

import random as _v31_random

LEARNING_DIR = Path("dataset/learning")
CORRECTIONS_DIR = LEARNING_DIR / "corrections"
LATEST_DIR = LEARNING_DIR / "latest"
MEMORY_PATH = LEARNING_DIR / "breakbeatai_loop32_memory.json"

DEFAULT_PAIR_ROLE_MAP = {
    0: "kick",
    1: "hat",
    2: "snare",
    3: "hat",
...
# Mapping validé à la main pour ton Atlantis Amen actuel.
# Tu pourras corriger les autres breaks via l'app, et l'auto-learn mémorisera.
PAIR_ROLE_MAPS_BY_BREAK = {
    "atlantis_amen": {
        0: "kick",
        1: "kick",
        2: "snare",
...
def v31_safe_int(value, default=0):
    try:
        return int(value)
    except Exception:
        return default
...
def v31_role_map_for_safe(safe):
    s = str(safe).lower()

    for key, role_map in PAIR_ROLE_MAPS_BY_BREAK.items():
        if key in s:
            return role_map

    return DEFAULT_PAIR_ROLE_MAP


def v31_pair_role_for_safe(pair, safe):
    pair = int(pair)
    role_map = v31_role_map_for_safe(safe)

    if pair in role_map:
        return clean_role(role_map[pair])

    return clean_role(DEFAULT_PAIR_ROLE_MAP[pair % 8])


def v31_load_project(self, pair_json):
    meta = json.loads(pair_json.read_text(encoding="utf-8"))
    safe = safe_name(pair_json)
    ...
    for block in meta.get("blocks", []):
        pair = int(block["pair"])
        role = v31_pair_role_for_safe(pair, safe)
...
def v31_formal_role_for_pair(self, pair):
    return v31_pair_role_for_safe(int(pair), self.project["safe"])


def v31_role_pair_pool(self, role):
    role = clean_role(role)
...
def v31_role_index_for_position(role, local_pos):
    role_positions = {
        "kick": [0, 5],
        "hat": [1, 3, 4, 7],
        "snare": [2, 6],
    }.get(clean_role(role), [local_pos])

    try:
        return role_positions.index(local_pos)
    except ValueError:
        return 0
...
def v31_load_memory():
    ...
    return {
        "version": "breakbeatai_loop32_memory_v01",
        "description": "Mémoire des corrections humaines. Randomize utilise ces scores.",
        "breaks": {},
        "global": {},
        "events_total": 0,
        "updated_at": None,
...
def v31_save_memory(memory):
    LEARNING_DIR.mkdir(parents=True, exist_ok=True)
    memory["updated_at"] = v31_now_stamp()
    MEMORY_PATH.write_text(json.dumps(memory, indent=2, ensure_ascii=False), encoding="utf-8")


def v31_memory_bucket(memory, safe, target_role, x_step):
    local_pos = str(v31_safe_int(x_step) % 8)
    target_role = clean_role(target_role)

    memory.setdefault("breaks", {})
    memory.setdefault("global", {})

    b = memory["breaks"].setdefault(safe, {})
    role_bucket = b.setdefault(target_role, {})
    pos_bucket = role_bucket.setdefault(local_pos, {
        "positive_pair_counts": {},
        "negative_pair_counts": {},
        "events": 0,
    })

    g_role_bucket = memory["global"].setdefault(target_role, {})
    g_pos_bucket = g_role_bucket.setdefault(local_pos, {
        "positive_pair_counts": {},
        "negative_pair_counts": {},
        "events": 0,
    })

    return pos_bucket, g_pos_bucket


def v31_increment_count(d, key, amount=1):
    key = str(key)
    d[key] = int(d.get(key, 0)) + int(amount)


def v31_pair_weight_from_memory(self, pair, target_role, x_step):
    memory = v31_load_memory()
    safe = self.project["safe"]
    local_pos = str(v31_safe_int(x_step) % 8)
    target_role = clean_role(target_role)
    pair_key = str(pair)
    ...
    try:
        g = memory.get("global", {}).get(target_role, {}).get(local_pos, {})
    ...
    try:
        b = memory.get("breaks", {}).get(safe, {}).get(target_role, {}).get(local_pos, {})
...
def v31_pick_pair(self, target_role, pos, randomize=False, rng=None):
    pos = int(pos)
    local_pos = pos % 8
    bar = pos // 8
    target_role = clean_role(target_role)
    ...
    role_index = v31_role_index_for_position(target_role, local_pos)

    # Sans mémoire et sans randomize : base lisible.
    if not MEMORY_PATH.exists() and not randomize:
        if bar == 0:
            return int(pool[role_index % len(pool)])
        return int(pool[(role_index + bar) % len(pool)])

    weights = [v31_pair_weight_from_memory(self, pair, target_role, pos) for pair in pool]
    ...
    # Non-randomize : meilleur choix appris, sinon variation déterministe.
...
def v31_build_pattern(self, randomize=False):
    pattern = []
    rng = _v31_random.Random(time.time_ns()) if randomize else None

    print("[v31] Génération loop32")
    print("[v31] break =", self.project["safe"])
    print("[v31] randomize =", randomize)
    print("[v31] pattern =", " ".join(BREAK8_ROLES))

    for pos in range(32):
        target_role = BREAK8_ROLES[pos % 8]
        pair = v31_pick_pair(self, target_role, pos, randomize=randomize, rng=rng)
        actual_role = self.formal_role_for_pair(pair)
        lane = self.pair_to_lane.get(pair, 0)
...
def v31_build_initial_pattern(self):
    return v31_build_pattern(self, randomize=False)


def v31_cell_snapshot(self, item):
    if item is None:
        return None

    lane_index = max(0, min(len(self.pair_values) - 1, v31_safe_int(item.get("lane"))))
    pair = int(self.lane_to_pair(lane_index))
    x_step = v31_safe_int(item.get("x_step"))
    target_role = item.get("role_target", BREAK8_ROLES[x_step % 8])
    actual_role = self.formal_role_for_pair(pair)

    return {
        "id": v31_safe_int(item.get("id")),
        "x_step": x_step,
        "lane": lane_index,
        "pair": pair,
        "role_target": clean_role(target_role),
        "role_actual": clean_role(actual_role),
        "is_role_match": clean_role(target_role) == clean_role(actual_role),
        "length": v31_safe_int(item.get("length"), 1),
        "variation_bar": v31_safe_int(item.get("variation_bar"), x_step // 8),
        "variation_pos": v31_safe_int(item.get("variation_pos"), x_step % 8),
    }


def v31_selected_snapshot(self):
    try:
        return v31_cell_snapshot(self, self.selected())
    except Exception:
        return None


def v31_write_latest_pattern(self, reason="correction"):
    LATEST_DIR.mkdir(parents=True, exist_ok=True)

    data = {
        "version": "breakbeatai_latest_pattern_v31",
        "reason": reason,
        "updated_at": v31_now_stamp(),
        "safe": self.project["safe"],
    ...
    path = LATEST_DIR / f"{self.project['safe']}_latest_pattern_v31.json"
    path.write_text(json.dumps(data, indent=2, ensure_ascii=False), encoding="utf-8")
    return path


def v31_record_correction(self, event_type, before=None, after=None, note=""):
    if before == after:
        return
    ...
    event = {
        "version": "breakbeatai_correction_event_v31",
        "time": v31_now_stamp(),
        "safe": safe,
    ...
    corrections_path = CORRECTIONS_DIR / f"{safe}_corrections_v31.jsonl"
    ...
    # Positif : choix après correction.
    if after is not None:
        target = after.get("role_target", BREAK8_ROLES[v31_safe_int(after.get("x_step")) % 8])
        x_step = v31_safe_int(after.get("x_step"))
        pair = after.get("pair")

        break_bucket, global_bucket = v31_memory_bucket(memory, safe, target, x_step)

        v31_increment_count(break_bucket["positive_pair_counts"], pair)
        v31_increment_count(global_bucket["positive_pair_counts"], pair)

        break_bucket["events"] = int(break_bucket.get("events", 0)) + 1
        global_bucket["events"] = int(global_bucket.get("events", 0)) + 1

    # Négatif : ancien choix remplacé.
    if before is not None and after is not None:
        before_pair = before.get("pair")
        after_pair = after.get("pair")

        if str(before_pair) != str(after_pair):
            target = before.get("role_target", BREAK8_ROLES[v31_safe_int(before.get("x_step")) % 8])
            x_step = v31_safe_int(before.get("x_step"))

            break_bucket, global_bucket = v31_memory_bucket(memory, safe, target, x_step)

            v31_increment_count(break_bucket["negative_pair_counts"], before_pair)
            v31_increment_count(global_bucket["negative_pair_counts"], before_pair)

    v31_save_memory(memory)
    latest_path = v31_write_latest_pattern(self, reason=event_type)
    ...
    print(f"[v31] correction sauvegardée : {event_type} -> {corrections_path}")


def v31_randomize_pattern(self):
    self.stop_playhead()
    self.stop_audio()
    self.looping = False

    self.pattern = v31_build_pattern(self, randomize=True)
    self.selected_id = self.pattern[0]["id"] if self.pattern else None

    self.draw()
    self.refresh_panel()
    latest_path = v31_write_latest_pattern(self, reason="randomize_loop32")

    self.output_label.config(
        text=(
            f"Randomize OK : nouvelle loop32 générée pour {self.project['safe']}.\\n"
            f"Latest : {latest_path}\\n"
            "La mémoire des corrections influence déjà le tirage."
        )
    )


def v31_refresh_breaks(self):
    self.break_jsons = v31_list_break_jsons()
    values = list(self.break_jsons.keys())

    if hasattr(self, "break_box"):
...
def v31_switch_break(self, event=None):
    ...
    pair_json = self.break_jsons[safe]
    ...
    self.draw()
    self.refresh_panel()
    latest_path = v31_write_latest_pattern(self, reason="switch_break")

    self.output_label.config(
        text=(
            f"Break chargé : {safe}.\\n"
            f"Latest : {latest_path}\\n"
            "Tu peux écouter, Randomize, corriger, puis Save validation."
        )
    )


# Monkey patch de la classe avant main()
TrackerEditorApp.load_project = v31_load_project
TrackerEditorApp.formal_role_for_pair = v31_formal_role_for_pair
TrackerEditorApp.role_pair_pool = v31_role_pair_pool
TrackerEditorApp.build_initial_pattern = v31_build_initial_pattern
TrackerEditorApp.v31_randomize_pattern = v31_randomize_pattern
TrackerEditorApp.v31_switch_break = v31_switch_break
TrackerEditorApp.v31_refresh_breaks = v31_refresh_breaks

_old_v31_build_ui = TrackerEditorApp.build_ui
_old_v31_on_click = TrackerEditorApp.on_click
_old_v31_on_release = TrackerEditorApp.on_release
_old_v31_move_selected = TrackerEditorApp.move_selected
_old_v31_set_pair = TrackerEditorApp.set_pair
_old_v31_set_lane_from_choice = TrackerEditorApp.set_lane_from_choice
_old_v31_delete_selected = TrackerEditorApp.delete_selected


def v31_build_ui(self):
    _old_v31_build_ui(self)

    # Barre choix de break + randomize
    frame = tk.Frame(self.root, bg="#15111f")
    ...
    self.break_box.bind("<<ComboboxSelected>>", self.v31_switch_break)

    tk.Button(
        frame,
        text="Charger break",
        command=self.v31_switch_break,
        bg="#30283f",
        fg="#f5eefe",
    ...
    tk.Button(
        ...
        command=self.v31_randomize_pattern,
        bg="#50315f",
        fg="#f5eefe",
    ).pack(side="left", padx=4)
    ...
    tk.Button(
        frame,
        text="Refresh breaks",
        command=self.v31_refresh_breaks,
        bg="#30283f",
        fg="#f5eefe",
    ).pack(side="left", padx=4)

    # Barre auto-learn
    learn_frame = tk.Frame(self.root, bg="#101820")
    learn_frame.pack(fill="x", padx=14, pady=(0, 10))
    ...
    tk.Label(
        learn_frame,
        text="Chaque correction est sauvegardée et influence les prochains Randomize.",
        bg="#101820",
        fg="#b9acc8",
    ).pack(side="left", padx=4)
    ...
    tk.Button(
        learn_frame,
        text="Voir dossier learning",
        command=lambda: self.output_label.config(
            text=(
                f"Corrections : {CORRECTIONS_DIR}\\n"
                f"Mémoire : {MEMORY_PATH}\\n"
                f"Latest : {LATEST_DIR}"
            )
        ),
...
def v31_on_click(self, event):
    before_ids = {int(i["id"]) for i in self.pattern}

    result = _old_v31_on_click(self, event)

    after_ids = {int(i["id"]) for i in self.pattern}
    new_ids = list(after_ids - before_ids)

    if new_ids:
        self._v31_drag_before = None
        self._v31_drag_created_id = int(new_ids[0])
    else:
        self._v31_drag_before = v31_selected_snapshot(self)
        self._v31_drag_created_id = None

    return result


def v31_on_release(self, event):
    before = getattr(self, "_v31_drag_before", None)
    created_id = getattr(self, "_v31_drag_created_id", None)

    result = _old_v31_on_release(self, event)

    after = v31_selected_snapshot(self)

    if created_id is not None:
        v31_record_correction(self, "add_case", before=None, after=after)
    else:
        if before != after:
            v31_record_correction(self, "drag_or_resize_case", before=before, after=after)
...
def v31_move_selected(self, dx, dy):
    before = v31_selected_snapshot(self)
    result = _old_v31_move_selected(self, dx, dy)
    after = v31_selected_snapshot(self)

    if before != after:
        v31_record_correction(self, "keyboard_move_case", before=before, after=after)

    return result


def v31_set_pair(self, pair):
    before = v31_selected_snapshot(self)
    result = _old_v31_set_pair(self, pair)
    after = v31_selected_snapshot(self)

    if before != after:
        v31_record_correction(self, "change_pair", before=before, after=after)

    return result


def v31_set_lane_from_choice(self, event=None):
    before = v31_selected_snapshot(self)
    result = _old_v31_set_lane_from_choice(self, event)
    after = v31_selected_snapshot(self)

    if before != after:
        v31_record_correction(self, "change_lane", before=before, after=after)

    return result


def v31_delete_selected(self):
    before = v31_selected_snapshot(self)
    result = _old_v31_delete_selected(self)

    if before is not None:
        v31_record_correction(self, "delete_case", before=before, after=None)

    return result


TrackerEditorApp.build_ui = v31_build_ui
TrackerEditorApp.on_click = v31_on_click
TrackerEditorApp.on_release = v31_on_release
TrackerEditorApp.move_selected = v31_move_selected
TrackerEditorApp.set_pair = v31_set_pair
TrackerEditorApp.set_lane_from_choice = v31_set_lane_from_choice
TrackerEditorApp.delete_selected = v31_delete_selected
