...
"""
BreakbeatAI Tracker Editor v39 — slice index
...
class SliceIndexTracker:
    def __init__(self, root, pair_json):
        ...
        self.root.title("BreakbeatAI Tracker Editor v39 — slice index")
    ...
    def detect_external_player(self):
        for name in ["pw-play", "paplay", "aplay", "ffplay"]:
            ...
            if path:
                print(f"[v39] Backend audio : {name} -> {path}")
                return name

        print("[v39] Aucun backend système trouvé, fallback sounddevice.")
        return None
    ...
    def build_initial_pattern(self, randomize=False):
        ...
        for slot in range(HIT_SLOTS):
            ...
            print(f"[v39] slot {slot:02d} | step {x_step:02d} -> slice {pair:02d}")
    ...
    def build_ui(self):
        ...
        self.output_label = tk.Label(
            panel,
            text="v39 : tempo par défaut 155 BPM ; clic case = son immédiat ; fermeture = stop audio.",
            bg="#1b1824",
            fg="#77f5b5",
            justify="left",
        )
    ...
    def record_correction(self, event_type, before=None, after=None):
        ...
        event = {
            "version": "breakbeatai_slice_index_correction_v39",
            "time": now_stamp(),
            "safe": safe,
        ...
        corrections_path = CORRECTIONS_DIR / f"{safe}_slice_index_corrections_v39.jsonl"
        ...
        print(f"[v39] correction sauvegardée : {event_type}")
    ...
    def stop_audition(self):
        """
        v39 : stoppe seulement le petit player d'écoute au clic,
    ...
    def play_audition_array(self, audio, label="audition"):
        ...
        audition_wav = OUT_DIR / f"{self.project['safe']}_v39_audition.wav"
        ...
        if self.external_player:
            try:
                ...
                print(f"[v39] Erreur audition {self.external_player} : {exc}")
    ...
    def audition_selected_case(self):
        """
        v39 : clic sur une case = on joue la slice de cette case.
        """
        item = self.selected()
    ...
    def on_close(self):
        """
        v39 : quand tu fermes la fenêtre, on coupe la loop, le playhead
        et le player externe avant de détruire Tk.
        """
        print("[v39] Fermeture : arrêt audio + destruction fenêtre")
    ...
    def play_audio_array(self, audio, label="audio"):
        ...
        audio = normalize(audio)
        live_wav = OUT_DIR / f"{self.project['safe']}_v39_live.wav"
        ...
        if self.external_player:
            try:
                ...
                print(f"[v39] Erreur backend {self.external_player} : {exc}")
    ...
    def render_preview_file(self):
        OUT_DIR.mkdir(parents=True, exist_ok=True)
        wav = OUT_DIR / f"{self.project['safe']}_tracker_app_v39_preview.wav"
    ...
    def write_latest_pattern(self, reason="correction"):
        ...
        path = LATEST_DIR / f"{self.project['safe']}_latest_slice_index_v39.json"

        data = {
            "version": "breakbeatai_latest_slice_index_v39",
            "reason": reason,
    ...
    def save(self):
        ...
        tracker_path = OUT_DIR / f"{self.project['safe']}_tracker_app_edit_v39.json"

        data = {
            "version": "tracker_app_edit_v39_slice_index",
        ...
        stamp = file_stamp()
        validated_path = VALIDATED_DIR / f"{self.project['safe']}_validated_slice_index_loop32_v39_{stamp}.json"

        validated = {
            "version": "breakbeatai_validated_slice_index_loop32_v39",
