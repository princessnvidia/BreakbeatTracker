...
"""
03_tracker_editor_app_v20.py

BreakbeatAI Tracker Editor v20
...
Usage :
    ...
    python pipeline/03_tracker_editor_app_v20.py --source "amen"
"""
...
class TrackerEditorApp:
    def __init__(self, root, pair_json):
        ...
        self.root.title("BreakbeatAI Tracker Editor v20 — hats / kicks / snares groupés")
    ...
    def load_project(self, pair_json):
        ...
        # v20 : ordre des lignes = hats en haut, kicks au milieu, snares en bas.
    ...
    def detect_external_player(self):
        for name in ["pw-play", "paplay", "aplay", "ffplay"]:
            ...
            if path:
                print(f"[v20] Backend audio système trouvé : {name} -> {path}")
                return name

        print("[v20] Aucun backend système trouvé, fallback sounddevice.")
        return None
    ...
    def build_ui(self):
        ...
        title = tk.Label(
            main,
            text="BreakbeatAI Tracker Editor v20 — HI-HAT en haut / KICK milieu / SNARE bas",
            bg="#111018",
            fg="#ff7acc",
            font=("Sans", 20, "bold"),
        )
        ...
        subtitle = tk.Label(
            ...
            text=(
                ...
                "Lignes triées par rôle formalisé : hats en haut, kicks au milieu, snares en bas."
            ),
            bg="#111018",
            fg="#b9acc8",
        )
        ...
        self.output_label = tk.Label(
            panel,
            text=(
                "v20 : hats en haut, kicks au milieu, snares en bas. "
    ...
    def bind_keys(self):
        ...
        print("[v20] Raccourcis : Espace loop | flèches = déplacer | Delete = supprimer")
    ...
    def play_audio_array(self, audio, label="audio"):
        ...
        live_wav = OUT_DIR / f"{self.project['safe']}_v20_live.wav"
        ...
        if self.external_player:
            ...
            try:
                ...
                print(f"[v20] {label} lancé avec {self.external_player} : {live_wav}")
                return duration_ms
            except Exception as exc:
                print(f"[v20] Erreur backend système {self.external_player} : {exc}")

        try:
            import sounddevice as sd
            sd.play(audio, SR)
            self.output_label.config(text=f"{label} lancé avec sounddevice — {duration_ms} ms")
            print(f"[v20] {label} lancé avec sounddevice")
            return duration_ms
        except Exception as exc:
    ...
    def refresh_panel(self):
        ...
        self.info_label.config(
            text=(
                f"case {item['id']} | sample/pair {pair} | rôle {role} | "
    ...
    def render_preview_file(self):
        OUT_DIR.mkdir(parents=True, exist_ok=True)
        wav = OUT_DIR / f"{self.project['safe']}_tracker_app_v20_preview.wav"
    ...
    def toggle_loop(self):
        if self.looping:
            ...
            print("[v20] Loop arrêtée")
            return

        self.stop_playhead()
        print("[v20] Lancement loop...")
        self.looping = True
    ...
    def save(self):
        ...
        data = {
            "version": "tracker_app_edit_v20_fixed_timeline_one_line_per_sample",
        ...
        path = OUT_DIR / f"{self.project['safe']}_tracker_app_edit_v20.json"
