...
"""
BreakbeatAI Tracker Editor v154 — slice index
...
def warp_audio_to_length_external(audio, target_len, cache_key=None):
    ...
    if rubberband:
        try:
            ...
            print(f"[v154] rubberband échec, fallback ffmpeg/no-speed : {exc}")
    ...
    if ffmpeg:
        try:
            ...
            print(f"[v154] ffmpeg atempo échec, fallback no-speed : {exc}")
...
class SliceIndexTracker:
    def __init__(self, root, pair_json):
        ...
        self.root.title("BreakbeatAI Tracker Editor v154 — slice index")
    ...
    def detect_external_player(self):
        for name in ["pw-play", "paplay", "aplay", "ffplay"]:
            ...
            if path:
                print(f"[v154] Backend audio : {name} -> {path}")
                return name

        print("[v154] Aucun backend système trouvé, fallback sounddevice.")
        return None
    ...
    def build_initial_pattern(self, randomize=False):
        ...
        print(
            f"[v154] Génération pattern | break={self.project['safe']} | "
        ...
        for slot in range(max_items):
            ...
            print(f"[v154] slot {slot:02d} | step {x_step:02d} -> slice {pair:02d}")
    ...
    def build_ui(self):
        ...
        title = tk.Label(
            main,
            text="BreakbeatAI v154 — random KSH full slots",
            bg="#111018",
            fg="#ff7acc",
            font=("Sans", 20, "bold"),
        )
    ...
    def record_correction(self, event_type, before=None, after=None):
        ...
        with corrections_path.open("a", encoding="utf-8") as f:
            f.write(json.dumps(event, ensure_ascii=False) + "[v154]n")
        ...
        self.output_label.config(
            text=(
                f"Auto-learn sauvegardé : {event_type}[v154]n"
                f"Correction : {corrections_path}[v154]n"
                f"Mémoire : {MEMORY_PATH}[v154]n"
                f"Latest : {latest_path}"
            )
        )

        print(f"[v154] correction sauvegardée : {event_type}")
    ...
    def randomize_pattern(self):
        ...
        self.output_label.config(text=f"Randomize OK. Mémoire appliquée.[v154]nLatest : {latest}")
    ...
    def play_audition_array(self, audio, label="audition"):
        ...
        audition_wav = OUT_DIR / f"{self.project['safe']}_v154_audition.wav"
        ...
        if self.external_player:
            try:
                ...
                print(f"[v154] Erreur audition {self.external_player} : {exc}")
    ...
    def on_close(self):
        ...
        """
        print("[v154] Fermeture : arrêt audio + destruction fenêtre")
    ...
    def play_audio_array(self, audio, label="audio"):
        ...
        audio = normalize(audio)
        live_wav = OUT_DIR / f"{self.project['safe']}_v154_live.wav"
        ...
        if self.external_player:
            try:
                ...
                print(f"[v154] Erreur backend {self.external_player} : {exc}")
    ...
    def render_preview_file(self):
        OUT_DIR.mkdir(parents=True, exist_ok=True)
        wav = OUT_DIR / f"{self.project['safe']}_tracker_app_v154_preview.wav"
    ...
    def write_latest_pattern(self, reason="correction"):
        ...
        path = LATEST_DIR / f"{self.project['safe']}_latest_slice_index_v154.json"

        data = {
            "version": "breakbeatai_latest_slice_index_v154",
            "reason": reason,
    ...
    def save(self):
        ...
        tracker_path = OUT_DIR / f"{self.project['safe']}_tracker_app_edit_v154.json"
        ...
        stamp = file_stamp()
        validated_path = VALIDATED_DIR / f"{self.project['safe']}_validated_slice_index_loop64_v154_{stamp}.json"

        validated = {
            "version": "breakbeatai_validated_slice_index_loop64_v154",
        ...
        self.output_label.config(
            text=(
                f"OK sauvegardé[v154]n"
                f"Tracker : {tracker_path}[v154]n"
                f"Validation IA : {validated_path}[v154]n"
                f"Latest : {latest}[v154]n"
                f"Preview : {wav}"
            )
        )
...
def v44_record_correction(self, event_type, before=None, after=None):
    ...
    print(f"[v154] correction sauvegardée : {event_type}")
    print(f"[v154] correction path : {corrections_path}")
    print(f"[v154] latest path : {latest_path}")
...
def v45_move_selected(self, dx, dy):
    ...
    if dy != 0 and before != after:
        try:
            ...
            print(f"[v154] audition après flèche haut/bas impossible : {exc}")
...
def v46_warp_loop_pitch_preserve(audio, target_len, cache_key):
    ...
    if rubberband:
        try:
            ...
            print(f"[v154] rubberband global warp échec : {exc}")
    ...
    if ffmpeg:
        try:
            ...
            print(f"[v154] ffmpeg global warp échec : {exc}")

    print("[v154] Aucun warp pitch-preserve dispo, fallback fit no-speed.")
    return v46_fit_audio_no_speed(audio, target_len)
...
def v46_build_initial_pattern(self, randomize=False):
    ...
    print(
        f"[v154] M8 pattern | break={self.project['safe']} | "
...
def v46_get_warped_loop(self):
    ...
    print(
        f"[v154] global warp OK | source_len={loop_end-loop_start} | "
...
def v46_build_ui(self):
    ...
    try:
        ...
        print(f"[v154] UI extension impossible : {exc}")
...
def v47_play_audio_array(self, audio, label="audio"):
    ...
    if len(audio) == 0 or v47_is_silent(audio):
        ...
        print(f"[v154] lecture annulée : audio silencieux ({label})")
        return 0
...
def v47_toggle_loop(self):
    ...
    if not getattr(self, "pattern", None):
        ...
        print("[v154] loop annulée : pattern vide")
        return "break"
...
def v47_delete_selected(self):
    ...
    if not getattr(self, "pattern", None):
        ...
        print("[v154] pattern vide après suppression : audio coupé")
...
def v48_build_initial_pattern(self, randomize=False):
    ...
    print("[v154] pattern initial forcé VIDE : aucun son ne doit jouer.")
    return []
...
def v48_load_first_slices(self):
    ...
    print(f"[v154] Load 16 slices : {len(self.pattern)} items")


def v48_debug_pattern(self, label="render"):
    print("")
    print(f"[v154] DEBUG PATTERN avant {label}")
    print(f"[v154] items = {len(getattr(self, 'pattern', []) or [])}")

    if not getattr(self, "pattern", None):
        print("[v154] pattern vide confirmé")
        print("")
        return
    ...
    for item in self.pattern:
        ...
        print(
            f"[v154] id={item_id} | step {x:02d}/{end:02d} | "
...
def v48_render_audio_with_timeline(self):
    ...
    if not getattr(self, "pattern", None):
        loop_samples = self.get_loop_samples()
        print(f"[v154] rendu silence total : pattern vide | samples={loop_samples}")
        return np.zeros(loop_samples, dtype=np.float32), []
    ...
    print(f"[v154] rendu audio peak={peak:.8f} | timeline_events={len(timeline) if timeline is not None else 'None'}")

    if timeline:
        print("[v154] timeline events:")
        for ev in timeline:
            print(f"[v154]   {ev}")
...
def v48_toggle_loop(self):
    if not getattr(self, "pattern", None):
        ...
        print("[v154] Space ignoré : pattern vide.")
        return "break"
...
def v48_build_ui(self):
    ...
    try:
        ...
        print(f"[v154] ajout UI impossible : {exc}")
...
def v49_add_item_at_xy(self, x, y):
    ...
    try:
        ...
        print(f"[v154] record_correction add impossible : {exc}")
    ...
    try:
        ...
        print(f"[v154] audition après ajout impossible : {exc}")

    print(f"[v154] add | id={item['id']} | step={x_step} | pair={pair} | lane={lane}")
...
def v49_duplicate_selected_forward(self, event=None):
    ...
    if item is None:
        ...
        print("[v154] duplicate annulé : aucune note sélectionnée")
        return "break"
    ...
    if new_x > max_x:
        ...
        print("[v154] duplicate annulé : fin de grille")
        return "break"
    ...
    try:
        ...
        print(f"[v154] record_correction duplicate impossible : {exc}")
    ...
    try:
        ...
        print(f"[v154] audition après duplicate impossible : {exc}")

    print(
        f"[v154] duplicate | old_id={item.get('id')} | new_id={new_item.get('id')} | "
...
def v49_build_ui(self):
    ...
    try:
        ...
        print(f"[v154] bind canvas click impossible : {exc}")
    ...
    try:
        ...
        print(f"[v154] bind keys impossible : {exc}")
...
def v50_load_style_model():
    ...
    try:
        ...
        print(f"[v154] impossible de charger le modèle IA : {exc}")
        return None
...
def v50_generate_ai_pattern(self, event=None):
    ...
    if model is None:
        ...
        print("[v154] modèle absent :", STYLE_MODEL_PATH)
        return "break"
    ...
    print("")
    print(f"[v154] GENERATE IA | safe={safe} | pairs={pair_values} | slots={max_slots}")
    ...
    for slot in range(max_slots):
        ...
        print(
            f"[v154] slot {slot:02d} | step {x_step:02d} | "
    ...
    try:
        ...
        print(f"[v154] write_latest_pattern impossible : {exc}")
...
def v50_build_ui(self):
    ...
    try:
        ...
        print(f"[v154] UI IA impossible : {exc}")
...
def v51_mouse_down(self, event):
    ...
    if item is not None:
        ...
        print(
            f"[v154] mouse down note | id={item.get('id')} | "
            f"step={item.get('x_step')} | pair={item.get('pair')}"
        )
    else:
        print("[v154] mouse down empty")
...
def v51_mouse_up(self, event):
    ...
    if state.get("empty"):
        ...
        print("[v154] drag depuis vide ignoré")
        return "break"
    ...
    if before != after:
        try:
            ...
            print(f"[v154] record_correction drag impossible : {exc}")
        ...
        print(
            f"[v154] drag move | id={after.get('id')} | "
            f"step {before.get('x_step')} -> {after.get('x_step')} | "
        ...
        try:
            ...
            print(f"[v154] audition après drag impossible : {exc}")
...
def v51_build_ui(self):
    ...
    try:
        ...
        print(f"[v154] bind souris impossible : {exc}")
    ...
    try:
        ...
        print(f"[v154] bind Suppr impossible : {exc}")
...
def v52_normalize_loaded_notes(self, notes):
    ...
    for raw in sorted(notes, key=lambda n: (int(float(n.get("x_step", 0))), int(float(n.get("pair", 0))))):
        ...
        if pair not in valid_pairs:
            print(f"[v154] slice ignorée car absente du break actuel : pair={pair}")
            continue
...
def v52_load_saved_beat(self, event=None):
    ...
    if not saves:
        ...
        print("[v154] aucune sauvegarde trouvée")
        return "break"
    ...
    if not pattern:
        ...
        print("[v154] sauvegarde incompatible :", chosen["path"])
        return "break"
    ...
    print("")
    print("[v154] BEAT RESTAURÉ")
    print("[v154] fichier :", chosen["path"])
    print("[v154] clé :", chosen["key"])
    print("[v154] notes :", len(pattern))
    print("")
...
def v52_list_saved_beats(self, event=None):
    ...
    print("")
    print("[v154] SAUVEGARDES TROUVÉES")
    for i, save in enumerate(saves[:30]):
        print(
            f"[v154] {i:02d} | notes={len(save['notes']):02d} | "
...
def v52_build_ui(self):
    ...
    try:
        ...
        print(f"[v154] UI load saved impossible : {exc}")
...
def v53_run_training(self):
    ...
    if not trainer.exists():
        ...
        print("[v154] trainer introuvable :", trainer)
        return False

    print("")
    print("[v154] AUTO-TRAIN IA après Save...")
    print("")
    ...
    try:
        ...
        if result.returncode != 0:
            ...
            print("[v154] auto-train échec code :", result.returncode)
            return False

        v53_status(self, "Save OK + IA entraînée automatiquement.")
        print("[v154] AUTO-TRAIN OK")
        return True

    except Exception as exc:
        v53_status(self, f"Save OK, mais auto-train impossible : {exc}")
        print("[v154] exception auto-train :", exc)
        return False
...
def v53_save(self):
    ...
    try:
        ...
        print(f"[v154] auto-train après save impossible : {exc}")
...
def v53_build_ui(self):
    ...
    try:
        ...
        print(f"[v154] UI auto-train impossible : {exc}")
...
def v55_mouse_up(self, event):
    ...
    if not state.get("moved"):
        ...
        try:
            ...
            print(f"[v154] audition clic note impossible : {exc}")
        ...
        print(
            f"[v154] click play/select | id={item.get('id')} | "
            f"step={item.get('x_step')} | pair={item.get('pair')}"
        )
...
def v58_add_item_at_xy(self, x, y):
    ...
    print(
        f"[v154] add | id={item['id']} | step={x_step} | "
...
def v58_toggle_selected_length(self, event=None):
    ...
    try:
        ...
        print(f"[v154] record_correction length impossible : {exc}")

    v58_refresh_after_edit(self, "toggle_note_length")

    v58_status(self, f"Longueur note : {old_len} -> {new_len} case(s).")
    print(f"[v154] length toggle | id={item.get('id')} | {old_len} -> {new_len}")
...
def v58_duplicate_selected_forward(self, event=None):
    ...
    try:
        ...
        print(f"[v154] audition duplicate impossible : {exc}")
...
def v58_build_ui(self):
    ...
    try:
        ...
        print(f"[v154] UI demi-grille impossible : {exc}")
    ...
    try:
        ...
        print(f"[v154] bind demi-grille impossible : {exc}")
...
def v59_rank_current_slices(self):
    ...
    for pair in self.pair_values:
        ...
        try:
            ...
            print(f"[v154] analyse pair {pair} impossible : {exc}")
            role = "other"
    ...
    print("")
    print("[v154] TOP ROLES CURRENT BREAK")
    for r in ("kick", "snare", "hat"):
        print(f"[v154] {r:5s}:", [(x["pair"], round(x["score"], 3)) for x in ranks[r][:6]])
    print("")
...
def v59_load_role_model():
    ...
    try:
        ...
        print(f"[v154] lecture modèle rôle impossible : {exc}")
        return None
...
def v59_generate_role_aware(self, event=None):
    ...
    if model is None:
        ...
        print("[v154] modèle rôle absent, fallback musical snare 6.")
        model = {
            "breaks": {},
            "global": {},
            "patterns_used": 0,
    ...
    print("")
    print("[v154] ROLE-AWARE GENERATION")
    print("[v154] model patterns_used:", model.get("patterns_used", 0))
    for item in pattern:
        print(
            f"[v154] step {item['x_step']:02d}/{item['x_step'] + item['length'] - 1:02d} | "
...
def v59_run_role_training(self, event=None):
    ...
    try:
        ...
        print(f"[v154] training rôle impossible : {exc}")
...
def v59_save(self):
    ...
    try:
        ...
        print(f"[v154] auto-training rôle après save impossible : {exc}")
...
def v59_build_ui(self):
    ...
    try:
        ...
        print(f"[v154] UI role-aware impossible : {exc}")
...
def v62_add_item_at_xy(self, x, y):
    ...
    print(f"[v154] add | id={item['id']} | step={x_step} | len={length} | pair={pair}")
...
def v62_mouse_down(self, event):
    ...
    if item is not None:
        ...
        print(
            f"[v154] mouse down {state['mode']} | id={item.get('id')} | "
            f"step={item.get('x_step')} | len={item.get('length')} | pair={item.get('pair')}"
        )
    else:
        print("[v154] mouse down empty")
    ...
    if mode == "resize":
        if not state.get("moved"):
            ...
            try:
                ...
                print(f"[v154] audition clic resize impossible : {exc}")
        ...
        if before != after:
            try:
                ...
                print(f"[v154] record_correction resize impossible : {exc}")
            ...
            try:
                ...
                print(f"[v154] audition après resize impossible : {exc}")
    ...
    if mode == "move":
        ...
        if before != after:
            try:
                ...
                print(f"[v154] record_correction move impossible : {exc}")
            ...
            try:
                ...
                print(f"[v154] audition après move impossible : {exc}")
...
def v62_toggle_selected_length(self, event=None):
    ...
    try:
        ...
        print(f"[v154] record_correction toggle length impossible : {exc}")

    v62_refresh_after_edit(self, "toggle_note_length_v62")
    v62_status(self, f"Longueur note : {old_len} → {new_len} case(s).")

    print(f"[v154] toggle length | id={item.get('id')} | {old_len} -> {new_len}")
...
def v62_draw(self):
    ...
    try:
        ...
        print(f"[v154] draw handles impossible : {exc}")
...
def v62_build_ui(self):
    ...
    try:
        ...
        print(f"[v154] bind resize impossible : {exc}")
    ...
    try:
        ...
        print(f"[v154] UI resize impossible : {exc}")
...
def v63_load_model():
    ...
    try:
        ...
        print(f"[v154] lecture modèle strict impossible : {exc}")
        return None
...
def v63_generate_safe_role_ai(self, event=None):
    ...
    print("")
    print("[v154] SAFE ROLE AI GENERATED")
    print("[v154] model strict:", "yes" if model else "no")
    print("[v154] snare main/alt:", main_snare, alt_snare)
    print("[v154] kick  main/alt:", main_kick, alt_kick)
    print("[v154] hat   main/alt:", main_hat, alt_hat)
    print("[v154] pattern:")
    for item in pattern:
        print(
            f"[v154] step {item['x_step']:02d}/{item['x_step'] + item['length'] - 1:02d} | "
...
def v63_run_strict_training(self, event=None):
    ...
    print("")
    print("[v154] TRAIN STRICT ROLE MODEL...")
    print("")
    ...
    try:
        ...
        print(f"[v154] training strict impossible : {exc}")
...
def v63_build_ui(self):
    ...
    try:
        ...
        print(f"[v154] UI safe impossible : {exc}")
...
def v65_rank_slices(self):
    ...
    for pair in self.pair_values:
        ...
        try:
            ...
            print(f"[v154] analyse slice {pair} impossible : {exc}")
            scores = {
                "kick": 0.0,
                "snare": 0.0,
                "hat": 0.0,
    ...
    print("")
    print("[v154] TOP SLICES PAR ROLE — break courant")
    for role in ("kick", "snare", "hat", "ghost_snare"):
        print(f"[v154] {role:12s}:", [(x["pair"], round(x["score"], 3)) for x in ranks[role][:8]])
    print("")
...
def v65_generate_from_role_template(self, event=None):
    ...
    try:
        ...
        print(f"[v154] template introuvable : {exc}")
        return "break"
    ...
    print("")
    print("[v154] CROSS-SONG ROLE TEMPLATE GENERATED")
    print("[v154] template:", V65_TEMPLATE_PATH)
    print("[v154] temperature:", temperature, "| hats:", hat_density, "| ghosts:", ghost_density)
    print("[v154] pattern:")
    for item in pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v154] step {item['x_step']:02d}/{end:02d} | "
...
def v65_build_ui(self):
    ...
    try:
        ...
        print(f"[v154] UI impossible : {exc}")
...
def v67_train_strict_if_available(self):
    ...
    if trainer is None:
        print("[v154] aucun trainer trouvé.")
        return False

    print("")
    print("[v154] TRAIN après validation :", trainer)
    print("")
    ...
    try:
        ...
        print(f"[v154] training impossible : {exc}")
        return False
...
def v67_generate_candidate(self, event=None):
    ...
    print("")
    print("[v154] CANDIDATE GENERATED")
    print("[v154] break:", self.v67_candidate_safe)
    print("[v154] notes:", len(candidate))
    print("[v154] Maintenant : corrige puis clique Good après modifs, ou Reject.")
    print("")
...
def v67_reject_candidate(self, event=None):
    ...
    print("")
    print("[v154] REJECTED")
    print("[v154] saved:", rejected_path)
    print("")
...
def v67_accept_corrected(self, event=None):
    ...
    print("")
    print("[v154] ACCEPTED GOOD AFTER EDIT")
    print("[v154] saved:", validated_path)
    print("[v154] trained:", trained)
    print("")
...
def v67_accept_without_training(self, event=None):
    ...
    print("[v154] accepted no-train:", validated_path)
...
def v67_build_ui(self):
    ...
    try:
        ...
        print(f"[v154] UI feedback impossible : {exc}")
...
def v68_generate_candidate(self, event=None):
    ...
    print("[v154] Length policy appliquée : 2 cases par défaut, 1 case rare.")
...
def v69_force_pattern_hard_2_grid(self, reason="v69_force_hard_2_grid"):
    ...
    print("")
    print("[v154] HARD 2 GRID APPLIQUÉ")
    for item in self.pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v154] step {item['x_step']:02d}/{end:02d} | "
...
def v70_duplicate_selected_forward(self, event=None):
    ...
    print("")
    print("[v154] CTRL+D DUPLICATE +2")
    print(f"[v154] old_id={item.get('id')} new_id={new_item.get('id')}")
    print(f"[v154] step {old_x}/{old_x + length - 1} -> {new_x}/{new_x + length - 1}")
    print(f"[v154] pair={new_item.get('pair')} len={length}")
    print("")
...
def v70_build_ui(self):
    ...
    try:
        ...
        print(f"[v154] bind Ctrl+D impossible : {exc}")
...
def v71_rank_slices(self):
    ...
    print("")
    print("[v154] PRE-LEARNING TOP SLICES")
    for role in ("kick", "snare", "hat", "ghost_snare"):
        print(f"[v154] {role:12s}:", [(x["pair"], round(x["score"], 3)) for x in ranks[role][:8]])
    print("")
...
def v71_prelearn_roles(self, force=False):
    ...
    print("")
    print("[v154] PRELEARN ROLES POUR", safe)
    for role in ("kick", "snare", "hat", "ghost_snare", "bad"):
        print(f"[v154] {role:12s}:", roles.get(role, []))
    print("")
...
def v71_generate_full_candidate(self, event=None):
    ...
    print("")
    print("[v154] FULL CANDIDATE GENERATED")
    print("[v154] break:", v71_safe(self))
    print("[v154] roles:", roles)
    print("[v154] pattern:")
    for item in pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
...
def v72_open_break(self, safe=None):
    ...
    print("")
    print("[v154] OPEN BREAK")
    print("[v154] current:", current)
    print("[v154] next   :", safe)
    print("[v154] cmd    :", " ".join(cmd))
    print("")
...
def v72_build_ui(self):
    ...
    try:
        ...
        print(f"[v154] UI break switcher impossible : {exc}")
...
def v73_sanitize_tracker_pattern(self, reason="v73_tracker_strict_no_overlap"):
    ...
    print("")
    print("[v154] TRACKER STRICT NO OVERLAP")
    print("[v154] kept:", len(accepted), "| rejected overlaps:", len(rejected))

    for item in accepted:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v154] KEEP step {item['x_step']:02d}/{end:02d} | "
    ...
    if rejected:
        ...
        for item in rejected:
            ...
            print(
                f"[v154] DROP step {item['x_step']:02d}/{end:02d} | "
                f"role={item.get('v73_role_key')} | pair={item.get('pair')}"
            )
...
def v81_launch_break(self, source):
    ...
    print("")
    print("[v154] CHARGER BREAK")
    print("[v154] courant :", current)
    print("[v154] nouveau :", source)
    print("[v154] cmd     :", " ".join(cmd))
    print("")
...
def v81_patch_charger_break_buttons(self):
    ...
    print(f"[v154] boutons Charger break patchés : {patched}")
...
def v81_generate_candidate(self, event=None):
    ...
    print("")
    print("[v154] GENERATE CANDIDATE")
    print("[v154] break chargé :", current)
    print("[v154] générateur   :", getattr(fn, "__name__", None))
    print("")
...
def v81_run_training(self):
    ...
    if script is None:
        print("[v154] Aucun script training trouvé.")
...
def v81_save_feedback(self, verdict, train_now=False):
    ...
    print("")
    print("[v154] FEEDBACK SAUVÉ")
    print("[v154] break  :", current)
    print("[v154] verdict:", verdict)
    print("[v154] train  :", train_now)
    print("[v154] fichier:", out_path)
    print("")
...
def v81_build_ui(self):
    ...
    try:
        self.root.title(f"BreakbeatAI v154 — random KSH full slots")
    except Exception:
        pass
    ...
    try:
        ...
        print(f"[v154] patch Charger break impossible : {exc}")

    try:
        v81_install_training_toolbar(self)
    except Exception as exc:
        print(f"[v154] toolbar impossible : {exc}")
...
def v82_auto_role_pools(self):
    ...
    print("")
    print("[v154] ROLE POOLS")
    for r in ["kick", "snare", "hat", "ghost_snare", "bad"]:
        print(f"[v154] {r:12s}:", roles.get(r, []))
    print("")
...
def v82_generate_candidate(self, event=None):
    ...
    print("")
    print("[v154] NEW CANDIDATE")
    print("[v154] break :", current)
    print("[v154] nonce :", nonce)
    print("[v154] rejected known:", len(rejected))
    print("[v154] fingerprint rejeté ?: ", chosen_fp in rejected)
    for item in self.pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v154] step {item['x_step']:02d}/{end:02d} "
...
def v82_patch_training_buttons(self):
    ...
    print(f"[v154] boutons training patchés : {patched}")
...
def v82_build_ui(self):
    ...
    try:
        ...
        print(f"[v154] patch boutons impossible : {exc}")
...
def v110_generate(self, event=None, reject_mode=False):
    ...
    print("")
    if reject_mode:
        print("[v154] REJECT/BAD -> variation premier 32 + autres lignes")
    else:
        print("[v154] GENERATE 64")
    print("[v154] break:", current)
    print("[v154] first32:", first32_id)
    print("[v154] response:", response_id)
    print("[v154] role_offset:", role_offset)
    for role in ["kick", "snare", "hat"]:
        lane = "?"
        try:
            ...
            pass
        print(f"[v154] {role:6s}: pair={chosen[role]['pair']} lane={lane} source={chosen[role]['source']}")
    print("")
    ...
    try:
        ...
        print(f"[v154] draw impossible : {exc}")
    ...
    try:
        ...
        print(f"[v154] refresh_panel impossible : {exc}")
...
def v110_render_pattern_to_audio(self):
    ...
    for item in pattern:
        ...
        if pair not in slice_map:
            skipped += 1
            print(f"[v154] skip pair introuvable : {pair}")
            continue
...
def v110_export_wav_current(self, event=None):
    try:
        ...
        print("")
        print("[v154] EXPORT WAV OK")
        print("[v154] break:", current)
        print("[v154] wav  :", out_path)
        print("[v154] json :", info_path)
        print("[v154] duration:", round(info["duration_seconds"], 3), "s")
        print("[v154] rendered:", info["rendered_notes"], "skipped:", info["skipped_notes"])
        print("")
...
def v110_space_play(self, event=None):
    ...
    try:
        ...
        if btn is not None:
            print("[v154] SPACE -> Loop / Space")
            try:
                btn.invoke()
            except Exception as exc:
                print("[v154] invoke Loop / Space impossible :", exc)
            return "break"

        print("[v154] SPACE : bouton Loop / Space introuvable.")
...
def v110_patch_buttons(self):
    ...
    print(f"[v154] boutons patchés : Generate={gen_patched}, Reject/Bad={bad_patched}")
...
def v110_build_ui(self):
    ...
    try:
        ...
        print("[v154] patch boutons impossible :", exc)

    try:
        v110_add_export_button(self)
    except Exception as exc:
        print("[v154] ajout Export WAV impossible :", exc)

    try:
        v110_install_space_bindings(self)
    except Exception as exc:
        print("[v154] bind espace impossible :", exc)
...
def v113_export(self, event=None):
    ...
    print("[v154] ERREUR : export_wav_current_v110 introuvable.")
...
def v113_place_export_button(self):
    ...
    try:
        ...
        print("[v154] bouton Export WAV visible ajouté.")
        v113_status(self, "v113 : Export WAV visible. Espace reste Loop / Space.")

    except Exception as exc:
        print("[v154] placement bouton Export WAV impossible :", exc)
        try:
            btn.place(x=760, y=32, height=30)
            btn.lift()
            self.export_button_v113 = btn
...
def v115_apply_cells_minus_25(self):
    ...
    print("")
    print("[v154] APPLICATION CASES -3.5px")
    ...
    for name in V115_WIDTH_GLOBAL_NAMES:
        try:
            ...
            if isinstance(value, (int, float)) and value > 6:
                ...
                print(f"[v154] global {name}: {value} -> {globals()[name]}")
        except Exception:
            pass
        ...
        try:
            if hasattr(self, attr):
                ...
                if isinstance(value, (int, float)) and value > 6:
                    ...
                    print(f"[v154] attr {attr}: {value} -> {getattr(self, attr)}")
        except Exception:
            pass
...
def v115_build_ui(self):
    ...
    try:
        self.root.title("BreakbeatAI v154 — random KSH full slots")
    except Exception:
        pass
    ...
    try:
        ...
        print("[v154] impossible d'appliquer -3.5px :", exc)
...
def v117_render_pattern_to_audio(self):
    ...
    print("")
    print("[v154] EXPORT EXACT CELLS")
    print("[v154] break:", current)
    print("[v154] pairblock:", pairblock_file)
    print("[v154] sr:", sr)
    print("[v154] median_pair_samples:", median_pair_samples)
    print("[v154] case_samples:", case_samples)
    print("[v154] export_samples:", total_samples)
    print("[v154] export_seconds:", round(total_samples / float(sr), 6))
    print("")
...
def v117_export_wav_current(self, event=None):
    try:
        ...
        print("[v154] wav  :", out_path)
        print("[v154] json :", info_path)
        print("[v154] duration:", round(info["duration_seconds"], 6), "s")
        print("[v154] case_seconds:", round(info["case_seconds"], 6))
        print("[v154] rendered:", info["rendered_notes"], "skipped:", info["skipped_notes"])
        print("")
...
def v118_current_break(self):
    ...
    if candidates:
        ...
        print("[v154] break courant:", stem, "| source:", source, "| raw:", raw)
        return stem

    print("[v154] WARNING : break courant introuvable, fallback Camo.")
    return "Camo_Break_-_3A"
...
def v118_find_best_playback_wav(since):
    ...
    print("")
    print("[v154] WAV candidats playback :")
    for score, mtime, size, wav in candidates[:12]:
        age = time.time() - mtime
        print(f"[v154] score={score:8d} age={age:7.2f}s size={size:10d} path={wav}")
    print("")
...
def v118_copy_playback_wav(self, since):
    ...
    if best is None:
        ...
        print("[v154] aucun WAV playback trouvé.")
        return "break"
    ...
    print("")
    print("[v154] EXPORT PLAYBACK OK")
    print("[v154] source :", best)
    print("[v154] export :", out)
    print("[v154] json   :", info_path)
    print("")
...
def v118_export_playback_wav(self, event=None):
    ...
    try:
        ...
        if btn is not None:
            print("[v154] déclenche Loop / Space pour générer le playback WAV...")
            btn.invoke()
        else:
            print("[v154] bouton Loop / Space introuvable, copie du dernier WAV playback existant.")
    except Exception as exc:
        print("[v154] invoke Loop / Space impossible :", exc)
...
def v121_find_best_long_wav(since):
    ...
    print("")
    print("[v154] WAV candidats longs/propre à raccourcir :")
    for row in candidates[:20]:
        age = time.time() - row["mtime"]
        print(
            f"[v154] score={row['score']:7d} "
...
def v121_export_trimmed_long(self, event=None):
    ...
    try:
        ...
        if btn is not None:
            print("[v154] déclenche Loop / Space pour générer le WAV long/propre...")
            btn.invoke()
    except Exception as exc:
        print("[v154] Loop / Space impossible :", exc)
    ...
    def do_export():
        try:
            ...
            try:
                ...
                print("[v154] ffmpeg trim impossible, fallback wave :", exc)
                ok = False
            ...
            print("")
            print("[v154] EXPORT TRIM OK")
            print("[v154] source :", src_wav)
            print("[v154] source durée :", round(src_dur, 6), "s")
            print("[v154] target durée :", round(target_seconds, 6), "s")
            print("[v154] export :", out)
            print("[v154] export durée :", round(float(out_dur or 0), 6), "s")
            print("[v154] json :", info_path)
            print("")
...
def v124_export_trimmed_long_no_json(self, event=None):
    ...
    def do_export():
        try:
            ...
            print("")
            print("[v154] EXPORT WAV ONLY OK")
            print("[v154] source :", src_wav)
            print("[v154] source durée :", round(src_dur, 6), "s")
            print("[v154] target durée :", round(target_seconds, 6), "s")
            print("[v154] export :", out)
            print("[v154] export durée :", round(float(out_dur or 0), 6), "s")
            print("[v154] aucun JSON exporté")
            print("")
...
def v125_select_lane(self, lane):
    ...
    print("")
    print(f"[v154] Slice {lane + 1} sélectionnée")
    print(f"[v154] notes sélectionnées : {ids}")
    print("")
...
def v125_redraw_after_move(self):
    try:
        ...
        print("[v154] draw impossible :", exc)
...
def v125_build_ui(self):
    ...
    if canvas is not None:
        try:
            ...
            print("[v154] bind canvas click impossible :", exc)
    ...
    try:
        ...
        print("[v154] bind flèches impossible :", exc)
...
def v126_select_slice(self, zero_index, slice_number, reason=""):
    ...
    print("")
    print("[v154] SÉLECTION SLICE")
    print("[v154] reason:", reason)
    print("[v154] Slice affichée:", slice_number)
    print("[v154] zero_index:", zero_index)
    print("[v154] pair_candidates:", pair_candidates)
    print("[v154] ids sélectionnés:", ids)
...
def v126_build_ui(self):
    ...
    if canvas is not None:
        try:
            ...
            print("[v154] bind canvas impossible :", exc)
...
def v127_select_slice(self, lane, displayed, reason=""):
    ...
    print("")
    print("[v154] SÉLECTION SLICE ALIGNÉE")
    print("[v154] reason:", reason)
    print("[v154] label affiché:", displayed)
    print("[v154] lane réelle:", lane)
    print("[v154] labels zero-based:", v127_labels_are_zero_based(self))
    print("[v154] pair_candidates:", pair_candidates)
    print("[v154] ids sélectionnés:", ids)
    for row in debug_rows:
        print("[v154] note:", row)
...
def v128_select_lane(self, lane):
    ...
    print("")
    print("[v154] MULTI-SÉLECTION LIGNE")
    print("[v154] slice/lane:", lane)
    print("[v154] ids:", ids)
    for row in debug:
        print("[v154] note:", row)
    print("")
...
def v129_draw_green_multiselect(self):
    ...
    print(f"[v154] contours verts dessinés : {drawn}")
...
def v129_draw(self, *args, **kwargs):
    ...
    try:
        ...
        print("[v154] contour vert impossible :", exc)
...
def v130_audition_after_move(self, notes):
    ...
    def do_play():
        ...
        for name in method_names:
            ...
            try:
                fn()
                print(f"[v154] audition via {name}()")
                played = True
                break
            except TypeError:
                try:
                    fn(None)
                    print(f"[v154] audition via {name}(None)")
                    played = True
                    break
                except TypeError:
                    try:
                        fn(note)
                        print(f"[v154] audition via {name}(note)")
                        played = True
                        break
                    except Exception:
                        pass
        ...
        if not played:
            ...
            if btn is not None:
                try:
                    btn.invoke()
                    print("[v154] audition via bouton Play slice")
                    played = True
                except Exception as exc:
                    print("[v154] Play slice invoke impossible :", exc)

        if not played:
            print("[v154] audition introuvable : aucune méthode Play slice trouvée")
...
def v131_generate_candidate(self, event=None, reject_mode=False):
    ...
    print("")
    print("[v154] GENERATE CANDIDATE STRICT")
    print("[v154] break:", current)
    print("[v154] sequence: kick hat snare hat | hat kick snare hat")
    print("[v154] répété sur 64 cases")
    for role in ["kick", "snare", "hat"]:
        print(f"[v154] {role:6s}: pair={chosen[role]['pair']} source={chosen[role]['source']}")
    print("")
...
def v131_build_ui(self):
    ...
    try:
        ...
        print("[v154] repatch boutons impossible :", exc)
...
# ---------------------------------------------------------------------
# v144 : toolbar intégrée dans la ligne du haut
# ---------------------------------------------------------------------
# Le hover v143 a montré que la grande barre était le root Tk vide.
# Donc plus de toolbar flottante/place en haut à droite.
#
# On met les boutons directement dans la ligne :
# Break | Charger break | Randomize | Refresh | Nouveau groove | Miroir 32 | Loop | Exporter .wav
#
# Garde :
# - grille intacte
# - export WAV only
# - multisélection + flèches + audition
# - generate strict kick/hat/snare
# ---------------------------------------------------------------------

P144_BG = "#0b0610"
P144_PANEL = "#17101f"
P144_PINK = "#ff8be8"
P144_PINK_DARK = "#7a2f68"
P144_PURPLE = "#30223e"
P144_BLUE = "#27304d"
P144_GREEN = "#226b45"
P144_TEXT = "#fff4fb"
...
def v144_walk(widget):
    out = [widget]
    try:
        for child in widget.winfo_children():
            out.extend(v144_walk(child))
    except Exception:
        pass
    return out


def v144_text(widget):
    try:
        return str(widget.cget("text")).strip()
    except Exception:
        return ""
...
def v144_contains(parent, child):
    cur = child
...
def v144_canvas(self):
    ...
    for widget in v144_walk(self.root):
        try:
...
def v144_new_groove(self, event=None):
    ...
    v144_status(self, "Nouveau groove introuvable.")
    return "break"


def v144_export(self, event=None):
    for name in [
        ...
        if callable(fn):
            try:
                ...
                print(f"[v154] {name} erreur :", exc)

    v144_status(self, "Export introuvable.")
    print("[v154] ERREUR : aucun moteur Export WAV trouvé.")
    return "break"


def v144_loop(self, event=None):
    for widget in v144_walk(self.root):
        try:
            if v144_text(widget).lower() == "loop / space":
                widget.invoke()
                return "break"
        except Exception:
            pass
    ...
    v144_status(self, "Loop introuvable.")
    return "break"


def v144_mirror_32(self, event=None):
    pattern = list(getattr(self, "pattern", []) or [])
    first = []
    ...
    if not first:
        v144_status(self, "Aucun premier 32 à dupliquer.")
        return "break"
    ...
    try:
        self.write_latest_pattern(reason="v144_mirror_32")
    except Exception:
        pass

    v144_status(self, "Miroir 32 appliqué.")
    return "break"


def v144_button(parent, text, command, bg):
    return tk.Button(
        parent,
        text=text,
        command=command,
...
def v144_find_top_row(self):
    for label in ["Refresh breaks", "Randomize loop64", "Charger break"]:
        for widget in v144_walk(self.root):
            if v144_text(widget) == label:
                try:
...
def v144_install_inline_toolbar(self):
    if getattr(self, "_v144_inline_toolbar_done", False):
        return

    row = v144_find_top_row(self)

    if row is None:
        print("[v154] top row introuvable")
        return
    ...
    sep.pack(side="left", padx=(8, 2))

    v144_button(row, "Nouveau groove", self.v144_new_groove, P144_PINK_DARK).pack(side="left", padx=3)
    v144_button(row, "Miroir 32", self.v144_mirror_32, P144_PURPLE).pack(side="left", padx=3)
    v144_button(row, "Loop", self.v144_loop, P144_BLUE).pack(side="left", padx=3)

    # Pas le texte "Export WAV", pour éviter les vieux filtres.
    v144_button(row, "Exporter .wav", self.v144_export, P144_GREEN).pack(side="left", padx=3)

    self._v144_inline_toolbar_done = True

    print("[v154] toolbar intégrée dans la ligne du haut.")
    v144_status(self, "v144 : toolbar intégrée, pas de barre flottante.")


def v144_hide_old_ui(self):
    try:
        self.root.update_idletasks()
    except Exception:
        pass

    canvas = v144_canvas(self)
    if canvas is None:
        return
    ...
    for widget in v144_walk(self.root):
        ...
        try:
            if v144_contains(widget, canvas):
                continue
        except Exception:
            pass

        low = v144_text(widget).lower()
        ...
        try:
            ...
            if w > 1 and h > 1 and y >= canvas_bottom + 1:
                should_hide = True
        except Exception:
            pass

        # Ne cache jamais notre nouvelle toolbar intégrée.
        if low in {"nouveau groove", "miroir 32", "loop", "exporter .wav"}:
            should_hide = False
    ...
    try:
        ...
        new_h = max(520, int(canvas.winfo_y() + canvas.winfo_height() + 18))
        self.root.geometry(f"{root_w}x{new_h}+{root_x}+{root_y}")
    except Exception:
        pass

    print(f"[v154] vieux widgets cachés : {hidden}")


def v144_focus_grid(self, event=None):
    canvas = v144_canvas(self)
...
def v144_bind_focus_fix(self):
    for widget in v144_walk(self.root):
        ...
        try:
            widget.bind("<<ComboboxSelected>>", lambda e: self.root.after(80, lambda: v144_focus_grid(self)), add="+")
            widget.bind("<Escape>", lambda e: v144_focus_grid(self), add="+")
        except Exception:
            pass


_old_v144_build_ui = SliceIndexTracker.build_ui


def v144_build_ui(self):
    _old_v144_build_ui(self)

    try:
        self.root.title("BreakbeatAI v154 — random KSH full slots")
        self.root.configure(bg=P144_BG)
    except Exception:
        pass

    try:
        v144_install_inline_toolbar(self)
    except Exception as exc:
        print("[v154] install toolbar impossible :", exc)

    try:
        v144_hide_old_ui(self)
    except Exception as exc:
        print("[v154] hide old UI impossible :", exc)

    try:
        v144_bind_focus_fix(self)
    except Exception:
        pass

    for delay in [250, 700, 1200]:
        try:
            self.root.after(delay, lambda: v144_install_inline_toolbar(self))
            self.root.after(delay + 60, lambda: v144_hide_old_ui(self))
        except Exception:
            pass

    try:
        self.root.bind("<F12>", self.v144_new_groove)
        self.root.bind("<Control-e>", self.v144_export)
    except Exception:
        pass


SliceIndexTracker.v144_new_groove = v144_new_groove
SliceIndexTracker.v144_export = v144_export
SliceIndexTracker.v144_loop = v144_loop
SliceIndexTracker.v144_mirror_32 = v144_mirror_32
SliceIndexTracker.export_wav_current_v144 = v144_export
SliceIndexTracker.build_ui = v144_build_ui
...
# v148 : bouton Syncopé
# ---------------------------------------------------------------------
# Ajoute un bouton "Syncopé" dans la vraie barre :
# self.v81_training_panel
#
# Garde :
# - grille intacte
# - export WAV
# - multisélection
# - flèches + audition
# - bouton Nouveau groove
#
# Syncopé :
# - break très décalé / contretemps
# - hats nerveux
# - kick/snare poussés en retard/en avance
# - deuxième 32 = miroir du premier 32
# ---------------------------------------------------------------------

P148_PANEL = "#1a1024"
P148_PINK = "#ff8be8"
P148_PINK_DARK = "#7a2f68"
P148_PURPLE = "#332442"
P148_BLUE = "#27304d"
P148_GREEN = "#226b45"
P148_ORANGE = "#b85c38"
...
def v148_walk(widget):
    out = [widget]
    try:
        for child in widget.winfo_children():
            out.extend(v148_walk(child))
    except Exception:
        pass
    return out


def v148_text(widget):
    try:
        return str(widget.cget("text")).strip()
    except Exception:
        return ""
...
def v148_find_training_panel(self):
    panel = getattr(self, "v81_training_panel", None)
    ...
    for widget in v148_walk(self.root):
        try:
        ...
        child_texts = [v148_text(c) for c in widget.winfo_children()]

        if (
            "Generate Candidate" in child_texts
            and "Good après modifs" in child_texts
            and "Reject / Bad" in child_texts
        ):
            self.v81_training_panel = widget
            return widget

    return None


def v148_clear_panel(panel):
...
def v148_button(parent, text, command, bg):
    return tk.Button(
        parent,
        text=text,
        command=command,
...
def v148_pick_role_pairs(self):
    fn = globals().get("v131_pick_role_pairs")
    if callable(fn):
        try:
            picked = fn(self)
            return {
                "kick": int(picked["kick"]["pair"]),
                "snare": int(picked["snare"]["pair"]),
                "hat": int(picked["hat"]["pair"]),
            }
        except Exception:
            pass

    pairs = v148_all_pairs(self)

    return {
        "kick": int(pairs[0]) if len(pairs) > 0 else 0,
        "snare": int(pairs[1]) if len(pairs) > 1 else 1,
        "hat": int(pairs[2]) if len(pairs) > 2 else 2,
    }


def v148_lane_for_pair(self, pair):
    try:
        return int(self.pair_to_lane.get(int(pair), 0))
    except Exception:
        try:
            return int(pair)
        except Exception:
...
def v148_make_note(self, idx, step, role, pair, length=2, accent="main"):
    lane = v148_lane_for_pair(self, pair)
    ...
    return {
        ...
        "learned_role": str(role),
        "prior_label": str(role),
        "main_role_source": "v148_syncopated",
        "ai_generated": False,
        "tracker_generated": True,
        "v148_syncopated": True,
        "v148_accent": str(accent),
    }


def v148_syncopated_templates():
    return [
        {
            "name": "late snare push",
            "hits": [
                (0,  "kick",  2, "main"),
                (3,  "hat",   1, "ghost_hat"),
                (5,  "snare", 2, "late_snare"),
                (7,  "hat",   1, "hat"),
                (9,  "hat",   1, "off_hat"),
                (11, "kick",  2, "late_kick"),
                (13, "snare", 2, "late_snare"),
                (15, "hat",   1, "pickup"),
            ],
        },
        {
...
def v148_generate_syncopated(self, event=None):
    import random
    import time
    ...
    pairs = v148_pick_role_pairs(self)
    templates = v148_syncopated_templates()

    rng = random.Random(time.time())

    t1 = rng.choice(templates)
    t2 = rng.choice([t for t in templates if t["name"] != t1["name"]] or templates)

    first32 = []

    for base, template in [(0, t1), (16, t2)]:
        for pos, role, length, accent in template["hits"]:
            first32.append((base + pos, role, length, accent))

    # Fills nerveux, sans casser les coups principaux.
    used_steps = {step for step, role, length, accent in first32}

    fill_candidates = [1, 7, 11, 15, 17, 23, 27, 31]

    for step in fill_candidates:
        if step not in used_steps and rng.random() < 0.65:
            first32.append((step, "hat", 1, "extra_sync_hat"))
            used_steps.add(step)

    # Ghost snares très jungle sur fins de phrase.
    for step in [14, 29]:
        if step not in used_steps and rng.random() < 0.55:
            first32.append((step, "snare", 1, "ghost_snare_push"))
            used_steps.add(step)

    first32 = sorted(first32, key=lambda x: x[0])

    all_hits = []

    for step, role, length, accent in first32:
        all_hits.append((step, role, length, accent))

    # Second 32 miroir strict du premier 32.
    for step, role, length, accent in first32:
        all_hits.append((step + 32, role, length, accent + "_mirror"))

    all_hits = sorted(all_hits, key=lambda x: x[0])

    pattern = []

    for idx, (step, role, length, accent) in enumerate(all_hits):
        pair = pairs.get(role, pairs.get("hat", 0))
        pattern.append(v148_make_note(self, idx, step, role, pair, length=length, accent=accent))
    ...
    try:
        self.write_latest_pattern(reason="v148_syncopated")
    except Exception:
        pass

    print("")
    print("[v154] SYNCOPÉ")
    print("[v154] template 0-15 :", t1["name"])
    print("[v154] template 16-31:", t2["name"])
    print("[v154] pairs:", pairs)
    print("[v154] hits first32:", [(s, r, a) for s, r, l, a in first32])
    print("[v154] second32 miroir strict")
    print("")

    v148_status(self, "v148 : break ultra syncopé généré.")
    return "break"


def v148_new_groove(self, event=None):
    fn = getattr(self, "v144_new_groove", None)
    if callable(fn):
        return fn(event)
...
def v148_mirror_32(self, event=None):
    fn = getattr(self, "v144_mirror_32", None)
    if callable(fn):
        return fn(event)
    return "break"


def v148_loop(self, event=None):
    fn = getattr(self, "v144_loop", None)
    if callable(fn):
        return fn(event)
    return "break"


def v148_export(self, event=None):
    fn = getattr(self, "v144_export", None)
    if callable(fn):
        return fn(event)
    ...
    v148_status(self, "Export introuvable.")
    return "break"


def v148_replace_training_panel(self):
    panel = v148_find_training_panel(self)

    if panel is None:
        print("[v154] v81_training_panel introuvable")
        return

    if getattr(self, "_v148_training_panel_done", False):
        try:
            panel.lift()
        except Exception:
            pass
    ...
    try:
        ...
        panel.place_configure(width=992, height=54)
    except Exception:
        pass

    title_box = tk.Frame(panel, bg=P148_PANEL)
    title_box.pack(side="left", padx=(12, 10), pady=6)
    ...
    buttons = tk.Frame(panel, bg=P148_PANEL)
    buttons.pack(side="left", padx=6, pady=7)

    v148_button(buttons, "Nouveau groove", self.v148_new_groove, P148_PINK_DARK).pack(side="left", padx=4)
    v148_button(buttons, "Syncopé", self.v148_generate_syncopated, P148_ORANGE).pack(side="left", padx=4)
    v148_button(buttons, "Miroir 32", self.v148_mirror_32, P148_PURPLE).pack(side="left", padx=4)
    v148_button(buttons, "Loop / Space", self.v148_loop, P148_BLUE).pack(side="left", padx=4)
    v148_button(buttons, "Exporter .wav", self.v148_export, P148_GREEN).pack(side="left", padx=4)

    self.v148_toolbar = panel
    self.v81_training_panel = panel
    self._v148_training_panel_done = True
    ...
    print("[v154] toolbar installée avec bouton Syncopé.")
    v148_status(self, "v148 : bouton Syncopé ajouté.")


def v148_remove_duplicate_inline_buttons(self):
    labels = {
        "Nouveau groove",
        "Miroir 32",
        "Loop",
    ...
    for widget in list(v148_walk(self.root)):
        try:
            panel = getattr(self, "v148_toolbar", None)
    ...
    if removed:
        print(f"[v154] doublons inline supprimés : {removed}")


_old_v148_build_ui = SliceIndexTracker.build_ui


def v148_build_ui(self):
    _old_v148_build_ui(self)
    ...
    try:
        v148_replace_training_panel(self)
        v148_remove_duplicate_inline_buttons(self)
    except Exception as exc:
        print("[v154] toolbar impossible :", exc)

    for delay in [250, 700, 1200, 2000]:
        try:
            self.root.after(delay, lambda: v148_replace_training_panel(self))
            self.root.after(delay + 50, lambda: v148_remove_duplicate_inline_buttons(self))
        except Exception:
            pass

    try:
        self.root.bind("<F12>", self.v148_new_groove)
        self.root.bind("<F10>", self.v148_generate_syncopated)
        self.root.bind("<Control-e>", self.v148_export)
    except Exception:
        pass


SliceIndexTracker.v148_new_groove = v148_new_groove
SliceIndexTracker.v148_generate_syncopated = v148_generate_syncopated
SliceIndexTracker.v148_mirror_32 = v148_mirror_32
SliceIndexTracker.v148_loop = v148_loop
SliceIndexTracker.v148_export = v148_export
SliceIndexTracker.export_wav_current_v148 = v148_export
SliceIndexTracker.build_ui = v148_build_ui



# ---------------------------------------------------------------------
# v149 : Syncopé basé sur patterns ASCII
# ---------------------------------------------------------------------
# Le bouton Syncopé ne fait plus un random mou.
# Il lit des phrases ASCII de 32 cases :
#
# K = kick fort
# k = kick ghost/offbeat
# S = snare forte
# s = snare ghost
# h = hat
# . = silence
#
# Le deuxième 32 est un miroir strict du premier 32.
# ---------------------------------------------------------------------

V149_ASCII_PATTERNS = [
    {
        "name": "jungle tilt",
        "ascii": "K.h.S..h.hK..S.hK..h.S.h..kS.h..",
    },
    {
        "name": "amen stumble",
        "ascii": "K..h.S.h..k.hS..hK.h..S.h.k.h...",
    },
    {
        "name": "late snare maze",
        "ascii": "K.h..S.hK..h.s..K.hS..h..k.S.h..",
    },
    {
        "name": "broken machine",
        "ascii": "K..hS.h..h.K..S.h.Kh..s.hK..S...",
    },
    {
        "name": "panic pickup",
        "ascii": "K.hS..h.k..h.S..h.K..s.hK..S.h..",
    },
]


def v149_status(self, text):
    try:
        if hasattr(self, "set_status"):
            self.set_status(text)
        elif hasattr(self, "output_label"):
...
def v149_pretty_ascii(s):
    s = str(s)
    return "|" + "|".join([s[i:i+8] for i in range(0, len(s), 8)]) + "|"


def v149_pick_role_pairs(self):
    fn = globals().get("v131_pick_role_pairs")
    if callable(fn):
        try:
            picked = fn(self)
...
def v149_lane_for_pair(self, pair):
    try:
        return int(self.pair_to_lane.get(int(pair), 0))
    except Exception:
        try:
...
def v149_char_to_hit(ch):
    if ch == "K":
        return "kick", 2, "kick_main"
    if ch == "k":
        return "kick", 1, "kick_ghost"
    if ch == "S":
        return "snare", 2, "snare_main"
    if ch == "s":
        return "snare", 1, "snare_ghost"
    if ch in {"h", "H"}:
        return "hat", 1, "hat_sync"
    return None


def v149_make_note(self, idx, step, role, pair, length, accent, ch):
    lane = v149_lane_for_pair(self, pair)
    ...
    return {
        ...
        "learned_role": str(role),
        "prior_label": str(role),
        "main_role_source": "v149_ascii_syncopated",
        "ai_generated": False,
        "tracker_generated": True,
        "v149_ascii_syncopated": True,
        "v149_accent": str(accent),
        "v149_char": str(ch),
    }


def v149_pattern_from_ascii(self, ascii_pattern, pairs):
    if len(ascii_pattern) != 32:
        raise RuntimeError(f"Pattern ASCII invalide, longueur {len(ascii_pattern)} au lieu de 32 : {ascii_pattern}")

    hits_first32 = []

    for step, ch in enumerate(ascii_pattern):
        hit = v149_char_to_hit(ch)

        if hit is None:
            continue

        role, length, accent = hit
        hits_first32.append((step, role, length, accent, ch))

    all_hits = []

    for hit in hits_first32:
        all_hits.append(hit)

    # Miroir strict du premier 32 vers le deuxième 32.
    for step, role, length, accent, ch in hits_first32:
        all_hits.append((step + 32, role, length, accent + "_mirror", ch))

    all_hits = sorted(all_hits, key=lambda x: x[0])

    pattern = []

    for idx, (step, role, length, accent, ch) in enumerate(all_hits):
        pair = pairs.get(role, pairs.get("hat", 0))
        pattern.append(v149_make_note(self, idx, step, role, pair, length, accent, ch))

    return pattern, hits_first32


def v149_generate_ascii_syncopated(self, event=None):
    try:
        v131_force_64(self)
    except Exception:
        pass

    idx = int(getattr(self, "_v149_ascii_index", 0) or 0)
    template = V149_ASCII_PATTERNS[idx % len(V149_ASCII_PATTERNS)]

    self._v149_ascii_index = idx + 1

    name = template["name"]
    ascii_pattern = template["ascii"]

    pairs = v149_pick_role_pairs(self)

    pattern, hits_first32 = v149_pattern_from_ascii(self, ascii_pattern, pairs)
    ...
    try:
        self.write_latest_pattern(reason="v149_ascii_syncopated")
    except Exception:
        pass

    print("")
    print("[v154] ASCII SYNCOPÉ")
    print("[v154] name      :", name)
    print("[v154] legend    : K kick | k kick ghost | S snare | s ghost snare | h hat | . silence")
    print("[v154] indices   : |01234567|89012345|67890123|45678901|")
    print("[v154] pattern   :", v149_pretty_ascii(ascii_pattern))
    print("[v154] pairs     :", pairs)
    print("[v154] first32   :", [(s, r, ch) for s, r, length, accent, ch in hits_first32])
    print("[v154] second32  : miroir strict +32")
    print("")

    v149_status(self, f"v149 : Syncopé ASCII — {name}")
    return "break"


def v149_repatch_sync_button(self):
    patched = 0
    ...
    for widget in walk(self.root):
        ...
        if text == "Syncopé":
            try:
                widget.configure(command=self.v149_generate_ascii_syncopated)
                patched += 1
            except Exception:
                pass

    if patched:
        print(f"[v154] bouton Syncopé patché vers ASCII : {patched}")


_old_v149_build_ui = SliceIndexTracker.build_ui


def v149_build_ui(self):
    _old_v149_build_ui(self)
    ...
    try:
        v149_repatch_sync_button(self)
    except Exception as exc:
        print("[v154] patch bouton Syncopé impossible :", exc)

    for delay in [250, 700, 1200, 2000]:
        try:
            self.root.after(delay, lambda: v149_repatch_sync_button(self))
        except Exception:
            pass

    try:
        self.root.bind("<F10>", self.v149_generate_ascii_syncopated)
    except Exception:
        pass

    v149_status(self, "v149 : bouton Syncopé = patterns ASCII.")


SliceIndexTracker.v149_generate_ascii_syncopated = v149_generate_ascii_syncopated

# Le bouton v148 appelait self.v148_generate_syncopated :
# on le redirige aussi vers le nouveau moteur ASCII.
SliceIndexTracker.v148_generate_syncopated = v149_generate_ascii_syncopated

SliceIndexTracker.build_ui = v149_build_ui



# ---------------------------------------------------------------------
# v150 : reconnaissance kick / snare / hat / open_hat / crash / perc
# ---------------------------------------------------------------------
# Nouveau bouton :
#   Reconnaître slices
#
# Fonction :
# - analyse les slices du break actuel
# - classe les pairs/slices par rôle
# - sauvegarde un cache :
#     dataset/learning/slice_roles_detected_v150.json
# - Nouveau groove + Syncopé utilisent les rôles détectés
# ---------------------------------------------------------------------

P150_PANEL = "#1a1024"
P150_PINK = "#ff8be8"
P150_PINK_DARK = "#7a2f68"
P150_PURPLE = "#332442"
P150_BLUE = "#27304d"
P150_GREEN = "#226b45"
P150_ORANGE = "#b85c38"
...
_v150_old_v131_pick_role_pairs = globals().get("v131_pick_role_pairs")
_v150_old_v149_pick_role_pairs = globals().get("v149_pick_role_pairs")
_v150_old_v149_char_to_hit = globals().get("v149_char_to_hit")


def v150_walk(widget):
    out = [widget]
    try:
        for child in widget.winfo_children():
            out.extend(v150_walk(child))
    except Exception:
        pass
    return out


def v150_text(widget):
    try:
        return str(widget.cget("text")).strip()
    except Exception:
        return ""
...
def v150_current_break(self):
    fn = globals().get("v131_current_break")
    if callable(fn):
        try:
            return str(fn(self))
        except Exception:
    ...
    for attr in ["safe", "source", "break_name", "current_break", "current_source"]:
        try:
    ...
    for widget in v150_walk(self.root):
        try:
            value = str(widget.get())
        except Exception:
            continue

        if "_Break" in value or "Break_" in value or "Amen" in value or "Champion" in value:
            return value
...
def v150_cache_path():
    from pathlib import Path
    return Path("dataset/learning/slice_roles_detected_v150.json")


def v150_load_cache():
    import json
    path = v150_cache_path()

    if not path.exists():
        return {"version": "v150", "breaks": {}}
...
def v150_find_pairblocks_path(break_name):
    from pathlib import Path

    base = Path("dataset/pair_blocks_v02")
    candidates = [
        base / f"{break_name}_pair_blocks_v02.json",
        base / f"{break_name}.json",
    ]
    ...
    if base.exists():
        clean = str(break_name).lower().replace(" ", "_")
        for p in base.glob("*_pair_blocks_v02.json"):
            low = p.name.lower()
            if clean in low or low.startswith(clean[:10]):
                return p

    return None


def v150_safe_int(x, default=None):
    try:
        return int(x)
    except Exception:
        try:
...
def v150_safe_float(x, default=None):
    try:
        return float(x)
    except Exception:
        return default
...
def v150_flatten_records(obj, path="root"):
    records = []
    ...
    if isinstance(obj, dict):
        ...
        if any(k in keys for k in [
            "pair", "pair_id", "pair_index", "slice", "slice_id", "slice_index",
            "start", "end", "start_sample", "end_sample", "start_sec", "end_sec",
            "samples", "role", "label", "instrument"
        ]):
        ...
        for k, v in obj.items():
            if isinstance(v, (dict, list)):
                records.extend(v150_flatten_records(v, f"{path}.{k}"))

    elif isinstance(obj, list):
        for i, item in enumerate(obj):
            records.extend(v150_flatten_records(item, f"{path}[{i}]"))

    return records


def v150_pair_from_record(rec, fallback=None):
    for key in ["pair", "pair_id", "pair_index", "pair_idx", "slice", "slice_id", "slice_index", "id", "index"]:
        if key in rec:
            value = v150_safe_int(rec.get(key), None)
            if value is not None:
                return value

    path = str(rec.get("_v150_path", ""))
    m = re.search(r"(?:pair|slice)[_ -]?([0-9]+)", path, flags=re.I)
    if m:
        return int(m.group(1))
...
def v150_label_from_record(rec):
    for key in ["role", "label", "instrument", "class", "kind", "type", "main_role", "learned_role", "prior_label"]:
        value = str(rec.get(key, "")).lower()
        if not value:
            continue

        if "kick" in value or value in {"bd", "bassdrum", "bass_drum"}:
            return "kick"
        if "snare" in value or value in {"sd"}:
            return "snare"
        if "crash" in value:
            return "crash"
        if "open" in value and "hat" in value:
            return "open_hat"
        if "hat" in value or "hihat" in value or "hi_hat" in value:
            return "hat"
        if "tom" in value:
            return "tom"
        if "perc" in value:
...
def v150_extract_positions(rec, sr=None):
    sample_keys = [
    ...
    for a, b in sample_keys:
        if a in rec and b in rec:
            s = v150_safe_int(rec.get(a), None)
            e = v150_safe_int(rec.get(b), None)
    ...
    if sr:
        for a, b in sec_keys:
            if a in rec and b in rec:
                s = v150_safe_float(rec.get(a), None)
                e = v150_safe_float(rec.get(b), None)
                if s is not None and e is not None and e > s:
                    return int(s * sr), int(e * sr)

        ms_pairs = [("start_ms", "end_ms"), ("ms_start", "ms_end")]
        for a, b in ms_pairs:
            if a in rec and b in rec:
                s = v150_safe_float(rec.get(a), None)
                e = v150_safe_float(rec.get(b), None)
                if s is not None and e is not None and e > s:
                    return int((s / 1000.0) * sr), int((e / 1000.0) * sr)

    return None


def v150_find_wav_for_break(break_name, pairblocks_data=None):
    from pathlib import Path

    # 1. chemins mentionnés dans le JSON
    possible = []
    ...
    def scan_paths(obj):
        if isinstance(obj, dict):
            for k, v in obj.items():
                low = str(k).lower()
                if any(word in low for word in ["wav", "audio", "source", "file", "path"]):
                    if isinstance(v, str) and v.lower().endswith((".wav", ".aif", ".aiff")):
                        possible.append(v)
                scan_paths(v)
        elif isinstance(obj, list):
            for x in obj:
    ...
    if pairblocks_data is not None:
        scan_paths(pairblocks_data)
    ...
    # 2. recherche filesystem
    names = {
        str(break_name),
        str(break_name).replace("_", " "),
        str(break_name).replace("_-_", " - "),
        str(break_name).replace("_", "-"),
    }

    wavs = []
    for root in [Path("dataset"), Path("audio"), Path("samples"), Path(".")]:
        if not root.exists():
            continue
        try:
            wavs.extend(root.glob("**/*.wav"))
            wavs.extend(root.glob("**/*.aif"))
            wavs.extend(root.glob("**/*.aiff"))
        except Exception:
            pass
    ...
    for p in wavs:
        ...
        for n in names:
            ...
            if nn and nn in low:
                ...
                parts = [x for x in re.split(r"[_\-\s]+", nn) if x]
                score += sum(1 for part in parts if part in low)
...
def v150_read_wav_mono(path):
    ...
    if width == 1:
        ...
        raise RuntimeError(f"Largeur WAV non supportée : {width}")
...
def v150_features(samples, sr):
    ...
    n = int(min(max(256, 2 ** int(np.ceil(np.log2(max(32, len(x)))))), 8192))
    pad = np.zeros(n, dtype=np.float32)
    take = min(len(x), n)
    pad[:take] = x[:take]

    win = np.hanning(n).astype(np.float32)
    spec = np.abs(np.fft.rfft(pad * win)) + 1e-12
    freqs = np.fft.rfftfreq(n, d=1.0 / float(sr))
    ...
    mid = band(800, 3000)
    high = band(3000, 9000)
    ultra = band(9000, min(18000, sr / 2))
    ...
    frame = max(64, int(sr * 0.008))
    env = []
    for i in range(0, len(x), frame):
        chunk = x[i:i+frame]
        if len(chunk):
            env.append(float(np.sqrt(np.mean(chunk * chunk))))
    env = np.array(env, dtype=np.float32) if env else np.array([0.0], dtype=np.float32)

    env_max = float(np.max(env)) + 1e-9
    early = float(np.mean(env[:max(1, min(3, len(env)))])) / env_max
    late = float(np.mean(env[max(0, len(env)//2):])) / env_max
...
def v150_classify_features(f):
    low_energy = f["sub"] + f["low"]
    mid_energy = f["lowmid"] + f["mid"]
    high_energy = f["high"] + f["ultra"]
    broad = min(f["low"] + f["mid"] + f["high"], 1.0)
    dur = f["duration"]

    kick = (
        3.0 * low_energy
        + 1.2 * f["sub"]
        + 0.8 * (1.0 if f["centroid"] < 900 else 0.0)
        + 0.4 * f["early"]
        - 0.8 * high_energy
    )
    ...
    hat = (
        3.0 * high_energy
        + 0.8 * f["zcr"]
        + 0.6 * (1.0 if f["centroid"] > 3500 else 0.0)
        - 1.0 * low_energy
        - 0.3 * f["late"]
    )
...
def v150_load_pair_records(self, break_name):
    import json

    pairblocks_path = v150_find_pairblocks_path(break_name)
    pairblocks_data = None
    records = []

    if pairblocks_path is not None:
        try:
            pairblocks_data = json.loads(pairblocks_path.read_text(encoding="utf-8"))
            records = v150_flatten_records(pairblocks_data)
        except Exception as exc:
            print("[v154] lecture pairblocks impossible :", exc)

    out = []
    seen = set()

    for i, rec in enumerate(records):
        pair = v150_pair_from_record(rec, fallback=None)
        if pair is None:
            continue
        ...
        seen.add(pair)
        rec = dict(rec)
        rec["_v150_pair"] = int(pair)
        rec["_v150_label"] = v150_label_from_record(rec)
        out.append(rec)
    ...
    if not out:
        ...
        for p in pairs:
            out.append({"_v150_pair": int(p), "_v150_label": None, "_v150_fallback": True})

    return pairblocks_path, pairblocks_data, out


def v150_slice_audio_for_record(rec, pair_index, total_pairs, source_sr, source_audio):
    import numpy as np

    if "samples" in rec and isinstance(rec.get("samples"), list):
        arr = np.array(rec["samples"], dtype=np.float32)
        if arr.size > 16:
            return arr

    if source_audio is None or source_sr is None:
        return None

    pos = v150_extract_positions(rec, sr=source_sr)

    if pos is not None:
        s, e = pos
        s = max(0, min(len(source_audio), int(s)))
        e = max(0, min(len(source_audio), int(e)))
        if e > s + 16:
            return source_audio[s:e]

    # Fallback : découpe uniforme du break en N slices.
    if total_pairs > 0:
        step = len(source_audio) / float(total_pairs)
        s = int(pair_index * step)
        e = int((pair_index + 1) * step)
        if e > s + 16:
            return source_audio[s:e]
...
def v150_detect_roles_for_break(self, break_name):
    import time

    pairblocks_path, pairblocks_data, records = v150_load_pair_records(self, break_name)

    wav_path = v150_find_wav_for_break(break_name, pairblocks_data=pairblocks_data)
    source_sr = None
    source_audio = None

    if wav_path is not None:
        try:
            source_sr, source_audio = v150_read_wav_mono(wav_path)
        except Exception as exc:
            print("[v154] lecture wav impossible :", wav_path, exc)
            wav_path = None

    pairs_sorted = sorted([int(r["_v150_pair"]) for r in records])
    pair_to_index = {p: i for i, p in enumerate(pairs_sorted)}
    ...
    for rec in records:
        pair = int(rec["_v150_pair"])
        pair_index = pair_to_index.get(pair, 0)

        label = rec.get("_v150_label")
        detected_role = None
        scores = {}
        features = {}

        audio = v150_slice_audio_for_record(
            rec,
            pair_index=pair_index,
            total_pairs=max(1, len(pairs_sorted)),
            source_sr=source_sr,
            source_audio=source_audio,
        )

        if audio is not None and source_sr is not None:
            f = v150_features(audio, source_sr)
            if f is not None:
                detected_role, scores = v150_classify_features(f)
                features = f

        if detected_role is None and label is not None:
            detected_role = label
            scores = {label: 1.0}
        ...
        best_score = float(scores.get(detected_role, 0.0))
        ...
        item = {
            ...
            "label_from_json": label,
        }

        pair_results[str(pair)] = item
        roles.setdefault(detected_role, []).append({
            "pair": int(pair),
            "score": best_score,
            "role": detected_role,
        })
    ...
    for role in list(roles.keys()):
        roles[role] = sorted(roles[role], key=lambda x: x.get("score", 0.0), reverse=True)

    result = {
        "break": break_name,
        "updated_at": time.strftime("%Y-%m-%d %H:%M:%S"),
        "pairblocks_path": str(pairblocks_path) if pairblocks_path else None,
...
def v150_detect_current_break(self, event=None):
    break_name = v150_current_break(self)
    v150_status(self, f"v150 : reconnaissance slices en cours — {break_name}")

    try:
        result = v150_detect_roles_for_break(self, break_name)
    except Exception as exc:
        print("[v154] ERREUR reconnaissance :", exc)
        v150_status(self, "Reconnaissance impossible. Regarde le terminal.")
        return "break"

    cache = v150_load_cache()
    cache.setdefault("breaks", {})
    cache["breaks"][break_name] = result
    v150_save_cache(cache)

    print("")
    print("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
    print("[v154] RECONNAISSANCE SLICES")
    print("[v154] break      :", break_name)
    print("[v154] pairblocks :", result.get("pairblocks_path"))
    print("[v154] wav        :", result.get("wav_path"))

    for role in ["kick", "snare", "hat", "open_hat", "crash", "tom", "perc", "other"]:
        top = result.get("roles", {}).get(role, [])[:8]
        print(f"[v154] {role:8s} :", [(x["pair"], round(float(x.get("score", 0)), 3)) for x in top])

    print("[v154] cache écrit : dataset/learning/slice_roles_detected_v150.json")
    print("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
    print("")

    v150_status(self, f"v150 : slices reconnues pour {break_name}.")
    return "break"


def v150_load_or_detect_current(self):
    break_name = v150_current_break(self)
    cache = v150_load_cache()
    entry = cache.get("breaks", {}).get(break_name)

    if entry:
        return break_name, entry

    entry = v150_detect_roles_for_break(self, break_name)
    cache.setdefault("breaks", {})
    cache["breaks"][break_name] = entry
    v150_save_cache(cache)

    return break_name, entry


def v150_best_pair(entry, role_group, fallback_pair=None):
    ...
    for role in role_group:
        ...
        if items:
            try:
                return int(items[0]["pair"])
            except Exception:
                pass

    if fallback_pair is not None:
        return int(fallback_pair)
...
def v150_detected_role_pairs_simple(self):
    break_name, entry = v150_load_or_detect_current()

    kick = v150_best_pair(entry, ["kick", "tom", "perc"], 0)
    snare = v150_best_pair(entry, ["snare", "perc", "crash"], kick)
    hat = v150_best_pair(entry, ["hat", "open_hat", "crash", "perc"], snare)
    crash = v150_best_pair(entry, ["crash", "open_hat", "hat"], hat)
    open_hat = v150_best_pair(entry, ["open_hat", "hat", "crash"], hat)

    print("[v154] rôles utilisés :", {
        "break": break_name,
        "kick": kick,
        "snare": snare,
        "hat": hat,
...
def v131_pick_role_pairs(self):
    try:
        pairs = v150_detected_role_pairs_simple(self)
        return {
            "kick": {"pair": int(pairs["kick"]), "source": "v150_detected_kick"},
            "snare": {"pair": int(pairs["snare"]), "source": "v150_detected_snare"},
            "hat": {"pair": int(pairs["hat"]), "source": "v150_detected_hat"},
        }
    except Exception as exc:
        print("[v154] fallback v131_pick_role_pairs :", exc)

    if callable(_v150_old_v131_pick_role_pairs):
        return _v150_old_v131_pick_role_pairs(self)

    return {
        "kick": {"pair": 0, "source": "fallback"},
        "snare": {"pair": 1, "source": "fallback"},
        "hat": {"pair": 2, "source": "fallback"},
    }
...
def v149_pick_role_pairs(self):
    try:
        ...
        return {
            "kick": int(pairs["kick"]),
            "snare": int(pairs["snare"]),
            "hat": int(pairs["hat"]),
            "open_hat": int(pairs["open_hat"]),
            "crash": int(pairs["crash"]),
        }
    except Exception as exc:
        print("[v154] fallback v149_pick_role_pairs :", exc)

    if callable(_v150_old_v149_pick_role_pairs):
        return _v150_old_v149_pick_role_pairs(self)

    return {"kick": 0, "snare": 1, "hat": 2, "open_hat": 2, "crash": 2}
...
def v149_char_to_hit(ch):
    ...
    if ch == "h":
        return "hat", 1, "hat_sync"
    if ch == "H":
        return "open_hat", 2, "open_hat"
    if ch == "C":
        return "crash", 2, "crash"
    return None


def v150_generate_ascii_syncopated(self, event=None):
    # Force détection avant syncopé, puis utilise le moteur ASCII v149 patché.
    try:
        v150_load_or_detect_current(self)
    except Exception as exc:
        print("[v154] auto-détection avant Syncopé impossible :", exc)

    fn = getattr(self, "v149_generate_ascii_syncopated", None)
    if callable(fn):
        return fn(event)

    fn = getattr(self, "v148_generate_syncopated", None)
    if callable(fn):
        return fn(event)

    v150_status(self, "Syncopé introuvable.")
    return "break"


def v150_new_groove(self, event=None):
    try:
        v150_load_or_detect_current(self)
    except Exception as exc:
        print("[v154] auto-détection avant Nouveau groove impossible :", exc)
...
def v150_mirror_32(self, event=None):
    for name in ["v148_mirror_32", "v147_mirror_32", "v144_mirror_32"]:
        fn = getattr(self, name, None)
        if callable(fn):
            return fn(event)
    return "break"
...
def v150_loop(self, event=None):
    for name in ["v148_loop", "v147_loop", "v144_loop"]:
        fn = getattr(self, name, None)
        if callable(fn):
            return fn(event)
    return "break"
...
def v150_export(self, event=None):
    for name in ["v148_export", "v147_export", "v144_export"]:
        fn = getattr(self, name, None)
        if callable(fn):
            return fn(event)
    ...
    v150_status(self, "Export introuvable.")
    return "break"


def v150_find_training_panel(self):
    panel = getattr(self, "v81_training_panel", None)
    ...
    for widget in v150_walk(self.root):
        ...
        child_texts = [v150_text(c) for c in widget.winfo_children()]

        if "Generate Candidate" in child_texts and "Reject / Bad" in child_texts:
            self.v81_training_panel = widget
            return widget

        if "Nouveau groove" in child_texts and "Exporter .wav" in child_texts:
            self.v81_training_panel = widget
            return widget
...
def v150_button(parent, text, command, bg):
    return tk.Button(
        parent,
        text=text,
        command=command,
...
def v150_replace_toolbar(self):
    panel = v150_find_training_panel(self)

    if panel is None:
        print("[v154] v81_training_panel introuvable")
        return

    if getattr(self, "_v150_toolbar_done", False):
        try:
            panel.lift()
        except Exception:
            pass
    ...
    try:
        ...
        panel.place_configure(width=1040, height=54)
    except Exception:
        pass

    title_box = tk.Frame(panel, bg=P150_PANEL)
    title_box.pack(side="left", padx=(12, 8), pady=6)
    ...
    tk.Label(
        title_box,
        text="slices reconnues",
        bg=P150_PANEL,
        fg=P150_MUTED,
        font=("Sans", 8),
    ).pack(side="top", anchor="w")

    buttons = tk.Frame(panel, bg=P150_PANEL)
    buttons.pack(side="left", padx=4, pady=7)

    v150_button(buttons, "Reconnaître", self.v150_detect_current_break, P150_CYAN).pack(side="left", padx=3)
    v150_button(buttons, "Nouveau groove", self.v150_new_groove, P150_PINK_DARK).pack(side="left", padx=3)
    v150_button(buttons, "Syncopé", self.v150_generate_ascii_syncopated, P150_ORANGE).pack(side="left", padx=3)
    v150_button(buttons, "Miroir 32", self.v150_mirror_32, P150_PURPLE).pack(side="left", padx=3)
    v150_button(buttons, "Loop / Space", self.v150_loop, P150_BLUE).pack(side="left", padx=3)
    v150_button(buttons, "Exporter .wav", self.v150_export, P150_GREEN).pack(side="left", padx=3)

    self.v150_toolbar = panel
    self.v81_training_panel = panel
    self._v150_toolbar_done = True

    print("[v154] toolbar installée avec Reconnaître slices.")
    v150_status(self, "v150 : Reconnaître slices prêt.")


_old_v150_build_ui = SliceIndexTracker.build_ui


def v150_build_ui(self):
    _old_v150_build_ui(self)
    ...
    try:
        v150_replace_toolbar(self)
    except Exception as exc:
        print("[v154] toolbar impossible :", exc)

    for delay in [250, 700, 1200, 2000]:
        try:
            self.root.after(delay, lambda: v150_replace_toolbar(self))
        except Exception:
            pass

    try:
        self.root.bind("<F8>", self.v150_detect_current_break)
        self.root.bind("<F10>", self.v150_generate_ascii_syncopated)
        self.root.bind("<F12>", self.v150_new_groove)
        self.root.bind("<Control-e>", self.v150_export)
    except Exception:
        pass


SliceIndexTracker.v150_detect_current_break = v150_detect_current_break
SliceIndexTracker.v150_new_groove = v150_new_groove
SliceIndexTracker.v150_generate_ascii_syncopated = v150_generate_ascii_syncopated
SliceIndexTracker.v150_mirror_32 = v150_mirror_32
SliceIndexTracker.v150_loop = v150_loop
SliceIndexTracker.v150_export = v150_export
SliceIndexTracker.export_wav_current_v150 = v150_export

SliceIndexTracker.build_ui = v150_build_ui



# ---------------------------------------------------------------------
# v151 : reconnaissance automatique des slices
# ---------------------------------------------------------------------
# Plus besoin de cliquer sur "Reconnaître".
#
# Auto :
# - au lancement
# - après sélection/changement de break
# - après Charger break
# - avant Nouveau groove
# - avant Syncopé
#
# Le cache v150 est conservé :
# dataset/learning/slice_roles_detected_v150.json
# ---------------------------------------------------------------------


def v151_walk(widget):
    out = [widget]
    try:
        for child in widget.winfo_children():
            out.extend(v151_walk(child))
    except Exception:
        pass
    return out


def v151_text(widget):
    try:
        return str(widget.cget("text")).strip()
    except Exception:
        return ""
...
def v151_current_break(self):
    fn = globals().get("v150_current_break")
    if callable(fn):
        try:
            return str(fn(self))
        except Exception:
...
def v151_cache_has_break(break_name):
    try:
        cache = v150_load_cache()
        return bool(cache.get("breaks", {}).get(str(break_name)))
    except Exception:
        return False


def v151_auto_prepare_roles(self, force=False, reason="auto"):
    break_name = v151_current_break(self)

    if not force and v151_cache_has_break(break_name):
        self._v151_roles_ready_break = break_name
        print(f"[v154] rôles déjà prêts pour {break_name} ({reason})")
        v151_status(self, f"v151 : rôles déjà prêts — {break_name}")
        return "ready"

    if getattr(self, "_v151_auto_detect_busy", False):
        print(f"[v154] détection déjà en cours pour {break_name}")
        return "busy"

    self._v151_auto_detect_busy = True

    try:
        print("")
        print("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
        print("[v154] AUTO-RECONNAISSANCE")
        print("[v154] reason :", reason)
        print("[v154] break  :", break_name)
        print("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
        print("")

        v151_status(self, f"v151 : reconnaissance auto — {break_name}")

        # Utilise le moteur v150.
        fn = getattr(self, "v150_detect_current_break", None)
        if callable(fn):
            result = fn(None)
        else:
            result = v150_detect_current_break(self, None)

        self._v151_roles_ready_break = break_name
        v151_status(self, f"v151 : rôles auto prêts — {break_name}")
        return result

    except Exception as exc:
        print("[v154] auto-reconnaissance impossible :", exc)
        v151_status(self, "v151 : auto-reconnaissance impossible.")
        return "error"

    finally:
        self._v151_auto_detect_busy = False


def v151_auto_prepare_roles_later(self, delay=250, force=False, reason="auto"):
    try:
        self.root.after(delay, lambda: v151_auto_prepare_roles(self, force=force, reason=reason))
    except Exception:
        v151_auto_prepare_roles(self, force=force, reason=reason)


def v151_patch_break_selector(self):
    patched = 0

    for widget in v151_walk(self.root):
        cls = ""
        try:
            cls = widget.winfo_class().lower()
        except Exception:
        ...
        if getattr(widget, "_v151_combo_patched", False):
            continue

        try:
            widget.bind(
                "<<ComboboxSelected>>",
                lambda e: v151_auto_prepare_roles_later(self, delay=450, force=False, reason="combobox selected"),
                add="+",
            )
            widget.bind(
                "<Return>",
                lambda e: v151_auto_prepare_roles_later(self, delay=450, force=False, reason="combobox return"),
                add="+",
            )
            widget.bind(
                "<FocusOut>",
                lambda e: v151_auto_prepare_roles_later(self, delay=700, force=False, reason="combobox focusout"),
                add="+",
            )
            widget._v151_combo_patched = True
            patched += 1
        except Exception:
            pass

    if patched:
        print(f"[v154] sélecteurs de break patchés auto-rôles : {patched}")


def v151_patch_charger_break(self):
    patched = 0

    for widget in v151_walk(self.root):
        if v151_text(widget).lower() != "charger break":
            continue

        if getattr(widget, "_v151_charger_patched", False):
            continue
        ...
        def wrapped(cmd=old_cmd):
            try:
                if cmd:
                    self.root.tk.call(cmd)
            except Exception as exc:
                print("[v154] ancienne commande Charger break erreur :", exc)

            v151_auto_prepare_roles_later(self, delay=650, force=False, reason="charger break")

        try:
            widget.configure(command=wrapped)
            widget._v151_charger_patched = True
            patched += 1
        except Exception:
            pass

    if patched:
        print(f"[v154] bouton Charger break patché auto-rôles : {patched}")


def v151_patch_toolbar_label(self):
    """
    Renomme le bouton Reconnaître pour indiquer que c'est automatique.
    Il reste cliquable pour forcer une nouvelle analyse.
    """
    for widget in v151_walk(self.root):
        if v151_text(widget) == "Reconnaître":
            try:
                widget.configure(text="Auto-rôles", command=lambda: v151_auto_prepare_roles(self, force=True, reason="manual force"))
            except Exception:
                pass


def v151_new_groove(self, event=None):
    v151_auto_prepare_roles(self, force=False, reason="before nouveau groove")

    fn = getattr(self, "v150_new_groove", None)
    if callable(fn):
        return fn(event)
...
def v151_syncopated(self, event=None):
    v151_auto_prepare_roles(self, force=False, reason="before syncopé")

    fn = getattr(self, "v150_generate_ascii_syncopated", None)
    if callable(fn):
        return fn(event)
...
def v151_repatch_main_buttons(self):
    """
    Repointe Nouveau groove / Syncopé vers les versions qui préparent les rôles.
    """
    patched = 0

    for widget in v151_walk(self.root):
        text = v151_text(widget)

        try:
            if text == "Nouveau groove":
                widget.configure(command=self.v151_new_groove)
                patched += 1
            elif text == "Syncopé":
                widget.configure(command=self.v151_syncopated)
                patched += 1
        except Exception:
            pass

    if patched:
        print(f"[v154] boutons principaux repatchés auto-rôles : {patched}")


_old_v151_build_ui = SliceIndexTracker.build_ui


def v151_build_ui(self):
    _old_v151_build_ui(self)
    ...
    try:
        v151_patch_toolbar_label(self)
        v151_patch_break_selector(self)
        v151_patch_charger_break(self)
        v151_repatch_main_buttons(self)
    except Exception as exc:
        print("[v154] patch UI auto-rôles impossible :", exc)

    # Détection automatique au lancement, après que le break soit chargé.
    v151_auto_prepare_roles_later(self, delay=900, force=False, reason="startup")

    for delay in [400, 1000, 1800, 3000]:
        try:
            self.root.after(delay, lambda: v151_patch_toolbar_label(self))
            self.root.after(delay, lambda: v151_patch_break_selector(self))
            self.root.after(delay, lambda: v151_patch_charger_break(self))
            self.root.after(delay, lambda: v151_repatch_main_buttons(self))
        except Exception:
            pass

    try:
        self.root.bind("<F8>", lambda e: v151_auto_prepare_roles(self, force=True, reason="F8 force"))
        self.root.bind("<F10>", self.v151_syncopated)
        self.root.bind("<F12>", self.v151_new_groove)
    except Exception:
        pass

    v151_status(self, "v151 : reconnaissance slices automatique.")


SliceIndexTracker.v151_new_groove = v151_new_groove
SliceIndexTracker.v151_syncopated = v151_syncopated
SliceIndexTracker.v151_auto_prepare_roles = v151_auto_prepare_roles

SliceIndexTracker.build_ui = v151_build_ui



# ---------------------------------------------------------------------
# v152 : Variations kick / snare / hat
# ---------------------------------------------------------------------
# Remplace l'idée "Syncopé" par :
# - variations musicales kick / snare / hat
# - snare lisible
# - hats dans les trous
# - kicks variés à partir de patterns de batterie
# - second 32 miroir strict
# - utilise les slices reconnues automatiquement v150/v151
# ---------------------------------------------------------------------

V152_VARIATIONS = [
    {
        "name": "two step propre",
        "kick":  [0, 10, 16, 26],
        "snare": [4, 12, 20, 28],
    },
    {
        "name": "double kick léger",
        "kick":  [0, 6, 10, 16, 24, 26],
        "snare": [4, 12, 20, 28],
    },
    {
        "name": "amen bass drum simple",
        "kick":  [0, 8, 10, 16, 18, 26],
        "snare": [4, 12, 20, 28],
    },
    {
        "name": "funk push",
        "kick":  [0, 6, 14, 16, 22, 26],
        "snare": [4, 12, 20, 28],
    },
    {
        "name": "rolling kick",
    ...
    {
        "name": "breakbeat stable",
        "kick":  [0, 8, 16, 24, 26, 30],
        "snare": [4, 12, 20, 28],
    },
    {
        "name": "pickup kick",
        "kick":  [0, 10, 15, 16, 24, 26, 31],
        "snare": [4, 12, 20, 28],
    },
    {
        "name": "snare ghost propre",
        "kick":  [0, 10, 16, 22, 26],
        "snare": [4, 12, 20, 23, 28],
    },
]
...
def v152_detected_pairs(self):
    try:
        if hasattr(self, "v151_auto_prepare_roles"):
            self.v151_auto_prepare_roles(force=False, reason="before variations")
    except Exception:
        pass
    ...
    try:
        ...
        print("[v154] fallback rôles détectés :", exc)
    ...
    return {"kick": 0, "snare": 1, "hat": 2}


def v152_lane_for_pair(self, pair):
    try:
        return int(self.pair_to_lane.get(int(pair), 0))
    except Exception:
        try:
...
def v152_make_note(self, idx, step, role, pair, length=2, accent="main"):
    lane = v152_lane_for_pair(self, pair)
    ...
    return {
        ...
        "learned_role": str(role),
        "prior_label": str(role),
        "main_role_source": "v152_detected_variation",
        "ai_generated": False,
        "tracker_generated": True,
        "v152_variation": True,
        "v152_accent": str(accent),
    }


def v152_hat_positions(kick_positions, snare_positions):
    """
    Hats réguliers, mais jamais sur les cases kick/snare.
    Ça évite les superpositions moches et garde la lecture tracker propre.
    """
    blocked = set(int(x) for x in kick_positions) | set(int(x) for x in snare_positions)
    hats = []
    ...
    # Quelques hats de relance dans les trous, mais contrôlés.
    for step in [7, 15, 23, 31]:
        if step not in blocked:
            hats.append(step)

    return sorted(set(hats))


def v152_ascii_line(positions, char, length=32):
    arr = ["."] * length
    for p in positions:
        if 0 <= int(p) < length:
            arr[int(p)] = char
    return "|" + "|".join("".join(arr[i:i+8]) for i in range(0, length, 8)) + "|"


def v152_generate_variations(self, event=None):
    try:
        v131_force_64(self)
    except Exception:
        pass

    pairs = v152_detected_pairs(self)

    idx = int(getattr(self, "_v152_variation_index", 0) or 0)
    template = V152_VARIATIONS[idx % len(V152_VARIATIONS)]
    self._v152_variation_index = idx + 1

    kick = sorted(set(int(x) for x in template["kick"]))
    snare = sorted(set(int(x) for x in template["snare"]))
    hat = v152_hat_positions(kick, snare)

    hits_first32 = []

    for step in kick:
        hits_first32.append((step, "kick", 2, "kick_variation"))

    for step in snare:
        # Ghost snare si elle n'est pas exactement sur les backbeats principaux.
        length = 2 if step in [4, 12, 20, 28] else 1
        accent = "snare_main" if length == 2 else "snare_ghost"
        hits_first32.append((step, "snare", length, accent))

    for step in hat:
        length = 1 if step % 2 else 2
        hits_first32.append((step, "hat", length, "hat_fill"))

    # Une seule note par case : priorité snare > kick > hat.
    priority = {"snare": 0, "kick": 1, "hat": 2}
    by_step = {}

    for step, role, length, accent in sorted(hits_first32, key=lambda x: (x[0], priority.get(x[1], 9))):
        if step not in by_step:
            by_step[step] = (step, role, length, accent)

    hits_first32 = [by_step[k] for k in sorted(by_step.keys())]
    ...
    # Miroir strict +32.
    for step, role, length, accent in hits_first32:
        all_hits.append((step + 32, role, length, accent + "_mirror"))
    ...
    for note_id, (step, role, length, accent) in enumerate(all_hits):
        pair = int(pairs.get(role, pairs.get("hat", 0)))
        pattern.append(v152_make_note(self, note_id, step, role, pair, length=length, accent=accent))
    ...
    try:
        self.write_latest_pattern(reason="v152_kick_snare_hat_variation")
    except Exception:
        pass

    print("")
    print("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
    print("[v154] VARIATION KICK / SNARE / HAT")
    print("[v154] name  :", template["name"])
    print("[v154] pairs :", pairs)
    print("[v154] KICK  :", v152_ascii_line(kick, "K"))
    print("[v154] SNARE :", v152_ascii_line(snare, "S"))
    print("[v154] HAT   :", v152_ascii_line(hat, "h"))
    print("[v154] second32 : miroir strict +32")
    print("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
    print("")

    v152_status(self, f"v152 : variation — {template['name']}")
    return "break"


def v152_repatch_toolbar(self):
    patched = 0
    ...
    for widget in v151_walk(self.root):
        ...
        try:
            if text == "Syncopé":
                widget.configure(text="Variations", command=self.v152_generate_variations)
                patched += 1
            elif text == "Nouveau groove":
                # Nouveau groove reste la base stable auto-rôles.
                widget.configure(command=self.v151_new_groove)
            elif text == "Variations":
                widget.configure(command=self.v152_generate_variations)
                patched += 1
        except Exception:
            pass

    if patched:
        print(f"[v154] bouton Variations patché : {patched}")


_old_v152_build_ui = SliceIndexTracker.build_ui


def v152_build_ui(self):
    _old_v152_build_ui(self)
    ...
    try:
        v152_repatch_toolbar(self)
    except Exception as exc:
        print("[v154] patch toolbar impossible :", exc)

    for delay in [250, 700, 1200, 2000, 3200]:
        try:
            self.root.after(delay, lambda: v152_repatch_toolbar(self))
        except Exception:
            pass

    try:
        self.root.bind("<F10>", self.v152_generate_variations)
    except Exception:
        pass

    v152_status(self, "v152 : Syncopé remplacé par Variations kick/snare/hat.")


SliceIndexTracker.v152_generate_variations = v152_generate_variations

# Tous les anciens chemins Syncopé pointent maintenant vers Variations.
SliceIndexTracker.v151_syncopated = v152_generate_variations
SliceIndexTracker.v150_generate_ascii_syncopated = v152_generate_variations
SliceIndexTracker.v149_generate_ascii_syncopated = v152_generate_variations
SliceIndexTracker.v148_generate_syncopated = v152_generate_variations

SliceIndexTracker.build_ui = v152_build_ui



# ---------------------------------------------------------------------
# v153 : Random K/S/H propre
# ---------------------------------------------------------------------
# Remplace Variations par Random K/S/H.
#
# But :
# - pas de hats bizarres
# - pas de cases impaires
# - pas de longueurs 1
# - une seule note par case
# - kick/snare/hat bougent tous
# - second 32 = miroir strict
# ---------------------------------------------------------------------
...
V153_HAT_BANK = [
    ...
    [2, 6, 10, 12, 14, 18, 22, 26, 28, 30],
    [2, 4, 6, 8, 10, 14, 18, 22, 26, 30],
]


def v153_walk(widget):
    out = [widget]
    try:
        for child in widget.winfo_children():
            out.extend(v153_walk(child))
    except Exception:
        pass
    return out


def v153_text(widget):
    try:
        return str(widget.cget("text")).strip()
    except Exception:
        return ""
...
def v153_detected_pairs(self):
    try:
        if hasattr(self, "v151_auto_prepare_roles"):
            self.v151_auto_prepare_roles(force=False, reason="before random KSH")
    except Exception:
        pass
...
def v153_lane_for_pair(self, pair):
    try:
        return int(self.pair_to_lane.get(int(pair), 0))
    except Exception:
        try:
...
def v153_make_note(self, idx, step, role, pair):
    lane = v153_lane_for_pair(self, pair)
    ...
    return {
        ...
        "learned_role": str(role),
        "prior_label": str(role),
        "main_role_source": "v153_random_ksh_detected",
        "ai_generated": False,
        "tracker_generated": True,
        "v153_random_ksh": True,
    }


def v153_ascii_line(positions, char, length=32):
    arr = ["."] * length
    for p in positions:
        p = int(p)
        if 0 <= p < length:
...
def v153_clean_even_positions(values):
    out = []

    for x in values:
        x = int(x)

        # On force en cases paires pour éviter les hats chelous et les demi-cases.
        if x % 2 != 0:
            x -= 1
...
def v153_randomize_ksh(self, event=None):
    import random
    import time
    ...
    pairs = v153_detected_pairs(self)

    rng = random.Random(time.time())

    kick = v153_clean_even_positions(rng.choice(V153_KICK_BANK))
    snare = v153_clean_even_positions(rng.choice(V153_SNARE_BANK))
    hats_raw = v153_clean_even_positions(rng.choice(V153_HAT_BANK))

    # Petit random propre : parfois on ajoute/retire un kick ou snare,
    # mais toujours sur cases paires et sans devenir le bazar.
    kick_extra_pool = [2, 6, 8, 10, 14, 18, 22, 24, 26, 30]
    snare_extra_pool = [6, 10, 14, 22, 26, 30]

    if rng.random() < 0.45:
        k = rng.choice(kick_extra_pool)
        if k not in kick:
            kick.append(k)

    if rng.random() < 0.35:
        s = rng.choice(snare_extra_pool)
        if s not in snare:
            snare.append(s)
    ...
    # Priorité : snare > kick > hat.
    # Si une snare tombe sur un kick, la snare gagne.
    snare_set = set(snare)
    kick = [x for x in kick if x not in snare_set]

    occupied = set(kick) | set(snare)

    # Hats uniquement dans les cases libres.
    hat = [x for x in hats_raw if x not in occupied]

    # Si trop peu de hats, on remplit doucement les trous pairs libres.
    if len(hat) < 5:
        for x in range(2, 32, 4):
            if x not in occupied and x not in hat:
                hat.append(x)
    ...
    for step in kick:
        hits_first32.append((step, "kick"))

    for step in snare:
        hits_first32.append((step, "snare"))

    for step in hat:
        hits_first32.append((step, "hat"))

    priority = {"snare": 0, "kick": 1, "hat": 2}
    hits_first32 = sorted(hits_first32, key=lambda x: (x[0], priority.get(x[1], 9)))

    # Une seule note par case.
    by_step = {}

    for step, role in hits_first32:
        if step not in by_step:
            by_step[step] = role

    hits_first32 = [(step, by_step[step]) for step in sorted(by_step.keys())]

    all_hits = []

    for step, role in hits_first32:
        all_hits.append((step, role))

    # Miroir strict +32.
    for step, role in hits_first32:
        all_hits.append((step + 32, role))

    pattern = []

    for note_id, (step, role) in enumerate(all_hits):
        pair = int(pairs.get(role, pairs.get("hat", 0)))
        pattern.append(v153_make_note(self, note_id, step, role, pair))
    ...
    try:
        self.write_latest_pattern(reason="v153_random_ksh")
    except Exception:
        pass

    print("")
    print("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
    print("[v154] RANDOM KICK / SNARE / HAT")
    print("[v154] pairs :", pairs)
    print("[v154] KICK  :", v153_ascii_line(kick, "K"))
    print("[v154] SNARE :", v153_ascii_line(snare, "S"))
    print("[v154] HAT   :", v153_ascii_line(hat, "h"))
    print("[v154] règles: cases paires seulement, longueur 2, une note par case")
    print("[v154] second32 : miroir strict +32")
    print("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
    print("")

    v153_status(self, "v153 : Random K/S/H propre généré.")
    return "break"


def v153_repatch_toolbar(self):
    patched = 0

    for widget in v153_walk(self.root):
        text = v153_text(widget)

        try:
            if text in {"Variations", "Syncopé", "Random K/S/H"}:
                widget.configure(text="Random K/S/H", command=self.v153_randomize_ksh)
                patched += 1
        except Exception:
            pass

    if patched:
        print(f"[v154] bouton Random K/S/H patché : {patched}")


_old_v153_build_ui = SliceIndexTracker.build_ui


def v153_build_ui(self):
    _old_v153_build_ui(self)
    ...
    try:
        v153_repatch_toolbar(self)
    except Exception as exc:
        print("[v154] patch toolbar impossible :", exc)

    for delay in [250, 700, 1200, 2000, 3200]:
        try:
            self.root.after(delay, lambda: v153_repatch_toolbar(self))
        except Exception:
            pass

    try:
        self.root.bind("<F10>", self.v153_randomize_ksh)
    except Exception:
        pass

    v153_status(self, "v153 : Variations remplacé par Random K/S/H propre.")


SliceIndexTracker.v153_randomize_ksh = v153_randomize_ksh

# Tous les anciens chemins syncopé/variations pointent vers le random propre.
SliceIndexTracker.v152_generate_variations = v153_randomize_ksh
SliceIndexTracker.v151_syncopated = v153_randomize_ksh
SliceIndexTracker.v150_generate_ascii_syncopated = v153_randomize_ksh
SliceIndexTracker.v149_generate_ascii_syncopated = v153_randomize_ksh
SliceIndexTracker.v148_generate_syncopated = v153_randomize_ksh

SliceIndexTracker.build_ui = v153_build_ui



# ---------------------------------------------------------------------
# v154 : Random K/S/H plein
# ---------------------------------------------------------------------
# Fix :
# - le random remplit toutes les cases utiles
# - chaque slot pair 0,2,4,...30 reçoit une note
# - priorité : snare > kick > hat
# - hats remplissent tous les trous
# - length = 2 partout
# - second 32 = miroir strict
# ---------------------------------------------------------------------

V154_SLOTS = list(range(0, 32, 2))
...
def v154_walk(widget):
    out = [widget]
    try:
        for child in widget.winfo_children():
            out.extend(v154_walk(child))
    except Exception:
        pass
    return out


def v154_text(widget):
    try:
        return str(widget.cget("text")).strip()
    except Exception:
        return ""
...
def v154_detected_pairs(self):
    try:
        if hasattr(self, "v151_auto_prepare_roles"):
            self.v151_auto_prepare_roles(force=False, reason="before random KSH full")
    except Exception:
        pass
...
def v154_lane_for_pair(self, pair):
    try:
        return int(self.pair_to_lane.get(int(pair), 0))
    except Exception:
        try:
...
def v154_make_note(self, idx, step, role, pair):
    lane = v154_lane_for_pair(self, pair)
    ...
    return {
        ...
        "learned_role": str(role),
        "prior_label": str(role),
        "main_role_source": "v154_random_ksh_full_slots",
        "ai_generated": False,
        "tracker_generated": True,
        "v154_random_ksh_full": True,
    }


def v154_ascii_line(role_by_step, target_role, char, length=32):
    arr = ["."] * length
    for step, role in role_by_step.items():
        if role == target_role and 0 <= int(step) < length:
            arr[int(step)] = char
    return "|" + "|".join("".join(arr[i:i+8]) for i in range(0, length, 8)) + "|"


def v154_full_line(role_by_step, length=32):
    chars = {"kick": "K", "snare": "S", "hat": "h"}
    arr = ["."] * length
    for step, role in role_by_step.items():
        if 0 <= int(step) < length:
            arr[int(step)] = chars.get(role, "?")
    return "|" + "|".join("".join(arr[i:i+8]) for i in range(0, length, 8)) + "|"


def v154_clean_slots(values):
    out = []
...
def v154_randomize_ksh_full(self, event=None):
    import random
    import time
    ...
    pairs = v154_detected_pairs(self)

    rng = random.Random(time.time())

    kick = v154_clean_slots(rng.choice(V154_KICK_BANK))
    snare = v154_clean_slots(rng.choice(V154_SNARE_BANK))

    # Petites variations propres.
    if rng.random() < 0.55:
        extra_kick = rng.choice([2, 6, 8, 10, 14, 18, 22, 24, 26, 30])
        if extra_kick not in kick:
            kick.append(extra_kick)

    if rng.random() < 0.35:
        extra_snare = rng.choice([6, 10, 14, 18, 22, 26, 30])
        if extra_snare not in snare:
            snare.append(extra_snare)

    kick = sorted(set(v154_clean_slots(kick)))
    snare = sorted(set(v154_clean_slots(snare)))

    # Priorité snare > kick > hat.
    role_by_step = {}

    for step in V154_SLOTS:
        role_by_step[int(step)] = "hat"

    for step in kick:
        if step in role_by_step:
            role_by_step[int(step)] = "kick"

    for step in snare:
        if step in role_by_step:
            role_by_step[int(step)] = "snare"

    # Validation : tous les slots pairs doivent être remplis.
    missing = [step for step in V154_SLOTS if step not in role_by_step]

    if missing:
        for step in missing:
            role_by_step[int(step)] = "hat"

    first32 = [(step, role_by_step[step]) for step in V154_SLOTS]

    all_hits = []

    for step, role in first32:
        all_hits.append((step, role))
    ...
    for note_id, (step, role) in enumerate(all_hits):
        pair = int(pairs.get(role, pairs.get("hat", 0)))
        pattern.append(v154_make_note(self, note_id, step, role, pair))
    ...
    try:
        self.write_latest_pattern(reason="v154_random_ksh_full")
    except Exception:
        pass

    print("")
    print("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
    print("[v154] RANDOM KICK / SNARE / HAT — FULL SLOTS")
    print("[v154] pairs :", pairs)
    print("[v154] FULL  :", v154_full_line(role_by_step))
    print("[v154] KICK  :", v154_ascii_line(role_by_step, "kick", "K"))
    print("[v154] SNARE :", v154_ascii_line(role_by_step, "snare", "S"))
    print("[v154] HAT   :", v154_ascii_line(role_by_step, "hat", "h"))
    print("[v154] règles: 16 slots remplis sur 0-31, length 2, miroir +32")
    print("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
    print("")

    v154_status(self, "v154 : Random K/S/H plein généré.")
    return "break"


def v154_repatch_toolbar(self):
    patched = 0

    for widget in v154_walk(self.root):
        text = v154_text(widget)

        try:
            if text in {"Random K/S/H", "Random K/S/H plein", "Variations", "Syncopé"}:
                widget.configure(text="Random K/S/H plein", command=self.v154_randomize_ksh_full)
                patched += 1
        except Exception:
            pass

    if patched:
        print(f"[v154] bouton Random K/S/H plein patché : {patched}")


_old_v154_build_ui = SliceIndexTracker.build_ui


def v154_build_ui(self):
    _old_v154_build_ui(self)

    try:
        self.root.title("BreakbeatAI v154 — Random K/S/H Full Slots")
    except Exception:
        pass

    try:
        v154_repatch_toolbar(self)
    except Exception as exc:
        print("[v154] patch toolbar impossible :", exc)

    for delay in [250, 700, 1200, 2000, 3200]:
        try:
            self.root.after(delay, lambda: v154_repatch_toolbar(self))
        except Exception:
            pass

    try:
        self.root.bind("<F10>", self.v154_randomize_ksh_full)
    except Exception:
        pass

    v154_status(self, "v154 : Random K/S/H remplit tous les slots.")


SliceIndexTracker.v154_randomize_ksh_full = v154_randomize_ksh_full

# Tous les anciens chemins pointent vers le random plein.
SliceIndexTracker.v153_randomize_ksh = v154_randomize_ksh_full
SliceIndexTracker.v152_generate_variations = v154_randomize_ksh_full
SliceIndexTracker.v151_syncopated = v154_randomize_ksh_full
SliceIndexTracker.v150_generate_ascii_syncopated = v154_randomize_ksh_full
SliceIndexTracker.v149_generate_ascii_syncopated = v154_randomize_ksh_full
SliceIndexTracker.v148_generate_syncopated = v154_randomize_ksh_full

SliceIndexTracker.build_ui = v154_build_ui
