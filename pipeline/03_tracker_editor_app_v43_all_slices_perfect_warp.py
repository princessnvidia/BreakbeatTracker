...
"""
BreakbeatAI Tracker Editor v43 — slice index
...
def warp_audio_to_length_external(audio, target_len, cache_key=None):
    ...
    tmp_dir = OUT_DIR / "_warp_cache_v43"
    ...
    if rubberband:
        try:
            ...
            print(f"[v43] rubberband échec, fallback ffmpeg/no-speed : {exc}")
    ...
    if ffmpeg:
        try:
            ...
            print(f"[v43] ffmpeg atempo échec, fallback no-speed : {exc}")
...
class SliceIndexTracker:
    def __init__(self, root, pair_json):
        ...
        self.step_count = max(32, len(self.pair_values) * HIT_SPACING_STEPS)
        self.left_width = 112
        self.step_width = 36
        self.row_height = 38
        self.case_height = 29
        ...
        self.root.title("BreakbeatAI Tracker Editor v43 — slice index")
    ...
    def detect_external_player(self):
        for name in ["pw-play", "paplay", "aplay", "ffplay"]:
            ...
            if path:
                print(f"[v43] Backend audio : {name} -> {path}")
                return name

        print("[v43] Aucun backend système trouvé, fallback sounddevice.")
        return None
    ...
    def build_initial_pattern(self, randomize=False):
        ...
        total_slots = self.step_count // HIT_SPACING_STEPS
        max_items = min(len(self.pair_values), total_slots)

        print(
            f"[v43] Génération pattern | break={self.project['safe']} | "
        ...
        for slot in range(max_items):
            ...
            print(f"[v43] slot {slot:02d} | step {x_step:02d} -> slice {pair:02d}")
    ...
    def build_ui(self):
        ...
        self.output_label = tk.Label(
            panel,
            text="v43 : tempo par défaut 155 BPM ; clic case = son immédiat ; fermeture = stop audio.",
            bg="#1b1824",
            fg="#77f5b5",
            justify="left",
        )
    ...
    def record_correction(self, event_type, before=None, after=None):
        ...
        event = {
            "version": "breakbeatai_slice_index_correction_v43",
            "time": now_stamp(),
            "safe": safe,
        ...
        corrections_path = CORRECTIONS_DIR / f"{safe}_slice_index_corrections_v43.jsonl"
        ...
        print(f"[v43] correction sauvegardée : {event_type}")
    ...
    def stop_audition(self):
        """
        v43 : stoppe seulement le petit player d'écoute au clic,
    ...
    def play_audition_array(self, audio, label="audition"):
        ...
        audition_wav = OUT_DIR / f"{self.project['safe']}_v43_audition.wav"
        ...
        if self.external_player:
            try:
                ...
                print(f"[v43] Erreur audition {self.external_player} : {exc}")
    ...
    def audition_selected_case(self):
        """
        v43 : clic sur une case = on joue la slice de cette case.
        """
        item = self.selected()
    ...
    def on_close(self):
        """
        v43 : quand tu fermes la fenêtre, on coupe la loop, le playhead
        et le player externe avant de détruire Tk.
        """
        print("[v43] Fermeture : arrêt audio + destruction fenêtre")
    ...
    def play_audio_array(self, audio, label="audio"):
        ...
        audio = normalize(audio)
        live_wav = OUT_DIR / f"{self.project['safe']}_v43_live.wav"
        ...
        if self.external_player:
            try:
                ...
                print(f"[v43] Erreur backend {self.external_player} : {exc}")
    ...
    def warp_slice_audio(self, pair, audio, target_len):
        """
        v43 : cache par slice + durée cible.
        Évite de recalculer rubberband/ffmpeg à chaque lecture.
        """
        pair = int(pair)
        target_len = int(target_len)
        key = (self.project["safe"], pair, target_len, "pitch_preserve_v43")
    ...
    def render_preview_file(self):
        OUT_DIR.mkdir(parents=True, exist_ok=True)
        wav = OUT_DIR / f"{self.project['safe']}_tracker_app_v43_preview.wav"
    ...
    def write_latest_pattern(self, reason="correction"):
        ...
        path = LATEST_DIR / f"{self.project['safe']}_latest_slice_index_v43.json"

        data = {
            "version": "breakbeatai_latest_slice_index_v43",
            "reason": reason,
            ...
            "warp_mode": "pitch_preserve_rubberband_or_ffmpeg_v43",
    ...
    def save(self):
        ...
        tracker_path = OUT_DIR / f"{self.project['safe']}_tracker_app_edit_v43.json"

        data = {
            "version": "tracker_app_edit_v43_slice_index",
        ...
        stamp = file_stamp()
        validated_path = VALIDATED_DIR / f"{self.project['safe']}_validated_slice_index_loop32_v43_{stamp}.json"

        validated = {
            "version": "breakbeatai_validated_slice_index_loop32_v43",
...
# ---------------------------------------------------------------------
# v43 EXTENSION : statut compact, sans casser la v39
# ---------------------------------------------------------------------

def v43_set_status(self, text):
    """
    Statut compact :
...
_old_v43_build_ui = SliceIndexTracker.build_ui


def v43_build_ui(self):
    _old_v43_build_ui(self)

    try:
        self.output_label.config(
            text="v43 : toutes les slices du break | warp pitch-preserve | 155 BPM.",
            width=130,
            height=2,
            wraplength=1180,
            anchor="w",
...
def v43_record_correction(self, event_type, before=None, after=None):
    """
    ...
    print(f"[v43] correction sauvegardée : {event_type}")
    print(f"[v43] correction path : {corrections_path}")
    print(f"[v43] latest path : {latest_path}")


def v43_randomize_pattern(self):
    self.stop_playhead()
    self.stop_audio()
    self.looping = False
...
def v43_render_preview_only(self):
    wav = self.render_preview_file()
    self.set_status(f"Preview rendue : {wav.name}")


def v43_switch_break(self, event=None):
    safe = self.break_var.get()
...
# Monkey patch final.
SliceIndexTracker.set_status = v43_set_status
SliceIndexTracker.build_ui = v43_build_ui
SliceIndexTracker.record_correction = v43_record_correction
SliceIndexTracker.randomize_pattern = v43_randomize_pattern
SliceIndexTracker.render_preview_only = v43_render_preview_only
SliceIndexTracker.switch_break = v43_switch_break
SliceIndexTracker.refresh_breaks = v43_refresh_breaks
