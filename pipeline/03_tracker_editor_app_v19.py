...
"""
03_tracker_editor_app_v19.py

BreakbeatAI Tracker Editor v19
...
Usage :
    ...
    python pipeline/03_tracker_editor_app_v19.py --source "amen"
"""
...
class TrackerEditorApp:
    def __init__(self, root, pair_json):
        ...
        self.root.title("BreakbeatAI Tracker Editor v19 — timeline fixe")
    ...
    def detect_external_player(self):
        for name in ["pw-play", "paplay", "aplay", "ffplay"]:
            ...
            if path:
                print(f"[v19] Backend audio système trouvé : {name} -> {path}")
                return name

        print("[v19] Aucun backend système trouvé, fallback sounddevice.")
        return None
    ...
    def build_ui(self):
        ...
        title = tk.Label(
            main,
            text="BreakbeatAI Tracker Editor v19 — une ligne par sample + timeline fixe",
            bg="#111018",
            fg="#ff7acc",
            font=("Sans", 20, "bold"),
        )
        ...
        subtitle = tk.Label(
            main,
            text=(
                f"Backend audio : {backend} | Changer de sample ne décale plus les autres. "
                "Les samples trop longs sont coupés à la longueur de leur case."
            ),
            bg="#111018",
            fg="#b9acc8",
        )
        ...
        self.output_label = tk.Label(
            panel,
            text=(
                "v19 : rendu timeline fixe. Le x_step garde la position. "
                "Changer de sample ne décale plus le pattern."
            ),
            bg="#1b1824",
            fg="#77f5b5",
            justify="left",
    ...
    def bind_keys(self):
        ...
        print("[v19] Raccourcis : Espace loop | flèches = déplacer | Delete = supprimer")
    ...
    def play_audio_array(self, audio, label="audio"):
        ...
        live_wav = OUT_DIR / f"{self.project['safe']}_v19_live.wav"
        ...
        if self.external_player:
            ...
            try:
                ...
                print(f"[v19] {label} lancé avec {self.external_player} : {live_wav}")
                return duration_ms
            except Exception as exc:
                print(f"[v19] Erreur backend système {self.external_player} : {exc}")

        try:
            import sounddevice as sd
            sd.play(audio, SR)
            self.output_label.config(text=f"{label} lancé avec sounddevice — {duration_ms} ms")
            print(f"[v19] {label} lancé avec sounddevice")
            return duration_ms
        except Exception as exc:
    ...
    def render_preview_file(self):
        OUT_DIR.mkdir(parents=True, exist_ok=True)
        wav = OUT_DIR / f"{self.project['safe']}_tracker_app_v19_preview.wav"
    ...
    def toggle_loop(self):
        if self.looping:
            ...
            print("[v19] Loop arrêtée")
            return

        self.stop_playhead()
        print("[v19] Lancement loop...")
        self.looping = True
    ...
    def save(self):
        ...
        data = {
            "version": "tracker_app_edit_v19_fixed_timeline_one_line_per_sample",
        ...
        path = OUT_DIR / f"{self.project['safe']}_tracker_app_edit_v19.json"
