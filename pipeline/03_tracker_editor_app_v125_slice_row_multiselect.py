...
"""
BreakbeatAI Tracker Editor v125 — slice index
...
def warp_audio_to_length_external(audio, target_len, cache_key=None):
    ...
    if rubberband:
        try:
            ...
            print(f"[v125] rubberband échec, fallback ffmpeg/no-speed : {exc}")
    ...
    if ffmpeg:
        try:
            ...
            print(f"[v125] ffmpeg atempo échec, fallback no-speed : {exc}")
...
class SliceIndexTracker:
    def __init__(self, root, pair_json):
        ...
        self.root.title("BreakbeatAI Tracker Editor v125 — slice index")
    ...
    def detect_external_player(self):
        for name in ["pw-play", "paplay", "aplay", "ffplay"]:
            ...
            if path:
                print(f"[v125] Backend audio : {name} -> {path}")
                return name

        print("[v125] Aucun backend système trouvé, fallback sounddevice.")
        return None
    ...
    def build_initial_pattern(self, randomize=False):
        ...
        print(
            f"[v125] Génération pattern | break={self.project['safe']} | "
        ...
        for slot in range(max_items):
            ...
            print(f"[v125] slot {slot:02d} | step {x_step:02d} -> slice {pair:02d}")
    ...
    def build_ui(self):
        ...
        title = tk.Label(
            main,
            text="BreakbeatAI v125 — slice row multiselect",
            bg="#111018",
            fg="#ff7acc",
            font=("Sans", 20, "bold"),
        )
    ...
    def record_correction(self, event_type, before=None, after=None):
        ...
        print(f"[v125] correction sauvegardée : {event_type}")
    ...
    def play_audition_array(self, audio, label="audition"):
        ...
        audition_wav = OUT_DIR / f"{self.project['safe']}_v125_audition.wav"
        ...
        if self.external_player:
            try:
                ...
                print(f"[v125] Erreur audition {self.external_player} : {exc}")
    ...
    def on_close(self):
        ...
        """
        print("[v125] Fermeture : arrêt audio + destruction fenêtre")
    ...
    def play_audio_array(self, audio, label="audio"):
        ...
        audio = normalize(audio)
        live_wav = OUT_DIR / f"{self.project['safe']}_v125_live.wav"
        ...
        if self.external_player:
            try:
                ...
                print(f"[v125] Erreur backend {self.external_player} : {exc}")
    ...
    def render_preview_file(self):
        OUT_DIR.mkdir(parents=True, exist_ok=True)
        wav = OUT_DIR / f"{self.project['safe']}_tracker_app_v125_preview.wav"
    ...
    def write_latest_pattern(self, reason="correction"):
        ...
        path = LATEST_DIR / f"{self.project['safe']}_latest_slice_index_v125.json"

        data = {
            "version": "breakbeatai_latest_slice_index_v125",
            "reason": reason,
    ...
    def save(self):
        ...
        tracker_path = OUT_DIR / f"{self.project['safe']}_tracker_app_edit_v125.json"
        ...
        stamp = file_stamp()
        validated_path = VALIDATED_DIR / f"{self.project['safe']}_validated_slice_index_loop64_v125_{stamp}.json"

        validated = {
            "version": "breakbeatai_validated_slice_index_loop64_v125",
...
def v44_record_correction(self, event_type, before=None, after=None):
    ...
    print(f"[v125] correction sauvegardée : {event_type}")
    print(f"[v125] correction path : {corrections_path}")
    print(f"[v125] latest path : {latest_path}")
...
def v45_move_selected(self, dx, dy):
    ...
    if dy != 0 and before != after:
        try:
            ...
            print(f"[v125] audition après flèche haut/bas impossible : {exc}")
...
def v46_warp_loop_pitch_preserve(audio, target_len, cache_key):
    ...
    if rubberband:
        try:
            ...
            print(f"[v125] rubberband global warp échec : {exc}")
    ...
    if ffmpeg:
        try:
            ...
            print(f"[v125] ffmpeg global warp échec : {exc}")

    print("[v125] Aucun warp pitch-preserve dispo, fallback fit no-speed.")
    return v46_fit_audio_no_speed(audio, target_len)
...
def v46_build_initial_pattern(self, randomize=False):
    ...
    print(
        f"[v125] M8 pattern | break={self.project['safe']} | "
...
def v46_get_warped_loop(self):
    ...
    print(
        f"[v125] global warp OK | source_len={loop_end-loop_start} | "
...
def v46_build_ui(self):
    ...
    try:
        ...
        print(f"[v125] UI extension impossible : {exc}")
...
def v47_play_audio_array(self, audio, label="audio"):
    ...
    if len(audio) == 0 or v47_is_silent(audio):
        ...
        print(f"[v125] lecture annulée : audio silencieux ({label})")
        return 0
...
def v47_toggle_loop(self):
    ...
    if not getattr(self, "pattern", None):
        ...
        print("[v125] loop annulée : pattern vide")
        return "break"
...
def v47_delete_selected(self):
    ...
    if not getattr(self, "pattern", None):
        ...
        print("[v125] pattern vide après suppression : audio coupé")
...
def v48_build_initial_pattern(self, randomize=False):
    ...
    print("[v125] pattern initial forcé VIDE : aucun son ne doit jouer.")
    return []
...
def v48_load_first_slices(self):
    ...
    print(f"[v125] Load 16 slices : {len(self.pattern)} items")


def v48_debug_pattern(self, label="render"):
    print("")
    print(f"[v125] DEBUG PATTERN avant {label}")
    print(f"[v125] items = {len(getattr(self, 'pattern', []) or [])}")

    if not getattr(self, "pattern", None):
        print("[v125] pattern vide confirmé")
        print("")
        return
    ...
    for item in self.pattern:
        ...
        print(
            f"[v125] id={item_id} | step {x:02d}/{end:02d} | "
...
def v48_render_audio_with_timeline(self):
    ...
    if not getattr(self, "pattern", None):
        loop_samples = self.get_loop_samples()
        print(f"[v125] rendu silence total : pattern vide | samples={loop_samples}")
        return np.zeros(loop_samples, dtype=np.float32), []
    ...
    print(f"[v125] rendu audio peak={peak:.8f} | timeline_events={len(timeline) if timeline is not None else 'None'}")

    if timeline:
        print("[v125] timeline events:")
        for ev in timeline:
            print(f"[v125]   {ev}")
...
def v48_toggle_loop(self):
    if not getattr(self, "pattern", None):
        ...
        print("[v125] Space ignoré : pattern vide.")
        return "break"
...
def v48_build_ui(self):
    ...
    try:
        ...
        print(f"[v125] ajout UI impossible : {exc}")
...
def v49_add_item_at_xy(self, x, y):
    ...
    try:
        ...
        print(f"[v125] record_correction add impossible : {exc}")
    ...
    try:
        ...
        print(f"[v125] audition après ajout impossible : {exc}")

    print(f"[v125] add | id={item['id']} | step={x_step} | pair={pair} | lane={lane}")
...
def v49_duplicate_selected_forward(self, event=None):
    ...
    if item is None:
        ...
        print("[v125] duplicate annulé : aucune note sélectionnée")
        return "break"
    ...
    if new_x > max_x:
        ...
        print("[v125] duplicate annulé : fin de grille")
        return "break"
    ...
    try:
        ...
        print(f"[v125] record_correction duplicate impossible : {exc}")
    ...
    try:
        ...
        print(f"[v125] audition après duplicate impossible : {exc}")

    print(
        f"[v125] duplicate | old_id={item.get('id')} | new_id={new_item.get('id')} | "
...
def v49_build_ui(self):
    ...
    try:
        ...
        print(f"[v125] bind canvas click impossible : {exc}")
    ...
    try:
        ...
        print(f"[v125] bind keys impossible : {exc}")
...
def v50_load_style_model():
    ...
    try:
        ...
        print(f"[v125] impossible de charger le modèle IA : {exc}")
        return None
...
def v50_generate_ai_pattern(self, event=None):
    ...
    if model is None:
        ...
        print("[v125] modèle absent :", STYLE_MODEL_PATH)
        return "break"
    ...
    print("")
    print(f"[v125] GENERATE IA | safe={safe} | pairs={pair_values} | slots={max_slots}")
    ...
    for slot in range(max_slots):
        ...
        print(
            f"[v125] slot {slot:02d} | step {x_step:02d} | "
    ...
    try:
        ...
        print(f"[v125] write_latest_pattern impossible : {exc}")
...
def v50_build_ui(self):
    ...
    try:
        ...
        print(f"[v125] UI IA impossible : {exc}")
...
def v51_mouse_down(self, event):
    ...
    if item is not None:
        ...
        print(
            f"[v125] mouse down note | id={item.get('id')} | "
            f"step={item.get('x_step')} | pair={item.get('pair')}"
        )
    else:
        print("[v125] mouse down empty")
...
def v51_mouse_up(self, event):
    ...
    if state.get("empty"):
        ...
        print("[v125] drag depuis vide ignoré")
        return "break"
    ...
    if before != after:
        try:
            ...
            print(f"[v125] record_correction drag impossible : {exc}")
        ...
        print(
            f"[v125] drag move | id={after.get('id')} | "
            f"step {before.get('x_step')} -> {after.get('x_step')} | "
        ...
        try:
            ...
            print(f"[v125] audition après drag impossible : {exc}")
...
def v51_build_ui(self):
    ...
    try:
        ...
        print(f"[v125] bind souris impossible : {exc}")
    ...
    try:
        ...
        print(f"[v125] bind Suppr impossible : {exc}")
...
def v52_normalize_loaded_notes(self, notes):
    ...
    for raw in sorted(notes, key=lambda n: (int(float(n.get("x_step", 0))), int(float(n.get("pair", 0))))):
        ...
        if pair not in valid_pairs:
            print(f"[v125] slice ignorée car absente du break actuel : pair={pair}")
            continue
...
def v52_load_saved_beat(self, event=None):
    ...
    if not saves:
        ...
        print("[v125] aucune sauvegarde trouvée")
        return "break"
    ...
    if not pattern:
        ...
        print("[v125] sauvegarde incompatible :", chosen["path"])
        return "break"
    ...
    print("")
    print("[v125] BEAT RESTAURÉ")
    print("[v125] fichier :", chosen["path"])
    print("[v125] clé :", chosen["key"])
    print("[v125] notes :", len(pattern))
    print("")
...
def v52_list_saved_beats(self, event=None):
    ...
    print("")
    print("[v125] SAUVEGARDES TROUVÉES")
    for i, save in enumerate(saves[:30]):
        print(
            f"[v125] {i:02d} | notes={len(save['notes']):02d} | "
...
def v52_build_ui(self):
    ...
    try:
        ...
        print(f"[v125] UI load saved impossible : {exc}")
...
def v53_run_training(self):
    ...
    if not trainer.exists():
        ...
        print("[v125] trainer introuvable :", trainer)
        return False

    print("")
    print("[v125] AUTO-TRAIN IA après Save...")
    print("")
    ...
    try:
        ...
        if result.returncode != 0:
            ...
            print("[v125] auto-train échec code :", result.returncode)
            return False

        v53_status(self, "Save OK + IA entraînée automatiquement.")
        print("[v125] AUTO-TRAIN OK")
        return True

    except Exception as exc:
        v53_status(self, f"Save OK, mais auto-train impossible : {exc}")
        print("[v125] exception auto-train :", exc)
        return False
...
def v53_save(self):
    ...
    try:
        ...
        print(f"[v125] auto-train après save impossible : {exc}")
...
def v53_build_ui(self):
    ...
    try:
        ...
        print(f"[v125] UI auto-train impossible : {exc}")
...
def v55_mouse_up(self, event):
    ...
    if not state.get("moved"):
        ...
        try:
            ...
            print(f"[v125] audition clic note impossible : {exc}")
        ...
        print(
            f"[v125] click play/select | id={item.get('id')} | "
            f"step={item.get('x_step')} | pair={item.get('pair')}"
        )
...
def v58_add_item_at_xy(self, x, y):
    ...
    print(
        f"[v125] add | id={item['id']} | step={x_step} | "
...
def v58_toggle_selected_length(self, event=None):
    ...
    try:
        ...
        print(f"[v125] record_correction length impossible : {exc}")

    v58_refresh_after_edit(self, "toggle_note_length")

    v58_status(self, f"Longueur note : {old_len} -> {new_len} case(s).")
    print(f"[v125] length toggle | id={item.get('id')} | {old_len} -> {new_len}")
...
def v58_duplicate_selected_forward(self, event=None):
    ...
    try:
        ...
        print(f"[v125] audition duplicate impossible : {exc}")
...
def v58_build_ui(self):
    ...
    try:
        ...
        print(f"[v125] UI demi-grille impossible : {exc}")
    ...
    try:
        ...
        print(f"[v125] bind demi-grille impossible : {exc}")
...
def v59_rank_current_slices(self):
    ...
    for pair in self.pair_values:
        ...
        try:
            ...
            print(f"[v125] analyse pair {pair} impossible : {exc}")
            role = "other"
    ...
    print("")
    print("[v125] TOP ROLES CURRENT BREAK")
    for r in ("kick", "snare", "hat"):
        print(f"[v125] {r:5s}:", [(x["pair"], round(x["score"], 3)) for x in ranks[r][:6]])
    print("")
...
def v59_load_role_model():
    ...
    try:
        ...
        print(f"[v125] lecture modèle rôle impossible : {exc}")
        return None
...
def v59_generate_role_aware(self, event=None):
    ...
    if model is None:
        ...
        print("[v125] modèle rôle absent, fallback musical snare 6.")
        model = {
            "breaks": {},
            "global": {},
            "patterns_used": 0,
    ...
    print("")
    print("[v125] ROLE-AWARE GENERATION")
    print("[v125] model patterns_used:", model.get("patterns_used", 0))
    for item in pattern:
        print(
            f"[v125] step {item['x_step']:02d}/{item['x_step'] + item['length'] - 1:02d} | "
...
def v59_run_role_training(self, event=None):
    ...
    try:
        ...
        print(f"[v125] training rôle impossible : {exc}")
...
def v59_save(self):
    ...
    try:
        ...
        print(f"[v125] auto-training rôle après save impossible : {exc}")
...
def v59_build_ui(self):
    ...
    try:
        ...
        print(f"[v125] UI role-aware impossible : {exc}")
...
def v62_add_item_at_xy(self, x, y):
    ...
    print(f"[v125] add | id={item['id']} | step={x_step} | len={length} | pair={pair}")
...
def v62_mouse_down(self, event):
    ...
    if item is not None:
        ...
        print(
            f"[v125] mouse down {state['mode']} | id={item.get('id')} | "
            f"step={item.get('x_step')} | len={item.get('length')} | pair={item.get('pair')}"
        )
    else:
        print("[v125] mouse down empty")
    ...
    if mode == "resize":
        if not state.get("moved"):
            ...
            try:
                ...
                print(f"[v125] audition clic resize impossible : {exc}")
        ...
        if before != after:
            try:
                ...
                print(f"[v125] record_correction resize impossible : {exc}")
            ...
            try:
                ...
                print(f"[v125] audition après resize impossible : {exc}")
    ...
    if mode == "move":
        ...
        if before != after:
            try:
                ...
                print(f"[v125] record_correction move impossible : {exc}")
            ...
            try:
                ...
                print(f"[v125] audition après move impossible : {exc}")
...
def v62_toggle_selected_length(self, event=None):
    ...
    try:
        ...
        print(f"[v125] record_correction toggle length impossible : {exc}")

    v62_refresh_after_edit(self, "toggle_note_length_v62")
    v62_status(self, f"Longueur note : {old_len} → {new_len} case(s).")

    print(f"[v125] toggle length | id={item.get('id')} | {old_len} -> {new_len}")
...
def v62_draw(self):
    ...
    try:
        ...
        print(f"[v125] draw handles impossible : {exc}")
...
def v62_build_ui(self):
    ...
    try:
        ...
        print(f"[v125] bind resize impossible : {exc}")
    ...
    try:
        ...
        print(f"[v125] UI resize impossible : {exc}")
...
def v63_load_model():
    ...
    try:
        ...
        print(f"[v125] lecture modèle strict impossible : {exc}")
        return None
...
def v63_generate_safe_role_ai(self, event=None):
    ...
    print("")
    print("[v125] SAFE ROLE AI GENERATED")
    print("[v125] model strict:", "yes" if model else "no")
    print("[v125] snare main/alt:", main_snare, alt_snare)
    print("[v125] kick  main/alt:", main_kick, alt_kick)
    print("[v125] hat   main/alt:", main_hat, alt_hat)
    print("[v125] pattern:")
    for item in pattern:
        print(
            f"[v125] step {item['x_step']:02d}/{item['x_step'] + item['length'] - 1:02d} | "
...
def v63_run_strict_training(self, event=None):
    ...
    print("")
    print("[v125] TRAIN STRICT ROLE MODEL...")
    print("")
    ...
    try:
        ...
        print(f"[v125] training strict impossible : {exc}")
...
def v63_build_ui(self):
    ...
    try:
        ...
        print(f"[v125] UI safe impossible : {exc}")
...
def v65_rank_slices(self):
    ...
    for pair in self.pair_values:
        ...
        try:
            ...
            print(f"[v125] analyse slice {pair} impossible : {exc}")
            scores = {
                "kick": 0.0,
                "snare": 0.0,
                "hat": 0.0,
    ...
    print("")
    print("[v125] TOP SLICES PAR ROLE — break courant")
    for role in ("kick", "snare", "hat", "ghost_snare"):
        print(f"[v125] {role:12s}:", [(x["pair"], round(x["score"], 3)) for x in ranks[role][:8]])
    print("")
...
def v65_generate_from_role_template(self, event=None):
    ...
    try:
        ...
        print(f"[v125] template introuvable : {exc}")
        return "break"
    ...
    print("")
    print("[v125] CROSS-SONG ROLE TEMPLATE GENERATED")
    print("[v125] template:", V65_TEMPLATE_PATH)
    print("[v125] temperature:", temperature, "| hats:", hat_density, "| ghosts:", ghost_density)
    print("[v125] pattern:")
    for item in pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v125] step {item['x_step']:02d}/{end:02d} | "
...
def v65_build_ui(self):
    ...
    try:
        ...
        print(f"[v125] UI impossible : {exc}")
...
def v67_train_strict_if_available(self):
    ...
    if trainer is None:
        print("[v125] aucun trainer trouvé.")
        return False

    print("")
    print("[v125] TRAIN après validation :", trainer)
    print("")
    ...
    try:
        ...
        print(f"[v125] training impossible : {exc}")
        return False
...
def v67_generate_candidate(self, event=None):
    ...
    print("")
    print("[v125] CANDIDATE GENERATED")
    print("[v125] break:", self.v67_candidate_safe)
    print("[v125] notes:", len(candidate))
    print("[v125] Maintenant : corrige puis clique Good après modifs, ou Reject.")
    print("")
...
def v67_reject_candidate(self, event=None):
    ...
    print("")
    print("[v125] REJECTED")
    print("[v125] saved:", rejected_path)
    print("")
...
def v67_accept_corrected(self, event=None):
    ...
    print("")
    print("[v125] ACCEPTED GOOD AFTER EDIT")
    print("[v125] saved:", validated_path)
    print("[v125] trained:", trained)
    print("")
...
def v67_accept_without_training(self, event=None):
    ...
    print("[v125] accepted no-train:", validated_path)
...
def v67_build_ui(self):
    ...
    try:
        ...
        print(f"[v125] UI feedback impossible : {exc}")
...
def v68_generate_candidate(self, event=None):
    ...
    print("[v125] Length policy appliquée : 2 cases par défaut, 1 case rare.")
...
def v69_force_pattern_hard_2_grid(self, reason="v69_force_hard_2_grid"):
    ...
    print("")
    print("[v125] HARD 2 GRID APPLIQUÉ")
    for item in self.pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v125] step {item['x_step']:02d}/{end:02d} | "
...
def v70_duplicate_selected_forward(self, event=None):
    ...
    print("")
    print("[v125] CTRL+D DUPLICATE +2")
    print(f"[v125] old_id={item.get('id')} new_id={new_item.get('id')}")
    print(f"[v125] step {old_x}/{old_x + length - 1} -> {new_x}/{new_x + length - 1}")
    print(f"[v125] pair={new_item.get('pair')} len={length}")
    print("")
...
def v70_build_ui(self):
    ...
    try:
        ...
        print(f"[v125] bind Ctrl+D impossible : {exc}")
...
def v71_rank_slices(self):
    ...
    print("")
    print("[v125] PRE-LEARNING TOP SLICES")
    for role in ("kick", "snare", "hat", "ghost_snare"):
        print(f"[v125] {role:12s}:", [(x["pair"], round(x["score"], 3)) for x in ranks[role][:8]])
    print("")
...
def v71_prelearn_roles(self, force=False):
    ...
    print("")
    print("[v125] PRELEARN ROLES POUR", safe)
    for role in ("kick", "snare", "hat", "ghost_snare", "bad"):
        print(f"[v125] {role:12s}:", roles.get(role, []))
    print("")
...
def v71_generate_full_candidate(self, event=None):
    ...
    print("")
    print("[v125] FULL CANDIDATE GENERATED")
    print("[v125] break:", v71_safe(self))
    print("[v125] roles:", roles)
    print("[v125] pattern:")
    for item in pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
...
def v72_open_break(self, safe=None):
    ...
    print("")
    print("[v125] OPEN BREAK")
    print("[v125] current:", current)
    print("[v125] next   :", safe)
    print("[v125] cmd    :", " ".join(cmd))
    print("")
...
def v72_build_ui(self):
    ...
    try:
        ...
        print(f"[v125] UI break switcher impossible : {exc}")
...
def v73_sanitize_tracker_pattern(self, reason="v73_tracker_strict_no_overlap"):
    ...
    print("")
    print("[v125] TRACKER STRICT NO OVERLAP")
    print("[v125] kept:", len(accepted), "| rejected overlaps:", len(rejected))

    for item in accepted:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v125] KEEP step {item['x_step']:02d}/{end:02d} | "
    ...
    if rejected:
        ...
        for item in rejected:
            ...
            print(
                f"[v125] DROP step {item['x_step']:02d}/{end:02d} | "
                f"role={item.get('v73_role_key')} | pair={item.get('pair')}"
            )
...
def v81_launch_break(self, source):
    ...
    print("")
    print("[v125] CHARGER BREAK")
    print("[v125] courant :", current)
    print("[v125] nouveau :", source)
    print("[v125] cmd     :", " ".join(cmd))
    print("")
...
def v81_patch_charger_break_buttons(self):
    ...
    print(f"[v125] boutons Charger break patchés : {patched}")
...
def v81_generate_candidate(self, event=None):
    ...
    print("")
    print("[v125] GENERATE CANDIDATE")
    print("[v125] break chargé :", current)
    print("[v125] générateur   :", getattr(fn, "__name__", None))
    print("")
...
def v81_run_training(self):
    ...
    if script is None:
        print("[v125] Aucun script training trouvé.")
...
def v81_save_feedback(self, verdict, train_now=False):
    ...
    print("")
    print("[v125] FEEDBACK SAUVÉ")
    print("[v125] break  :", current)
    print("[v125] verdict:", verdict)
    print("[v125] train  :", train_now)
    print("[v125] fichier:", out_path)
    print("")
...
def v81_build_ui(self):
    ...
    try:
        self.root.title(f"BreakbeatAI v125 — slice row multiselect")
    except Exception:
        pass
    ...
    try:
        ...
        print(f"[v125] patch Charger break impossible : {exc}")

    try:
        v81_install_training_toolbar(self)
    except Exception as exc:
        print(f"[v125] toolbar impossible : {exc}")
...
def v82_auto_role_pools(self):
    ...
    print("")
    print("[v125] ROLE POOLS")
    for r in ["kick", "snare", "hat", "ghost_snare", "bad"]:
        print(f"[v125] {r:12s}:", roles.get(r, []))
    print("")
...
def v82_generate_candidate(self, event=None):
    ...
    print("")
    print("[v125] NEW CANDIDATE")
    print("[v125] break :", current)
    print("[v125] nonce :", nonce)
    print("[v125] rejected known:", len(rejected))
    print("[v125] fingerprint rejeté ?: ", chosen_fp in rejected)
    for item in self.pattern:
        end = int(item["x_step"]) + int(item["length"]) - 1
        print(
            f"[v125] step {item['x_step']:02d}/{end:02d} "
...
def v82_patch_training_buttons(self):
    ...
    print(f"[v125] boutons training patchés : {patched}")
...
def v82_build_ui(self):
    ...
    try:
        ...
        print(f"[v125] patch boutons impossible : {exc}")
...
def v110_generate(self, event=None, reject_mode=False):
    ...
    print("")
    if reject_mode:
        print("[v125] REJECT/BAD -> variation premier 32 + autres lignes")
    else:
        print("[v125] GENERATE 64")
    print("[v125] break:", current)
    print("[v125] first32:", first32_id)
    print("[v125] response:", response_id)
    print("[v125] role_offset:", role_offset)
    for role in ["kick", "snare", "hat"]:
        lane = "?"
        try:
            ...
            pass
        print(f"[v125] {role:6s}: pair={chosen[role]['pair']} lane={lane} source={chosen[role]['source']}")
    print("")
    ...
    try:
        ...
        print(f"[v125] draw impossible : {exc}")
    ...
    try:
        ...
        print(f"[v125] refresh_panel impossible : {exc}")
...
def v110_render_pattern_to_audio(self):
    ...
    for item in pattern:
        ...
        if pair not in slice_map:
            skipped += 1
            print(f"[v125] skip pair introuvable : {pair}")
            continue
...
def v110_export_wav_current(self, event=None):
    try:
        ...
        print("")
        print("[v125] EXPORT WAV OK")
        print("[v125] break:", current)
        print("[v125] wav  :", out_path)
        print("[v125] json :", info_path)
        print("[v125] duration:", round(info["duration_seconds"], 3), "s")
        print("[v125] rendered:", info["rendered_notes"], "skipped:", info["skipped_notes"])
        print("")
...
def v110_space_play(self, event=None):
    ...
    try:
        ...
        if btn is not None:
            print("[v125] SPACE -> Loop / Space")
            try:
                btn.invoke()
            except Exception as exc:
                print("[v125] invoke Loop / Space impossible :", exc)
            return "break"

        print("[v125] SPACE : bouton Loop / Space introuvable.")
...
def v110_patch_buttons(self):
    ...
    print(f"[v125] boutons patchés : Generate={gen_patched}, Reject/Bad={bad_patched}")
...
def v110_build_ui(self):
    ...
    try:
        ...
        print("[v125] patch boutons impossible :", exc)

    try:
        v110_add_export_button(self)
    except Exception as exc:
        print("[v125] ajout Export WAV impossible :", exc)

    try:
        v110_install_space_bindings(self)
    except Exception as exc:
        print("[v125] bind espace impossible :", exc)
...
def v113_export(self, event=None):
    ...
    print("[v125] ERREUR : export_wav_current_v110 introuvable.")
...
def v113_place_export_button(self):
    ...
    try:
        ...
        print("[v125] bouton Export WAV visible ajouté.")
        v113_status(self, "v113 : Export WAV visible. Espace reste Loop / Space.")

    except Exception as exc:
        print("[v125] placement bouton Export WAV impossible :", exc)
        try:
            btn.place(x=760, y=32, height=30)
            btn.lift()
            self.export_button_v113 = btn
...
def v115_apply_cells_minus_25(self):
    ...
    print("")
    print("[v125] APPLICATION CASES -3.5px")
    ...
    for name in V115_WIDTH_GLOBAL_NAMES:
        try:
            ...
            if isinstance(value, (int, float)) and value > 6:
                ...
                print(f"[v125] global {name}: {value} -> {globals()[name]}")
        except Exception:
            pass
        ...
        try:
            if hasattr(self, attr):
                ...
                if isinstance(value, (int, float)) and value > 6:
                    ...
                    print(f"[v125] attr {attr}: {value} -> {getattr(self, attr)}")
        except Exception:
            pass
...
def v115_build_ui(self):
    ...
    try:
        self.root.title("BreakbeatAI v125 — slice row multiselect")
    except Exception:
        pass
    ...
    try:
        ...
        print("[v125] impossible d'appliquer -3.5px :", exc)
...
def v117_render_pattern_to_audio(self):
    ...
    print("")
    print("[v125] EXPORT EXACT CELLS")
    print("[v125] break:", current)
    print("[v125] pairblock:", pairblock_file)
    print("[v125] sr:", sr)
    print("[v125] median_pair_samples:", median_pair_samples)
    print("[v125] case_samples:", case_samples)
    print("[v125] export_samples:", total_samples)
    print("[v125] export_seconds:", round(total_samples / float(sr), 6))
    print("")
...
def v117_export_wav_current(self, event=None):
    try:
        ...
        print("[v125] wav  :", out_path)
        print("[v125] json :", info_path)
        print("[v125] duration:", round(info["duration_seconds"], 6), "s")
        print("[v125] case_seconds:", round(info["case_seconds"], 6))
        print("[v125] rendered:", info["rendered_notes"], "skipped:", info["skipped_notes"])
        print("")
...
def v118_current_break(self):
    ...
    if candidates:
        ...
        print("[v125] break courant:", stem, "| source:", source, "| raw:", raw)
        return stem

    print("[v125] WARNING : break courant introuvable, fallback Camo.")
    return "Camo_Break_-_3A"
...
def v118_find_best_playback_wav(since):
    ...
    print("")
    print("[v125] WAV candidats playback :")
    for score, mtime, size, wav in candidates[:12]:
        age = time.time() - mtime
        print(f"[v125] score={score:8d} age={age:7.2f}s size={size:10d} path={wav}")
    print("")
...
def v118_copy_playback_wav(self, since):
    ...
    if best is None:
        ...
        print("[v125] aucun WAV playback trouvé.")
        return "break"
    ...
    print("")
    print("[v125] EXPORT PLAYBACK OK")
    print("[v125] source :", best)
    print("[v125] export :", out)
    print("[v125] json   :", info_path)
    print("")
...
def v118_export_playback_wav(self, event=None):
    ...
    try:
        ...
        if btn is not None:
            print("[v125] déclenche Loop / Space pour générer le playback WAV...")
            btn.invoke()
        else:
            print("[v125] bouton Loop / Space introuvable, copie du dernier WAV playback existant.")
    except Exception as exc:
        print("[v125] invoke Loop / Space impossible :", exc)
...
def v121_find_best_long_wav(since):
    ...
    print("")
    print("[v125] WAV candidats longs/propre à raccourcir :")
    for row in candidates[:20]:
        age = time.time() - row["mtime"]
        print(
            f"[v125] score={row['score']:7d} "
...
def v121_export_trimmed_long(self, event=None):
    ...
    try:
        ...
        if btn is not None:
            print("[v125] déclenche Loop / Space pour générer le WAV long/propre...")
            btn.invoke()
    except Exception as exc:
        print("[v125] Loop / Space impossible :", exc)
    ...
    def do_export():
        try:
            ...
            try:
                ...
                print("[v125] ffmpeg trim impossible, fallback wave :", exc)
                ok = False
            ...
            print("")
            print("[v125] EXPORT TRIM OK")
            print("[v125] source :", src_wav)
            print("[v125] source durée :", round(src_dur, 6), "s")
            print("[v125] target durée :", round(target_seconds, 6), "s")
            print("[v125] export :", out)
            print("[v125] export durée :", round(float(out_dur or 0), 6), "s")
            print("[v125] json :", info_path)
            print("")
...
def v124_export_trimmed_long_no_json(self, event=None):
    ...
    def do_export():
        try:
            ...
            print("")
            print("[v125] EXPORT WAV ONLY OK")
            print("[v125] source :", src_wav)
            print("[v125] source durée :", round(src_dur, 6), "s")
            print("[v125] target durée :", round(target_seconds, 6), "s")
            print("[v125] export :", out)
            print("[v125] export durée :", round(float(out_dur or 0), 6), "s")
            print("[v125] aucun JSON exporté")
            print("")
...
def v125_select_lane(self, lane):
    ...
    print("")
    print(f"[v125] Slice {lane + 1} sélectionnée")
    print(f"[v125] notes sélectionnées : {ids}")
    print("")
...
def v125_redraw_after_move(self):
    try:
        ...
        print("[v125] draw impossible :", exc)
...
def v125_build_ui(self):
    ...
    if canvas is not None:
        try:
            ...
            print("[v125] bind canvas click impossible :", exc)
    ...
    try:
        ...
        print("[v125] bind flèches impossible :", exc)
