...
"""
BreakbeatAI Tracker Editor v40 — slice index
...
class SliceIndexTracker:
    def __init__(self, root, pair_json):
        ...
        self.root.title("BreakbeatAI Tracker Editor v40 — slice index")
    ...
    def detect_external_player(self):
        for name in ["pw-play", "paplay", "aplay", "ffplay"]:
            ...
            if path:
                print(f"[v40] Backend audio : {name} -> {path}")
                return name

        print("[v40] Aucun backend système trouvé, fallback sounddevice.")
        return None
    ...
    def build_initial_pattern(self, randomize=False):
        ...
        for slot in range(HIT_SLOTS):
            ...
            print(f"[v40] slot {slot:02d} | step {x_step:02d} -> slice {pair:02d}")
    ...
    def build_ui(self):
        ...
        tk.Button(panel, text="Play slice", command=self.play_selected_pair, bg="#30283f", fg="#f5eefe").grid(row=1, column=7, padx=4)
        ...
        self.output_label = tk.Label(
            panel,
            text="v40 : tempo par défaut 155 BPM ; clic case = son immédiat ; fermeture = stop audio.",
            bg="#1b1824",
            fg="#77f5b5",
            justify="left",
        )
    ...
    def record_correction(self, event_type, before=None, after=None):
        ...
        event = {
            "version": "breakbeatai_slice_index_correction_v40",
            "time": now_stamp(),
            "safe": safe,
        ...
        corrections_path = CORRECTIONS_DIR / f"{safe}_slice_index_corrections_v40.jsonl"
        ...
        print(f"[v40] correction sauvegardée : {event_type}")
    ...
    def stop_audition(self):
        """
        v40 : stoppe seulement le petit player d'écoute au clic,
    ...
    def play_audition_array(self, audio, label="audition"):
        ...
        audition_wav = OUT_DIR / f"{self.project['safe']}_v40_audition.wav"
        ...
        if self.external_player:
            try:
                ...
                print(f"[v40] Erreur audition {self.external_player} : {exc}")
    ...
    def audition_selected_case(self):
        """
        v40 : clic sur une case = on joue la slice de cette case.
        """
        item = self.selected()
    ...
    def on_close(self):
        """
        v40 : quand tu fermes la fenêtre, on coupe la loop, le playhead
        et le player externe avant de détruire Tk.
        """
        print("[v40] Fermeture : arrêt audio + destruction fenêtre")
    ...
    def play_audio_array(self, audio, label="audio"):
        ...
        audio = normalize(audio)
        live_wav = OUT_DIR / f"{self.project['safe']}_v40_live.wav"
        ...
        if self.external_player:
            try:
                ...
                print(f"[v40] Erreur backend {self.external_player} : {exc}")
    ...
    def render_preview_file(self):
        OUT_DIR.mkdir(parents=True, exist_ok=True)
        wav = OUT_DIR / f"{self.project['safe']}_tracker_app_v40_preview.wav"
    ...
    def write_latest_pattern(self, reason="correction"):
        ...
        path = LATEST_DIR / f"{self.project['safe']}_latest_slice_index_v40.json"

        data = {
            "version": "breakbeatai_latest_slice_index_v40",
            "reason": reason,
    ...
    def save(self):
        ...
        tracker_path = OUT_DIR / f"{self.project['safe']}_tracker_app_edit_v40.json"

        data = {
            "version": "tracker_app_edit_v40_slice_index",
        ...
        stamp = file_stamp()
        validated_path = VALIDATED_DIR / f"{self.project['safe']}_validated_slice_index_loop32_v40_{stamp}.json"

        validated = {
            "version": "breakbeatai_validated_slice_index_loop32_v40",
...
# ---------------------------------------------------------------------
# v40 EXTENSION : statut compact, sans casser la v39
# ---------------------------------------------------------------------

def v40_set_status(self, text):
    """
    Statut compact :
...
_old_v40_build_ui = SliceIndexTracker.build_ui


def v40_build_ui(self):
    _old_v40_build_ui(self)

    try:
        self.output_label.config(
            text="v40 : tempo 155 BPM | clic case = play | auto-learn compact.",
            width=130,
            height=2,
            wraplength=1180,
            anchor="w",
...
def v40_record_correction(self, event_type, before=None, after=None):
    """
    ...
    print(f"[v40] correction sauvegardée : {event_type}")
    print(f"[v40] correction path : {corrections_path}")
    print(f"[v40] latest path : {latest_path}")


def v40_randomize_pattern(self):
    self.stop_playhead()
    self.stop_audio()
    self.looping = False
...
def v40_render_preview_only(self):
    wav = self.render_preview_file()
    self.set_status(f"Preview rendue : {wav.name}")


def v40_switch_break(self, event=None):
    safe = self.break_var.get()
...
# Monkey patch final.
SliceIndexTracker.set_status = v40_set_status
SliceIndexTracker.build_ui = v40_build_ui
SliceIndexTracker.record_correction = v40_record_correction
SliceIndexTracker.randomize_pattern = v40_randomize_pattern
SliceIndexTracker.render_preview_only = v40_render_preview_only
SliceIndexTracker.switch_break = v40_switch_break
SliceIndexTracker.refresh_breaks = v40_refresh_breaks
