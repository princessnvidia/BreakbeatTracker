...
Puis il écrit :
    dataset/learning/beat_style_model_v01.json
...
def counter_to_dict(counter):
    return {str(k): float(v) for k, v in sorted(counter.items(), key=lambda kv: int(kv[0]))}
