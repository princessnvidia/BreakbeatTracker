...
"""
BreakbeatAI Tracker Editor v24
...
def load_wav(path):
    ...
    # v24 : pas de normalisation/fade par sample.
...
class TrackerEditorApp:
    def __init__(self, root, pair_json):
        ...
        # v24 : grille 32 positions = 4 cycles de break8.
        ...
        self.root.title("BreakbeatAI Tracker Editor v24")
    ...
    def guess_step_ms(self):
        # v24 : une colonne = une position break8.
    ...
    def detect_external_player(self):
        for name in ["pw-play", "paplay", "aplay", "ffplay"]:
            ...
            if path:
                print(f"[v24] Backend audio système trouvé : {name} -> {path}")
                return name

        print("[v24] Aucun backend système trouvé, fallback sounddevice.")
        return None
    ...
    def role_pair_pool(self, role):
        ...
        for pair in self.pair_values:
            b = self.block_by_pair.get(int(pair), {})
            if clean_role(b.get("role", "unknown")) == role:
    ...
    def choose_pair_for_role_variation(self, role, pos):
        """
        v24 : choix déterministe mais varié.
        Tu peux corriger à la main ensuite, puis sauvegarder comme validation IA.
        """
        role = clean_role(role)
        pool = self.role_pair_pool(role)

        if not pool:
            pool = [int(p) for p in self.pair_values]
        ...
        # Variations simples :
        # - cycle 0 : choix standard
        # - cycle 1 : hats alternés
        # - cycle 2 : kick/snare légèrement alternés
        # - cycle 3 : plus de variation sur hats
        idx = 0

        if role == "hat":
            idx = (local + bar * 2 + (1 if pos in (11, 15, 27, 31) else 0)) % len(pool)

        elif role == "kick":
            idx = (bar + (1 if pos in (13, 29) else 0)) % len(pool)

        elif role == "snare":
            idx = (bar + (1 if pos in (22, 30) else 0)) % len(pool)
    ...
    def build_initial_pattern(self):
        ...
        v24 :
        - 32 positions
        - rôles formels répétés : K H S H H K S H
        - variations de samples sur 4 cycles
        - vertical = sample/pair choisi
        - horizontal = position 0..31
        """
        pattern = []

        for pos in range(32):
            role = BREAK8_ROLES[pos % 8]
            pair = self.choose_pair_for_role_variation(role, pos)
    ...
    def build_ui(self):
        ...
        title = tk.Label(
            main,
            text="BreakbeatAI Tracker Editor v24 — loops 32 avec variations",
            bg="#111018",
            fg="#ff7acc",
            font=("Sans", 20, "bold"),
        )
        ...
        self.output_label = tk.Label(
            panel,
            text="v24 : loop 32 avec variations. Modifie puis Save validation pour améliorer l’IA.",
            bg="#1b1824",
            fg="#77f5b5",
            justify="left",
        )
    ...
    def bind_keys(self):
        ...
        print("[v24] Raccourcis : Espace loop | flèches = déplacer | Delete = supprimer")
    ...
    def play_audio_array(self, audio, label="audio"):
        ...
        live_wav = OUT_DIR / f"{self.project['safe']}_v24_live.wav"
        ...
        if self.external_player:
            ...
            try:
                ...
                print(f"[v24] {label} lancé avec {self.external_player} : {live_wav}")
                return duration_ms
            except Exception as exc:
                print(f"[v24] Erreur backend système {self.external_player} : {exc}")
    ...
    def refresh_panel(self):
        ...
        role_target = item.get("role_target", role)
        ...
        self.info_label.config(
            text=(
                f"case {item['id']} | sample/pair {pair} | rôle ligne {role} | rôle voulu {role_target} | "
    ...
    def render_preview_file(self):
        OUT_DIR.mkdir(parents=True, exist_ok=True)
        wav = OUT_DIR / f"{self.project['safe']}_tracker_app_v24_preview.wav"
    ...
    def toggle_loop(self):
        if self.looping:
            ...
            print("[v24] Loop arrêtée")
            return

        self.stop_playhead()
        print("[v24] Lancement loop...")
        self.looping = True
    ...
    def clean_pattern(self):
        ...
        for item in sorted(self.pattern, key=lambda e: (int(e["x_step"]), int(e["id"]))):
            ...
            out.append({
                ...
                "role_target": item.get("role_target", self.block_by_pair[pair].get("role", "unknown")),
    ...
    def save(self):
        ...
        data = {
            "version": "tracker_app_edit_v24_exact_cells_no_sample_fade",
            ...
            "grid": {
                ...
                "bars_of_8": 4,
                "variation_generation": "deterministic role-based sample alternation; human edits become validation dataset",
                "fixed_timeline": True,
        ...
        path = OUT_DIR / f"{self.project['safe']}_tracker_app_edit_v24.json"
