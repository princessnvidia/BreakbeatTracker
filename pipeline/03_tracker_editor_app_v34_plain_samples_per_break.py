...
"""
BreakbeatAI Tracker Editor v34 — plain samples
...
# v34 : règles par break.
#
# Amen / Atlantis Amen :
#   position : 0 1 2 3 4 5 6 7
#   sample   : 0 3 2 4 5 1 7 6
#
# Camo :
#   pas de kick forcé, pas de pool caché.
#   sample 0..7 répété, Randomize dans tous les samples.
BASE_SEQUENCE_BY_MODE = {
    "amen_corrected": [0, 3, 2, 4, 5, 1, 7, 6],
    "plain_no_kick": None,
}

POSITION_POOLS_BY_MODE = {
    "amen_corrected": {
        0: [0, 1],
        1: [3, 4, 5, 6],
        2: [2, 7],
...
class PlainSampleTracker:
    def __init__(self, root, pair_json):
        ...
        self.root.title("BreakbeatAI Tracker Editor v34 — plain samples")
    ...
    def detect_external_player(self):
        for name in ["pw-play", "paplay", "aplay", "ffplay"]:
            ...
            if path:
                print(f"[v34] Backend audio : {name} -> {path}")
                return name

        print("[v34] Aucun backend système trouvé, fallback sounddevice.")
        return None
    ...
    def break_mode(self):
        ...
        for key, mode in BREAK_MODE_RULES.items():
            if key in safe:
                return mode

        # Par défaut : aucune grammaire cachée.
        return "plain_no_kick"

    def base_sequence_for_break(self):
        return BASE_SEQUENCE_BY_MODE.get(self.break_mode())

    def position_pools_for_break(self):
        return POSITION_POOLS_BY_MODE.get(self.break_mode())

    def pool_for_pos(self, pos):
        """
        v34 :
        - Amen utilise encore ses pools corrigés.
        - Camo n'a pas de kick : on ne force aucun pool, toutes les positions
          peuvent prendre tous les samples.
        """
        existing = [int(p) for p in self.pair_values]

        if not existing:
            return []

        pools = self.position_pools_for_break()
        ...
        pool = [
            int(pair)
            for pair in pools.get(local, existing)
            if int(pair) in set(existing)
        ]
    ...
    def base_pair_for_pos(self, pos):
        """
        v34 :
        - Amen : base corrigée, position 6 -> sample 7.
        - Camo : base simple sample 0..7 répété, sans kick forcé.
        """
        existing = [int(p) for p in self.pair_values]
        ...
        local = int(pos) % 8
        sequence = self.base_sequence_for_break()

        if sequence is None:
            return int(existing[local % len(existing)])

        wanted = int(sequence[local])

        if wanted in set(existing):
            return wanted

        pool = self.pool_for_pos(pos)
    ...
    def pick_pair_for_pos(self, pos, randomize=False, rng=None):
        ...
        # Base : spécifique au break.
        if not randomize and not MEMORY_PATH.exists():
            return self.base_pair_for_pos(pos)
    ...
    def build_initial_pattern(self, randomize=False):
        ...
        print(f"[v34] Génération loop32 | break={self.project['safe']} | mode={self.break_mode()} | randomize={randomize}")
        ...
        for pos in range(32):
            ...
            print(f"[v34] pos {pos:02d} -> sample {pair:02d}")
    ...
    def build_ui(self):
        ...
        title = tk.Label(
            main,
            text="BreakbeatAI v34 — plain samples, règles par break",
            bg="#111018",
            fg="#ff7acc",
            font=("Sans", 20, "bold"),
        )
        ...
        self.output_label = tk.Label(
            panel,
            text="v34 : sample-only. Amen garde son ordre corrigé ; Camo n’a aucun kick forcé.",
            bg="#1b1824",
            fg="#77f5b5",
            justify="left",
        )
    ...
    def bind_keys(self):
        ...
        print("[v34] Raccourcis : Espace loop | flèches déplacer | Delete supprimer")
    ...
    def record_correction(self, event_type, before=None, after=None):
        ...
        event = {
            "version": "breakbeatai_plain_correction_v34",
            "time": now_stamp(),
            "safe": safe,
        ...
        corrections_path = CORRECTIONS_DIR / f"{safe}_plain_corrections_v34.jsonl"
        ...
        print(f"[v34] correction sauvegardée : {event_type}")
    ...
    def play_audio_array(self, audio, label="audio"):
        ...
        live_wav = OUT_DIR / f"{self.project['safe']}_v34_live.wav"
        ...
        if self.external_player:
            try:
                ...
                print(f"[v34] Erreur backend {self.external_player} : {exc}")
    ...
    def render_preview_file(self):
        OUT_DIR.mkdir(parents=True, exist_ok=True)
        wav = OUT_DIR / f"{self.project['safe']}_tracker_app_v34_preview.wav"
    ...
    def write_latest_pattern(self, reason="correction"):
        ...
        path = LATEST_DIR / f"{self.project['safe']}_latest_plain_v34.json"

        data = {
            "version": "breakbeatai_latest_plain_v34",
            "reason": reason,
            ...
            "base_sequence_8": self.base_sequence_for_break(),
            "position_pools_8": self.position_pools_for_break(),
    ...
    def save(self):
        ...
        tracker_path = OUT_DIR / f"{self.project['safe']}_tracker_app_edit_v34.json"

        data = {
            "version": "tracker_app_edit_v34_plain_samples",
            "display_mode": "plain_samples_no_kick_hat_snare",
            "break_mode": self.break_mode(),
            "base_sequence_8": self.base_sequence_for_break(),
            "position_pools_8": self.position_pools_for_break(),
        ...
        stamp = file_stamp()
        validated_path = VALIDATED_DIR / f"{self.project['safe']}_validated_plain_loop32_v34_{stamp}.json"

        validated = {
            "version": "breakbeatai_validated_plain_loop32_v34",
