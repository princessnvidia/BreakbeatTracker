...
"""
BreakbeatAI Tracker Editor v38 — slice index
...
class SliceIndexTracker:
    def __init__(self, root, pair_json):
        ...
        self.root.title("BreakbeatAI Tracker Editor v38 — slice index")
    ...
    def detect_external_player(self):
        for name in ["pw-play", "paplay", "aplay", "ffplay"]:
            ...
            if path:
                print(f"[v38] Backend audio : {name} -> {path}")
                return name

        print("[v38] Aucun backend système trouvé, fallback sounddevice.")
        return None
    ...
    def guess_step_ms(self):
        try:
            value = float(self.project.get("step_ms_from_json") or 0)

            if 10 <= value <= 2000:
                return round(max(10.0, value / 2.0), 4)
        ...
        if durations:
            return round(max(10.0, float(np.median(durations)) / 2.0), 4)
    ...
    def build_initial_pattern(self, randomize=False):
        ...
        for slot in range(HIT_SLOTS):
            ...
            print(f"[v38] slot {slot:02d} | step {x_step:02d} -> slice {pair:02d}")
    ...
    def build_ui(self):
        ...
        title = tk.Label(
            main,
            text="BreakbeatAI v38 — clic case = play",
            bg="#111018",
            fg="#ff7acc",
            font=("Sans", 20, "bold"),
        )
        ...
        subtitle = tk.Label(
            main,
            text=(
                f"Backend audio : {backend} | source unique + JSON start/end | "
                f"méthode : {self.project.get('slice_method')}"
            ),
            bg="#111018",
            fg="#b9acc8",
        )
        ...
        self.output_label = tk.Label(
            panel,
            text="v38 : clic sur une case = son immédiat ; fermeture = stop audio complet.",
            bg="#1b1824",
            fg="#77f5b5",
            justify="left",
        )
    ...
    def record_correction(self, event_type, before=None, after=None):
        ...
        event = {
            "version": "breakbeatai_slice_index_correction_v38",
            "time": now_stamp(),
            "safe": safe,
        ...
        corrections_path = CORRECTIONS_DIR / f"{safe}_slice_index_corrections_v38.jsonl"
        ...
        print(f"[v38] correction sauvegardée : {event_type}")
    ...
    def stop_audition(self):
        """
        v38 : stoppe seulement le petit player d'écoute au clic,
    ...
    def play_audition_array(self, audio, label="audition"):
        ...
        audition_wav = OUT_DIR / f"{self.project['safe']}_v38_audition.wav"
        ...
        if self.external_player:
            try:
                ...
                print(f"[v38] Erreur audition {self.external_player} : {exc}")
    ...
    def audition_selected_case(self):
        """
        v38 : clic sur une case = on joue la slice de cette case.
        """
        item = self.selected()
    ...
    def on_close(self):
        """
        v38 : quand tu fermes la fenêtre, on coupe la loop, le playhead
        et le player externe avant de détruire Tk.
        """
        print("[v38] Fermeture : arrêt audio + destruction fenêtre")
    ...
    def play_audio_array(self, audio, label="audio"):
        ...
        audio = normalize(audio)
        live_wav = OUT_DIR / f"{self.project['safe']}_v38_live.wav"
        ...
        if self.external_player:
            try:
                ...
                print(f"[v38] Erreur backend {self.external_player} : {exc}")
    ...
    def render_preview_file(self):
        OUT_DIR.mkdir(parents=True, exist_ok=True)
        wav = OUT_DIR / f"{self.project['safe']}_tracker_app_v38_preview.wav"
    ...
    def write_latest_pattern(self, reason="correction"):
        ...
        path = LATEST_DIR / f"{self.project['safe']}_latest_slice_index_v38.json"

        data = {
            "version": "breakbeatai_latest_slice_index_v38",
            "reason": reason,
    ...
    def save(self):
        ...
        tracker_path = OUT_DIR / f"{self.project['safe']}_tracker_app_edit_v38.json"

        data = {
            "version": "tracker_app_edit_v38_slice_index",
        ...
        stamp = file_stamp()
        validated_path = VALIDATED_DIR / f"{self.project['safe']}_validated_slice_index_loop32_v38_{stamp}.json"

        validated = {
            "version": "breakbeatai_validated_slice_index_loop32_v38",
