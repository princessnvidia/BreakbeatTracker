...
"""
03_tracker_editor_app_v18.py

BreakbeatAI Tracker Editor v18

Principe :
- Une ligne = un sample / pair audio.
- Plus de classification obligatoire hat/kick/snare.
- Le numéro de la ligne correspond au sample joué.
- Le numéro dans la case correspond aussi à la pair audio.
- Déplacer une case verticalement change le sample joué.
- Déplacer une case horizontalement change sa position dans le pattern.
- Le rendu audio reste simple et stable : lecture des cases par ordre x_step.
- Backend audio robuste : pw-play, paplay, aplay, ffplay, puis sounddevice.
- Loop gapless longue + playhead verte.

Usage :
    cd ~/Applications/BreakbeatAI
    python pipeline/03_tracker_editor_app_v18.py --source "amen"
"""
...
def find_pair_json(source_query):
    ...
    if not matches:
        ...
        print(f'Essaie d’abord : python pipeline/01_find_pair_blocks_v03.py --source "{source_query}" --compat-v02')
        sys.exit(1)
...
class TrackerEditorApp:
    def __init__(self, root, pair_json):
        ...
        self.visible_canvas_height = min(620, max(220, self.row_height * max(1, len(self.pair_values))))
        ...
        self.root.title("BreakbeatAI Tracker Editor v18 — une ligne par sample")
    ...
    def detect_external_player(self):
        for name in ["pw-play", "paplay", "aplay", "ffplay"]:
            ...
            if path:
                print(f"[v18] Backend audio système trouvé : {name} -> {path}")
                return name

        print("[v18] Aucun backend système trouvé, fallback sounddevice.")
        return None

    def lane_color(self, lane_index):
        return PALETTE[int(lane_index) % len(PALETTE)]
    ...
    def pair_to_color(self, pair):
        ...
        return self.lane_color(lane)

    def build_initial_pattern(self):
        """
        Au départ, on affiche chaque sample une fois.
        Chaque sample est sur sa propre ligne.
        """
        pattern = []

        for idx, pair in enumerate(self.pair_values):
            pattern.append({
                "id": idx,
                "x_step": (idx * 2) % self.step_count,
                "lane": idx,
                "pair": int(pair),
                "length": 2,
            })
    ...
    def build_ui(self):
        ...
        title = tk.Label(
            main,
            text="BreakbeatAI Tracker Editor v18 — une ligne par sample",
            bg="#111018",
            fg="#ff7acc",
            font=("Sans", 20, "bold"),
        )
        ...
        subtitle = tk.Label(
            main,
            text=(
                f"Backend audio : {backend} | Chaque ligne correspond à un sample/pair audio. "
                "Déplacer une case verticalement change le sample joué."
            ),
            bg="#111018",
            fg="#b9acc8",
        ...
        self.info_label.grid(row=0, column=0, columnspan=12, sticky="w", padx=10, pady=8)
        ...
        tk.Button(panel, text="Play sample", command=self.play_selected_pair, bg="#30283f", fg="#f5eefe").grid(row=1, column=4, padx=4)
        tk.Button(panel, text="Loop / Space", command=self.toggle_loop, bg="#30283f", fg="#f5eefe").grid(row=1, column=5, padx=4)
        tk.Button(panel, text="Render preview", command=self.render_preview_only, bg="#30283f", fg="#f5eefe").grid(row=1, column=6, padx=4)
        tk.Button(panel, text="Save", command=self.save, bg="#30513f", fg="#f5eefe").grid(row=1, column=7, padx=4)
        tk.Button(panel, text="Delete", command=self.delete_selected, bg="#4a2630", fg="#f5eefe").grid(row=1, column=8, padx=4)
        tk.Button(panel, text="Reset", command=self.reset, bg="#30283f", fg="#f5eefe").grid(row=1, column=9, padx=4)

        self.output_label = tk.Label(
            panel,
            text="v18 : une ligne = un sample. Numéro dans la case = sample/pair joué.",
            bg="#1b1824",
            fg="#77f5b5",
            justify="left",
        )
        self.output_label.grid(row=2, column=0, columnspan=12, sticky="w", padx=10, pady=8)
    ...
    def bind_keys(self):
        ...
        print("[v18] Raccourcis : Espace loop | flèches = déplacer | Delete = supprimer")
    ...
    def play_audio_array(self, audio, label="audio"):
        ...
        live_wav = OUT_DIR / f"{self.project['safe']}_v18_live.wav"
        ...
        if self.external_player:
            ...
            try:
                ...
                print(f"[v18] {label} lancé avec {self.external_player} : {live_wav}")
                return duration_ms
            except Exception as exc:
                print(f"[v18] Erreur backend système {self.external_player} : {exc}")

        try:
            import sounddevice as sd
            sd.play(audio, SR)
            self.output_label.config(text=f"{label} lancé avec sounddevice — {duration_ms} ms")
            print(f"[v18] {label} lancé avec sounddevice")
            return duration_ms
        except Exception as exc:
    ...
    def refresh_panel(self):
        ...
        self.info_label.config(
            text=(
                f"case {item['id']} | sample/pair {pair} | ligne {lane_index + 1}/{len(self.pair_values)} | "
                f"step {item['x_step']} | length {item['length']} | durée sample {duration:.1f} ms"
            )
        )
    ...
    def render_preview_file(self):
        OUT_DIR.mkdir(parents=True, exist_ok=True)
        wav = OUT_DIR / f"{self.project['safe']}_tracker_app_v18_preview.wav"
    ...
    def toggle_loop(self):
        if self.looping:
            ...
            print("[v18] Loop arrêtée")
            return

        self.stop_playhead()
        print("[v18] Lancement loop...")
        self.looping = True
    ...
    def save(self):
        ...
        data = {
            "version": "tracker_app_edit_v18_one_line_per_sample",
            "audio_rule": "render sorted by x_step only; each lane maps directly to one pair/sample",
            "ui_rule": "one row per audio sample/pair; row number and box number are the pair number",
        ...
        path = OUT_DIR / f"{self.project['safe']}_tracker_app_edit_v18.json"
