...
"""
Tracker Lane Base v04

Correction principale :
- les hi-hats sont beaucoup plus audibles
- les découpes hats sont plus longues
- logique vraiment tracker : une cellule = une note/slice qui occupe toute la ligne
...
Usage :
    python tracker_lane_base_v04.py --source "Camo"

Si les hats sont encore trop courts :
    python tracker_lane_base_v04.py --source "Camo" --cell-ms 220 --hat-boost 0.55

Si c'est trop lent :
    python tracker_lane_base_v04.py --source "Camo" --cell-ms 160
"""
...
OUT_DIR = Path("exports/tracker_lane_base_v04")
...
def make_macro_cells(y, cell_samples):
    ...
    while start < len(y):
        ...
        # Important : en tracker, la slice occupe la cellule.
        # On fade très légèrement seulement pour éviter les clics.
...
def score_snare(c):
    ...
    return f["mid"] * 2.0 + f["rms"] * 4.0 + f["peak"] * 1.4 + f["high"] * 0.45 + centroid_ok


def score_hat(c):
    f = c["features"]
    # v04 : on favorise les hats longs/brillants, pas les micro-tics.
...
def mutate_base(length, mutation=0.20, hat_density=0.90):
    grid = list(make_base(length))

    # Remplit les trous avec des hats comme une vraie piste tracker.
    for i in range(length):
        if grid[i] == ".":
            if random.random() < hat_density:
                grid[i] = "H"

    # Ghosts près des snares, mais pas au détriment de tous les hats.
    ...
    # Quelques kicks de relance très rares.
...
def render(grid, kit, cell_samples, hat_boost):
    ...
    for ch in grid:
        ...
        # Tracker v04 : on ne raccourcit plus les hats.
        # Chaque note remplit une cellule entière.
...
def main():
    ...
    parser.add_argument("--hat-boost", type=float, default=0.48)
    ...
    base_wav = outdir / f"{safe}_lane_base_v04_base.wav"
    ...
    for i in range(1, args.count + 1):
        ...
        wav = outdir / f"{safe}_lane_base_v04_{i:03d}.wav"
    ...
    metadata = {
        "version": "tracker_lane_base_v04",
        "source": str(src),
        "source_only": True,
        "logic": "tracker cells, no overlap, longer hats",
        "base": "K..S|..KS|K..S|..KS",
    ...
    (outdir / "tracker_lane_base_report_v04.txt").write_text("\n".join(report), encoding="utf-8")
    (outdir / "tracker_lane_base_metadata_v04.json").write_text(json.dumps(metadata, indent=2), encoding="utf-8")
