...
"""
Jungle Grammar v03 Longform

But :
Faire des breaks plus longs, moins répétitifs, mais sans redevenir fouillis.

Différences avec v02 safe :
- génère des phrases longues : 4, 8, 16 boucles
- variations progressives par section
- mutations safe deux par deux
- fills seulement en fin de phrase
- pas de .txt par variation
- WAV + metadata global uniquement

Usage :
    python jungle_grammar_v03_longform.py --source "Amen" --count 32

Plus long :
    python jungle_grammar_v03_longform.py --source "Amen" --bars 16 --count 16

Plus varié mais encore contrôlé :
    python jungle_grammar_v03_longform.py --source "Amen" --bars 8 --mutation 0.18 --fill-chance 0.35 --count 32

Très sage :
    python jungle_grammar_v03_longform.py --source "Amen" --bars 8 --mutation 0.10 --fill-chance 0.18 --count 32
"""
...
OUT_DIR = Path("exports/jungle_grammar_v03_longform")
SR = 44100
...
def mutate_safe_pairs(base, mutation=0.12, allow_fill=False, fill_strength=0.25):
    ...
    for i in range(len(pairs)):
        ...
        elif action == "neighbor_shift":
            # remplace un bloc par un voisin proche, pas de saut énorme
            j = max(0, min(len(pairs) - 1, i + random.choice([-1, 1])))
            pairs[i] = pairs[j][:]

    # Fills seulement en fin de phrase
    if allow_fill and len(pairs) >= 2 and random.random() < fill_strength:
        fill_len = random.choice([1, 2])
        start = max(0, len(pairs) - fill_len)

        mode = random.choice(["repeat_prev", "swap_last", "reverse_last", "chop_local"])

        if mode == "repeat_prev" and start > 0:
            for i in range(start, len(pairs)):
                pairs[i] = pairs[start - 1][:]
            ...
            pairs[start:] = list(reversed(pairs[start:]))

        elif mode == "chop_local":
            pool = flatten(pairs[max(0, start - 2):start + 1])
            if pool:
                for i in range(start, len(pairs)):
                    pairs[i] = [random.choice(pool) for _ in pairs[i]]

    return flatten(pairs)[:len(base)]


def make_longform(base, bars=8, mutation=0.12, fill_chance=0.22, variation_growth=0.08):
    """
    Crée une phrase longue.
    Chaque bar reprend la base avec des variations progressives.
    Les fills arrivent surtout en fin de 4/8/16 bars.
    """
    long_seq = []
    bar_plans = []

    for bar in range(1, bars + 1):
        # sections : début sobre, milieu plus varié, fin avec fill
        section_boost = (bar - 1) / max(1, bars - 1)
        local_mut = mutation + section_boost * variation_growth

        end_of_phrase = (bar % 4 == 0) or (bar == bars)
        allow_fill = end_of_phrase
        local_fill = fill_chance if end_of_phrase else fill_chance * 0.15

        # tous les 1ers temps de section, rester proche de la base
        if bar in [1, 5, 9, 13]:
            local_mut *= 0.45
            allow_fill = False
        ...
        bar_plans.append({
            ...
            "fill": bool(allow_fill),
            "sequence": [int(x) for x in seq],
        })

    return long_seq, bar_plans
...
def main():
    ...
    parser.add_argument("--mutation", type=float, default=0.14)
    parser.add_argument("--fill-chance", type=float, default=0.25)
    parser.add_argument("--variation-growth", type=float, default=0.08)
    ...
    base_long = base * args.bars
    base_audio = render_sequence(slices, base_long)

    base_wav = outdir / f"{safe}_longform_base_{args.bars}bars.wav"
    sf.write(base_wav, base_audio, SR)

    print(f"Slices : {len(slices)}")
    print("Base longue :", base_wav)

    renders = []

    for i in range(1, args.count + 1):
        seq, bar_plans = make_longform(
            base,
            bars=args.bars,
            mutation=args.mutation,
            fill_chance=args.fill_chance,
            variation_growth=args.variation_growth,
        )

        audio = render_sequence(slices, seq)

        wav = outdir / f"{safe}_longform_{args.bars}bars_{i:03d}.wav"
    ...
    metadata = {
        "version": "jungle_grammar_v03_longform",
        "source": str(source),
        "source_only": True,
        "no_overlap": True,
    ...
    (outdir / "metadata_jungle_grammar_v03_longform.json").write_text(json.dumps(metadata, indent=2), encoding="utf-8")
